...
FText UAnimGraphNode_ArmCompressionResponse::GetNodeTitle(
    ENodeTitleType::Type TitleType) const {
  return GetControllerDescription();
}

FText UAnimGraphNode_ArmCompressionResponse::GetTooltipText() const {
  return LOCTEXT("ArmCompressionResponse_Tooltip",
                 "Procedural arm compression IK based on velocity or target.");
}
...
FText UAnimGraphNode_ArmCompressionResponse::GetVisualWarningTooltipText()
    const {
