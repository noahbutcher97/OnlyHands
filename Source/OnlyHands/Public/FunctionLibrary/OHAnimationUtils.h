...
public:
  ...
  static FTransform GetCurrentAnimPoseTransform(USkeletalMeshComponent *Mesh,
                                                FName BoneName);
  ...
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Animation")
  static float GetRotationDriftError(UOHPhysicsManager *Manager,
                                     FName BoneName);

  /** Draw debug line from anim pose to tracked pose */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Animation|Debug")
  static void DrawPoseDeviation(UOHPhysicsManager *Manager, FName BoneName,
  ...
  static TArray<FName>
  GetBonesWithHighDeviation(UOHPhysicsManager *Manager,
                            float PositionThreshold = 5.f,
