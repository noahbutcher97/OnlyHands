...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Movement|Foot")
	static bool IsFootApproachingGround(
	...
	static bool IsArmAlignedWithMovement(
		USkeletalMeshComponent* Mesh,
		FName ClavicleBone,
		FName HandBone,
	...
	UFUNCTION(BlueprintPure, Category = "Motion|StrikeLogic")
	static float GetArmToFacingAngle(
		USkeletalMeshComponent* Mesh,
		FName ClavicleBone,
		FName HandBone
	);
	...
	UFUNCTION(BlueprintPure, Category = "Motion|StrikeLogic")
	static float GetStrikePrepAlignmentScore(
		USkeletalMeshComponent* Mesh,
		FName ClavicleBone,
		FName HandBone,
		FVector ActorVelocity
	...
	UFUNCTION(BlueprintPure, Category = "Motion|StrikeLogic")
	static bool IsStrikeArcingTowardTarget(
		USkeletalMeshComponent* Mesh,
		FName ClavicleBone,
		FName HandBone,
		AActor* Target,
...
static bool GetPlayerCameraView(
	...
	static bool IsActorWithinRange(
		const AActor* Source,
		const AActor* Target,
		float MaxDistance,
		float Radius = 0.f);
	...
	static EOHHitDirection ClassifyHitDirection(
	...
	static void DebugDrawHitDirection(
		const FHitResult& Hit,
		const AActor* TargetActor,
		bool bUse8Directions = true,
		float Duration = 2.f,
...
#pragma region Animation
	...
	static FStrikeContactMetrics ComputeStrikeContactMetrics(
	...
	static FVector GetBoneVelocitySafe(
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float DeltaTime
	);
	...
	static void BeginDeferredStrikeSweep(
		UPARAM(ref)
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Notify")
	static void TickDeferredStrikeSweep(
		UPARAM(ref)
		FDeferredStrikeSweepState& State,
		float DeltaTime
	);
	...
	static bool GetActiveMontageBlendWeight(
		USkeletalMeshComponent* Mesh,
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Animation", meta = (WorldContext = "Mesh"))
	static FOHMontagePlaybackState GetMontagePlaybackState(
		USkeletalMeshComponent* Mesh);
	...
	static bool GetMontageSectionTimeRange(
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Animation")
	static bool GetMontageSectionNormalizedTime(
		UAnimMontage* Montage,
		FName SectionName,
		float PlaybackTime,
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Animation")
	static bool IsInMontageSectionWindow(
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Animation")
	static bool ExtractActiveSequenceAndTimeFromMesh(
		USkeletalMeshComponent* Mesh,
		UAnimSequence*& OutSequence,
		float& OutLocalTime);

	static bool ResolveAnimSequenceAtTimeRecursive(
...
#pragma region StrikeComputation
	...
	static bool IsStrikeContactValid(
		FVector Velocity,
	...
	static float ComputeContactScore(
		FVector Velocity,
		FVector ContactNormal,
		float EstimatedForce);
	...
	UFUNCTION(BlueprintPure, Category = "Combat|HitScoring")
	static FVector ConvertWorldToActorLocal(
		const AActor* ReferenceActor,
		const FVector& WorldLocation);


	UFUNCTION(BlueprintPure, Category = "Combat|HitValidation")
	static bool IsVelocityAlignedWithImpactNormal(
		const FVector& Velocity,
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|HitScoring")
	static void FilterRedundantHitsByActor(
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Debug")
	static void DrawDebugStrikeCone(
		const UObject* WorldContextObject,
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Motion")
	static FVector ComputeBoneLinearVelocityFromSamples(
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Motion")
	static FVector ComputeBoneVelocityMeshRelative(
		const USkeletalMeshComponent* Mesh,
		FName BoneName,
		float DeltaTime = 0.016f);
	...
	static FVector PredictBonePositionFromAnimation(
		const USkeletalMeshComponent* Mesh,
		FName BoneName,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Sampling")
	static bool GetBoneTransformWorldAtTime(
		UAnimSequence* Sequence,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		double Time,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Sampling")
	static bool GetBoneTransformFromAnimBase(
		UAnimSequenceBase* AnimBase,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		double Time,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Sampling")
	static bool GetBoneRotationAtTime(
		UAnimSequence* Sequence,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		double Time,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Sampling")
	static bool GetBoneVelocityAtTime(
		UAnimSequence* Sequence,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		double Time,
	...
	static FTransform GetBoneComponentSpaceTransformAtTime(
		UAnimSequence* Sequence,
		const USkeletalMeshComponent* Mesh,
		FName BoneName,
		double Time);
	...
	static FAnimExtractContext MakeExtractContext(
		double Time,
	...
	static FAnimExtractContext MakeExtractContext(
		double Time,
		const FDeltaTimeRecord& DeltaTime,
		bool bExtractRootMotion = false,
		bool bLooping = false)
	{
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Prediction")
	static void SweepCapsuleAlongStrikeArc(
		const UObject* WorldContextObject,
		const TArray<FVector>& ArcPoints,
		float Radius,
		float HalfHeight,
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Prediction")
	static bool DetectGJKContactFromStrikeArc(
		const TArray<FVector>& ArcPoints,
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Prediction")
	static void GenerateStrikeArcFromLiveMesh(
		const USkeletalMeshComponent* Mesh,
		FName BoneName,
		int32 NumSamples,
	...
	static bool GenerateStrikeArcFromAnimBase(
		UAnimSequenceBase* AnimBase,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float StartTime,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|ArcSampling")
	static bool GenerateStrikeArcFromAnimSequence(
		UAnimSequence* Sequence,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float StartTime,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|ArcSampling")
	static bool GenerateStrikeArcFromComposite(
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|ArcSampling")
	static bool GenerateStrikeArcFromMontage(
		UAnimMontage* Montage,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float StartTime,
...
#pragma region Sampling
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Prediction")
	static void ComputeMotionDerivatives(
		const TArray<FVector>& Points,
		float SampleDelta,
 ...
 */
	...
	/** Extracts a usable UAnimSequence from any UAnimSequenceBase, Composite, or Montage. Returns nullptr if none found. */
	UFUNCTION(BlueprintCallable, Category = "Combat|Prediction")
	...
	static void SampleMotionDerivatives(
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Debug")
	static void QuickDrawBoneArcFromMesh(
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float PredictDuration = 0.5f,
		FColor Color = FColor::Green,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Combat|Debug")
	static void ScheduleDeferredArcDraw(
		USkeletalMeshComponent* Mesh,
...
private:
	...
	static FOHBoneMotionSample SampleBoneMotionInternal(
		UAnimSequenceBase* AnimBase,
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		int32 NumSamples,
	...
	static void GetBoneChainAngularMotion(
		UOHPhysicsManager* PhysicsManager,
		const TArray<FName>& BoneChain,
	...
	static FVector GetBlendedChainVelocity(
		UOHPhysicsManager* PhysicsManager,
		const USkeletalMeshComponent* Mesh,
		FName StartBone,
		FName EndBone,
	...
	UFUNCTION(BlueprintCallable, Category = "OH|Physics")
	static FVector GetBlendedChainAcceleration(
		UOHPhysicsManager* PhysicsManager,
		const USkeletalMeshComponent* Mesh,
		FName StartBone,
		FName EndBone,
...
#pragma region MotionWarping
	...
	static void AddWarpTargetToNearestEnemy(
...
#pragma region ForceFeedback
	...
	static float ComputeImpulseVulnerabilityScore(
	...
	static FVector ComputeImpulseVectorForBone(
	const FOHPhysicsGraphNode& Graph,
	FName BoneName,
	const FHitResult& Hit,
	const FVector& RootLocation,
	float BaseStrength,
	EOHImpulseMode ModeOverride,
	bool bAutoInferMode,
	...
	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static FVector GetImpulseDirectionFromHit(const FOHBoneData& BoneData, const FHitResult& Hit, EImpulseDirectionMode Mode);

	// Strength-only wrappers
	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static float ComputeImpulseStrengthFromNormal(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier = 1.0f);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static float ComputeImpulseStrengthFromImpactVector(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier = 1.0f);

	// Final impulse vector (strength × direction)
	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static FVector ComputeImpulseVectorFromHit(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier = 1.0f, EImpulseDirectionMode Mode = EImpulseDirectionMode::FromHitNormal);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static FVector ComputeImpulseVectorFromHitNormal(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier = 1.0f);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static FVector ComputeImpulseVectorFromImpactVector(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier = 1.0f);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static EImpulseDirectionMode InferBestImpulseDirectionMode(
		const FOHBoneData& BoneData,
		const FHitResult& Hit,
		float AlignmentThreshold = 0.5f);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Impulse")
	static FVector ComputeSelfReactionImpulse(
		const FOHBoneData& BoneData,
		const FHitResult& Hit,
		const FVector& RootLocation,
	...
	static FVector ComputeImpulseVectorFromBone(const FOHBoneData& BoneData, const FHitResult& Hit,
	                                            const FVector& RootLocation,
	                                            float BaseStrength, EImpulseDirectionMode ModeOverride, bool bAutoInferMode);
#pragma endregion
...
public:
	...
	static bool PredictiveCapsuleGroundTrace(
		USkeletalMeshComponent* Mesh,
		FName BoneName,
		float MaxDistance,
