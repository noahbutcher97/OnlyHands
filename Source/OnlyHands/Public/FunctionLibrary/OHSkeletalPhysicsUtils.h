...
// ----------------------------
/** Shorthand for creating a bone reference map from any supported context
 * (AnimInstance, BoneContainer, etc.) */
#define OH_CREATE_BONE_MAP(Context)                                            \
  UOHSkeletalPhysicsUtils::CreateBoneReferenceMap(Context)
/** Shorthand for creating a bone reference map from any supported context
 * (AnimInstance, BoneContainer, etc.) */
#define OH_CREATE_BONE_MAP_SAFE(Context)                                       \
  UOHSkeletalPhysicsUtils::CreateBoneReferenceMapSafe(Context)
/** Safely retrieve a compact pose index from a bone reference map */
#define OH_GET_POSE_INDEX(Map, Bone) (Map.GetPoseIndex(Bone))
/** Safely retrieve a compact pose index from a bone reference map */
#define OH_GET_POSE_INDEX_SAFE(Map, Bone)                                      \
  (Map.HasValidReference(Bone) ? Map.GetPoseIndex(Bone) : INDEX_NONE)
/** Safely retrieve a bone reference struct */
#define OH_GET_BONE_REF(Map, Bone) (Map.GetReference(Bone))
/** Safely retrieve a bone reference struct */
#define OH_GET_BONE_REF_SAFE(Map, Bone)                                        \
...
/** Enum string helper */
#define OH_ENUM_STRING_SAFE(EnumVal)                                           \
  (EnumVal ? (*UEnum::GetValueAsString(EnumVal)) : TEXT("Unknown"))

#pragma endregion

/**
 * Specialized function library for skeletal physics and advanced bone analysis
 * in OnlyHands
 *
 ...
 *
 * For simpler utility functions or single-bone classification, use
 * OHHelperUtils or the FORCE INLINE functions in EOHPhysicsEnums.h instead.
 */
...
#pragma region BoneResolution
  ...
  static FName
  ResolveBoneNameFromSkeletalBone(EOHSkeletalBone Bone,
                                  const USkeletalMeshComponent *SkelComp);

  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static FOHResolvedBoneData
  ResolveResolvedDataFromSkeletalBone(EOHSkeletalBone Bone,
                                      const USkeletalMeshComponent *SkelComp);
  ...
  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static FOHResolvedBoneData
  ResolveResolvedDataFromSkeletalBone_Static(EOHSkeletalBone Bone);
...
#pragma region Enums
  ...
  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static EOHFunctionalBoneGroup
  GetFunctionalGroupFromBone(EOHSkeletalBone Bone);

  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static TArray<FName> GetPrimaryBoneNamesFromBodyPart(EOHBodyPart BodyPart);

  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static EOHSkeletalBone
  ResolveSkeletalBoneFromNameSmart(const FName &Input,
                                   const USkeletalMeshComponent *SkelComp,
                                   float ScoreThreshold = 0.65f);

  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static EOHSkeletalBone
  GetRootBoneInBodyPartFromMesh(const USkeletalMeshComponent *SkelComp,
                                EOHBodyPart BodyPart);

  UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
  static EOHSkeletalBone
  GetEndBoneInBodyPartFromMesh(const USkeletalMeshComponent *SkelComp,
                               EOHBodyPart BodyPart);
...
#pragma region Bone Queries
  ...
  static TArray<FName>
  GetBoneChainBetweenByName(const USkeletalMeshComponent *Mesh,
                            const FName &ParentBoneName,
                            const FName &ChildBoneName);

  /**
   * Returns the full bone chain (as bone names) from the specified
   * skeletal-enum up to the root of the bone hierarchy. E.g. passing Hand_R
   * will give { Hand_R, LowerArm_R, UpperArm_R, Clavicle_R, …, Pelvis }.
   */
  ...
  static void BuildValidatedBoneChain(EOHSkeletalBone EffectorEnum,
...
#pragma endregion
  ...
  static TArray<FName>
  GetBoneChainReverseByName(const USkeletalMeshComponent *Mesh,
                            const FName &StartingBone);

  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Debug")
  static void DrawDebugBoneChainByName(const USkeletalMeshComponent *Mesh,
                                       const FName &ParentBoneName,
                                       const FName &ChildBoneName,
                                       FColor Color = FColor::Cyan,
                                       float Duration = 2.0f);

  /** Returns all bones that are hierarchically below the given root bone
   * (inclusive). */
   ...
   * @return Ordered an array of bones that should be affected, starting with
   * the origin bone
   */
  ...
  GetBonesInFunctionalBoneGroup(EOHFunctionalBoneGroup Group);
  /**
   ...
   * Depth of 1 returns only direct children, depth of 2 includes grandchildren,
   * etc.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Physics|Skeletal|Structure",
            meta = (DisplayName = "Get Child Bones By Depth",
                    ToolTip = "Returns all children up to a specified depth "
                              "below a given bone."))
  static TArray<EOHSkeletalBone> GetChildBonesByDepth(EOHSkeletalBone RootBone,
                                                      int32 MaxDepth);

  /**
   * Returns the depth between a parent and target bone in the skeletal
   * hierarchy. If the parent is not an ancestor of the target, returns -1.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Physics|Skeletal|Structure")
  static int32 GetBoneDepthRelativeTo(EOHSkeletalBone ParentBone,
                                      EOHSkeletalBone TargetBone);

  /**
   * Returns the full bone lineage from the target bone up to the specified
   * ancestor or root. Includes the target bone itself. Stops at ParentBone or
   * when reaching the root.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Physics|Skeletal|Structure")
  static TArray<EOHSkeletalBone> GetBoneLineage(EOHSkeletalBone TargetBone,
                                                EOHSkeletalBone StopAtBone);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Physics|Skeletal|Structure",
            meta = (DisplayName = "Get BoneEnum Lineage (Full)"))
  static TArray<EOHSkeletalBone>
  GetBoneLineageToRoot(EOHSkeletalBone TargetBone);
  /**
  ...
  /**
   * Solves a two-bone IK chain to position the end effector at the target
   * location.
   *
   * @param OutUpper          Upper bone transform (shoulder/hip) - modified
   * in-place
   * @param OutLower          Lower bone transform (elbow/knee) - modified
   * in-place
   * @param OutEnd            End effector transform (wrist/ankle) - modified
   * in-place
   ...
   * @param bAllowStretching  Whether to allow chain stretching to reach targets
   * beyond max distance
   * @param StartStretchRatio Ratio of max reach at which stretching begins
   * (0-1)
  ...
  static void
  SolveIK_TwoBone(UPARAM(ref) FTransform &OutUpper,
                  UPARAM(ref) FTransform &OutLower,
   ...
   * @param WristLookAtAxis    Local axis of the wrist that should align with
   * target
  ...
  static void SolveIK_ArmChain(
      UPARAM(ref) FTransform &Clavicle, UPARAM(ref) FTransform &UpperArm,
      UPARAM(ref) FTransform &LowerArm, UPARAM(ref) FTransform &Hand,
      const FVector &IKTarget, const FVector &ElbowHint,
      bool bUseClavicle = false, bool bAllowStretching = false,
      float MaxStretchScale = 1.25f, bool bEnableWristRotation = true,
      const FVector &WristLookAtAxis = FVector(1.0f, 0.0f, 0.0f),
      UWorld *World = nullptr, bool bDrawDebug = false);

  // ------------------------------- Solver Utility Functions
  // -------------------------------- //
  /**
   * Utility function to compute fallback elbow bend axis when hint direction is
   * unreliable.
   *
  ...
  static FVector ComputeFallbackBendAxis(const FTransform &Root,
                                         const FTransform &Mid);

  /**
   * Validates bone transforms for a standard IK chain to ensure they meet
   * engine requirements. Particularly useful before submitting transforms to
   ...
   * @return                  Number of invalid transforms removed (0 if all
   * valid)
   */
  int32 static ValidateAndSortBoneTransforms(
      TArray<FBoneTransform> &OutBoneTransforms, bool bSortByIndex = true,
   ...
   * @param MirrorAxis  A unit vector whose X/Y/Z components indicate which axes
   * to flip.
   * @return            The mirrored point.
   */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|IK")
  static FVector MirrorPointAcrossAxes(const FVector &Point,
                                       const FVector &Pivot,
                                       const FVector &MirrorAxis);
  /**
   * Compute a velocity-based target: HandPosition minus the velocity direction
   * scaled by Threshold.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static FVector ComputeVelocityBasedTarget(FVector HandPosition,
                                            FVector Velocity,
  ...
  /**
   * Apply a predictive offset along Velocity: BaseTarget + Velocity *
   * PredictiveOffset.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static FVector ApplyPredictiveOffset(FVector BaseTarget, FVector Velocity,
                                       float PredictiveOffset);

  /**
   * Mirror a vector across the specified axes. Any non‐zero component in
   * MirrorAxis flips that component.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static FVector MirrorVectorAcrossAxis(FVector Vector, FVector MirrorAxis);

  /**
   * If Curve is valid, returns Curve->GetFloatValue(InputAlpha); otherwise
   * returns DefaultValue.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static float EvaluateCurveAtAlpha(const UCurveFloat *Curve, float InputAlpha,
  ...
  /**
   * Computes a simple falloff alpha: 1 − (Distance / FalloffRadius), clamped to
   * [0,1]. If FalloffRadius ≤ 0, returns 1.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static float ComputeFalloffAlpha(float Distance, float FalloffRadius);

  /**
   * Applies a “soft zone” offset: remaps Alpha such that below SoftZoneOffset →
   * 0, above 1 → 1, linearly in between. If SoftZoneOffset ≤ 0, returns Alpha
   * unchanged.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|IKHelpers")
  static float ComputeSoftZoneAlpha(float Alpha, float SoftZoneOffset);
  /**
   * Compute the total rest‐pose length of a bone chain by summing distances
   * between successive bones.
   * @param SkelComp  The skeletal mesh component
   * @param BoneNames Ordered list of bone names (e.g.
   * {"upperarm_r","lowerarm_r","hand_r"})
   * @return          Sum of segment lengths in component space
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static float ComputeBoneChainLength(USkeletalMeshComponent *SkelComp,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static void GetBoneChainWorldPositions(USkeletalMeshComponent *SkelComp,
                                         TArray<FName> BoneNames,
                                         TArray<FVector> &OutPositions);

  /**
   * Find the closest point on a polyline (chain of points) to a given world
   * point.
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector GetClosestPointOnBoneChain(TArray<FVector> ChainPoints,
                                            FVector Point);

  /**
   * Perform a capsule sweep along each bone‐segment in the chain to collect
   * collision hits.
  ...
  static bool SweepCapsuleAlongBoneChain(UWorld *World,
                                         USkeletalMeshComponent *SkelComp,
                                         TArray<FName> BoneNames, float Radius,
                                         TArray<FHitResult> &OutHits);

  /**
   * Approximate the center of mass of a bone chain by averaging each bone’s
   * body‐instance COM.
   * @param SkelComp   The skeletal mesh component
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputeBoneChainCenterOfMass(USkeletalMeshComponent *SkelComp,
                                              TArray<FName> BoneNames);

  /**
   * Predict future positions for a chain given current positions and velocities
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static void PredictBoneChainPositions(TArray<FVector> CurrentPositions,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static void DecomposeSwingTwist(FQuat Rotation, FVector TwistAxis,
                                  FQuat &OutSwing, FQuat &OutTwist);

  /**
   * Compute linear velocity of a bone by sampling its world positions over a
   * timestep.
   * @param SkelComp   The skeletal mesh component
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputeBoneVelocity(USkeletalMeshComponent *SkelComp,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputeAngularVelocity(FQuat PrevRotation, FQuat CurrRotation,
                                        float DeltaTime);
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static float PredictBoneTimeToCollision(USkeletalMeshComponent *SkelComp,
                                          FName BoneName, FVector Velocity,
                                          float MaxTime,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputeReactiveImpulseFromHit(const FHitResult &Hit,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static bool IsBoneChainIntersecting(USkeletalMeshComponent *SkelComp,
                                      TArray<FName> BoneNames,
                                      float SphereRadius,
                                      ECollisionChannel Channel);
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ProjectPointOntoBoneCapsule(FVector Start, FVector End,
                                             float Radius, FVector Point);
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static float ComputeCollisionBlendWeight(FVector BonePos,
                                           FVector CollisionPos,
                                           float DistanceThreshold);

  /**
   * Given a bone chain’s world positions and a moving sphere
   * (center+radius+velocity), predicts which segment it will first collide with
   * and at what time.
   * @return tuple (HitTime, SegmentIndex, HitPoint). If no hit,
   * HitTime=MaxTime, SegmentIndex=-1.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static void PredictChainCollisionSegment(
      const TArray<FVector> &ChainPositions, FVector SphereCenter, float Radius,
      FVector Velocity, float MaxTime, float &OutHitTime,
      int32 &OutSegmentIndex, FVector &OutHitPoint);
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static float ComputeCapsulePenetrationDepth(FVector Point,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputePenetrationCorrection(FVector Point,
                                              FVector CapsuleStart,
                                              FVector CapsuleEnd, float Radius);

  /**
   * Given a chain of predicted collision hits, compute a composite collision
   * normal by summing and normalizing all hit normals.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComposeCollisionNormal(TArray<FHitResult> Hits);

  /**
   * Maps a collision normal and incoming bone velocity to an outgoing bounce
   * velocity, clamped by RestitutionCoefficient.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static FVector ComputeBounceVelocity(FVector VelocityIn, FVector Normal,
   ...
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static void ComputeChainStiffnessMap(UPhysicsAsset *PhysAsset,
                                       TArray<FName> BoneNames,
                                       float BaseStiffness,
                                       TMap<FName, float> &OutStiffnessMap);

  /**
   * Predicts the time‐of‐impact along a bone chain when hit by a moving
   * capsule, returning the earliest impact time or MaxTime if none.
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  static float PredictCapsuleChainTimeToImpact(
  ...
  /**
   * For a reactive effect, computes an exponential decay of collision response
   * over Time, using DecayRate (e.g. spring/damper fall‐off).
   */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
  ...
  static float GetArmChainLength(USkeletalMeshComponent *Mesh, FName UpperArm,
                                 FName LowerArm, FName Hand);

  /** Computes current effector direction (unit vector) */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Skeletal IK")
  static FVector GetArmEffectorDirection(USkeletalMeshComponent *Mesh,
  ...
  static void DebugDrawArmIK(USkeletalMeshComponent *Mesh, FName UpperArm,
  ...
  static FVector PredictEffectorTarget(USkeletalMeshComponent *Mesh,
                                       FName StartBone, FVector Velocity,
                                       float Distance = 60.f);
  ...
  static float PredictCompressionTrend(USkeletalMeshComponent *Mesh,
  ...
  static bool IsBoneOverlapping(USkeletalMeshComponent *Mesh, FName BoneName,
                                float Radius = 5.f);

  // Forward prediction based on velocity
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Skeletal IK|Collision")
  static bool PredictBoneCollision(USkeletalMeshComponent *Mesh, FName BoneName,
                                   FVector Velocity, float Distance,
                                   FHitResult &OutHit);

  // Trace from hand to world (punch prediction)
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Skeletal IK|Collision")
  static bool TracePunchCollision(USkeletalMeshComponent *Mesh, FName HandBone,
  ...
  static void SyncIKFromArmPose(UObject *AnimInstance, FName UpperArm,
                                FName LowerArm, FName Hand, float MinLength,
                                float MaxLength);

  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Skeletal IK|Tracking")
  static void TrackImpactCamera(UObject *WorldContext, FVector ImpactLocation,
                                float Duration = 1.0f);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Transform")
  static FTransform OffsetTransform(const FTransform &Base,
                                    const FVector &TranslationOffset,
                                    const FRotator &RotationOffset);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Transform")
  static FTransform InterpTransform(const FTransform &A, const FTransform &B,
                                    float Alpha);

  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Skeletal IK|Stretch")
  static FVector ComputeOverstretchedEffector(FVector Origin, FVector Target,
                                              float StretchRatio = 1.2f);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Skeletal IK|General")
  static float
  GetChainLengthFromBoneEnums(USkeletalMeshComponent *Mesh,
                              const TArray<EOHSkeletalBone> &BoneChain);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Skeletal IK|Leg")
  static float GetLegCompressionAlpha(USkeletalMeshComponent *Mesh,
                                      EOHSkeletalBone Thigh,
                                      EOHSkeletalBone Calf,
                                      EOHSkeletalBone Foot, float MinLength,
                                      float MaxLength);

  UFUNCTION(BlueprintPure, Category = "OnlyHands|Skeletal IK|Spine")
  static float GetSpineBendRatio(USkeletalMeshComponent *Mesh,
...
#pragma region PhysicsSimulation
  ...
  // Robustly sets physics blend weight for a single bone (0=animation,
  // 1=physics)
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool SetPhysicsBlendWeightForBone(USkeletalMeshComponent *Mesh,
                                           FName BoneName, float BlendWeight);

  // Robustly sets blend weight for a set of bones (arbitrary, not validated as
  // chain)
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static void SetPhysicsBlendWeightForBones(USkeletalMeshComponent *Mesh,
                                            const TArray<FName> &BoneNames,
                                            float BlendWeight);

  // Robustly sets blend weight for a contiguous chain of bones (inclusive)
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static void SetPhysicsBlendWeightForBoneChain(USkeletalMeshComponent *Mesh,
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool DisablePhysicalAnimationForBone(USkeletalMeshComponent *Mesh,
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool DisablePhysicalAnimationForBones(USkeletalMeshComponent *Mesh,
                                               UPhysicalAnimationComponent *PAC,
                                               const TArray<FName> &BoneNames);
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool
  DisablePhysicalAnimationForBoneChain(USkeletalMeshComponent *Mesh,
                                       UPhysicalAnimationComponent *PAC,
                                       FName StartBone, FName EndBone);

  // Minimal physics sim enable/disable for a single bone (no PAC/profile, just
  // sim state)
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool EnablePhysicsSimulationForBone(USkeletalMeshComponent *Mesh,
                                             FName BoneName);

  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Test")
  static bool DisablePhysicsSimulationForBone(USkeletalMeshComponent *Mesh,
                                              FName BoneName);
  ...
  static void BlendOutPhysicalAnimationForBone(
      ...
      UCurveFloat *BlendCurve = nullptr, UObject *WorldContextObject = nullptr);
...
#pragma endregion
  ...
  static bool
  TryGetPoseIndex(EOHSkeletalBone Bone,
                  const TMap<EOHSkeletalBone, FCompactPoseBoneIndex> &Indices,
                  FCompactPoseBoneIndex &OutIndex);

#pragma region BoneHierarchy

  static TArray<FConstraintInstance *>
  GetAllParentConstraints(USkeletalMeshComponent *SkelMesh, FName BoneName);

  static TArray<FName> GetAllParentBoneNames(USkeletalMeshComponent *SkelMesh,
                                             FName BoneName);

  float ComputeBoneLength(const USkeletalMeshComponent *SkeletalMesh,
                          FName BoneA, FName BoneB);
...
#pragma region Internals_DerivedCalculations_
   ...
   * returning an array of capsule primitives (Start, End, Radius) for collision
   * tests.
   */
  static void GenerateBoneChainMotionCapsules(
   ...
   */
  static void ComputeBoneMassMap(USkeletalMeshComponent *SkelComp,
                                 TArray<FName> BoneNames,
                                 TMap<FName, float> &OutBoneMass);

  /**
   * Blends a reactive pose sample into the current pose delta‐array according
   * to BlendWeight.
   */
  static void
  BlendReactivePose(const TArray<FBoneTransform> &ReactiveTransforms,
                    TArray<FBoneTransform> &OutBoneTransforms,
                    float BlendWeight);

  /**
   * Samples the PhysicsAsset to find the world‐space capsule or sphere
   * representing the bone’s body, returning its shape parameters.
   */
  static bool GetBoneCollisionShape(UPhysicsAsset *PhysAsset, FName BoneName,
