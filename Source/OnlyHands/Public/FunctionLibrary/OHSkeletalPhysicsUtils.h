...
#define OH_GET_BONE_REF_SAFE(Map, Bone) (Map.HasValidReference(Bone) ? Map.GetReference(Bone) : FBoneReference())
/** Safely retrieve a bone reference struct */
#define OH_GET_BONE_REF_SAFE_NO_RETURN(Map, Bone) (Map.HasValidReference(Bone) ? Map.GetReference(Bone) : FBoneReference())
// --------------------
// Debugging & Logging
// --------------------
...
if (Condition) { UE_LOG(LogTemp, Warning, TEXT(Format), ##__VA_ARGS__); }
// --------------------
// Enum safety
// --------------------
/** Enum string helper */
...
#pragma region BoneResolution
	...
	static bool ValidateAndSuggestMannequinSkeletonSetup(const USkeletalMeshComponent* Mesh, const TArray<FName>& TrackedBones, const TArray<FName>& ExcludedBones, bool bAutoLog);


	// Replace: static FName ResolveBoneNameFromSkeletalBone(EOHSkeletalBone Bone, ...);
	UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
	static FName ResolveBoneNameSmart(const FName& Input, const USkeletalMeshComponent* SkelComp, float ScoreThreshold = 0.65f);
	...
	static FName GetBestMatchingBone(
		const FString& Query,
		const USkeletalMeshComponent* SkelComp,
		float& OutScore,
		FString& OutAlgorithm,
	...
	static void FindAllBonesMatching(
		const FString& Query,
		const USkeletalMeshComponent* SkelComp,
	...
	static TArray<FName> FindAllBonesMatching(
		const FString& Query,
		const USkeletalMeshComponent* SkelComp,
		float ScoreThreshold /*= 0.6f*/);

	static void ResolveImpactBoneRedirect(
		const USkeletalMeshComponent* SkelComp,
		const FName& HitBone,
	...
	UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
	static TArray<FName> GetBoneChainToRoot(const USkeletalMeshComponent* SkelComp, FName StartBone, FName StopAtBone = NAME_None);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Skeletal")
	static EOHSkeletalBone ResolveSkeletalBoneFromNameSmart(const FName& Input, const USkeletalMeshComponent* SkelComp, float ScoreThreshold = 0.65f);
	...
	/** 
* Returns the full bone chain (as bone names) from the specified skeletal-enum up to the root of the bone hierarchy.
* E.g. passing Hand_R will give { Hand_R, LowerArm_R, UpperArm_R, Clavicle_R, …, Pelvis }.
*/
 ...
 */
	...
	static TArray<FName> GetBoneChainBetweenByName(const USkeletalMeshComponent* Mesh, const FName& ParentBoneName, const FName& ChildBoneName);
...
*/
	...
	static void BuildValidatedBoneChain(
...
#pragma endregion
  ...
  */
	...
	static void SolveIK_TwoBone(
		...
		float MaxStretchScale = 1.25f,
		UWorld* WorldDebugContext = nullptr,
 ...
 */
	UFUNCTION(BlueprintCallable, Category="OnlyHands|IK")
	static FVector MirrorPointAcrossAxes(
		const FVector& Point,
		const FVector& Pivot,
	...
	static FVector ComputeVelocityBasedTarget(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|IKHelpers")
	static FVector ApplyPredictiveOffset(
		FVector BaseTarget,
		FVector Velocity,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|IKHelpers")
	static FVector MirrorVectorAcrossAxis(
		FVector Vector,
		FVector MirrorAxis
	);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|IKHelpers")
	static float EvaluateCurveAtAlpha(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|IKHelpers")
	static float ComputeFalloffAlpha(
		float Distance,
		float FalloffRadius
	);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|IKHelpers")
	static float ComputeSoftZoneAlpha(
		float Alpha,
	...
	static float ComputeBoneChainLength(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static void GetBoneChainWorldPositions(
		USkeletalMeshComponent* SkelComp,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector GetClosestPointOnBoneChain(
	...
	static bool SweepCapsuleAlongBoneChain(
		UWorld* World,
		USkeletalMeshComponent* SkelComp,
		TArray<FName> BoneNames,
		float Radius,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComputeBoneChainCenterOfMass(
		USkeletalMeshComponent* SkelComp,
		TArray<FName> BoneNames
	);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static void PredictBoneChainPositions(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComputeBoneVelocity(
		USkeletalMeshComponent* SkelComp,
		FName BoneName,
		float DeltaTime
	);
	...
	static FVector ComputeAngularVelocity(
		FQuat PrevRotation,
		FQuat CurrRotation,
		float DeltaTime
	);
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static float PredictBoneTimeToCollision(
		USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FVector Velocity,
		float MaxTime,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComputeReactiveImpulseFromHit(
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "OnlyHands|SkeletalPhysics")
    static void ApplyImpulseToBone(USkeletalMeshComponent* SkeletalMesh, FName BoneName, const FVector& Impulse, bool bVelChange);

    /** 
     * Apply an impulse to a chain of bones starting from a root bone, with falloff applied along the chain.
     */
    UFUNCTION(BlueprintCallable, Category = "OnlyHands|SkeletalPhysics")
    static void ApplyImpulseToBoneChain(USkeletalMeshComponent* SkeletalMesh, FName RootBone, const FVector& Impulse, float Falloff, bool bVelChange, int32 MaxDepth = -1);

	/** C++-only overload with component-specific filtering */
	static void ApplyImpulseToBoneChain(
	...
	static TArray<FName> GetBoneChain(
		USkeletalMeshComponent* SkeletalMesh, 
		FName RootBone, 
		int32 MaxDepth = -1,
		bool bUseDFS = false
...
private:
	...
	static TArray<FName> GetBoneChain_Internal(
		USkeletalMeshComponent* SkeletalMesh, 
		FName RootBone, 
		int32 MaxDepth,
		const TFunctionRef<bool(FName)>& Filter,
...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static bool IsBoneChainIntersecting(
		USkeletalMeshComponent* SkelComp,
		TArray<FName> BoneNames,
		float SphereRadius,
		ECollisionChannel Channel
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ProjectPointOntoBoneCapsule(
		FVector Start,
		FVector End,
		float Radius,
		FVector Point
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static float ComputeCollisionBlendWeight(
		FVector BonePos,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static float ComputeCapsulePenetrationDepth(
		FVector Point,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComputePenetrationCorrection(
		FVector Point,
		FVector CapsuleStart,
		FVector CapsuleEnd,
		float Radius
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComposeCollisionNormal(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static FVector ComputeBounceVelocity(
		FVector VelocityIn,
		FVector Normal,
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static void ComputeChainStiffnessMap(
	 ...
	 */
	UFUNCTION(BlueprintPure, Category="OnlyHands|SkeletalPhysics")
	static float ComputeCollisionDecay(
		float Time,
		float DecayRate
	);
	...
	static float GetChainLengthFromBoneEnums(
	...
	static float GetLegCompressionAlpha(
		USkeletalMeshComponent* Mesh,
	...
	static float GetSpineBendRatio(
		USkeletalMeshComponent* Mesh,
	 ...
	 */
	static void ComputeBoneMassMap(
		USkeletalMeshComponent* SkelComp,
		TArray<FName> BoneNames,
	...
	static void DrawDebugBoneChainByName(const USkeletalMeshComponent* Mesh, const FName& ParentBoneName, const FName& ChildBoneName, FColor Color = FColor::Cyan, float Duration = 2.0f);
...
#pragma region Motion Helpers
	...
	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FVector GetBoneVelocityInReferenceFrame(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
 ...
 */
	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FVector GetBoneVelocitySingleFrame(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FName ReferenceBone = NAME_None,
		EOHReferenceSpace RefFrame = EOHReferenceSpace::WorldSpace
	...
	static FVector CalculateBoneWorldVelocity(
		const FVector& CurrentBoneWorldPosition,
	...
	static FVector CalculateBoneVelocityInSpace(
		USkeletalMeshComponent* Mesh,
		FName BoneName,
	...
	static FVector CalculateBoneWorldAcceleration(
		const FVector& CurrentWorldPosition,
		const FVector& PreviousWorldPosition,
	...
	static float CalculateBoneImpactForce(
...
// Calculate force vector for bone impact
	...
	static FVector CalculateBoneForceVector(
    const FVector& StrikingBoneWorldVelocity,
    const FVector& ContactNormal,
...
// Calculate impulse from bone motion
	...
	static FVector CalculateBoneImpulse(
...
UFUNCTION(BlueprintPure, Category = "OH|Combat|Motion", meta = (DisplayName = "Calculate Chain Average World Velocity"))
static FVector CalculateChainAverageWorldVelocity(
...
static FVector CalculateChainWorldCenter(
...
static float CalculateChainStrikeRange(
    const TArray<FVector>& BoneWorldPositions,
    const FVector& ChainRootWorldPosition
);

	
	static FConstraintInstance* GetActiveConstraintBetweenBones(
	const USkeletalMeshComponent* SkelMeshComp,
	const FName& BoneName1,
	const FName& BoneName2);
	

	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FName GetConstraintNameBetweenBones(
	...
	static bool GetBonesFromConstraintName(
		USkeletalMeshComponent* SkelMeshComp,
	...
	static void GetBoneNamesFromConstraintInstance(const FConstraintInstance& Constraint, FName& OutBoneA, FName& OutBoneB);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Physics", meta = (DisplayName = "Estimate Velocity From Hit"))
	static FVector EstimateVelocityFromHit(
		const FHitResult& Hit,
		UPrimitiveComponent* Component = nullptr,
		bool bIsStriker = true,
 ...
 */
	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FVector GetBoneAccelerationInReferenceFrame(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FName ReferenceBone = NAME_None,
		EOHReferenceSpace RefFrame = EOHReferenceSpace::WorldSpace,
	...
	/** 
	 * Returns single-frame acceleration of BoneName, optionally relative to ReferenceBone, in the selected reference frame.
	 * GC-safe. Uses only previous velocity.
	 */
	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FVector GetBoneAccelerationSingleFrame(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FName ReferenceBone = NAME_None,
		EOHReferenceSpace RefFrame = EOHReferenceSpace::WorldSpace
	...
	UFUNCTION(BlueprintPure, Category = "OnlyHands|SkeletalPhysics")
	static FVector GetBoneJerkSmoothed(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
...
#pragma region PhysicalAnimationHelpers
	...
	static void ComputePhysicsTweaksForBone(
	...
	UFUNCTION(BlueprintCallable, Category="OH|Physics")
	static FSimpleConstraintProfile ComputeOptimalConstraintSettings(
		USkeletalMeshComponent* SkeletalMeshComponent,
		FName BoneName
	);
	...
	*/
	// C++-only version
	static void ComputeOptimalConstraintSettings(
		USkeletalMeshComponent* SkeletalMeshComponent,
	...
	static FSimpleConstraintProfile ConstraintProfileToSimple(
	...
	UFUNCTION(BlueprintCallable, Category="OH|Physics")
	static FSimpleConstraintProfile GetConstraintProfileForBoneRelationship(
		USkeletalMeshComponent* SkeletalMeshComponent,
		FName ChildBone,
		FName ParentBone
	);

	static FPhysicalAnimationData LerpProfiles(const FPhysicalAnimationData& A,
	                                           const FPhysicalAnimationData& B, float Alpha)
	{
	...
	static FPhysicalAnimationData GetStrongerProfile(
   const FPhysicalAnimationData& A,
...
#pragma region Physics Extraction
	...
	static int32 GetBoneHierarchyDistance(
