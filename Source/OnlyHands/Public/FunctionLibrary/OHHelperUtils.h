...
class ONLYHANDS_API UOHHelperUtils : public UBlueprintFunctionLibrary {
  ...
  public:
     ...
     */
    template <typename TEnum> static FORCEINLINE FName GetEnumDisplayName(TEnum EnumValue) {
        static_assert(TIsEnum<TEnum>::Value, "GetEnumDisplayName can only be used with enum types");
     ...
     */
    template <typename TEnum> static FORCEINLINE FString GetEnumValueAsString(TEnum EnumValue) {
        static_assert(TIsEnum<TEnum>::Value, "GetEnumValueAsString can only be used with enum types");
        ...
        FString FullName = EnumPtr->GetNameByValue(static_cast<int64>(EnumValue)).ToString();

        // Optionally strip enum prefix, leaving just the value
        FString CleanName;
        FullName.Split(TEXT("::"), nullptr, &CleanName, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
    ...
    static FORCEINLINE TEnum GetEnumValueFromString(const FString& EnumName, TEnum DefaultValue) {
        static_assert(TIsEnum<TEnum>::Value, "GetEnumValueFromString can only be used with enum types");
        ...
        return EnumValue != INDEX_NONE ? static_cast<TEnum>(EnumValue) : DefaultValue;
    }
    ...
    /**
     * Check if a bone is a finger bone (wrapper for IsFingerBone in EOHPhysicsEnums.h)
     */
    ...
    /**
     * Get the body BodyPart for a bone (wrapper for GetBodyPartForBone in EOHPhysicsEnums.h)
     */
    UFUNCTION(BlueprintPure, Category = "OnlyHands|Utils|Bones")
