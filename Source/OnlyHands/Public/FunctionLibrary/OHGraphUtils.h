 ...
 * UOHGraphUtils
 *
 * Static utilities for building, validating, and querying the OHPhysicsGraph structure.
 * Leverages OHSafeMapUtils for robust collection operations and validation logic.
 */
UCLASS()
class ONLYHANDS_API UOHGraphUtils : public UBlueprintFunctionLibrary {
  ...
  public:
    ...
    static bool IsBoneValid(const FOHBoneData& Bone, EValidationStrictness Strictness);

    /** Inserts bone unless an irreconcilable conflict exists with current entry */
    static bool AddBoneIfNoConflict(TMap<FName, FOHBoneData>& BoneMap, const FOHBoneData& BoneData);

    /** Inserts all bones from source into target if they pass validation at the given strictness level */
    static int32 InsertAllBonesIfValid(TMap<FName, FOHBoneData>& TargetMap, const TMap<FName, FOHBoneData>& SourceMap,
                                       EValidationStrictness Strictness);

    /** Inserts all structurally valid constraints into the graph */
    static int32 InsertAllConstraintsIfValid(TArray<FOHConstraintInstanceData>& TargetList,
    ...
    static TArray<FName> GetChildBones(const TMap<FName, FOHBoneData>& BoneMap, FName ParentBone);

    /** Gets all constraints referencing a specific bone as parent or child */
    static TArray<FOHConstraintInstanceData>
    GetConstraintsForBone(FName BoneName, const TArray<FOHConstraintInstanceData>& ConstraintList);

    /** Gets all bone names that are flagged as having simulated bodies */
    static TArray<FName> GetSimulatedBones(const TMap<FName, FOHBoneData>& BoneMap);

    /** Removes any constraints that reference non-existent or non-simulated bones */
    static void RemoveInvalidConstraints(TArray<FOHConstraintInstanceData>& ConstraintList,
                                         const TMap<FName, FOHBoneData>& BoneMap);
    ...
    /** Computes the Shannon entropy of the bone name key distribution (graph complexity metric) */
    ...
    static void RemoveBoneMassOutliers(TMap<FName, FOHBoneData>& BoneMap, float ThresholdZ = 2.0f);

    /** Logs runtime stats and health checks for a given physics graph */
    static void LogGraphStats(const TMap<FName, FOHBoneData>& BoneMap,
    ...
    // Rebuilds graph defensively, maintaining existing state when partial rebuild fails.
    static bool SafeRebuildGraph(const USkeletalMeshComponent* SkelComp, FOHPhysicsGraphNode& Graph,
    ...
    static TSet<FName> GetAllDescendants(const FOHPhysicsGraphNode& Graph, FName RootBone);

    // Builds adjacency map for constraints between simulated bodies.
    static TMap<FName, TArray<FName>> BuildAdjacencyMap(const FOHPhysicsGraphNode& Graph);

    // Gathers all connected components (islands of connected bodies).
    static TArray<TSet<FName>> ExtractConnectedComponents(const FOHPhysicsGraphNode& Graph);
    ...
    static TMap<FName, FString> GetConstraintHealthReport(const FOHPhysicsGraphNode& Graph);
    ...
    static float CalculateConstraintInfluenceScore(const FOHConstraintInstanceData& Constraint,
                                                   const FOHPhysicsGraphNode& Graph,
                                                   const USkeletalMeshComponent* MeshComp);

    static void DrawPhysicsGraphOverlay(const FOHPhysicsGraphNode& Graph, const USkeletalMeshComponent* MeshComp,
                                        UWorld* World, float Duration = 1.f);

    /** Detect if the physics graph contains any cycles */
    static bool DetectCyclesInGraph(const TMap<FName, FOHBoneData>& BoneMap,
                                    const TArray<FOHConstraintInstanceData>& Constraints);
    ...
    static void RefreshConstraintInstances(FOHPhysicsGraphNode& Graph, USkeletalMeshComponent* SkeletalMesh);

    // Runs a BFS from a root bone to collect reachable nodes.
    static TSet<FName> TraverseGraphBFS(const FOHPhysicsGraphNode& Graph, FName RootBone);

    // Breadth-first traversal: returns visited bones up to limit
    UFUNCTION(BlueprintCallable, Category = "OnlyHands|Graph")
    static TSet<FName> TraverseGraphBFS(const FOHPhysicsGraphNode& Graph, FName Start, int32 MaxNodesToVisit = 512);
    ...
    UFUNCTION(BlueprintCallable, Category = "OnlyHands|Graph")
    static bool ValidateGraph(FOHPhysicsGraphNode& Graph, EValidationStrictness Strictness);

    // Example: prune invalid bones or constraints from graph
    UFUNCTION(BlueprintCallable, Category = "OnlyHands|Graph")
    static void PruneInvalidGraphElements(FOHPhysicsGraphNode& Graph, EValidationStrictness Strictness);
...
static FAutoConsoleCommand Cmd_ShowPhysicsGraphOverlay(
    TEXT("ShowPhysicsGraphOverlay"), TEXT("Set OnlyHands physics graph overlay: 0=None, 1=Full, 2=Anomalies, 3=Both"),
    FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args) {
        if (Args.Num() == 0) {
            UE_LOG(LogTemp, Log, TEXT("Current overlay mode: %d"), (int32)UOHGraphUtils::GetPhysicsGraphOverlayMode());
            return;
        }
        int32 Mode = FCString::Atoi(*Args[0]);
        Mode = FMath::Clamp(Mode, 0, 3);
        UOHGraphUtils::SetPhysicsGraphOverlayMode(static_cast<EPhysicsGraphOverlayMode>(Mode));
        UE_LOG(LogTemp, Log, TEXT("Physics Graph Overlay set to mode %d"), Mode);
    }));
#pragma endregion