...
public:
	...
	static int32 InsertAllBonesIfValid(
	...
	static void RemoveInvalidConstraints(
	...
	static bool ValidateAllChainsStable(
		const TMap<FName, FOHBoneData>& BoneMap,
		const TArray<FOHConstraintInstanceData>& Constraints,
		int32 MaxDepth = 16);
	...
	static bool RefreshConstraintsOnlyFromComponent(USkeletalMeshComponent* SkeletalMeshComp, FOHPhysicsGraphNode& Graph,
	...
	static bool DetectCyclesInGraph(
		const TMap<FName, FOHBoneData>& BoneMap,
		const TArray<FOHConstraintInstanceData>& Constraints);
	...
	static void RefreshConstraintInstances(
		FOHPhysicsGraphNode& Graph,
	...
	static TSet<FName> TraverseGraphBFS(
		const FOHPhysicsGraphNode& Graph,
		FName Start,
		int32 MaxNodesToVisit = 512);

	

	// Runs DFS from a root bone, calling visitor lambda per node.
	static void TraverseGraphDFS(
		const FOHPhysicsGraphNode& Graph,
		FName Start,
	...
	UFUNCTION(BlueprintCallable, Category = "OnlyHands|Graph")
	static void PruneInvalidGraphElements(
		FOHPhysicsGraphNode& Graph,
		EValidationStrictness Strictness);
	...
	static void DrawPhysicsGraphOverlay(const FOHPhysicsGraphNode& Graph, const USkeletalMeshComponent* MeshComp,
	                                    UWorld* World,
	                                    float Duration, EPhysicsGraphOverlayMode OverlayMode, bool bDrawLabels);
...
static FAutoConsoleCommand Cmd_ShowPhysicsGraphOverlay(
	...
	TEXT("Set OnlyHands physics graph overlay: 0=None, 1=Full, 2=Anomalies, 3=Both"),
