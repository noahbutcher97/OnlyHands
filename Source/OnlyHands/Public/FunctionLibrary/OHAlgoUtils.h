...
>>>>>>> 0627b7d296554ee97d27b39fb5f7c959d6da32c9
            ...
            UFUNCTION(BlueprintCallable, Category = "OnlyHands|Algo|Filtering") static FOHKalmanState KalmanPredict(
                const FOHKalmanState& CurrentState, const FVector& Measurement, float ProcessNoise,
                float MeasurementNoise, float DeltaTime = 0.016f) {
...
#pragma region AlgorithmTemplates
        ...
        static FORCEINLINE FVector PredictQuadratic(const FOHMotionFrameSample& Sample, float PredictTime,
            ...
            const FVector& Acceleration =
                bUseLocalSpace ? Sample.LocalLinearAcceleration : Sample.WorldLinearAcceleration;
            return Sample.WorldPosition + Velocity * PredictTime + 0.5f * Acceleration * FMath::Square(PredictTime);
        }
        ...
        // 1. Template version (works for any SampleType with GetLocation())
        template <typename SampleType>
        static FORCEINLINE FVector PredictDamped(const SampleType& Current, float PredictTime,
                                                 float DampingRatio = 3.0f) {
            const float Decay = FMath::Exp(-FMath::Abs(DampingRatio) * PredictTime);
            const FVector DampedVel = Current.GetLinearVelocity() * Decay;
            return Current.GetLocation() + DampedVel * PredictTime;
        ...
        // Damped prediction for FOHMotionFrameSample
        static FORCEINLINE FVector PredictDamped(const FOHMotionFrameSample& Sample, float PredictTime,
                                                 float DampingFactor, bool bUseLocalVelocity) {
            FVector Velocity = bUseLocalVelocity ? Sample.LocalLinearVelocity : Sample.WorldLinearVelocity;
        ...
        template <typename SampleType>
        // 1. Template version (works for any SampleType with GetLocation())
        static FORCEINLINE float EstimateTimeToReachTargetLinear(const SampleType& Current,
                                                                 const FVector& TargetPosition) {
            const FVector Dir = TargetPosition - Current.GetLocation();
            const float Speed = Current.GetLinearVelocity().Size();
            if (Speed <= SMALL_NUMBER) {
        ...
        static FORCEINLINE SampleType IntegrateVerletSample(const SampleType& Current, const FVector& PreviousPosition,
                                                            float DeltaTime) {
            const FVector NewPos = 2 * Current.GetLocation() - PreviousPosition +
        ...
        // 3. Legacy overload for Blueprint or struct
        static FRK4State IntegrateRK4(const FRK4State& InitialState, float DeltaTime,
                                      const FVector& ConstantAcceleration);
        ...
        IntegrateCriticallyDampedSpringMotionSample(const FOHMotionFrameSample& CurrentSample,
                                                    const FVector& TargetPosition, float SpringFrequency,
                                                    float TimeStep) {
            FOHMotionFrameSample Result = CurrentSample;
            ...
            // Update local velocities
            if (!Result.ComponentTransform.Equals(FTransform::Identity)) {
                Result.LocalLinearVelocity =
                    Result.ComponentTransform.InverseTransformVector(Result.WorldLinearVelocity);
                Result.LocalLinearAcceleration =
                    Result.ComponentTransform.InverseTransformVector(Result.WorldLinearAcceleration);
                Result.LocalSpeed = Result.LocalLinearVelocity.Size();
            }
        ...
        // ----- Spring Blend -----
        template <typename SampleType>
        static FORCEINLINE SampleType IntegrateSpringBlendSample(const SampleType& Current,
            ...
            const FVector NewPos = Current.GetLocation() + NewVel * DeltaTime;
            return SampleType::CreateFromState(FTransform(Current.GetRotation(), NewPos), NewVel,
                                               Current.GetAngularVelocity(), SpringForce,
                                               Current.GetAngularAcceleration(), Current.GetTimeStamp() + DeltaTime);
        }
...
#pragma region SolverUtils
        ...
        static void ProjectChainToTargetRotation(const TArray<FName>& ChainBones, TMap<FName, FOHBoneData>& BoneMap,
                                                 const FQuat& TargetRotation, float Stiffness = 1.0f,
                                                 int32 Iterations = 3);
        ...
        static void SolveAndPushChainToTargetTransform(const TArray<FName>& ChainBones,
...
#pragma region PhysicsOperations
        < < < < < < < HEAD
                          ...
                          UFUNCTION(BlueprintPure, Category = "OH|Combat|Physics",
                                    meta = (DisplayName = "Apply Drag")) static FVector
            ApplyDrag(const FVector& Velocity, float DragCoefficient = 0.47f, float CrossSectionalArea = 50.0f,
        ...
        UFUNCTION(BlueprintPure, Category = "OH|Physics|Velocity",
                  meta = (DisplayName = "Calculate Velocity Reflection"))
        ...
        UFUNCTION(BlueprintPure, Category = "OH|Physics|Energy",
                  meta = (DisplayName = "Calculate Kinetic Energy Vector"))
        ...
        UFUNCTION(BlueprintPure, Category = "OH|Physics|Energy",
                  meta = (DisplayName = "Calculate Elastic Energy Fraction"))
        ...
        UFUNCTION(BlueprintPure, Category = "OH|Physics|Materials")
        static float DeriveRestitutionCoefficient(float YoungsModulus1, float YoungsModulus2,
                                                  float PoissonsRatio1 = 0.3f, float PoissonsRatio2 = 0.3f);
        ...
        UFUNCTION(BlueprintPure, Category = "OH|Physics|Impulse",
                  meta = (DisplayName = "Calculate Impulse From Momentum"))
        ...
        static float
        CalculateMomentOfInertia(float Mass, float Radius,
        ...
        static float CalculateCollisionImpulseMagnitude(float RelativeNormalVelocity, float ReducedMass,
                                                        float Restitution);
        ...
        == == == =
                     // Apply drag to velocity
            UFUNCTION(BlueprintPure, Category = "OH|Combat|Physics", meta = (DisplayName = "Apply Drag")) static FVector
            ApplyDrag(const FVector& Velocity, float DragCoefficient = 0.47f, float CrossSectionalArea = 50.0f,
                      float Mass = 5.0f, float DeltaTime = 0.016f);
...
#pragma region MotionPredictors
                ...
                UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Prediction") static TArray<FVector>
                GetCubicBezierControlPointsFromBoneData(const FOHBoneData& BoneData, float PredictTime = 0.3f);
...
#pragma region NameResolution
        ...
        static FOHNameMatchResult FindBestNameMatchUsingStrategy(const FString& Target,
                                                                 const TArray<FString>& Candidates,
        ...
        static TArray<FOHNameMatchResult> RankFNameToEnumMap(const FString& Target,
        ...
        static FOHNameMatchResult FindBestNameMatchAutoStrategy(const FString& Target,
                                                                const TArray<FString>& Candidates);
...
#pragma region DistanceFormulas
          ...
          UFUNCTION(BlueprintPure, Category = "OnlyHands|Algo|Distance") static float
          ManhattanDistance(const FVector& A, const FVector& B);
...
#pragma endregion
        ...
        < < < < < < < HEAD
            ...
            static float
            CalculateMotionQuality(const FOHBoneMotionData& MotionData);
        ...
        static TArray<FVector> GetBezierControlPointsFromBoneData(const FOHBoneMotionData& MotionData,
                                                                  float PredictTime, bool bCubic = true);
...
#pragma region Debug
        ...
        void DrawDebugQuadraticBezier(const UWorld* World, const TArray<FVector>& Points, int32 Steps,
...
#pragma endregion
      ...
      < < < < < < < HEAD private :
          ...
          static TMap<EOHBiologicalMaterial, FMaterialPhysicsProperties>
          GetMaterialDatabase();
