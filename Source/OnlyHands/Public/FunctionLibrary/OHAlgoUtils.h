...
public:
  ...
  template <typename TEnum> static FString GetEnumName(TEnum EnumVal) {
    return StaticEnum<TEnum>()
        ->GetNameStringByValue(static_cast<int64>(EnumVal))
        .ToLower();
  }
  ...
  static int32 GetBoneHierarchyDepth(const USkeletalMeshComponent *Mesh,
                                     FName Bone);
...
#pragma region IntegrationUtils
  ...
  IntegrateSemiImplicitEulerSample(const FOHMotionSample &Current,
                                   float DeltaTime);

  /** Verlet method requiring previous position */
  static FOHMotionSample IntegrateVerletSample(const FOHMotionSample &Current,
                                               const FVector &PreviousPosition,
                                               float DeltaTime);

  /** Velocity Verlet method requiring next acceleration */
  static FOHMotionSample
  IntegrateVelocityVerletSample(const FOHMotionSample &Current,
  ...
  static FOHMotionSample
  IntegrateImplicitEulerSample(const FOHMotionSample &Current, float DeltaTime);
  ...
  static FOHMotionSample IntegrateAdaptiveRK4Sample(
      const FOHMotionSample &Current,
      TFunction<FVector(const FVector &, const FVector &)> AccelFunc,
      float &InOutDeltaTime, float MinStep = 0.0001f, float MaxStep = 0.033f,
      float Tolerance = 0.01f);

  /** Critically Damped Spring Integrator */
  static FOHMotionSample
  IntegrateCriticallyDampedSpringSample(const FOHMotionSample &Current,
                                        const FVector &TargetPosition,
                                        float AngularFreq, float DeltaTime);

  /** Exponential Integration */
  static FOHMotionSample
  IntegrateExponentialSample(const FOHMotionSample &Current, float DampingRatio,
                             float DeltaTime);

  /** Spring-based positional blend (targeting) */
  static FOHMotionSample
  IntegrateSpringBlendSample(const FOHMotionSample &Current,
...
#pragma region SolverUtils
  ...
  static float ComputeBaumgarteTerm(float ConstraintError, float Beta,
                                    float DeltaTime);

  /** Iteratively projects a bone chain toward a target position using
   * spring-like correction */
  static void ProjectChainToTargetPosition(const TArray<FName> &ChainBones,
  ...
  static void ProjectChainToTargetRotation(const TArray<FName> &ChainBones,
                                           TMap<FName, FOHBoneData> &BoneMap,
  ...
  static void ProjectChainToTargetTransform(const TArray<FName> &ChainBones,
                                            TMap<FName, FOHBoneData> &BoneMap,
  ...
  /** Projects a chain to a target transform and pushes the result as motion
   * samples */
...
#pragma region MotionEstimators
  ...
  static FVector ComputeLinearJerkFromSamples(const FOHMotionSample &Newer,
                                              const FOHMotionSample &Older);

  UFUNCTION(BlueprintPure, Category = "OH|Physics|Motion")
  static FVector ComputeAngularJerkFromSamples(const FOHMotionSample &Newer,
                                               const FOHMotionSample &Older);

  /** Estimates jerk (d/dt of acceleration) from a series of motion samples */
  static FVector EstimateJerk(const TArray<FOHMotionSample> &Samples,
                              float DeltaTime);
  ...
  static FVector SmoothVelocityEMA(const TArray<FOHMotionSample> &Samples,
                                   float Alpha);
...
#pragma region MotionPredictors
  ...
  GetCubicBezierControlPointsFromBoneData(const FOHBoneData &BoneData,
                                          float PredictTime = 0.3f);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Prediction")
  static TArray<FVector>
  GetQuadraticBezierControlPointsFromBoneData(const FOHBoneData &BoneData,
                                              float PredictTime = 0.3f);

  /** Predicts future position using linear extrapolation */
  static FVector PredictLinear(const FOHMotionSample &Current,
                               float PredictTime);

  /** Predicts future position using quadratic extrapolation (includes
   * acceleration) */
  static FVector PredictQuadratic(const FOHMotionSample &Current,
                                  float PredictTime);

  /** Predicts future position with velocity damping */
  static FVector PredictDamped(const FOHMotionSample &Current,
                               float PredictTime, float DampingRatio = 3.0f);

  /** Predicts a future position along the curved path from recent samples */
  static FVector
  PredictCurvedFromSamples(const TArray<FOHMotionSample> &Samples,
                           float PredictTime);

  /** Samples a point along a quadratic Bezier defined by three points */
  static FVector SampleBezierQuadratic(const FVector &P0, const FVector &P1,
                                       const FVector &P2, float T);

  /** Samples a point along a cubic Bezier defined by four control points */
  static FVector SampleBezierCubic(const FVector &P0, const FVector &P1,
                                   const FVector &P2, const FVector &P3,
                                   float T);

  /** Estimates time to reach a target point under linear velocity */
  static float EstimateTimeToReachTargetLinear(const FOHMotionSample &Current,
                                               const FVector &TargetPosition);

  /** Estimates time until stop under linear damping */
  static float EstimateTimeToStopDamped(const FOHMotionSample &Current,
...
#pragma region FeedbackControllers
  ...
  static FVector PIDControl(const FVector &Error, const FVector &ErrorIntegral,
                            const FVector &ErrorDerivative, float Kp, float Ki,
                            float Kd, float MaxOutput = BIG_NUMBER);
...
#pragma region ControlAndStabilization
  ...
  static FVector ComputeCriticallyDampedSpring(const FVector &Pos,
                                               const FVector &Vel,
  ...
  static float ComputeBaumgarteStabilization(float ConstraintError, float Beta,
                                             float DeltaTime);
...
#pragma region NameResolution
  ...
  static FOHNameMatchResult
  FindBestNameMatch(const FString &Target, const TArray<FString> &Candidates);
  ...
  static float CosineTFIDFSimilarity(const FString &A, const FString &B,
                                     const TArray<FString> &Corpus);

  static EOHNameMatchingStrategy
  DetermineBestMatchingStrategy(const FString &Target,
                                const TArray<FString> &Candidates);

  static FOHNameMatchResult
  FindBestNameMatchUsingStrategy(const FString &Target,
                                 const TArray<FString> &Candidates,
                                 EOHNameMatchingStrategy Strategy);

  /** Matches multiple target strings to the best candidates using the selected
   * strategy */
  static TArray<FOHNameMatchResult>
  BatchMatchTargetsToCandidates(const TArray<FString> &Targets,
                                const TArray<FString> &Candidates,
                                EOHNameMatchingStrategy Strategy);
  ...
  BatchAutoMatch(const TArray<FString> &Targets,
                 const TArray<FString> &Candidates);

  static TArray<FOHNameMatchResult>
  ScoreMatchesAgainstCandidates(const FString &Target,
                                const TArray<FString> &Candidates,
                                EOHNameMatchingStrategy Strategy);

  static TArray<FOHNameMatchResult> RankFNameToEnumMap(
      const FString &Target, const TMap<FName, EOHSkeletalBone> &Map,
      EOHNameMatchingStrategy Strategy, float MinScoreThreshold = 0.0f);

  // Matching score helpers
  static float GetNormalizedLevenshteinScore(const FString &A,
                                             const FString &B);
  ...
  static FOHNameMatchResult
  FindBestNameMatchAutoStrategy(const FString &Target,
                                const TArray<FString> &Candidates);

  static FOHNameMatchResult
  FindBestNameMatchAutoStrategy(const FName &InTarget,
...
#pragma region Debug
  ...
  void DrawDebugQuadraticBezier(const UWorld *World,
                                const TArray<FVector> &Points, int32 Steps,
                                FLinearColor Color, float Thickness,
                                float Duration);

  UFUNCTION(BlueprintCallable, Category = "OH|Locomotion|Debug")
  void DrawDebugCubicBezier(const UWorld *World, const TArray<FVector> &Points,
