...
#pragma region AlgorithmTemplates
	...
	static FORCEINLINE FVector ComputeLinearJerkFromSamples(const FOHMotionFrameSample& Newer, const FOHMotionFrameSample& Older, bool bUseLocalSpace = false)
	{
	...
	static FORCEINLINE FVector ComputeAngularJerkFromSamples(const FOHMotionFrameSample& Newer, const FOHMotionFrameSample& Older)
	{
		const float Dt = FMath::Max(Newer.TimeStamp - Older.TimeStamp, KINDA_SMALL_NUMBER);
	...
	static float EstimateCurvatureFromBoneData(
		const FOHBoneMotionData& BoneData)
	{
		if (BoneData.GetHistoryDepth() < 3) return 0.0f;
    
		// Get three positions
		FVector P0 = BoneData.GetHistoricalPosition(2);
		FVector P1 = BoneData.GetHistoricalPosition(1);
		FVector P2 = BoneData.GetCurrentPosition();
		...
		if (V1.IsNearlyZero() || V2.IsNearlyZero()) return 0.0f;
	...
	static FORCEINLINE FVector PredictLinear(const FOHMotionFrameSample& Sample, float PredictTime, bool bUseLocalVelocity)
	{
	...
	static FORCEINLINE FVector PredictQuadratic(const FOHMotionFrameSample& Sample, float PredictTime, bool bUseLocalSpace = false)
	{
	...
	static FORCEINLINE FVector PredictDamped(const FOHMotionFrameSample& Sample, float PredictTime, float DampingFactor, bool bUseLocalVelocity)
	{
		FVector Velocity = bUseLocalVelocity ? Sample.LocalLinearVelocity : Sample.WorldLinearVelocity;
	...
	static FORCEINLINE SampleType IntegrateSemiImplicitEulerSample(const SampleType& Current, float DeltaTime)
	{
		...
		return SampleType::CreateFromState(
	...
	static FORCEINLINE void IntegrateSemiImplicitEuler(
	...
	{
		const FVector NewVel = Current.GetLinearVelocity() + Current.GetLinearAcceleration() * DeltaTime;
		const FVector NewPos = Current.GetLocation() + (Current.GetLinearVelocity() + Current.GetLinearAcceleration() *
			DeltaTime) * DeltaTime;
	...
	// 2. Raw-data version for position, velocity, acceleration
	static FORCEINLINE void IntegrateImplicitEuler(
		const FVector& Position,
		const FVector& Velocity,
		const FVector& Acceleration,
		float DeltaTime,
	...
	static FORCEINLINE SampleType IntegrateHeunSample(
		const SampleType& Current,
		AccelFuncType AccelFunc,
		float DeltaTime)
	{
	...
	static FORCEINLINE void IntegrateHeun(
		const FVector& Position,
		const FVector& Velocity,
		AccelFuncType AccelFunc,
		float DeltaTime,
	...
	static FORCEINLINE SampleType IntegrateVerletSample(const SampleType& Current, const FVector& PreviousPosition,
	                                                    float DeltaTime)
	{
		const FVector NewPos = 2 * Current.GetLocation() - PreviousPosition + Current.GetLinearAcceleration() *
			FMath::Square(DeltaTime);
	...
	static FORCEINLINE void IntegrateVerlet(
	...
	{
		const FVector NewPos = Current.GetLocation() + Current.GetLinearVelocity() * DeltaTime + 0.5f * Current.
			GetLinearAcceleration() * FMath::Square(DeltaTime);
		const FVector NewVel = Current.GetLinearVelocity() + 0.5f * (Current.GetLinearAcceleration() + NextAcceleration)
			* DeltaTime;
		return SampleType::CreateFromState(
			FTransform(Current.GetRotation(), NewPos),
			NewVel,
	...
	static FORCEINLINE void IntegrateVelocityVerlet(
		const FVector& Position,
		const FVector& Velocity,
		const FVector& Acceleration,
	...
	static FOHMotionFrameSample IntegrateVelocityVerlet(const FOHMotionFrameSample& Current, const FVector& NextAcceleration, float DeltaTime);
	...
	static FORCEINLINE StateType IntegrateRK4(
	...
	static FORCEINLINE void IntegrateRK4(
		const FVector& Position,
		const FVector& Velocity,
		const FVector& ConstantAcceleration,
		float DeltaTime,
	...
	static FRK4State IntegrateRK4State(
		const FRK4State& CurrentState,
		const FVector& ForceAcceleration,
		float TimeStep)
	{
		FRK4State Result;
	...
	template <typename SampleType, typename AccelFuncType>
	static FORCEINLINE SampleType IntegrateRK4Sample(
		const SampleType& Current,
		AccelFuncType AccelFunc,
		float DeltaTime)
	{
	...
	template <typename SampleType, typename AccelFuncType>
	static FORCEINLINE SampleType IntegrateAdaptiveRK4Sample(
		const SampleType& Current,
		AccelFuncType AccelFunc,
	...
	template <typename SampleType>
	// 1. Template version (works for any SampleType with GetLocation())
	static FORCEINLINE SampleType IntegrateCriticallyDampedSpringSample(
		const SampleType& Current,
		const FVector& TargetPosition,
		float AngularFreq,
		float DeltaTime)
	...
	UFUNCTION(BlueprintCallable, Category = "OnlyHands|Algo|Physics")
	static FOHMotionFrameSample IntegrateCriticallyDampedSpringMotionSample(
		const FOHMotionFrameSample& CurrentSample,
		const FVector& TargetPosition,
		...
		float TimeStep)
	{
		...
		if (!Result.ComponentTransform.Equals(FTransform::Identity))
		{
			...
			Result.LocalLinearAcceleration = Result.ComponentTransform.InverseTransformVector(Result.WorldLinearAcceleration);
	...
	template <typename SampleType>
	// 1. Template version (works for any SampleType with GetLocation())
	static FORCEINLINE SampleType IntegrateExponentialSample(
		const SampleType& Current,
		float DampingRatio,
		float DeltaTime)
	{
	...
	// ----- Spring Blend -----
	template <typename SampleType>
	static FORCEINLINE SampleType IntegrateSpringBlendSample(
		const SampleType& Current,
		const FVector& TargetPosition,
		float BlendAlpha,
		float SpringCoeff,
	...
	static FOHKalmanState KalmanPredict(
		const FOHKalmanState& CurrentState,
		const FVector& Measurement,
		float ProcessNoise,
	...
	UFUNCTION(BlueprintCallable, Category = "OnlyHands|Algo|Filtering")
	static FOHKalmanState PredictKalmanState(
		const FOHKalmanState& CurrentState,
...
#pragma region PhysicsOperations
	...
	static FVector ApplyDrag(
		...
		float CrossSectionalArea = 50.0f,
		float Mass = 5.0f,
		float DeltaTime = 0.016f);
	...
	static float CalculateDistanceFalloff(
		float Distance,
		float FalloffRadius,
	...
	static FVector CalculateFrictionForce(
		const FVector& RelativeVelocity,
		const FVector& NormalForce,
		float StaticFrictionCoeff = 0.6f,
		float KineticFrictionCoeff = 0.4f,
		float StaticThreshold = 1.0f);

	UFUNCTION(BlueprintPure, Category = "OH|Combat|Analysis", meta = (DisplayName = "Get Contact Bistangential"))
	static void GetContactBistangential(
	...
	static float CalculateHertzContactArea(
		float AppliedForce,
		float BodyRadius,
		float ElasticModulus = 1000000.0f);
	...
	static void CalculateShearComponents(
		const FVector& Velocity,
		const FVector& SurfaceNormal,
		FVector& OutShearDirection,
		float& OutShearMagnitude);
	...
	static FVector CalculateVelocityReflection(
	...
	static void DistributeImpactEnergy(
		float TotalEnergy,
		float Restitution,
		float& OutElasticEnergy,
		float& OutPlasticEnergy,
		float& OutHeatEnergy,
		float& OutSoundEnergy);

	// Calculate momentum
	UFUNCTION(BlueprintPure, Category = "OH|Physics|Momentum", meta = (DisplayName = "Calculate Momentum"))
	static FVector CalculateMomentum(
		float Mass,
		const FVector& Velocity);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Physics|Collision")
	static FVector CalculateImpulseWithFriction(
		const FVector& RelativeVelocity,
    ...
    static float CalculateCrossSectionalArea(
    ...
    static float EstimateBoneMassFromVolume(
        const FVector& BoundsExtent,
    ...
    static bool ValidatePhysicsParameters(
        float Restitution,
        float StaticFriction,
        float KineticFriction,
        FString& OutWarnings
    ...
    static void CalibrateCoefficientsFromMotion(
	...
	static FVector CalculateImpulseFromMomentum(
		const FVector& InitialMomentum,
		const FVector& FinalMomentum);
	...
	static float CalculateRestitutionFromVelocities(
		float RelativeVelocityBefore,
		float RelativeVelocityAfter);
	...
	static float CalculateMomentOfInertia(
		float Mass,
		float Radius,
	...
	static FVector CalculateTorque(
		const FVector& Force,
	...
	static float CalculateCollisionImpulseMagnitude(
		float RelativeNormalVelocity,
		float ReducedMass,
		float Restitution);
	...
	static void CalculatePostCollisionVelocities(
		const FVector& Velocity1,
		const FVector& Velocity2,
		float Mass1,
		float Mass2,
		const FVector& CollisionNormal,
		float Restitution,
		FVector& OutVelocity1,
		FVector& OutVelocity2);


	UFUNCTION(BlueprintPure, Category = "OH|Combat|Analysis", meta = (DisplayName = "Analyze Contact Patch"))
	static void AnalyzeContactPatch(
		const FHitResult& Hit,
		float& OutContactArea,
		float& OutPressure,
		FVector& OutShearDirection,
		float& OutShearMagnitude);
	...
	static void CalculateImpactEnergyDistribution(
		const FHitResult& Hit,
		float& OutKineticEnergyTransfer,
		float& OutRotationalEnergy,
		float& OutDeformationEnergy,
		float& OutSoundEnergy,
		float& OutHeatEnergy);
...
#pragma region MotionPredictors
	...
	static TArray<FVector> GetCubicBezierControlPointsFromBoneData(const FOHBoneMotionData& BoneData, float PredictTime);
	...
	static TArray<FVector>GetQuadraticBezierControlPointsFromBoneData(const FOHBoneMotionData& BoneData, float PredictTime);
...
#pragma endregion
	...
	static TArray<FVector> GetBezierControlPointsFromBoneData(const FOHBoneMotionData& MotionData, float PredictTime, bool bCubic = true);
