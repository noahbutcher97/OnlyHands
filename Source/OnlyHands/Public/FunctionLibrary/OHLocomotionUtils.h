...
#pragma region Basic Movement
  public:
    // Lock-On/Strafe
    static FVector CalculateLockOnMovementInputVector(const FVector2D& Input, const ACharacter* Character,
                                                      const AActor* TargetActor);

    // Momentum/Inertia Blend
    static FVector CalculateMomentumBlendedMovementInputVector(const FVector2D& Input, const FRotator& ControlRotation,
                                                               const FVector& Velocity, float MomentumBlendAlpha);
    ...
    /** Blends between previous (momentum) and new (input) direction. Responsiveness: 0 = only momentum, 1 = instantly
     * matches input. */
    ...
    /** Returns an adaptive deadzone based on recent joystick activity (smaller deadzone if user is very active, larger
     * if mostly idle or drifting). */
    UFUNCTION(BlueprintPure, Category = "OH|Input")
    static float GetDynamicDeadzone(const TArray<FVector2D>& RecentInputHistory, float MinDeadzone = 0.1f,
                                    float MaxDeadzone = 0.3f);

    UFUNCTION(BlueprintCallable, Category = "OH|Movement")
    static void ApplyWeightedMovementInput(AActor* Owner, const FVector& MoveVector, const FVector& LastDir,
                                           float Weight);

    UFUNCTION(BlueprintPure, Category = "OH|Movement")
    static FVector CalculateMovementInputVector(const FVector2D& LocalInput, const FRotator& ControlRotation);

    UFUNCTION(BlueprintPure, Category = "OH|Movement")
    static FVector CalculateSmoothedMovementInputVector(
        const FVector2D& RawInput, const FRotator& ControlRotation, float Deadzone = 0.15f,
    ...
    UFUNCTION(BlueprintPure, Category = "OH|Movement")
    static FVector
    CalculateBlendedMovementInputVector(const FVector2D& Input, const FRotator& CameraRotation,
    ...
    static FRotator
    CalculateBlendedHybridFacing(const FRotator& CurrentFacing, // Pawn's current rotation (yaw-only is fine)
                                 const FVector& CharacterLocation, const FVector& TargetLocation,
...
<<<<<<< HEAD
    ...
    static void GetLockOnMovementBasis(ACharacter* Character, AActor* LockOnTarget, FVector& ForwardVector,
                                       FVector& RightVector);
    == == == = UFUNCTION(BlueprintPure) static void GetLockOnMovementBasis(
                 ACharacter * Character, AActor* LockOnTarget, FVector& ForwardVector, FVector& RightVector);
