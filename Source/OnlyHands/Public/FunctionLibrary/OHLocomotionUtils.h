...
/** 
	...
	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeDirectionFromInput(const FVector2D& InputVector, const APlayerController* PlayerController, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeDirectionFromVelocity(const APlayerController* PlayerController, const ACharacter* Character, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeDirectionToTarget(const AActor* TargetActor, const APlayerController* PlayerController, const ACharacter* Character, bool bNormalize = true);
...
#pragma region Player-Relative Directions
	...
	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputePlayerRelativeDirectionFromInput(const FVector2D& InputVector, const ACharacter* Character, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputePlayerRelativeDirectionFromVelocity(const ACharacter* Character, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputePlayerRelativeDirectionToTarget(const AActor* TargetActor, const ACharacter* Character, bool bNormalize = true);
...
#pragma region World-Relative Directions
	...
	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeWorldRelativeDirectionFromInput(const FVector2D& InputVector, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeWorldRelativeDirectionFromVelocity(const ACharacter* Character, bool bNormalize = true);

	UFUNCTION(BlueprintPure)
	static FScreenRelativeDirectionData ComputeWorldRelativeDirectionToTarget(const AActor* TargetActor, const ACharacter* Character, bool bNormalize = true);
#pragma endregion
...
#pragma region Actor-Relative Directions
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
	static FScreenRelativeDirectionData ComputeActorRelativeDirectionFromInput(
		const FVector2D& InputVector,
		const AActor* ReferenceActor,
		bool bNormalize = true);

	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
	static FScreenRelativeDirectionData ComputeActorRelativeDirectionFromVelocity(
		const ACharacter* SourceCharacter,
		const AActor* ReferenceActor,
		bool bNormalize = true);

	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
	static FScreenRelativeDirectionData ComputeActorRelativeDirectionToTarget(
		const AActor* TargetActor,
		const AActor* ReferenceActor,
		bool bNormalize = true);
...
#pragma region Component-Relative Directions
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
	static FScreenRelativeDirectionData ComputeComponentRelativeDirectionFromInput(
		const FVector2D& InputVector,
		const USceneComponent* ReferenceComponent,
		bool bNormalize = true);

	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
	static FScreenRelativeDirectionData ComputeComponentRelativeDirectionFromVelocity(
		const ACharacter* SourceCharacter,
		const USceneComponent* ReferenceComponent,
		bool bNormalize = true);
...
#pragma region ComputeRaw
	...
	static float ComputeScreenRelativeDirection(
		...
		const ACharacter* Character,
		bool bClampToCardinal = false);
	
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Helpers")
	static float ComputeScreenRelativeAngle(const FVector2D& FromVector, const FVector2D& ToVector, bool bNormalize = true);
	...
	static float PredictMovementAngle(ACharacter* Character, const FRotator& FacingRotation, float DeltaTime,
	                                  ...
	                                  float BaseSmoothingAlpha, float SpeedSmoothScale, float PelvisTranslationThreshold,
	                                  float FootLockSpeedThreshold, float FootLockAccelThreshold, float PivotScale,
	                                  float PivotDeadzone, float StrafeBias, float JitterThreshold, float MaxAngleJumpPerFrame,
	                                  float JerkTriggerScale, float MaxTrajectoryBlend);

	UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
	static FVector2D GetScreenRelativeVector(
		const FVector2D& InputVector,
		const APlayerController* PlayerController);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
	static float GetScreenRelativeAngleToTarget(
		const AActor* TargetActor,
		const APlayerController* PlayerController,
		const ACharacter* Character);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
	static float GetScreenRelativeAngleFromVelocity(
		const APlayerController* PlayerController,
		const ACharacter* Character);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
	static float GetScreenRelativeAngleFromInput(
		const FVector2D& InputVector,
		const APlayerController* PlayerController);
	...
	static FVector GetWorldMovementVectorFromInput(
		const FVector2D& InputVector,
		const APlayerController* PlayerController,
		bool bNormalize = true);

	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
	static FRotator GetFacingRotationFromInput(
		const FVector2D& InputVector,
		const APlayerController* PlayerController,
		bool bNormalize = true);
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
	static FVector GetStrafeMovementVectorFromBlendFactors(
		const FVector2D& BlendFactors,
		const FRotator& FacingRotation);
...
#pragma region BPHelpers
	...
	UFUNCTION(BlueprintPure)
	static float PredictMovementAngleMonsterExtended(ACharacter* Character, const FRotator& FacingRotation, float DeltaTime,
	...
	UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Helpers")
	static FVector2D ComputeBlendFactors(const FVector2D& Direction, const FRotator& CameraRotation, bool bNormalize = true);
...
#pragma region InlineHelpers
	...
	FORCEINLINE float static ComputeScreenRelativeAngle_Inline(const FVector2D& Forward, const FVector2D& TargetDirection, bool bNormalize)
	{
		...
		const float Rad = FMath::Atan2(FVector2D::CrossProduct(NormFwd, NormTarget), FVector2D::DotProduct(NormFwd, NormTarget));
...
#pragma endregion
	...
	static float PredictMovementAngleFromHistory(
		...
		float DeltaTime,
		float PredictionTime      = 0.2f,
		float SpeedPredictionScale= 0.004f,
		float PivotDeadzone       = 0.1f,
		float PivotScale          = 20.f,
	...
	static float PredictMovementAngleWithBezierDebug(
		...
		UWorld*    World,
		float      DrawDuration,
		FColor     CurveColor        = FColor::Green,
		int32      NumSegments       = 16,
		float      PredictionTime      = 0.2f,
		float      SpeedPredictionScale= 0.004f,
		float      PivotDeadzone       = 0.1f,
		float      PivotScale          = 20.f,
		float      JerkThreshold       = 0.1f,
		float      StrafeBias          = 0.f
	);
    ...
    float PredictMovementAngleMonster(
        ...
        float      SpeedPredictionScale    = 0.004f,
        float      AccelPredictionScale    = 0.1f,
        float      JerkScale               = 0.01f,
        float      CurvatureScale          = 0.1f,
        float      MaxBlend                = 1.0f,
        float      PivotDeadzone           = 0.1f,
        float      PivotScale              = 20.f,
        float      StrafeBias              = 0.f,
        float      RotationThreshold       = 30.f,
