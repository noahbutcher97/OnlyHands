...
struct FScreenRelativeDirectionData {
  ...
  ShouldMirrorFromBlendFactors(const FVector2D &BlendFactors) {
    return BlendFactors.X < 0.f;
  }

  FORCEINLINE static bool
  ShouldMirrorFromDirection(const FScreenRelativeDirectionData &Dir) {
    return Dir.ClampedStrafeDirection < 0.f;
  }

  FORCEINLINE static bool
  IsStrafingLeft(const FScreenRelativeDirectionData &Dir) {
    return Dir.ClampedStrafeDirection < 0.f;
  }

  FORCEINLINE static bool
  IsStrafingRight(const FScreenRelativeDirectionData &Dir) {
    return Dir.ClampedStrafeDirection > 0.f;
  }

  FORCEINLINE static bool IsStrafing(const FScreenRelativeDirectionData &Dir,
...
#pragma region Screen-Relative Directions
  ...
  static void ApplyStableCameraRelativeMovement(ACharacter *Character,
                                                const FVector2D &Input);

  UFUNCTION(BlueprintCallable, Category = "Input")
  static void ApplyJoystickMovementRelativeToActor(
      ACharacter *Character, const FVector2D &Input,
      const FRotator &ActorRotation, float Deadzone = 0.05f);

  UFUNCTION(BlueprintPure)
  void GetLockOnMovementBasis(ACharacter *Character, AActor *LockOnTarget,
  ...
  static float ComputeSmoothedQuadrantMovementAngle(
      ...
      float PreviousAngle, float DeltaTime, float PredictionWeight = 0.6f,
      float BiasStrength = 0.7f);

  /**
   * Computes the predictive smoothed direction angle and associated animation
   * data. Internally caches the last angle per character and automatically
   * detects physics manager or velocity fallback. Requires no setup. Ideal for
   * plug-and-play use in AnimBP.
   */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FPredictedMotionDirectionData ComputeSmartAnimationDirectionAuto(
      ACharacter *Character, float DeltaTime, float StrafeBias = 0.0f,
      float PredictionTime = 0.2f, float SmoothingStrength = 0.7f,
      float PelvisTranslationThreshold = 1.25f);

  UFUNCTION(BlueprintCallable, Category = "Input")
  static void ApplyJoystickMovementWithBasis(
      APawn *Pawn, const FVector2D &Input, const FRotator &BasisRotation,
      FVector UpVector = FVector::UpVector, float Deadzone = 0.05f);

  UFUNCTION(BlueprintCallable, Category = "Input")
  static void ApplyAutoCameraRelativeJoystickMovement(ACharacter *Character,
                                                      const FVector2D &Input,
                                                      float Deadzone);

  UFUNCTION(BlueprintCallable, Category = "Input")
  static void ApplySmartCameraRelativeJoystickMovement(
      ACharacter *Character, const FVector2D &Input,
      bool bFaceMovementDirection = false, bool bConstrainToXYPlane = true,
      float Deadzone = 0.05f);
  UFUNCTION(BlueprintCallable, Category = "Input")
  static void ApplyFullCameraRelativeJoystickMovement(
      ACharacter *Character, const FVector2D &Input,
      bool bFaceMovementDirection = false, bool bConstrainToXYPlane = true,
  ...
  ComputeDirectionFromInput(const FVector2D &InputVector,
                            const APlayerController *PlayerController,
                            bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputeDirectionFromVelocity(const APlayerController *PlayerController,
                               const ACharacter *Character,
                               bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputeDirectionToTarget(const AActor *TargetActor,
                           const APlayerController *PlayerController,
...
#pragma region Player-Relative Directions
  ...
  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputePlayerRelativeDirectionFromInput(const FVector2D &InputVector,
                                          const ACharacter *Character,
                                          bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputePlayerRelativeDirectionFromVelocity(const ACharacter *Character,
                                             bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputePlayerRelativeDirectionToTarget(const AActor *TargetActor,
                                         const ACharacter *Character,
                                         bool bNormalize = true);
...
#pragma region World-Relative Directions
  ...
  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputeWorldRelativeDirectionFromInput(const FVector2D &InputVector,
                                         bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputeWorldRelativeDirectionFromVelocity(const ACharacter *Character,
                                            bool bNormalize = true);

  UFUNCTION(BlueprintPure)
  static FScreenRelativeDirectionData
  ComputeWorldRelativeDirectionToTarget(const AActor *TargetActor,
                                        const ACharacter *Character,
                                        bool bNormalize = true);
#pragma endregion
...
#pragma region Actor-Relative Directions
  ...
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData
  ComputeActorRelativeDirectionFromInput(const FVector2D &InputVector,
                                         const AActor *ReferenceActor,
                                         bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData
  ComputeActorRelativeDirectionFromVelocity(const ACharacter *SourceCharacter,
                                            const AActor *ReferenceActor,
                                            bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData
  ComputeActorRelativeDirectionToTarget(const AActor *TargetActor,
                                        const AActor *ReferenceActor,
                                        bool bNormalize = true);
...
#pragma region Component-Relative Directions
  ...
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData
  ComputeComponentRelativeDirectionFromInput(
      const FVector2D &InputVector, const USceneComponent *ReferenceComponent,
      bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData
  ComputeComponentRelativeDirectionFromVelocity(
      const ACharacter *SourceCharacter,
      const USceneComponent *ReferenceComponent, bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Direction")
  static FScreenRelativeDirectionData ComputeComponentRelativeDirectionToTarget(
...
#pragma region ComputeRaw
  ...
  static float ComputeScreenRelativeDirection(
  ...
  static float ComputeScreenRelativeAngle(const FVector2D &FromVector,
                                          const FVector2D &ToVector,
                                          bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float PredictMovementAngle(
      ACharacter *Character, const FRotator &FacingRotation, float DeltaTime,
      float &InOutPrevAngle, float BasePredictionTime,
      float SpeedPredictionScale, float BaseSmoothingAlpha,
      float SpeedSmoothScale, float PelvisTranslationThreshold,
      float FootLockSpeedThreshold, float FootLockAccelThreshold,
      float PivotScale, float PivotDeadzone, float StrafeBias,
      float JitterThreshold, float MaxAngleJumpPerFrame, float JerkTriggerScale,
      float MaxTrajectoryBlend);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector2D
  GetScreenRelativeVector(const FVector2D &InputVector,
                          const APlayerController *PlayerController);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static EScreenDirection4 GetScreenRelativeCardinalDirectionToTarget(
      const AActor *TargetActor, const APlayerController *PlayerController,
      const ACharacter *Character);

  /** Returns the screen-relative angle [-180, 180] from the character to the
   * target actor */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float
  GetScreenRelativeAngleToTarget(const AActor *TargetActor,
                                 const APlayerController *PlayerController,
                                 const ACharacter *Character);

  /** Returns the 8-way screen-relative direction from the character to the
   * target actor */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static EScreenDirection8 GetScreenRelative8WayDirectionToTarget(
      const AActor *TargetActor, const APlayerController *PlayerController,
      const ACharacter *Character);

  /** Returns blend factors (X: Right, Y: Forward) for screen-relative direction
   * to the target actor */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector2D GetScreenRelativeBlendFactorsToTarget(
      const AActor *TargetActor, const APlayerController *PlayerController,
      const ACharacter *Character);

  /** Returns the screen-relative angle [-180, 180] based on the character's
   * velocity */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float
  GetScreenRelativeAngleFromVelocity(const APlayerController *PlayerController,
                                     const ACharacter *Character);

  /** Returns the 8-way screen-relative direction based on the character's
   * velocity */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static EScreenDirection8 GetScreenRelative8WayDirectionFromVelocity(
      const APlayerController *PlayerController, const ACharacter *Character);

  /** Returns blend factors (X: Right, Y: Forward) for screen-relative direction
   * based on velocity */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector2D GetScreenRelativeBlendFactorsFromVelocity(
      const APlayerController *PlayerController, const ACharacter *Character);

  /** Returns the screen-relative angle [-180, 180] based on input vector */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float
  GetScreenRelativeAngleFromInput(const FVector2D &InputVector,
                                  const APlayerController *PlayerController);

  /** Returns the 8-way screen-relative direction based on input vector */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static EScreenDirection8 GetScreenRelative8WayDirectionFromInput(
      const FVector2D &InputVector, const APlayerController *PlayerController);

  /** Returns blend factors (X: Right, Y: Forward) for screen-relative direction
   * based on input vector */
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector2D GetScreenRelativeBlendFactorsFromInput(
      const FVector2D &InputVector, const APlayerController *PlayerController);

  ///////////////////////////////////////////////
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
  static FVector
  GetWorldMovementVectorFromInput(const FVector2D &InputVector,
                                  const APlayerController *PlayerController,
                                  bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
  static FRotator
  GetFacingRotationFromInput(const FVector2D &InputVector,
                             const APlayerController *PlayerController,
                             bool bNormalize = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
  static FVector GetWorldMovementVectorFromBlendFactors(
      const FVector2D &BlendFactors, const APlayerController *PlayerController);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
  static FRotator GetFacingRotationFromDirection(
      const FScreenRelativeDirectionData &DirectionData,
      const APlayerController *PlayerController);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Output")
  static FVector
  GetStrafeMovementVectorFromBlendFactors(const FVector2D &BlendFactors,
                                          const FRotator &FacingRotation);

  ///////////////////////////////////////////////
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  FVector
  GetCameraRelativeMovementVectorFromInput(const FVector2D &InputVector,
                                           const FRotator &CameraRotation,
                                           bool bNormalize);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  FRotator
  GetCameraRelativeFacingRotationFromInput(const FVector2D &InputVector,
                                           const FRotator &CameraRotation,
                                           bool bNormalize);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector
  GetActorRelativeMovementVectorFromInput(const FVector2D &InputVector,
                                          const AActor *ReferenceActor,
                                          bool bNormalize);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FRotator
  GetActorRelativeFacingRotationFromInput(const FVector2D &InputVector,
                                          const AActor *ReferenceActor,
                                          bool bNormalize);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FVector GetComponentRelativeMovementVectorFromInput(
      const FVector2D &InputVector, const USceneComponent *ReferenceComponent,
      bool bNormalize);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static FRotator GetComponentRelativeFacingRotationFromInput(
      const FVector2D &InputVector, const USceneComponent *ReferenceComponent,
      bool bNormalize);
...
#pragma region StructuredDirectionInput
  ...
  static FScreenRelativeDirectionData BuildScreenDirectionFromInput(
      const FVector2D &InputVector, const FRotator &CameraRotation,
      const AActor *OptionalActor, const APlayerController *OptionalController,
      bool bAllowMirroring = true, bool bClamp = false);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Animation")
  static FVector2D GetOptimalAnimationBlendVector(
      const FScreenRelativeDirectionData &DirectionData, bool bNormalize = true,
      bool bClamp = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Animation")
  static FVector2D GetVisualConsistentAnimationBlendVector(
      const FScreenRelativeDirectionData &DirectionData,
      const AActor *ReferenceActor, const APlayerController *PlayerController,
      bool bNormalize = true, bool bClamp = true, bool bApplyMirroring = true);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Animation")
  static float GetVisualConsistentAnimationDirectionAngle(
      const FScreenRelativeDirectionData &DirectionData,
      const AActor *ReferenceActor, const APlayerController *PlayerController,
...
#pragma region Bone-Based Locomotion
  ...
  static float GetPredictedDirectionAngleFromBoneMotion(
      const FOHBoneData &BoneData, const AActor *ReferenceActor,
      const APlayerController *PlayerController, float PredictAheadTime = 0.2f,
      bool bApplyMirroring = true, bool bNormalize = false,
      bool bClamp = false);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Physics")
  static FVector2D GetPredictedBlendVectorFromBoneMotion(
      const FOHBoneData &BoneData, const AActor *ReferenceActor,
      const APlayerController *PlayerController, float PredictAheadTime = 0.2f,
      bool bApplyMirroring = true, bool bNormalize = true, bool bClamp = true);
...
#pragma region BPHelpers
  ...
  UFUNCTION(BlueprintPure, Category = "OH|Locomotion|Helpers")
  static FVector2D ComputeBlendFactors(const FVector2D &Direction,
                                       const FRotator &CameraRotation,
                                       bool bNormalize = true);
...
#pragma region InlineHelpers
  ...
  FORCEINLINE static FVector2D
  ComputeBlendFactors_Inline(const FVector2D &Direction,
                             const FRotator &ReferenceRot,
                             bool bNormalize = true) {
  ...
  FORCEINLINE float static ComputeScreenRelativeAngle_Inline(
      const FVector2D &Forward, const FVector2D &TargetDirection,
      bool bNormalize) {
    const FVector2D NormFwd = bNormalize ? Forward.GetSafeNormal() : Forward;
    const FVector2D NormTarget =
        bNormalize ? TargetDirection.GetSafeNormal() : TargetDirection;
    const float Rad = FMath::Atan2(FVector2D::CrossProduct(NormFwd, NormTarget),
                                   FVector2D::DotProduct(NormFwd, NormTarget));
    return FMath::RadiansToDegrees(Rad);
  }

  FORCEINLINE
      EScreenDirection8 static MapAngleTo8WayDirection_Inline(float Angle) {
  ...
  FORCEINLINE
      EScreenDirection4 static MapAngleTo4WayDirection_Inline(float Angle) {
    const float Normalized = FMath::Fmod(Angle + 360.f, 360.f);
  ...
  FORCEINLINE
      EScreenDirectionQuadrant static MapAngleToQuadrant_Inline(float Angle) {
    const float Normalized = FMath::Fmod(Angle + 360.f, 360.f);
    int32 Sector = FMath::RoundToInt(Normalized / 90.f) % 4;
...
#pragma endregion
  ...
  /**
   * Predicts a movement‐direction angle (in degrees) relative to
   * FacingRotation, entirely driven by the root‐bone’s motion sample history—no
   * previous‐angle input.
   *
   ...
   * @param PredictionTime      How far ahead to predict (unused for
   * history‐based).
   * @param SpeedPredictionScale Scale factor if you ever wanted to weight
   * speed.
   * @param PivotDeadzone       Minimum curve strength before pivoting.
   * @param PivotScale          How strongly to pivot when curve > deadzone.
   * @param StrafeBias          −1=always strafe left, +1=always strafe right,
   * 0=no bias.
   * @return                    Angle in [0,360).
   */

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float PredictMovementAngleFromHistory(
      ACharacter *Character, const FRotator &FacingRotation, float DeltaTime,
  ...
  /**
   * Same as above, but also draws a quadratic Bézier (P0→P1→P2) in‐world for
   * debugging.
   *
  ...
  static float PredictMovementAngleWithBezierDebug(
      ACharacter *Character, const FRotator &FacingRotation, float DeltaTime,
      UWorld *World, float DrawDuration, FColor CurveColor = FColor::Green,
      int32 NumSegments = 16, float PredictionTime = 0.2f,
      float SpeedPredictionScale = 0.004f, float PivotDeadzone = 0.1f,
      float PivotScale = 20.f, float JerkThreshold = 0.1f,
      float StrafeBias = 0.f);
   ...
   * @param World                If non-null & DrawDuration>0, used for
   * DrawDebugLine()
   * @param DrawDuration         Lifetime of debug Bézier lines (≤0 disables
   * draw)
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "OH|Locomotion")
  float PredictMovementAngleMonster(
      ACharacter *Character, const FRotator &FacingRotation, float DeltaTime,
      ...
      float JerkScale = 0.01f, float CurvatureScale = 0.1f,
      float MaxBlend = 1.0f, float PivotDeadzone = 0.1f,
      float PivotScale = 20.f, float StrafeBias = 0.f,
      float RotationThreshold = 30.f, float MoveThreshold = 5.f,
      float ProcNoise = 1e-3f, float MeasNoise = 1e-1f);

  UFUNCTION(BlueprintPure, Category = "OH|Locomotion")
  static float PredictMovementAngleMonsterNoDebug(
      ACharacter *Character, const FRotator &FacingRotation, float DeltaTime,
      float PredictionTime = 0.3f, float SpeedPredictionScale = 0.004f,
      float AccelPredictionScale = 0.1f, float JerkScale = 0.01f,
      float CurvatureScale = 0.1f, float MaxBlend = 1.0f,
      float PivotDeadzone = 0.1f, float PivotScale = 20.f,
      float StrafeBias = 0.f, float RotationThreshold = 30.f,
      float MoveThreshold = 5.f, float ProcNoise = 1e-3f,
