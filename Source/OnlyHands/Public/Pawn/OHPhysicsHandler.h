...
public:
  ...
  static AActor *FindClosestActor(const TArray<AActor *> &Actors,
                                  const FVector &ToLocation, float MaxDistance,
                                  float &OutDistance);

  // 2. Find the closest simulating physics UPrimitiveComponent on an actor
  static bool FindClosestSimulatingComponent(AActor *Actor,
  ...
  // 3. Find the closest simulating FBodyInstance in a skeletal mesh (and bone
  // name)
  static bool FindClosestSimulatingBodyInstance(
...
protected:
  ...
  FVector GrabbedOffset =
      FVector::ZeroVector; // offset from grab point to object COM
  FVector2D AccumulatedGrabDelta =
      FVector2D::ZeroVector; // Mouse axis delta since grab
  FVector CurrentGrabTargetLocation = FVector::ZeroVector;
  FVector2D PreviousMousePosition;
  bool bDraggingObject =
      false; // Set this to true in Grab(), false in Release()
...
private:
  ...
  bool
  TryGrabUnderCursor(const TArray<TEnumAsByte<EObjectTypeQuery>> &ObjectTypes,
