...
public:
    ...
    static AActor* FindClosestActor(const TArray<AActor*>& Actors, const FVector& ToLocation, float MaxDistance, float& OutDistance);

    // 2. Find the closest simulating physics UPrimitiveComponent on an actor
    static bool FindClosestSimulatingComponent(
        AActor* Actor,
