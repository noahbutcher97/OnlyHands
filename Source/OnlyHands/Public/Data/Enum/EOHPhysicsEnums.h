...
enum class EOHImpulseMode : uint8 {
  Directional UMETA(DisplayName =
                        "Directional"), // Applies impulse in hit direction
  Radial UMETA(DisplayName = "Radial"), // Impulse radiates from a center point
  VerticalOnly UMETA(DisplayName =
                         "Vertical Only"), // Ignores lateral, uses upward force
  Custom UMETA(DisplayName = "Custom"),    // User-defined application logic
  Auto UMETA(DisplayName =
                 "Auto Infer") // Let system infer best mode from graph or hit
};
...
enum class EOHBoneSimulationState : uint8 {
  ...
  Blending UMETA(DisplayName =
                     "Blending"), // Partially simulating (during fade)
...
enum class EOHBlendState : uint8 {
  ...
  Blending UMETA(DisplayName =
                     "Blending"), // Blending between animation and physics
...
enum class EOHDriveType : uint8 {
  Kinematic UMETA(DisplayName =
                      "Kinematic"), // Driven Entirely By Animation Transform
  Simulated UMETA(DisplayName =
                      "Simulated"), // Driven Entirely By Physics Transform
  AttachedToSimulated UMETA(
      DisplayName =
...
enum class EOHForceApplicationSource : uint8 {
  ...
  PhysicalAnimation UMETA(
      DisplayName = "Physical Animation"), // Force Applied By Physical
                                           // Animation Component
...
enum class EOHBodyZone : uint8 {
  ...
  // Right Arm
  Arm_R_Upper UMETA(DisplayName =
                        "Upper Arm - Right"), // clavicle_r, upperarm_r
...
UENUM(
    ...
    meta =
        (HiddenItems =
             "None,EndOfSpine,EndOfHead,EndOfArm_L,EndOfHand_L,FirstFinger_L,"
             "LastFinger_L,EndOfArm_R,EndOfHand_R,FirstFinger_R,LastFinger_R,"
             "EndOfLeg_L,EndOfLeg_R,FirstIKBone,LastIKBone,FirstBone,LastBone"))
...
FORCEINLINE bool IsLeftFingerBone(const EOHSkeletalBone Bone) {
  return IsInBoneRange(Bone, EOHSkeletalBone::FirstFinger_L,
                       EOHSkeletalBone::LastFinger_L);
}

/** Check if a bone is a right finger bone */
FORCEINLINE bool IsRightFingerBone(const EOHSkeletalBone Bone) {
  return IsInBoneRange(Bone, EOHSkeletalBone::FirstFinger_R,
                       EOHSkeletalBone::LastFinger_R);
}
...
FORCEINLINE bool IsLeftLegBone(const EOHSkeletalBone Bone) {
  return IsInBoneRange(Bone, EOHSkeletalBone::Thigh_L,
                       EOHSkeletalBone::EndOfLeg_L);
}

/** Check if a bone is part of the right leg */
FORCEINLINE bool IsRightLegBone(const EOHSkeletalBone Bone) {
  return IsInBoneRange(Bone, EOHSkeletalBone::Thigh_R,
                       EOHSkeletalBone::EndOfLeg_R);
}
...
FORCEINLINE bool IsIKBone(const EOHSkeletalBone Bone) {
  return IsInBoneRange(Bone, EOHSkeletalBone::FirstIKBone,
                       EOHSkeletalBone::LastIKBone);
}

/** Check if a bone is on the left side of the body */
FORCEINLINE bool IsLeftSideBone(const EOHSkeletalBone Bone) {
  return IsLeftArmBone(Bone) || IsLeftFingerBone(Bone) || IsLeftLegBone(Bone) ||
         Bone == EOHSkeletalBone::IK_Hand_L ||
...
FORCEINLINE bool IsRightSideBone(const EOHSkeletalBone Bone) {
  return IsRightArmBone(Bone) || IsRightFingerBone(Bone) ||
         IsRightLegBone(Bone) || Bone == EOHSkeletalBone::IK_Hand_R ||
         Bone == EOHSkeletalBone::IK_Foot_R;
}

FORCEINLINE FName GetBoneNameFromEnum(EOHSkeletalBone Bone) {
  const UEnum *EnumPtr = StaticEnum<EOHSkeletalBone>();
  return EnumPtr ? FName(EnumPtr->GetNameStringByValue(static_cast<int64>(Bone))
                             .ToLower())
                 : NAME_None;
}
...
FORCEINLINE EOHSkeletalBone GetEnumFromBoneName(FName BoneName) {
  ...
  for (int32 i = 0; i < EnumPtr->NumEnums(); ++i) {
    if (EnumPtr->GetNameStringByIndex(i).Equals(BoneStr,
...
FORCEINLINE TArray<EOHSkeletalBone>
GetBonesInBodyPart(const EOHBodyPart BodyPart) {
  TArray<EOHSkeletalBone> Bones;
  for (uint8 BoneIndex = static_cast<uint8>(EOHSkeletalBone::FirstBone);
       BoneIndex <= static_cast<uint8>(EOHSkeletalBone::LastBone);
       ++BoneIndex) {
...
FORCEINLINE TArray<EOHSkeletalBone>
GetBonesInBodyRegion(const EOHBodyRegion Region) {
  TArray<EOHSkeletalBone> Bones;
  for (uint8 BoneIndex = static_cast<uint8>(EOHSkeletalBone::FirstBone);
       BoneIndex <= static_cast<uint8>(EOHSkeletalBone::LastBone);
       ++BoneIndex) {
...
FORCEINLINE TArray<EOHBodyPart>
GetBodyPartsInBodyRegion(const EOHBodyRegion Region) {
...
UENUM(BlueprintType)
enum class EOHRelativeDirection : uint8 {
  Forward,
  Backward,
  Left,
  Right,
...
UENUM(BlueprintType)
enum class EDriveAccessMode : uint8 {
