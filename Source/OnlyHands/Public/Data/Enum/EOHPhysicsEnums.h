...
UENUM()
enum class EValidationStrictness : uint8
{
	CriticalOnly,
	Standard,
	Strict
...
UENUM(BlueprintType)
enum class EOHResolutionContext : uint8
{
	Trackable,
	Simulatable
};
...
UENUM(BlueprintType,
	meta=(HiddenItems=
		"None,EndOfSpine,EndOfHead,EndOfArm_L,EndOfHand_L,FirstFinger_L,LastFinger_L,EndOfArm_R,EndOfHand_R,FirstFinger_R,LastFinger_R,EndOfLeg_L,EndOfLeg_R,FirstIKBone,LastIKBone,FirstBone,LastBone"
	))
...
FORCEINLINE bool IsLeftSideBone(const EOHSkeletalBone Bone)
{
	return IsLeftArmBone(Bone) || IsLeftFingerBone(Bone) || IsLeftLegBone(Bone) ||
		Bone == EOHSkeletalBone::IK_Hand_L || Bone == EOHSkeletalBone::IK_Foot_L;
}
...
FORCEINLINE TArray<EOHSkeletalBone> GetBonesInBodyPart(const EOHBodyPart BodyPart)
{
	...
	for (uint8 BoneIndex = static_cast<uint8>(EOHSkeletalBone::FirstBone); BoneIndex <= static_cast<uint8>(
		     EOHSkeletalBone::LastBone); ++BoneIndex)
	{
...
FORCEINLINE TArray<EOHBodyPart> GetBodyPartsInBodyRegion(const EOHBodyRegion Region)
{
	...
	for (uint8 PartIndex = static_cast<uint8>(EOHBodyPart::Head); PartIndex <= static_cast<uint8>(
		     EOHBodyPart::Leg_Right); ++PartIndex)
	{
