...
public:
  ...
  FORCEINLINE void SetAll(float InOrientationStrength,
                          ...
                          float InPositionStrength, float InVelocityStrength,
...
public:
  ...
  FORCEINLINE void
  SetLastReadback(const FConstraintRuntimeReadback &InReadback) {
...
#endif
#define GET_OH_REF_SAFE(Map, BoneEnum)                                         \
  (Map.HasValidReference(BoneEnum) ? Map.GetReference(BoneEnum) : nullptr)
#define GET_OH_INDEX_SAFE(Map, BoneEnum)                                       \
  (Map.HasValidReference(BoneEnum) ? Map.GetPoseIndex(BoneEnum)                \
...
public:
  ...
  // Full constructor
  FOHMotionSample(const FTransform &InTransform, float InTimeStamp,
  ...
  // -- Static Factory --
  static FOHMotionSample
  CreateFromState(const FTransform &Transform, const FVector &LinearVel,
                  const FVector &AngularVel, const FVector &LinAccel,
                  const FVector &AngAccel, float TimeStamp) {
  ...
  FORCEINLINE bool HasNaNs() const {
    return Transform.ContainsNaN() || LinearVelocity.ContainsNaN() ||
           AngularVelocity.ContainsNaN() || LinearAcceleration.ContainsNaN() ||
...
#pragma region Cone Limit Drive
  ...
  FORCEINLINE void SetConeLimitStiffness(float InVal,
...
#pragma region Linear Drive
  ...
  FORCEINLINE float GetLinearDriveStiffness() const {
    return GetConstraintInstance()
               ? GetConstraintInstance()
                     ->ProfileInstance.LinearDrive.XDrive.Stiffness
               : LinearDriveStiffness;
  }
  FORCEINLINE float GetLinearDriveDamping() const {
    return GetConstraintInstance()
               ? GetConstraintInstance()
                     ->ProfileInstance.LinearDrive.XDrive.Damping
               : LinearDriveDamping;
  }
...
#pragma region Angular Drive
  ...
  FORCEINLINE bool
  IsAngularOrientationDriveEnabled(bool bUseLive = true) const {
    return (bUseLive && GetConstraintInstance())
               ? GetConstraintInstance()
                     ->ProfileInstance.AngularDrive.IsOrientationDriveEnabled()
               : bOrientationDriveEnabled; // Reuse or separate if needed
  }

  FORCEINLINE void
  SetAngularOrientationDriveEnabled(bool bEnabled, bool bWriteThrough = true) {
    bPositionDriveEnabled = bEnabled;
    if (bWriteThrough && GetConstraintInstance()) {
      GetConstraintInstance()->ProfileInstance.AngularDrive.AngularDriveMode =
          IsAngularOrientationDriveEnabled() ? EAngularDriveMode::SLERP
  ...
  FORCEINLINE float GetAngularSwingDamping() const {
    return GetConstraintInstance()
               ? GetConstraintInstance()
                     ->ProfileInstance.AngularDrive.SwingDrive.Damping
               : AngularSwingDamping;
  }
  ...
  FORCEINLINE float GetAngularTwistDamping() const {
    return GetConstraintInstance()
               ? GetConstraintInstance()
                     ->ProfileInstance.AngularDrive.TwistDrive.Damping
               : AngularTwistDamping;
  }

  // --- Angular Swing Setters
  FORCEINLINE void SetAngularSwingStiffness(float Val,
                                            bool bWriteToLive = true) {
    AngularSwingStiffness = Val;
    if (bWriteToLive && GetConstraintInstance())
      GetConstraintInstance()
          ->ProfileInstance.AngularDrive.SwingDrive.Stiffness = Val;
  }
...
#pragma region Drive State
  ...
  FORCEINLINE bool IsPositionDriveEnabled(bool bUseLive = true) const {
    return (bUseLive && GetConstraintInstance())
               ? GetConstraintInstance()
                     ->ProfileInstance.LinearDrive.IsPositionDriveEnabled()
...
#pragma endregion
  ...
  FORCEINLINE float GetMaxDriveForce() const {
    return GetConstraintInstance()
               ? GetConstraintInstance()
                     ->ProfileInstance.LinearDrive.XDrive.MaxForce
               : 0.f;
  }
  ...
  float GetConstraintAnchorToBoneDistance() const {
    ...
    FTransform BoneTM =
        SkelMesh->GetSocketTransform(GetParentBone(), RTS_World);
  ...
  bool ValidateConstraintGeometry() const {
    ...
    if (ParentIndex == INDEX_NONE || ChildIndex == INDEX_NONE) {
      UE_LOG(LogTemp, Error,
             TEXT("[Constraint] Invalid bone indices: Parent=%s (%d), Child=%s "
                  "(%d)"),
    ...
    if (AnchorDistance > 50.f) // Suspiciously far anchor
    {
      UE_LOG(LogTemp, Warning,
             TEXT("[Constraint] Anchor distance unusually large: %.2f for %s"),
  ...
  float GetPositionStrain() const {
    ...
    const FVector CurrentPosition =
        CachedOwnerComponent->GetBoneLocation(ChildBone);
  ...
  float GetVelocityStrain() const {
    ...
    const FVector CurrentVelocity =
        CachedOwnerComponent->GetPhysicsLinearVelocity(ChildBone);
    return (CachedVelocityTarget - CurrentVelocity).Size();
  }

  // --- Strain methods ---
  /**
   * Angular‐orientation strain: the absolute angle (deg) between current and
   * target orientation
   */
  ...
  /**
   * Angular‐velocity strain: magnitude difference between desired and actual
   * angular velocity
   */
  float GetAngularVelocityStrain() const {
    ...
    const FVector CurrentAV =
        CachedOwnerComponent->GetPhysicsAngularVelocityInDegrees(ChildBone);
  ...
  float GetCurrentStrain() const {
    if (FConstraintInstance *CI = GetConstraintInstance()) {
      ...
      float MaxSwingStrain =
          FMath::Max(FMath::Abs(Swing1) / FMath::Max(Swing1Limit, 1.f),
  ...
  float GetAngularBias() const {
    if (FConstraintInstance *CI = GetConstraintInstance()) {
      ...
      return FMath::Max3(FMath::Abs(Swing1Bias), FMath::Abs(Swing2Bias),
                         FMath::Abs(TwistBias));
    }
    return 0.f;
  }
  ...
  FString GetAngularBiasLabel() const {
    if (FConstraintInstance *CI = GetConstraintInstance()) {
      ...
      float Swing1Bias =
          (Swing1Limit != 0.f) ? FMath::Abs(Swing1 / Swing1Limit) : 0.f;
      float Swing2Bias =
          (Swing2Limit != 0.f) ? FMath::Abs(Swing2 / Swing2Limit) : 0.f;
      float TwistBias =
          (TwistLimit != 0.f) ? FMath::Abs(Twist / TwistLimit) : 0.f;
  ...
  FORCEINLINE void SetConstraintBreakable(bool bBreakable,
  ...
  GetConstraintFatigueRatio(float WearTime, float CriticalTime = 3.0f) {
  ...
  float PredictTimeToConstraintLimit(const FVector &AngularVelocity,
    ...
    if (FConstraintInstance *CI = GetConstraintInstance()) {
      ...
      float Swing1Limit =
          FMath::Abs(CI->ProfileInstance.ConeLimit.Swing1LimitDegrees);
      float Swing2Limit =
          FMath::Abs(CI->ProfileInstance.ConeLimit.Swing2LimitDegrees);
      float TwistLimit =
          FMath::Abs(CI->ProfileInstance.TwistLimit.TwistLimitDegrees);

      // Project angular velocity onto constraint axes
      FTransform ConstraintTM = CI->GetRefFrame(EConstraintFrame::Frame2);
      FVector LocalAVel =
          ConstraintTM.InverseTransformVectorNoScale(AngularVelocity);

      float TimeToSwing1 = (Swing1Limit - FMath::Abs(Swing1)) /
                           (FMath::Abs(LocalAVel.Y) + KINDA_SMALL_NUMBER);
      float TimeToSwing2 = (Swing2Limit - FMath::Abs(Swing2)) /
                           (FMath::Abs(LocalAVel.Z) + KINDA_SMALL_NUMBER);
      float TimeToTwist = (TwistLimit - FMath::Abs(Twist)) /
                          (FMath::Abs(LocalAVel.X) + KINDA_SMALL_NUMBER);
  ...
  FORCEINLINE void AutoTuneFromStrain(float BaseDamp = 10.f,
  ...
  static FOHConstraintInstanceData
  FromPhysicsConstraint(const FConstraintInstance *Instance,
    ...
    Data.SetSwing1LimitDegrees(
        Instance->ProfileInstance.ConeLimit.Swing1LimitDegrees);
    Data.SetSwing2LimitDegrees(
        Instance->ProfileInstance.ConeLimit.Swing2LimitDegrees);
    Data.SetTwistLimitDegrees(
        Instance->ProfileInstance.TwistLimit.TwistLimitDegrees);
...
public:
  ...
  FORCEINLINE FBodyInstance *GetBodyInstance() const {
    if (!BodyInstance) {
      ...
      return SkeletalMesh ? SkeletalMesh->GetBodyInstance(GetBoneName())
                          : nullptr;
    }
    return BodyInstance;
  }
  ...
  FName GetParentConstraint(
      const TMap<FName, FOHConstraintInstanceData> &ConstraintMap) const {
  ...
  void SetIsSimulating(bool bSimulate) {
    ...
    if (!bSet)
      UE_LOG(
          LogTemp, Warning,
          TEXT("SetIsSimulating: Set only cache, engine body missing for %s"),
  ...
  FORCEINLINE FOHMotionSample GetMotionSampleAt(int32 Index) const {
    return MotionHistory.IsValidIndex(Index) ? MotionHistory[Index]
                                             : FOHMotionSample();
  }
  ...
  FORCEINLINE void PushMotionSample(const FOHMotionSample &Sample) {
    if (!Sample.IsValidSample()) {
      UE_LOG(LogTemp, Warning,
             TEXT("PushMotionSample: Invalid sample ignored."));
      return;
    }
  ...
  void SetAngularDamping(float NewValue) {
    if (!FMath::IsFinite(NewValue)) {
      UE_LOG(LogTemp, Warning,
             TEXT("SetAngularDamping rejected NaN/Inf value"));
      return;
    }
  ...
  float GetMass() const {
    ...
    return GetOwnerComponent() ? GetOwnerComponent()
                                     ->GetBodyInstance(GetBoneName())
                                     ->GetBodyMass()
                               : -1.0f;
  }
  ...
  float GetBoneToConstraintAnchorDistance() const {
    if (FConstraintInstance *CI = GetParentConstraintInstance()) {
      FTransform ConstraintTM = CI->GetRefFrame(
          EConstraintFrame::Frame2); // Frame2 is typically child bone
      FTransform BoneTM =
          GetOwnerComponent()->GetSocketTransform(BoneName, RTS_World);
      return FVector::Dist(BoneTM.GetLocation(), ConstraintTM.GetLocation());
    }
    return -1.f;
  }
  ...
  float GetConstraintAnchorToBoneDistance() const {
    if (FConstraintInstance *CI = GetParentConstraintInstance()) {
      FTransform ConstraintTM = CI->GetRefFrame(
          EConstraintFrame::Frame1); // Frame1 is typically parent bone
      FTransform BoneTM =
          GetOwnerComponent()->GetSocketTransform(BoneName, RTS_World);
      return FVector::Dist(BoneTM.GetLocation(), ConstraintTM.GetLocation());
    }
  ...
  float GetRecommendedSwingLimitDegrees() const {
    // Heuristic: longer bones, or those with farther anchor, can use higher
    // limits
    float BoneLength = GetEstimatedBoneLength(); // Implement using ref skeleton
                                                 // or mesh vertices
    ...
    return 25.f * SpatialFactor; // Example: 25 degrees is default, scaled for
                                 // long/short bones
  }
  ...
  void SetParentConstraintAngularDriveParams(float Stiffness, float Damping,
                                             float MaxForce) const {
    if (FConstraintInstance *CI = GetParentConstraintInstance())
      CI->SetAngularDriveParams(Stiffness, Damping, MaxForce);
  }

  static float GetConstraintFatigueRatio(float WearTime,
  ...
  // void ApplyImpulse(const FVector& Direction, float Magnitude, bool
  // bVelChange = true); void SetSimBlendWeight(float Alpha);
  ...
  // --- Angular bias: How much is the bone "favoring" one side of its allowed
  // motion? (0=centered, >0=biased) --- Get the constraint instance linking
  // this bone to its parent
  FConstraintInstance *GetParentConstraintInstance() const;

  // === Derivations ===
  // Returns average distance from this bone to its immediate neighbors (parent
  // + all children)
  ...
  FVector ComputeImpulseDirection(const FHitResult &Hit,
                                  EImpulseDirectionMode Mode) const;
  FVector ComputeImpulseVector(const FHitResult &Hit,
                               const FVector &RootLocation, float BaseStrength,
                               EImpulseDirectionMode ModeOverride,
                               bool bAutoInferMode,
                               const FOHPhysicsGraphNode *PhysicsGraph) const;
  EImpulseDirectionMode
  InferBestImpulseDirectionMode(const FHitResult &Hit) const;
  ...
  void InitializeFromBodyInstance(const FBodyInstance *BI,
                                  const FReferenceSkeleton &RefSkeleton);
  void InitializeFromBodySetup(const UBodySetup *Setup,
                               const FReferenceSkeleton &RefSkeleton);

  // Motion Samples
  void AddMotionSample(const FTransform &NewTransform, float CurrentTime,
                       float DeltaTime);
  ...
  // Averages
  FVector
  GetAverageLinearVelocity(int32 SampleCount = GetDefaultSampleCount()) const;
  FVector
  GetAverageAngularVelocity(int32 SampleCount = GetDefaultSampleCount()) const;
  ...
  // Speed
  float
  GetAverageLinearSpeed(int32 SampleCount = GetDefaultSampleCount()) const;
  float
  GetAverageAngularSpeed(int32 SampleCount = GetDefaultSampleCount()) const;
  ...
  // Blended
  FVector
  GetBlendedLinearVelocity(float HistoryWeight = 0.5f,
                           int32 SampleCount = GetDefaultSampleCount()) const;
  FVector GetBlendedLinearAcceleration(
      float HistoryWeight = 0.5f,
      int32 SampleCount = GetDefaultSampleCount()) const;
  FVector
  GetBlendedAngularVelocity(float HistoryWeight = 0.5f,
                            int32 SampleCount = GetDefaultSampleCount()) const;
  ...
  // === Kinematic Update ===
  void UpdateKinematics(const FVector &NewPosition, const FQuat &NewRotation,
...
public:
  ...
  // -- ParentToChildrenMap --
  /* FORCEINLINE const TMap<FName, FOHBoneNameArrayWrapper>&
     GetParentToChildrenMap() const { return ParentToChildrenMap; } FORCEINLINE
     TMap<FName, FOHBoneNameArrayWrapper>& GetParentToChildrenMap() { return
     ParentToChildrenMap; } FORCEINLINE void SetParentToChildrenMap(const
     TMap<FName, FOHBoneNameArrayWrapper>& InMap) { ParentToChildrenMap = InMap;
     }*/
  ...
  FORCEINLINE TArray<FName> GetChildrenOfBone(FName ParentBone) const {
    TArray<FName> Out;
    ParentToChildrenMultiMap.MultiFind(ParentBone, Out,
                                       /*bMaintainOrder=*/true);
    return Out;
  }
  ...
  /* FORCEINLINE const TMap<FName, FOHConstraintInstanceArrayWrapper>&
     GetBoneToConstraintsMap() const { return BoneToConstraintsMap; }
       FORCEINLINE TMap<FName, FOHConstraintInstanceArrayWrapper>&
     GetBoneToConstraintsMap() { return BoneToConstraintsMap; } FORCEINLINE void
  ...
  GetConstraintsOfBone(FName BoneName) const {
    ...
    BoneToConstraintsMultiMap.MultiFind(BoneName, Ptrs,
                                        /*bMaintainOrder=*/true);
  ...
  /// Return the constraint data where this bone is the *child* (i.e. the
  /// incoming joint). Bones (except the root) appear exactly once as a child in
  /// ConstraintLinks. Mutable overload: return a non-const pointer so you can
  /// modify the entry.
  ...
  TArray<FName>
  GetInvalidConstraints(TArray<FString> *OutReasons = nullptr) const;
  // === High-Level Queries ===
  ...
  /** Validate the entire graph, optionally auto-repair invalid constraints, and
   * log issues. Returns true if valid. */
  ...
  void DiffBoneSnapshots(const TSet<FName> &OldSnap, const TSet<FName> &NewSnap,
  ...
  void InitializeConstraintComponents(USkeletalMeshComponent *OwnerComponent) {
    if (!OwnerComponent) {
      UE_LOG(LogTemp, Error,
             TEXT("[PhysicsGraph] Cannot initialize constraints: null "
                  "OwnerComponent"));
      return;
    }
    ...
    UE_LOG(
        LogTemp, Log,
        TEXT("[PhysicsGraph] Initialized %d constraints with OwnerComponent"),
        ConstraintLinks.Num());
  }
  ...
  bool ValidateAllConstraintGeometry() const {
    if (!MeshComponent) {
      UE_LOG(LogTemp, Error,
             TEXT("[PhysicsGraph] No MeshComponent cached for validation"));
      return false;
    }
    ...
    for (const FOHConstraintInstanceData &Constraint : ConstraintLinks) {
      if (Constraint.ValidateConstraintGeometry()) {
        ...
        bAllValid = false;
        UE_LOG(LogTemp, Warning,
               TEXT("[PhysicsGraph] Constraint validation failed: %s->%s"),
    ...
    UE_LOG(LogTemp, Log,
           TEXT("[PhysicsGraph] Constraint validation: %d/%d valid"),
  ...
  void LogConstraintPerformanceReport() const {
    ...
    for (const FOHConstraintInstanceData &Constraint : ConstraintLinks) {
      ...
      if (Metrics.bIsValid) {
        UE_LOG(
            LogTemp, Log,
            TEXT("[%s] Anchor:%.1f | Lever:%.1f | Align:%.2f° | Strain:%.2f"),
            *Constraint.GetChildBone().ToString(), Metrics.AnchorDistance,
            Metrics.LeverArm, FMath::RadiansToDegrees(Metrics.AlignmentError),
...
struct FOHNameMatchResult {
  ...
  FOHNameMatchResult(const FString &InCandidate, float InScore,
...
struct ONLYHANDS_API FBlendOperation {
  ...
  /** When blending to simulated, apply transform propagation downward to child
   * bones. */
  UPROPERTY()
  bool bPropagateAttachedRoleToChildren = false;

  /** Optional curve to control how force strength blends over time
  ...
  static FBlendOperation CreateFadeIn(EOHSkeletalBone Bone, float Duration,
                                      float CurrentWeight,
                                      FName ProfileNameOverride = NAME_None) {
    FBlendOperation Op;
    Op.Bone = Bone;
    ...
    Op.BlendOperationId =
        HashCombine(GetTypeHash(Bone), GetTypeHash(FPlatformTime::Seconds()));

    return Op;
  }

  // Static helper to create a fade-out blend operation
  static FBlendOperation CreateFadeOut(EOHSkeletalBone Bone, float Duration,
                                       float CurrentWeight,
                                       FName ProfileNameOverride = NAME_None) {
    FBlendOperation Op;
    Op.Bone = Bone;
  ...
  float CalculateCurrentWeight(double CurrentTime) const {
    ...
    const float Alpha =
        FMath::Clamp(static_cast<float>(ElapsedTime / Duration), 0.0f, 1.0f);
  ...
  float CalculateStrengthMultiplier(double CurrentTime) const {
    ...
    // No override or curve, return -1 to indicate default strength should be
    // used
    return -1.0f;
  }
...
struct FOHBoneTrail {
  ...
  const TArray<FVector> &GetTrail() const { return PositionHistory; }
...
struct FOHPIDControllerm {
  ...
  FVector Compute(const FVector &Target, const FVector &Current,
                  float DeltaTime);
};
#pragma endregion
