...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnRootMotionModeChangedSignature, ERootMotionMode::Type, PreviousMode,
                                             ERootMotionMode::Type, CurrentMode);
...
class ONLYHANDS_API UOHAnimInstance_Base : public UAnimInstance {
  ...
  public:
    ...
    FORCEINLINE ERootMotionMode::Type GetRootMotionModePublic() const {
        return RootMotionMode;
    }

    UFUNCTION(BlueprintPure, Category = "RootMotion")
    FORCEINLINE ERootMotionMode::Type GetLastRootMotionModePublic() const {
        return LastRootMotionMode;
    }
  ...
  private:
    ...
    static bool CapsuleGroundTrace(const USkeletalMeshComponent* Mesh, const FName& BoneName, float MaxDistance,
                                   float ForwardBias, float CapsuleRadius, float CapsuleHalfHeight,
                                   const FVector& Velocity, FHitResult& OutHit, bool bDrawDebug = false);

    // --- Sphere-based ground trace (simpler, less edge artifacts for small feet/shoes)
    static bool SphereGroundTrace(const USkeletalMeshComponent* Mesh, const FName& BoneName, float MaxDistance,
                                  float ForwardBias, float SphereRadius, const FVector& Velocity, FHitResult& OutHit,
                                  bool bDrawDebug = false);

    // --- Dual-trace with backup (avoids foot "dangling" on steps/gaps)
    static bool SafeDoubleTrace(const USkeletalMeshComponent* Mesh, FName Bone, float MaxDist, float Bias, float Radius,
                                const FVector& Velocity, FHitResult& OutMain, FHitResult& OutBackup,
                                float BackupDepth = 8.f);
    ...
    static float FindHighestNearbyFloorZ(const FVector& Center, UWorld* World, float Radius = 15.f,
