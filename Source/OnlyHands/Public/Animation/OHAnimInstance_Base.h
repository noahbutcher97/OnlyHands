...
struct FFootTraceBuffer {
  ...
  FVector GetVelocity() const {
    ...
    return (Dt > KINDA_SMALL_NUMBER) ? (Positions[Prev] - Positions[Old]) / Dt
                                     : FVector::ZeroVector;
  }
...
struct FFootPhaseSample {
  ...
  FFootPhaseSample() {}
  FFootPhaseSample(EFootPhase InPhase, float InTime, FVector InPos,
                   float InSpeed)
...
struct FFootPhaseBuffer {
  ...
  void Add(EFootPhase Phase, float Time,
           const FVector &Pos = FVector::ZeroVector, float Speed = 0.f) {
  ...
  EFootPhase PredictNextPhase() const {
    ...
    if (Last == EFootPhase::Swing && Prev == EFootPhase::Lift)
      return EFootPhase::Plant;
    if (Last == EFootPhase::Lift && Prev == EFootPhase::Plant)
      return EFootPhase::Swing;
    // If only two, oscillate Plant <-> Swing
    if ((Last == EFootPhase::Plant && Prev == EFootPhase::Swing) ||
        (Last == EFootPhase::Swing && Prev == EFootPhase::Plant))
      return (Last == EFootPhase::Plant) ? EFootPhase::Swing
                                         : EFootPhase::Plant;
    // Default
    return Latest().Phase;
  }
  ...
  float GetPhaseConfidence() const {
    ...
    for (int32 i = 1; i < Valid; ++i) {
      int32 CheckIdx = (Index - 1 - i + BufferSize) % BufferSize;
      if (Buffer[CheckIdx].Phase == LatestPhase)
        StableFrames++;
      else
        break;
    }
  ...
  void Reset() {
    for (int32 i = 0; i < BufferSize; ++i)
      Buffer[i] =
          FFootPhaseSample(EFootPhase::Plant, 0.f, FVector::ZeroVector, 0.f);
    Index = 0;
    Valid = 0;
  }
};
...
public:
  ...
  ERootMotionMode::Type GetRootMotionModePublic() const {
  ...
  // --- Offsets for floor mesh (per foot, easily tweaked if shoes or mesh
  // change)
  ...
  FVector LeftFootIKEffector = FVector::ZeroVector;
  UPROPERTY(BlueprintReadOnly, Category = "IK")
  FVector RightFootIKEffector = FVector::ZeroVector;
  UPROPERTY(BlueprintReadOnly, Category = "IK")
  ...
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float LeftFootSlopeAngle = 0.f;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float RightFootSlopeAngle = 0.f;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float LeftFootStrideConfidence = 1.f;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float RightFootStrideConfidence = 1.f;
  // --- Stride phase state ---
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug") int32 LeftStridePhase = 0;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  int32 RightStridePhase = 0;
  // --- Stride ghost step lookahead time (for visualization/tuning) ---
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float LeftGhostLookaheadTime = 0.0f;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  float RightGhostLookaheadTime = 0.0f;
  // --- World effector debug values (component/local space) ---
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  FVector NewLockedLEffector;
  UPROPERTY(BlueprintReadOnly, Category = "IK|Debug")
  FVector NewLockedREffector;
...
private:
  ...
  int32 LFootZBufIndex = 0, RFootZBufIndex = 0, LFootZBufValidSamples = 0,
        RFootZBufValidSamples = 0;
  ...
  // --- HELPERS (cpp)
  // --- Capsule-based predictive trace under a bone (for dynamic dodge/slide,
  // most robust for high speed)
  static bool CapsuleGroundTrace(const USkeletalMeshComponent *Mesh,
                                 const FName &BoneName, float MaxDistance,
                                 float ForwardBias, float CapsuleRadius,
                                 float CapsuleHalfHeight,
                                 const FVector &Velocity, FHitResult &OutHit,
                                 bool bDrawDebug = false);

  // --- Sphere-based ground trace (simpler, less edge artifacts for small
  // feet/shoes)
  static bool SphereGroundTrace(const USkeletalMeshComponent *Mesh,
                                const FName &BoneName, float MaxDistance,
                                float ForwardBias, float SphereRadius,
                                const FVector &Velocity, FHitResult &OutHit,
                                bool bDrawDebug = false);

  // --- Dual-trace with backup (avoids foot "dangling" on steps/gaps)
  static bool SafeDoubleTrace(const USkeletalMeshComponent *Mesh, FName Bone,
                              float MaxDist, float Bias, float Radius,
                              const FVector &Velocity, FHitResult &OutMain,
                              FHitResult &OutBackup, float BackupDepth = 8.f);

  // --- Median for temporal smoothing (robust outlier rejection)
  static float Median(const float *Arr, int32 Count);

  // --- Simple Catmull-Rom for 3-point splines (anti-snapping for foot
  // effector)
  static FVector CatmullRomInterp(const TArray<FVector> &P, float T);

  // --- Find highest valid floor nearby (8 directions, for
  // floor-awareness/blending)
  static float FindHighestNearbyFloorZ(const FVector &Center, UWorld *World,
                                       float Radius = 15.f,
                                       float ProbeDepth = 12.f);
  ...
  static float GetHeelToeBlendAlpha(EFootPhase Phase,
                                    const FFootTraceBuffer &Buffer);

  // --- Critically-damped spring smoothing (jitterless!)
  static FVector CriticallyDampedSmoothing(const FVector &Current,
                                           const FVector &Target,
                                           float DeltaTime, float Spring = 80.f,
                                           float Damping = 1.2f);
};