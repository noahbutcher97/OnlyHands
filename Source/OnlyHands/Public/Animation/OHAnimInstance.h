 ...
 * Integrates with UOHPhysicsComponent if present. Exposes control for runtime IK shaping.
 */
UCLASS()
...
public:
	...
	/** Drive effector target from a given bone's space (e.g. for missed punches or fallback) */
	UFUNCTION(BlueprintCallable, Category = "Arm Compression IK")
	...
	UFUNCTION(BlueprintCallable, Category = "Arm Compression IK")
	void SetEffectorByContext(const FVector& WorldTarget, float Force = 1.0f, bool bForceOverride = false);

	/** Poll physics component and enable compression if high-impulse hit occurred */
	//UFUNCTION(BlueprintCallable, Category = "Arm Compression IK")
	//void EnableCompressionFromPhysicsIfCollisionDetected(float ForceThreshold = 300.f);
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "OnlyHands|IK", meta = (PinShownByDefault))
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AnimGraph", meta = (AllowPrivateAccess = "true"))
	...
	UFUNCTION(BlueprintPure, Category = "Arm Compression IK")
	const FAnimNode_ArmCompressionResponse& GetCompressionNode() const { return ArmCompressionNode; }
