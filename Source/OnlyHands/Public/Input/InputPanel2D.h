...
UENUM(BlueprintType)
enum EInputType { None, Tap, ShortSwipe, LongSwipe };
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnStartSwipe, ESwipeDirection,
                                               SwipeDirection, EInputType,
                                               InputType, FVector2D,
                                               SwipeVelocity);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnDuringSwipe, ESwipeDirection,
                                               SwipeDirection, EInputType,
                                               InputType, FVector2D,
                                               SwipeVelocity);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnCommitSwipe, ESwipeDirection,
                                             SwipeDirection, EInputType,
                                             InputType);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnCancelSwipe);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnTap, ESwipeDirection,
                                            SwipeDirection);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnTouch, ESwipeDirection,
                                            SwipeDirection);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnMoveTouch, FVector2D, Output);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnRawInputVector, FVector2D,
                                            RawInputVector);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnInputStateChange, EInputState,
                                            InputState);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSwipeVelocity, EInputState,
                                            InputState);
...
public:
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Helper gizmos for swipes. "))
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "If >= 0: sends tap event after this many frames "
                              "while touch is held."))
  int AutoTriggerTapFrames = -1;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Number of frames delay for start hold. Also max "
                              "number of frames for tap."))
  int HoldFrames = 3;

  UPROPERTY(
      EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
      meta = (ToolTip = "Min swipe velocity to start registering swipes. "))
  float MinSwipeVelocity = 1000.f;

  UPROPERTY(
      EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
      meta = (ToolTip = "Min swipe distance to start registering swipes. "))
  float DeadZone = 50.f;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Swipe distance needed to trigger short swipe."))
  float ShortSwipeDistance = 150.f;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Swipe distance needed to trigger long swipe."))
  ...
  UPROPERTY(
      EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
      meta = (ToolTip = "Change input mode for diagonal swipes and taps. "))
  bool bUseDiagonalSwipeInputs = false;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Joystick bounds."))
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "0.0-1.0, how hard it is to draw curve, closer "
                              "to 1 means easier"))
  float CurveTolerance = 0.8f;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "How sharp the curve must be"))
  int CurveRankThreshold = 3;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Input Parameters",
            meta = (ToolTip = "Touch move velocity to trigger nudge."))
  float MinNudgeVelocity = 200.f;

  UPROPERTY(EditAnywhere, Category = "Axis Events",
            meta = (ToolTip = "Joystick horizontal axis."))
  FKey JoystickAxisX;
  UPROPERTY(EditAnywhere, Category = "Axis Events",
            meta = (ToolTip = "Joystick vertical axis."))
  FKey JoystickAxisY;
