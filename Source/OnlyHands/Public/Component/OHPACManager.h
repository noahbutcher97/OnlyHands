...
SafeLog(FString::Printf(TEXT("Invalid or stale BodyInstance for %s at %s:%d"), \
*BoneName.ToString(), TEXT(__FILE__), __LINE__), true); \
RefreshBodyInstances(); \
BodyPtr = GetBodyInstanceDirect(BoneName); \
if (!BodyPtr || !BodyPtr->IsValidBodyInstance()) { \
SafeLog(FString::Printf(TEXT("BodyInstance unrecoverable for %s at %s:%d"), \
*BoneName.ToString(), TEXT(__FILE__), __LINE__), true); \
return; \
} \
}
...
struct FOHConstraintData
{
    ...
    FORCEINLINE bool IsConstraintBroken(const USkeletalMeshComponent* SkelMesh) const
    {
        ...
        if (!GetConstraintInstance(SkelMesh)){return false;}
...
struct FOHPhysicsBlendParams
{
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Advanced", 
              meta = (DisplayName = "Use Native Force Propagation", 
                      ToolTip = "When affecting chains, use Unreal's built-in force falloff calculation (recommended for stability)"))
...
struct FOHActiveBlend
{
    ...
    { 
        ...
        return bForceComplete || 
               CurrentPhase == EOHBlendPhase::Inactive ||
    ...
    {
        if (PhysicsBlendWeight_Target <= 0.0f) return 0.0f;
...
protected:
    ...
    void EnableBonePhysicsSimple(
        FName BoneName,
        EOHPhysicsStrength Strength = EOHPhysicsStrength::Medium,
        bool bIncludeChain = false
    );
    
    UFUNCTION(BlueprintCallable, Category = "PAC|Quick Actions",
              meta = (DisplayName = "Disable Bone Physics Simple"))
    void DisableBonePhysicsSimple(
        FName BoneName,
        bool bIncludeChain = false
    );

    UFUNCTION()
    float CalculateCombinedBlendWeight(const TArray<FOHActiveBlend>& Blends, FPhysicalAnimationData& OutCombinedProfile);
    ...
    void ApplyBlendToBody(FName BoneName, const TArray<FOHActiveBlend>& Blends, float CombinedWeight, const FPhysicalAnimationData&
                          CombinedProfile);
...
public:
    ...
    FVector GetBoneVelocity(FName BoneName, EOHReferenceSpace Space = EOHReferenceSpace::WorldSpace, float SmoothingAlpha = 0.0f) const;

    UFUNCTION(BlueprintPure, Category = "PAC|Motion")
    FVector GetBoneAcceleration(FName BoneName, EOHReferenceSpace Space = EOHReferenceSpace::WorldSpace, float SmoothingAlpha = 0.0f) const;
...
private:
    ...
    FVector2D CalculateMovementFromImpact(const FHitResult& Hit, const FVector& RelativeVelocity, const FOHBoneMotionData* MotionData, float DragCoeff, float
                                          CrossSection, float Mass) const;
...
public:
    ...
    FVector2D GetAccumulatedMovement() const { return AccumulatedMovementVector; }

    UFUNCTION(BlueprintCallable, Category = "PAC|Collision")
    void ClearAccumulatedMovement() { AccumulatedMovementVector = FVector2D::ZeroVector; }
    // Runtime State
...
protected:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PAC|Movement Pushback", meta = (ClampMin = "0.0", ClampMax = "3.0"))
    float CoreHitPushbackMultiplier = 1.5f;
    
    /** Multiplier for blocked/extremity hits */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PAC|Movement Pushback", meta = (ClampMin = "0.0", ClampMax = "2.0"))
    float ExtremityHitPushbackMultiplier = 0.8f;
    
    /** Minimum pushback force to ensure visible effect */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PAC|Movement Pushback", meta = (ClampMin = "0.0", ClampMax = "2000.0"))
    
    float MinimumPushbackForce = 800.0f;
private:
    // Core bone definitions
    TSet<FName> CoreBones = {
...
private:
    ...
    bool ProcessChainHit(const FOHCombatChainData& AttackingChain, UOHPACManager* DefenderManager, const FCombatTargetCandidate& Target);
...
public:
    ...
    const TMap<FName, FOHCombatChainData>& GetCombatChains() const { return CombatChains; }
