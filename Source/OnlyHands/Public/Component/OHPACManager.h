...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
    FOnHitReactionStarted, 
    FName, RootBone, 
    const TArray<FName>&, AffectedBones, 
    FName, ReactionTag
);
...
public:
    ...
    FORCEINLINE FVector GetPosition() const { return CurrentPosition; }
    FORCEINLINE FVector GetPreviousPosition() const { return PreviousPosition; }
    FORCEINLINE FQuat GetRotation() const { return CurrentRotation; }
    ...
    FORCEINLINE bool IsSimulating() const { return bIsSimulating; }
    FORCEINLINE const TArray<FOHMotionSample>& GetHistory() const { return MotionHistory; }
    FORCEINLINE bool IsValid() const { return !BoneName.IsNone(); }

    // === MUTATORS ===
    FORCEINLINE void SetBoneName(FName InName) { BoneName = InName; }
    FORCEINLINE void SetPosition(const FVector& Pos) { CurrentPosition = Pos; }
    FORCEINLINE void SetPreviousPosition(const FVector& Pos) { PreviousPosition = Pos; }
    FORCEINLINE void SetRotation(const FQuat& Rot) { CurrentRotation = Rot; }
    FORCEINLINE void SetPreviousRotation(const FQuat& Rot) { PreviousRotation = Rot; }
    FORCEINLINE void SetLinearVelocity(const FVector& Vel) { LinearVelocity = Vel; }
    FORCEINLINE void SetAngularVelocity(const FVector& AngVel) { AngularVelocity = AngVel; }
    FORCEINLINE void SetLinearAcceleration(const FVector& Accel) { LinearAcceleration = Accel; }
    FORCEINLINE void SetAngularAcceleration(const FVector& AngAccel) { AngularAcceleration = AngAccel; }
    FORCEINLINE void SetIsSimulating(bool bSim) { bIsSimulating = bSim; }
    FORCEINLINE void SetLastDeltaTime(float DT) { LastDeltaTime = FMath::Max(DT, 0.001f); }
...
public:
    ...
    FORCEINLINE float GetCurrentStrain() const { return CurrentStrain; }
    FORCEINLINE float GetJitterMetric() const { return JitterMetric; }
    FORCEINLINE bool IsValid() const { return !ConstraintName.IsNone() && ConstraintInstance != nullptr; }
    ...
    FORCEINLINE void SetConstraintInstance(FConstraintInstance* Instance) { ConstraintInstance = Instance; }
...
struct ONLYHANDS_API FOHBlendState
{
    ...
    FORCEINLINE bool IsInherited() const { return bInheritedFromPrevious; }
    FORCEINLINE float GetBlendRange() const { return TargetAlpha - StartAlpha; }
    FORCEINLINE bool IsPaused() const { return PauseCount > 0; }
    FORCEINLINE bool IsComplete() const { return Phase == EOHBlendPhase::BlendOut && ElapsedTime >= BlendOutDuration; }
    FORCEINLINE float GetTotalDuration() const { return BlendInDuration + HoldDuration + BlendOutDuration; }
};
#pragma endregion
...
#pragma region CONFIGURATION
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Motion Tracking")
    TSet<FName> SimulationExclusions = {
        "root", "pelvis", "neck_01"
    };
...
#pragma region PUBLIC API
    ...
    void PlayHitReaction(
        FName BoneName,
        EOHPhysicsProfile Profile = EOHPhysicsProfile::Medium,
        float BlendIn = 0.15f,
        float Hold = 0.25f,
        float BlendOut = 0.2f,
    ...
    bool StartBonePhysicalAnimation(
    FName BoneName, 
    ...
    UFUNCTION(BlueprintCallable, Category="PAC Manager|Physics")
    bool StartChainPhysicalAnimation(
    FName RootBone, 
    const FPhysicalAnimationData& Profile, 
    ...
    bool StartChainPhysicalAnimation_Filtered(FName RootBone, const FPhysicalAnimationData& Profile, bool bUseNativePropagation, bool bEnableCollision, TFunctionRef<
                                              bool(FName)> BoneFilter, bool bWakeBody, bool bVerboseLog);

    UFUNCTION(BlueprintCallable, Category="PAC Manager|Physics")
    void StopChainPhysicalAnimation(FName RootBone, bool bUseNativePropagation = true);

    void StopChainPhysicalAnimation_Filtered(FName RootBone, bool bUseNativePropagation, TFunctionRef<bool(FName)> BoneFilter, bool bClearVelocities, bool
                                             bPutToSleep, bool bVerboseLog);
    ...
    UFUNCTION(BlueprintPure, Category = "PAC Manager|Utility")
    TArray<FName> GetSimulatableBones() const { return SimulatableBones.Array(); }
    ...
    static void GetSimulatingBodiesBySimulatable(
...
#pragma region INTERNAL METHODS
    ...
    // === SIMULATION CONTROL ===
    bool StartSimulation(FName BoneName, const FPhysicalAnimationData& Profile, bool bAllBelow = false, bool bEnableCollision = true);
