...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOHOnHitReactionBonesUpdated, FName, RootBone, const TArray<FName>&, AffectedBones);
...
#pragma region Initialization
	...
	/** Rebuilds SimulatableBones by filtering TrackedBoneDefinitions against the skeletal mesh’s physics asset and exclusion rules */
...
#pragma region Motion Tracking
	...
	FORCEINLINE bool IsTrackedBone(FName Bone) const { return TrackedBoneDefinitions.Contains(Bone); }

	UFUNCTION(BlueprintPure, Category = "Physics|Simulation")
	FORCEINLINE bool IsSimulatableBone(FName Bone) const { return SimulatableBones.Contains(Bone); }

	UFUNCTION(BlueprintPure, Category = "OH|Physics|Simulation")
	FORCEINLINE TArray<FName> GetSimulatableBoneNames() const{return SimulatableBones.Array();}
...
#pragma endregion
	...
	void ComputeOptimalConstraintSettings(const FConstraintInstance* ConstraintInstance, FConstraintProfileProperties& OutProfile, const FName& BoneName) const;
...
#pragma region SimulationLifecycle
	...
	UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Simulation")
	void ApplyUnifiedPhysicsProfile(FName BoneName, const FPhysicalAnimationData& Profile, float CustomLinearDamping = 5.0f, float CustomAngularDamping = 10.0f, bool bVerboseLog = false);

	/** Recursively applies a unified physical animation profile to a bone and all of its children */
	UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Simulation")
	void ApplyUnifiedPhysicsProfileToChain(FName RootBone, const FPhysicalAnimationData& Profile, float CustomLinearDamping = 5.0f, float CustomAngularDamping = 10.0f, bool bVerboseLog = false);
...
#pragma region BlendState
	...
	static FActivePhysicsBlend CreatePhysicsBlendState(FName RootBone, float StartAlpha, float BlendIn, float Hold, float BlendOut, FName ReactionTag = NAME_None);
...
#pragma region Utilities
	...
	FPhysicalAnimationData GetPhysicalAnimationData(
		FName BoneName,
		EDriveAccessMode AccessMode = EDriveAccessMode::Live
	) const;
	UFUNCTION(BlueprintCallable)
...
#pragma region SkeletalAssetValidation
	...
	static TArray<FName> BP_GetChildrenOfBone(
	...
	UFUNCTION(BlueprintCallable, Category="PhysicsGraph")
	static TArray<FOHConstraintInstanceData> BP_GetConstraintsOfBone(
		const FOHPhysicsGraphNode& Graph,
		FName BoneName
	)
	{
