...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOHHitReactionComplete, FName,
                                            BoneName);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOHOnHitReactionBonesUpdated,
                                             FName, RootBone,
                                             const TArray<FName> &,
                                             AffectedBones);
...
public:
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OnlyHands|Physics|Simulation")
...
#pragma region Initialization
  ...
  /** Rebuilds SimulatableBones by filtering TrackedBoneDefinitions against the
   * skeletal mesh’s physics asset and exclusion rules */
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Simulation")
  void
  InitializeAllPhysicalAnimationProfiles(const FPhysicalAnimationData &Profile);
...
#pragma region Motion Tracking
  ...
  /** Bones excluded from simulation even if they are tracked and present in the
   * physics asset */
  UPROPERTY(EditDefaultsOnly, Category = "OnlyHands|Physics|MotionTracking")
  ...
  /** Bones allowed to be simulated, based on current physics asset and
   * exclusions */
...
#pragma region BoneDataAccessors
  ...
  FORCEINLINE FVector GetAngularVelocity(FName Bone) const {
    const FOHBoneData *Data = GetBoneData(Bone);
    return Data && Data->IsValid() ? Data->GetBodyAngularVelocity()
                                   : FVector::ZeroVector;
  }
  ...
  FORCEINLINE FQuat GetRotation(FName Bone) const {
    const FOHBoneData *Data = GetBoneData(Bone);
    return Data && Data->IsValid() ? Data->GetCurrentRotation()
                                   : FQuat::Identity;
  }
#pragma endregion
#pragma endregion
...
#pragma region HitReaction
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics")
  void PlayPhysicsHitReaction(FName BoneName, EOHPhysicsProfile Profile,
                              float BlendIn = 0.15f, float Hold = 0.25f,
                              float BlendOut = 0.2f,
                              FVector OptionalImpulseDir = FVector::ZeroVector,
                              float OptionalImpulseStrength = 0.f,
                              FName ReactionTag = NAME_None);

  /** Plays a custom PAC-driven hit reaction using a fully defined PAC profile
   */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics")
  void PlayPhysicsHitReactionCustom(
      FName BoneName, const FPhysicalAnimationData &CustomProfile,
  ...
  void PlayPhysicsHitReaction_Internal(
  ...
  void ApplySimSettingsToBone(const FName &Bone,
                              const FPhysicalAnimationData &BaseProfile);
...
protected:
  ...
  // Direct pointer comparison maps (for testing) - using weak pointers to avoid
  // GC issues
  mutable TMap<FName, TWeakObjectPtr<UBodySetup>> DirectBodySetupMap;
  mutable TMap<FName, TWeakObjectPtr<UPhysicsConstraintTemplate>>
      DirectConstraintTemplateMap;
  mutable TMap<FName, FConstraintInstance *>
      DirectConstraintInstanceMap; // Runtime instances
  mutable TMap<FName, int32> DirectBoneIndexMap;
  mutable TMap<FName, FBodyInstance *>
      DirectBodyInstanceMap; // Safe - these are components, not UObjects
...
#pragma endregion
  ...
  /** Show debug overlays for tracked bone kinematics (velocity, acceleration,
   * angular velocity) */
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OnlyHands|Debug")
  ...
  void ComputeOptimalConstraintSettings(
  ...
  void ComputePhysicsTweaksForBone(const FName &Bone, float &OutPACMultiplier,
  ...
  GetScaledPACProfile(const FName &Bone,
  ...
  UFUNCTION(CallInEditor, Category = "OnlyHands|Debug",
            meta = (CallInEditor = "true",
                    DisplayName = "Validate Skeleton Compatibility"))
...
#pragma region SimulationLifecycle
  ...
  void ActivateSimulationForChain(FName RootBone,
                                  const FPhysicalAnimationData &ProfileData,
                                  float Alpha);

  /** Increments sim ref count and activates simulation if this is the first
   * activation */
  ...
  /** Applies a unified physical animation profile to a single bone (PAC +
   * constraint + body tuning) */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Simulation")
  void ApplyUnifiedPhysicsProfile(FName BoneName,
                                  const FPhysicalAnimationData &Profile,
                                  float CustomLinearDamping = 5.0f,
                                  float CustomAngularDamping = 10.0f,
                                  bool bVerboseLog = false);

  /** Recursively applies a unified physical animation profile to a bone and all
   * of its children */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics|Simulation")
  void ApplyUnifiedPhysicsProfileToChain(FName RootBone,
                                         const FPhysicalAnimationData &Profile,
                                         float CustomLinearDamping = 5.0f,
                                         float CustomAngularDamping = 10.0f,
                                         bool bVerboseLog = false);
...
#pragma region BlendState
  ...
  static FActivePhysicsBlend
  CreatePhysicsBlendState(FName RootBone, float StartAlpha, float BlendIn,
                          float Hold, float BlendOut,
                          FName ReactionTag = NAME_None);
  ...
  /** Finalizes all active physics blends and clears associated simulation and
   * PAC */
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics")
...
#pragma region Impulse
  ...
  UFUNCTION(BlueprintCallable, Category = "OnlyHands|Physics")
  void PropagateImpulse(FName OriginBone, FVector Impulse, float Strength,
                        EOHPhysicsProfile Profile, int32 Depth);
...
#pragma region Utilities
  ...
  FPhysicalAnimationData GetPhysicalAnimationData(
      FName BoneName,
      EDriveAccessMode AccessMode = EDriveAccessMode::Live) const;
  UFUNCTION(BlueprintCallable)
  void SetPhysicalAnimationData(const FName BoneName,
                                const FPhysicalAnimationData &Data);

  /** Compare two FPhysicalAnimationData structs (debug only) */
  UFUNCTION(BlueprintPure, Category = "OnlyHands|Debug")
  static bool ArePACProfilesEqual(const FPhysicalAnimationData &A,
...
#pragma region SkeletalAssetValidation
  ...
  static TArray<FName> BP_GetChildrenOfBone(const FOHPhysicsGraphNode &Graph,
                                            FName ParentBone) {
    return Graph.GetChildrenOfBone(ParentBone);
  }

  UFUNCTION(BlueprintCallable, Category = "PhysicsGraph")
  static TArray<FOHConstraintInstanceData>
  BP_GetConstraintsOfBone(const FOHPhysicsGraphNode &Graph, FName BoneName) {
...
public:
  ...
  const FOHPhysicsGraphNode &GetPhysicsGraph() const { return PhysicsGraph; }
...
protected:
  ...
  static bool
  IsConstraintInstanceValid(const FConstraintInstance *ConstraintInstance);
...
private:
  ...
  mutable uint64 PhysicsGraphVersion =
      0; // Monotonically increasing version counter
  mutable uint64 LastGraphBuildVersion =
      0; // Version at last successful graph build
  ...
  UPROPERTY(EditAnywhere, Category = "OnlyHands|Physics|Hierarchy",
