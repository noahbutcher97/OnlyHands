...
struct FOHMovementHistoryFrame
{
	...
	static FOHMovementHistoryFrame Create(
		const FTransform& CurrentTransform,
		const FVector2D& CurrentInput,
	...
	static const FOHMovementHistoryFrame* UpdateBuffer(
		BufferType& Buffer,
		const FTransform& CurrentTransform,
		const FVector2D& CurrentInput,
		const FVector& CurrentVelocity,
	...
	)
	{
		...
		FOHMovementHistoryFrame Frame = Create(
			CurrentTransform,
			CurrentInput,
			CurrentVelocity,
			DeltaTime,
	...
	template<typename BufferType>
	static void ClearBuffer(BufferType& Buffer)
	{
		Buffer.Num = 0;
		Buffer.Start = 0;
	}
	...
	template<typename BufferType>
	static void RemoveFrame(BufferType& Buffer, int32 N = 0)
	{
		if (N >= Buffer.Num) return;
		int32 RemoveIndex = (Buffer.Start + Buffer.Num - 1 - N + Buffer.Capacity) % Buffer.Capacity;
		// Move all newer frames down
		for (int32 i = RemoveIndex; i != (Buffer.Start + Buffer.Num - 1) % Buffer.Capacity; i = (i + 1) % Buffer.Capacity)
		{
	...
	FVector GetForward()    const { return Transform.GetRotation().GetForwardVector(); }
	FVector GetRight()      const { return Transform.GetRotation().GetRightVector();   }
	FVector GetUp()         const { return Transform.GetRotation().GetUpVector();      }

	float GetSpeed()        const { return Velocity.Size(); }
	float GetAccelerationMagnitude() const { return Acceleration.Size(); }
	float GetJerkMagnitude()         const { return Jerk.Size(); }
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetFrameClosestToTime(
		const BufferType& Buffer, float SecondsAgo)
	{
		float Accum = 0.f;
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastFrameWithForwardInput(
		const BufferType& Buffer, float Threshold = 0.1f)
	{
		for (int32 i = 0; i < Buffer.NumFrames(); ++i)
		{
			if (const FOHMovementHistoryFrame* F = Buffer.GetLatest(i))
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastFrameWithLateralInput(
		const BufferType& Buffer, float Threshold = 0.1f)
	{
		for (int32 i = 0; i < Buffer.NumFrames(); ++i)
		{
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastFrameWithInputMovementMismatch(
		const BufferType& Buffer, float AngleThresholdDeg = 60.f)
	{
		for (int32 i = 0; i < Buffer.NumFrames(); ++i)
		{
			if (const FOHMovementHistoryFrame* F = Buffer.GetLatest(i))
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastDirectionChange(
		const BufferType& Buffer, float AngleThresholdDeg = 45.f)
	{
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastFrameWhereStopped(const BufferType& Buffer, float SpeedThreshold = 5.f)
	{
		for (int32 i = 0; i < Buffer.NumFrames(); ++i)
		{
			if (const FOHMovementHistoryFrame* F = Buffer.GetLatest(i))
	...
	template<typename BufferType>
	static const FOHMovementHistoryFrame* GetLastFrameWithFacingDirection(const BufferType& Buffer, const FRotator& TargetRot, float ToleranceDeg = 10.f)
	{
		for (int32 i = 0; i < Buffer.NumFrames(); ++i)
		{
			if (const FOHMovementHistoryFrame* F = Buffer.GetLatest(i))
	...
	template<typename BufferType>
static const FOHMovementHistoryFrame* GetFirstFrameAfterStopWithInput(
	const BufferType& Buffer, 
	float StopSpeedThreshold = 5.f, 
	float InputThreshold = 0.1f, 
	int32 MaxFramesBack = 10)
	{
	...
	template<typename BufferType>
	static float GetAverageSpeed(const BufferType& Buffer, int32 Frames)
	{
		float Sum = 0.f; int32 Count = 0;
	...
	template<typename BufferType>
	static FVector GetAverageVelocity(const BufferType& Buffer, int32 Frames)
	{
	...
	// Get average acceleration over last N frames
	template<typename BufferType>
	static FVector GetAverageAcceleration(const BufferType& Buffer, int32 Frames)
	{
		FVector Sum = FVector::ZeroVector;
		int32 Count = 0;
		for (int32 i = 0; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
	...
	template<typename BufferType>
	static float GetMaxJerkMagnitude(const BufferType& Buffer, int32 Frames)
	{
		float Max = 0.f;
		for (int32 i = 0; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
	...
	template<typename BufferType>
	static float GetAverageTurnRate(const BufferType& Buffer, int32 Frames)
	{
		float Sum = 0.f; int32 Count = 0;
		for (int32 i = 1; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
			...
			{
				float DeltaYaw = FMath::Abs(FMath::FindDeltaAngleDegrees(Curr->GetRotation().Yaw, Prev->GetRotation().Yaw));
	...
	template<typename BufferType>
	static float GetAverageInputAlignment(const BufferType& Buffer, int32 Frames)
	{
		float Sum = 0.f; int32 Count = 0;
		for (int32 i = 0; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
	...
	template<typename BufferType>
	static float GetRollingCurvature(const BufferType& Buffer, int32 Frames)
	{
		float Sum = 0.f; int32 Count = 0;
		for (int32 i = 1; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
	...
	template<typename BufferType>
	static float GetPeakAcceleration(const BufferType& Buffer, int32 Frames)
	{
		float Max = 0.f;
		for (int32 i = 0; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
	...
	template<typename BufferType>
	static float GetDistanceTraveled(const BufferType& Buffer, int32 Frames)
	{
		float Dist = 0.f;
		FVector PrevPos;
		bool bFirst = true;
	...
	template<typename BufferType>
static bool WasMovingTowardDirection(
	const BufferType& Buffer, 
	const FVector& WorldDirection, 
	float AngleToleranceDeg = 30.f, 
	int32 FramesToCheck = 10)
	{
	...
	template<typename BufferType>
	static FVector PredictNextVelocity(const BufferType& Buffer)
	{
		if (Buffer.NumFrames() < 2) return FVector::ZeroVector;
	...
	template<typename BufferType>
	static bool WasRecentPivot(const BufferType& Buffer, float InputAngleThreshold = 90.f, float VelocityAngleThreshold = 45.f, int32 FramesToCheck = 5)
	{
	...
	template<typename BufferType>
	static int32 FindRecentPivotWithinTime(
		const BufferType& Buffer,
	...
	template<typename BufferType>
static bool DetectMicroStutter(
	const BufferType& Buffer,
	float MinAngle = 10.f,
	int32 MinStutters = 2,
...
static float GetAverageDirectionChangeRateDegPerSec(const BufferType& Buffer, int32 Frames)
	{
		...
		for (int32 i = 1; i < FMath::Min(Frames, Buffer.NumFrames()); ++i)
		{
			...
			if (Curr && Prev)
			{
				...
				{
					float Angle = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(FVector::DotProduct(DirA, DirB), -1.f, 1.f)));
					SumDeg += Angle;
					TotalTime += DeltaT;
				}
			}
	...
	template<typename BufferType>
static float GetDirectionChangeVarianceDeg(const BufferType& Buffer, int32 Frames)
	{
		...
		if (AngleDeltas.Num() == 0) return 0.f;
	...
	template<typename BufferType>
	static float GetDirectionalRampFactor(const BufferType& Buffer, int32 FramesToCheck = 2, float MaxSlowAngle = 120.f)
	{
		if (Buffer.NumFrames() < FramesToCheck) return 1.f;
	...
	template<typename BufferType>
	static float GetAxisWeightedRamp(const BufferType& Buffer, float FwdWeight = 1.0f, float SideWeight = 0.6f, float BackWeight = 0.3f)
	{
		if (Buffer.NumFrames() < 2) return 1.f;
		const FOHMovementHistoryFrame* Curr = Buffer.GetLatest(0);
		const FOHMovementHistoryFrame* Prev = Buffer.GetLatest(1);
		if (Curr && Prev)
		{
			...
			if (!PrevDir.IsNearlyZero() && !CurrInputDir.IsNearlyZero())
			{
				float Angle = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(FVector::DotProduct(PrevDir, CurrInputDir), -1.f, 1.f)));
				// Forward (<45°), Side (45–135°), Back (>135°)
				if (Angle < 45.f)      return FwdWeight;
				else if (Angle < 135.f) return SideWeight;
				else                  return BackWeight;
			}
		}
		return 1.f;
	}
...
struct FOHMovementConfig
{
	...
	FORCEINLINE float GetWalkSpeedThreshold() const { return WalkSpeedThreshold; }
	FORCEINLINE float GetRunSpeedThreshold()  const { return RunSpeedThreshold; }
	FORCEINLINE float GetSprintSpeedThreshold() const { return SprintSpeedThreshold; }
	FORCEINLINE float GetMaxAccelerationWalking() const { return MaxAccelerationWalking; }
	FORCEINLINE float GetMaxAccelerationRunning() const { return MaxAccelerationRunning; }
	FORCEINLINE float GetMaxAccelerationSprinting() const { return MaxAccelerationSprinting; }
	FORCEINLINE float GetBrakingDecelerationWalking() const { return BrakingDecelerationWalking; }
	FORCEINLINE float GetBrakingDecelerationRunning() const { return BrakingDecelerationRunning; }
	FORCEINLINE float GetBrakingDecelerationSprinting() const { return BrakingDecelerationSprinting; }
	FORCEINLINE float GetGroundFrictionWalking() const { return GroundFrictionWalking; }
	FORCEINLINE float GetGroundFrictionRunning() const { return GroundFrictionRunning; }
	FORCEINLINE float GetGroundFrictionSprinting() const { return GroundFrictionSprinting; }
	FORCEINLINE float GetMaxFriction() const { return MaxFriction; }
	FORCEINLINE float GetMinFriction() const { return MinFriction; }
	FORCEINLINE float GetNoInputSpeedDecayRate() const { return NoInputSpeedDecayRate; }
	FORCEINLINE float GetInputSpeedBuildRate() const { return InputSpeedBuildRate; }
	FORCEINLINE float GetSpeedRetentionOnDirectionChange() const { return SpeedRetentionOnDirectionChange; }
	FORCEINLINE float GetStrafingThreshold() const { return StrafingThreshold; }
	FORCEINLINE float GetDirectionChangeSharpness() const { return DirectionChangeSharpness; }
	FORCEINLINE float GetPivotThreshold() const { return PivotThreshold; }
	FORCEINLINE float GetGaitSpeedTransitionRate() const { return GaitSpeedTransitionRate; }
	FORCEINLINE float GetGaitAccelerationMultiplier() const { return GaitAccelerationMultiplier; }
	FORCEINLINE float GetGaitDecelerationMultiplier() const { return GaitDecelerationMultiplier; }
	FORCEINLINE float GetRunHysteresis() const { return RunHysteresis; }
	FORCEINLINE float GetSprintHysteresis() const { return SprintHysteresis; }
	FORCEINLINE float GetInputBufferRiseRate() const { return InputBufferRiseRate; }
	FORCEINLINE float GetInputBufferDecay() const { return InputBufferDecay; }
	FORCEINLINE float GetHoldTimeAboveRunThreshold() const { return HoldTimeAboveRunThreshold; }
	FORCEINLINE float GetHoldTimeAboveSprintThreshold() const { return HoldTimeAboveSprintThreshold; }
	FORCEINLINE float GetGaitInterpSpeed() const { return GaitInterpSpeed; }
	FORCEINLINE float GetBackpedalSpeedMultiplier() const { return BackpedalSpeedMultiplier; }
	FORCEINLINE float GetStrafeSpeedMultiplier() const { return StrafeSpeedMultiplier; }
	FORCEINLINE float GetDiagonalSpeedMultiplier() const { return DiagonalSpeedMultiplier; }
	FORCEINLINE float GetJoystickDeadzone() const { return JoystickDeadzone; }
	FORCEINLINE float GetJoystickWalkZone() const { return JoystickWalkZone; }
	FORCEINLINE float GetJoystickRunZone() const { return JoystickRunZone; }
	FORCEINLINE float GetInputSmoothingBlendSpeed() const { return InputSmoothingBlendSpeed; }
	FORCEINLINE float GetInputIntentMagnitudeThreshold() const { return InputIntentMagnitudeThreshold; }
	FORCEINLINE float GetMomentumBasedMovement() const { return bUseMomentumBasedMovement; }
	FORCEINLINE float GetMomentumBasedMovementWeight() const { return MovementWeight; }
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnPhysicsInputReceived, FVector2D, PhysicsInput, FVector2D, EffectiveMovement);
...
public:
    ...
    UFUNCTION(BlueprintPure, Category = "Stance")
    float GetStrideCadence() const { return StanceState.StrideCadence; }
    
    UFUNCTION(BlueprintPure, Category = "Stance")
    EOHStanceType GetCurrentStance() const { return StanceState.CurrentStance; }
    
    UFUNCTION(BlueprintPure, Category = "Stance")
    bool IsStancePivoting() const { return StanceState.bIsPivoting; }
...
private:
	...
	void DrawDebug_KinematicChainData(const FOHFootStanceData& FootData, bool bIsLeft, USkeletalMeshComponent* Mesh) const;
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movement|Spacing", meta = (ClampMin = "50.0", ClampMax = "300.0"))
...
private:
	 ...
	 * Small values are cleared from the physics input vector. Additionally, it can convert remaining physics input to impulse for unified handling if bUseImpulseBasedPhysics is enabled.
	 *
