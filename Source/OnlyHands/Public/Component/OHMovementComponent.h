...
struct FOHMovementSettings {
  ...
  // Curve vector mapping normalized speed [0..1] to (Acceleration,
  // BrakingDeceleration, GroundFriction)
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movement|Curves")
  UCurveVector *MovementCurve = nullptr;
};

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnMovementInputUpdated, FVector2D,
                                            MovementInput);
...
public:
  ...
  bool bUseInputMagnitudeForSpeed =
      true; // Scale movement speed by input magnitude
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Input")
  float JoystickRunZone =
      0.85f; // Input magnitude below this = run, above = sprint intent

  // === Momentum Settings ===
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config")
  bool bUseMomentumBasedMovement =
      true; // Enable weighted, momentum-based movement

  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Config|InputBuffering")
  float NoInputSpeedDecayRate = 4.0f; // How fast to decay speed when no input

  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Config|InputBuffering")
  float InputSpeedBuildRate =
      2.5f; // How fast to build speed when input detected

  // === Movement Speed Curves ===
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Curves")
  UCurveFloat *DirectionalSpeedCurve; // Curve mapping strafe angle (0-180) to
                                      // speed multiplier

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Curves")
  UCurveFloat *RotationalSpeedCurve; // Curve mapping angular velocity to speed
                                     // multiplier
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Config|Multipliers")
  float BackpedalSpeedMultiplier =
      0.75f; // Speed reduction when moving backward

  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Config|Multipliers")
  float StrafeSpeedMultiplier =
      0.85f; // Speed reduction for pure strafe (90 degrees)
  ...
  float DirectionChangeSharpness =
      3.0f; // How quickly direction changes (lower = heavier)

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Weight")
  float SpeedRetentionOnDirectionChange =
      0.7f; // Speed multiplier when changing direction sharply
  ...
  UPROPERTY(BlueprintReadOnly, Category = "OH|Runtime|State")
  float DirectionalSpeedMultiplier =
      1.0f; // Current direction-based speed modifier

  UPROPERTY(BlueprintReadOnly, Category = "OH|Runtime|State")
  float RotationalSpeedMultiplier =
      1.0f; // Current rotation-based speed modifier

  UPROPERTY(BlueprintReadOnly, Category = "OH|Runtime|State")
  float CurrentMovementAngle =
      0.f; // Angle between movement and facing direction

  UPROPERTY(BlueprintReadOnly, Category = "OH|Runtime|State")
  FVector LastMovementDirection =
      FVector::ZeroVector; // For direction change detection
  ...
  float GaitSpeedTransitionRate =
      2.0f; // How fast to transition between gait speeds

  // Multiplier for Acceleration
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Gait")
  float GaitAccelerationMultiplier =
      1.5f; // Multiply acceleration when transitioning up

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OH|Config|Gait")
  float GaitDecelerationMultiplier =
      0.7f; // Multiply deceleration when transitioning down
  ...
  bool bMovementInitialized =
      false; // Track whether movement has been initialized
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Movement|Gait|Thresholds")
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Movement|Gait|Buffering")
  float
      InputBufferDecay; // How fast buffer decays when input drops (per second)
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "OH|Movement|Gait|Hysteresis")
  ...
  // Public setter to update input vector (to be called from player controller
  // or input system)
  ...
  // Updates character movement component properties dynamically based on
  // allowed gait
  UFUNCTION(BlueprintCallable, Category = "OH|Movement")
  ...
  // Calculates movement play rate (for animation speed scaling) based on speed
  // and input
  UFUNCTION(BlueprintPure, Category = "OH|Movement")
  ...
  // Calculates a custom rotation looking from owner to locked target (implement
  // in cpp)
  UFUNCTION(BlueprintPure, Category = "OH|Movement|Pivot")
  FRotator CalculateRotationToTarget_Internal() const;

  // Calculates the offset rotator between owner rotation and target rotation
  // (implement in cpp)
  UFUNCTION(BlueprintPure, Category = "OH|Movement|Pivot")
  FRotator CalculateTargetOffset_Internal() const;

  // Movement mode changed handler, to be called when CharacterMovement changes
  // movement mode
  UFUNCTION()
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "Movement|Gait|Buffering")
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite,
            Category = "Movement|Gait|Interpolation")
  ...
  // Returns whether the character should perform a pivot based on the pivot
  // threshold
...
private:
  ...
  // Helper to get locked target actor (implement this to suit your locking
  // logic)
