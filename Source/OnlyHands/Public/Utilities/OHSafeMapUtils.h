...

{
	...
	struct TRollingBuffer
	{
		...
		int32 NumFrames() const { return Num; }
		static int32 CapacityFrames() { return Capacity; }
    ...
    static FORCEINLINE ValueType SafeGet(const TMap<KeyType, ValueType>& Map, const KeyType& Key, const ValueType& DefaultValue)
    {
     ...
     */
    template<typename KeyType, typename ValueType>
    static FORCEINLINE const ValueType& SafeGetRef(const TMap<KeyType, ValueType>& Map, const KeyType& Key, const ValueType& DefaultValue)
    {
        const ValueType* FoundValue = Map.Find(Key);
        return FoundValue ? *FoundValue : DefaultValue;
    }
     ...
     */
    template<typename KeyType, typename ValueType>
    static FORCEINLINE ValueType& SafeGetOrAdd(TMap<KeyType, ValueType>& Map, const KeyType& Key, const ValueType& DefaultValue)
    {
	...
	template<typename KeyType, typename ValueType>
	FORCEINLINE const ValueType& GetReferenceChecked(
		...
		const TCHAR* Context = TEXT("GetReferenceChecked"))
	{
		if (!Map.Contains(Key))
		{
			UE_LOG(LogSafeMapUtils, Error, TEXT("[%s] Key missing in map: %s"), Context, *TTypeToString<KeyType>::ToString(Key));
	...
	template<typename KeyType, typename ValueType>
FORCEINLINE ValueType& GetMutableReferenceChecked(
	...
	EOHMutationResult TryApplyRuntimeMutation(
		const TContext& ContextID,
		const TValue& NewValue,
		TField TStruct::* CachedFieldPtr,
		TFunctionRef<bool(TStruct&, const TValue&)> ApplyToLiveInstanceFn,
		TStruct& TargetStruct,
		const FString& DebugLabel = TEXT("UnnamedField"),
		bool bStrict = false,
		int32* OutMutationCounter = nullptr,
	...
	)
	{
		...
		if (!bSame || bStrict)
		{
			if (OutMutationCounter) (*OutMutationCounter)++;
			OnMutation();
		}

		if (bApplied)
		{
			UE_LOG(LogTemp, VeryVerbose, TEXT("Mutation [%s] applied to live instance (%s)."), *DebugLabel, *ContextID.ToString());
...
FORCEINLINE bool AssertMapContainsAllKeys(
	const TMap<K, V>& Map,
	const KeyContainerType& RequiredKeys,
	...
	FORCEINLINE bool EnsureMapIsBijective(const TMap<K, V>& Map, const TCHAR* Context = TEXT("EnsureMapIsBijective"))
	{
		...
		for (const auto& Pair : Map)
		{
			...
			{
				UE_LOG(LogSafeMapUtils, Error, TEXT("[%s] Non-bijective mapping detected for key: %s, value: %s"), Context, *TTypeToString<K>::ToString(Pair.Key), *TTypeToString<V>::ToString(Pair.Value));
				bBijective = false;
			}
			else
			{
	...
	void BuildGraphHierarchyAndConstraintMap(
		const TArray<TEdge>& Edges,
		TMap<TKey, TArrayWrapper>& ParentToChildrenMap,
...
FORCEINLINE float ComputeMapMean(const TMap<K, V>& Map, TFunctionRef<float(const V&)> Extract)
	...
	template <typename K, typename V>
	TMap<int32, TArray<K>> BucketizeMap(
		const TMap<K, V>& Map,
...
FORCEINLINE TMap<K, float> ComputeZScores(const TMap<K, V>& Map, TFunctionRef<float(const V&)> Extract)
	...
	FORCEINLINE TMap<K, V> MergeMapsWithConflictResolution(
		const TMap<K, V>& A,
		const TMap<K, V>& B,
...
FORCEINLINE TMap<K, V> MergeMapsWithConflictResolution(
	...
	FORCEINLINE TArray<T> MakeFilterIterator(
...
FORCEINLINE TArray<T> MakeFilterIterator(
	...
	template <typename K>
	bool AssertNoCycles(const TMap<K, TArray<K>>& Graph, FString Label = TEXT("Graph"))
	{
	...
	template <typename K>
	TSet<K> GetSubtreeFromNode(
		const K& Start,
		const TMap<K, TArray<K>>& Graph)
	{
		TSet<K> Result;
		TQueue<K> Queue;
		Queue.Enqueue(Start);
	...
	struct TPipeMap
	{
		...
		// Generic version
		template <typename VOut, typename Transformer>
		TPipeMap<K, VOut> MapValues(Transformer&& Transform) const
		{
			TMap<K, VOut> Result;
		...
		TPipeMap<K, V> FilterPairs(Predicate&& Pred) const
		{
			TMap<K, V> Result;
			for (const auto& Pair : Data)
			{
		...
		// Generic version
		template <typename OutT, typename ReducerType>
		OutT Reduce(ReducerType&& Reducer, OutT Init) const
		{
			for (const auto& Pair : Data)
			{
		...
		OutT Reduce(TFunctionRef<OutT(const OutT&, const K&, const V&)> Reducer, OutT Init) const
		{
			for (const auto& Pair : Data)
			{
				Init = Reducer(Init, Pair.Key, Pair.Value);
	...
	template <typename K, typename V>
	TPipeMap<K, V> MakeMapPipe(TMap<K, V> Input)
	{
	...
	FORCEINLINE bool ValidateBoneMassRange(
		const TMap<K, V>& BoneMap,
		Getter&& GetMass,
		float Min = 0.01f,
		float Max = 1000.f)
	{
...
FORCEINLINE TArray<TArray<K>> FindChainsMatching(
	const TMap<K, TArray<K>>& Graph,
	Predicate&& IsChainRoot)
	{
		TArray<TArray<K>> Chains;
		TSet<K> Visited;
...
FORCEINLINE TArray<TArray<K>> FindChainsMatching(
	...
	FORCEINLINE TSet<FName> FindUnconstrainedBones(
...
FORCEINLINE TArray<TConstraint> ValidateConstraintMassBalance(
	const TArray<TConstraint>& Constraints,
	const TMap<FName, TBone>& BoneMap,
	GetParentType&& GetParent,
...
FORCEINLINE TArray<TConstraint> DetectUnidirectionalConstraints(
	const TArray<TConstraint>& Constraints,
	GetAType&& GetA,
	GetBType&& GetB)
	{
...
FORCEINLINE TArray<TConstraint> DetectUnidirectionalConstraints(
	...
	template <typename TConstraint, typename GetAType, typename GetBType>
FORCEINLINE TMap<FName, TArray<FName>> BuildBoneConstraintAdjacencyGraph(
	const TArray<TConstraint>& Constraints,
	GetAType&& GetA,
	GetBType&& GetB)
	{
...
FORCEINLINE TMap<FName, TArray<FName>> BuildBoneConstraintAdjacencyGraph(
	...
	template <typename TConstraint, typename GetAType, typename GetBType>
FORCEINLINE TMap<FName, int32> MeasureConstraintDensityPerBone(
	const TArray<TConstraint>& Constraints,
	GetAType&& GetA,
	GetBType&& GetB)
	{
...
FORCEINLINE TMap<FName, int32> MeasureConstraintDensityPerBone(
	...
	FORCEINLINE TMap<FName, TArray<FName>> BuildParentToChildrenMap(
		const TArray<T>& Constraints,
		GetParentType&& GetParent,
		GetChildType&& GetChild)
	{
...
FORCEINLINE TMap<FName, TArray<FName>> BuildParentToChildrenMap(
	...
	template <typename T, typename GetParentType, typename GetChildType>
	FORCEINLINE TMap<FName, FName> BuildChildToParentMap(
		const TArray<T>& Constraints,
		GetParentType&& GetParent,
		GetChildType&& GetChild)
	{
...
FORCEINLINE TMap<FName, FName> BuildChildToParentMap(
	...
	FORCEINLINE TMap<FName, TArray<T>> BuildBoneToConstraintsMap(
		const TArray<T>& Constraints,
		GetAType&& GetA,
		GetBType&& GetB)
	{
...
FORCEINLINE TMap<FName, TArray<T>> BuildBoneToConstraintsMap(
	...
	template <typename TNode, typename TContainer>
	TArray<TNode> FindIsolatedNodesInMap(
		const TMap<TNode, TContainer>& ParentToChildren,
	...
	TSet<TNode> ExtractParentsFromChildMap(const TMap<TNode, TArray<TNode>>& ChildToParents)
	{
		TSet<TNode> Parents;
...
FORCEINLINE TMap<TKey, RawType> ExtractRawMapFromWrapper(
	const TMap<TKey, TValue>& Input,
	ExtractFnType&& ExtractFn)
	{
...
FORCEINLINE TMap<TKey, RawType> ExtractRawMapFromWrapper(
	...
	FORCEINLINE bool AnyEntryFailsPredicate(
...
FORCEINLINE bool AnyEntryFailsPredicate(
	...
	template <typename TKey, typename TValue, typename PredicateType>
FORCEINLINE bool AllEntriesPassPredicate(
	const TMap<TKey, TValue>& Map,
	PredicateType&& Predicate)
	{
		for (const auto& Pair : Map)
...
FORCEINLINE bool AllEntriesPassPredicate(
	...
	template <typename TKey, typename TValue, typename PredicateType>
	FORCEINLINE TOptional<TKey> FindFirstFailingEntry(
		const TMap<TKey, TValue>& Map,
		PredicateType&& Predicate)
	{
		for (const auto& Pair : Map)
...
FORCEINLINE TOptional<TKey> FindFirstFailingEntry(
	...
	FORCEINLINE TMap<TKey, FString> MapToReport(
		const TArray<TElement>& Array,
		GetKeyType&& GetKey,
...
FORCEINLINE void DrawConstraintConnections(
	const TArray<T>& Constraints,
	GetLocationAType&& GetLocationA,
	GetLocationBType&& GetLocationB,
...
FORCEINLINE void DrawConstraintConnections(
	...
	FORCEINLINE void DrawBonePoints(
...
FORCEINLINE void DrawBonePoints(
	...
	template <typename T, typename GetLocationType>
	FORCEINLINE void DrawBoneLabels(
		const TMap<FName, T>& BoneMap,
		GetLocationType&& GetLocation,
		...
		float Duration = 0.f)
	{
		for (const auto& Pair : BoneMap)
		{
			DrawDebugString(
				GWorld,
				GetLocation(Pair.Value) + FVector(0, 0, 5),
				Pair.Key.ToString(),
				nullptr,
				Color,
				Duration,
...
FORCEINLINE void DrawBoneLabels(
	...
	template <typename T>
	void DrawBoneHierarchyLinks(
		const TMap<FName, TArray<FName>>& Hierarchy,
	...
	FORCEINLINE void DrawDirectionalConstraintArrows(
		const TArray<T>& Constraints,
		GetFromType&& GetFrom,
		GetToType&& GetTo,
...
FORCEINLINE void DrawConstraintMassRatioText(
	const TArray<T>& Constraints,
	const TMap<FName, TBone>& BoneMap,
	GetAType&& GetA,
	GetBType&& GetB,
...
FORCEINLINE void DrawConstraintMassRatioText(
	...
	FORCEINLINE void DrawRootIndicators(
		const TSet<FName>& Roots,
		const TMap<FName, T>& BoneMap,
...
FORCEINLINE void DrawRootIndicators(
	...
	template <typename T>
	void DrawSubtreeHighlight(
		const TSet<FName>& Subtree,
		const TMap<FName, T>& BoneMap,
		TFunctionRef<FVector(const T&)> GetPosition,
	...
	template <typename T, typename GetAType, typename GetBType>
	FORCEINLINE void DrawConstraintEndpoints(
		const TArray<T>& Constraints,
		GetAType&& GetA,
		GetBType&& GetB,
...
FORCEINLINE void DrawGraphCyclePaths(
	const TMap<K, TArray<K>>& Graph,
	GetLocationType&& GetLocation,
		...
		Recurse = [&](const K& Node) -> bool
		{
			if (Stack.Contains(Node))
			{
				...
				if (StartIndex != INDEX_NONE)
				{
					...
					{
						DrawDebugLine(GWorld, GetLocation(Path[i]), GetLocation(Path[i + 1]), Color, false, Duration, 0, Thickness);
					}
				}
				return true;
			}
...
FORCEINLINE void DrawGraphCyclePaths(
	...
	template <typename K, typename GetLocationType>
	FORCEINLINE void DrawGraphIslands(
		const TMap<K, TArray<K>>& Graph,
		GetLocationType&& GetLocation,
		float Radius = 5.f,
		float Duration = 0.f)
...
FORCEINLINE TMap<TKeyOut, TValue> TransformMapKeys(
	const TMap<TKeyIn, TValue>& Input,
	KeyTransformType&& KeyTransform)
	{
...
FORCEINLINE TMap<TKeyOut, TValue> TransformMapKeys(
	 ...
	 */
	template <typename TKey, typename TValue>
	FORCEINLINE TArray<TKey> ExtractMapKeys(const TMap<TKey, TValue>& Map)
	{
		TArray<TKey> Keys;
...
FORCEINLINE int32 RemoveKeys(TMap<TKey, TValue>& Map, const KeyArrayType& Keys)
	...
	FORCEINLINE TMap<TKeyNew, TValue> RemapKeys(
		const TMap<TKeyOld, TValue>& Map,
		KeyMapperType&& KeyMapper)
	{
...
FORCEINLINE TResult ReduceMap(
	 ...
	 */
	template <typename TKey, typename TValue, typename PredicateType>
FORCEINLINE TOptional<TPair<TKey, TValue>> FindFirstMatch(
	const TMap<TKey, TValue>& Map,
	PredicateType&& Predicate)
	{
		for (const auto& Pair : Map)
...
FORCEINLINE TOptional<TPair<TKey, TValue>> FindFirstMatch(
	...
	FORCEINLINE TArray<TOut> MapValuesToArray(
		const TMap<TKey, TValue>& Map,
...
FORCEINLINE TArray<TOut> MapValuesToArray(
	...
	TSet<TKey> IntersectKeySets(
		const TMap<TKey, TValueA>& A,
...
FORCEINLINE float ScoreMapSimilarity(
	const TMap<TKey, TValue>& A,
	const TMap<TKey, TValue>& B,
...
FORCEINLINE float ScoreMapSimilarity(
	...
	FORCEINLINE void RemoveMapOutliers(
		TMap<TKey, TValue>& Map,
		MetricFuncType&& MetricFunc,
		float ThresholdZScore = 2.0f)
	{
		...
		const float Variance = Algo::Accumulate(Metrics, 0.0f, [=](float Acc, float V)
		{
			return Acc + FMath::Square(V - Mean);
		}) / Metrics.Num();
...
FORCEINLINE void RemoveMapOutliers(
	...
	FORCEINLINE void NormalizeMapValues(
		TMap<TKey, TValue>& Map,
		ExtractFuncType&& ExtractFunc,
...
FORCEINLINE TOptional<TValue> SelectWeightedRandom(
	const TMap<TKey, TValue>& Map,
	WeightFuncType&& WeightFunc)
	{
		if (Map.Num() == 0)
		{
			return {};
...
FORCEINLINE TOptional<TValue> SelectWeightedRandom(
	...
	FORCEINLINE void ResolveConflictingEntries(
		TMap<TKey, TValue>& Map,
		IsConflictType&& IsConflict,
...
FORCEINLINE void ResolveConflictingEntries(
	...
	FORCEINLINE TMap<TKey, TValue> MergeMapsWithResolution(
		const TMap<TKey, TValue>& A,
		const TMap<TKey, TValue>& B,
...
FORCEINLINE TMap<TGroupKey, TArray<TValue>> MapGroupBy(
	const TMap<TKey, TValue>& Map,
...
FORCEINLINE TArray<TKey> DetectStableValues(
	const TMap<TKey, TValue>& Current,
...
FORCEINLINE TMap<TKey, TValue> BuildTypedMap(
	const TArray<TSource*>& SourceArray,
	GetKeyType&& GetKey,
...
FORCEINLINE void ResolveDuplicateValuesToCanonical(
	TMap<TKey, TValue>& Map,
	AreEquivalentType&& AreEquivalent,
...
FORCEINLINE void FillMapDefaultsByPatternMatching(
	TMap<TKey, TValue>& Map,
	const TSet<TKey>& RequiredKeys,
...
FORCEINLINE void ResolveAmbiguousMappings(
...
FORCEINLINE void ResolveAmbiguousMappings(
	...
	void BackfillMissingMapValuesFromReferenceMap(
		TMap<TKey, TValue>& Target,
		const TMap<TKey, TValue>& Reference)
	{
	...
	FORCEINLINE void SafeApplyToLiveObjects(
...
FORCEINLINE void SafeApplyToLiveObjects(
	 ...
	 */
	template <typename TKey>
	void RemoveSelfReferences(TMap<TKey, TKey>& Map)
	{
		for (auto It = Map.CreateIterator(); It; ++It)
		{
			if (It->Key == It->Value)
	 ...
	 */
	template <typename TKey>
	void ValidateMapObjectTypes(
		TMap<TKey, UObject*>& Map,
	...
	TSet<TEnum> DetectConflictingEnumAssignments(const TMap<TEnum, TSet<FName>>& EnumToGroupMap)
	{
	 ...
	 */
	template <typename TEnum>
bool ValidateEnumClosureChains(
	const TMap<TEnum, TEnum>& IndirectionMap,
	 ...
	 */
	template <typename TEnum>
	TArray<TEnum> TraceEnumMappingPath(const TMap<TEnum, TEnum>& IndirectionMap, TEnum Start)
	{
		TArray<TEnum> Path;
		TSet<TEnum> Visited;
	 ...
	 */
	template <typename TEnum>
	TMap<TEnum, TArray<TEnum>> GenerateInverseEnumMap(const TMap<TEnum, TEnum>& ForwardMap)
	{
	 ...
	 */
	template <typename TEnum>
	TSet<TEnum> ValidateCompleteEnumCoverage(
		const TSet<TEnum>& AllExpectedEnums,
	 ...
	 */
	template <typename TEnum>
	TSet<TEnum> DetectUnreachableSemanticTargets(const TMap<TEnum, TEnum>& IndirectionMap)
	{
	 ...
	 */
	template <typename TEnum>
	TSet<TEnum> DetectSelfReferentialEnums(const TMap<TEnum, TEnum>& IndirectionMap)
	{
	 ...
	 */
	template <typename TEnum>
	TMap<TEnum, TEnum> FlattenEnumIndirection(const TMap<TEnum, TEnum>& IndirectionMap)
	{
	...
	bool IsStructDefault(const TStruct& Struct)
	{
...
FORCEINLINE TSet<TNode> TraverseConnectedGraph(
	TNode Start,
	GetConnectedNodesType&& GetConnectedNodes,
	int32 MaxDepth = 100)
	{
		TSet<TNode> Visited;
		TQueue<TNode> Queue;
...
FORCEINLINE TSet<TNode> TraverseConnectedGraph(
	TNode Start,
	TFunctionRef<TArray<TNode>(const TNode&)> GetConnectedNodes,
	int32 MaxDepth = 100)
	{
		TSet<TNode> Visited;
		TQueue<TNode> Queue;
...
TArray<TNode> FlattenGraphToLinearPath(
	...
	GetDependenciesType&& GetDependencies)
	{
		TArray<TNode> Result;
		TSet<TNode> Visited;
...
TArray<TNode> FindShortestPathInGraph(
		...
		const TMap<TNode, TArray<TNode>>& DirectConnections)
	{
		...
		{
			TNode Key = Pair.Key;
			TSet<TNode> Reached = TraverseConnectedGraph(Key,
			                                             [&](const TNode& Node)
			                                             {
				                                             const TArray<TNode>* Neighbors = DirectConnections.Find(
					                                             Node);
	...
	FORCEINLINE TMap<TEnum, TArray<TEnum>> BuildEnumToStructGraph(
		const TArray<TStruct>& Transitions,
		GetFromType&& GetFrom,
		GetToType&& GetTo)
	{
...
FORCEINLINE TMap<TEnum, TArray<TEnum>> BuildEnumToStructGraph(
	 ...
	 */
	template <typename TEnum>
	TArray<TEnum> DetectInvalidEnumLinksInGraph(
...
TOptional<TArray<TNode>> TraverseUntilGoal(
	const TNode& Start,
	GetNeighborsType&& GetNeighbors,
	IsGoalType&& IsGoal)
	{
...
FORCEINLINE TSet<TNode> TraverseWithNodeLimit(
    const TNode& Start,
    GetNeighborsType&& GetNeighbors,
    VisitorType&& Visitor,
    ...
    int32 MaxNodesToVisit = -1)
{
    return TraverseWithNodeLimit<TNode>(
        Start,
        Forward<GetNeighborsType>(GetNeighbors),
        [](const TNode&) {}, // No-op visitor
        MaxNodesToVisit);
}
...
int32 MaxNodesToVisit = -1)
	...
	FORCEINLINE TSet<TNode> TraverseWithNodeLimitUntil(
		const TNode& Start,
		GetNeighborsType&& GetNeighbors,
		VisitorType&& Visitor,
	...
	FORCEINLINE TSet<TNode> TraverseWithNodeLimitUntil(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetNeighbors,
		TFunctionRef<void(const TNode&)> Visitor,
		TFunctionRef<bool(const TNode&)> ShouldStop,
		int32 MaxNodesToVisit = -1)
	{
		return TraverseWithNodeLimitUntil<TNode,
			TFunctionRef<TArray<TNode>(const TNode&)>,
			TFunctionRef<void(const TNode&)>,
			TFunctionRef<bool(const TNode&)>>(
				Start,
				GetNeighbors,
				Visitor,
				ShouldStop,
	...
	FORCEINLINE TSet<TKey> TraverseWithProjection(
		const TNode& Start,
		GetNeighborsType&& GetNeighbors,
		VisitorType&& Visitor,
		GetKeyType&& GetKey,
	...
	FORCEINLINE TSet<TKey> TraverseWithProjection(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetNeighbors,
		TFunctionRef<void(const TNode&)> Visitor,
		TFunctionRef<TKey(const TNode&)> GetKey,
		int32 MaxNodesToVisit = -1)
	{
		return TraverseWithProjection<TNode, TKey,
			TFunctionRef<TArray<TNode>(const TNode&)>,
			TFunctionRef<void(const TNode&)>,
			TFunctionRef<TKey(const TNode&)>>(
				Start,
				GetNeighbors,
				Visitor,
				GetKey,
	...
	FORCEINLINE TArray<TResult> TraverseAndMap(
		const TNode& Start,
		GetNeighborsType&& GetNeighbors,
		TransformType&& Transform,
		int32 MaxNodesToVisit = -1)
	...
	FORCEINLINE TArray<TResult> TraverseAndMap(
		...
		int32 MaxNodesToVisit = -1)
	{
		return TraverseAndMap<TNode, TResult,
			TFunctionRef<TArray<TNode>(const TNode&)>,
			TFunctionRef<TResult(const TNode&)>>(
				Start,
				GetNeighbors,
				Transform,
				MaxNodesToVisit);
	...
	template <typename TNode, typename GetNeighborsType>
	FORCEINLINE TSet<TNode> LoopAwareSubgraphExtract(
		const TNode& Root,
		GetNeighborsType&& GetNeighbors,
		int32 MaxDepth,
	...
	FORCEINLINE TSet<TNode> LoopAwareSubgraphExtract(
		...
		bool bIncludeLoopEntrances = true)
	{
		return LoopAwareSubgraphExtract<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			Root,
			GetNeighbors,
			MaxDepth,
	...
	void CloneSubgraph(
		const TNode& Root,
		GetChildrenType&& GetChildren,
		CloneNodeType&& CloneNode,
	...
	bool ValidateAllPathsSatisfyConstraint(
		...
		int32 MaxDepth = 12)
	{
		return ValidateAllPathsSatisfyConstraint<TNode,
			TFunctionRef<TArray<TNode>(const TNode&)>,
			TFunctionRef<bool(const TArray<TNode>&)>>(
				Root,
				GetChildren,
				IsPathValid,
				MaxDepth);
	...
	{
		return DetectCycleInStructGraph<TStruct, TFunctionRef<TArray<TStruct>(const TStruct&)>>(
			Root,
			GetLinks);
	}
	...
	// 1. Generic callable version for GetLinks
	template <typename TStruct, typename GetLinksType>
	TArray<TArray<TStruct>> ReportCyclesInGraph(
		const TStruct& Root,
		GetLinksType&& GetLinks)
	{
	...
	TArray<TArray<TStruct>> ReportCyclesInGraph(
		...
		TFunctionRef<TArray<TStruct>(const TStruct&)> GetLinks)
	{
		return ReportCyclesInGraph<TStruct, TFunctionRef<TArray<TStruct>(const TStruct&)>>(
			Root,
			GetLinks);
	}
	...
	{
		return DetectCycleInDataAssetGraph<TAsset, TFunctionRef<TArray<TAsset*>(const TAsset*)>>(
			Root,
			GetLinks);
	}
	 ...
	 */
	template <typename TEnum>
	bool ValidateEnumGraphHasNoLoops(
		const TMap<TEnum, TArray<TEnum>>& Graph)
	{
		TSet<TEnum> Visited;
		TSet<TEnum> Path;
	...
	bool DetectCycleWithMaxDepth(
		...
		int32 MaxDepth = 32)
	{
		return DetectCycleWithMaxDepth<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			Root,
			GetLinks,
			MaxDepth);
	}
	...
	TSet<TNode> RevertGraphToCheckpoint(
		const TSet<TNode>& AllNodes,
		const TSet<TNode>& CheckpointRoots,
		GetNeighborsType&& GetNeighbors)
	{
	...
	TSet<TNode> RevertGraphToCheckpoint(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetNeighbors)
	{
		return RevertGraphToCheckpoint<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			AllNodes,
			CheckpointRoots,
			GetNeighbors);
	}
	...
	TSet<TNode> PruneGraphFromInvalidNodes(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks)
	{
		return PruneGraphFromInvalidNodes<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			AllNodes,
			ValidRoots,
			GetLinks);
	}
	...
	void CopyIfValid(
		...
		CopyFnType&& CopyFn,
		ValidPredicateType&& ValidPredicate)
	{
	...
	{
		return InitializeObjectsFromSourceArray<TData, TSource, TFunctionRef<bool(TData&, const TSource*)>>(
			Sources,
			Initializer);
	}
	...
	template <typename TKey, typename TValue, typename TElement,
			  typename GetKeyAType, typename GetKeyBType,
			  typename ValidateValueType, typename ExtraConstraintCheckType>
	void RemoveIfAnyMissingOrInvalid(
		TArray<TElement>& Array,
		const TMap<TKey, TValue>& ReferenceMap,
		const GetKeyAType& GetKeyA,
		const GetKeyBType& GetKeyB,
		const ValidateValueType& ValidateValue,
	...
	{
		RemoveIfAnyMissingOrInvalid<TKey, TValue, TElement,
			TFunctionRef<TKey(const TElement&)>,
			TFunctionRef<TKey(const TElement&)>,
			TFunctionRef<bool(const TValue&)>,
			TFunctionRef<bool(const TElement&)>>(
				Array,
				ReferenceMap,
				GetKeyA,
				GetKeyB,
				ValidateValue,
				ExtraConstraintCheck);
	}

	// 1. Overload for TFunctionRef parameters (most specific)
	template <typename TKey, typename TValue, typename TElement>
	void RemoveIfInvalidByKey(
		TArray<TElement>& Array,
		const TMap<TKey, TValue>& Map,
	...
	template <typename TKey, typename TValue, typename TElement,
			  typename GetKeyType, typename ValidateType>
	void RemoveIfInvalidByKey(
		TArray<TElement>& Array,
		const TMap<TKey, TValue>& Map,
		const GetKeyType& GetKey,
	...
	{
		RemoveIfInvalidByKey<TKey, TValue, TElement>(
			Array,
			Map,
	...
	// 1. Generic callable version for GetLinks
	template <typename TNode, typename GetLinksType>
	TSet<TNode> CaptureGraphSnapshot(
		const TNode& Root,
		GetLinksType&& GetLinks)
	{
		TSet<TNode> Snapshot;
	...
	TSet<TNode> CaptureGraphSnapshot(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks)
	{
		return CaptureGraphSnapshot<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			Root,
			GetLinks);
	}
	 ...
	 */
	template <typename TNode>
	void DiffGraphStates(
		const TSet<TNode>& OldState,
		const TSet<TNode>& NewState,
		TSet<TNode>& OutRemoved,
		TSet<TNode>& OutAdded)
	{
	 ...
	 */
	template <typename TNode>
	TSet<TNode> RestoreGraphFromSnapshot(
		const TSet<TNode>& CurrentState,
	...
	TMap<TNode, float> ScoreRollbackSources(
		const TSet<TNode>& Sources,
		ScoreFuncType&& ScoreFunc)
	{
	...
	{
		return ScoreRollbackSources<TNode, TFunctionRef<float(const TNode&)>>(
			Sources,
			ScoreFunc);
	}
	...
	// 1. Generic callable version for ScoreFunc
	template <typename TNode, typename ScoreFuncType>
	TOptional<TNode> SelectBestRollbackSource(
		const TSet<TNode>& Candidates,
		ScoreFuncType&& ScoreFunc)
	{
	...
	TOptional<TNode> SelectBestRollbackSource(
		...
		TFunctionRef<float(const TNode&)> ScoreFunc)
	{
		return SelectBestRollbackSource<TNode, TFunctionRef<float(const TNode&)>>(
			Candidates,
			ScoreFunc);
	}
	 ...
	 */
	template <typename TNode>
	TSet<TNode> MergeGraphSnapshots(
		const TArray<TSet<TNode>>& Snapshots,
	 ...
	 */
	template <typename TNode>
	TMap<TNode, int32> IsolateRollbackDivergences(const TArray<TSet<TNode>>& Snapshots)
	{
	 ...
	 */
	template <typename TNode>
	TSet<TNode> BuildConsensusRollbackGraph(
		const TArray<TSet<TNode>>& Snapshots,
	...
	// 1. Generic callable version for GetNeighbors
	template <typename TNode, typename GetNeighborsType>
	TMap<TNode, TArray<TNode>> BuildForwardGraph(
		const TNode& Root,
		GetNeighborsType&& GetNeighbors)
	{
	...
	TMap<TNode, TArray<TNode>> BuildForwardGraph(
		...
		TFunctionRef<TArray<TNode>(const TNode&)> GetNeighbors)
	{
		return BuildForwardGraph<TNode, TFunctionRef<TArray<TNode>(const TNode&)>>(
			Root,
			GetNeighbors);
	}
	...
	TMap<TNode, AdjacencyContainer> BuildReverseGraph(
		const TMap<TNode, AdjacencyContainer>& ForwardGraph)
	{
	...
	template <typename TNode>
	TMap<TNode, TArray<TNode>> BuildReverseGraph(
		const TMap<TNode, TArray<TNode>>& ForwardGraph)
	{
	 ...
	 */
	template <typename TNode>
	TOptional<TArray<TNode>> FindPathToNode(
		const TNode& Target,
	...
	TMap<TKey, TValue> ExtractSubgraph(
		const TKey& Start,
		const TMap<TKey, TValue>& FullGraph,
		GetNeighborsType&& GetNeighbors,
		int32 MaxNodes = -1)
	{
	...
	TMap<TKey, TValue> ExtractSubgraph(
		...
		int32 MaxNodes = -1)
	{
		return ExtractSubgraph<TKey, TValue, TFunctionRef<TArray<TKey>(const TKey&)>>(
			Start,
			FullGraph,
			GetNeighbors,
			MaxNodes);
	...
	TSet<TNode> ExtractSubgraphToNode(
		const TNode& Target,
		const ReverseGraphType& ReverseGraph,
		const TNode& Root)
	{
		TSet<TNode> Subgraph;
		TFunction<void(const TNode&)> Recurse;
	...
	TSet<TNode> ExtractSubgraphToNode(
		...
		const TNode& Root)
	{
		return ExtractSubgraphToNode<TNode, TMap<TNode, TArray<TNode>>>(
			Target,
			ReverseGraph,
			Root);
	}
	...
	TOptional<TArray<TNode>> SearchGraphByPredicate(
		const TNode& Root,
		GetChildrenType&& GetChildren,
		PredicateType&& Predicate)
	{
	...
	TMap<TKey, TArray<TNode>> IndexGraphByKey(
		const TNode& Root,
		GetNeighborsType&& GetNeighbors,
	 ...
	 */
	template <typename TNode>
	TArray<TNode> CollectAllMatchingInGraph(
		const TNode& Root,
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks,
		TFunctionRef<bool(const TNode&)> Predicate)
	{
	...
	TArray<TValue> ExtractFieldValuesFromGraph(
		const TNode& Root,
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks,
	...
	TArray<TStruct> MapGraphToStructs(
		const TNode& Root,
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks,
	 ...
	 */
	template <typename TNode, typename TValue>
	TMap<TNode, TValue> BuildGraphValueCache(
		const TNode& Root,
		TFunctionRef<TArray<TNode>(const TNode&)> GetLinks,
		TFunctionRef<TValue(const TNode&)> Extract)
	{
 ...
 */
	...
	{
		template <typename T>
		TArray<T> FilterArray(const TArray<T>& Source, TFunctionRef<bool(const T&)> Predicate)
		{
			TArray<T> Result;
	...
	TSet<S> MapArrayToSet(const TArray<T>& Source, TFunctionRef<S(const T&)> Projector)
	{
		TSet<S> Result;
		for (const T& Item : Source)
		{
	...
	template <typename T>
	TArray<T> FilterArrayByIndex(const TArray<T>& Source, TFunctionRef<bool(int32)> IndexPredicate)
	{
		TArray<T> Result;
	...
	template <typename T, typename S>
	TArray<S> MapSetToArray(const TSet<T>& Source, TFunctionRef<S(const T&)> Projector)
	{
		TArray<S> Result;
		for (const T& Item : Source)
		{
	...
	template <typename T>
	void SortArrayStableByKey(TArray<T>& Array, TFunctionRef<bool(const T&, const T&)> Comparator)
	{
	...
	template <typename T, typename S>
	TSet<S> MapUnique(const TArray<T>& Source, TFunctionRef<S(const T&)> Projector)
	{
		TSet<S> Result;
		for (const T& Item : Source)
		{
	...
	template <typename T>
	TMap<int32, int32> StableIndexRemap(const TArray<T>& Original, const TArray<T>& Transformed)
	{
	...
	template <typename T>
	TArray<T> RemapArrayUsingMap(const TArray<T>& Source, const TMap<int32, int32>& Remap)
	{
		TArray<T> Result;
	...
	template <typename T>
	bool VerifyGraphMappingCompleteness(
		const TSet<T>& AllNodes,
 ...
 */
	...
	public:
		TGenericGraphAStarTraits(
			FGetNeighborsFunc InGetNeighbors,
			FGetCostFunc InGetCost,
			FHeuristicFunc InHeuristic,
			FIsValidNeighborFunc InIsValidNeighbor = [](const TNode&, const TNode&, const TContext*) { return true; }
		)
			: GetNeighborsFunc(InGetNeighbors)
			  , GetCostFunc(InGetCost)
			  , HeuristicFunc(InHeuristic)
	 ...
	 */
	template <typename TNode, typename TContext = void, typename TKeyFuncs = TDefaultMapHashableKeyFuncs<
		          TNode, void, false>, typename TEqualityComparer = TEqualTo<TNode>>
	class TAStarSearch
	{
	public:
	...
	template <typename TElement, typename TKey>
	TMap<TKey, TArray<TElement>> GroupBy(
		const TArray<TElement>& Items,
		TFunctionRef<TKey(const TElement&)> GetKey)
	{
	...
	TMap<K2, V> TransformKeys(
		const TMap<K1, V>& Input,
