...
#include "OHSafeMacroUtils.h"
#include "Templates/Function.h"

/**
 * OHSafeMapUtils
 *
 * Type-agnostic template utilities to prevent data desync, dangling keys,
 * invalid values, and maintain structural integrity across maps, arrays, and
 * key references.
 */
namespace OHSafeMapUtils {

template <typename KeyType, typename ValueType>
FORCEINLINE const ValueType *GetReference(const TMap<KeyType, ValueType> &Map,
                                          const KeyType &Key) {
  return Map.Contains(Key) ? &Map[Key] : nullptr;
}

template <typename KeyType, typename ValueType>
FORCEINLINE ValueType *GetMutableReference(TMap<KeyType, ValueType> &Map,
                                           const KeyType &Key) {
  return Map.Contains(Key) ? &Map[Key] : nullptr;
}

template <typename KeyType, typename ValueType>
FORCEINLINE const ValueType &
GetReferenceChecked(const TMap<KeyType, ValueType> &Map, const KeyType &Key) {
  checkf(Map.Contains(Key), TEXT("Key %s not found in map!"), *Key.ToString());
  return Map[Key];
}

template <typename KeyType, typename ValueType>
FORCEINLINE ValueType &GetMutableReferenceChecked(TMap<KeyType, ValueType> &Map,
                                                  const KeyType &Key) {
  checkf(Map.Contains(Key), TEXT("Key %s not found in map!"), *Key.ToString());
  return Map[Key];
}

template <typename TContext, typename TValue, typename TStruct,
          typename TField = TValue>
EOHMutationResult TryApplyRuntimeMutation(
    const TContext &ContextID, const TValue &NewValue,
    TField TStruct::*CachedFieldPtr,
    TFunctionRef<bool(TStruct &, const TValue &)> ApplyToLiveInstanceFn,
    TStruct &TargetStruct, const FString &DebugLabel = TEXT("UnnamedField"),
    bool bStrict = false, int32 *OutMutationCounter = nullptr,
    TFunctionRef<void()> OnMutation = []() {}) {
  // Validate pointer
  if (!CachedFieldPtr) {
    UE_LOG(LogTemp, Error,
           TEXT("Mutation failed: Null cached field ptr for [%s]"),
           *DebugLabel);
  ...
  if (bApplied) {
    UE_LOG(LogTemp, VeryVerbose,
           TEXT("Mutation [%s] applied to live instance (%s)."), *DebugLabel,
           *ContextID.ToString());
    return EOHMutationResult::SuccessDirect;
  } else if (!bSame || bStrict) {
    if (bStrict) {
      UE_LOG(LogTemp, Error, TEXT("Strict mutation failed: [%s] on [%s]"),
             *DebugLabel, *ContextID.ToString());
      ensureMsgf(false, TEXT("Strict mutation failure [%s]"), *DebugLabel);
      return EOHMutationResult::Failure;
    } else {
      UE_LOG(LogTemp, Warning,
             TEXT("Fallback mutation: [%s] cached only (no live instance)."),
             *DebugLabel);
...
bool AssertMapContainsAllKeys(
    const TMap<K, V> &Map, const TSet<K> &RequiredKeys,
    FString Context = TEXT("AssertMapContainsAllKeys")) {
  for (const K &Key : RequiredKeys) {
    if (!Map.Contains(Key)) {
      UE_LOG(LogTemp, Error, TEXT("[%s] Missing key: %s"), *Context,
             *Key.ToString());
      return false;
    }
  }
...
template <typename K> bool EnsureMapIsBijective(const TMap<K, K> &Map) {
  TSet<K> SeenValues;
  for (const auto &Pair : Map) {
    if (!SeenValues.Add(Pair.Value)) {
      UE_LOG(LogTemp, Error,
             TEXT("Non-bijective mapping detected for value: %s"),
             *Pair.Value.ToString());
      return false;
    }
  }
...
template <typename TKey, typename TEdge, typename TArrayWrapper,
          typename TArrayWrapper2>
void BuildGraphHierarchyAndConstraintMap(
    const TArray<TEdge> &Edges, TMap<TKey, TArrayWrapper> &ParentToChildrenMap,
...
template <typename K, typename V>
TMap<K, V> GraphDiff(const TMap<K, V> &A, const TMap<K, V> &B) {
  TMap<K, V> Result;
...
template <typename K, typename V>
float ComputeMapMean(const TMap<K, V> &Map,
                     TFunctionRef<float(const V &)> Extract) {
  if (Map.Num() == 0) {
    return 0.f;
  }
  float Sum = 0.f;
...
template <typename K, typename V>
TMap<int32, TArray<K>> BucketizeMap(const TMap<K, V> &Map,
                                    TFunctionRef<float(const V &)> Extract,
                                    float BucketSize) {
...
template <typename K, typename V>
float ComputeMapVariance(const TMap<K, V> &Map,
                         TFunctionRef<float(const V &)> Extract) {
  float Mean = ComputeMapMean(Map, Extract);
  float Accum = 0.f;
  for (const auto &Pair : Map) {
    const float Val = Extract(Pair.Value);
    Accum += FMath::Square(Val - Mean);
  }
  return Map.Num() > 0 ? Accum / Map.Num() : 0.f;
}

template <typename K, typename V>
TMap<K, float> ComputeZScores(const TMap<K, V> &Map,
                              TFunctionRef<float(const V &)> Extract) {
  TMap<K, float> Result;
  float Mean = ComputeMapMean(Map, Extract);
  float Variance = ComputeMapVariance(Map, Extract);
  float StdDev = FMath::Sqrt(Variance);

  for (const auto &Pair : Map) {
    float Z = StdDev > 0.f ? (Extract(Pair.Value) - Mean) / StdDev : 0.f;
...
template <typename K>
TArray<TArray<K>> CompressLinearChains(const TMap<K, TArray<K>> &Graph) {
  TSet<K> Visited;
...
template <typename K, typename V>
TMap<K, V> MergeMapsWithConflictResolution(
    const TMap<K, V> &A, const TMap<K, V> &B,
...
template <typename K, typename V>
bool AreGraphsIsomorphic(const TMap<K, V> &A, const TMap<K, V> &B) {
  if (A.Num() != B.Num()) {
    return false;
  }
...
template <typename T>
TArray<T> MakeFilterIterator(const TArray<T> &Input,
                             TFunctionRef<bool(const T &)> Predicate,
                             int32 MaxItems = -1) {
  TArray<T> Result;
...
template <typename K>
bool AssertNoCycles(const TMap<K, TArray<K>> &Graph,
                    FString Label = TEXT("Graph")) {
  TSet<K> Visited, Stack;

  TFunction<bool(const K &)> Recurse;
  Recurse = [&](const K &Node) -> bool {
    if (Stack.Contains(Node)) {
      UE_LOG(LogTemp, Error, TEXT("[%s] Cycle detected at: %s"), *Label,
             *Node.ToString());
      return false;
    }
...
template <typename K>
TSet<K> GetSubtreeFromNode(const K &Start, const TMap<K, TArray<K>> &Graph) {
  TSet<K> Result;
  TQueue<K> Queue;
  Queue.Enqueue(Start);
...
template <typename T> struct TPipe {
  ...
  template <typename Reducer, typename OutT>
  OutT Reduce(Reducer R, OutT Init) const {
    for (const auto &Item : Data) {
      Init = R(Init, Item);
    }
    return Init;
...
template <typename K, typename V> struct TPipeMap {
  ...
  TPipeMap<K, VOut> MapValues(TFunctionRef<VOut(const V &)> Transform) const {
    TMap<K, VOut> Result;
  ...
  TPipeMap<K, V>
  FilterPairs(TFunctionRef<bool(const K &, const V &)> Pred) const {
    TMap<K, V> Result;
    for (const auto &Pair : Data) {
  ...
  OutT Reduce(TFunctionRef<OutT(const OutT &, const K &, const V &)> Reducer,
              OutT Init) const {
    for (const auto &Pair : Data) {
  ...
  const TMap<K, V> &Get() const { return Data; }
  TMap<K, V> &&Move() { return MoveTemp(Data); }
};
...
template <typename K>
bool ValidateParentLinkage(const TMap<K, TArray<K>> &ParentToChildren) {
  TSet<K> AllChildren;
  ...
  for (const K &Child : AllChildren) {
    if (!ParentToChildren.Contains(Child)) {
      UE_LOG(LogTemp, Warning, TEXT("Child %s has no parent entry."),
             *Child.ToString());
      return false;
    }
  }
...
template <typename K, typename V>
bool ValidateBoneMassRange(const TMap<K, V> &BoneMap,
                           TFunctionRef<float(const V &)> GetMass,
                           float Min = 0.01f, float Max = 1000.f) {
  for (const auto &Pair : BoneMap) {
    const float M = GetMass(Pair.Value);
    if (M < Min || M > Max) {
      UE_LOG(LogTemp, Warning, TEXT("Bone %s has invalid mass: %f"),
             *Pair.Key.ToString(), M);
      return false;
    }
  }
...
template <typename K>
TArray<TArray<K>>
FindChainsMatching(const TMap<K, TArray<K>> &Graph,
...
template <typename K>
bool IsLinearChain(const TMap<K, TArray<K>> &Graph, const K &Root) {
  int32 NodeCount = 0;
  K Current = Root;
...
TSet<FName>
FindUnconstrainedBones(const TMap<FName, TBone> &BoneMap,
...
template <typename TConstraint, typename TBone>
TArray<TConstraint> ValidateConstraintMassBalance(
    const TArray<TConstraint> &Constraints, const TMap<FName, TBone> &BoneMap,
...
TArray<TConstraint>
DetectUnidirectionalConstraints(const TArray<TConstraint> &Constraints,
                                TFunctionRef<FName(const TConstraint &)> GetA,
                                TFunctionRef<FName(const TConstraint &)> GetB) {
...
template <typename TConstraint>
TMap<FName, TArray<FName>> BuildBoneConstraintAdjacencyGraph(
    const TArray<TConstraint> &Constraints,
    TFunctionRef<FName(const TConstraint &)> GetA,
    TFunctionRef<FName(const TConstraint &)> GetB) {
...
template <typename TConstraint>
TMap<FName, int32>
MeasureConstraintDensityPerBone(const TArray<TConstraint> &Constraints,
                                TFunctionRef<FName(const TConstraint &)> GetA,
                                TFunctionRef<FName(const TConstraint &)> GetB) {
...
template <typename T>
TMap<FName, TArray<FName>>
BuildParentToChildrenMap(const TArray<T> &Constraints,
...
template <typename T>
TMap<FName, FName>
BuildChildToParentMap(const TArray<T> &Constraints,
                      TFunctionRef<FName(const T &)> GetParent,
                      TFunctionRef<FName(const T &)> GetChild) {
...
template <typename T>
TMap<FName, TArray<T>>
BuildBoneToConstraintsMap(const TArray<T> &Constraints,
...
TArray<TNode>
FindAllRootsInMap(const TMap<TNode, TContainer> &ParentToChildren) {
...
template <typename K>
TSet<K> FindAllRoots(const TMap<K, TArray<K>> &Adjacency) {
...
template <typename TNode, typename TContainer>
TArray<TNode>
FindAllLeavesInMap(const TMap<TNode, TContainer> &ParentToChildren) {
  TSet<TNode> AllChildren;
  for (const auto &Pair : ParentToChildren) {
    for (const TNode &Child : Pair.Value) {
      AllChildren.Add(Child);
...
template <typename K>
TSet<K> FindAllLeaves(const TMap<K, TArray<K>> &Adjacency) {
  TSet<K> All, Children;
  for (const auto &Pair : Adjacency) {
    All.Add(Pair.Key);
...
template <typename TNode, typename TContainer>
TArray<TNode>
FindIsolatedNodesInMap(const TMap<TNode, TContainer> &ParentToChildren,
                       const TSet<TNode> &AllNodes) {
...
template <typename TNode, typename TContainer>
TMap<TNode, TArray<TNode>>
InvertMapToMultiMap(const TMap<TNode, TContainer> &ParentToChildren) {
...
TSet<TNode>
ExtractParentsFromChildMap(const TMap<TNode, TArray<TNode>> &ChildToParents) {
  TSet<TNode> Parents;
...
TMap<TKey, RawType>
ExtractRawMapFromWrapper(const TMap<TKey, TValue> &Input,
...
// Returns true if any entry in the map fails the given predicate
template <typename TKey, typename TValue>
bool AnyEntryFailsPredicate(
    const TMap<TKey, TValue> &Map,
...
template <typename TKey, typename TValue>
TOptional<TKey> FindFirstFailingEntry(
    const TMap<TKey, TValue> &Map,
    TFunctionRef<bool(const TKey &, const TValue &)> Predicate) {
  for (const auto &Pair : Map) {
    if (!Predicate(Pair.Key, Pair.Value)) {
...
TMap<TKey, FString>
MapToReport(const TArray<TElement> &Array,
            TFunctionRef<TKey(const TElement &)> GetKey,
...
template <typename T>
void DrawConstraintConnections(const TArray<T> &Constraints,
                               TFunctionRef<FVector(const T &)> GetLocationA,
                               TFunctionRef<FVector(const T &)> GetLocationB,
                               const FColor &Color = FColor::Red,
                               float Thickness = 2.f, float Duration = 0.f) {
  for (const T &C : Constraints) {
    DrawDebugLine(GWorld, GetLocationA(C), GetLocationB(C), Color, false,
                  Duration, 0, Thickness);
  }
}

template <typename T>
void DrawBonePoints(const TMap<FName, T> &BoneMap,
                    TFunctionRef<FVector(const T &)> GetLocation,
                    const FColor &Color = FColor::Blue, float Radius = 3.f,
                    float Duration = 0.f) {
  for (const auto &Pair : BoneMap) {
    DrawDebugSphere(GWorld, GetLocation(Pair.Value), Radius, 8, Color, false,
                    Duration);
  }
}

template <typename T>
void DrawBoneLabels(const TMap<FName, T> &BoneMap,
                    TFunctionRef<FVector(const T &)> GetLocation,
                    const FColor &Color = FColor::White, float Duration = 0.f) {
  for (const auto &Pair : BoneMap) {
    DrawDebugString(GWorld, GetLocation(Pair.Value) + FVector(0, 0, 5),
                    Pair.Key.ToString(), nullptr, Color, Duration, false);
  }
}

template <typename T>
void DrawBoneHierarchyLinks(const TMap<FName, TArray<FName>> &Hierarchy,
                            const TMap<FName, FVector> &BonePositions,
  ...
  for (const auto &Pair : Hierarchy) {
    ...
    for (const FName &Child : Pair.Value) {
      if (const FVector *End = BonePositions.Find(Child)) {
        DrawDebugLine(GWorld, *Start, *End, Color, false, Duration, 0,
                      Thickness);
      }
    }
  }
...
template <typename T>
void DrawDirectionalConstraintArrows(const TArray<T> &Constraints,
                                     TFunctionRef<FVector(const T &)> GetFrom,
  ...
  for (const T &C : Constraints) {
    ...
    DrawDebugDirectionalArrow(GWorld, Start, End, ArrowSize, Color, false,
                              Duration, 0, Thickness);
  }
}

template <typename T, typename TBone>
void DrawConstraintMassRatioText(const TArray<T> &Constraints,
  ...
  for (const T &C : Constraints) {
    ...
    DrawDebugString(GWorld, Mid, FString::Printf(TEXT("%.2f"), Ratio), nullptr,
                    Color, Duration, false);
  }
}

template <typename T>
void DrawRootIndicators(const TSet<FName> &Roots, const TMap<FName, T> &BoneMap,
                        TFunctionRef<FVector(const T &)> GetPosition,
  ...
  for (const FName &Root : Roots) {
    if (const T *Bone = BoneMap.Find(Root)) {
      DrawDebugSphere(GWorld, GetPosition(*Bone), Radius, 8, Color, false,
                      Duration);
    }
  }
}
...
template <typename T>
void DrawSubtreeHighlight(const TSet<FName> &Subtree,
                          const TMap<FName, T> &BoneMap,
                          TFunctionRef<FVector(const T &)> GetPosition,
  ...
  for (const FName &Bone : Subtree) {
    if (const T *Entry = BoneMap.Find(Bone)) {
      DrawDebugSphere(GWorld, GetPosition(*Entry), Radius, 6, Color, false,
                      Duration);
    }
  }
}
...
template <typename T>
void DrawConstraintEndpoints(const TArray<T> &Constraints,
                             TFunctionRef<FVector(const T &)> GetA,
                             TFunctionRef<FVector(const T &)> GetB,
                             const FColor &ColorA = FColor::Red,
                             const FColor &ColorB = FColor::Blue,
...
template <typename K>
void DrawGraphCyclePaths(const TMap<K, TArray<K>> &Graph,
                         TFunctionRef<FVector(const K &)> GetLocation,
    ...
    if (Stack.Contains(Node)) {
      ...
      if (StartIndex != INDEX_NONE) {
        for (int32 i = StartIndex; i < Path.Num() - 1; ++i) {
          DrawDebugLine(GWorld, GetLocation(Path[i]), GetLocation(Path[i + 1]),
...
template <typename K>
void DrawGraphIslands(const TMap<K, TArray<K>> &Graph,
                      TFunctionRef<FVector(const K &)> GetLocation,
                      float Radius = 5.f, float Duration = 0.f) {
  TSet<K> Visited;
  TArray<FColor> Palette = {FColor::Red,    FColor::Green,   FColor::Blue,
                            FColor::Cyan,   FColor::Magenta, FColor::Yellow,
  ...
  for (const auto &Pair : Graph) {
    ...
    for (const K &Node : Island) {
      DrawDebugSphere(GWorld, GetLocation(Node), Radius, 6, Color, false,
                      Duration);
    }
...
template <typename K, typename V>
void DebugLogMap(const TMap<K, V> &Map, FString Label,
                 TFunctionRef<FString(const V &)> ToString) {
  UE_LOG(LogTemp, Log, TEXT("--- %s ---"), *Label);
  for (const auto &Pair : Map) {
    UE_LOG(LogTemp, Log, TEXT("%s -> %s"), *Pair.Key.ToString(),
           *ToString(Pair.Value));
  }
}
#pragma endregion
...
TMap<TKeyOut, TValue>
TransformMapKeys(const TMap<TKeyIn, TValue> &Input,
 ...
 */
template <typename TKey, typename TValue>
TArray<TKey> ExtractMapKeys(const TMap<TKey, TValue> &Map) {
  TArray<TKey> Keys;
  Map.GetKeys(Keys);
  return Keys;
}
 ...
 */
template <typename TKey, typename TValue>
TArray<TValue> ExtractMapValues(const TMap<TKey, TValue> &Map) {
 ...
 */
template <typename TKey, typename TValue>
bool ContainsAnyKeys(const TMap<TKey, TValue> &Map, const TArray<TKey> &Keys) {
 ...
 */
template <typename TKey, typename TValue>
void RemoveKeys(TMap<TKey, TValue> &Map, const TArray<TKey> &Keys) {
  for (const TKey &Key : Keys) {
    Map.Remove(Key);
  }
}
...
/**
 * Creates a new map where each key is remapped using a provided transform
 * function. Values remain unchanged. Useful for adapting formats or normalizing
 * map key structures.
 */
template <typename TKeyOld, typename TKeyNew, typename TValue>
TMap<TKeyNew, TValue> RemapKeys(const TMap<TKeyOld, TValue> &Map,
...
TResult ReduceMap(
    const TMap<TKey, TValue> &Map, TResult InitialValue,
 ...
 */
template <typename TKey, typename TValue>
TOptional<TPair<TKey, TValue>>
FindFirstMatch(const TMap<TKey, TValue> &Map,
...
TArray<TOut> MapValuesToArray(const TMap<TKey, TValue> &Map,
                              TFunction<TOut(const TValue &)> Transform) {
  TArray<TOut> Out;
  Out.Reserve(Map.Num());
  for (const auto &Pair : Map) {
...
TSet<TKey> IntersectKeySets(const TMap<TKey, TValueA> &A,
                            const TMap<TKey, TValueB> &B) {
...
template <typename TKey, typename TValue>
float ScoreMapSimilarity(
    const TMap<TKey, TValue> &A, const TMap<TKey, TValue> &B,
...
template <typename TKey, typename TValue>
void RemoveMapOutliers(TMap<TKey, TValue> &Map,
                       TFunction<float(const TValue &)> MetricFunc,
  ...
  const float Variance =
      Algo::Accumulate(
          Metrics, 0.0f,
          [=](float Acc, float V) { return Acc + FMath::Square(V - Mean); }) /
      Metrics.Num();

  const float StdDev = FMath::Sqrt(Variance);

  for (auto It = Map.CreateIterator(); It; ++It) {
    const float Z = FMath::Abs((MetricFunc(It->Value) - Mean) /
                               (StdDev + KINDA_SMALL_NUMBER));
...
template <typename TKey, typename TValue>
void NormalizeMapValues(TMap<TKey, TValue> &Map,
                        TFunction<float(const TValue &)> ExtractFunc,
...
template <typename TKey, typename TValue>
float EntropyOfMapKeys(const TMap<TKey, TValue> &Map) {
  if (Map.Num() == 0) {
    return 0.f;
  }
...
template <typename TKey, typename TValue>
TOptional<TValue>
SelectWeightedRandom(const TMap<TKey, TValue> &Map,
...
template <typename TKey, typename TValue>
void ResolveConflictingEntries(
...
template <typename TKey, typename TValue>
TMap<TKey, TValue> MergeMapsWithResolution(
    const TMap<TKey, TValue> &A, const TMap<TKey, TValue> &B,
...
TMap<TGroupKey, TArray<TValue>>
MapGroupBy(const TMap<TKey, TValue> &Map,
...
template <typename TKey, typename TValue>
TArray<TKey>
DetectStableValues(const TMap<TKey, TValue> &Current,
                   const TMap<TKey, TValue> &Previous,
 ...
 */
template <typename TKey, typename TValue>
void AutoFillMissingKeysWithInference(
    TMap<TKey, TValue> &Map, const TSet<TKey> &RequiredKeys,
    TFunction<TOptional<TValue>(const TKey &MissingKey,
                                const TMap<TKey, TValue> &Existing)>
        InferFunc) {
...
TMap<TKey, TValue>
BuildTypedMap(const TArray<TSource *> &SourceArray,
 ...
 */
template <typename TKey, typename TValue>
void ResolveDuplicateValuesToCanonical(
    TMap<TKey, TValue> &Map,
...
/**
 * Adds default values to a map where keys match a known naming or structural
 * pattern. This is useful for procedural systems where structured identifiers
 * like "Bone_01" exist.
 */
template <typename TKey, typename TValue>
void FillMapDefaultsByPatternMatching(
    TMap<TKey, TValue> &Map, const TSet<TKey> &RequiredKeys,
 ...
 */
template <typename TKey, typename TValue>
void ResolveAmbiguousMappings(
    TMap<TKey, TArray<TValue>> &AmbiguousMap,
  ...
  for (auto &Pair : AmbiguousMap) {
    ...
    if (Best) {
      Pair.Value = {*Best}; // Reduce to best match
    }
  }
}
 ...
 */
template <typename TKey, typename TValue>
void BackfillMissingMapValuesFromReferenceMap(
    TMap<TKey, TValue> &Target, const TMap<TKey, TValue> &Reference) {
...
void RemoveNullOrInvalidUObjects(TMap<TKey, TObject *> &Map) {
  for (auto It = Map.CreateIterator(); It; ++It) {
...
/**
 * Returns a filtered copy of the input map where all values are valid
 * (non-null, non-destroyed). This protects against dereferencing or operating
 * on stale object references.
 */
template <typename TKey, typename TValue>
TMap<TKey, TValue *> FilterMapToValidPointers(const TMap<TKey, TValue *> &Map) {
...
template <typename TKey, typename TObject>
TMap<TKey, TObject>
DereferenceMapWithCheck(const TMap<TKey, TObject *> &Source) {
...
/**
 * Applies a callback to each valid object pointer in a map without modifying
 * it. Ensures safe iteration and access, skipping invalid objects.
 */
template <typename TKey, typename TObject>
void SafeApplyToLiveObjects(
    const TMap<TKey, TObject *> &Map,
...
/**
 * Removes entries from an actor pointer map where the actor is destroyed or
 * invalid. Protects systems from accessing pending-kill actors (e.g., during GC
 * or cleanup).
 */
template <typename TKey>
void RemoveStaleReferencesToDestroyedActors(TMap<TKey, AActor *> &Map) {
  for (auto It = Map.CreateIterator(); It; ++It) {
 ...
 * Ensures multiple keys don't point to the exact same object unless
 * intentional.
 */
template <typename TKey, typename TValue>
void PurgeDuplicatePointerValues(TMap<TKey, TValue *> &Map) {
  TSet<TValue *> Seen;
  for (auto It = Map.CreateIterator(); It; ++It) {
...
/**
 * Validates that all UObject* values in the map are of a required class type
 * (or derived from it). Useful for enforcing component types or interface
 * compliance.
 */
template <typename TKey>
void ValidateMapObjectTypes(TMap<TKey, UObject *> &Map, UClass *RequiredClass) {
  for (auto It = Map.CreateIterator(); It; ++It) {
 ...
 */
template <typename TKey, typename TObject>
void RemoveObjectsInUnloadedWorlds(TMap<TKey, TObject *> &Map) {
  for (auto It = Map.CreateIterator(); It; ++It) {
...
/**
 * Removes any components that are detached, invalid, or whose owning actor is
 * no longer valid. Protects against runtime errors from stale UI, scene, or
 * actor-bound systems.
 */
template <typename TKey, typename TComponent>
void RemoveInvalidComponentReferences(TMap<TKey, TComponent *> &Map) {
  static_assert(TIsDerivedFrom<TComponent, UActorComponent>::IsDerived,
                "Must be a UActorComponent-derived type");
...
TSet<TEnum> DetectConflictingEnumAssignments(
    const TMap<TEnum, TSet<FName>> &EnumToGroupMap) {
 ...
 */
template <typename TEnum>
bool ValidateEnumClosureChains(const TMap<TEnum, TEnum> &IndirectionMap,
                               TArray<TEnum> &OutCycle) {
  TSet<TEnum> Visited;
 ...
 */
template <typename TEnum>
TArray<TEnum> TraceEnumMappingPath(const TMap<TEnum, TEnum> &IndirectionMap,
                                   TEnum Start) {
  TArray<TEnum> Path;
  TSet<TEnum> Visited;
 ...
 */
template <typename TEnum>
TMap<TEnum, TArray<TEnum>>
GenerateInverseEnumMap(const TMap<TEnum, TEnum> &ForwardMap) {
...
/**
 * Checks which enum values from an expected set are missing from the mapped
 * set. Useful for validating full coverage of a skeletal rig or input schema.
 */
template <typename TEnum>
TSet<TEnum> ValidateCompleteEnumCoverage(const TSet<TEnum> &AllExpectedEnums,
                                         const TSet<TEnum> &UsedEnums) {
...
template <typename TEnum>
TSet<TEnum>
DetectUnreachableSemanticTargets(const TMap<TEnum, TEnum> &IndirectionMap) {
...
template <typename TEnum>
TSet<TEnum>
DetectSelfReferentialEnums(const TMap<TEnum, TEnum> &IndirectionMap) {
...
/**
 * Identifies target enums that are referenced by multiple different source
 * enums. This can indicate semantic overlaps or mapping collisions.
 */
template <typename TEnum>
TMap<TEnum, TArray<TEnum>>
DetectShadowedEnumMappings(const TMap<TEnum, TEnum> &IndirectionMap) {
 ...
 * This flattens the structure and resolves nested mappings into single-layer
 * output.
 */
template <typename TEnum>
TMap<TEnum, TEnum>
FlattenEnumIndirection(const TMap<TEnum, TEnum> &IndirectionMap) {
 ...
 */
template <typename TEnum>
bool VerifyMappingInjectivity(const TMap<TEnum, TEnum> &IndirectionMap) {
  TSet<TEnum> Targets;
  for (const auto &Pair : IndirectionMap) {
...
/**
 * Returns a diff report between a struct and an object, listing all differing
 * fields with their struct and object values.
 */
template <typename TStruct, typename TObject>
TMap<FName, FString> StructObjectDiffReport(const TStruct &Struct,
  ...
  for (TFieldIterator<FProperty> PropIt(TStruct::StaticStruct()); PropIt;
       ++PropIt) {
    const FProperty *StructProp = *PropIt;
    FProperty *ObjectProp =
        FindFProperty<FProperty>(Object->GetClass(), StructProp->GetFName());

    if (ObjectProp && StructProp->SameType(ObjectProp)) {
      const void *StructValue =
          StructProp->ContainerPtrToValuePtr<const void>(&Struct);
      const void *ObjectValue =
          ObjectProp->ContainerPtrToValuePtr<const void>(Object);

      if (!StructProp->Identical(StructValue, ObjectValue)) {
        FString StructStr, ObjectStr;
        StructProp->ExportTextItem(StructStr, StructValue, nullptr, nullptr,
                                   PPF_None);
        ObjectProp->ExportTextItem(ObjectStr, ObjectValue, nullptr, nullptr,
                                   PPF_None);
        Report.Add(StructProp->GetFName(),
                   FString::Printf(TEXT("Struct: %s | Object: %s"), *StructStr,
                                   *ObjectStr));
      }
    }
  }
...
template <typename TStruct> bool IsStructDefault(const TStruct &Struct) {
  ...
  return TStruct::StaticStruct()->CompareScriptStruct(&Struct, &DefaultStruct,
                                                      PPF_None) == false;
}
...
TMap<FName, FString> StructToStringMap(const TStruct &Struct) {
  ...
  for (TFieldIterator<FProperty> PropIt(TStruct::StaticStruct()); PropIt;
    ...
    FString Exported;
    Prop->ExportTextItem(Exported, Value, nullptr, nullptr, PPF_None);
...
/**
 * Selectively applies specific fields from a struct to an object, based on a
 * provided update mask. Useful for partial patching during gameplay, hot
 * reload, or rollback.
 */
template <typename TStruct, typename TObject>
void MergeStructIntoObjectWithMask(const TStruct &Struct, TObject *Object,
                                   const TSet<FName> &FieldsToUpdate) {
  if (!IsValid(Object)) {
    return;
  }
...
/**
 * Serializes a struct into a clean, readable, sorted multiline string for logs
 * or developer output.
 */
template <typename TStruct>
FString SerializeStructForDebug(const TStruct &Struct) {
  ...
  for (TFieldIterator<FProperty> PropIt(TStruct::StaticStruct()); PropIt;
    ...
    const void *Value = Prop->ContainerPtrToValuePtr<const void>(&Struct);
    FString Str;
    Prop->ExportTextItem(Str, Value, nullptr, nullptr, PPF_None);
...
/**
 * Performs a safe breadth-first traversal of a node graph using a connection
 * lambda. Returns all reachable nodes from the starting point.
 */
template <typename TNode>
TSet<TNode> TraverseConnectedGraph(
    TNode Start, TFunctionRef<TArray<TNode>(const TNode &)> GetConnectedNodes,
    int32 MaxDepth = 100) {
  TSet<TNode> Visited;
  TQueue<TNode> Queue;
  Queue.Enqueue(Start);
  ...
  while (!Queue.IsEmpty() && Depth++ < MaxDepth) {
    ...
    // Fix: Only add if not already visited
...
/**
 * Recursively detects cycles in a relational graph. Returns true if a cycle is
 * found. Outputs the path that triggered the cycle if applicable.
 */
template <typename TNode>
bool DetectCyclesInGraph(
    TNode Start, TFunctionRef<TArray<TNode>(const TNode &)> GetConnectedNodes,
...
/**
 * Sorts a directed acyclic graph (DAG) into a valid execution or evaluation
 * order. Ignores cycles and treats them as already visited.
 */
template <typename TNode>
TArray<TNode> FlattenGraphToLinearPath(
    const TArray<TNode> &Nodes,
...
bool ValidateGraphConnectivity(
    const TNode &Root, const TSet<TNode> &AllNodes,
...
/**
 * Traces a lineage path from a given node up to the root using a single-parent
 * lookup. Returns the ordered path from child to ancestor.
 */
template <typename TNode>
TArray<TNode>
TracePathToRoot(const TNode &Start,
                TFunctionRef<TOptional<TNode>(const TNode &)> GetParent) {
  TArray<TNode> Path;
  TSet<TNode> Seen;
  TNode Current = Start;
 ...
 */
template <typename TNode>
TSet<TNode> CollapseGraphToMinimalRoots(
    const TSet<TNode> &AllNodes,
    TFunctionRef<TArray<TNode>(const TNode &)> GetDependencies) {
 ...
 */
template <typename TNode>
TSet<TNode> TraceGraphInfluenceSources(
    const TNode &Target,
    TFunctionRef<TArray<TNode>(const TNode &)> GetParents) {
  TSet<TNode> Sources;
  TQueue<TNode> Queue;
 ...
 */
template <typename TNode>
TArray<TSet<TNode>> BuildGraphComponentSets(
    const TSet<TNode> &AllNodes,
    TFunctionRef<TArray<TNode>(const TNode &)> GetConnected) {
 ...
 */
template <typename TNode>
TArray<TNode> FindShortestPathInGraph(
    const TNode &Start, const TNode &Goal,
 ...
 */
template <typename TNode>
TMap<TNode, TSet<TNode>> ResolveTransitiveClosureMap(
    const TMap<TNode, TArray<TNode>> &DirectConnections) {
 ...
 */
template <typename TEnum>
TEnum ResolveEnumChain(const TMap<TEnum, TEnum> &EnumMap, TEnum Start) {
  TSet<TEnum> Visited;
  TEnum Current = Start;
...
TMap<TEnum, TArray<TEnum>>
BuildEnumToStructGraph(const TArray<TStruct> &Transitions,
...
template <typename TEnum>
TArray<TEnum>
DetectInvalidEnumLinksInGraph(const TMap<TEnum, TArray<TEnum>> &Graph,
                              const TSet<TEnum> &ValidEnums) {
...
/**
 * Checks whether a path exists between two enum values in a directed enum
 * graph.
 */
template <typename TEnum>
bool ValidateEnumPathExists(const TEnum &Start, const TEnum &Goal,
                            const TMap<TEnum, TArray<TEnum>> &Graph) {
  TSet<TEnum> Visited;
  TQueue<TEnum> Queue;
  Queue.Enqueue(Start);
 ...
 */
template <typename TEnum>
TArray<TEnum> TraceEnumPath(const TEnum &Start, const TEnum &Goal,
                            const TMap<TEnum, TArray<TEnum>> &Graph) {
...
/**
 * Performs a depth-first traversal of a graph, applying a Visit lambda to each
 * unique node.
 */
template <typename TNode>
void DepthFirstVisit(const TNode &Start,
                     TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
 ...
 */
template <typename TNode>
void BreadthFirstVisit(const TNode &Start,
                       TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
                       TFunctionRef<void(const TNode &)> Visit) {
  TSet<TNode> Visited;
  TQueue<TNode> Queue;
 ...
 */
template <typename TNode>
bool TopologicalSort(const TSet<TNode> &AllNodes,
                     TFunctionRef<TArray<TNode>(const TNode &)> GetDependencies,
 ...
 */
template <typename TNode>
TMap<TNode, float> DijkstraShortestPaths(
    const TNode &Start, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
 ...
 */
template <typename TNode>
void EnumerateAllPaths(const TNode &Start, const TNode &Goal,
                       TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
...
/**
 * Traverses until a node satisfying a goal condition is reached. Returns first
 * path to that goal.
 */
template <typename TNode>
TOptional<TArray<TNode>>
TraverseUntilGoal(const TNode &Start,
                  TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
...
/**
 * Performs a breadth-first traversal from Start node,
 ...
 */
template <typename TNode>
TSet<TNode> TraverseWithNodeLimit(
    const TNode &Start, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
...
template <typename TNode>
TSet<TNode> TraverseWithNodeLimitUntil(
    const TNode &Start, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
    TFunctionRef<void(const TNode &)> Visitor,
    TFunctionRef<bool(const TNode &)> ShouldStop, int32 MaxNodesToVisit = -1) {
  TSet<TNode> Visited;
  TQueue<TNode> Queue;
  Queue.Enqueue(Start);
...
template <typename TNode>
TSet<TNode>
TraverseWithNodeLimit(const TNode &Start,
                      TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
...
template <typename TNode>
void TraverseWithNodeLimit_VisitorOnly(
    const TNode &Start, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
    TFunctionRef<void(const TNode &)> Visitor, int32 MaxNodesToVisit = -1) {
  TraverseWithNodeLimit<TNode>(Start, GetNeighbors, Visitor, MaxNodesToVisit);
}

template <typename TNode, typename TKey>
TSet<TKey> TraverseWithProjection(
    const TNode &Start, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
    TFunctionRef<void(const TNode &)> Visitor,
...
TArray<TResult>
TraverseAndMap(const TNode &Start,
               TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
               TFunctionRef<TResult(const TNode &)> Transform,
               int32 MaxNodesToVisit = -1) {
  TSet<TNode> Visited;
  TQueue<TNode> Queue;
...
/**
 * Extracts a subgraph from a root node to a limited depth, with optional loop
 * awareness.
 */
template <typename TNode>
TSet<TNode> LoopAwareSubgraphExtract(
    const TNode &Root, TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
    int32 MaxDepth, bool bIncludeLoopEntrances = true) {
  TSet<TNode> Subgraph;
  TSet<TNode> Path;
...
/**
 * Recursively clones a node graph starting from a root using user-provided
 * clone logic.
 */
template <typename TNode>
void CloneSubgraph(const TNode &Root,
                   TFunctionRef<TArray<TNode>(const TNode &)> GetChildren,
                   TFunctionRef<void(const TNode &)> CloneNode,
  ...
  for (const TNode &Child : GetChildren(Root)) {
    CloneSubgraph(Child, GetChildren, CloneNode, OutVisited);
  }
}

/**
 * Validates that every path from a root to a leaf node satisfies a constraint
 * function.
 */
template <typename TNode>
bool ValidateAllPathsSatisfyConstraint(
    const TNode &Root, TFunctionRef<TArray<TNode>(const TNode &)> GetChildren,
    TFunctionRef<bool(const TArray<TNode> &)> IsPathValid,
    int32 MaxDepth = 12) {
  TArray<TNode> CurrentPath;
  TSet<TNode> Seen;
 ...
 */
template <typename TStruct>
TArray<TArray<TStruct>>
ReportCyclesInGraph(const TStruct &Root,
                    TFunctionRef<TArray<TStruct>(const TStruct &)> GetLinks) {
...
bool DetectCycleInDataAssetGraph(
    const TAsset *Root,
    TFunctionRef<TArray<TAsset *>(const TAsset *)> GetLinks) {
 ...
 */
template <typename TEnum>
bool ValidateEnumGraphHasNoLoops(const TMap<TEnum, TArray<TEnum>> &Graph) {
  TSet<TEnum> Visited;
  TSet<TEnum> Path;
...
/**
 * Performs defensive cycle detection using a max-depth limit to avoid infinite
 * loops.
 */
template <typename TNode>
bool DetectCycleWithMaxDepth(
    const TNode &Root, TFunctionRef<TArray<TNode>(const TNode &)> GetLinks,
    int32 MaxDepth = 32) {
  TSet<TNode> Path;
  TFunction<bool(const TNode &, int32)> Recurse;
 ...
 */
template <typename TNode>
TSet<TNode> RevertGraphToCheckpoint(
    const TSet<TNode> &AllNodes, const TSet<TNode> &CheckpointRoots,
    TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors) {
 ...
 */
template <typename TNode>
TSet<TNode> PruneGraphFromInvalidNodes(
    const TSet<TNode> &AllNodes, const TSet<TNode> &ValidRoots,
...
void CopyIfValid(TData &Target, const TSource &Source,
                 TFunctionRef<void(TData &, const TSource &)> CopyFn,
...
template <typename TData, typename TSource>
TArray<TData> InitializeObjectsFromSourceArray(
...
void RemoveIfAnyMissingOrInvalid(
    TArray<TElement> &Array, const TMap<TKey, TValue> &ReferenceMap,
    TFunctionRef<TKey(const TElement &)> GetKeyA,
    TFunctionRef<TKey(const TElement &)> GetKeyB,
    TFunctionRef<bool(const TValue &)> ValidateValue,
    TFunctionRef<bool(const TElement &)> ExtraConstraintCheck =
        [](const TElement &) { return true; }) {
...
template <typename TNode>
TSet<TNode>
CaptureGraphSnapshot(const TNode &Root,
                     TFunctionRef<TArray<TNode>(const TNode &)> GetLinks) {
  TSet<TNode> Snapshot;
  TQueue<TNode> Queue;
  Queue.Enqueue(Root);
 ...
 */
template <typename TNode>
void DiffGraphStates(const TSet<TNode> &OldState, const TSet<TNode> &NewState,
                     TSet<TNode> &OutRemoved, TSet<TNode> &OutAdded) {
 ...
 */
template <typename TNode>
TSet<TNode> RestoreGraphFromSnapshot(const TSet<TNode> &CurrentState,
                                     const TSet<TNode> &Snapshot) {
...
template <typename TNode>
TMap<TNode, float>
ScoreRollbackSources(const TSet<TNode> &Sources,
                     TFunctionRef<float(const TNode &)> ScoreFunc) {
...
template <typename TNode>
TOptional<TNode>
SelectBestRollbackSource(const TSet<TNode> &Candidates,
                         TFunctionRef<float(const TNode &)> ScoreFunc) {
 ...
 */
template <typename TNode>
TSet<TNode> MergeGraphSnapshots(const TArray<TSet<TNode>> &Snapshots,
                                bool bRequireConsensus = false) {
  if (Snapshots.Num() == 0) {
    return {};
  }
...
template <typename TNode>
TMap<TNode, int32>
IsolateRollbackDivergences(const TArray<TSet<TNode>> &Snapshots) {
 ...
 */
template <typename TNode>
TSet<TNode> BuildConsensusRollbackGraph(const TArray<TSet<TNode>> &Snapshots,
                                        int32 QuorumThreshold) {
...
template <typename TNode>
TMap<TNode, TArray<TNode>>
BuildForwardGraph(const TNode &Root,
                  TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors) {
...
template <typename TNode>
TMap<TNode, TArray<TNode>>
BuildReverseGraph(const TMap<TNode, TArray<TNode>> &ForwardGraph) {
  TMap<TNode, TArray<TNode>> ReverseGraph;
...
template <typename TNode>
TOptional<TArray<TNode>>
FindPathToNode(const TNode &Target,
               const TMap<TNode, TArray<TNode>> &ReverseGraph,
               const TNode &Root) {
  TMap<TNode, TNode> CameFrom;
  TQueue<TNode> Queue;
  TSet<TNode> Visited;
...
template <typename TKey, typename TValue>
TMap<TKey, TValue>
ExtractSubgraph(const TKey &Start, const TMap<TKey, TValue> &FullGraph,
                TFunctionRef<TArray<TKey>(const TKey &)> GetNeighbors,
                int32 MaxNodes = -1) {
  TSet<TKey> Reachable = TraverseWithNodeLimit<TKey>(
      Start, GetNeighbors, [](const TKey &) {}, MaxNodes);
...
template <typename TNode>
TSet<TNode>
ExtractSubgraphToNode(const TNode &Target,
                      const TMap<TNode, TArray<TNode>> &ReverseGraph,
                      const TNode &Root) {
  TSet<TNode> Subgraph;
  TFunction<void(const TNode &)> Recurse;
  Recurse = [&](const TNode &Node) {
    ...
    const TArray<TNode> *Parents = ReverseGraph.Find(Node);
    if (Parents) {
...
template <typename TNode>
TOptional<TArray<TNode>>
SearchGraphByPredicate(const TNode &Root,
                       TFunctionRef<TArray<TNode>(const TNode &)> GetChildren,
 ...
 */
template <typename TNode, typename TKey>
TMap<TKey, TArray<TNode>>
IndexGraphByKey(const TNode &Root,
                TFunctionRef<TArray<TNode>(const TNode &)> GetNeighbors,
...
template <typename TNode>
TArray<TNode>
CollectAllMatchingInGraph(const TNode &Root,
                          TFunctionRef<TArray<TNode>(const TNode &)> GetLinks,
                          TFunctionRef<bool(const TNode &)> Predicate) {
...
TArray<TValue>
ExtractFieldValuesFromGraph(const TNode &Root,
                            TFunctionRef<TArray<TNode>(const TNode &)> GetLinks,
...
TArray<TStruct>
MapGraphToStructs(const TNode &Root,
                  TFunctionRef<TArray<TNode>(const TNode &)> GetLinks,
...
template <typename TNode, typename TValue>
TMap<TNode, TValue>
BuildGraphValueCache(const TNode &Root,
                     TFunctionRef<TArray<TNode>(const TNode &)> GetLinks,
                     TFunctionRef<TValue(const TNode &)> Extract) {
...
bool TryInsertOrResolve(TMap<TKey, TValue> &Map, const TKey &Key,
                        const TValue &Value,
...
bool TryInsertIfNoConflict(TMap<TKey, TValue> &Map, const TKey &Key,
                           const TValue &Value,
...
int32 InsertAllValidated(TMap<TKey, TValue> &Map,
                         const TMap<TKey, TValue> &Source,
...
template <typename TKey, typename TValue, typename TConflictPredicate>
int32 InsertAllIfNoConflict(TMap<TKey, TValue> &Map,
                            const TMap<TKey, TValue> &Source,
                            TConflictPredicate &&ConflictPredicate) {
  int32 Count = 0;
  for (const auto &Pair : Source) {
    if (!Map.Contains(Pair.Key)) {
      ...
      if (!ConflictPredicate(Existing, Pair.Value)) {
        ++Count; // counted as non-conflict skip
      }
    }
  }
  return Count;
...
int32 ResolveAllConflicts(TMap<TKey, TValue> &Map,
                          const TMap<TKey, TValue> &Source,
                          TResolver &&Resolver) {
  int32 Resolved = 0;
  for (const auto &Pair : Source) {
...
TArray<TKey> FilterKeysByValue(const TMap<TKey, TValue> &Map,
...
template <typename K, typename V>
TMap<K, V> FilterMapValues(const TMap<K, V> &Source,
                           TFunctionRef<bool(const V &)> Predicate) {
  TMap<K, V> Result;
...
TArray<T> FilterArray(const TArray<T> &Input, TPredicate &&Predicate) {
  TArray<T> Output;
  for (const T &Item : Input) {
    if (Predicate(Item)) {
      Output.Add(Item);
...
namespace OHSafeMapUtils {
template <typename T>
TArray<T> FilterArray(const TArray<T> &Source,
                      TFunctionRef<bool(const T &)> Predicate) {
  TArray<T> Result;
...
template <typename TKey, typename TValue, typename TPredicate>
TArray<TValue> FilterMapValues(const TMap<TKey, TValue> &Map,
                               TPredicate &&Predicate) {
...
TSet<T> MapArrayToSet(const TArray<T> &Input, TTransform &&Func) {
  TSet<T> Result;
  for (const T &Item : Input) {
...
TSet<S> MapArrayToSet(const TArray<T> &Source,
                      TFunctionRef<S(const T &)> Projector) {
  TSet<S> Result;
  for (const T &Item : Source) {
...
// With BreakOnFalse
template <typename K, typename V>
TMap<K, V> FilterMapValuesBreakable(const TMap<K, V> &Source,
                                    TFunctionRef<bool(const V &)> Predicate) {
  TMap<K, V> Result;
  for (const TPair<K, V> &Pair : Source) {
    if (!Predicate(Pair.Value)) {
...
TMap<K, V>
FilterMapValuesWithContext(const TMap<K, V> &Source, const C &Context,
...
template <typename T, typename S>
TSet<S>
MapArrayToSetSuppressNull(const TArray<T> &Source,
                          TFunctionRef<TOptional<S>(const T &)> Projector) {
  TSet<S> Result;
  for (const T &Item : Source) {
...
TSet<S>
MapArrayToSetWithContext(const TArray<T> &Source, const C &Context,
...
TSet<S> MapMapToSet(const TMap<K, V> &Source,
                    TFunctionRef<S(const V &)> Projector) {
  TSet<S> Result;
  for (const auto &Pair : Source) {
...
TSet<S>
MapMapToSetWithContext(const TMap<K, V> &Source, const C &Context,
...
template <typename K, typename V, typename S>
TSet<S>
MapMapToSetSuppressNull(const TMap<K, V> &Source,
                        TFunctionRef<TOptional<S>(const V &)> Projector) {
  TSet<S> Result;
  for (const auto &Pair : Source) {
...
template <typename T>
TArray<T> FilterArrayByIndex(const TArray<T> &Source,
                             TFunctionRef<bool(int32)> IndexPredicate) {
  TArray<T> Result;
...
template <typename T>
TArray<T>
FilterArrayByIndexValue(const TArray<T> &Source,
                        TFunctionRef<bool(const T &, int32)> Predicate) {
  TArray<T> Result;
  for (int32 i = 0; i < Source.Num(); ++i) {
...
template <typename T, typename S>
TArray<S> MapSetToArray(const TSet<T> &Source,
                        TFunctionRef<S(const T &)> Projector) {
  TArray<S> Result;
  for (const T &Item : Source) {
    Result.Add(Projector(Item));
...
template <typename T, typename S, typename C>
TArray<S>
MapSetToArrayWithContext(const TSet<T> &Source, const C &Context,
                         TFunctionRef<S(const T &, const C &)> Projector) {
  TArray<S> Result;
  for (const T &Item : Source) {
    Result.Add(Projector(Item, Context));
...
template <typename T>
void SortArrayStableByKey(TArray<T> &Array,
                          TFunctionRef<bool(const T &, const T &)> Comparator) {
  Array.Sort(Comparator); // Stable in UE5
}

template <typename T, typename K>
void SortArrayStableByField(TArray<T> &Array,
                            TFunctionRef<K(const T &)> KeyFunc,
...
// Filter → Map
template <typename T, typename S>
TArray<S> FilterThenMap(const TArray<T> &Source,
                        TFunctionRef<bool(const T &)> Filter,
...
// Map → Filter
template <typename T, typename S>
TArray<S> MapThenFilter(const TArray<T> &Source,
                        TFunctionRef<S(const T &)> Project,
...
template <typename T, typename S>
TArray<S> FilterMapSetToArray(const TSet<T> &Source,
                              TFunctionRef<bool(const T &)> Filter,
                              TFunctionRef<S(const T &)> Project) {
  TArray<S> Result;
  for (const T &Item : Source) {
...
template <typename T, typename S>
TArray<S> FilterThenMapIndexed(const TArray<T> &Source,
                               TFunctionRef<bool(const T &, int32)> Filter,
...
template <typename T, typename S>
TArray<S> MapThenFilterIndexed(const TArray<T> &Source,
                               TFunctionRef<S(const T &, int32)> Project,
...
TArray<S>
MapThenSort(const TArray<T> &Source, TFunctionRef<S(const T &)> Project,
...
template <typename T, typename S, typename K>
TArray<S> MapThenSortIndexed(const TArray<T> &Source,
                             TFunctionRef<S(const T &, int32)> Project,
                             TFunctionRef<K(const S &)> KeyFunc,
                             bool bAscending = true) {
  TArray<S> Result;
  for (int32 i = 0; i < Source.Num(); ++i) {
    Result.Add(Project(Source[i], i));
...
template <typename T, typename S>
TSet<S> MapUnique(const TArray<T> &Source,
                  TFunctionRef<S(const T &)> Projector) {
  TSet<S> Result;
  for (const T &Item : Source) {
    Result.Add(Projector(Item));
...
template <typename T>
TMap<int32, int32> StableIndexRemap(const TArray<T> &Original,
                                    const TArray<T> &Transformed) {
...
template <typename T>
void ReverseArrayWithIndexMap(const TArray<T> &Source, TArray<T> &OutReversed,
                              TMap<int32, int32> &OutIndexRemap) {
...
template <typename K, typename V>
TMultiMap<V, K> InvertMap(const TMap<K, V> &Source) {
...
bool ValidateRemapConsistency(const TMap<A, B> &Forward,
                              const TMultiMap<B, A> &Reverse) {
...
template <typename T>
TArray<T> RemapArrayUsingMap(const TArray<T> &Source,
                             const TMap<int32, int32> &Remap) {
  TArray<T> Result;
...
template <typename T>
bool VerifyGraphMappingCompleteness(const TSet<T> &AllNodes,
                                    const TMap<T, T> &Mapping,
                                    bool bAllowSelfMap = false) {
...
public:
  ...
  bool Contains(const TItem &Item) const { return ItemIndexMap.Contains(Item); }
 ...
 * @tparam TContext Optional context type for dynamic graph info; default is
 * void.
 */
template <typename TNode, typename TContext = void> struct TAStarGraphTraits {
   ...
   */
  virtual TArray<TNode>
  GetNeighbors(const TNode &Node, const TContext *Context = nullptr) const = 0;

  /**
   * Get cost from NodeA to NodeB.
   * Can use Context to modify cost dynamically.
   */
  virtual float GetCost(const TNode &NodeA, const TNode &NodeB,
                        const TContext *Context = nullptr) const = 0;

  /**
   * Heuristic estimate cost from Node to Goal.
   * Should be admissible (never overestimate).
   * Can use Context to adjust heuristic dynamically.
   */
  virtual float Heuristic(const TNode &Node, const TNode &Goal,
                          const TContext *Context = nullptr) const = 0;

  /**
   * Check if a neighbor is valid to traverse from a given node.
   * Allows skipping invalid or blocked edges.
   * Returns true by default (all neighbors valid).
   */
   ...
   */
  virtual void OnNodeOpened(const TNode &Node,
                            const TContext *Context = nullptr) const {}

  /**
   * Optional hook called when a node is moved from the open set to the closed
   * set. Useful for debugging or instrumentation.
   */
  virtual void OnNodeClosed(const TNode &Node,
                            const TContext *Context = nullptr) const {}
   ...
   */
  virtual void OnPathFound(const TArray<TNode> &Path,
                           const TContext *Context = nullptr) const {}
};
...
public:
  ...
  using FGetNeighborsFunc =
      TFunction<TArray<TNode>(const TNode &, const TContext *)>;
  using FGetCostFunc =
      TFunction<float(const TNode &, const TNode &, const TContext *)>;
  using FHeuristicFunc =
      TFunction<float(const TNode &, const TNode &, const TContext *)>;
  using FIsValidNeighborFunc =
      TFunction<bool(const TNode &, const TNode &, const TContext *)>;
...
public:
  TGenericGraphAStarTraits(
      FGetNeighborsFunc InGetNeighbors, FGetCostFunc InGetCost,
      FHeuristicFunc InHeuristic,
      FIsValidNeighborFunc InIsValidNeighbor =
          [](const TNode &, const TNode &, const TContext *) { return true; })
      : GetNeighborsFunc(InGetNeighbors), GetCostFunc(InGetCost),
        HeuristicFunc(InHeuristic), IsValidNeighborFunc(InIsValidNeighbor) {}

  virtual TArray<TNode>
  GetNeighbors(const TNode &Node,
  ...
  virtual bool
  IsValidNeighbor(const TNode &FromNode, const TNode &Neighbor,
                  const TContext *Context = nullptr) const override {
 ...
 * @tparam TKeyFuncs Key funcs for hashing (default Unreal default).
 * @tparam TEqualityComparer Equality comparer for TNode (default Unreal
 * default).
 */
template <typename TNode, typename TContext = void,
...
private:
  ...
  TMap<TNode, TAStarNodeData<TNode>, FDefaultSetAllocator, TKeyFuncs>
      NodeDataMap;
...
public:
  explicit TAStarSearch(const TAStarGraphTraits<TNode, TContext> &InTraits,
                        const TContext *InContext = nullptr,
                        int32 InMaxNodesProcessed = 10000)
      : GraphTraits(InTraits), Context(InContext),
        MaxNodesProcessed(InMaxNodesProcessed) {}

  void SetHeuristicFunction(
      TFunction<float(const TNode &, const TNode &)> InHeuristic) {
    RuntimeHeuristic = MoveTemp(InHeuristic);
  }

  void SetCostModifierFunction(
      TFunction<float(const TNode &, const TNode &)> InCostModifier) {
  ...
  ESearchStatus GetStatus() const { return CurrentStatus; }

  // Initialize or reset search with start and goal set
  void InitializeSearch(
      const TNode &Start,
      const TSet<TNode, FDefaultSetAllocator, TKeyFuncs> &InGoals) {
    check(InGoals.Num() > 0);
  ...
  // Incremental search step: process up to MaxIterations nodes; returns false
  ...
  bool StepSearch(int32 MaxIterations) {
    ...
    while (!OpenSet.IsEmpty() && Iterations < MaxIterations) {
      ...
      for (const TNode &Neighbor : GraphTraits.GetNeighbors(Current, Context)) {
        ...
        float TentativeG =
            NodeDataMap[Current].GCost + ComputeCost(Current, Neighbor);
        ...
        if (TentativeG < NeighborData.GCost) {
          ...
          NeighborData.FCost =
              TentativeG + ComputeMinHeuristic(Neighbor, Goals);
  ...
  // Synchronous multi-goal search for compatibility (calls StepSearch
  // internally)
  bool FindPath(const TNode &Start,
                const TSet<TNode, FDefaultSetAllocator, TKeyFuncs> &InGoals,
  ...
  bool IsSearchComplete() const {
    return CurrentStatus == ESearchStatus::Found ||
           CurrentStatus == ESearchStatus::Aborted ||
           CurrentStatus == ESearchStatus::NotFound;
  }
...
private:
  ...
  float ComputeMinHeuristic(
      const TNode &Node,
      const TSet<TNode, FDefaultSetAllocator, TKeyFuncs> &InGoals) const {
    if (InGoals.Num() == 1) {
  ...
  void ReconstructPath(const TNode &Start, const TNode &Goal,
                       TArray<TNode> &OutPath) const {
    OutPath.Empty();
...
template <typename TElement, typename TKey>
TMap<TKey, TArray<TElement>>
GroupBy(const TArray<TElement> &Items,
        TFunctionRef<TKey(const TElement &)> GetKey) {
...
TMap<K2, V> TransformKeys(const TMap<K1, V> &Input,
                          TFunctionRef<K2(const K1 &)> KeyTransform) {
  TMap<K2, V> Result;
  for (const auto &Pair : Input) {
    Result.Add(KeyTransform(Pair.Key), Pair.Value);
  }
...
TResult
Aggregate(const TArray<TElement> &Items, TResult Initial,
...
template <typename K, typename V>
TMap<K, V> FilterMap(const TMap<K, V> &Input,
                     TFunctionRef<bool(const K &, const V &)> Predicate) {
  TMap<K, V> Result;
  for (const auto &Pair : Input) {
    if (Predicate(Pair.Key, Pair.Value)) {
      Result.Add(Pair.Key, Pair.Value);
