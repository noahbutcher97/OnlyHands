...
#pragma region Motion Data
    void
    FOHBoneMotionData::Initialize(FName InBoneName) {
    BoneName = InBoneName;
...
void FOHBoneMotionData::AddMotionSample(const FVector& WorldPos, const FQuat& WorldRot,
    ...
    FOHMotionFrameSample NewSample =
        FOHMotionFrameSample::CreateFromState(WorldPos, WorldRot,
    ...
    if (PrevSample && DeltaTime > KINDA_SMALL_NUMBER) {
        ...
        if (!ComponentVelocity.ContainsNaN() && !ComponentVelocity.IsNearlyZero()) {
            ...
            NewSample.ComponentLinearVelocity =
                ComponentTransform.InverseTransformVector(NewSample.WorldLinearVelocity);
        }
        ...
        if (!PrevSample->WorldLinearVelocity.ContainsNaN()) {
            NewSample.WorldLinearAcceleration =
                (NewSample.WorldLinearVelocity - PrevSample->WorldLinearVelocity) / DeltaTime;
        }

        if (!PrevSample->LocalLinearVelocity.ContainsNaN()) {
            NewSample.LocalLinearAcceleration =
                (NewSample.LocalLinearVelocity - PrevSample->LocalLinearVelocity) / DeltaTime;
        }

        // Component space acceleration
        if (!ComponentTransform.ContainsNaN()) {
            NewSample.ComponentLinearAcceleration =
                ComponentTransform.InverseTransformVector(NewSample.WorldLinearAcceleration);
        }
...
void FOHBoneMotionData::DrawDebugVisualization(UWorld* World, const FVector& CurrentPosition, float Scale) const {
    ...
    for (int32 i = 0; i < FMath::Min(10, MotionHistory.NumFrames()); i++) {
        if (const FOHMotionFrameSample* Sample = GetHistoricalSample(i)) {
            ...
            DrawDebugLine(World, PrevPos, Sample->WorldPosition, FColor::Green.WithAlpha(Alpha * 255), false, -1.0f, 0,
                          1.0f);
    ...
    if (const FOHMotionFrameSample* Latest = GetLatestSample()) {
        ...
        if (!Latest->WorldLinearVelocity.ContainsNaN()) {
            DrawDebugLine(World, CurrentPosition, CurrentPosition + Latest->WorldLinearVelocity * Scale, FColor::Blue,
                          false, -1.0f, 0, 3.0f);
        }

        // Local velocity (green)
        if (!Latest->LocalLinearVelocity.ContainsNaN()) {
            DrawDebugLine(World, CurrentPosition, CurrentPosition + Latest->LocalLinearVelocity * Scale, FColor::Green,
                          false, -1.0f, 0, 2.0f);
        }

        // Component velocity (cyan)
        if (!Latest->ComponentLinearVelocity.ContainsNaN()) {
            DrawDebugLine(World, CurrentPosition, CurrentPosition + Latest->ComponentLinearVelocity * Scale,
                          FColor::Cyan, false, -1.0f, 0, 2.0f);
        }

        // Acceleration (red)
        if (!Latest->WorldLinearAcceleration.ContainsNaN() && Latest->WorldLinearAcceleration.Size() > 100.0f) {
            DrawDebugLine(World, CurrentPosition, CurrentPosition + Latest->WorldLinearAcceleration * Scale * 0.01f,
        ...
        if (MotionQuality > 0.5f) {
            ...
            if (BezierPoints.Num() >= 4) {
                ...
                for (int32 t = 1; t <= Segments; t++) {
                    ...
                    FVector Point = UOHAlgoUtils::SampleBezierCubic(BezierPoints[0], BezierPoints[1], BezierPoints[2],
                                                                    BezierPoints[3], Alpha);

                    if (!Point.ContainsNaN()) {
                        DrawDebugLine(World, LastPoint, Point, FColor::Yellow, false, -1.0f, 0, 0.5f);
                        LastPoint = Point;
                    }
                }
            }
        ...
        FString DebugText = FString::Printf(TEXT("Q:%.2f S:%s P:%.0f"), MotionQuality,
                                            IsLikelyStrike() ? TEXT("Y") : TEXT("N"), PeakAcceleration);

        DrawDebugString(World, CurrentPosition + FVector(0, 0, 50), DebugText, nullptr, QualityColor, 0.0f, true);

        // Strike indicator
        if (IsLikelyStrike()) {
            DrawDebugSphere(World, CurrentPosition, 15.0f, 8, FColor::Orange, false, -1.0f, 0, 2.0f);
        }
    }
#endif
}
...
#pragma region BoneNameResolver
    void
    FOHNameResolver::Resolve(const TSet<FName>& Inputs, const TArray<FName>& Candidates) {
    ResolvedMap.Empty();
    MatchResults.Empty();
...
bool FOHPhysicsGraphNode::RemoveConstraint(const FName& ConstraintName) {
    int32 Removed = ConstraintLinks.RemoveAll(
        [&](const FOHConstraintInstanceData& C) { return C.GetConstraintName() == ConstraintName; });
    if (Removed > 0) {
        RebuildWrappers();
        return true;
    }
...
bool FOHPhysicsGraphNode::ValidateGraph(bool bLog, bool bAutoRepair) {
    ...
    OHSafeMapUtils::RemoveIfAnyMissingOrInvalid<FName, FOHBoneData, FOHConstraintInstanceData>(
        ConstraintLinks, BoneMap, [](const FOHConstraintInstanceData& C) { return C.GetParentBone(); },
    ...
    if (ConstraintLinks.Num() != OldConstraintNum) {
        ...
        if (bLog) {
            UE_LOG(LogTemp, Warning,
                   TEXT("[PhysicsGraphNode] Removed %d invalid constraints referencing missing or invalid bones."),
    ...
    for (const auto& BonePair : BoneMap) {
        ...
        if (!ReferencedBones.Contains(BoneName)) {
            ...
            bAllValid = false;
            if (bLog)
                UE_LOG(LogTemp, Warning, TEXT("[PhysicsGraphNode] Orphan bone: %s (not referenced by any constraint)"),
                       *BoneName.ToString());
        }
    }
    ...
    if (Islands.Num() > 1) {
        bAllValid = false;
        if (bLog)
            UE_LOG(LogTemp, Warning, TEXT("[PhysicsGraphNode] Found %d disconnected islands (subgraphs) in the graph."),
                   Islands.Num());
    }
    ...
    if (bLog) {
        if (bAllValid) {
            UE_LOG(
                LogTemp, Log,
                TEXT("[PhysicsGraphNode] Graph validation PASSED: all bones and constraints are valid and connected."));
        } else {
...
TArray<TSet<FName>> FOHPhysicsGraphNode::GetIslands(bool bBidirectional) const {
    ...
    return OHSafeMapUtils::BuildGraphComponentSets<FName>(AllBones, [this, bBidirectional](const FName& Node) {
        TArray<FName> Adj;
...
TSet<FName> FOHPhysicsGraphNode::CaptureReachableBones(const FName& StartBone) const {
    return OHSafeMapUtils::CaptureGraphSnapshot<FName>(StartBone, [this](const FName& Node) {
        TArray<FName> Adj;
        ParentToChildrenMultiMap.MultiFind(Node, Adj, /*bMaintainOrder=*/true);
        return Adj;
    });
...
void FOHPhysicsGraphNode::InitializeConstraintsFromSource(const TArray<FOHConstraintInstanceData*>& Sources) {
    ConstraintLinks =
        OHSafeMapUtils::InitializeObjectsFromSourceArray<FOHConstraintInstanceData, FOHConstraintInstanceData>(
            Sources, [](FOHConstraintInstanceData& Out, const FOHConstraintInstanceData* Src) {
                if (!Src)
                    return false;
                Out = *Src;
                return true;
...
TSet<FName> FOHPhysicsGraphNode::CaptureBoneSnapshot(const FName& RootBone) const {
    return OHSafeMapUtils::CaptureGraphSnapshot<FName>(RootBone,
                                                       [this](const FName& Node) { return GetChildrenOfBone(Node); });
}
...
>>>>>>> 0627b7d296554ee97d27b39fb5f7c959d6da32c9
    ...
    TArray<FName> FOHPhysicsGraphNode::FindShortestPath(FName Start, FName End) const {
        return OHSafeMapUtils::FindShortestPathInGraph<FName>(Start, End, [this](const FName& Node) {
            TArray<FName> Adj;
            ParentToChildrenMultiMap.MultiFind(Node, Adj, /*bMaintainOrder=*/true);
            return Adj;
        });
    ...
    void FOHPhysicsGraphNode::PrintGraphState(const FString& Tag) const {
        UE_LOG(LogTemp, Log, TEXT("[%s] BoneCount: %d, ConstraintCount: %d"), *Tag, BoneMap.Num(),
               ConstraintLinks.Num());
        UE_LOG(LogTemp, Log, TEXT("Roots: %s"),
               *FString::JoinBy(GetRootBones(), TEXT(", "), [](const FName& Name) { return Name.ToString(); }));
        UE_LOG(LogTemp, Log, TEXT("Leaves: %s"),
               *FString::JoinBy(GetLeafBones(), TEXT(", "), [](const FName& Name) { return Name.ToString(); }));
...
#pragma region BoneData
    ...
    void FOHBoneData::UpdateKinematics(const FVector& NewPosition, const FQuat& NewRotation, float DeltaTime,
                                       float TimeStamp) {
    ...
    FConstraintInstance* FOHBoneData::GetParentConstraintInstance() const {
        ...
        for (UPhysicsConstraintTemplate* ConstraintTemplate : PhysAsset->ConstraintSetup) {
            ...
            if (FConstraintInstance* CI = &ConstraintTemplate->DefaultInstance;
                (CI->ConstraintBone1 == ParentName && CI->ConstraintBone2 == BoneName) ||
                ...
                if (RefSkel.GetParentIndex(i) == Cur.BoneIdx) {
                    ...
                    for (UPhysicsConstraintTemplate* ConstraintTemplate : PhysAsset->ConstraintSetup) {
                        ...
                        FConstraintInstance* CI =
                            const_cast<FConstraintInstance*>(&ConstraintTemplate->DefaultInstance);
    ...
    void FOHBoneData::AddMotionSample(const FTransform& NewTransform, float CurrentTime, float DeltaTime) {
        ...
        FOHMotionSample Sample = FOHMotionSample::CreateFromState(
            NewTransform, GetBodyLinearVelocity(), static_cast<FVector>(GetPhysicsBlendWeight()),
            GetLinearAcceleration(), GetAngularAcceleration(), CurrentTime);
    ...
    FVector FOHBoneData::GetSmoothedAngularAcceleration() const {
        ...
        const FVector Previous =
            (MotionHistory.Num() >= 2) ? MotionHistory[MotionHistory.Num() - 2].GetAngularVelocity() : AngularVelocity;
        return (Current - Previous) / LastDeltaTime;
    }
    ...
    float FOHBoneData::ComputeImpulseVulnerabilityScore(const FVector& ImpactNormal, const FVector& RootLocation,
                                                        const FOHPhysicsGraphNode* PhysicsGraph)
        const // Add graph parameter
    {
        ...
        const float Score = (VelocityAlignment * 0.3f) + (FMath::Clamp(AngularSpeed / 300.f, 0.f, 1.f) * 0.2f) +
    ...
    FVector FOHBoneData::ComputeImpulseVector(const FHitResult& Hit, const FVector& RootLocation, float BaseStrength,
                                              ...
                                              const FOHPhysicsGraphNode* PhysicsGraph) const // Add graph parameter
    {
        ...
        const FVector Direction =
            ComputeImpulseDirection(Hit, bAutoInferMode ? InferBestImpulseDirectionMode(Hit) : ModeOverride);
    ...
    float FOHBoneData::GetUndershootRatio(const FVector& TargetPosition) const {
        ...
        const float VelocityTowardTarget =
            FVector::DotProduct(LinearVelocity, (TargetPosition - WorldPosition).GetSafeNormal());
    ...
    void FOHBoneData::InitializeFromBodyInstance(FBodyInstance * InBodyInstance) {
        ...
        SetBoneName(InBodyInstance->BodySetup.IsValid() ? InBodyInstance->BodySetup->BoneName : NAME_None);
...
#pragma region ConstraintInstanceData
    ...
    void FOHConstraintInstanceData::UpdateFromLiveConstraint() {
        if (!GetConstraintInstance()) {
            UE_LOG(LogTemp, Warning, TEXT("[FOHConstraintInstanceData] Missing GetConstraintInstance() for %s"),
