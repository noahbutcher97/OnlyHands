...
void FOHNameResolver::Resolve(const TSet<FName> &Inputs,
                              const TArray<FName> &Candidates) {
  ResolvedMap.Empty();
  MatchResults.Empty();

  for (const FName &Input : Inputs) {
    const FOHNameMatchResult Match =
        UOHAlgoUtils::FindBestNameMatchAutoStrategy(Input, Candidates);
...
bool FOHPhysicsGraphNode::RemoveBone(const FName &BoneName) {
  ...
  ConstraintLinks.RemoveAll([&](const FOHConstraintInstanceData &Constraint) {
    return Constraint.GetParentBone() == BoneName ||
           Constraint.GetChildBone() == BoneName;
  });
...
bool FOHPhysicsGraphNode::AddConstraint(
    const FOHConstraintInstanceData &Constraint) {
  if (!IsBoneValid(Constraint.GetParentBone()) ||
      !IsBoneValid(Constraint.GetChildBone()))
    return false;

  int32 FoundIdx = ConstraintLinks.IndexOfByPredicate(
      [&](const FOHConstraintInstanceData &Existing) {
...
bool FOHPhysicsGraphNode::RemoveConstraint(const FName &ConstraintName) {
  int32 Removed =
      ConstraintLinks.RemoveAll([&](const FOHConstraintInstanceData &C) {
...
bool FOHPhysicsGraphNode::ValidateGraph(bool bLog, bool bAutoRepair) {
  ...
  OHSafeMapUtils::RemoveIfAnyMissingOrInvalid<FName, FOHBoneData,
                                              FOHConstraintInstanceData>(
      ConstraintLinks, BoneMap,
  ...
  if (ConstraintLinks.Num() != OldConstraintNum) {
    ...
    if (bLog) {
      UE_LOG(LogTemp, Warning,
             TEXT("[PhysicsGraphNode] Removed %d invalid constraints "
                  "referencing missing or invalid bones."),
  ...
  for (const auto &BonePair : BoneMap) {
    ...
    if (!ReferencedBones.Contains(BoneName)) {
      ...
      if (bLog)
        UE_LOG(LogTemp, Warning,
               TEXT("[PhysicsGraphNode] Orphan bone: %s (not referenced by any "
                    "constraint)"),
               *BoneName.ToString());
    }
  }
  ...
  if (bAutoRepair && OrphanBones.Num() > 0) {
    ...
    if (bLog)
      UE_LOG(LogTemp, Warning,
             TEXT("[PhysicsGraphNode] Removed %d orphan bones."),
             OrphanBones.Num());
  }
  ...
  if (HasCycles()) {
    ...
    if (bLog)
      UE_LOG(LogTemp, Warning,
             TEXT("[PhysicsGraphNode] Constraint graph contains at least one "
                  "cycle!"));
  }
  ...
  if (Islands.Num() > 1) {
    ...
    if (bLog)
      UE_LOG(LogTemp, Warning,
             TEXT("[PhysicsGraphNode] Found %d disconnected islands "
  ...
  if (bLog) {
    if (bAllValid) {
      UE_LOG(LogTemp, Log,
             TEXT("[PhysicsGraphNode] Graph validation PASSED: all bones and "
                  "constraints are valid and connected."));
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[PhysicsGraphNode] Graph validation FAILED. See warnings "
                  "above for issues."));
    }
  }
  return bAllValid;
...
TArray<TSet<FName>> FOHPhysicsGraphNode::GetIslands(bool bBidirectional) const {
  ...
  return OHSafeMapUtils::BuildGraphComponentSets<FName>(
      AllBones, [this, bBidirectional](const FName &Node) {
        TArray<FName> Adj;
...
TSet<FName>
FOHPhysicsGraphNode::CaptureReachableBones(const FName &StartBone) const {
  return OHSafeMapUtils::CaptureGraphSnapshot<FName>(
      StartBone, [this](const FName &Node) {
        TArray<FName> Adj;
        ParentToChildrenMultiMap.MultiFind(Node, Adj, /*bMaintainOrder=*/true);
        return Adj;
      });
...
void FOHPhysicsGraphNode::InitializeBonesFromSource(
    const TArray<FOHBoneData *> &Sources) {
  TArray<FOHBoneData> Result =
      OHSafeMapUtils::InitializeObjectsFromSourceArray<FOHBoneData,
                                                       FOHBoneData>(
          Sources, [](FOHBoneData &Out, const FOHBoneData *Src) {
            if (!Src)
              return false;
            Out = *Src;
            return Out.IsValid();
...
void FOHPhysicsGraphNode::InitializeConstraintsFromSource(
    const TArray<FOHConstraintInstanceData *> &Sources) {
  ConstraintLinks = OHSafeMapUtils::InitializeObjectsFromSourceArray<
      FOHConstraintInstanceData, FOHConstraintInstanceData>(
      Sources,
...
TArray<FName>
FOHPhysicsGraphNode::GetInvalidBones(TArray<FString> *OutReasons) const {
  ...
  for (const auto &Pair : BoneMap) {
    ...
    // Duplicate check not needed for TMap, but can add more validation as
    // needed
...
TArray<FName>
FOHPhysicsGraphNode::GetInvalidConstraints(TArray<FString> *OutReasons) const {
  TArray<FName> Invalid;
  for (const FOHConstraintInstanceData &C : ConstraintLinks) {
    bool bBad = false;
    FString Reason;
    if (!IsBoneValid(C.GetParentBone())) {
      bBad = true;
      Reason = FString::Printf(TEXT("Parent bone '%s' not found."),
                               *C.GetParentBone().ToString());
    } else if (!IsBoneValid(C.GetChildBone())) {
      bBad = true;
      Reason = FString::Printf(TEXT("Child bone '%s' not found."),
                               *C.GetChildBone().ToString());
    }
    if (bBad) {
...
TSet<FName>
FOHPhysicsGraphNode::CaptureBoneSnapshot(const FName &RootBone) const {
  return OHSafeMapUtils::CaptureGraphSnapshot<FName>(
      RootBone, [this](const FName &Node) { return GetChildrenOfBone(Node); });
}
...
void FOHPhysicsGraphNode::DiffBoneSnapshots(const TSet<FName> &OldSnap,
  ...
  for (const FName &Root : AllBones) {
    ...
    bool bFound = OHSafeMapUtils::DetectCyclesInGraph<FName>(
        ...
        [this](const FName &Node) {
          TArray<FName> Adj;
          ParentToChildrenMultiMap.MultiFind(Node, Adj,
                                             /*bMaintainOrder=*/true);
          return Adj;
        },
        Cycle);
...
TArray<FName> FOHPhysicsGraphNode::FindShortestPath(FName Start,
                                                    FName End) const {
  return OHSafeMapUtils::FindShortestPathInGraph<FName>(
      Start, End, [this](const FName &Node) {
        TArray<FName> Adj;
        ParentToChildrenMultiMap.MultiFind(Node, Adj, /*bMaintainOrder=*/true);
        return Adj;
      });
...
void FOHPhysicsGraphNode::PrintGraphState(const FString &Tag) const {
  UE_LOG(LogTemp, Log, TEXT("[%s] BoneCount: %d, ConstraintCount: %d"), *Tag,
         BoneMap.Num(), ConstraintLinks.Num());
  UE_LOG(LogTemp, Log, TEXT("Roots: %s"),
         *FString::JoinBy(GetRootBones(), TEXT(", "),
                          [](const FName &Name) { return Name.ToString(); }));
  UE_LOG(LogTemp, Log, TEXT("Leaves: %s"),
         *FString::JoinBy(GetLeafBones(), TEXT(", "),
                          [](const FName &Name) { return Name.ToString(); }));
  UE_LOG(LogTemp, Log, TEXT("HasCycles: %s"),
         HasCycles() ? TEXT("YES") : TEXT("NO"));
  auto Islands = GetIslands();
  for (int32 i = 0; i < Islands.Num(); ++i) {
    UE_LOG(LogTemp, Log, TEXT("Island %d: %s"), i,
           *FString::JoinBy(Islands[i].Array(), TEXT(", "),
  ...
  for (int32 i = 0; i < BadBones.Num(); ++i) {
    UE_LOG(LogTemp, Warning, TEXT("Invalid Bone: %s — %s"),
           *BadBones[i].ToString(),
           BoneReasons.IsValidIndex(i) ? *BoneReasons[i]
                                       : TEXT("Unknown reason"));
  }
  for (int32 i = 0; i < BadConstraints.Num(); ++i) {
    UE_LOG(LogTemp, Warning, TEXT("Invalid Constraint: %s — %s"),
           *BadConstraints[i].ToString(),
           ConstraintReasons.IsValidIndex(i) ? *ConstraintReasons[i]
                                             : TEXT("Unknown reason"));
  }
}
...
void FOHBoneData::UpdateKinematics(const FVector &NewPosition,
                                   const FQuat &NewRotation, float DeltaTime,
                                   float TimeStamp) {
  if (DeltaTime <= KINDA_SMALL_NUMBER) {
    UE_LOG(LogTemp, Warning,
           TEXT("UpdateKinematics: DeltaTime too small — update aborted"));
    return;
  }
  if (!NewRotation.IsNormalized()) {
    UE_LOG(LogTemp, Warning,
           TEXT("UpdateKinematics: Invalid rotation (not normalized) — update "
                "aborted"));
    return;
  }
  ...
  const FVector RawLinearAccel =
      (RawLinearVelocity - GetBodyLinearVelocity()) / SafeDelta;
  ...
  if (!DeltaQuat.IsNormalized()) {
    UE_LOG(LogTemp, Warning,
           TEXT("UpdateKinematics: DeltaQuat not normalized — update aborted"));
    return;
  }
  ...
  const FVector RawAngularAccel =
      (RawAngularVelocity - GetBodyAngularVelocity()) / SafeDelta;
...
void FOHBoneData::UpdateFromComponent(float CurrentTime) {
  ...
  if (!Mesh->DoesSocketExist(BoneName)) {
    UE_LOG(LogTemp, Warning,
           TEXT("UpdateFromComponent: Socket %s does not exist"),
           *BoneName.ToString());
    return;
  }
...
void FOHBoneData::UpdateFromAnimationPose(float TimeStamp) {
  ...
  const FTransform BoneAnimTransform =
      Mesh->GetBoneTransform(PoseBoneIndex, FTransform::Identity);
  UpdateKinematics(BoneAnimTransform.GetLocation(),
                   BoneAnimTransform.GetRotation(), LastDeltaTime, TimeStamp);
}
...
TArray<FName> FOHBoneData::GetBonesOfMaxDepthUp(int32 MaxDepth) const {
  ...
  if (!GetOwnerComponent() || MaxDepth < 1)
    return OutBones;
  USkeletalMeshComponent *SkelMesh = GetOwnerComponent();
...
FConstraintInstance *FOHBoneData::GetParentConstraintInstance() const {
  ...
  for (UPhysicsConstraintTemplate *ConstraintTemplate :
    ...
    if (!ConstraintTemplate)
      continue;
    if (FConstraintInstance *CI = &ConstraintTemplate->DefaultInstance;
        (CI->ConstraintBone1 == ParentName &&
         CI->ConstraintBone2 == BoneName) ||
        (CI->ConstraintBone1 == BoneName &&
...
FOHBoneData::GetChildConstraintsDownToDepth(int32 MaxDepth) const {
  USkeletalMeshComponent *SkelMesh = GetOwnerComponent();
  TArray<FConstraintInstance *> Result;
  if (!SkelMesh || MaxDepth < 1)
    return Result;
      ...
      if (RefSkel.GetParentIndex(i) == Cur.BoneIdx) {
        ...
        for (UPhysicsConstraintTemplate *ConstraintTemplate :
          ...
          FConstraintInstance *CI = const_cast<FConstraintInstance *>(
              &ConstraintTemplate->DefaultInstance);
          if ((CI->ConstraintBone1 == ParentName &&
               CI->ConstraintBone2 == ChildName) ||
              (CI->ConstraintBone1 == ChildName &&
               CI->ConstraintBone2 == ParentName)) {
            Result.Add(CI);
            break;
          }
  ...
  for (int32 i = 0; i < NumBones; ++i) {
    if (RefSkel.GetParentIndex(i) == MyIdx) {
      ...
      for (UPhysicsConstraintTemplate *ConstraintTemplate :
        ...
        FConstraintInstance *CI = const_cast<FConstraintInstance *>(
            &ConstraintTemplate->DefaultInstance);
        if ((CI->ConstraintBone1 == BoneName &&
             CI->ConstraintBone2 == ChildName) ||
            (CI->ConstraintBone1 == ChildName &&
             CI->ConstraintBone2 == BoneName)) {
          Result.Add(CI);
...
TArray<FConstraintInstance *>
FOHBoneData::GetParentConstraintsUpToDepth(int32 MaxDepth) const {
  USkeletalMeshComponent *SkelMesh = GetOwnerComponent();
  TArray<FConstraintInstance *> Result;
  if (!SkelMesh || MaxDepth < 1)
    return Result;
...
// Motion Sampling
void FOHBoneData::AddMotionSample(const FTransform &NewTransform,
                                  float CurrentTime, float DeltaTime) {
  if (DeltaTime <= KINDA_SMALL_NUMBER || !NewTransform.IsValid()) {
    UE_LOG(LogTemp, Warning,
           TEXT("AddMotionSample: Invalid data — sample skipped"));
    return;
  }

  FOHMotionSample Sample = FOHMotionSample::CreateFromState(
      NewTransform, GetBodyLinearVelocity(),
      static_cast<FVector>(GetPhysicsBlendWeight()), GetLinearAcceleration(),
      GetAngularAcceleration(), CurrentTime);

  if (!Sample.IsValidSample()) {
    UE_LOG(
        LogTemp, Warning,
        TEXT("AddMotionSample: Sample failed IsValidSample — sample skipped"));
    return;
  }
...
FVector FOHBoneData::GetSmoothedLinearAcceleration() const {
  ...
  const FVector Previous =
      (PreviousPosition.IsZero() || LastDeltaTime <= KINDA_SMALL_NUMBER)
          ? Current
          : (PreviousPosition - GetPositionBeforePrevious()) / LastDeltaTime;
...
FVector FOHBoneData::GetSmoothedAngularAcceleration() const {
  ...
  const FVector Previous =
      (MotionHistory.Num() >= 2)
          ? MotionHistory[MotionHistory.Num() - 2].GetAngularVelocity()
          : AngularVelocity;
  return (Current - Previous) / LastDeltaTime;
}
...
FVector FOHBoneData::GetAverageLinearVelocity(int32 SampleCount) const {
  if (SampleCount <= 0)
    SampleCount = GetDefaultSampleCount();
  ...
  for (int32 i = MotionHistory.Num() - 1; i >= 0 && Count < SampleCount;
       --i, ++Count) {
...
FQuat FOHBoneData::GetAverageRotation(int32 SampleCount) const {
  ...
  for (int32 i = MotionHistory.Num() - 1;
       i >= 0 && Rotations.Num() < SampleCount; --i) {
...
FVector FOHBoneData::GetEstimatedLinearAcceleration(int32 SampleCount) const {
  ...
  const FOHMotionSample &Oldest =
      MotionHistory[MotionHistory.Num() - SampleCount];

  const float DeltaTime = Newest.GetTimeStamp() - Oldest.GetTimeStamp();
  if (DeltaTime <= KINDA_SMALL_NUMBER)
    return LinearAcceleration;

  const FVector DeltaVel =
      Newest.GetLinearVelocity() - Oldest.GetLinearVelocity();
...
FVector FOHBoneData::GetEstimatedAngularAcceleration(int32 SampleCount) const {
  ...
  const FVector DeltaVel =
      Newest.GetAngularVelocity() - Oldest.GetAngularVelocity();
  return DeltaVel / DeltaTime;
}

// Blended Force Computation

FVector FOHBoneData::GetBlendedLinearVelocity(float HistoryWeight,
  ...
  return FMath::Lerp(Current, Historical,
                     FMath::Clamp(HistoryWeight, 0.f, 1.f));
}

FVector FOHBoneData::GetBlendedAngularVelocity(float HistoryWeight,
                                               int32 SampleCount) const {
...
FVector FOHBoneData::GetBlendedLinearAcceleration(float HistoryWeight,
                                                  int32 SampleCount) const {
...
FVector FOHBoneData::GetBlendedAngularAcceleration(float HistoryWeight,
                                                   int32 SampleCount) const {
...
float FOHBoneData::GetPoseDriftScore() const {
  ...
  const float RotDrift = FQuat::ErrorAutoNormalize(
      CurrentRotation, AvgRotation); // Angular difference in radians
...
float FOHBoneData::GetLinearDampingRatio() const {
  ...
  const float SpeedPrev =
      (PreviousPosition - WorldPosition).Size() / LastDeltaTime;
  if (SpeedPrev <= KINDA_SMALL_NUMBER)
    return 0.f;
...
float FOHBoneData::GetTrajectoryShiftScore() const {
  if (LastDeltaTime <= KINDA_SMALL_NUMBER || LinearVelocity.IsNearlyZero() ||
      MotionHistory.Num() < 2)
    return 0.f;
...
float FOHBoneData::GetImpulseSignatureScore() const {
  ...
  const FVector PrevAccel =
      (LinearVelocity - GetSmoothedLinearVelocity()) / GetLastDeltaTime();
...
float FOHBoneData::GetCompositeInstabilityScore() const {
  ...
  return LinSpike * 0.5f + AngJitter * 0.5f + Drift * 0.05f +
         Trajectory * 0.02f;
}
    ...
    const FOHPhysicsGraphNode *PhysicsGraph) const // Add graph parameter
{
  ...
  const float VelocityAlignment =
      FVector::DotProduct(BoneVelocity.GetSafeNormal(), -ImpactNormal);
  ...
  const float Score = (VelocityAlignment * 0.3f) +
                      (FMath::Clamp(AngularSpeed / 300.f, 0.f, 1.f) * 0.2f) +
                      (FMath::Clamp(DistFromRoot / 100.f, 0.f, 1.f) * 0.2f) +
                      (FMath::Clamp(ConstraintStress, 0.f, 1.f) *
                       0.3f); // Constraint stress now major factor
...
FVector FOHBoneData::ComputeImpulseVector(
    const FHitResult &Hit, const FVector &RootLocation, float BaseStrength,
    ...
    const FOHPhysicsGraphNode *PhysicsGraph) const // Add graph parameter
{
  ...
  const FVector Direction = ComputeImpulseDirection(
      Hit, bAutoInferMode ? InferBestImpulseDirectionMode(Hit) : ModeOverride);

  const float Multiplier = ComputeImpulseVulnerabilityScore(
      Hit.ImpactNormal, RootLocation, PhysicsGraph);

  return Direction * BaseStrength * Multiplier;
}

FVector FOHBoneData::ComputeImpulseDirection(const FHitResult &Hit,
  ...
  case EImpulseDirectionMode::FromBoneVelocity: {
    ...
    return Vel.SizeSquared() > KINDA_SMALL_NUMBER ? Vel.GetSafeNormal()
                                                  : Default;
  }
...
FOHBoneData::InferBestImpulseDirectionMode(const FHitResult &Hit) const {
  ...
  if (Velocity.SizeSquared() > FMath::Square(25.f)) {
    const float Alignment =
        FVector::DotProduct(Velocity.GetSafeNormal(), -Hit.ImpactNormal);
    if (Alignment > 0.7f) {
...
float FOHBoneData::GetJerkMagnitude() const {
  const FVector PrevAccel =
      (LinearVelocity - GetSmoothedLinearVelocity()) / LastDeltaTime;
  return (LinearAcceleration - PrevAccel).Size();
}

FVector FOHBoneData::GetPositionBeforePrevious() const {
  return MotionHistory.Num() >= 2
             ? MotionHistory[MotionHistory.Num() - 2].GetLocation()
             : PreviousPosition;
}
...
void FOHBoneData::InitializeFromBodyInstance(
    const FBodyInstance *BI, const FReferenceSkeleton &RefSkeleton) {
...
void FOHBoneData::InitializeFromBodySetup(
    const UBodySetup *Setup, const FReferenceSkeleton &RefSkeleton) {
  if (!Setup)
    return;
...
bool FOHConstraintInstanceData::InitializeFromTemplate(
  ...
  SetAngularSwingStiffness(
      CI.ProfileInstance.AngularDrive.SlerpDrive.Stiffness);
  SetAngularSwingDamping(CI.ProfileInstance.AngularDrive.SlerpDrive.Damping);

  SetAngularTwistStiffness(
      CI.ProfileInstance.AngularDrive.TwistDrive.Stiffness);
  SetAngularTwistDamping(CI.ProfileInstance.AngularDrive.TwistDrive.Damping);

  // Drive‐enable flags
  SetPositionDriveEnabled(
      CI.ProfileInstance.LinearDrive.IsPositionDriveEnabled());
  SetVelocityDriveEnabled(
      CI.ProfileInstance.LinearDrive.IsVelocityDriveEnabled());

  // Initialize cached kinematic targets to zero—will be set when blending
  // begins
...
void FOHConstraintInstanceData::UpdateFromLiveConstraint() {
  if (!GetConstraintInstance()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[FOHConstraintInstanceData] Missing GetConstraintInstance() "
                "for %s"),
  ...
  // === Cone Limits ===
  SetSwing1LimitDegrees(
      GetConstraintInstance()->ProfileInstance.ConeLimit.Swing1LimitDegrees);
  SetSwing2LimitDegrees(
      GetConstraintInstance()->ProfileInstance.ConeLimit.Swing2LimitDegrees);
  SetTwistLimitDegrees(
      GetConstraintInstance()->ProfileInstance.TwistLimit.TwistLimitDegrees);

  // === Linear Drive ===
  SetLinearDriveStiffness(
      GetConstraintInstance()->ProfileInstance.LinearDrive.XDrive.Stiffness);
  SetLinearDriveDamping(
      GetConstraintInstance()->ProfileInstance.LinearDrive.XDrive.Damping);

  // === Angular Drive: Swing and Twist ===
  SetAngularSwingStiffness(
      GetConstraintInstance()
          ->ProfileInstance.AngularDrive.SwingDrive.Stiffness);
  SetAngularSwingDamping(
      GetConstraintInstance()->ProfileInstance.AngularDrive.SwingDrive.Damping);
  SetAngularTwistStiffness(
      GetConstraintInstance()
          ->ProfileInstance.AngularDrive.TwistDrive.Stiffness);
  SetAngularTwistDamping(
      GetConstraintInstance()->ProfileInstance.AngularDrive.TwistDrive.Damping);
}
