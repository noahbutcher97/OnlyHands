...
void AOHPlayerController::AcknowledgePossession(APawn *NewPawn) {
  ...
  if (CharacterPawn) {
    CharacterPawn->GetAbilitySystemComponent()->InitAbilityActorInfo(
...
void AOHPlayerController::SetupInputComponent() {
  ...
  InputComponent->BindAxis("MoveForward", this,
                           &AOHPlayerController::HandleMoveForward);
  InputComponent->BindAxis("MoveRight", this,
                           &AOHPlayerController::HandleMoveRight);
}
  ...
  /*#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
          if (GEngine)
          {
                  GEngine->AddOnScreenDebugMessage(
                          (uint64)((PTRINT)this)+12345, // unique key for
  forward 0.05f, // Short duration FColor::Green,
                          FString::Printf(TEXT("Controller Forward Axis: %.2f"),
  Value)
                  );
          }
  #endif*/
  ...
  /*#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
          if (GEngine)
          {
                  GEngine->AddOnScreenDebugMessage(
                          ...
                          0.05f,
                          FColor::Cyan,
                          FString::Printf(TEXT("Controller Right Axis: %.2f"),
  Value)
                  );
          }
  #endif*/
