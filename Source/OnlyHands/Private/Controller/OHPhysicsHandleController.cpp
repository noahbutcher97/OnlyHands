...
AOHPhysicsHandleController::AOHPhysicsHandleController() {
  ...
  // Enable mouse click and mouse over events (needed for
...
void AOHPhysicsHandleController::BeginPlay() {
  ...
  InputMode.SetLockMouseToViewportBehavior(
      EMouseLockMode::DoNotLock); // or EMouseLockMode::LockAlways if you prefer
...
void AOHPhysicsHandleController::SetCameraLookMode(bool bEnable) {
  if (bEnable) {
    ...
    InputMode.SetConsumeCaptureMouseDown(
        true); // Optional, ensures capture is clean
