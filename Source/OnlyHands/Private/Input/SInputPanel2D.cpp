...
bool SInputPanel2D::SupportsKeyboardFocus() const { return false; }
...
FReply SInputPanel2D::OnTouchStarted(const FGeometry &MyGeometry,
  ...
  // Tap vector relative to widget center, calculate quadrant for sending tap
  // event.
...
FReply SInputPanel2D::OnTouchMoved(const FGeometry &MyGeometry,
                                   const FPointerEvent &Event) {
  // New position when touch moved
  FVector2D swipePosition =
      MyGeometry.AbsoluteToLocal(Event.GetScreenSpacePosition());
  ...
  // GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Purple,
  // FString::Printf(TEXT("VELOCITY :: %s"),
  ...
  // Start Swipe
  if ((InputState == EInputState::Touch || InputState == EInputState::Hold ||
       InputState == EInputState::Nudge) &&
      swipeVelocity.Length() > MinSwipeVelocity &&
      IsPositionOutsideDeadZone(SwipePosition) && bInputAccept) {
  ...
  if (SwipePositions.Num() > 0) {
    ...
    float PrevSwipeVectorLength =
        (PrevSwipePos - InitialTouchPosition).Length();
    float NextSwipeVectorLength =
        (NextSwipePos - InitialTouchPosition).Length();
  ...
  if (InputState == EInputState::Swipe &&
      IsPositionOutsideDeadZone(SwipePosition)) {
    ...
    if (SwipePositions.Num() > 0) {
      if (SwipePositions.Num() == 1) {
        ESwipeDirection SwipeDirection =
            GetSwipeDirection(SwipePosition, InitialTouchPosition);
        OwningWidget->InputDelay =
            FMath::TruncToInt(1000 * (SwipeTimeStamp - SwipeStartTimestamp));
        OwningWidget->OnStartSwipe.Broadcast(
            SwipeDirection, EInputType::ShortSwipe, swipeVelocity);
      } else if (SwipePositions.Num() > 1) {
        float swipeVectorLength =
            (SwipePosition - InitialTouchPosition).Length();
        ...
        float PrevSwipeVectorLength =
            (PrevSwipePos - InitialTouchPosition).Length();
        float NextSwipeVectorLength =
            (SwipePositions[SwipePositions.Num() - 1] - InitialTouchPosition)
                .Length();
  ...
  // Touch -> Nudge
  // Swipe -> Nudge
  if (swipeVelocity.Length() > MinNudgeVelocity &&
      swipeVelocity.Length() < MinSwipeVelocity) {
    // Cancel Swipe
    if (InputState == EInputState::Swipe) {
      // OwningWidget->OnCancelSwipe.Broadcast();
  ...
  FVector2D joystickLimit = FVector2D(JoystickLimitMod.X * FMath::Cos(angle),
                                      JoystickLimitMod.Y * FMath::Sin(angle));

  // GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Cyan,
  // FString::Printf(TEXT("AXIS :: %s, %s"), *FString::SanitizeFloat(rawAxis.X),
  // *FString::SanitizeFloat(rawAxis.Y)));
  ...
  AxisOutput = FVector2D(FMath::Clamp(rawAxis.X / div, -1.f, 1.f),
                         FMath::Clamp(rawAxis.Y / div, -1.f, 1.f));
  ...
  OwningWidget->OnRawInputVector.Broadcast(SwipePosition -
...
void SInputPanel2D::TriggerSwipeEvent() {
  ...
  if (swipeLength > ShortSwipeThreshold &&
      CheckCurve(SwipePositions[0], SwipePositions[SwipePositions.Num() - 1],
                 InitialTouchPosition, CurveTolerance))
    inputType = EInputType::LongSwipe;

  // Check swipe direction
  ESwipeDirection swipeDirection =
      GetSwipeDirection(SwipePositions[0], InitialTouchPosition);
...
FReply SInputPanel2D::OnTouchEnded(const FGeometry &MyGeometry,
                                   const FPointerEvent &Event) {
  ReleasePosition = MyGeometry.AbsoluteToLocal(Event.GetScreenSpacePosition());

  // Touch -> Tap
  if (InputState == EInputState::Touch && FramesSinceTouch < HoldFrames &&
      OwningWidget->AutoTriggerTapFrames < 0) {
    ESwipeDirection tapDirection =
        GetSwipeDirection(ReleasePosition, WidgetCenter);
    FString tapTime = FString::SanitizeFloat(1000 * (t - TouchStartTimestamp));
    OwningWidget->OnTap.Broadcast(tapDirection);
    OwningWidget->InputDelay =
        FMath::TruncToInt(1000 * (t - TouchStartTimestamp));
  ...
  // Cancel swipe on touch ended ? Makes you have to swipe farther and more
  // consistently.
  // OwningWidget->OnCancelSwipe.Broadcast();
...
/*
        -----------------TICK---------------------
*/
void SInputPanel2D::Tick(const FGeometry &AllottedGeometry,
  ...
  if (InputState == EInputState::Touch) {
    ...
    if (FramesSinceTouch >= OwningWidget->AutoTriggerTapFrames && bTapGate) {
      ESwipeDirection TapDirection =
          GetSwipeDirection(SwipePosition, WidgetCenter);
      OwningWidget->OnTap.Broadcast(TapDirection);
      bTapGate = false;
      StateTransition(EInputState::Tap);
    }
  ...
  // Swipe -> Hold
  //	if (InputState == EInputState::Swipe && SwipeTimeStamp + SwipeTimeOut <=
  //t) 		StateTransition(EInputState::Hold);
...
int32 SInputPanel2D::OnPaint(const FPaintArgs &Args,
  ...
  FVector2D VisualSize =
      FVector2D(JoystickThumbRadius * 2.f, JoystickThumbRadius * 2.f);
  FVector2D JoystickBoundsSize = JoystickLimit * 2.f;

  // Draw virtual joystick thumb and backgroud images
  if (InputState != EInputState::None && InputState != EInputState::Tap &&
      InputState != EInputState::Touch) {
    FSlateDrawElement::MakeBox(
        OutDrawElements, RetLayerId++,
        AllottedGeometry.ToPaintGeometry(
            VisualSize,
            FSlateLayoutTransform(JoystickPosition - (VisualSize * 0.5f))),
    ...
    FSlateDrawElement::MakeBox(
        ...
        AllottedGeometry.ToPaintGeometry(
            ...
            FSlateLayoutTransform(InitialTouchPosition -
                                  (JoystickBoundsSize * 0.5f))),
        &(OwningWidget->JoystickBoundsImage));
  }

  // Ghosted backgroud indicating tap quadrants
  if (InputState != EInputState::Tap && InputState != EInputState::None &&
      InputState != EInputState::Touch) {
    FSlateDrawElement::MakeBox(
        OutDrawElements, RetLayerId++,
        AllottedGeometry.ToPaintGeometry(
  ...
  if (bHelpersEnabled) {
    if (InputState != EInputState::Swipe && InputState != EInputState::Hold &&
        InputState != EInputState::Nudge) {
      ...
      if (bUseDiagonalSwipeInputs) {
        TapQuadLineEndPoint0 =
            FVector2D(WidgetCenter.X + lineLength, WidgetCenter.Y);
        TapQuadLineEndPoint1 =
            FVector2D(WidgetCenter.X, WidgetCenter.Y + lineLength);
        TapQuadLineEndPoint2 =
            FVector2D(WidgetCenter.X, WidgetCenter.Y - lineLength);
        TapQuadLineEndPoint3 =
            FVector2D(WidgetCenter.X - lineLength, WidgetCenter.Y);
      } else {
        TapQuadLineEndPoint0 =
            FVector2D(WidgetCenter.X + lineLength, WidgetCenter.Y + lineLength);
        TapQuadLineEndPoint1 =
            FVector2D(WidgetCenter.X - lineLength, WidgetCenter.Y + lineLength);
        TapQuadLineEndPoint2 =
            FVector2D(WidgetCenter.X + lineLength, WidgetCenter.Y - lineLength);
    ...
    if (InputState == EInputState::Hold || InputState == EInputState::Swipe ||
        InputState == EInputState::Nudge) {
      ...
      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          CirclePoints, ESlateDrawEffect::None, CircleColor,
          true, // bAntialias
          0.5f  // Thickness
      );
      ...
      for (int32 i = 0; i <= NumSegments; ++i) {
        float Angle = (2 * PI * i) / NumSegments;
        CirclePoints.Add(InitialTouchPosition +
      ...
      for (int32 i = 0; i <= NumSegments; ++i) {
        float Angle = (2 * PI * i) / NumSegments;
        CirclePoints.Add(JoystickPosition +
                         FVector2D(FMath::Cos(Angle), FMath::Sin(Angle)) *
                             Radius);
      }
      ...
      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          SwipeLines, ESlateDrawEffect::None, FColor::Green,
          true, // bAntialias
          5.0f  // Thickness
      );
      ...
      if (bUseDiagonalSwipeInputs) {
        ...
        GhostLineEndPoint0 =
            FVector2D(Center.X + lineLength, Center.Y + lineLength);
        GhostLineEndPoint1 =
            FVector2D(Center.X - lineLength, Center.Y + lineLength);
        GhostLineEndPoint2 =
            FVector2D(Center.X + lineLength, Center.Y - lineLength);
      ...
      const FVector2D GhostLineLongSwipeEndPoint0 =
          FVector2D(Center.X + lineLength, Center.Y + lineLength);
      const FVector2D GhostLineLongSwipeEndPoint1 =
          FVector2D(Center.X - lineLength, Center.Y + lineLength);
      const FVector2D GhostLineLongSwipeEndPoint2 =
          FVector2D(Center.X + lineLength, Center.Y - lineLength);
      const FVector2D GhostLineLongSwipeEndPoint3 =
          FVector2D(Center.X - lineLength, Center.Y - lineLength);

      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          GhostLinePoints0, ESlateDrawEffect::None, FLinearColor::Gray,
          true, // bAntialias
          0.5f  // Thickness
      );
      ...
      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          GhostLinePoints1, ESlateDrawEffect::None, FLinearColor::Gray,
          true, // bAntialias
          0.5f  // Thickness
      );
      ...
      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          GhostLinePoints2, ESlateDrawEffect::None, FLinearColor::Gray,
          true, // bAntialias
          0.5f  // Thickness
      );
      ...
      FSlateDrawElement::MakeLines(
          OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(),
          GhostLinePoints3, ESlateDrawEffect::None, FLinearColor::Gray,
          true, // bAntialias
          0.5f  // Thickness
      );
...
bool SInputPanel2D::ForwardGamepadAxis(const FGamepadKeyNames::Type KeyName,
                                       float AnalogValue) {
  const FInputDeviceId PrimaryInputDevice =
      IPlatformInputDeviceMapper::Get().GetPrimaryInputDeviceForUser(
          FSlateApplicationBase::SlateAppPrimaryPlatformUser);
  return FSlateApplication::Get().OnControllerAnalog(
      KeyName, FSlateApplicationBase::SlateAppPrimaryPlatformUser,
      PrimaryInputDevice, AnalogValue);
  return false;
}

ESwipeDirection
SInputPanel2D::GetSwipeDirection(FVector2D _SwipePosition,
                                 FVector2D OriginPosition) const {
...
void SInputPanel2D::StateTransition(EInputState NewState) {
  ...
  GEngine->AddOnScreenDebugMessage(1, 2.0f, FColor::Silver,
                                   *UEnum::GetValueAsString(InputState));
}

void SInputPanel2D::CancelSwipe() { OwningWidget->OnCancelSwipe.Broadcast(); }

bool SInputPanel2D::CheckCurve(FVector2D StartPosition, FVector2D EndPosition,
                               FVector2D OriginPosition, float Tolerance) {
  ...
  // FString::Printf(TEXT("DOT PRODUCT :: %s"),
