...
FReply SInputPanel2D::OnTouchMoved(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	...
	//GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Purple, FString::Printf(TEXT("VELOCITY :: %s"), *FString::SanitizeFloat(swipeVelocity.Length())));

	SwipePosition = swipePosition;
	SwipeTimeStamp = swipeTimeStamp;

	//Start Swipe
	if ((InputState == EInputState::Touch || InputState == EInputState::Hold || InputState == EInputState::Nudge) && swipeVelocity.Length() > MinSwipeVelocity && IsPositionOutsideDeadZone(SwipePosition) && bInputAccept)
	{
	...
	if (SwipePositions.Num() > 0)
	{
		...
		GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Cyan, FString::Printf(TEXT("PREV :: %s, NEXT :: %s"), *FString::SanitizeFloat(PrevSwipeVectorLength), *FString::SanitizeFloat(NextSwipeVectorLength)));
	...
	if (InputState == EInputState::Swipe && IsPositionOutsideDeadZone(SwipePosition))
	{
		...
		if (SwipePositions.Num() > 0)
		{
			...
			else if (SwipePositions.Num() > 1)
			{
				...
				float PrevSwipeVectorLength = (PrevSwipePos - InitialTouchPosition).Length();
				float NextSwipeVectorLength = (SwipePositions[SwipePositions.Num() - 1] - InitialTouchPosition).Length();

				//Check for cancel swipe
				if (NextSwipeVectorLength < PrevSwipeVectorLength)
				{
					OwningWidget->OnCancelSwipe.Broadcast();
					GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Orange, FString::Printf(TEXT("----SWIPE CANCEL-----")));
...
//					TouchPosition = swipePosition;
	...
	//GEngine->AddOnScreenDebugMessage(20, 2.0f, FColor::Cyan, FString::Printf(TEXT("AXIS :: %s, %s"), *FString::SanitizeFloat(rawAxis.X), *FString::SanitizeFloat(rawAxis.Y)));
...
void SInputPanel2D::TriggerSwipeEvent() 
{
	...
	if (swipeLength > ShortSwipeThreshold && CheckCurve(SwipePositions[0], SwipePositions[SwipePositions.Num() - 1], InitialTouchPosition, CurveTolerance))
...
int32 SInputPanel2D::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry,
								   ...
								   bool bParentEnabled) const
{
	...
	if (InputState != EInputState::None && InputState != EInputState::Tap && InputState != EInputState::Touch)
	{
		...
		FSlateDrawElement::MakeBox(
			OutDrawElements,
			RetLayerId++,
			AllottedGeometry.ToPaintGeometry(JoystickBoundsSize, FSlateLayoutTransform(InitialTouchPosition - (JoystickBoundsSize * 0.5f))),
	...
	if (InputState != EInputState::Tap && InputState != EInputState::None && InputState != EInputState::Touch)
	{
		FSlateDrawElement::MakeBox(
			OutDrawElements,
			RetLayerId++,
			AllottedGeometry.ToPaintGeometry(OwningWidget->QuadrantGhost.ImageSize, FSlateLayoutTransform(InitialTouchPosition - (FVector2D(OwningWidget->QuadrantGhost.ImageSize * 0.5f)))),
			&(OwningWidget->QuadrantGhost),
	...
	if (bHelpersEnabled)
	{
		...
		if (InputState == EInputState::Hold || InputState == EInputState::Swipe || InputState == EInputState::Nudge)
		{
			...
			// Draw polygon
		//	FSlateResourceHandle ResourceHandle = FSlateApplication::Get().GetRenderer()->GetResourceHandle(*GhostMat);
		//
		...
		//
		//	Vertices.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(AllottedGeometry.ToPaintGeometry().GetAccumulatedRenderTransform(), P0, FVector2f::ZeroVector, FillColor));
		//	Vertices.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(AllottedGeometry.ToPaintGeometry().GetAccumulatedRenderTransform(), P1, FVector2f::ZeroVector, FillColor));
		//	Vertices.Add(FSlateVertex::Make<ESlateVertexRounding::Disabled>(AllottedGeometry.ToPaintGeometry().GetAccumulatedRenderTransform(), P2, FVector2f::ZeroVector, FillColor));
		//
		//	Indices.Add(0);
		//	Indices.Add(1);
		//	Indices.Add(2);
...
{
	bool xCheck = PositionToCheck.X <= InitialTouchPosition.X - DeadZone.X || PositionToCheck.X >= InitialTouchPosition.X + DeadZone.X;
	bool yCheck = PositionToCheck.Y <= InitialTouchPosition.Y - DeadZone.Y || PositionToCheck.Y >= InitialTouchPosition.Y + DeadZone.Y;
...
bool SInputPanel2D::CheckCurve(FVector2D StartPosition, FVector2D EndPosition, FVector2D OriginPosition, float Tolerance)
{
	...
	//GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Orange, FString::Printf(TEXT("DOT PRODUCT :: %s"), *FString::SanitizeFloat(DotProd)));
