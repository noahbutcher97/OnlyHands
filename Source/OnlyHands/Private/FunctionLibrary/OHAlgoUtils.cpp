...
FOHMotionSample UOHAlgoUtils::IntegrateSemiImplicitEulerSample(const FOHMotionSample& Current, float DeltaTime)
{
	...
	return FOHMotionSample::CreateFromState(
		FTransform(Current.GetRotation(), NewPos),
		NewVel,
		Current.GetAngularVelocity(),
		Current.GetLinearAcceleration(),
		Current.GetAngularAcceleration(),
...
{
	const FVector NewPos = 2 * Current.GetLocation() - PreviousPosition + Current.GetLinearAcceleration() * FMath::Square(DeltaTime);
...
{
	const FVector NewPos = Current.GetLocation() + Current.GetLinearVelocity() * DeltaTime + 0.5f * Current.GetLinearAcceleration() * FMath::Square(DeltaTime);
	const FVector NewVel = Current.GetLinearVelocity() + 0.5f * (Current.GetLinearAcceleration() + NextAcceleration) * DeltaTime;
	...
	float DeltaTime)
{
	...
	return FOHMotionSample::CreateFromState(
		FTransform(Current.GetRotation(), FinalPos),
		FinalVel,
		Current.GetAngularVelocity(),
		FinalAccel,
		Current.GetAngularAcceleration(),
...
{
	const FVector NewPos = Current.GetLocation() + (Current.GetLinearVelocity() + Current.GetLinearAcceleration() * DeltaTime) * DeltaTime;
	const FVector NewVel = Current.GetLinearVelocity() + Current.GetLinearAcceleration() * DeltaTime;
...
FVector UOHAlgoUtils::ComputeLinearJerkFromSamples(
...
FVector UOHAlgoUtils::ComputeAngularJerkFromSamples(
	const FOHMotionSample& Newer,
	const FOHMotionSample& Older)
{
	const float DeltaTime = FMath::Max(Newer.GetTimeStamp() - Older.GetTimeStamp(), KINDA_SMALL_NUMBER);
...
{
	if (Samples.Num() < 1) return FVector::ZeroVector;
...
{
	if (Samples.Num() < 3) return FVector::ZeroVector;
...
float UOHAlgoUtils::EstimateTimeToReachTargetLinear(const FOHMotionSample& Current, const FVector& TargetPosition)
{
	...
	if (Speed <= SMALL_NUMBER) return -1.f;
...
{
	const float Speed = Current.GetLinearVelocity().Size();
	if (Speed <= SMALL_NUMBER || DampingRatio <= SMALL_NUMBER) return 0.f;
