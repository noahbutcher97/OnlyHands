...
#pragma region IntegrationUtils
    ...
    UOHAlgoUtils::IntegrateSemiImplicitEuler(const FOHMotionFrameSample& Current, float DeltaTime) {
...
#pragma region IntegrationUtils
                 ...
                 FOHMotionFrameSample UOHAlgoUtils::IntegrateSemiImplicitEuler(const FOHMotionFrameSample& Current,
                                                                               float DeltaTime) {
        FOHMotionFrameSample Next = Current;
        Next.TimeStamp += DeltaTime;
        Next.DeltaTime = DeltaTime;
    ...
    FOHMotionSample UOHAlgoUtils::IntegrateRK4Sample(
        const FOHMotionSample& Current, TFunction<FVector(const FVector&, const FVector&)> AccelFunc, float DeltaTime) {
        return IntegrateRK4Sample<FOHMotionSample, TFunction<FVector(const FVector&, const FVector&)>>(
            Current, AccelFunc, DeltaTime);
    }

    FOHMotionSample UOHAlgoUtils::IntegrateAdaptiveRK4Sample(
        const FOHMotionSample& Current, TFunction<FVector(const FVector&, const FVector&)> AccelFunc,
    ...
    FOHMotionSample UOHAlgoUtils::IntegrateCriticallyDampedSpringSample(
        const FOHMotionSample& Current, const FVector& TargetPosition, float AngularFreq, float DeltaTime) {
        return IntegrateCriticallyDampedSpringSample<FOHMotionSample>(Current, TargetPosition, AngularFreq, DeltaTime);
    }
    ...
    FOHMotionSample UOHAlgoUtils::IntegrateSpringBlendSample(const FOHMotionSample& Current,
                                                             const FVector& TargetPosition, float BlendAlpha,
                                                             float SpringCoeff, float DeltaTime) {
        return IntegrateSpringBlendSample<FOHMotionSample>(Current, TargetPosition, BlendAlpha, SpringCoeff, DeltaTime);
...
#pragma region SolverUtils
    ...
    void UOHAlgoUtils::SolveAndPushChainToTargetTransform(
        const TArray<FName>& ChainBones, TMap<FName, FOHBoneData>& BoneMap, const FTransform& Target,
        float PosStiffness, float RotStiffness, int32 Iterations, float TimeStamp) {
        if (ChainBones.Num() < 2 || Iterations <= 0)
            return;
    ...
    FVector UOHAlgoUtils::ApplyDrag(const FVector& Velocity, float DragCoefficient, float CrossSectionalArea,
                                    float Mass,
<<<<<<< HEAD
                                    float DeltaTime) {
...
void UOHAlgoUtils::CalibrateCoefficientsFromMotion(
    const TArray<FVector>& PreImpactVelocities, const TArray<FVector>& PostImpactVelocities,
    const TArray<FVector>& ImpactNormals, float& OutRestitution, float& OutFriction) {
...
<<<<<<< HEAD FVector UOHAlgoUtils::PredictLinear(const FOHMotionSample& Current, float PredictTime) { return PredictLinear<FOHMotionSample>(Current, PredictTime);
}
...
<<<<<<< HEAD
    ...
    TArray<FVector> UOHAlgoUtils::GetCubicBezierControlPointsFromBoneData(const FOHBoneData& BoneData,
                                                                          float PredictTime) {
        const FVector P0 = BoneData.GetCurrentPosition();
        ...
        == == == = TArray<FVector> UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(
                     const FOHBoneMotionData& BoneData, float PredictTime) {
            const FVector P0 = BoneData.GetCurrentPosition();
            const FVector P2 = PredictQuadratic(*BoneData.GetLatestSample(), PredictTime);
...
<<<<<<< HEAD
        ...
        float UOHAlgoUtils::EstimateTimeToReachTargetLinear(const FOHMotionSample& Current,
...
#pragma region FeedbackControllers
        ...
        FVector UOHAlgoUtils::PIDControl(const FVector& Error, const FVector& ErrorIntegral,
                                         const FVector& ErrorDerivative, float Kp, float Ki, float Kd,
                                         float MaxOutput) {
...
#pragma region ControlAndStabilization
        ...
        FVector UOHAlgoUtils::ComputeCriticallyDampedSpring(const FVector& Pos, const FVector& Vel,
                                                            const FVector& TargetPos, float Stiffness, float Damping) {
...
#pragma region NameResolution
        ...
        FOHNameMatchResult UOHAlgoUtils::FindBestNameMatchUsingStrategy(
            const FString& Target, const TArray<FString>& Candidates, EOHNameMatchingStrategy Strategy) {
            FOHNameMatchResult Best;
            float MaxScore = -1.f;
        ...
        TArray<FOHNameMatchResult> UOHAlgoUtils::BatchMatchTargetsToCandidates(
            const TArray<FString>& Targets, const TArray<FString>& Candidates, EOHNameMatchingStrategy Strategy) {
        ...
        TMap<FString, FOHNameMatchResult> UOHAlgoUtils::MatchTargetAcrossSets(
            const FString& Target, const TMap<FString, TArray<FString>>& CandidateSetsByLabel,
        ...
        TArray<FOHNameMatchResult> UOHAlgoUtils::RankFNameToEnumMap(
            const FString& Target, const TMap<FName, EOHSkeletalBone>& Map, EOHNameMatchingStrategy Strategy,
        ...
        FOHNameMatchResult UOHAlgoUtils::FindBestNameMatchAutoStrategy(const FName& InTarget,
...
<<<<<<< HEAD
            float UOHAlgoUtils::EstimateTimeToReachTarget(const FOHBoneMotionData& MotionData,
                                                          const FVector& TargetPosition) {
...
<<<<<<< HEAD
                    ...
                    if (VelocityTowardTarget <= 0.0f) {
                        if (AccelerationTowardTarget > 0.0f) {
                            ...
                            float DistanceWhileStopping = VelocityTowardTarget * TimeToStop +
                                                          0.5f * AccelerationTowardTarget * TimeToStop * TimeToStop;
                            float RemainingDistance = Distance + FMath::Abs(DistanceWhileStopping);

                            // Estimate time from stopping point
                            float TimeFromStop =
                                FMath::Sqrt(2.0f * RemainingDistance / FMath::Max(AccelerationTowardTarget, 1.0f));
                    ...
                    if (FMath::Abs(AccelerationTowardTarget) > KINDA_SMALL_NUMBER) {
                        float Discriminant =
                            VelocityTowardTarget * VelocityTowardTarget + 2.0f * AccelerationTowardTarget * Distance;
...
<<<<<<< HEAD
                FVector UOHAlgoUtils::EstimateJerkFromMotionData(const FOHBoneMotionData& MotionData,
                                                                 int32 SampleWindow) {
                    ...
                    for (int32 i = 0; i < SampleWindow; i++) {
                        ...
                        if (Current && Next && Current->DeltaTime > KINDA_SMALL_NUMBER) {
                            FVector Acceleration =
                                (Current->WorldLinearVelocity - Next->WorldLinearVelocity) / Current->DeltaTime;
                ...
                TArray<FVector> UOHAlgoUtils::GetBezierControlPointsFromBoneData(const FOHBoneMotionData& MotionData,
                                                                                 float PredictTime, bool bCubic) {
                    ...
                    if (bCubic) {
                        ...
                        FVector MidPoint = CurrentPos + CurrentVel * (PredictTime * 0.5f) +
                ...
                FVector UOHAlgoUtils::PredictCurvedFromMotionData(const FOHBoneMotionData& MotionData,
                                                                  float PredictTime) {
                    ...
                    if (BezierPoints.Num() >= 4) {
                        ...
                        return SampleBezierCubic(BezierPoints[0], BezierPoints[1], BezierPoints[2], BezierPoints[3],
                                                 1.0f);
...
#pragma region Debug
                ...
                void UOHAlgoUtils::DrawDebugQuadraticBezier(const UWorld* World, const TArray<FVector>& Points,
                                                            int32 Steps, FLinearColor Color, float Thickness,
                                                            float Duration) {
