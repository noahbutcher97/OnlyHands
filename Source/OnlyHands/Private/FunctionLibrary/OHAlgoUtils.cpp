...
FOHMotionSample UOHAlgoUtils::IntegrateVerletSample(const FOHMotionSample& Current, const FVector& PreviousPosition, float DeltaTime)
{
	return IntegrateVerletSample<FOHMotionSample>(Current, PreviousPosition, DeltaTime);
}


FOHMotionFrameSample UOHAlgoUtils::IntegrateVelocityVerlet(const FOHMotionFrameSample& Current, const FVector& NextAcceleration, float DeltaTime)
{
	FOHMotionFrameSample Next = Current;
	Next.TimeStamp += DeltaTime;
	Next.DeltaTime = DeltaTime;
	...
	Next.WorldPosition = Current.WorldPosition + 
						Current.WorldLinearVelocity * DeltaTime + 
						Current.WorldLinearAcceleration * (0.5f * DeltaTime * DeltaTime);
    
	// v(t+dt) = v(t) + 0.5*(a(t) + a(t+dt))*dt
	Next.WorldLinearVelocity = Current.WorldLinearVelocity + 
							  (Current.WorldLinearAcceleration + NextAcceleration) * (0.5f * DeltaTime);
...
FOHMotionSample UOHAlgoUtils::IntegrateVelocityVerletSample(const FOHMotionSample& Current, const FVector& NextAcceleration, float DeltaTime)
{
	return IntegrateVelocityVerletSample<FOHMotionSample>(Current, NextAcceleration, DeltaTime);
}


FRK4State UOHAlgoUtils::IntegrateRK4(const FRK4State& InitialState, float DeltaTime, const FVector& ConstantAcceleration)
{
	return IntegrateRK4<FRK4State>(InitialState, DeltaTime, ConstantAcceleration);
}

FOHMotionSample UOHAlgoUtils::IntegrateRK4Sample(
	const FOHMotionSample& Current,
	TFunction<FVector(const FVector&, const FVector&)> AccelFunc,
	float DeltaTime)
{
	return IntegrateRK4Sample<FOHMotionSample, TFunction<FVector(const FVector&, const FVector&)>>(Current, AccelFunc, DeltaTime);
}

FOHMotionSample UOHAlgoUtils::IntegrateAdaptiveRK4Sample(
	const FOHMotionSample& Current,
	TFunction<FVector(const FVector&, const FVector&)> AccelFunc,
...
FOHMotionSample UOHAlgoUtils::IntegrateCriticallyDampedSpringSample(
	const FOHMotionSample& Current,
...
FOHMotionSample UOHAlgoUtils::IntegrateHeunSample(
	const FOHMotionSample& Current,
	TFunction<FVector(const FVector&, const FVector&)> AccelFunc,
	float DeltaTime)
{
	return IntegrateHeunSample<FOHMotionSample, TFunction<FVector(const FVector&, const FVector&)>>(
		Current, AccelFunc, DeltaTime
	);
}


FOHMotionSample UOHAlgoUtils::IntegrateExponentialSample(
	const FOHMotionSample& Current,
	float DampingRatio,
	float DeltaTime)
{
...
FOHMotionSample UOHAlgoUtils::IntegrateSpringBlendSample(
	const FOHMotionSample& Current,
	const FVector& TargetPosition,
	float BlendAlpha,
	float SpringCoeff,
...
void UOHAlgoUtils::ProjectChainToTargetPosition(
	...
	int32 Iterations)
{
	...
	for (int32 It = 0; It < Iterations; ++It)
	{
		...
		for (int32 i = ChainBones.Num() - 1; i >= 0; --i)
		{
			...
			if (!BoneMap.Contains(Bone)) continue;
...
void UOHAlgoUtils::ProjectChainToTargetRotation(
	const TArray<FName>& ChainBones,
	TMap<FName, FOHBoneData>& BoneMap,
...
void UOHAlgoUtils::ProjectChainToTargetTransform(
	const TArray<FName>& ChainBones,
	TMap<FName, FOHBoneData>& BoneMap,
...
void UOHAlgoUtils::SolveAndPushChainToTargetTransform(
	const TArray<FName>& ChainBones,
	TMap<FName, FOHBoneData>& BoneMap,
	const FTransform& Target,
	float PosStiffness,
	...
	float TimeStamp)
{
	if (ChainBones.Num() < 2 || Iterations <= 0) return;
	...
	for (const FName& Bone : ChainBones)
	{
		...
		const FOHMotionSample SolvedSample = FOHMotionSample::CreateFromState(
			FTransform(Rot, Pos),
			BoneData.GetBodyLinearVelocity(),
			BoneData.GetBodyAngularVelocity(),
			BoneData.GetLinearAcceleration(),
			BoneData.GetAngularAcceleration(),
			TimeStamp
		);
...
FVector UOHAlgoUtils::CalculateFrictionForce(const FVector& RelativeVelocity, const FVector& NormalForce,
	float StaticFrictionCoeff, float KineticFrictionCoeff, float StaticThreshold)
{
...
float UOHAlgoUtils::CalculateImpulseMagnitude(
...
FVector UOHAlgoUtils::CalculateImpulseWithFriction(
    ...
    float KineticFriction)
{
    ...
    if (VrelNormal >= 0.0f) return FVector::ZeroVector; // Separating
...
FMaterialPhysicsProperties UOHAlgoUtils::GetBiologicalMaterialProperties(
    EOHBiologicalMaterial MaterialType)
{
...
float UOHAlgoUtils::DeriveRestitutionCoefficient(
	float YoungsModulus1,
	float YoungsModulus2,
	float PoissonsRatio1,
...
void UOHAlgoUtils::DeriveFrictionCoefficients(
	EPhysicalSurface Surface1,
	EPhysicalSurface Surface2,
	float& OutStaticFriction,
	float& OutKineticFriction)
{
	...
	TPair<EPhysicalSurface, EPhysicalSurface> Key = 
		Surface1 <= Surface2 ? 
...
float UOHAlgoUtils::CalculateDragCoefficient(
    EOHShapeType ShapeType,
    float AspectRatio)
{
...
float UOHAlgoUtils::CalculateCrossSectionalArea(
...
float UOHAlgoUtils::EstimateBoneMassFromVolume(
    const FVector& BoundsExtent,
    EOHBiologicalMaterial TissueType,
    float BonePercentage)
{
    ...
    float AverageDensity = TissueProps.Density * (1.0f - BonePercentage) + 
                          BoneProps.Density * BonePercentage;
...
float UOHAlgoUtils::CalculateReducedMass(float Mass1, float Mass2)
{
    ...
    if (Mass2 > 1e6f) return Mass1;
    if (Mass1 > 1e6f) return Mass2;
    
    return (Mass1 * Mass2) / (Mass1 + Mass2);
}

bool UOHAlgoUtils::ValidatePhysicsParameters(
    float Restitution,
    float StaticFriction,
    float KineticFriction,
    FString& OutWarnings)
...
void UOHAlgoUtils::CalibrateCoefficientsFromMotion(
    const TArray<FVector>& PreImpactVelocities,
    const TArray<FVector>& PostImpactVelocities,
    const TArray<FVector>& ImpactNormals,
    float& OutRestitution,
    float& OutFriction)
{
    if (PreImpactVelocities.Num() != PostImpactVelocities.Num() || 
        PreImpactVelocities.Num() != ImpactNormals.Num() ||
...
void UOHAlgoUtils::CalculatePostCollisionVelocities(const FVector& Velocity1, const FVector& Velocity2, float Mass1,
	float Mass2, const FVector& CollisionNormal, float Restitution, FVector& OutVelocity1, FVector& OutVelocity2)
{
...
void UOHAlgoUtils::CalculateImpactEnergyDistribution(const FHitResult& Hit, float& OutKineticEnergyTransfer,
	float& OutRotationalEnergy, float& OutDeformationEnergy, float& OutSoundEnergy, float& OutHeatEnergy)
{
	// Initialize
...
{
	if (MotionData.MotionHistory.NumFrames() < 3) return 0.0f;
    
	// Get three consecutive samples
	const FOHMotionFrameSample* S0 = MotionData.MotionHistory.GetLatest(FMath::Min(2, MotionData.MotionHistory.NumFrames() - 1));
	...
	if (VelMag < KINDA_SMALL_NUMBER) return 0.0f;
...
{
	if (MotionData.GetHistoryDepth() < 3) return false;
	...
	if (!Current || !Previous1 || !Previous2) return false;
...
FVector UOHAlgoUtils::SampleBezierCubic(const FVector& P0, const FVector& P1, const FVector& P2, const FVector& P3, float T)
{
	const FVector A = FMath::Lerp(P0, P1, T);
	const FVector B = FMath::Lerp(P1, P2, T);
...
TArray<FVector> UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(const FOHBoneData& BoneData, float PredictTime)
{
...
TArray<FVector> UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(const FOHBoneMotionData& BoneData, float PredictTime)
{
	const FVector P0 = BoneData.GetCurrentPosition();
	const FVector P2 = PredictQuadratic(*BoneData.GetLatestSample(), PredictTime);
	const FVector P1 = P0 + BoneData.GetWorldLinearVelocity() * (PredictTime * 0.5f);
...
TArray<FVector> UOHAlgoUtils::GetCubicBezierControlPointsFromBoneData(const FOHBoneMotionData& BoneData, float PredictTime)
{
	const FVector P0 = BoneData.GetCurrentPosition();
	const FVector P3 = PredictQuadratic(*BoneData.GetLatestSample(), PredictTime);
	const FVector P1 = P0 + BoneData.GetWorldLinearVelocity() * (PredictTime / 3.f);
	const FVector P2 = P3 - BoneData.GetWorldLinearVelocity() * (PredictTime / 3.f);
...
FVector UOHAlgoUtils::PIDControl(
	const FVector& Error,
...
FVector UOHAlgoUtils::ComputeCriticallyDampedSpring(const FVector& Pos, const FVector& Vel, const FVector& TargetPos, float Stiffness, float Damping)
{
...
{
	return FVector(
		FMath::Abs(Value.X) > Threshold ? Value.X : 0.f,
		FMath::Abs(Value.Y) > Threshold ? Value.Y : 0.f,
...
int32 UOHAlgoUtils::LevenshteinDistance(const FString& A, const FString& B)
{
	...
	DP.SetNum(LenA + 1);
	for (int32 i = 0; i <= LenA; ++i) DP[i].SetNumZeroed(LenB + 1);

	for (int32 i = 0; i <= LenA; ++i) DP[i][0] = i;
	for (int32 j = 0; j <= LenB; ++j) DP[0][j] = j;
...
float UOHAlgoUtils::JaccardIndex(const FString& A, const FString& B)
{
	...
	for (TCHAR C : A) SetA.Add(C);
	for (TCHAR C : B) SetB.Add(C);
...
FString UOHAlgoUtils::SoundexCode(const FString& Input)
{
	...
	while (Encoded.Len() < 4) Encoded.AppendChar('0');
	if (Encoded.Len() > 4) Encoded = Encoded.Left(4);
...
FString UOHAlgoUtils::MetaphoneCode(const FString& Input)
{
	...
	for (int32 i = 0; i < Word.Len(); ++i)
	{
		...
		if (C == 'P' && Next == 'H') { Append('F'); ++i; continue; }
		if (C == 'C' && Next == 'H') { Append('X'); ++i; continue; }
		if (C == 'S' && Next == 'H') { Append('X'); ++i; continue; }
		if (C == 'G' && Next == 'H') { Append('F'); ++i; continue; }
		if (C == 'T' && Next == 'H') { Append('0'); ++i; continue; }

		switch (C)
		{
		case 'B': case 'F': case 'P': case 'V': Append('1'); break;
		case 'C': case 'G': case 'J': case 'K': case 'Q': case 'S': case 'X': case 'Z': Append('2'); break;
		case 'D': case 'T': Append('3'); break;
		case 'L': Append('4'); break;
		case 'M': case 'N': Append('5'); break;
		case 'R': Append('6'); break;
		default: break; // ignore vowels and silent letters
		}
	}
...
float UOHAlgoUtils::CosineTFIDFSimilarity(const FString& A, const FString& B, const TArray<FString>& Corpus)
{
	...
	for (const FString& Token : TokensA) TfA.FindOrAdd(Token) += 1.f;
	for (const FString& Token : TokensB) TfB.FindOrAdd(Token) += 1.f;
	...
	for (const TPair<FString, float>& Pair : TfA)
	{
		...
		for (const FString& Doc : Corpus)
			if (Doc.Contains(Token)) ++DocsWith;
...
EOHNameMatchingStrategy UOHAlgoUtils::DetermineBestMatchingStrategy(const FString& Target, const TArray<FString>& Candidates)
{
...
FOHNameMatchResult UOHAlgoUtils::FindBestNameMatchUsingStrategy(
	...
	EOHNameMatchingStrategy Strategy)
{
	...
	for (const FString& C : Candidates)
	{
		...
		case EOHNameMatchingStrategy::Levenshtein:
			Score = 1.f - static_cast<float>(LevenshteinDistance(Target, C)) / FMath::Max(1, FMath::Max(Target.Len(), C.Len()));
			Best.AlgorithmUsed = TEXT("Levenshtein");
			break;

		case EOHNameMatchingStrategy::DamerauLevenshtein:
			Score = 1.f - static_cast<float>(DamerauLevenshteinDistance(Target, C)) / FMath::Max(1, FMath::Max(Target.Len(), C.Len()));
...
TArray<FOHNameMatchResult> UOHAlgoUtils::BatchMatchTargetsToCandidates(
...
TMap<FString, FOHNameMatchResult> UOHAlgoUtils::MatchTargetAcrossSets(
	const FString& Target,
...
TMap<FString, FOHNameMatchResult> UOHAlgoUtils::BatchAutoMatch(
	const TArray<FString>& Targets,
	const TArray<FString>& Candidates)
{
	TMap<FString, FOHNameMatchResult> Results;
...
TArray<FOHNameMatchResult> UOHAlgoUtils::ScoreMatchesAgainstCandidates(
	const FString& Target,
	const TArray<FString>& Candidates,
	EOHNameMatchingStrategy Strategy)
{
	...
	for (const FString& Candidate : Candidates)
	{
		...
		Results.Emplace(Candidate, Score, StaticEnum<EOHNameMatchingStrategy>()->GetNameStringByValue(static_cast<int64>(Strategy)));
	}

	Results.Sort([](const FOHNameMatchResult& A, const FOHNameMatchResult& B)
	{
...
TArray<FOHNameMatchResult> UOHAlgoUtils::RankFNameToEnumMap(
	const FString& Target,
...
float UOHAlgoUtils::GetContainsScore(const FString& A, const FString& B)
{
	...
	if (BLower.Contains(ALower)) return 0.9f;
	if (ALower.Contains(BLower)) return 0.8f;
	return 0.f;
}
...
float UOHAlgoUtils::GetStrategyWeight(EOHNameMatchingStrategy Strategy)
{
	switch (Strategy)
	{
	case EOHNameMatchingStrategy::Prefix: return 1.25f;
	case EOHNameMatchingStrategy::Suffix: return 1.2f;
	case EOHNameMatchingStrategy::Contains: return 1.1f;
	case EOHNameMatchingStrategy::Levenshtein: return 0.95f;
	case EOHNameMatchingStrategy::DamerauLevenshtein: return 0.9f;
	case EOHNameMatchingStrategy::JaroWinkler: return 0.85f;
	case EOHNameMatchingStrategy::Jaccard: return 0.8f;
	case EOHNameMatchingStrategy::Bigram: return 0.75f;
	case EOHNameMatchingStrategy::Soundex: return 0.5f;
	case EOHNameMatchingStrategy::Metaphone: return 0.5f;
	case EOHNameMatchingStrategy::CosineTFIDF: return 0.4f;
	default: return 0.1f;
	}
}
...
const TArray<EOHNameMatchingStrategy>& UOHAlgoUtils::GetAllMatchingStrategies()
{
	static const TArray<EOHNameMatchingStrategy> AllStrategies = {
		EOHNameMatchingStrategy::Exact,
		EOHNameMatchingStrategy::Prefix,
		EOHNameMatchingStrategy::Suffix,
		EOHNameMatchingStrategy::Contains,
		EOHNameMatchingStrategy::Levenshtein,
		EOHNameMatchingStrategy::DamerauLevenshtein,
		EOHNameMatchingStrategy::JaroWinkler,
		EOHNameMatchingStrategy::Jaccard,
		EOHNameMatchingStrategy::Bigram,
		EOHNameMatchingStrategy::Soundex,
		EOHNameMatchingStrategy::Metaphone,
		EOHNameMatchingStrategy::CosineTFIDF
	};
...
FOHNameMatchResult UOHAlgoUtils::FindBestNameMatchAutoStrategy(const FString& Target, const TArray<FString>& Candidates)
{
	FOHNameMatchResult Best;
	Best.Score = -1.f;
	...
	for (int32 EnumIndex = 0; EnumIndex < StaticEnum<EOHNameMatchingStrategy>()->NumEnums(); ++EnumIndex)
	{
		...
		{
			Best = Match;
			Best.AlgorithmUsed = StaticEnum<EOHNameMatchingStrategy>()->GetNameStringByValue(static_cast<int64>(Strategy));
		}
	}
	...
	if (Best.Score < 0.6f)
	{
		if (CandidatesUseNamingConvention(Candidates))
		{
			FOHNameMatchResult Alt = FindBestNameMatchUsingStrategy(Target, Candidates, EOHNameMatchingStrategy::Prefix);
		...
		{
			FOHNameMatchResult Alt = FindBestNameMatchUsingStrategy(Target, Candidates, EOHNameMatchingStrategy::Contains);
			if (Alt.Score > Best.Score)
			{
				Best = Alt;
...
{
    if (MotionData.GetHistoryDepth() < 3) return 0.0f;
        ...
        if (Current && Next)
        {
            ...
            if (!Current->LocalLinearVelocity.IsNearlyZero() && !Next->LocalLinearVelocity.IsNearlyZero())
            {
                float DotProduct = FVector::DotProduct(
                    Current->LocalLinearVelocity.GetSafeNormal(), 
    ...
    if (ValidComparisons == 0) return 0.0f;
...
float UOHAlgoUtils::EstimateTimeToReachTarget(const FOHBoneMotionData& MotionData, const FVector& TargetPosition)
{
    ...
    if (!Latest) return -1.0f;
    
    FVector ToTarget = TargetPosition - Latest->WorldPosition;
    float Distance = ToTarget.Size();
    
    if (Distance < 1.0f) return 0.0f; // Already at target
    ...
    if (VelocityTowardTarget <= 0.0f)
    {
        if (AccelerationTowardTarget > 0.0f)
        {
            ...
            float DistanceWhileStopping = VelocityTowardTarget * TimeToStop + 0.5f * AccelerationTowardTarget * TimeToStop * TimeToStop;
    ...
    if (FMath::Abs(AccelerationTowardTarget) > KINDA_SMALL_NUMBER)
    {
        ...
        if (Discriminant >= 0.0f)
        {
            ...
            if (T1 > 0.0f && T2 > 0.0f) return FMath::Min(T1, T2);
            else if (T1 > 0.0f) return T1;
...
{
	if (MotionData.GetHistoryDepth() < SampleWindow + 1) return FVector::ZeroVector;
	...
	if (Accelerations.Num() < 2) return FVector::ZeroVector;
...
TArray<FVector> UOHAlgoUtils::GetBezierControlPointsFromBoneData(const FOHBoneMotionData& MotionData, float PredictTime, bool bCubic)
{
	TArray<FVector> ControlPoints;
	const FOHMotionFrameSample* Latest = MotionData.GetLatestSample();
    
	if (!Latest) return ControlPoints;
	...
	if (bCubic)
	{
		...
		FVector MidPoint = CurrentPos + CurrentVel * (PredictTime * 0.5f) + CurrentAccel * (PredictTime * PredictTime * 0.125f);
	...
	else
	{
		...
		// P1 - control point
		ControlPoints.Add(CurrentPos + CurrentVel * (PredictTime * 0.5f) + CurrentAccel * (PredictTime * PredictTime * 0.125f));
...
float UOHAlgoUtils::GetTrajectoryDeviation(const FOHBoneMotionData& MotionData, int32 MaxSamples)
{
	...
	if (TrajectoryLength < KINDA_SMALL_NUMBER) return 0.0f;
...
void UOHAlgoUtils::DrawDebugQuadraticBezier(const UWorld* World, const TArray<FVector>& Points, int32 Steps, FLinearColor Color, float Thickness, float Duration)
{
	if (Points.Num() != 3 || !World) return;
...
void UOHAlgoUtils::DrawDebugCubicBezier(const UWorld* World, const TArray<FVector>& Points, int32 Steps, FLinearColor Color, float Thickness, float Duration)
{
	if (Points.Num() != 4 || !World) return;
