...
int32 UOHAlgoUtils::GetBoneHierarchyDepth(const USkeletalMeshComponent *Mesh,
                                          FName Bone) {
  if (!Mesh || Bone.IsNone())
    return 0;
...
FOHMotionSample
UOHAlgoUtils::IntegrateSemiImplicitEulerSample(const FOHMotionSample &Current,
                                               float DeltaTime) {
  const FVector NewVel =
      Current.GetLinearVelocity() + Current.GetLinearAcceleration() * DeltaTime;
  const FVector NewPos = Current.GetLocation() + NewVel * DeltaTime;

  return FOHMotionSample::CreateFromState(
      FTransform(Current.GetRotation(), NewPos), NewVel,
      Current.GetAngularVelocity(), Current.GetLinearAcceleration(),
      Current.GetAngularAcceleration(), Current.GetTimeStamp() + DeltaTime);
}

FOHMotionSample
UOHAlgoUtils::IntegrateVerletSample(const FOHMotionSample &Current,
                                    const FVector &PreviousPosition,
                                    float DeltaTime) {
  const FVector NewPos =
      2 * Current.GetLocation() - PreviousPosition +
      Current.GetLinearAcceleration() * FMath::Square(DeltaTime);
...
FOHMotionSample
UOHAlgoUtils::IntegrateVelocityVerletSample(const FOHMotionSample &Current,
                                            const FVector &NextAcceleration,
                                            float DeltaTime) {
  const FVector NewPos =
      Current.GetLocation() + Current.GetLinearVelocity() * DeltaTime +
      0.5f * Current.GetLinearAcceleration() * FMath::Square(DeltaTime);
  const FVector NewVel =
      Current.GetLinearVelocity() +
      0.5f * (Current.GetLinearAcceleration() + NextAcceleration) * DeltaTime;
...
FOHMotionSample UOHAlgoUtils::IntegrateRK4Sample(
  ...
  const FVector FinalPos =
      x0 + (DeltaTime / 6.f) * (v0 + 2.f * v1 + 2.f * v2 + v3);
  const FVector FinalVel =
      v0 + (DeltaTime / 6.f) * (a0 + 2.f * a1 + 2.f * a2 + a3);
  const FVector FinalAccel = AccelFunc(FinalPos, FinalVel);

  return FOHMotionSample::CreateFromState(
      FTransform(Current.GetRotation(), FinalPos), FinalVel,
      Current.GetAngularVelocity(), FinalAccel,
      Current.GetAngularAcceleration(), Current.GetTimeStamp() + DeltaTime);
}

FOHMotionSample
UOHAlgoUtils::IntegrateImplicitEulerSample(const FOHMotionSample &Current,
                                           float DeltaTime) {
  const FVector NewPos =
      Current.GetLocation() + (Current.GetLinearVelocity() +
                               Current.GetLinearAcceleration() * DeltaTime) *
                                  DeltaTime;
  const FVector NewVel =
      Current.GetLinearVelocity() + Current.GetLinearAcceleration() * DeltaTime;
...
FOHMotionSample UOHAlgoUtils::IntegrateCriticallyDampedSpringSample(
    const FOHMotionSample &Current, const FVector &TargetPosition,
  ...
  const FVector NewPos =
      TargetPosition +
      (Displacement + Current.GetLinearVelocity() / Omega) * Exp;
...
FOHMotionSample UOHAlgoUtils::IntegrateAdaptiveRK4Sample(
    const FOHMotionSample &Current,
    TFunction<FVector(const FVector &, const FVector &)> AccelFunc,
    float &InOutDeltaTime, float MinStep, float MaxStep, float Tolerance) {
  ...
  // One large step
  FOHMotionSample Large =
      IntegrateRK4Sample(Current, AccelFunc, InOutDeltaTime);
...
FOHMotionSample
UOHAlgoUtils::IntegrateExponentialSample(const FOHMotionSample &Current,
...
FOHMotionSample UOHAlgoUtils::IntegrateSpringBlendSample(
  ...
  const FVector BlendedTarget =
      FMath::Lerp(Current.GetLocation(), TargetPosition, BlendAlpha);
  const FVector SpringForce =
      -SpringCoeff * (Current.GetLocation() - BlendedTarget);
...
float UOHAlgoUtils::ComputeBaumgarteTerm(float ConstraintError, float Beta,
                                         float DeltaTime) {
  return -(Beta / DeltaTime) * ConstraintError;
}

void UOHAlgoUtils::ProjectChainToTargetPosition(
    const TArray<FName> &ChainBones, TMap<FName, FOHBoneData> &BoneMap,
...
void UOHAlgoUtils::ProjectChainToTargetRotation(
  ...
  for (int32 It = 0; It < Iterations; ++It) {
    ...
    for (int32 i = ChainBones.Num() - 1; i >= 0; --i) {
      ...
      const FQuat Adjusted = FQuat::Slerp(FQuat::Identity, Delta,
                                          FMath::Clamp(Stiffness, 0.f, 1.f));
...
void UOHAlgoUtils::ProjectChainToTargetTransform(
  ...
  for (int32 It = 0; It < Iterations; ++It) {
    ...
    for (int32 i = ChainBones.Num() - 1; i >= 0; --i) {
      ...
      const FVector PosCorrection =
          ToGoal * FMath::Clamp(PosStiffness, 0.f, 1.f);
      ...
      const FQuat Adjusted = FQuat::Slerp(FQuat::Identity, Delta,
                                          FMath::Clamp(RotStiffness, 0.f, 1.f));
      Data.SetCurrentRotation((Adjusted * Rot).GetNormalized());
...
void UOHAlgoUtils::SolveAndPushChainToTargetTransform(
  ...
  // Step 1: Solve
  ProjectChainToTargetTransform(ChainBones, BoneMap, Target, PosStiffness,
                                RotStiffness, Iterations);
  ...
  for (const FName &Bone : ChainBones) {
    ...
    const FOHMotionSample SolvedSample = FOHMotionSample::CreateFromState(
        FTransform(Rot, Pos), BoneData.GetBodyLinearVelocity(),
        BoneData.GetBodyAngularVelocity(), BoneData.GetLinearAcceleration(),
        BoneData.GetAngularAcceleration(), TimeStamp);
...
FVector
UOHAlgoUtils::ComputeLinearJerkFromSamples(const FOHMotionSample &Newer,
                                           const FOHMotionSample &Older) {
  const float DeltaTime = FMath::Max(
      Newer.GetTimeStamp() - Older.GetTimeStamp(), KINDA_SMALL_NUMBER);
  const FVector DeltaAccel =
      Newer.GetLinearAcceleration() - Older.GetLinearAcceleration();
  return DeltaAccel / DeltaTime;
}

FVector
UOHAlgoUtils::ComputeAngularJerkFromSamples(const FOHMotionSample &Newer,
                                            const FOHMotionSample &Older) {
  const float DeltaTime = FMath::Max(
      Newer.GetTimeStamp() - Older.GetTimeStamp(), KINDA_SMALL_NUMBER);
  const FVector DeltaAccel =
      Newer.GetAngularAcceleration() - Older.GetAngularAcceleration();
  return DeltaAccel / DeltaTime;
}

FVector UOHAlgoUtils::EstimateJerk(const TArray<FOHMotionSample> &Samples,
                                   float DeltaTime) {
...
FVector UOHAlgoUtils::SmoothVelocityEMA(const TArray<FOHMotionSample> &Samples,
                                        float Alpha) {
  if (Samples.Num() < 1)
    return FVector::ZeroVector;
...
bool UOHAlgoUtils::IsVelocityZeroCrossing(
    const TArray<FOHMotionSample> &Samples) {
  if (Samples.Num() < 2)
    return false;
...
float UOHAlgoUtils::GetTrajectoryDeviation(
    const TArray<FOHMotionSample> &Samples) {
  if (Samples.Num() < 3)
    return 0.f;
  ...
  for (int32 i = 1; i < Samples.Num() - 1; ++i) {
    ...
    const FVector ToLine =
        FVector::PointPlaneProject(Point, Start, Line.GetSafeNormal());
...
FVector UOHAlgoUtils::PredictLinear(const FOHMotionSample &Current,
                                    float PredictTime) {
  return Current.GetLocation() + Current.GetLinearVelocity() * PredictTime;
}

FVector UOHAlgoUtils::PredictQuadratic(const FOHMotionSample &Current,
                                       float PredictTime) {
  return Current.GetLocation() + Current.GetLinearVelocity() * PredictTime +
         0.5f * Current.GetLinearAcceleration() * FMath::Square(PredictTime);
}

FVector UOHAlgoUtils::PredictDamped(const FOHMotionSample &Current,
                                    float PredictTime, float DampingRatio) {
...
FVector
UOHAlgoUtils::PredictCurvedFromSamples(const TArray<FOHMotionSample> &Samples,
                                       float PredictTime) {
  if (Samples.Num() < 3)
    return FVector::ZeroVector;
  ...
  const float T = FMath::Clamp(
      PredictTime / (Samples[1].GetTimeStamp() - Samples[0].GetTimeStamp()),
      0.f, 1.f);
  return SampleBezierQuadratic(P0, EstControl, P2, T);
}

FVector UOHAlgoUtils::SampleBezierQuadratic(const FVector &P0,
                                            const FVector &P1,
                                            const FVector &P2, float T) {
  const FVector A = FMath::Lerp(P0, P1, T);
  const FVector B = FMath::Lerp(P1, P2, T);
  return FMath::Lerp(A, B, T);
}

FVector UOHAlgoUtils::SampleBezierCubic(const FVector &P0, const FVector &P1,
...
TArray<FVector> UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(
    const FOHBoneData &BoneData, float PredictTime) {
  const FVector P0 = BoneData.GetCurrentPosition();
  const FVector P2 =
      PredictQuadratic(BoneData.GetLatestMotionSample(), PredictTime);
  const FVector P1 =
      P0 + BoneData.GetBodyLinearVelocity() * (PredictTime * 0.5f);

  return {P0, P1, P2};
}

TArray<FVector> UOHAlgoUtils::GetCubicBezierControlPointsFromBoneData(
    const FOHBoneData &BoneData, float PredictTime) {
  const FVector P0 = BoneData.GetCurrentPosition();
  const FVector P3 =
      PredictQuadratic(BoneData.GetLatestMotionSample(), PredictTime);
  const FVector P1 =
      P0 + BoneData.GetBodyLinearVelocity() * (PredictTime / 3.f);
  const FVector P2 =
      P3 - BoneData.GetBodyLinearVelocity() * (PredictTime / 3.f);

  return {P0, P1, P2, P3};
}

float UOHAlgoUtils::EstimateTimeToReachTargetLinear(
    const FOHMotionSample &Current, const FVector &TargetPosition) {
  ...
  const float Distance =
      FVector::DotProduct(Dir, Current.GetLinearVelocity().GetSafeNormal());
  return Distance / Speed;
}

float UOHAlgoUtils::EstimateTimeToStopDamped(const FOHMotionSample &Current,
                                             float DampingRatio) {
  const float Speed = Current.GetLinearVelocity().Size();
...
FVector UOHAlgoUtils::PIDControl(const FVector &Error,
                                 const FVector &ErrorIntegral,
                                 const FVector &ErrorDerivative, float Kp,
                                 float Ki, float Kd, float MaxOutput) {
...
FVector UOHAlgoUtils::ComputeCriticallyDampedSpring(const FVector &Pos,
                                                    const FVector &Vel,
...
float UOHAlgoUtils::ComputeBaumgarteStabilization(float ConstraintError,
                                                  float Beta, float DeltaTime) {
  return -(Beta / DeltaTime) * ConstraintError;
}

FVector UOHAlgoUtils::ApplyDeadzone(const FVector &Value, float Threshold) {
  return FVector(FMath::Abs(Value.X) > Threshold ? Value.X : 0.f,
...
int32 UOHAlgoUtils::LevenshteinDistance(const FString &A, const FString &B) {
  ...
  for (int32 i = 1; i <= LenA; ++i)
    for (int32 j = 1; j <= LenB; ++j)
      DP[i][j] = FMath::Min3(DP[i - 1][j] + 1, DP[i][j - 1] + 1,
                             DP[i - 1][j - 1] + (A[i - 1] == B[j - 1] ? 0 : 1));

  return DP[LenA][LenB];
}

int32 UOHAlgoUtils::DamerauLevenshteinDistance(const FString &A,
                                               const FString &B) {
  const int32 LenA = A.Len(), LenB = B.Len();
  TArray<TArray<int32>> DP;
  DP.SetNum(LenA + 1);
...
float UOHAlgoUtils::JaccardIndex(const FString &A, const FString &B) {
  ...
  return Union.Num() > 0 ? static_cast<float>(Intersection.Num()) / Union.Num()
                         : 0.f;
}

FOHNameMatchResult
UOHAlgoUtils::FindBestNameMatch(const FString &Target,
...
float UOHAlgoUtils::CosineTFIDFSimilarity(const FString &A, const FString &B,
                                          const TArray<FString> &Corpus) {
  TMap<FString, float> TfA, TfB, Idf;

  TSet<FString> StopWords = {TEXT("the"), TEXT("is"), TEXT("a"),  TEXT("an"),
                             TEXT("of"),  TEXT("in"), TEXT("and")};
  ...
  return (MagA > 0 && MagB > 0) ? Dot / (FMath::Sqrt(MagA) * FMath::Sqrt(MagB))
                                : 0.f;
}

EOHNameMatchingStrategy
UOHAlgoUtils::DetermineBestMatchingStrategy(const FString &Target,
                                            const TArray<FString> &Candidates) {
  ...
  for (int32 EnumIndex = 0;
       EnumIndex < StaticEnum<EOHNameMatchingStrategy>()->NumEnums();
       ++EnumIndex) {
    EOHNameMatchingStrategy Strategy =
        static_cast<EOHNameMatchingStrategy>(EnumIndex);
    const TArray<FOHNameMatchResult> Scored =
        ScoreMatchesAgainstCandidates(Target, Candidates, Strategy);
...
FOHNameMatchResult
UOHAlgoUtils::FindBestNameMatchUsingStrategy(const FString &Target,
                                             const TArray<FString> &Candidates,
                                             EOHNameMatchingStrategy Strategy) {
  FOHNameMatchResult Best;
  float MaxScore = -1.f;
...
TArray<FOHNameMatchResult>
UOHAlgoUtils::BatchMatchTargetsToCandidates(const TArray<FString> &Targets,
                                            const TArray<FString> &Candidates,
                                            EOHNameMatchingStrategy Strategy) {
  ...
  for (const FString &Target : Targets) {
    FOHNameMatchResult Match =
        FindBestNameMatchUsingStrategy(Target, Candidates, Strategy);
    Results.Add(Match);
  }
...
UOHAlgoUtils::BatchAutoMatch(const TArray<FString> &Targets,
                             const TArray<FString> &Candidates) {
  TMap<FString, FOHNameMatchResult> Results;

  for (const FString &Target : Targets) {
    EOHNameMatchingStrategy BestStrategy =
        DetermineBestMatchingStrategy(Target, Candidates);
    FOHNameMatchResult Match =
        FindBestNameMatchUsingStrategy(Target, Candidates, BestStrategy);
...
TArray<FOHNameMatchResult>
UOHAlgoUtils::ScoreMatchesAgainstCandidates(const FString &Target,
                                            const TArray<FString> &Candidates,
                                            EOHNameMatchingStrategy Strategy) {
  TArray<FOHNameMatchResult> Results;
  ...
  for (const FString &Candidate : Candidates) {
    ...
    Results.Emplace(Candidate, Score,
                    StaticEnum<EOHNameMatchingStrategy>()->GetNameStringByValue(
                        static_cast<int64>(Strategy)));
  }
...
TArray<FOHNameMatchResult> UOHAlgoUtils::RankFNameToEnumMap(
    const FString &Target, const TMap<FName, EOHSkeletalBone> &Map,
  ...
  TArray<FOHNameMatchResult> Ranked =
      ScoreMatchesAgainstCandidates(Target, Candidates, Strategy);
  Ranked.RemoveAll(
      [=](const FOHNameMatchResult &R) { return R.Score < MinScoreThreshold; });
  return Ranked;
}

float UOHAlgoUtils::GetNormalizedLevenshteinScore(const FString &A,
                                                  const FString &B) {
  const int32 Distance = LevenshteinDistance(A, B);
  const int32 MaxLen = FMath::Max(A.Len(), B.Len());
  return MaxLen > 0
             ? 1.0f - static_cast<float>(Distance) / static_cast<float>(MaxLen)
             : 0.f;
}
...
const TArray<EOHNameMatchingStrategy> &
UOHAlgoUtils::GetAllMatchingStrategies() {
...
bool UOHAlgoUtils::CandidatesAreTokenized(const TArray<FString> &Candidates) {
  ...
  return static_cast<float>(Tokenized) / static_cast<float>(Candidates.Num()) >
         0.6f;
}

bool UOHAlgoUtils::CandidatesUseNamingConvention(
    const TArray<FString> &Candidates) {
  for (const FString &C : Candidates) {
...
FOHNameMatchResult
UOHAlgoUtils::FindBestNameMatchAutoStrategy(const FString &Target,
                                            const TArray<FString> &Candidates) {
  FOHNameMatchResult Best;
  Best.Score = -1.f;
  ...
  for (int32 EnumIndex = 0;
       EnumIndex < StaticEnum<EOHNameMatchingStrategy>()->NumEnums();
       ++EnumIndex) {
    const EOHNameMatchingStrategy Strategy =
        static_cast<EOHNameMatchingStrategy>(EnumIndex);
  ...
  if (Best.Score < 0.6f) {
    if (CandidatesUseNamingConvention(Candidates)) {
      FOHNameMatchResult Alt = FindBestNameMatchUsingStrategy(
          Target, Candidates, EOHNameMatchingStrategy::Prefix);
      ...
      FOHNameMatchResult Alt = FindBestNameMatchUsingStrategy(
          Target, Candidates, EOHNameMatchingStrategy::Contains);
      if (Alt.Score > Best.Score) {
        Best = Alt;
...
FOHNameMatchResult
UOHAlgoUtils::FindBestNameMatchAutoStrategy(const FName &InTarget,
...
void UOHAlgoUtils::DrawDebugQuadraticBezier(const UWorld *World,
                                            const TArray<FVector> &Points,
                                            int32 Steps, FLinearColor Color,
  ...
  for (int32 i = 0; i < Steps; ++i) {
    ...
    const FVector P0 =
        SampleBezierQuadratic(Points[0], Points[1], Points[2], T0);
    const FVector P1 =
        SampleBezierQuadratic(Points[0], Points[1], Points[2], T1);

    DrawDebugLine(World, P0, P1, Color.ToFColor(true), false, Duration, 0,
                  Thickness);
  }
}

void UOHAlgoUtils::DrawDebugCubicBezier(const UWorld *World,
                                        const TArray<FVector> &Points,
                                        int32 Steps, FLinearColor Color,
                                        float Thickness, float Duration) {
  ...
  for (int32 i = 0; i < Steps; ++i) {
    ...
    const FVector P0 =
        SampleBezierCubic(Points[0], Points[1], Points[2], Points[3], T0);
    const FVector P1 =
        SampleBezierCubic(Points[0], Points[1], Points[2], Points[3], T1);
