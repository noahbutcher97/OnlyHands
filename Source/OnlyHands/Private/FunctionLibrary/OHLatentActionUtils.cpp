...
public:
	...
	FLatentRotateComponentTo(USceneComponent* InPivot, FRotator InTarget, float InDuration, float InEaseAlpha, const FLatentActionInfo& LatentInfo)
		: Pivot(InPivot)
		, TargetRotation(InTarget)
		, Duration(FMath::Max(0.01f, InDuration))
		, EaseAlpha(FMath::Clamp(InEaseAlpha, 0.0f, 10.0f))
	...
	virtual void UpdateOperation(FLatentResponse& Response) override
	{
		...
		float ActualAlpha = (EaseAlpha == 1.0f) ? Alpha : UKismetMathLibrary::Ease(0.f, 1.f, Alpha, EEasingFunc::EaseInOut, EaseAlpha);
...
public:
	...
	FLatentMoveComponentTo(USceneComponent* InTarget, FVector InEnd, float InDuration, const FLatentActionInfo& LatentInfo)
	...
	virtual void UpdateOperation(FLatentResponse& Response) override
	{
		if (!Target.IsValid()) { Response.DoneIf(true); return; }
		Elapsed += Response.ElapsedTime();
		float Alpha = FMath::Clamp(Elapsed / Duration, 0.f, 1.f);
		FVector NewLoc = FMath::Lerp(Start, End, Alpha);
		Target->SetWorldLocation(NewLoc);
		Response.DoneIf(Alpha >= 1.f);
		if (Alpha >= 1.f && Target.IsValid()) Target->SetWorldLocation(End);
	}
};
...
public:
	...
	FLatentInterpFloatTo(float* InResultPtr, float InStart, float InEnd, float InDuration, const FLatentActionInfo& LatentInfo)
		: Start(InStart), End(InEnd), Duration(FMath::Max(InDuration, 0.01f)), ResultPtr(InResultPtr),
		  ExecutionFunction(LatentInfo.ExecutionFunction), OutputLink(LatentInfo.Linkage), CallbackTarget(LatentInfo.CallbackTarget) {}

	virtual void UpdateOperation(FLatentResponse& Response) override
	{
		if (!ResultPtr) { Response.DoneIf(true); return; }
		Elapsed += Response.ElapsedTime();
		float Alpha = FMath::Clamp(Elapsed / Duration, 0.f, 1.f);
		*ResultPtr = FMath::Lerp(Start, End, Alpha);
		Response.DoneIf(Alpha >= 1.f);
		if (Alpha >= 1.f) *ResultPtr = End;
	}
};
...
void UOHLatentActionUtils::RotateComponentTo(
...
{
	if (!WorldContextObject || !Pivot) return;

	UWorld* World = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
	if (!World) return;

	FLatentActionManager& LatentManager = World->GetLatentActionManager();
	if (LatentManager.FindExistingAction<FLatentRotateComponentTo>(LatentInfo.CallbackTarget, LatentInfo.UUID) == nullptr)
	{
...
void UOHLatentActionUtils::MoveComponentTo(
	UObject* WorldContextObject,
	...
	FLatentActionInfo LatentInfo)
{
	if (!WorldContextObject || !Target) return;
	UWorld* World = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
	if (!World) return;
	FLatentActionManager& LatentManager = World->GetLatentActionManager();
...
void UOHLatentActionUtils::InterpFloatTo(
	UObject* WorldContextObject,
	float StartValue,
	float TargetValue,
	float Duration,
...
void UOHLatentActionUtils::LatentDelay(
	UObject* WorldContextObject,
	float Duration,
	FLatentActionInfo LatentInfo)
{
