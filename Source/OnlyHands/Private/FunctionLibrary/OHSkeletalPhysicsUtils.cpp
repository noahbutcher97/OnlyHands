...
FName UOHSkeletalPhysicsUtils::ResolveBoneNameFromSkeletalBone(
  ...
  const FReferenceSkeleton &RefSkel =
      SkelComp->GetSkeletalMeshAsset()->GetRefSkeleton();

  // Try ideal mapping first
  if (const FName *Ideal =
          OHSkeletalMappings::SkeletalBoneToFNameMap.Find(Bone)) {
  ...
  FOHNameMatchResult Match = UOHAlgoUtils::FindBestNameMatchAutoStrategy(
      FName(UEnum::GetValueAsString(Bone)), BoneNames);
  return FName(*Match.Candidate);
}

FOHResolvedBoneData
UOHSkeletalPhysicsUtils::ResolveResolvedDataFromSkeletalBone(
    EOHSkeletalBone Bone, const USkeletalMeshComponent *SkelComp) {
  ...
  if (const FName *Ideal =
    ...
    if (RefSkel.FindBoneIndex(*Ideal) != INDEX_NONE) {
      ...
      Result.MatchResult =
          FOHNameMatchResult(Ideal->ToString(), 0.f, TEXT("Direct"));
...
EOHSkeletalBone UOHSkeletalPhysicsUtils::GetRootBoneInBodyPartFromMesh(
    const USkeletalMeshComponent *SkelComp, EOHBodyPart BodyPart) {
  if (!SkelComp || !SkelComp->GetSkeletalMeshAsset())
...
EOHSkeletalBone UOHSkeletalPhysicsUtils::GetEndBoneInBodyPartFromMesh(
    const USkeletalMeshComponent *SkelComp, EOHBodyPart BodyPart) {
  if (!SkelComp || !SkelComp->GetSkeletalMeshAsset())
    return EOHSkeletalBone::None;
...
FName UOHSkeletalPhysicsUtils::ResolveBoneNameFromSkeletalBone_Static(
    EOHSkeletalBone Bone) {
  if (const FName *Found =
          OHSkeletalMappings::SkeletalBoneToFNameMap.Find(Bone)) {
    return *Found;
  }
  ...
  FOHNameMatchResult Match = UOHAlgoUtils::FindBestNameMatchAutoStrategy(
      FName(UEnum::GetValueAsString(Bone)), AllNames);
  return FName(*Match.Candidate);
}

FOHResolvedBoneData
UOHSkeletalPhysicsUtils::ResolveResolvedDataFromSkeletalBone_Static(
    EOHSkeletalBone Bone) {
  FOHResolvedBoneData Result;
  Result.LogicalBone = Bone;
  ...
  if (const FName *Found =
    ...
    Result.MatchResult =
        FOHNameMatchResult(Found->ToString(), 0.f, TEXT("Direct"));
    Result.bResolved = true;
  } else {
    TArray<FName> AllNames;
    OHSkeletalMappings::SkeletalBoneToFNameMap.GenerateValueArray(AllNames);
...
EOHBodyPart UOHSkeletalPhysicsUtils::GetBodyPartFromBone(EOHSkeletalBone Bone) {
  if (const EOHBodyPart *Found =
          OHSkeletalMappings::BoneToBodyPartMap.Find(Bone)) {
    return *Found;
  }
  return EOHBodyPart::None;
}

EOHFunctionalBoneGroup
UOHSkeletalPhysicsUtils::GetFunctionalGroupFromBone(EOHSkeletalBone Bone) {
  if (const EOHFunctionalBoneGroup *Found =
          OHSkeletalMappings::BoneToFunctionalGroupMap.Find(Bone)) {
    return *Found;
  }
...
TArray<FName>
UOHSkeletalPhysicsUtils::GetPrimaryBoneNamesFromBodyPart(EOHBodyPart BodyPart) {
  TArray<FName> Result;
  ...
  if (const TArray<EOHSkeletalBone> *Bones =
    ...
    for (EOHSkeletalBone Bone : *Bones) {
      if (const FName *Name =
              OHSkeletalMappings::PrimaryBoneToFNameMap.Find(Bone)) {
        Result.Add(*Name);
      }
    }
  }
...
EOHSkeletalBone UOHSkeletalPhysicsUtils::ResolveSkeletalBoneFromNameSmart(
    const FName &Input, const USkeletalMeshComponent *SkelComp,
  ...
  FOHNameMatchResult Match =
      UOHAlgoUtils::FindBestNameMatchAutoStrategy(Input, AllBoneNames);
  if (Match.Score >= ScoreThreshold) {
    const FName MatchedName(*Match.Candidate);
    for (const TPair<EOHSkeletalBone, FName> &Pair :
         OHSkeletalMappings::SkeletalBoneToFNameMap) {
...
bool UOHSkeletalPhysicsUtils::GetBoneCollisionShape(
    UPhysicsAsset *PhysAsset, FName BoneName, FTransform &OutBoneTransform,
  ...
  const USkeletalBodySetup *BodySetup =
      PhysAsset->SkeletalBodySetups[BodyIndex];
  ...
  if (Capsule) {
    ...
    OutShape =
        FCollisionShape::MakeCapsule(Capsule->Radius, Capsule->Length * 0.5f);
    return true;
  }
...
bool TryGetBoneReferenceFromEnum_Internal(EOHSkeletalBone BoneEnum,
...
TArray<FConstraintInstance *> UOHSkeletalPhysicsUtils::GetAllParentConstraints(
    USkeletalMeshComponent *SkelMesh, FName BoneName) {
  ...
  const FReferenceSkeleton &RefSkel =
      SkelMesh->GetSkeletalMeshAsset()->GetRefSkeleton();
  ...
  while (MyIdx != INDEX_NONE) {
    ...
    for (UPhysicsConstraintTemplate *ConstraintTemplate :
         PhysAsset->ConstraintSetup) {
      if (!ConstraintTemplate)
        continue;
      FConstraintInstance *CI = const_cast<FConstraintInstance *>(
          &ConstraintTemplate->DefaultInstance);
      if ((CI->ConstraintBone1 == ParentName &&
           CI->ConstraintBone2 == ThisName) ||
          (CI->ConstraintBone1 == ThisName &&
           CI->ConstraintBone2 == ParentName)) {
        Result.Add(CI);
        break;
      }
    }
...
TArray<FName>
UOHSkeletalPhysicsUtils::GetAllParentBoneNames(USkeletalMeshComponent *SkelMesh,
                                               FName BoneName) {
...
float UOHSkeletalPhysicsUtils::ComputeBoneLength(
    const USkeletalMeshComponent *SkeletalMesh, FName BoneA, FName BoneB) {
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetChildBonesByDepth(EOHSkeletalBone RootBone,
                                              int32 MaxDepth) {
...
EOHSkeletalBone
UOHSkeletalPhysicsUtils::GetDirectChildBone(EOHSkeletalBone ParentBone) {
  for (EOHSkeletalBone Bone = EOHSkeletalBone::FirstBone;
       Bone <= EOHSkeletalBone::LastBone;
...
int32 UOHSkeletalPhysicsUtils::GetBoneDepthRelativeTo(
    EOHSkeletalBone ParentBone, EOHSkeletalBone TargetBone) {
  if (ParentBone == EOHSkeletalBone::None ||
      TargetBone == EOHSkeletalBone::None) {
    return -1;
  }
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetBoneLineage(EOHSkeletalBone TargetBone,
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetBoneLineageToRoot(EOHSkeletalBone TargetBone) {
  return GetBoneLineage(TargetBone, EOHSkeletalBone::None);
}

int32 UOHSkeletalPhysicsUtils::CompareBoneDepth(EOHSkeletalBone A,
                                                EOHSkeletalBone B) {
  if (A == B) {
    return 0;
  }
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetBoneChainBelow(const EOHSkeletalBone RootBone) {
  static TMap<EOHSkeletalBone, TArray<EOHSkeletalBone>> BoneHierarchy = {
      {EOHSkeletalBone::Pelvis,
       {EOHSkeletalBone::Spine_01, EOHSkeletalBone::Spine_02,
        EOHSkeletalBone::Spine_03, EOHSkeletalBone::Clavicle_L,
        EOHSkeletalBone::UpperArm_L, EOHSkeletalBone::LowerArm_L,
        EOHSkeletalBone::Hand_L, EOHSkeletalBone::Clavicle_R,
        EOHSkeletalBone::UpperArm_R, EOHSkeletalBone::LowerArm_R,
        EOHSkeletalBone::Hand_R, EOHSkeletalBone::Thigh_L,
...
EOHSkeletalBone
UOHSkeletalPhysicsUtils::GetParentBone(EOHSkeletalBone TargetBone) {
  switch (TargetBone) {
  // Spine
...
TArray<EOHSkeletalBone> UOHSkeletalPhysicsUtils::GetBonesInFunctionalBoneGroup(
    EOHFunctionalBoneGroup Group) {
  ...
  case EOHFunctionalBoneGroup::FullBody:
    ...
    Bones.Append(
        GetBonesInFunctionalBoneGroup(EOHFunctionalBoneGroup::LowerBody));
    break;
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetChildBones(EOHSkeletalBone ParentBone) {
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetDescendantBones(EOHSkeletalBone RootBone) {
...
TArray<EOHSkeletalBone>
UOHSkeletalPhysicsUtils::GetBoneInfluenceChain(const EOHSkeletalBone Bone) {
  ...
  // Leg influence chains
    ...
    InfluenceChain.Add(EOHSkeletalBone::Spine_01);
  } else if (Bone == EOHSkeletalBone::Foot_R ||
             Bone == EOHSkeletalBone::Ball_R) {
...
void UOHSkeletalPhysicsUtils::SolveIK_TwoBone(
    ...
    const FVector &TargetPosition, const FVector &JointTarget,
    bool bAllowStretching, float StartStretchRatio, float MaxStretchScale,
    UWorld *WorldDebugContext, bool bDrawDebug) {
  ...
  const float CosAngle =
      FMath::Clamp((A * A + C * C - B * B) / (2 * A * C), -1.f, 1.f);
  ...
  const FVector RotationAxis =
      FVector::CrossProduct(ToJoint, DesiredDir).GetSafeNormal();

  // Fall back to a default axis if the cross product is nearly zero
  const FVector SafeRotationAxis =
      RotationAxis.IsNearlyZero() ? ComputeFallbackBendAxis(OutUpper, OutLower)
                                  : RotationAxis;

  const FQuat UpperRot = FQuat(SafeRotationAxis, Angle);
  const FVector NewElbowPos =
      RootPos + UpperRot.RotateVector(DirectionNorm * A);
  ...
  if (bDrawDebug && WorldDebugContext) {
    DrawDebugLine(WorldDebugContext, RootPos, NewElbowPos, FColor::Blue, false,
                  0.1f, 0, 2.f);
    DrawDebugLine(WorldDebugContext, NewElbowPos, NewWristPos, FColor::Green,
                  false, 0.1f, 0, 2.f);
    DrawDebugSphere(WorldDebugContext, NewElbowPos, 2.f, 8, FColor::Cyan, false,
                    0.1f);
    DrawDebugSphere(WorldDebugContext, NewWristPos, 2.f, 8, FColor::Yellow,
                    false, 0.1f);
  }
}

void UOHSkeletalPhysicsUtils::SolveIK_ArmChain(
    FTransform &Clavicle, FTransform &UpperArm, FTransform &LowerArm,
    FTransform &Hand, const FVector &IKTarget, const FVector &ElbowHint,
    bool bUseClavicle, bool bAllowStretching, float MaxStretchScale,
    bool bEnableWristRotation, const FVector &WristLookAtAxis, UWorld *World,
    bool bDrawDebug) {
  // Chain setup
  const FVector RootPos =
      bUseClavicle ? Clavicle.GetLocation() : UpperArm.GetLocation();
  ...
  // Stretching
  float StretchScale = 1.f;
  if (const float MaxReach = UpperLen + LowerLen;
      bAllowStretching && DesiredLen > MaxReach) {
  ...
  FVector RotationAxis =
      FVector::CrossProduct(DirectionNorm, ToHint).GetSafeNormal();
  ...
  if (bDrawDebug && World) {
    ...
    DrawDebugLine(World, NewElbow, NewWrist, FColor::Green, false, 0.1f, 0,
                  2.f);
...
FVector
UOHSkeletalPhysicsUtils::ComputeFallbackBendAxis(const FTransform &Root,
                                                 const FTransform &Mid) {
  ...
  const FVector CrossResult =
      FVector::CrossProduct(MidVec, LocalAxisY).GetSafeNormal();
...
int32 UOHSkeletalPhysicsUtils::ValidateAndSortBoneTransforms(
    TArray<FBoneTransform> &OutBoneTransforms, bool bSortByIndex,
    bool bLogWarnings) {
  // Early out if empty
  ...
  for (int32 i = OutBoneTransforms.Num() - 1; i >= 0; --i) {
    ...
    if (!BT.Transform.GetRotation().IsNormalized()) {
      if (bLogWarnings) {
        UE_LOG(LogTemp, Warning,
               TEXT("ValidateAndSortBoneTransforms: Transform at index %d has "
                    "invalid rotation"),
               i);
      }
    ...
    if (BT.Transform.ContainsNaN()) {
      if (bLogWarnings) {
        UE_LOG(LogTemp, Warning,
               TEXT("ValidateAndSortBoneTransforms: Transform at index %d "
                    "contains NaN"),
               i);
      }
      OutBoneTransforms.RemoveAt(i);
    ...
    if (!BT.BoneIndex.IsValid()) {
      if (bLogWarnings) {
        UE_LOG(LogTemp, Warning,
               TEXT("ValidateAndSortBoneTransforms: Invalid bone index at "
                    "index %d"),
               i);
      }
      OutBoneTransforms.RemoveAt(i);
  ...
  if (bSortByIndex && OutBoneTransforms.Num() > 1) {
    ...
    OutBoneTransforms.Sort(
        [](const FBoneTransform &A, const FBoneTransform &B) {
    ...
    if (!bSortValid && bLogWarnings) {
      UE_LOG(LogTemp, Error,
             TEXT("ValidateAndSortBoneTransforms: Sorting failed! This "
...
FVector UOHSkeletalPhysicsUtils::MirrorPointAcrossAxes(
    const FVector &Point, const FVector &Pivot, const FVector &MirrorAxis) {
...
float UOHSkeletalPhysicsUtils::ComputeBoneChainLength(
    USkeletalMeshComponent *SkelComp, const TArray<FName> &BoneNames) {
  if (!SkelComp) {
    return 0.f;
  }
  float Total = 0.f;
...
void UOHSkeletalPhysicsUtils::GetBoneChainWorldPositions(
    USkeletalMeshComponent *SkelComp, TArray<FName> BoneNames,
...
FVector
UOHSkeletalPhysicsUtils::GetClosestPointOnBoneChain(TArray<FVector> ChainPoints,
                                                    FVector Point) {
  FVector BestPoint = Point;
  float BestDistSqr = FLT_MAX;
  for (int32 i = 1; i < ChainPoints.Num(); ++i) {
    FVector P =
        FMath::ClosestPointOnSegment(Point, ChainPoints[i - 1], ChainPoints[i]);
...
void UOHSkeletalPhysicsUtils::PredictBoneChainPositions(
    TArray<FVector> CurrentPositions, TArray<FVector> Velocities,
    float DeltaTime, TArray<FVector> &OutPredicted) {
...
void UOHSkeletalPhysicsUtils::DecomposeSwingTwist(FQuat Rotation,
                                                  FVector TwistAxis,
                                                  FQuat &OutSwing,
                                                  FQuat &OutTwist) {
...
FVector
UOHSkeletalPhysicsUtils::ComputeBoneVelocity(USkeletalMeshComponent *SkelComp,
...
FVector UOHSkeletalPhysicsUtils::ComputeAngularVelocity(FQuat PrevRotation,
                                                        FQuat CurrRotation,
                                                        float DeltaTime) {
...
float UOHSkeletalPhysicsUtils::PredictBoneTimeToCollision(
    USkeletalMeshComponent *SkelComp, const FName BoneName, FVector Velocity,
    const float MaxTime, const ECollisionChannel Channel, FHitResult &OutHit) {
  ...
  const FVector End =
      Start + Velocity.GetSafeNormal() * Velocity.Size() * MaxTime;

  bool bHit = World->LineTraceSingleByChannel(
      OutHit, Start, End, Channel,
...
FVector
UOHSkeletalPhysicsUtils::ComputeReactiveImpulseFromHit(const FHitResult &Hit,
...
bool UOHSkeletalPhysicsUtils::IsBoneChainIntersecting(
    USkeletalMeshComponent *SkelComp, TArray<FName> BoneNames,
    const float SphereRadius, const ECollisionChannel Channel) {
  if (!SkelComp) {
    return false;
  }
  UWorld *World = SkelComp->GetWorld();
...
FVector UOHSkeletalPhysicsUtils::ProjectPointOntoBoneCapsule(FVector Start,
                                                             FVector End,
                                                             const float Radius,
                                                             FVector Point) {
  // Closest point on line segment
  FVector Closest =
      UKismetMathLibrary::FindClosestPointOnSegment(Point, Start, End);
...
float UOHSkeletalPhysicsUtils::ComputeCollisionBlendWeight(
    FVector BonePos, FVector CollisionPos, float DistanceThreshold) {
...
void UOHSkeletalPhysicsUtils::PredictChainCollisionSegment(
    const TArray<FVector> &ChainPositions, FVector SphereCenter, float Radius,
...
void UOHSkeletalPhysicsUtils::GenerateBoneChainMotionCapsules(
    TArray<FVector> CurrentPositions, TArray<FVector> PredictedPositions,
...
void UOHSkeletalPhysicsUtils::BlendReactivePose(
  ...
  for (const FBoneTransform &RT : ReactiveTransforms) {
    ...
    if (!bMerged && Alpha > 0.f) {
      ...
      FVector Scale =
          FVector::OneVector + (B.GetScale3D() - FVector::OneVector) * Alpha;

      OutBoneTransforms.Add(
          FBoneTransform(RT.BoneIndex, FTransform(Rot, Loc, Scale)));
    }
  }
}

// Predictive target offset based on current velocity and a directional
// multiplier
FVector UOHSkeletalPhysicsUtils::ComputeVelocityBasedTarget(
    FVector HandPosition, FVector Velocity, const float CompressionThreshold) {
...
FVector UOHSkeletalPhysicsUtils::BlendTargets(FVector A, FVector B,
                                              const float Alpha) {
  return FMath::Lerp(A, B, FMath::Clamp(Alpha, 0.f, 1.f));
}

// Move a point forward along its velocity for a given delta time
FVector UOHSkeletalPhysicsUtils::ApplyPredictiveOffset(FVector BaseTarget,
                                                       FVector Velocity,
...
FVector UOHSkeletalPhysicsUtils::MirrorVectorAcrossAxis(FVector Vector,
                                                        FVector MirrorAxis) {
...
float UOHSkeletalPhysicsUtils::EvaluateCurveAtAlpha(const UCurveFloat *Curve,
                                                    float InputAlpha,
                                                    float DefaultValue) {
...
float UOHSkeletalPhysicsUtils::ClampAlpha(float Alpha, float MinAlpha,
                                          float MaxAlpha) {
  return FMath::Clamp(Alpha, MinAlpha, MaxAlpha);
}

// Compute a falloff factor (1 at zero distance, 0 at or beyond Radius)
float UOHSkeletalPhysicsUtils::ComputeFalloffAlpha(float Distance,
                                                   float Radius) {
...
float UOHSkeletalPhysicsUtils::ComputeSoftZoneAlpha(float Alpha,
                                                    float SoftZoneOffset) {
  if (SoftZoneOffset > KINDA_SMALL_NUMBER) {
    return FMath::Clamp((Alpha - SoftZoneOffset) / (1.f - SoftZoneOffset), 0.f,
                        1.f);
  }
  return Alpha;
}
...
float UOHSkeletalPhysicsUtils::ComputeCapsulePenetrationDepth(
    FVector Point, FVector CapsuleStart, FVector CapsuleEnd, float Radius) {
  FVector Closest =
      FMath::ClosestPointOnSegment(Point, CapsuleStart, CapsuleEnd);
...
FVector
UOHSkeletalPhysicsUtils::ComposeCollisionNormal(TArray<FHitResult> Hits) {
...
FVector UOHSkeletalPhysicsUtils::ComputeBounceVelocity(
    FVector VelocityIn, FVector Normal, float RestitutionCoefficient) {
...
void UOHSkeletalPhysicsUtils::ComputeChainStiffnessMap(
    UPhysicsAsset * /*PhysAsset*/, TArray<FName> BoneNames,
    float StiffnessPerBone, TMap<FName, float> &OutStiffnessMap) {
...
float UOHSkeletalPhysicsUtils::PredictCapsuleChainTimeToImpact(
  ...
  for (const FName &Bone : BoneNames) {
    ...
    const bool bHit = World->SweepSingleByChannel(
        ...
        FCollisionQueryParams(NAME_None, /*bTraceComplex=*/false,
                              SkelComp->GetOwner()));
...
float UOHSkeletalPhysicsUtils::ComputeCollisionDecay(float Time,
                                                     float DecayRate) {
  return FMath::Exp(-DecayRate * Time);
}

float UOHSkeletalPhysicsUtils::GetArmChainLength(USkeletalMeshComponent *Mesh,
                                                 FName UpperArm, FName LowerArm,
                                                 FName Hand) {
  if (!Mesh) {
    return 0.f;
  }
...
FVector UOHSkeletalPhysicsUtils::GetArmEffectorDirection(
    USkeletalMeshComponent *Mesh, FName UpperArm, FVector Effector) {
  if (!Mesh) {
...
float UOHSkeletalPhysicsUtils::GetArmCompressionAlpha(float Distance, float Min,
                                                      float Max) {
...
void UOHSkeletalPhysicsUtils::DebugDrawArmIK(
    USkeletalMeshComponent *Mesh, FName UpperArm, FName LowerArm, FName Hand,
  ...
  DrawDebugLine(Mesh->GetWorld(), A, EffectorTarget, Color.ToFColor(true),
                false, Duration, 0, 1.f);

  const float CurrentLen = FVector::Distance(A, EffectorTarget);
  const FString Label =
      FString::Printf(TEXT("IK Len: %.1f [%.1f - %.1f]  Alpha: %.2f"),
                      CurrentLen, MinLen, MaxLen, CurrentAlpha);

  DrawDebugString(Mesh->GetWorld(), A + FVector(0, 0, 20), Label, nullptr,
                  Color.ToFColor(true), Duration, false);
}

FVector UOHSkeletalPhysicsUtils::PredictEffectorTarget(
    USkeletalMeshComponent *Mesh, FName StartBone, FVector Velocity,
    float Distance) {
  if (!Mesh) {
    return FVector::ZeroVector;
  }
...
float UOHSkeletalPhysicsUtils::PredictCompressionTrend(
  ...
  const FVector Prev =
      Mesh->GetBoneLocation(Hand) - Mesh->GetBoneLocation(UpperArm);
...
bool UOHSkeletalPhysicsUtils::IsBoneOverlapping(USkeletalMeshComponent *Mesh,
                                                FName BoneName, float Radius) {
  if (!Mesh) {
    return false;
  }
  ...
  return Mesh->GetWorld()->OverlapAnyTestByChannel(BoneLoc, FQuat::Identity,
                                                   ECC_Pawn, Shape);
}

bool UOHSkeletalPhysicsUtils::PredictBoneCollision(USkeletalMeshComponent *Mesh,
                                                   FName BoneName,
                                                   FVector Velocity,
                                                   float Distance,
                                                   FHitResult &OutHit) {
...
bool UOHSkeletalPhysicsUtils::TracePunchCollision(USkeletalMeshComponent *Mesh,
                                                  FName HandBone,
                                                  FVector Direction,
                                                  float Length,
                                                  FHitResult &OutHit) {
...
void UOHSkeletalPhysicsUtils::SyncIKFromArmPose(UObject *AnimInstance,
                                                FName UpperArm, FName LowerArm,
                                                FName Hand, float MinLength,
                                                float MaxLength) {
  if (!AnimInstance) {
    return;
  }
  USkeletalMeshComponent *Mesh =
      Cast<USkeletalMeshComponent>(AnimInstance->GetOuter());
  if (!Mesh) {
    return;
  }
...
void UOHSkeletalPhysicsUtils::TrackImpactCamera(UObject *WorldContext,
  ...
  DrawDebugSphere(World, ImpactLocation, 12.f, 16, FColor::Orange, false,
                  Duration);
  DrawDebugString(World, ImpactLocation + FVector(0, 0, 20), TEXT("Impact!"),
                  nullptr, FColor::Yellow, Duration);
}

FVector UOHSkeletalPhysicsUtils::ComputeOverstretchedEffector(
    FVector Origin, FVector Target, float StretchRatio) {
...
FTransform
UOHSkeletalPhysicsUtils::OffsetTransform(const FTransform &Base,
...
FTransform UOHSkeletalPhysicsUtils::InterpTransform(const FTransform &A,
                                                    const FTransform &B,
                                                    float Alpha) {
...
float UOHSkeletalPhysicsUtils::GetLegCompressionAlpha(
  ...
  float Length =
      FVector::Dist(Mesh->GetBoneLocation(A), Mesh->GetBoneLocation(B)) +
...
float UOHSkeletalPhysicsUtils::GetSpineBendRatio(USkeletalMeshComponent *Mesh,
  ...
  return FVector::DotProduct(DirStart,
                             DirEnd); // 1 = straight, 0 = 90°, -1 = folded
}
#pragma endregion

void UOHSkeletalPhysicsUtils::DrawDebugBoneChainByName(
    const USkeletalMeshComponent *Mesh, const FName &ParentBoneName,
    const FName &ChildBoneName, FColor Color, float Duration) {
  if (!Mesh)
    return;

  TArray<FName> BoneChain =
      GetBoneChainBetweenByName(Mesh, ParentBoneName, ChildBoneName);
  ...
  for (int32 i = 0; i < BoneChain.Num(); ++i) {
    ...
    DrawDebugString(World, Pos + FVector(0, 0, 6), Label, nullptr, Color,
                    Duration, false);
  }
}

TArray<FName> UOHSkeletalPhysicsUtils::GetBoneChainReverseByName(
    const USkeletalMeshComponent *Mesh, const FName &StartingBone) {
  TArray<FName> Chain;

  if (!Mesh || !Mesh->DoesSocketExist(StartingBone))
    return Chain;

  const USkeleton *Skeleton = Mesh->GetSkeletalMeshAsset()
...
void UOHSkeletalPhysicsUtils::BuildValidatedBoneChain(
    EOHSkeletalBone EffectorEnum, const FBoneContainer &Bones,
    TArray<FName> &OutValidBoneNames) {
  OutValidBoneNames.Reset();

  // 1) Get the raw enum lineage (your existing GetBoneLineage)
  TArray<EOHSkeletalBone> Raw =
      GetBoneLineage(EffectorEnum, EOHSkeletalBone::None);
...
TArray<FName> UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(
  ...
  if (!Mesh || !Mesh->DoesSocketExist(ParentBoneName) ||
      !Mesh->DoesSocketExist(ChildBoneName)) {
    return Chain;
  }
...
bool UOHSkeletalPhysicsUtils::SetPhysicsBlendWeightForBone(
    USkeletalMeshComponent *Mesh, FName BoneName, float BlendWeight) {
  if (!Mesh)
    return false;
  if (BoneName.IsNone())
    return false;
...
void UOHSkeletalPhysicsUtils::SetPhysicsBlendWeightForBones(
    USkeletalMeshComponent *Mesh, const TArray<FName> &BoneNames,
    float BlendWeight) {
...
void UOHSkeletalPhysicsUtils::SetPhysicsBlendWeightForBoneChain(
    USkeletalMeshComponent *Mesh, FName StartBone, FName EndBone,
    float BlendWeight) {
...
bool UOHSkeletalPhysicsUtils::EnablePhysicalAnimationForBone(
    USkeletalMeshComponent *Mesh, UPhysicalAnimationComponent *PAC,
...
bool UOHSkeletalPhysicsUtils::EnablePhysicalAnimationForBoneChain(
    USkeletalMeshComponent *Mesh, UPhysicalAnimationComponent *PAC,
    FName StartBone, FName EndBone, const FPhysicalAnimationData &Profile) {
...
void UOHSkeletalPhysicsUtils::BlendInPhysicalAnimationForBone(
    USkeletalMeshComponent *Mesh, UPhysicalAnimationComponent *PAC,
    FName BoneName, const FPhysicalAnimationData &Profile, float BlendDuration,
    UCurveFloat *BlendCurve, UObject *WorldContextObject) {
  ...
  World->GetTimerManager().SetTimer(
      ...
      [=, &Elapsed]() mutable {
        ...
        UOHSkeletalPhysicsUtils::SetPhysicsBlendWeightForBone(Mesh, BoneName,
                                                              Blend);
...
void UOHSkeletalPhysicsUtils::BlendOutPhysicalAnimationForBone(
    USkeletalMeshComponent *Mesh, UPhysicalAnimationComponent *PAC,
    FName BoneName, float BlendDuration, UCurveFloat *BlendCurve,
  ...
  World->GetTimerManager().SetTimer(
      ...
      [=, &Elapsed]() mutable {
        Elapsed += Step;
        float Alpha = FMath::Clamp(Elapsed / BlendDuration, 0.f, 1.f);
        float Blend = BlendCurve ? (1.0f - BlendCurve->GetFloatValue(Alpha))
                                 : (1.0f - Alpha);
        ...
        if (Alpha >= 1.f) {
          ...
          PAC->ApplyPhysicalAnimationSettings(BoneName,
...
void UOHSkeletalPhysicsUtils::BlendInPhysicalAnimationForBodyPart(
    ...
    float BlendDuration, UCurveFloat *BlendCurve, UObject *WorldContextObject) {
  ...
  for (EOHSkeletalBone BoneEnum : PartBones) {
    ...
    if (BoneName != NAME_None)
      BlendInPhysicalAnimationForBone(Mesh, PAC, BoneName, Profile,
                                      BlendDuration, BlendCurve,
                                      WorldContextObject);
  }
}
...
void UOHSkeletalPhysicsUtils::BlendOutPhysicalAnimationForBodyPart(
    USkeletalMeshComponent *Mesh, UPhysicalAnimationComponent *PAC,
    EOHBodyPart BodyPart, float BlendDuration, UCurveFloat *BlendCurve,
    UObject *WorldContextObject) {
  TArray<EOHSkeletalBone> PartBones = GetBonesInBodyPart(BodyPart);
  for (EOHSkeletalBone BoneEnum : PartBones) {
    FName BoneName = GetBoneNameFromEnum(BoneEnum);
    if (BoneName != NAME_None)
      BlendOutPhysicalAnimationForBone(Mesh, PAC, BoneName, BlendDuration,
                                       BlendCurve, WorldContextObject);
  }
}
