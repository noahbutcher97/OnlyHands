...
namespace
{
	...
	inline bool UpdateBoneVelocityHistory(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FVector Velocity,
		float Now,
	...
	inline bool UpdateBoneAccelHistory(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		FVector Acceleration,
		float Now,
	...
	inline bool GetAveragedVelocity(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		float Now,
		float HistorySeconds,
	...
	inline bool GetSmoothedAcceleration(
		const USkeletalMeshComponent* SkelComp,
		FName BoneName,
		float Now,
		float HistorySeconds,
    ...
    bool bAutoLog)
{
    ...
    {
        if (bAutoLog) UE_LOG(LogTemp, Error, TEXT("SkeletalMeshComponent or SkeletalMeshAsset is null!"));
        return false;
    }
    ...
    TArray<FName> MustHaveExcluded = {
        TEXT("root"), TEXT("ball_l"), TEXT("ball_r")
    };
    ...
    for (const FName& Bone : MustHaveTracked)
    {
        ...
        {
            if (bAutoLog) UE_LOG(LogTemp, Error, TEXT("Required tracked bone '%s' is missing from skeleton!"), *Bone.ToString());
            bSuccess = false;
        }
        if (!TrackedBones.Contains(Bone))
        {
            if (bAutoLog) UE_LOG(LogTemp, Warning, TEXT("Required tracked bone '%s' is missing from TrackedBones array!"), *Bone.ToString());
            bSuccess = false;
        }
    }
    ...
    for (const FName& Bone : MustHaveExcluded)
    {
        if (!BoneNamesInMesh.Contains(Bone))
        {
            if (bAutoLog) UE_LOG(LogTemp, Error, TEXT("Required excluded bone '%s' is missing from skeleton!"), *Bone.ToString());
            bSuccess = false;
        }
        if (!ExcludedBones.Contains(Bone))
        {
            if (bAutoLog) UE_LOG(LogTemp, Warning, TEXT("Required excluded bone '%s' is missing from ExcludedBones array!"), *Bone.ToString());
            bSuccess = false;
        }
    }
    ...
    for (const FName& Bone : TrackedSet)
    {
        ...
        {
            if (bAutoLog) UE_LOG(LogTemp, Error, TEXT("Bone '%s' is both tracked and excluded!"), *Bone.ToString());
            bSuccess = false;
        }
    }
    ...
    for (const FName& Bone : TrackedBones)
    {
        ...
        if (ParentIdx != INDEX_NONE)
        {
            ...
            {
                if (bAutoLog) UE_LOG(LogTemp, Warning, TEXT("Tracked bone '%s' has parent '%s' not tracked or excluded."), *Bone.ToString(), *ParentName.ToString());
            }
        }
    }
    ...
    if (bSuccess)
    {
        if (bAutoLog) UE_LOG(LogTemp, Display, TEXT("Skeleton validation PASSED for %s"), *Mesh->GetName());
    }
    else
    {
        if (bAutoLog) UE_LOG(LogTemp, Error, TEXT("Skeleton validation FAILED for %s (see above)."), *Mesh->GetName());
    }

    return bSuccess;
}

FName UOHSkeletalPhysicsUtils::ResolveBoneNameSmart(const FName& Input, const USkeletalMeshComponent* SkelComp, float ScoreThreshold)
{
	if (!SkelComp || !SkelComp->GetSkeletalMeshAsset()) return NAME_None;
...
FName UOHSkeletalPhysicsUtils::GetBestMatchingBone(const FString& Query, const USkeletalMeshComponent* SkelComp, float& OutScore, FString& OutAlgorithm, float ScoreThreshold)
{
	...
	if (!SkelComp || !SkelComp->GetSkeletalMeshAsset()) { OutAlgorithm = TEXT("None"); return NAME_None; }
	const FReferenceSkeleton& RefSkel = SkelComp->GetSkeletalMeshAsset()->GetRefSkeleton();
...
FName UOHSkeletalPhysicsUtils::GetBestMatchingBone(const FName& Query, const USkeletalMeshComponent* SkelComp, float& OutScore, float ScoreThreshold)
{
	OutScore = 0.f;
	if (!SkelComp || !SkelComp->GetSkeletalMeshAsset()) return NAME_None;
	const FReferenceSkeleton& RefSkel = SkelComp->GetSkeletalMeshAsset()->GetRefSkeleton();
...
void UOHSkeletalPhysicsUtils::FindAllBonesMatching(
    const FString& Query,
    const USkeletalMeshComponent* SkelComp,
	...
	const TMap<FName, TPair<FName, float>>& RedirectMap)
{
	...
	static const TArray<FName> FallbackMajorBones = {
		"spine_03", "spine_02", "spine_01", "pelvis", "clavicle_l", "clavicle_r", "upperarm_l", "upperarm_r", "neck_01", "head"
	};
	TArray<FName> Chain = GetBoneChainToRoot(SkelComp, HitBone);
	for (const FName& BoneInChain : Chain)
	{
		if (BoneInChain == HitBone) continue;
...
TArray<FName> UOHSkeletalPhysicsUtils::GetBoneChainToRoot(const USkeletalMeshComponent* SkelComp, FName StartBone, FName StopAtBone)
{
	TArray<FName> Chain;
	if (!SkelComp || !SkelComp->GetSkeletalMeshAsset()) return Chain;
	const FReferenceSkeleton& RefSkel = SkelComp->GetSkeletalMeshAsset()->GetRefSkeleton();
...
FName UOHSkeletalPhysicsUtils::GetPrimaryBoneNameForBodyPart(const FString& BodyPart, const USkeletalMeshComponent* SkelComp)
{
	// Use canonical Mannequin names
	static TMap<FString, FName> BodyPartToBone = {
		{ TEXT("Chest"), TEXT("spine_03") },
		{ TEXT("Spine"), TEXT("spine_01") },
		{ TEXT("Pelvis"), TEXT("pelvis") },
		{ TEXT("Head"), TEXT("neck_01") },
		{ TEXT("Left Arm"), TEXT("clavicle_l") },
		{ TEXT("RightArm"), TEXT("clavicle_r") },
		{ TEXT("Left Leg"), TEXT("thigh_l") },
		{ TEXT("Right Leg"), TEXT("thigh_r") },
		{ TEXT("Left Hand"), TEXT("hand_l") },
		{ TEXT("Right Hand"), TEXT("hand_r") },
		{ TEXT("Left Foot"), TEXT("foot_l") },
		{ TEXT("Right Foot"), TEXT("foot_r") },
...
int32 UOHSkeletalPhysicsUtils::CompareBoneDepth(const USkeletalMeshComponent* SkelComp, FName A, FName B)
{
	...
	if (ChainA.Num() == ChainB.Num()) return 0;
...
FName UOHSkeletalPhysicsUtils::GetParentBone(const USkeletalMeshComponent* SkelComp, FName Bone)
{
	...
	const FReferenceSkeleton& RefSkel = SkelComp->GetSkeletalMeshAsset()->GetRefSkeleton();
	int32 BoneIdx = RefSkel.FindBoneIndex(Bone);
	if (BoneIdx == INDEX_NONE) return NAME_None;
	int32 ParentIdx = RefSkel.GetParentIndex(BoneIdx);
...
FVector UOHSkeletalPhysicsUtils::ComputeAngularVelocity(const FRotator& CurrentRotation, const FRotator& PreviousRotation,
	float DeltaTime)
{
	if (DeltaTime <= 0.0f)
		return FVector::ZeroVector;
...
void UOHSkeletalPhysicsUtils::ApplyImpulseToBone(
...
)
{
	...
	if (FBodyInstance* Body = SkeletalMesh->GetBodyInstance(BoneName))
	{
		...
		{
			UE_LOG(LogTemp, Verbose, TEXT("ApplyImpulseToBone: Bone %s is not simulating physics"), *BoneName.ToString());
		}
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("ApplyImpulseToBone: Failed to find BodyInstance for bone %s"), *BoneName.ToString());
	}
}
...
)
{
	...
	TArray<FName> ChainBones = GetBoneChain(
		SkeletalMesh,
		RootBone,
		MaxDepth,
		SimulatableBones,
...
TArray<FName> UOHSkeletalPhysicsUtils::GetBoneChain(
	...
	FName RootBone,
	int32 MaxDepth,
	const TFunctionRef<bool(FName)>& Filter,
	bool bUseDFS
)
{
	return GetBoneChain_Internal(
...
)
{
    ...
    if (bVerboseLogging)
    {
        UE_LOG(LogTemp, Log, TEXT("GetBoneChain_Internal: Found %d bones in chain from %s (MaxDepth=%d)"),
...
FVector UOHSkeletalPhysicsUtils::GetBoneVelocitySingleFrame(
	const USkeletalMeshComponent* SkelComp,
	FName BoneName,
	FName ReferenceBone,
	EOHReferenceSpace RefFrame)
...
FVector UOHSkeletalPhysicsUtils::CalculateBoneVelocityInSpace(USkeletalMeshComponent* Mesh, FName BoneName,
	const FVector& CurrentWorldPosition, const FVector& PreviousWorldPosition, float DeltaTime,
...
FVector UOHSkeletalPhysicsUtils::CalculateBoneWorldAcceleration(const FVector& CurrentWorldPosition,
	const FVector& PreviousWorldPosition, const FVector& TwoFramesAgoWorldPosition, float DeltaTime)
{
	if (DeltaTime <= 0.0f)
		return FVector::ZeroVector;
...
float UOHSkeletalPhysicsUtils::CalculateBoneImpactForce(const FVector& StrikingBoneWorldVelocity,
	const FVector& TargetBoneWorldVelocity, const FVector& ContactNormal, float StrikingBoneMass, float TargetBoneMass,
	float Restitution)
{
	...
	if (NormalVelocity < 0) return 0.0f;
...
FVector UOHSkeletalPhysicsUtils::CalculateBoneForceVector(const FVector& StrikingBoneWorldVelocity,
	const FVector& ContactNormal, float StrikingBoneMass, float TargetMass)

{
	float ForceMagnitude = CalculateBoneImpactForce(
	StrikingBoneWorldVelocity,
	...
	const TArray<FVector>& PreviousBoneWorldPositions, float DeltaTime)
{
	...
	{
		FVector Velocity = CalculateBoneWorldVelocity(
			CurrentBoneWorldPositions[i],
			PreviousBoneWorldPositions[i],
			DeltaTime
		);
...
FConstraintInstance* UOHSkeletalPhysicsUtils::GetActiveConstraintBetweenBones(const USkeletalMeshComponent* SkelMeshComp,
...
bool UOHSkeletalPhysicsUtils::GetBonesFromConstraintName(USkeletalMeshComponent* SkelMeshComp,
	const FName& ConstraintInstanceName, FName& OutBoneA, FName& OutBoneB)
{
	if (!SkelMeshComp)
	{
		return false;
	...
	EOHReferenceSpace RefFrame)
{
	...
	if (ReferenceBone != NAME_None)
	{
		...
		if (PrevRefPosMap.Contains(RefKey))
		{
			...
			RefVel = (RefCurrentPos - RefPrevSample.Position) / FMath::Max(Now - RefPrevSample.Time, KINDA_SMALL_NUMBER);
		}
		...
		if (PrevVelMap.Contains(RefKey))
		{
			...
			RefAccel = (RefVel - RefPrevVelSample.Velocity) / FMath::Max(Now - RefPrevVelSample.Time, KINDA_SMALL_NUMBER);
		}
...
FVector UOHSkeletalPhysicsUtils::GetBoneJerkSmoothed(
	const USkeletalMeshComponent* SkelComp,
	FName BoneName,
	float HistorySeconds)
{
    ...
    FName BoneName)
{
    ...
    // (Optional) Debug log:
    UE_LOG(LogTemp, Log,
        TEXT("[OHSkeletalPhysicsUtils][BP] [%s] ConstraintProfile | Swing=%.1f Twist=%.1f | MassRatio=%.2f | Length=%.1f"),
...
{
    if (!SkeletalMeshComponent || !ConstraintInstance) return;
    ...
    const FConstraintProfileProperties& Profile)
{
    ...
    auto ConvertAngularMotion = [](EAngularConstraintMotion In)
    {
        switch (In)
        {
            case EAngularConstraintMotion::ACM_Locked:  return EOHAngularConstraintMotion::ACM_Locked;
            case EAngularConstraintMotion::ACM_Limited: return EOHAngularConstraintMotion::ACM_Limited;
            case EAngularConstraintMotion::ACM_Free:    return EOHAngularConstraintMotion::ACM_Free;
            default:                                    return EOHAngularConstraintMotion::ACM_Locked;
        }
    };
    ...
    auto ConvertLinearMotion = [](ELinearConstraintMotion In)
    {
        switch (In)
        {
            case ELinearConstraintMotion::LCM_Locked:  return EOHLinearConstraintMotion::LCM_Locked;
            case ELinearConstraintMotion::LCM_Limited: return EOHLinearConstraintMotion::LCM_Limited;
            case ELinearConstraintMotion::LCM_Free:    return EOHLinearConstraintMotion::LCM_Free;
            default:                                   return EOHLinearConstraintMotion::LCM_Locked;
        }
    };
...
FConstraintProfileProperties UOHSkeletalPhysicsUtils::SimpleToConstraintProfile(const FSimpleConstraintProfile& Simple)
{
    ...
    auto ConvertAngularMotion = [](EOHAngularConstraintMotion In)
    {
        switch (In)
        {
            case EOHAngularConstraintMotion::ACM_Locked:  return EAngularConstraintMotion::ACM_Locked;
            case EOHAngularConstraintMotion::ACM_Limited: return EAngularConstraintMotion::ACM_Limited;
            case EOHAngularConstraintMotion::ACM_Free:    return EAngularConstraintMotion::ACM_Free;
            default:                                      return EAngularConstraintMotion::ACM_Locked;
        }
    };
    ...
    auto ConvertLinearMotion = [](EOHLinearConstraintMotion In)
    {
        switch (In)
        {
            case EOHLinearConstraintMotion::LCM_Locked:  return ELinearConstraintMotion::LCM_Locked;
            case EOHLinearConstraintMotion::LCM_Limited: return ELinearConstraintMotion::LCM_Limited;
            case EOHLinearConstraintMotion::LCM_Free:    return ELinearConstraintMotion::LCM_Free;
            default:                                     return ELinearConstraintMotion::LCM_Locked;
        }
    };
