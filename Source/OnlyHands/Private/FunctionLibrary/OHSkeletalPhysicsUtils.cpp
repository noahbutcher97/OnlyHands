...
FOHResolvedBoneData
UOHSkeletalPhysicsUtils::ResolveResolvedDataFromSkeletalBone(EOHSkeletalBone Bone,
                                                             const USkeletalMeshComponent* SkelComp) {
...
void UOHSkeletalPhysicsUtils::SolveIK_TwoBone(FTransform& OutUpper, FTransform& OutLower, FTransform& OutEnd,
...
void UOHSkeletalPhysicsUtils::SolveIK_ArmChain(FTransform& Clavicle, FTransform& UpperArm, FTransform& LowerArm,
...
void UOHSkeletalPhysicsUtils::GetBoneChainWorldPositions(USkeletalMeshComponent* SkelComp, TArray<FName> BoneNames,
...
void UOHSkeletalPhysicsUtils::PredictBoneChainPositions(TArray<FVector> CurrentPositions, TArray<FVector> Velocities,
...
void UOHSkeletalPhysicsUtils::DecomposeSwingTwist(FQuat Rotation, FVector TwistAxis, FQuat& OutSwing, FQuat& OutTwist) {
...
FVector UOHSkeletalPhysicsUtils::ComputeBoneVelocity(USkeletalMeshComponent* SkelComp, const FName BoneName,
...
float UOHSkeletalPhysicsUtils::PredictBoneTimeToCollision(USkeletalMeshComponent* SkelComp, const FName BoneName,
                                                          FVector Velocity, const float MaxTime,
    ...
    bool bHit = World->LineTraceSingleByChannel(OutHit, Start, End, Channel,
...
bool UOHSkeletalPhysicsUtils::IsBoneChainIntersecting(USkeletalMeshComponent* SkelComp, TArray<FName> BoneNames,
    ...
    for (FName Bone : BoneNames) {
        ...
        bool bOverlap =
            World->OverlapAnyTestByChannel(P, FQuat::Identity, Channel, FCollisionShape::MakeSphere(SphereRadius),
                                           FCollisionQueryParams(NAME_None, false, SkelComp->GetOwner()));
        if (bOverlap) {
            return true;
        }
...
FVector UOHSkeletalPhysicsUtils::ProjectPointOntoBoneCapsule(FVector Start, FVector End, const float Radius,
                                                             FVector Point) {
...
void UOHSkeletalPhysicsUtils::PredictChainCollisionSegment(const TArray<FVector>& ChainPositions, FVector SphereCenter,
                                                           float Radius, FVector Velocity, float MaxTime,
                                                           float& OutHitTime, int32& OutSegmentIndex,
                                                           FVector& OutHitPoint) {
    OutHitTime = MaxTime;
    OutSegmentIndex = -1;
...
void UOHSkeletalPhysicsUtils::GenerateBoneChainMotionCapsules(TArray<FVector> CurrentPositions,
                                                              TArray<FVector> PredictedPositions, float Radius,
...
void UOHSkeletalPhysicsUtils::ComputeBoneMassMap(USkeletalMeshComponent* SkelComp, TArray<FName> BoneNames,
...
FVector UOHSkeletalPhysicsUtils::ComputeVelocityBasedTarget(FVector HandPosition, FVector Velocity,
    ...
    FVector Dir = Velocity.SizeSquared() > KINDA_SMALL_NUMBER ? Velocity.GetSafeNormal() : FVector::ZeroVector;
...
FVector UOHSkeletalPhysicsUtils::ApplyPredictiveOffset(FVector BaseTarget, FVector Velocity, float PredictiveOffset) {
...
float UOHSkeletalPhysicsUtils::EvaluateCurveAtAlpha(const UCurveFloat* Curve, float InputAlpha, float DefaultValue) {
...
float UOHSkeletalPhysicsUtils::ComputeCapsulePenetrationDepth(FVector Point, FVector CapsuleStart, FVector CapsuleEnd,
                                                              float Radius) {
...
FVector UOHSkeletalPhysicsUtils::ComputePenetrationCorrection(FVector Point, FVector CapsuleStart, FVector CapsuleEnd,
                                                              float Radius) {
    FVector Closest = FMath::ClosestPointOnSegment(Point, CapsuleStart, CapsuleEnd);
...
void UOHSkeletalPhysicsUtils::ComputeChainStiffnessMap(UPhysicsAsset* /*PhysAsset*/, TArray<FName> BoneNames,
...
float UOHSkeletalPhysicsUtils::PredictCapsuleChainTimeToImpact(UWorld* World, USkeletalMeshComponent* SkelComp,
                                                               TArray<FName> BoneNames, FVector CapsuleVelocity,
                                                               float CapsuleRadius, float MaxTime) {
    if (!World || !SkelComp) {
        return -1.f;
    }
...
FVector UOHSkeletalPhysicsUtils::GetArmEffectorDirection(USkeletalMeshComponent* Mesh, FName UpperArm,
                                                         FVector Effector) {
    if (!Mesh) {
...
void UOHSkeletalPhysicsUtils::DebugDrawArmIK(USkeletalMeshComponent* Mesh, FName UpperArm, FName LowerArm, FName Hand,
    ...
    const FString Label =
        FString::Printf(TEXT("IK Len: %.1f [%.1f - %.1f]  Alpha: %.2f"), CurrentLen, MinLen, MaxLen, CurrentAlpha);
...
float UOHSkeletalPhysicsUtils::GetChainLengthFromBoneEnums(USkeletalMeshComponent* Mesh,
...
float UOHSkeletalPhysicsUtils::GetLegCompressionAlpha(USkeletalMeshComponent* Mesh, EOHSkeletalBone Thigh,
                                                      EOHSkeletalBone Calf, EOHSkeletalBone Foot, float MinLength,
                                                      float MaxLength) {
...
float UOHSkeletalPhysicsUtils::GetSpineBendRatio(USkeletalMeshComponent* Mesh, EOHSkeletalBone SpineStart,
                                                 EOHSkeletalBone SpineMid, EOHSkeletalBone SpineEnd) {
...
void UOHSkeletalPhysicsUtils::DrawDebugBoneChainByName(const USkeletalMeshComponent* Mesh, const FName& ParentBoneName,
