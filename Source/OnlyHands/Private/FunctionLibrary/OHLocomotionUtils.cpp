	...
	const AActor* TargetActor)
{
	...
	GetLockOnMovementBasis(const_cast<ACharacter*>(Character), const_cast<AActor*>(TargetActor), ForwardToTarget, RightToTarget);
...
void UOHLocomotionUtils::GetLockOnMovementBasis(
...
float UOHLocomotionUtils::GetDynamicDeadzone(const TArray<FVector2D>& RecentInputHistory, float MinDeadzone, float MaxDeadzone)
{
...
void UOHLocomotionUtils::ApplyWeightedMovementInput(AActor* Owner, const FVector& MoveVector, const FVector& LastDir, float Weight)
{
	if (!Owner) return;
...
FVector UOHLocomotionUtils::CalculateMovementInputVector(
...
FVector UOHLocomotionUtils::CalculateSmoothedMovementInputVector(const FVector2D& RawInput,
	const FRotator& ControlRotation, float Deadzone, float InputSmoothingBlendSpeed, const FVector2D& PreviousInput)
{
	// 1. Deadzone filter
...
float UOHLocomotionUtils::SampleGaitCurve(UCurveVector* Curve, float Input, EOHGait Gait, float SpeedFactor)
{
	...
	switch (Gait)
	{
	case EOHGait::Walking:   return Val.X * SpeedFactor;
	case EOHGait::Running:   return Val.Y * SpeedFactor;
	case EOHGait::Sprinting: return Val.Z * SpeedFactor;
	}
	return 0.f;
}
	...
	float PelvisTranslationThreshold)
{
	...
	if (PhysicsManager)
	{
		...
		FinalAccel = FMath::Lerp(
			...
			PhysicsManager->GetLinearAcceleration(FName("pelvis")),
			BlendAlpha
		);
	...
	bool bNormalize)
{
	...
	const FVector2D ToTarget = FVector2D(TargetActor->GetActorLocation() - Character->GetActorLocation()).GetSafeNormal();
	...
	bool bNormalize)
{
	...
	const FVector2D ToTarget = FVector2D(TargetActor->GetActorLocation() - ReferenceActor->GetActorLocation()).GetSafeNormal();
	...
	bool bNormalize)
{
	...
	const FVector2D ToTarget = FVector2D(TargetActor->GetActorLocation() - ReferenceComponent->GetComponentLocation()).GetSafeNormal();
	...
	const ACharacter* Character)
{
	...
	const float AngleRad = FMath::Atan2(
		CamForward2D.X * ToTarget.Y - CamForward2D.Y * ToTarget.X,
    ...
    bool bClamp)
{
    ...
    {
        const FVector2D ActorForward = FVector2D(ReferenceActor->GetActorForwardVector().X, ReferenceActor->GetActorForwardVector().Y).GetSafeNormal();
        const FVector2D CameraForward = FVector2D(PlayerController->PlayerCameraManager->GetCameraRotation().Vector().X, PlayerController->PlayerCameraManager->GetCameraRotation().Vector().Y).GetSafeNormal();
	...
	bool bNormalize)
{
	...
	const FVector2D Blend = ComputeBlendFactors_Inline(InputVector, PlayerController->PlayerCameraManager->GetCameraRotation(), bNormalize);
	const FVector Forward = PlayerController->PlayerCameraManager->GetCameraRotation().Vector();
	const FVector Right = FRotationMatrix(PlayerController->PlayerCameraManager->GetCameraRotation()).GetUnitAxis(EAxis::Y);
	...
	bool bApplyMirroring)
{
	...
	if (bApplyMirroring)
	{
		...
		const FVector CameraForward = PlayerController->PlayerCameraManager->GetCameraRotation().Vector().GetSafeNormal2D();

		// Determine if actor is facing leftward on screen (from camera's perspective)
		const float Dot = FVector2D::DotProduct(FVector2D(ActorForward.X, ActorForward.Y), FVector2D(CameraForward.X, CameraForward.Y));
	...
	bool bClamp)
{
	...
	if (bApplyMirroring)
	{
		...
		const FVector2D CameraForward = FVector2D(PlayerController->PlayerCameraManager->GetCameraRotation().Vector()).GetSafeNormal();
...
float UOHLocomotionUtils::ComputeScreenRelativeAngle(
	const FVector2D& FromVector,
	const FVector2D& ToVector,
	bool bNormalize)
{
	...
	const float AngleRad = FMath::Atan2(
		FVector2D::CrossProduct(From, To),
...
float UOHLocomotionUtils::PredictMovementAngle(
    ...
    float BasePredictionTime,
    float SpeedPredictionScale,
    float BaseSmoothingAlpha,
    float SpeedSmoothScale,
    float PelvisTranslationThreshold,
    float FootLockSpeedThreshold,
    float FootLockAccelThreshold,
    float PivotScale,
    float PivotDeadzone,
    float StrafeBias,
    float JitterThreshold,
    float MaxAngleJumpPerFrame,

    float JerkTriggerScale,
    float MaxTrajectoryBlend
)
{
    ...
    if (const auto* MoveComp = Character->GetCharacterMovement())
    {
        ...
        {
            case MOVE_Falling:
                PredictionTime *= 1.2f;  Alpha *= 0.9f;    break;
            case MOVE_Swimming:
            case MOVE_Custom:
                PredictionTime *= 0.5f;  Alpha *= 1.2f;    break;
            default: break;
        }
    }
    ...
    if (auto* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>())
    {
        ...
        FinalAccel = FMath::Lerp(
            PhysMgr->GetLinearAcceleration(FName("root")),
            PhysMgr->GetLinearAcceleration(FName("pelvis")),
            B
        );
    ...
    float Smoothed = InOutPrevAngle
                   + Alpha * FMath::FindDeltaAngleDegrees(InOutPrevAngle, TargetA);
...
float UOHLocomotionUtils::PredictMovementAngleMonster(ACharacter* Character, const FRotator& FacingRotation,
	float DeltaTime, UWorld* World, float DrawDuration, float PredictionTime, float SpeedPredictionScale,
	float AccelPredictionScale, float JerkScale, float CurvatureScale, float MaxBlend, float PivotDeadzone,
	float PivotScale, float StrafeBias, float RotationThreshold, float MoveThreshold, float ProcNoise, float MeasNoise)
{
// --- Kalman state (static across calls) ---
    static bool  bInit = false;
    static float PrevAng = 0.f, PrevVar = 0.f, PrevAngVel = 0.f;
    if (!bInit) { PrevVar = MeasNoise; bInit = true; }
    ...
    if (Hists.Num() == 0) return PrevAng;

    // get the shortest history length
    int32 Count = INT_MAX;
    for (auto Hist : Hists)
        Count = FMath::Min(Count, Hist->Num());
    if (Count < 3) return PrevAng;

    // build centroids[] and times[] from the first history
    TArray<FVector> Centroids; Centroids.SetNum(Count);
    TArray<float>   Times;     Times.SetNum(Count);
    ...
    float PolyAng = 0.f;
    {
        ...
        if (FMath::Abs(Det) > KINDA_SMALL_NUMBER)
        {
            ...
            double A1 = ( SX0*(S2*S4 - S3*S3)
                        - S1*(SX1*S4 - S3*SX2)
                        + S2*(SX1*S3 - S2*SX2) ) / Det;
            double B1 = ( SY0*(S2*S4 - S3*S3)
                        - S1*(SY1*S4 - S3*SY2)
                        + S2*(SY1*S3 - S2*SY2) ) / Det;
            double A2 = ( S0*(SX1*S4 - S3*SX2)
                        - SX0*(S1*S4 - S3*S2)
                        + S2*(S1*SX2 - SX1*S2) ) / Det;
            double B2 = ( S0*(SY1*S4 - S3*SY2)
                        - SY0*(S1*S4 - S3*S2)
    ...
    if (!bSGInit)
    {
        for (int i = 0; i < 5; ++i) SGBuf[i] = Meas;
        bSGInit = true;
    }
    else
    {
        for (int i = 0; i < 4; ++i) SGBuf[i] = SGBuf[i+1];
        SGBuf[4] = Meas;
    }
...
float UOHLocomotionUtils::PredictMovementAngleMonsterNoDebug(ACharacter* Character, const FRotator& FacingRotation,
	float DeltaTime, float PredictionTime, float SpeedPredictionScale, float AccelPredictionScale, float JerkScale,
	...
	float RotationThreshold, float MoveThreshold, float ProcNoise, float MeasNoise)
{
    ...
    int32 Count = INT_MAX;
    for (auto Hist : Hists) Count = FMath::Min(Count, Hist->Num());
    if (Count < 3) return PrevAng;
    ...
    FVector RawDir;
    {
        ...
        if (RawDir.IsNearlyZero()) return PrevAng;
    }
...
FVector2D UOHLocomotionUtils::ComputeBlendFactors(
...
float UOHLocomotionUtils::PredictMovementAngleMonsterExtended(
    ...
    float        MoveThreshold,
    float        SGCurvThreshold,
    float        FootPlantSpeedThresh,
    float        FootPlantAccelThresh,
    float        FootPlantHoldTime,
...
)
{
    ...
    if (!Phys) return PrevAng;
    const FOHBoneData* RootBD = Phys->GetBoneData(TEXT("root"));
    if (!RootBD || !RootBD->HasMotionHistory()) return PrevAng;
    ...
    auto DetectPlant = [&](const FName& BoneName)->float
    {
        ...
        if (!BD || !BD->HasMotionHistory()) return -1.f;
    ...
    LastPlantTime = FMath::Max(LastPlantTime,
        FMath::Max( DetectPlant(TEXT("foot_l")), DetectPlant(TEXT("foot_r")) )
    );
    ...
    FVector MeasDir(FMath::Cos(FMath::DegreesToRadians(MeasAng)),
                    FMath::Sin(FMath::DegreesToRadians(MeasAng)),
                    0.f);
    // shift buffer
    if (DirCount<7) ++DirCount;
    for(int i=0;i<DirCount-1;++i) DirHist[i] = DirHist[i+1];
