// Returns the character's movement angle relative to the screen/camera direction.
...
<<<<<<< HEAD
    // Lock-On/Strafe
    FVector
    UOHLocomotionUtils::CalculateLockOnMovementInputVector(const FVector2D& Input, const ACharacter* Character,
...
void UOHLocomotionUtils::GetLockOnMovementBasis(ACharacter* Character, AActor* LockOnTarget, FVector& ForwardVector,
    ...
    == == == =
                 ...
                 // Lock-On/Strafe
        FVector UOHLocomotionUtils::CalculateLockOnMovementInputVector(
            const FVector2D& Input, const ACharacter* Character, const AActor* TargetActor) {
        if (!Character || !TargetActor)
            return FVector::ZeroVector;
    ...
    // Momentum/Inertia Blend
    FVector UOHLocomotionUtils::CalculateMomentumBlendedMovementInputVector(
        const FVector2D& Input, const FRotator& ControlRotation, const FVector& Velocity, float MomentumBlendAlpha) {
        FVector CameraMove = CalculateMovementInputVector(Input, ControlRotation);
...
<<<<<<< HEAD
    ...
    void UOHLocomotionUtils::ApplyWeightedMovementInput(AActor * Owner, const FVector& MoveVector,
                                                        const FVector& LastDir, float Weight) {
        if (!Owner)
            return;
    ...
    FVector UOHLocomotionUtils::CalculateMovementInputVector(const FVector2D& LocalInput,
                                                             const FRotator& ControlRotation) {
        // 1. Copy input
    ...
    FVector UOHLocomotionUtils::CalculateSmoothedMovementInputVector(
        const FVector2D& RawInput, const FRotator& ControlRotation, float Deadzone, float InputSmoothingBlendSpeed,
    ...
    FVector UOHLocomotionUtils::CalculateBlendedMovementInputVector(
        const FVector2D& Input, const FRotator& CameraRotation, const FRotator& FacingRotation, float FacingBlend) {
    ...
    FRotator UOHLocomotionUtils::CalculateBlendedHybridFacing(
        const FRotator& CurrentFacing, const FVector& CharacterLocation, const FVector& TargetLocation,
        float DeadzoneAngle, float MaxOffsetAngle, float PullStrength, float HardSnapStrength, float DeltaTime) {
    ...
    float UOHLocomotionUtils::ComputeSmoothedQuadrantMovementAngle(
        const FVector& Velocity, const FRotator& FacingRotation, float PreviousAngle, float DeltaTime,
        float PredictionWeight, float BiasStrength) {
...
>>>>>>> 0627b7d296554ee97d27b39fb5f7c959d6da32c9
        ...
        FRotator UOHLocomotionUtils::CalculateControlRotation(const AActor* Owner, bool bIncludePitch /*= false*/) {
            ...
            const FRotator FallbackRotation =
                bIncludePitch ? Owner->GetActorRotation()                            // Use the actor's full rotation
                              : FRotator(0.0f, Owner->GetActorRotation().Yaw, 0.0f); // Yaw-only rotation
        ...
        FVector UOHLocomotionUtils::BlendDirectionalInput(const FVector& LastDir, const FVector& NewDir,
        ...
        float UOHLocomotionUtils::CalculateRequiredForce(float TargetSpeed, float CurrentSpeed, float Mass,
                                                         float DeltaTime) {
            if (DeltaTime <= 0.f || Mass <= 0.f)
                return 0.f;
            float DeltaV = TargetSpeed - CurrentSpeed;
        ...
        void UOHLocomotionUtils::GetLockOnMovementBasis(ACharacter * Character, AActor * LockOnTarget,
            ...
            FVector ToTarget = LockOnTarget->GetActorLocation() - Character->GetActorLocation();
            ToTarget.Z = 0.0f;
        ...
        float UOHLocomotionUtils::ComputeSmoothedQuadrantMovementAngle(
            ...
            const FVector LocalVelocity = FacingMatrix.GetTransposed().TransformVector(Velocity);

            // 2. Get current input angle relative to character
            float RawAngle = FMath::Atan2(LocalVelocity.Y, LocalVelocity.X); // Y = right, X = forward
            float CurrentAngle = FMath::RadiansToDegrees(RawAngle);
            CurrentAngle = FMath::Fmod(CurrentAngle + 360.0f, 360.0f); // Normalize to [0, 360)

            // 3. Predict future angle delta
            const float SmoothedDelta = FMath::UnwindDegrees(CurrentAngle - PreviousAngle);
            const float PredictedAngle = CurrentAngle + (SmoothedDelta * PredictionWeight);

            // 4. Bias toward previous angle to reduce quadrant jitter
            const float FinalAngle = FMath::Lerp(PreviousAngle, PredictedAngle, 1.0f - BiasStrength);
        ...
        FPredictedMotionDirectionData UOHLocomotionUtils::ComputeSmartAnimationDirectionAuto(
            ACharacter * Character, float DeltaTime, float StrafeBias, float PredictionTime, float SmoothingStrength,
            ...
            UOHPhysicsManager* PhysicsManager = Character->FindComponentByClass<UOHPhysicsManager>();
            ...
            if (PhysicsManager) {
                ...
                const float BlendAlpha =
                    FMath::Clamp(PelvisMag / (RootMag * PelvisTranslationThreshold + KINDA_SMALL_NUMBER), 0.f, 1.f);

                FinalVel = FMath::Lerp(RootVel, PelvisVel, BlendAlpha);
                FinalAccel = FMath::Lerp(PhysicsManager->GetLinearAcceleration(FName("root")),
                                         PhysicsManager->GetLinearAcceleration(FName("pelvis")), BlendAlpha);

                const FVector PredictDelta =
                    FinalVel * PredictionTime + 0.5f * FinalAccel * FMath::Square(PredictionTime);
            ...
            const FVector LocalDir = FRotationMatrix(FacingRotation).GetTransposed().TransformVector(MoveDirWorld);
            float AngleRad = FMath::Atan2(LocalDir.Y, LocalDir.X);
            float AngleDeg = FMath::Fmod(FMath::RadiansToDegrees(AngleRad) + 360.f, 360.f);

            // === Pivot anticipation via curve arc detection ===
            const float ArcStrength = FVector::CrossProduct(FinalVel.GetSafeNormal(), FinalAccel.GetSafeNormal()).Z;
            ...
            // === Smooth result ===
            const float Smoothed = FMath::Lerp(PreviousAngle, AngleDeg, 1.0f - SmoothingStrength);
        ...
        void UOHLocomotionUtils::ApplyStableCameraRelativeMovement(ACharacter * Character, const FVector2D& Input) {
            ...
            float AngleDelta =
                FMath::Acos(FVector2D::DotProduct(Input.GetSafeNormal(), LastInputDirection.GetSafeNormal()));
            bool bInputChanged = AngleDelta > FMath::DegreesToRadians(10.f); // ~10 degrees change
        ...
        void UOHLocomotionUtils::ApplyJoystickMovementWithBasis(
            APawn * Pawn, const FVector2D& Input, const FRotator& BasisRotation, FVector UpVector, float Deadzone) {
        ...
        void UOHLocomotionUtils::ApplyAutoCameraRelativeJoystickMovement(ACharacter * Character, const FVector2D& Input,
                                                                         float Deadzone) {
            if (!Character || Input.SizeSquared() < Deadzone * Deadzone)
                return;
        ...
        void UOHLocomotionUtils::ApplySmartCameraRelativeJoystickMovement(
            ACharacter * Character, const FVector2D& Input, bool bFaceMovementDirection, bool bConstrainToXYPlane,
            float Deadzone) {
            if (!Character || Input.SizeSquared() < Deadzone * Deadzone)
                return;
        ...
        void UOHLocomotionUtils::ApplyFullCameraRelativeJoystickMovement(
            ACharacter * Character, const FVector2D& Input, bool bFaceMovementDirection, bool bConstrainToXYPlane,
            float Deadzone, float TurnSpeed, int32 SnapToCardinalDirections) {
            if (!Character || Input.SizeSquared() < Deadzone * Deadzone)
                return;
            ...
            if (bFaceMovementDirection && MoveDir.SizeSquared() > KINDA_SMALL_NUMBER) {
                ...
                FRotator NewRotation = FMath::RInterpTo(Character->GetActorRotation(), TargetRotation,
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionFromInput(
            const FVector2D& InputVector, const APlayerController* PlayerController, bool bNormalize) {
            ...
            Result.Angle = ComputeScreenRelativeAngle(CamForward, InputVector, bNormalize);
            ...
            Result.ClampedStrafeDirection = FMath::Clamp(Result.BlendFactors.X, -1.f, 1.f);
            return Result;
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionFromVelocity(
            const APlayerController* PlayerController, const ACharacter* Character, bool bNormalize) {
            if (!PlayerController || !Character || Character->GetVelocity().IsNearlyZero())
                return FScreenRelativeDirectionData::Default();

            const FVector2D Velocity2D(Character->GetVelocity().X, Character->GetVelocity().Y);
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionToTarget(
            const AActor* TargetActor, const APlayerController* PlayerController, const ACharacter* Character,
            bool bNormalize) {
            if (!PlayerController || !TargetActor || !Character)
                return FScreenRelativeDirectionData::Default();

            const FVector2D ToTarget =
                FVector2D(TargetActor->GetActorLocation() - Character->GetActorLocation()).GetSafeNormal();
            return ComputeDirectionFromInput(ToTarget, PlayerController, bNormalize);
        }

        // ===== Player-Relative =====

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionFromInput(
            const FVector2D& InputVector, const ACharacter* Character, bool bNormalize) {
            if (!Character || InputVector.IsNearlyZero())
                return FScreenRelativeDirectionData::Default();

            const FVector2D PlayerForward(Character->GetActorForwardVector().X, Character->GetActorForwardVector().Y);

            FScreenRelativeDirectionData Result;
            Result.Angle = ComputeScreenRelativeAngle(PlayerForward, InputVector, bNormalize);
            Result.WrappedAngle = FMath::Fmod(Result.Angle + 360.f, 360.f);
            Result.BlendFactors = ComputeBlendFactors(InputVector, Character->GetActorRotation(), bNormalize);
            Result.Direction = MapAngleTo8WayDirection_Inline(Result.Angle);
            Result.Direction4 = MapAngleTo4WayDirection_Inline(Result.Angle);
            Result.Quadrant = MapAngleToQuadrant_Inline(Result.Angle);
            Result.ClampedStrafeDirection = FMath::Clamp(Result.BlendFactors.X, -1.f, 1.f);
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionFromVelocity(
            const ACharacter* Character, bool bNormalize) {
            if (!Character || Character->GetVelocity().IsNearlyZero())
                return FScreenRelativeDirectionData::Default();

            const FVector2D Velocity2D(Character->GetVelocity().X, Character->GetVelocity().Y);
            return ComputePlayerRelativeDirectionFromInput(Velocity2D, Character, bNormalize);
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionToTarget(
            ...
            const FVector2D ToTarget =
                FVector2D(TargetActor->GetActorLocation() - Character->GetActorLocation()).GetSafeNormal();
            return ComputePlayerRelativeDirectionFromInput(ToTarget, Character, bNormalize);
        }

        // ===== World-Relative =====

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionFromInput(
            ...
            FScreenRelativeDirectionData Result;
            Result.Angle = ComputeScreenRelativeAngle(WorldForward, InputVector, bNormalize);
            Result.WrappedAngle = FMath::Fmod(Result.Angle + 360.f, 360.f);
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionFromVelocity(
            const ACharacter* Character, bool bNormalize) {
            if (!Character || Character->GetVelocity().IsNearlyZero())
                return FScreenRelativeDirectionData::Default();
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionToTarget(
            const AActor* TargetActor, const ACharacter* Character, bool bNormalize) {
            if (!TargetActor || !Character)
                return FScreenRelativeDirectionData::Default();
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeActorRelativeDirectionFromInput(
            const FVector2D& InputVector, const AActor* ReferenceActor, bool bNormalize) {
            ...
            Result.WrappedAngle = FMath::Fmod(Result.Angle + 360.f, 360.f);
            Result.BlendFactors = ComputeBlendFactors_Inline(InputVector, RefRot, bNormalize);
            Result.Direction = MapAngleTo8WayDirection_Inline(Result.Angle);
            Result.Direction4 = MapAngleTo4WayDirection_Inline(Result.Angle);
            Result.Quadrant = MapAngleToQuadrant_Inline(Result.Angle);
            Result.ClampedStrafeDirection = FMath::Clamp(Result.BlendFactors.X, -1.f, 1.f);
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeActorRelativeDirectionFromVelocity(
            const ACharacter* SourceCharacter, const AActor* ReferenceActor, bool bNormalize) {
            if (!SourceCharacter || !ReferenceActor || SourceCharacter->GetVelocity().IsNearlyZero()) {
                return FScreenRelativeDirectionData::Default();
            }

            const FVector2D Velocity2D(SourceCharacter->GetVelocity().X, SourceCharacter->GetVelocity().Y);
            return ComputeActorRelativeDirectionFromInput(Velocity2D, ReferenceActor, bNormalize);
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeActorRelativeDirectionToTarget(
            ...
            const FVector2D ToTarget =
                FVector2D(TargetActor->GetActorLocation() - ReferenceActor->GetActorLocation()).GetSafeNormal();
            return ComputeActorRelativeDirectionFromInput(ToTarget, ReferenceActor, bNormalize);
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeComponentRelativeDirectionFromInput(
            const FVector2D& InputVector, const USceneComponent* ReferenceComponent, bool bNormalize) {
            if (!ReferenceComponent || InputVector.IsNearlyZero()) {
                return FScreenRelativeDirectionData::Default();
            }

            const FVector2D Forward(ReferenceComponent->GetForwardVector().X, ReferenceComponent->GetForwardVector().Y);
        ...
        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeComponentRelativeDirectionFromVelocity(
            const ACharacter* SourceCharacter, const USceneComponent* ReferenceComponent, bool bNormalize) {
            if (!SourceCharacter || !ReferenceComponent || SourceCharacter->GetVelocity().IsNearlyZero()) {
                return FScreenRelativeDirectionData::Default();
            }

            const FVector2D Velocity2D(SourceCharacter->GetVelocity().X, SourceCharacter->GetVelocity().Y);
            return ComputeComponentRelativeDirectionFromInput(Velocity2D, ReferenceComponent, bNormalize);
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::ComputeComponentRelativeDirectionToTarget(
            const AActor* TargetActor, const USceneComponent* ReferenceComponent, bool bNormalize) {
            if (!TargetActor || !ReferenceComponent) {
                return FScreenRelativeDirectionData::Default();
            }

            const FVector2D ToTarget =
                FVector2D(TargetActor->GetActorLocation() - ReferenceComponent->GetComponentLocation()).GetSafeNormal();
            return ComputeComponentRelativeDirectionFromInput(ToTarget, ReferenceComponent, bNormalize);
        }

        // ===========================================================================================
        float UOHLocomotionUtils::ComputeScreenRelativeDirection(const FVector2D& InputVector,
            ...
            const float ForwardDot = FVector2D::DotProduct(InputDir, Forward2D.GetSafeNormal());
            const float RightDot = FVector2D::DotProduct(InputDir, Right2D.GetSafeNormal());
        ...
        FVector2D UOHLocomotionUtils::GetScreenRelativeVector(const FVector2D& InputVector,
                                                              const APlayerController* PlayerController) {
            if (!PlayerController || InputVector.IsNearlyZero()) {
        ...
        float UOHLocomotionUtils::GetScreenRelativeAngleToTarget(
            ...
            const float AngleRad = FMath::Atan2(CamForward2D.X * ToTarget.Y - CamForward2D.Y * ToTarget.X,
                                                CamForward2D.X * ToTarget.X + CamForward2D.Y * ToTarget.Y);

            return FMath::RadiansToDegrees(AngleRad);
        }

        EScreenDirection4 UOHLocomotionUtils::GetScreenRelativeCardinalDirectionToTarget(
            const AActor* TargetActor, const APlayerController* PlayerController, const ACharacter* Character) {
            const float Angle = GetScreenRelativeAngleToTarget(TargetActor, PlayerController, Character);
        ...
        FVector2D UOHLocomotionUtils::GetScreenRelativeBlendFactorsToTarget(
            ...
            const FVector ToTarget =
                (TargetActor->GetActorLocation() - Character->GetActorLocation()).GetSafeNormal2D();
            const FRotator CamRot = PlayerController->PlayerCameraManager->GetCameraRotation();
        ...
        // Returns the character's movement angle relative to the screen/camera direction.
        float UOHLocomotionUtils::GetScreenRelativeAngleFromVelocity(const APlayerController* PlayerController,
            ...
            const FRotator CamRot = PlayerController->PlayerCameraManager->GetCameraRotation();
            const FVector2D CamForward(FVector2D(CamRot.Vector().X, CamRot.Vector().Y).GetSafeNormal());
            const FVector2D Velocity2D(Velocity.X, Velocity.Y);

            return ComputeScreenRelativeAngle(CamForward, Velocity2D, false);
        }

        EScreenDirection8 UOHLocomotionUtils::GetScreenRelative8WayDirectionFromVelocity(
            const APlayerController* PlayerController, const ACharacter* Character) {
            const float Angle = GetScreenRelativeAngleFromVelocity(PlayerController, Character);
            return MapAngleTo8WayDirection(Angle);
        }

        float UOHLocomotionUtils::GetScreenRelativeAngleFromInput(const FVector2D& InputVector,
                                                                  const APlayerController* PlayerController) {
            if (!PlayerController || InputVector.IsNearlyZero()) {
                return 0.0f;
            }
        ...
        EScreenDirection8 UOHLocomotionUtils::GetScreenRelative8WayDirectionFromInput(
            const FVector2D& InputVector, const APlayerController* PlayerController) {
            const float Angle = GetScreenRelativeAngleFromInput(InputVector, PlayerController);
            return MapAngleTo8WayDirection(Angle);
        }

        FVector UOHLocomotionUtils::GetCameraRelativeMovementVectorFromInput(
            const FVector2D& InputVector, const FRotator& CameraRotation, bool bNormalize) {
            if (InputVector.IsNearlyZero()) {
                return FVector::ZeroVector;
            }

            const FVector2D Blend = ComputeBlendFactors_Inline(InputVector, CameraRotation, bNormalize);
        ...
        FRotator UOHLocomotionUtils::GetCameraRelativeFacingRotationFromInput(
            ...
            const FVector MoveVector =
                GetCameraRelativeMovementVectorFromInput(InputVector, CameraRotation, bNormalize);
            return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
        }
        ...
        FVector UOHLocomotionUtils::GetActorRelativeMovementVectorFromInput(
            ...
            const FVector2D Blend = ComputeBlendFactors_Inline(InputVector, Rot, bNormalize);
        ...
        FRotator UOHLocomotionUtils::GetActorRelativeFacingRotationFromInput(
            const FVector2D& InputVector, const AActor* ReferenceActor, bool bNormalize) {
            const FVector MoveVector = GetActorRelativeMovementVectorFromInput(InputVector, ReferenceActor, bNormalize);
            return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
        }
        ...
        FRotator UOHLocomotionUtils::GetComponentRelativeFacingRotationFromInput(
            const FVector2D& InputVector, const USceneComponent* ReferenceComponent, bool bNormalize) {
            const FVector MoveVector =
                GetComponentRelativeMovementVectorFromInput(InputVector, ReferenceComponent, bNormalize);
            return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
        }

        FScreenRelativeDirectionData UOHLocomotionUtils::BuildScreenDirectionFromInput(
            const FVector2D& InputVector, const FRotator& ReferenceRotation, const AActor* ReferenceActor,
            const APlayerController* PlayerController, bool bAllowMirroring, bool bClamp) {
            FScreenRelativeDirectionData Result;
            ...
            const FVector Right3D = FRotationMatrix(ReferenceRotation).GetUnitAxis(EAxis::Y);
            ...
            float AngleDeg = FMath::RadiansToDegrees(FMath::Atan2(RightBlend, ForwardBlend));
            ...
            if (bAllowMirroring && ReferenceActor && PlayerController) {
                const FVector2D ActorForward =
                    FVector2D(ReferenceActor->GetActorForwardVector().X, ReferenceActor->GetActorForwardVector().Y)
                        .GetSafeNormal();
                const FVector2D CameraForward =
                    FVector2D(PlayerController->PlayerCameraManager->GetCameraRotation().Vector().X,
                              PlayerController->PlayerCameraManager->GetCameraRotation().Vector().Y)
                        .GetSafeNormal();
            ...
            Result.Direction = UOHLocomotionUtils::MapAngleTo8WayDirection(WrappedAngleDeg);
            Result.Direction4 = UOHLocomotionUtils::MapAngleTo4WayDirection(WrappedAngleDeg);
        ...
        FVector UOHLocomotionUtils::GetWorldMovementVectorFromInput(
            ...
            const FVector2D Blend = ComputeBlendFactors_Inline(
                InputVector, PlayerController->PlayerCameraManager->GetCameraRotation(), bNormalize);
            const FVector Forward = PlayerController->PlayerCameraManager->GetCameraRotation().Vector();
            const FVector Right =
                FRotationMatrix(PlayerController->PlayerCameraManager->GetCameraRotation()).GetUnitAxis(EAxis::Y);
        ...
        FRotator UOHLocomotionUtils::GetFacingRotationFromInput(
            const FVector2D& InputVector, const APlayerController* PlayerController, bool bNormalize) {
            const FVector MoveDir = GetWorldMovementVectorFromInput(InputVector, PlayerController, bNormalize);
            return MoveDir.IsNearlyZero() ? FRotator::ZeroRotator : MoveDir.Rotation();
        }

        FVector UOHLocomotionUtils::GetWorldMovementVectorFromBlendFactors(const FVector2D& BlendFactors,
                                                                           const APlayerController* PlayerController) {
        ...
        FRotator UOHLocomotionUtils::GetFacingRotationFromDirection(const FScreenRelativeDirectionData& DirectionData,
                                                                    const APlayerController* PlayerController) {
            return GetWorldMovementVectorFromBlendFactors(DirectionData.BlendFactors, PlayerController).Rotation();
        }

        FVector UOHLocomotionUtils::GetStrafeMovementVectorFromBlendFactors(const FVector2D& BlendFactors,
        ...
        FVector2D UOHLocomotionUtils::GetOptimalAnimationBlendVector(const FScreenRelativeDirectionData& DirectionData,
                                                                     bool bNormalize, bool bClamp) {
        ...
        FVector2D UOHLocomotionUtils::GetVisualConsistentAnimationBlendVector(
            const FScreenRelativeDirectionData& DirectionData, const AActor* ReferenceActor,
            const APlayerController* PlayerController, bool bNormalize, bool bClamp, bool bApplyMirroring) {
            if (!ReferenceActor || !PlayerController || DirectionData.BlendFactors.IsNearlyZero()) {
                return FVector2D::ZeroVector;
            }
            ...
            if (bApplyMirroring) {
                const FVector ActorForward = ReferenceActor->GetActorForwardVector().GetSafeNormal2D();
                const FVector CameraForward =
                    PlayerController->PlayerCameraManager->GetCameraRotation().Vector().GetSafeNormal2D();

                // Determine if actor is facing leftward on screen (from camera's perspective)
                const float Dot = FVector2D::DotProduct(FVector2D(ActorForward.X, ActorForward.Y),
        ...
        float UOHLocomotionUtils::GetVisualConsistentAnimationDirectionAngle(
            const FScreenRelativeDirectionData& DirectionData, const AActor* ReferenceActor,
            const APlayerController* PlayerController, bool bApplyMirroring, bool bNormalize, bool bClamp) {
            ...
            if (bClamp) {
                Angle = FMath::Clamp(Angle, bNormalize ? -1.f : -180.f, bNormalize ? 1.f : 180.f);
            }
        ...
        float UOHLocomotionUtils::GetPredictedDirectionAngleFromBoneMotion(
            const FOHBoneData& BoneData, const AActor* ReferenceActor, const APlayerController* PlayerController,
            float PredictAheadTime, bool bApplyMirroring, bool bNormalize, bool bClamp) {
            if (!ReferenceActor || !PlayerController || !BoneData.HasMotionHistory()) {
                return 0.f;
            }

            const FVector Start = BoneData.GetCurrentPosition();
            const FVector End = Start + BoneData.GetBodyLinearVelocity() * PredictAheadTime;
            ...
            if (bApplyMirroring) {
                const FVector2D ActorForward = FVector2D(ReferenceActor->GetActorForwardVector()).GetSafeNormal();
                const FVector2D CameraForward =
                    FVector2D(PlayerController->PlayerCameraManager->GetCameraRotation().Vector()).GetSafeNormal();
            ...
            if (bClamp) {
                AngleDeg = FMath::Clamp(AngleDeg, bNormalize ? -1.f : -180.f, bNormalize ? 1.f : 180.f);
            }
         ...
         * @param bNormalize If true, normalizes the input vectors before computing the angle.
         * @return The angle in degrees between the FromVector and ToVector in a screen-relative context.
         */
        float UOHLocomotionUtils::ComputeScreenRelativeAngle(const FVector2D& FromVector, const FVector2D& ToVector,
                                                             bool bNormalize) {
            ...
            const float AngleRad = FMath::Atan2(FVector2D::CrossProduct(From, To), FVector2D::DotProduct(From, To));

            return FMath::RadiansToDegrees(AngleRad); // Range: [-180, 180]
        }

        float UOHLocomotionUtils::PredictMovementAngle(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, float& InOutPrevAngle,

            float BasePredictionTime, float SpeedPredictionScale, float BaseSmoothingAlpha, float SpeedSmoothScale,
            float PelvisTranslationThreshold, float FootLockSpeedThreshold, float FootLockAccelThreshold,
            float PivotScale, float PivotDeadzone, float StrafeBias, float JitterThreshold, float MaxAngleJumpPerFrame,
            ...
            float Alpha = FMath::Clamp(BaseSmoothingAlpha - Speed * SpeedSmoothScale, 0.f, 1.f);
            ...
            if (auto* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>()) {
                ...
                float PM = PV.Size();
                float B = FMath::Clamp(PM / (RM * PelvisTranslationThreshold + KINDA_SMALL_NUMBER), 0.f, 1.f);

                FinalVel = FMath::Lerp(RV, PV, B);
                FinalAccel = FMath::Lerp(PhysMgr->GetLinearAcceleration(FName("root")),
                                         PhysMgr->GetLinearAcceleration(FName("pelvis")), B);

                MoveDir =
                    (FinalVel * PredictionTime + 0.5f * FinalAccel * PredictionTime * PredictionTime).GetSafeNormal();
            }
            ...
            if (MoveDir.IsNearlyZero()) {
                ...
                if (FinalVel.Size() < FootLockSpeedThreshold && FinalAccel.Size() > FootLockAccelThreshold) {
                    return InOutPrevAngle;
                }
            ...
            float TargetA = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LocalDir.Y, LocalDir.X)) + 360.f, 360.f);
            ...
            if (auto* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>()) {
                if (const FOHBoneData* PelvisData = PhysMgr->GetBoneData(FName("pelvis"))) {
                    ...
                    float TrajBlend = FMath::Clamp(JerkMag * JerkTriggerScale, 0.f, MaxTrajectoryBlend);

                    if (TrajBlend > KINDA_SMALL_NUMBER) {
                        TArray<FVector> CP =
                            UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(*PelvisData, PredictionTime);
                        ...
                        if (!SumDir.IsNearlyZero()) {
                            float AvgAngDeg =
                                FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(SumDir.Y, SumDir.X)) + 360.f, 360.f);
            ...
            // 8) Clamp & jitter-filter
            {
                ...
                if (FMath::Abs(DeltaA) > MaxAngleJumpPerFrame)
                    TargetA = InOutPrevAngle + FMath::Clamp(DeltaA, -MaxAngleJumpPerFrame, MaxAngleJumpPerFrame);
                if (FMath::Abs(DeltaA) < JitterThreshold)
                    TargetA = InOutPrevAngle;
            }

            // 9) Exponential smoothing
            float Smoothed = InOutPrevAngle + Alpha * FMath::FindDeltaAngleDegrees(InOutPrevAngle, TargetA);
        ...
        float UOHLocomotionUtils::PredictMovementAngleFromHistory(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, float /*PredictionTime*/,
            float /*SpeedPredictionScale*/, float PivotDeadzone, float PivotScale, float StrafeBias) {
            if (!Character)
                return 0.f;

            // Grab physics manager and root‐bone data
            const UOHPhysicsManager* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>();
            FVector MoveDir = FVector::ZeroVector;
            ...
            if (PhysMgr) {
                ...
                if (MoveDir.IsNearlyZero() && PhysMgr) {
                    ...
                    if (RootData2) {
                        ...
                        // FOHBoneData::GetSmoothedLinearVelocity :contentReference[oaicite:0]{index=0}
                    }
                }
            }
            ...
            if (PhysMgr) {
                ...
                if (RD) {
                    ...
                    // FOHBoneData::GetSmoothedLinearAcceleration :contentReference[oaicite:1]{index=1}
        ...
        float UOHLocomotionUtils::PredictMovementAngleWithBezierDebug(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, UWorld* World, float DrawDuration,
            FColor CurveColor, int32 NumSegments, float PredictionTime, float SpeedPredictionScale, float PivotDeadzone,
            ...
            // Fetch root data
            const UOHPhysicsManager* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>();
            const FOHBoneData* RD = PhysMgr ? PhysMgr->GetBoneData(FName("root")) : nullptr;
            ...
            for (int32 i = 1; i <= NumSegments; ++i) {
                ...
                FVector B = FMath::Lerp(P1, P2, t);
                FVector Pt = FMath::Lerp(A, B, t);
                DrawDebugLine(World, PrevPoint, Pt, CurveColor, false, DrawDuration, 0, LineWidth);
                PrevPoint = Pt;
            }
        ...
        float UOHLocomotionUtils::PredictMovementAngleMonster(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, UWorld* World, float DrawDuration,
            float PredictionTime, float SpeedPredictionScale, float AccelPredictionScale, float JerkScale,
            float CurvatureScale, float MaxBlend, float PivotDeadzone, float PivotScale, float StrafeBias,
            float RotationThreshold, float MoveThreshold, float ProcNoise, float MeasNoise) {
            ...
            static const FName BoneNames[] = {TEXT("root"), TEXT("pelvis"), TEXT("spine_01"), TEXT("spine_02")};
            UOHPhysicsManager* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>();
            if (!PhysMgr)
                return PrevAng;
            ...
            FVector RawDir;
            {
                ...
                if (DP.IsNearlyZero()) {
                    ...
                    RawDir = R ? R->GetSmoothedLinearVelocity().GetSafeNormal() : FVector::ZeroVector;
                } else {
            ...
            float PolyAng = 0.f;
            {
                ...
                if (FMath::Abs(Det) > KINDA_SMALL_NUMBER) {
                    ...
                    double A1 =
                        (SX0 * (S2 * S4 - S3 * S3) - S1 * (SX1 * S4 - S3 * SX2) + S2 * (SX1 * S3 - S2 * SX2)) / Det;
                    double B1 =
                        (SY0 * (S2 * S4 - S3 * S3) - S1 * (SY1 * S4 - S3 * SY2) + S2 * (SY1 * S3 - S2 * SY2)) / Det;
                    double A2 =
                        (S0 * (SX1 * S4 - S3 * SX2) - SX0 * (S1 * S4 - S3 * S2) + S2 * (S1 * SX2 - SX1 * S2)) / Det;
                    double B2 =
                        (S0 * (SY1 * S4 - S3 * SY2) - SY0 * (S1 * S4 - S3 * S2) + S2 * (S1 * SY2 - SY1 * S2)) / Det;
                    ...
                    if (FMath::Abs(dx) + FMath::Abs(dy) > SMALL_NUMBER) {
                        ...
                        PolyAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(L.Y, L.X)) + 360.f, 360.f);
                    }
                }
            }

            // --- 7) Quadratic Bézier & debug draw → BezAng ---
            TArray<FVector> CP = UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(
            ...
            float BezAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LB.Y, LB.X)) + 360.f, 360.f);

            // --- 8) Build measured angle: raw→poly→bezier blend ---
            FVector LR = FacingRotation.UnrotateVector(RawDir);
            float RawAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LR.Y, LR.X)) + 360.f, 360.f);
            ...
            {
                const FOHBoneData* RD = PhysMgr->GetBoneData(TEXT("root"));
                FVector Accn = RD ? RD->GetSmoothedLinearAcceleration() : FVector::ZeroVector;
            ...
            static const float SG[5] = {-3.f / 35.f, 12.f / 35.f, 17.f / 35.f, 12.f / 35.f, -3.f / 35.f};
            float SmMeas = 0.f;
            for (int i = 0; i < 5; ++i)
                SmMeas += SG[i] * SGBuf[i];
        ...
        float UOHLocomotionUtils::PredictMovementAngleMonsterNoDebug(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, float PredictionTime,
            float SpeedPredictionScale, float AccelPredictionScale, float JerkScale, float CurvatureScale,
            float MaxBlend, float PivotDeadzone, float PivotScale, float StrafeBias, float RotationThreshold,
            float MoveThreshold, float ProcNoise, float MeasNoise) {
            ...
            FVector LR = FacingRotation.UnrotateVector(RawDir);
            float RawAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LR.Y, LR.X)) + 360.f, 360.f);
            float Meas = FMath::Lerp(FMath::Lerp(RawAng, PolyAng, PolyBlend), BezAng, BezBlend);
            ...
            // --- 9) Pivot & strafe bias ---
            {
                ...
                float BiasT = FMath::Lerp(180.f, 0.f, (StrafeBias + 1.f) * 0.5f);
                Meas = FMath::Fmod(FMath::Lerp(Meas, BiasT, FMath::Abs(StrafeBias)) + 360.f, 360.f);
            }
...
#pragma region INLINE Wrappers
        ...
        FVector2D UOHLocomotionUtils::ComputeBlendFactors(const FVector2D& Direction, const FRotator& CameraRotation,
                                                          bool bNormalize) {
        ...
        float UOHLocomotionUtils::PredictMovementAngleMonsterExtended(
            ACharacter * Character, const FRotator& FacingRotation, float DeltaTime, float& OutBezierT,
            float PredictionTime, float SpeedPredictionScale, float AccelPredictionScale, float JerkScale,
            float CurvatureScale, float MaxBlend, float PivotDeadzone, float PivotScale, float StrafeBias,
            float RotationThreshold, float MoveThreshold, float SGCurvThreshold, float FootPlantSpeedThresh,
            float FootPlantAccelThresh, float FootPlantHoldTime, float TeleportThreshold, float VarMin, float VarMax,
            ...
            LastPlantTime =
                FMath::Max(LastPlantTime, FMath::Max(DetectPlant(TEXT("foot_l")), DetectPlant(TEXT("foot_r"))));
            ...
            float FinalT = PredictionTime + Character->GetVelocity().Size() * SpeedPredictionScale;
            FinalT *= 1.f + (RootBD->GetSmoothedLinearAcceleration().Size() * AccelPredictionScale);
            ...
            float PolyAng = 0.f;
            {
                ...
                double SY0 = 0, SY1 = 0, SY2 = 0;
                for (int i = 0; i < Count; ++i) {
                    double t = H[i].GetTimeStamp(), x = H[i].GetLocation().X, y = H[i].GetLocation().Y;
                    double t2 = t * t, t3 = t2 * t, t4 = t2 * t2;
                    S0 += 1;
                    S1 += t;
                    S2 += t2;
            ...
            TArray<FVector> CP = UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(*RootBD, FinalT);
            const FVector &P0 = CP[0], &P1 = CP[1], &P2 = CP[2];
            FVector BezDir = (P2 - P1).GetSafeNormal();
            FVector LB = FacingRotation.UnrotateVector(BezDir);
            float BezAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LB.Y, LB.X)) + 360.f, 360.f);
            ...
            FVector LR = FacingRotation.UnrotateVector(RawDir);
            float RawAng = FMath::Fmod(FMath::RadiansToDegrees(FMath::Atan2(LR.Y, LR.X)) + 360.f, 360.f);
            float MeasAng = FMath::Lerp(FMath::Lerp(RawAng, PolyAng, Pblend), BezAng, Bblend);
            ...
            // —11) Pivot anticipation & strafe bias on angle—
            {
                ...
                MeasAng = FMath::Fmod(FMath::Lerp(MeasAng, BT, FMath::Abs(StrafeBias)) + 360.f, 360.f);
            }

            // —12) Per-axis SG smoothing of direction vector—
            FVector MeasDir(FMath::Cos(FMath::DegreesToRadians(MeasAng)), FMath::Sin(FMath::DegreesToRadians(MeasAng)),
                            0.f);
            // shift buffer
            if (DirCount < 7)
                ++DirCount;
            ...
            static const float SG5[5] = {-3.f / 35, 12.f / 35, 17.f / 35, 12.f / 35, -3.f / 35};
            static const float SG7[7] = {-2.f / 21, 3.f / 21, 6.f / 21, 7.f / 21, 6.f / 21, 3.f / 21, -2.f / 21};
            FVector S(0, 0, 0);
            if (DirCount >= Win) {
