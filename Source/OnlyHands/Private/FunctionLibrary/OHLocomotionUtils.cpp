...
FPredictedMotionDirectionData UOHLocomotionUtils::ComputeSmartAnimationDirectionAuto(ACharacter* Character,
                                                                                     float DeltaTime, float StrafeBias,
    ...
    if (PhysicsManager) {
        ...
        const FVector PredictDelta = FinalVel * PredictionTime + 0.5f * FinalAccel * FMath::Square(PredictionTime);
...
void UOHLocomotionUtils::ApplyStableCameraRelativeMovement(ACharacter* Character, const FVector2D& Input) {
    ...
    float AngleDelta = FMath::Acos(FVector2D::DotProduct(Input.GetSafeNormal(), LastInputDirection.GetSafeNormal()));
...
void UOHLocomotionUtils::ApplyJoystickMovementWithBasis(APawn* Pawn, const FVector2D& Input,
                                                        const FRotator& BasisRotation, FVector UpVector,
                                                        float Deadzone) {
...
void UOHLocomotionUtils::ApplySmartCameraRelativeJoystickMovement(ACharacter* Character, const FVector2D& Input,
                                                                  bool bFaceMovementDirection, bool bConstrainToXYPlane,
                                                                  float Deadzone) {
    if (!Character || Input.SizeSquared() < Deadzone * Deadzone)
        return;
...
void UOHLocomotionUtils::ApplyFullCameraRelativeJoystickMovement(ACharacter* Character, const FVector2D& Input,
                                                                 bool bFaceMovementDirection, bool bConstrainToXYPlane,
                                                                 float Deadzone, float TurnSpeed,
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionFromInput(const FVector2D& InputVector,
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionFromVelocity(const APlayerController* PlayerController,
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeDirectionToTarget(const AActor* TargetActor,
                                                                          const APlayerController* PlayerController,
                                                                          const ACharacter* Character,
                                                                          bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionFromInput(const FVector2D& InputVector,
                                                                                         const ACharacter* Character,
                                                                                         bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionFromVelocity(const ACharacter* Character,
                                                                                            bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputePlayerRelativeDirectionToTarget(const AActor* TargetActor,
                                                                                        const ACharacter* Character,
                                                                                        bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionFromInput(const FVector2D& InputVector,
                                                                                        bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionFromVelocity(const ACharacter* Character,
                                                                                           bool bNormalize) {
    if (!Character || Character->GetVelocity().IsNearlyZero())
        return FScreenRelativeDirectionData::Default();
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeWorldRelativeDirectionToTarget(const AActor* TargetActor,
                                                                                       const ACharacter* Character,
                                                                                       bool bNormalize) {
    if (!TargetActor || !Character)
        return FScreenRelativeDirectionData::Default();
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeActorRelativeDirectionFromInput(const FVector2D& InputVector,
    ...
    const FVector2D Forward(ReferenceActor->GetActorForwardVector().X, ReferenceActor->GetActorForwardVector().Y);
...
UOHLocomotionUtils::ComputeActorRelativeDirectionFromVelocity(const ACharacter* SourceCharacter,
                                                              const AActor* ReferenceActor, bool bNormalize) {
...
FScreenRelativeDirectionData UOHLocomotionUtils::ComputeActorRelativeDirectionToTarget(const AActor* TargetActor,
                                                                                       const AActor* ReferenceActor,
                                                                                       bool bNormalize) {
...
float UOHLocomotionUtils::GetScreenRelativeAngleToTarget(const AActor* TargetActor,
                                                         const APlayerController* PlayerController,
                                                         const ACharacter* Character) {
    if (!PlayerController || !TargetActor || !Character) {
        return 0.0f;
...
EScreenDirection8 UOHLocomotionUtils::GetScreenRelative8WayDirectionToTarget(const AActor* TargetActor,
                                                                             const APlayerController* PlayerController,
                                                                             const ACharacter* Character) {
    const float Angle = GetScreenRelativeAngleToTarget(TargetActor, PlayerController, Character);
...
FVector2D UOHLocomotionUtils::GetScreenRelativeBlendFactorsToTarget(const AActor* TargetActor,
                                                                    const APlayerController* PlayerController,
                                                                    const ACharacter* Character) {
    if (!PlayerController || !TargetActor || !Character) {
        return FVector2D::ZeroVector;
    ...
    const FVector ToTarget = (TargetActor->GetActorLocation() - Character->GetActorLocation()).GetSafeNormal2D();
    const FRotator CamRot = PlayerController->PlayerCameraManager->GetCameraRotation();

    return ComputeBlendFactors(FVector2D(ToTarget.X, ToTarget.Y), CamRot);
}

FVector2D UOHLocomotionUtils::GetScreenRelativeBlendFactorsFromVelocity(const APlayerController* PlayerController,
                                                                        const ACharacter* Character) {
...
FVector2D UOHLocomotionUtils::GetScreenRelativeBlendFactorsFromInput(const FVector2D& InputVector,
                                                                     const APlayerController* PlayerController) {
    if (!PlayerController || InputVector.IsNearlyZero()) {
        return FVector2D::ZeroVector;
    }
...
EScreenDirection8
UOHLocomotionUtils::GetScreenRelative8WayDirectionFromVelocity(const APlayerController* PlayerController,
                                                               const ACharacter* Character) {
...
EScreenDirection8
UOHLocomotionUtils::GetScreenRelative8WayDirectionFromInput(const FVector2D& InputVector,
                                                            const APlayerController* PlayerController) {
    const float Angle = GetScreenRelativeAngleFromInput(InputVector, PlayerController);
    return MapAngleTo8WayDirection(Angle);
}

FVector UOHLocomotionUtils::GetCameraRelativeMovementVectorFromInput(const FVector2D& InputVector,
                                                                     const FRotator& CameraRotation, bool bNormalize) {
    if (InputVector.IsNearlyZero()) {
...
FRotator UOHLocomotionUtils::GetCameraRelativeFacingRotationFromInput(const FVector2D& InputVector,
                                                                      const FRotator& CameraRotation, bool bNormalize) {
    const FVector MoveVector = GetCameraRelativeMovementVectorFromInput(InputVector, CameraRotation, bNormalize);
    return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
}

FVector UOHLocomotionUtils::GetActorRelativeMovementVectorFromInput(const FVector2D& InputVector,
                                                                    const AActor* ReferenceActor, bool bNormalize) {
    if (!ReferenceActor || InputVector.IsNearlyZero()) {
        return FVector::ZeroVector;
    }
...
FRotator UOHLocomotionUtils::GetActorRelativeFacingRotationFromInput(const FVector2D& InputVector,
                                                                     const AActor* ReferenceActor, bool bNormalize) {
    const FVector MoveVector = GetActorRelativeMovementVectorFromInput(InputVector, ReferenceActor, bNormalize);
    return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
}

FVector UOHLocomotionUtils::GetComponentRelativeMovementVectorFromInput(const FVector2D& InputVector,
...
FRotator UOHLocomotionUtils::GetComponentRelativeFacingRotationFromInput(const FVector2D& InputVector,
                                                                         const USceneComponent* ReferenceComponent,
                                                                         bool bNormalize) {
    const FVector MoveVector = GetComponentRelativeMovementVectorFromInput(InputVector, ReferenceComponent, bNormalize);
    return MoveVector.IsNearlyZero() ? FRotator::ZeroRotator : MoveVector.Rotation();
}
    ...
    if (bAllowMirroring && ReferenceActor && PlayerController) {
        const FVector2D ActorForward =
            FVector2D(ReferenceActor->GetActorForwardVector().X, ReferenceActor->GetActorForwardVector().Y)
                .GetSafeNormal();
        const FVector2D CameraForward = FVector2D(PlayerController->PlayerCameraManager->GetCameraRotation().Vector().X,
...
FVector UOHLocomotionUtils::GetWorldMovementVectorFromInput(const FVector2D& InputVector,
                                                            const APlayerController* PlayerController,
                                                            bool bNormalize) {
    if (!PlayerController || InputVector.IsNearlyZero()) {
        return FVector::ZeroVector;
    ...
    const FVector2D Blend =
        ComputeBlendFactors_Inline(InputVector, PlayerController->PlayerCameraManager->GetCameraRotation(), bNormalize);
...
FRotator UOHLocomotionUtils::GetFacingRotationFromInput(const FVector2D& InputVector,
                                                        const APlayerController* PlayerController, bool bNormalize) {
...
FVector2D UOHLocomotionUtils::GetVisualConsistentAnimationBlendVector(const FScreenRelativeDirectionData& DirectionData,
                                                                      const AActor* ReferenceActor,
                                                                      const APlayerController* PlayerController,
                                                                      bool bNormalize, bool bClamp,
                                                                      bool bApplyMirroring) {
...
float UOHLocomotionUtils::GetVisualConsistentAnimationDirectionAngle(const FScreenRelativeDirectionData& DirectionData,
                                                                     const AActor* ReferenceActor,
                                                                     const APlayerController* PlayerController,
                                                                     bool bApplyMirroring, bool bNormalize,
                                                                     bool bClamp) {
...
float UOHLocomotionUtils::GetPredictedDirectionAngleFromBoneMotion(const FOHBoneData& BoneData,
                                                                   const AActor* ReferenceActor,
                                                                   const APlayerController* PlayerController,
                                                                   float PredictAheadTime, bool bApplyMirroring,
                                                                   bool bNormalize, bool bClamp) {
...
FVector2D UOHLocomotionUtils::GetPredictedBlendVectorFromBoneMotion(const FOHBoneData& BoneData,
                                                                    const AActor* ReferenceActor,
                                                                    const APlayerController* PlayerController,
                                                                    float PredictAheadTime, bool bApplyMirroring,
                                                                    bool bNormalize, bool bClamp) {
...
float UOHLocomotionUtils::PredictMovementAngle(ACharacter* Character, const FRotator& FacingRotation, float DeltaTime,
    ...
    if (auto* PhysMgr = Character->FindComponentByClass<UOHPhysicsManager>()) {
        ...
        MoveDir = (FinalVel * PredictionTime + 0.5f * FinalAccel * PredictionTime * PredictionTime).GetSafeNormal();
    }
...
float UOHLocomotionUtils::PredictMovementAngleFromHistory(ACharacter* Character, const FRotator& FacingRotation,
                                                          float DeltaTime, float /*PredictionTime*/,
                                                          float /*SpeedPredictionScale*/, float PivotDeadzone,
                                                          float PivotScale, float StrafeBias) {
    if (!Character)
        return 0.f;
...
float UOHLocomotionUtils::PredictMovementAngleWithBezierDebug(ACharacter* Character, const FRotator& FacingRotation,
                                                              float DeltaTime, UWorld* World, float DrawDuration,
                                                              FColor CurveColor, int32 NumSegments,
                                                              float PredictionTime, float SpeedPredictionScale,
                                                              float PivotDeadzone, float PivotScale,
                                                              float JerkThreshold, float StrafeBias) {
    if (!Character || !World)
        return 0.f;
...
float UOHLocomotionUtils::PredictMovementAngleMonster(ACharacter* Character, const FRotator& FacingRotation,
                                                      float DeltaTime, UWorld* World, float DrawDuration,
                                                      float PredictionTime, float SpeedPredictionScale,
                                                      float AccelPredictionScale, float JerkScale, float CurvatureScale,
                                                      float MaxBlend, float PivotDeadzone, float PivotScale,
    ...
    float PolyAng = 0.f;
    {
        ...
        if (FMath::Abs(Det) > KINDA_SMALL_NUMBER) {
            ...
            double A1 = (SX0 * (S2 * S4 - S3 * S3) - S1 * (SX1 * S4 - S3 * SX2) + S2 * (SX1 * S3 - S2 * SX2)) / Det;
            double B1 = (SY0 * (S2 * S4 - S3 * S3) - S1 * (SY1 * S4 - S3 * SY2) + S2 * (SY1 * S3 - S2 * SY2)) / Det;
            double A2 = (S0 * (SX1 * S4 - S3 * SX2) - SX0 * (S1 * S4 - S3 * S2) + S2 * (S1 * SX2 - SX1 * S2)) / Det;
            double B2 = (S0 * (SY1 * S4 - S3 * SY2) - SY0 * (S1 * S4 - S3 * S2) + S2 * (S1 * SY2 - SY1 * S2)) / Det;
    ...
    TArray<FVector> CP =
        UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(*PhysMgr->GetBoneData(TEXT("root")), FinalPredTime);
    const FVector &P0 = CP[0], &P1 = CP[1], &P2 = CP[2];
...
float UOHLocomotionUtils::PredictMovementAngleMonsterNoDebug(ACharacter* Character, const FRotator& FacingRotation,
                                                             float DeltaTime, float PredictionTime,
                                                             float SpeedPredictionScale, float AccelPredictionScale,
                                                             float JerkScale, float CurvatureScale, float MaxBlend,
                                                             float PivotDeadzone, float PivotScale, float StrafeBias,
                                                             float RotationThreshold, float MoveThreshold,
                                                             float ProcNoise, float MeasNoise) {
...
float UOHLocomotionUtils::PredictMovementAngleMonsterExtended(
    ...
    LastPlantTime = FMath::Max(LastPlantTime, FMath::Max(DetectPlant(TEXT("foot_l")), DetectPlant(TEXT("foot_r"))));
    ...
    FVector MeasDir(FMath::Cos(FMath::DegreesToRadians(MeasAng)), FMath::Sin(FMath::DegreesToRadians(MeasAng)), 0.f);
    // shift buffer
    if (DirCount < 7)
        ++DirCount;
