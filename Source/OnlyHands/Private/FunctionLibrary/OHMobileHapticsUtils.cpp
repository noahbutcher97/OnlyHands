...
void UOHMobileHapticsUtils::PlayMobileHapticPatternEx(UObject* WorldContext, const TArray<FOHHapticEventEx>& Events, bool bLeftHand)
{
#if PLATFORM_IOS
...
void UOHMobileHapticsUtils::PlayMobileHapticSimple(UObject* WorldContext, float Intensity, float Sharpness, float Duration, bool bTransient, bool bLeftHand)
{
#if PLATFORM_IOS
...
namespace
{
    ...
    void BakeCurveToArray(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes, TArray<float>& OutValues)
    {
    ...
    bool bLeftHand)
{
    ...
    for (const FOHHapticEventCurve& Src : Events)
    {
        ...
        BakeCurveToArray(Src.IntensityCurve, Src.Duration, SampleRate, Baked.IntensityCurve.Times, Baked.IntensityCurve.Values);
        BakeCurveToArray(Src.SharpnessCurve, Src.Duration, SampleRate, Baked.SharpnessCurve.Times, Baked.SharpnessCurve.Values);
...
void UOHMobileHapticsUtils::PlayMobileHapticCurve(
	UObject* WorldContext,
...
namespace
{
    ...
    void BakeCurveToArray_Fixed(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes, TArray<float>& OutValues)
    {
        int32 NumSamples = FMath::Max(2, FMath::RoundToInt(Duration * SampleRate));
        OutTimes.Reset(NumSamples);
        OutValues.Reset(NumSamples);
    ...
    void BakeCurveToArray_Auto(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes, TArray<float>& OutValues)
    {
...
void UOHMobileHapticsUtils::PlayMobileHapticCurve_FixedRate(
	UObject* WorldContext, UCurveFloat* IntensityCurve, float Duration, float SampleRate)
{
	PlayMobileHapticCurve_Internal(WorldContext, IntensityCurve, Duration, SampleRate, EHapticCurveSamplingMode::FixedRate);
}
