...
void UOHMobileHapticsUtils::PlayMobileHapticPatternEx(UObject* WorldContext, const TArray<FOHHapticEventEx>& Events,
                                                      bool bLeftHand) {
#if PLATFORM_IOS
    ...
    if (Events.Num() > 0) {
        ...
        float Strength = (First.IntensityCurve.Values.Num() > 0) ? First.IntensityCurve.Values[0] : 1.f;
        if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContext, 0)) {
            PC->PlayDynamicForceFeedback(Strength, First.Duration, true, true, true, true);
        }
    }
#endif
}
...
void UOHMobileHapticsUtils::PlayMobileHapticSimple(UObject* WorldContext, float Intensity, float Sharpness,
                                                   float Duration, bool bTransient, bool bLeftHand) {
#if PLATFORM_IOS
...
void BakeCurveToArray(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes,
...
void UOHMobileHapticsUtils::PlayMobileHapticPatternWithCurves(UObject* WorldContext,
    ...
    for (const FOHHapticEventCurve& Src : Events) {
        ...
        BakeCurveToArray(Src.IntensityCurve, Src.Duration, SampleRate, Baked.IntensityCurve.Times,
                         Baked.IntensityCurve.Values);
        BakeCurveToArray(Src.SharpnessCurve, Src.Duration, SampleRate, Baked.SharpnessCurve.Times,
                         Baked.SharpnessCurve.Values);
    ...
    if (Events.Num() > 0) {
        ...
        float Strength = (First.IntensityCurve) ? First.IntensityCurve->GetFloatValue(0.f) : 1.f;
        if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContext, 0)) {
            PC->PlayDynamicForceFeedback(Strength, First.Duration, true, true, true, true);
        }
    }
...
void UOHMobileHapticsUtils::PlayMobileHapticCurve(UObject* WorldContext, UCurveFloat* IntensityCurve, float Duration,
                                                  float SampleRate) {
    ...
    float Strength = (IntensityCurve->FloatCurve.Keys.Num() > 0) ? IntensityCurve->FloatCurve.Keys[0].Value : 1.f;
    if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContext, 0)) {
...
void BakeCurveToArray_Fixed(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes,
                            TArray<float>& OutValues) {
    int32 NumSamples = FMath::Max(2, FMath::RoundToInt(Duration * SampleRate));
    OutTimes.Reset(NumSamples);
    OutValues.Reset(NumSamples);
...
void BakeCurveToArray_Keys(UCurveFloat* Curve, float Duration, TArray<float>& OutTimes, TArray<float>& OutValues) {
...
void BakeCurveToArray_Auto(UCurveFloat* Curve, float Duration, float SampleRate, TArray<float>& OutTimes,
                           TArray<float>& OutValues) {
...
void UOHMobileHapticsUtils::PlayMobileHapticCurve_Internal(UObject* WorldContext, UCurveFloat* IntensityCurve,
...
void UOHMobileHapticsUtils::PlayMobileHapticCurve_FixedRate(UObject* WorldContext, UCurveFloat* IntensityCurve,
                                                            float Duration, float SampleRate) {
    PlayMobileHapticCurve_Internal(WorldContext, IntensityCurve, Duration, SampleRate,
                                   EHapticCurveSamplingMode::FixedRate);
}

void UOHMobileHapticsUtils::PlayMobileHapticCurve_Keys(UObject* WorldContext, UCurveFloat* IntensityCurve,
                                                       float Duration) {
    PlayMobileHapticCurve_Internal(WorldContext, IntensityCurve, Duration, 60.f, EHapticCurveSamplingMode::CurveKeys);
}

void UOHMobileHapticsUtils::PlayMobileHapticCurve_Auto(UObject* WorldContext, UCurveFloat* IntensityCurve,
                                                       float Duration, float SampleRate) {
    PlayMobileHapticCurve_Internal(WorldContext, IntensityCurve, Duration, SampleRate, EHapticCurveSamplingMode::Auto);
}