...
FVector UOHCombatUtils::MirrorVectorOverAxis(const FVector& Input, const FVector& Axis) {
    ...
    return Input - 2.f * FVector::DotProduct(Input, NormalizedAxis) * NormalizedAxis;
}

float UOHCombatUtils::PredictTravelTime(const FVector& From, const FVector& To, float Speed) {
    if (Speed <= 0.f) {
...
FVector UOHCombatUtils::ApplyLinearDrag(const FVector& Velocity, float DragCoefficient, float DeltaTime) {
    float DragFactor = 1.f - (DragCoefficient * DeltaTime);
    DragFactor = FMath::Clamp(DragFactor, 0.f, 1.f);
    return Velocity * DragFactor;
}

FVector UOHCombatUtils::ApplyFriction(FVector Velocity, float Friction, float DeltaTime) {
    const float Speed = Velocity.Size();
    const float Drop = Speed * Friction * DeltaTime;
    const float NewSpeed = FMath::Max(Speed - Drop, 0.0f);
    return (Speed > 0.f) ? Velocity.GetSafeNormal() * NewSpeed : FVector::ZeroVector;
}

float UOHCombatUtils::GetRelativeFootHeight(USkeletalMeshComponent* Mesh, FName FootBone) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetLegSeparationRatio(USkeletalMeshComponent* Mesh, FName LeftFootBone, FName RightFootBone) {
    if (!Mesh) {
        return 0.f;
    }
...
bool UOHCombatUtils::TraceGroundBelowBone(USkeletalMeshComponent* Mesh, FName BoneName, float MaxDistance,
                                          FHitResult& OutHit) {
    if (!Mesh) {
        return false;
    }
...
bool UOHCombatUtils::IsFootApproachingGround(USkeletalMeshComponent* Mesh, FName FootBone, FVector BoneVelocity,
                                             float MaxDistance, float MinDescendSpeed) {
    if (!Mesh) {
        return false;
    }
...
bool UOHCombatUtils::IsFootNearGround(USkeletalMeshComponent* Mesh, FName FootBone, float MaxDistance) {
    if (!Mesh) {
        return false;
    }
...
float UOHCombatUtils::GetKneeRaiseHeight(USkeletalMeshComponent* Mesh, FName FootBone, FName PelvisBone) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetStrideWidth(USkeletalMeshComponent* Mesh, FName LeftFootBone, FName RightFootBone) {
    if (!Mesh) {
        return 0.f;
    }
...
int32 UOHCombatUtils::GetStepLeadFoot(USkeletalMeshComponent* Mesh, FName LeftFootBone, FName RightFootBone) {
    if (!Mesh) {
        return 0;
    }
...
float UOHCombatUtils::GetTorsoLeanAngle(USkeletalMeshComponent* Mesh, FName SpineBone) {
    if (!Mesh) {
        return 0.f;
    }
    ...
    float Angle =
        FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(SpineForward.GetSafeNormal(), FVector::UpVector)));
    return SpineForward.Z > 0 ? -Angle : Angle; // Negative = forward lean
}

float UOHCombatUtils::GetUpperBodyTwist(USkeletalMeshComponent* Mesh, FName LowerSpine, FName UpperSpine) {
    if (!Mesh) {
        return 0.f;
    }
    ...
    float Angle = FMath::RadiansToDegrees(
        FMath::Acos(FVector::DotProduct(LowerForward.GetSafeNormal2D(), UpperForward.GetSafeNormal2D())));
    return Angle;
}

float UOHCombatUtils::GetPelvisCompressionRatio(USkeletalMeshComponent* Mesh, FName PelvisBone, float StandingHeight) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetSpineArchAngle(USkeletalMeshComponent* Mesh, FName UpperSpine, FName HeadBone) {
    if (!Mesh) {
        return 0.f;
    }
    ...
    float Angle = FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(Direction, FVector::UpVector)));
    return Angle;
}
...
float UOHCombatUtils::GetCumulativeLean(USkeletalMeshComponent* Mesh) {
    ...
    const float LeanScore = FMath::Clamp(LeanDeg / 45.f, 0.f, 1.f); // 1 = max lean
    ...
    // Weighted blend
    const float Composite = LeanScore * 0.4f + TwistScore * 0.2f + CompressionScore * 0.4f;

    return FMath::Clamp(Composite, 0.f, 1.f);
}

bool UOHCombatUtils::IsUpperBodyGuardDropped(USkeletalMeshComponent* Mesh, FName ChestBone, FName LeftElbow,
                                             FName RightElbow) {
    if (!Mesh) {
        return false;
    }
...
bool UOHCombatUtils::IsCharacterLeaningForward(USkeletalMeshComponent* Mesh, FName SpineBone) {
    if (!Mesh) {
        return false;
    }
    ...
    float Dot = FVector::DotProduct(SpineForward.GetSafeNormal2D(), ActorForward.GetSafeNormal2D());
    return Dot > 0.5f;
}

float UOHCombatUtils::GetArmExtensionRatio(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone) {
    if (!Mesh) {
        return 0.f;
    }
...
bool UOHCombatUtils::IsArmCrossedMidline(USkeletalMeshComponent* Mesh, FName HandBone, FName PelvisBone) {
    if (!Mesh) {
        return false;
    }
    ...
    float PelvisX = Mesh->GetComponentTransform().InverseTransformPosition(Pelvis).Y;

    return (LocalX * PelvisX < 0); // Opposite sides of centerline
}

float UOHCombatUtils::GetShoulderCompressionRatio(USkeletalMeshComponent* Mesh, FName ClavicleLeft,
                                                  FName ClavicleRight) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetHandReachExtent(USkeletalMeshComponent* Mesh, FName HandBone, FName PelvisBone) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetCrossBodyReach(USkeletalMeshComponent* Mesh, FName HandBone, FName PelvisBone) {
    if (!Mesh) {
        return 0.f;
    }

    FVector Hand = Mesh->GetComponentTransform().InverseTransformPosition(Mesh->GetBoneLocation(HandBone));
    return FMath::Abs(Hand.Y);
}

FVector UOHCombatUtils::GetArmLineDirection(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone) {
    if (!Mesh) {
...
bool UOHCombatUtils::IsHandWithinGuardZone(USkeletalMeshComponent* Mesh, FName HandBone, FName ChestBone,
                                           float MaxDistance) {
    if (!Mesh) {
        return false;
    }
...
bool UOHCombatUtils::IsArmAlignedWithMovement(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone,
                                              FVector ActorVelocity, float ThresholdDegrees) {
...
float UOHCombatUtils::GetArmToFacingAngle(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone) {
    if (!Mesh) {
        return 0.f;
    }
...
float UOHCombatUtils::GetStrikePrepAlignmentScore(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone,
    ...
    // Arm extension score
    float ExtensionRatio = GetArmExtensionRatio(Mesh, ClavicleBone, HandBone); // 0 = retracted, 1 = fully extended
    float ExtensionScore = 1.f - FMath::Abs(ExtensionRatio - 0.7f);            // Best score near 0.7

    // Lean score
    float LeanAngle = FMath::Abs(GetTorsoLeanAngle(Mesh));      // 0 = upright, 90 = forward lean
    float LeanScore = FMath::Clamp(LeanAngle / 45.f, 0.f, 1.f); // Best in range [20�45 deg]

    // Motion alignment score
    float MotionAngle = GetArmToFacingAngle(Mesh, ClavicleBone, HandBone);
    float MotionScore = 1.f - FMath::Clamp(MotionAngle / 90.f, 0.f, 1.f); // Best aligned with facing

    float Composite = (ExtensionScore * 0.4f) + (LeanScore * 0.3f) + (MotionScore * 0.3f);
    return FMath::Clamp(Composite, 0.f, 1.f);
}

bool UOHCombatUtils::IsStrikeArcingTowardTarget(USkeletalMeshComponent* Mesh, FName ClavicleBone, FName HandBone,
                                                AActor* Target, float ThresholdDegrees) {
    if (!Mesh || !Target) {
        return false;
    }
    ...
    FVector TargetVector = (Target->GetActorLocation() - Clavicle).GetSafeNormal();
...
FRotator UOHCombatUtils::GetActorRotationRelativeToCamera(const AActor* TargetActor) {
    if (!TargetActor) {
        return FRotator::ZeroRotator;
    }
...
float UOHCombatUtils::GetActorFacingAngleRelativeToCamera(const AActor* TargetActor) {
    if (!TargetActor) {
        return 0.f;
    }
    ...
    const float Angle = FMath::RadiansToDegrees(FMath::Atan2(FVector::CrossProduct(CameraForward, ActorForward).Z,
                                                             FVector::DotProduct(CameraForward, ActorForward)));

    return Angle;
}

float UOHCombatUtils::GetActorMovementDirectionRelativeToCamera(const AActor* TargetActor) {
    if (!TargetActor) {
        return 0.f;
    }
    ...
    const float Angle = FMath::RadiansToDegrees(FMath::Atan2(FVector::CrossProduct(CameraForward, MoveDirection).Z,
...
void UOHCombatUtils::DebugDrawActorMovementDirection(const AActor* TargetActor, float Duration, FColor Color) {
    ...
    const float MoveAngle = GetActorMovementDirectionRelativeToCamera(TargetActor);
    const FString Label = FString::Printf(TEXT("Move Direction Angle: %.1f°"), MoveAngle);

    const FVector Offset = FVector(0, 0, 120.f);
    DrawDebugString(TargetActor->GetWorld(), TargetActor->GetActorLocation() + Offset, Label, nullptr, Color, Duration,
                    false);
}

bool UOHCombatUtils::IsActorWithinRange(const AActor* Source, const AActor* Target, float MaxDistance, float Radius) {
    if (!Source || !Target) {
        return false;
    }
...
EOHHitDirection UOHCombatUtils::ClassifyHitDirection(const FHitResult& Hit, const AActor* TargetActor,
    ...
    float YawAngle = FMath::RadiansToDegrees(FMath::Atan2(FVector::CrossProduct(TargetForward, HitDirection).Z,
...
void UOHCombatUtils::DebugDrawHitDirection(const FHitResult& Hit, const AActor* TargetActor, bool bUse8Directions,
                                           float Duration, float ArrowSize) {
    if (!TargetActor || !Hit.bBlockingHit) {
        return;
    }
    ...
    const EOHHitDirection Dir = ClassifyHitDirection(Hit, TargetActor, bUse8Directions);
    ...
    DrawDebugDirectionalArrow(TargetActor->GetWorld(), HitLocation, HitLocation + TraceDirection * ArrowSize,
                              ArrowSize * 0.25f, Color, false, Duration, 0, 2.f);

    DrawDebugString(TargetActor->GetWorld(), HitLocation + FVector(0, 0, 50.f), UEnum::GetValueAsString(Dir), nullptr,
                    Color, Duration, false);
}

#pragma region Animation

FStrikeContactMetrics UOHCombatUtils::ComputeStrikeContactMetrics(const FVector& ContactNormal, const FVector& Velocity,
...
FVector UOHCombatUtils::GetBoneVelocitySafe(USkeletalMeshComponent* Mesh, FName BoneName, float DeltaTime) {
...
void UOHCombatUtils::BeginDeferredStrikeSweep(FDeferredStrikeSweepState& State, USkeletalMeshComponent* Mesh,
                                              AActor* Owner, const TArray<FName>& BoneChain, float SampleInterval) {
    State.Mesh = Mesh;
    State.Owner = Owner;
...
void UOHCombatUtils::TickDeferredStrikeSweep(FDeferredStrikeSweepState& State, float DeltaTime) {
    ...
    if (State.TimeSinceLastSample >= State.SampleInterval) {
        ...
        UOHCollisionUtils::SweepBoneChainWithMetrics(State.Mesh, State.BoneChain, State.Owner, SweepContacts, SweepHits,
...
bool UOHCombatUtils::GetActiveMontageBlendWeight(USkeletalMeshComponent* Mesh, float& OutBlendWeight,
    ...
    const int32 SectionIndex = ActiveMontage->Montage->GetSectionIndexFromPosition(ActiveMontage->GetPosition());
    if (SectionIndex != INDEX_NONE) {
        OutCurrentSectionName = ActiveMontage->Montage->GetSectionName(SectionIndex);
    }

    return true;
}

FOHMontagePlaybackState UOHCombatUtils::GetMontagePlaybackState(USkeletalMeshComponent* Mesh) {
    FOHMontagePlaybackState State;

    if (!Mesh || !Mesh->GetAnimInstance()) {
        UE_LOG(LogTemp, Warning, TEXT("[MontagePlaybackState] Invalid mesh or anim instance."));
        return State;
    }

    UAnimInstance* AnimInstance = Mesh->GetAnimInstance();
    const FAnimMontageInstance* MontageInstance = AnimInstance->GetActiveMontageInstance();
    ...
    if (SectionIndex != INDEX_NONE) {
        ...
        State.NormalizedTime =
            (State.SectionLength > 0.f) ? FMath::Clamp((Position - SectionStart) / State.SectionLength, 0.f, 1.f) : 0.f;
    } else {
        UE_LOG(LogTemp, Warning, TEXT("[MontagePlaybackState] Could not resolve section from playback position %.3f"),
               Position);
    }
                ...
                if (UAnimSequence* Seq = Cast<UAnimSequence>(Segment.GetAnimReference())) {
                    ...
                    State.SegmentNormalizedTime =
                        (State.SegmentLength > 0.f)
                            ? FMath::Clamp(State.SegmentPlayTime / State.SegmentLength, 0.f, 1.f)
                            : 0.f;
                    break;
                }
            }
    ...
    if (!State.CurrentAnimSequence) {
        UE_LOG(LogTemp, Warning, TEXT("[MontagePlaybackState] Could not extract UAnimSequence from current segment."));
    }
...
FOHAnimInstancePlaybackState UOHCombatUtils::GetAnimInstancePlaybackState(USkeletalMeshComponent* Mesh) {
    ...
    if (MontageInstance && MontageInstance->Montage) {
        ...
        State.PlayRate = MontageInstance->Montage->RateScale; // Not per-instance, but usable
        State.MontageBlendWeight = MontageInstance->GetWeight();

        const int32 SectionIndex = MontageInstance->Montage->GetSectionIndexFromPosition(State.PlayPosition);
        if (SectionIndex != INDEX_NONE) {
            State.CurrentMontageSection = MontageInstance->Montage->GetSectionName(SectionIndex);
        }
    }

    // If not a montage, we cannot safely access sequence state without engine internals
    return State;
}

FOHSlotBlendState UOHCombatUtils::GetSlotBlendState(USkeletalMeshComponent* Mesh, FName SlotName) {
    ...
    for (FAnimMontageInstance* MontageInstance : MontageInstances) {
        ...
        for (const FSlotAnimationTrack& SlotTrack : MontageInstance->Montage->SlotAnimTracks) {
...
FOHRootMotionState UOHCombatUtils::GetPoseRootMotionState(USkeletalMeshComponent* Mesh, float DeltaTime) {
    ...
    const FTransform DeltaTransform =
        MontageInstance->Montage->ExtractRootMotionFromTrackRange(CurrentTime, FutureTime);
    ...
    State.bHasRootMotion = !Translation.IsNearlyZero() || !Rotation.IsNearlyZero();

    return State;
}

FOHMontageSectionWindow UOHCombatUtils::GetAnimSectionTransitionWindow(USkeletalMeshComponent* Mesh,
                                                                       float CancelWindowStart, float CancelWindowEnd) {
    ...
    const float NormalizedTime = (SectionLength > 0.f) ? FMath::Clamp(LocalTime / SectionLength, 0.f, 1.f) : 0.f;
    ...
    State.bCanCancel = (NormalizedTime >= CancelWindowStart && NormalizedTime <= CancelWindowEnd);
...
void UOHCombatUtils::DrawPredictedRootMotionArc(const UObject* WorldContextObject, const FVector& StartLocation,
    ...
    const FVector EndLocation = StartLocation + RootMotion.RootMotionDelta.GetTranslation();

    // Draw trajectory line
    DrawDebugLine(World, StartLocation, EndLocation, Color, false, Duration, 0, Thickness);

    // Optional: draw direction arrow
    DrawDebugDirectionalArrow(World, StartLocation, EndLocation, 30.f, Color, false, Duration, 0, Thickness);
}

bool UOHCombatUtils::GetMontageSectionTimeRange(UAnimMontage* Montage, FName SectionName, float& OutStartTime,
                                                float& OutEndTime) {
    OutStartTime = 0.f;
    OutEndTime = 0.f;
...
bool UOHCombatUtils::GetMontageSectionNormalizedTime(UAnimMontage* Montage, FName SectionName, float PlaybackTime,
    ...
    OutNormalizedTime = (Length > 0.f) ? FMath::Clamp((PlaybackTime - Start) / Length, 0.f, 1.f) : 0.f;

    return true;
}

bool UOHCombatUtils::IsInMontageSectionWindow(const FOHMontagePlaybackState& PlaybackState, float WindowStart,
                                              float WindowEnd) {
    if (!PlaybackState.bIsValid) {
        UE_LOG(LogTemp, Warning, TEXT("[IsInMontageSectionWindow] PlaybackState is invalid."));
        return false;
    }

    if (WindowStart < 0.f || WindowEnd > 1.f || WindowEnd <= WindowStart) {
        UE_LOG(LogTemp, Warning, TEXT("[IsInMontageSectionWindow] Invalid window range: Start=%.2f End=%.2f"),
    ...
    // UE_LOG(LogTemp, Log, TEXT("[IsInMontageSectionWindow] NormalizedTime=%.3f InWindow=%s"), T, bInWindow ?
    // TEXT("YES") : TEXT("NO"));

    return bInWindow;
}

TArray<FInputWindow> UOHCombatUtils::ExtractInputWindowsFromMontage(UAnimMontage* Montage, FName SectionName) {
    ...
    if (SectionIndex == INDEX_NONE) {
        UE_LOG(LogTemp, Warning, TEXT("Invalid Montage section name: %s"), *SectionName.ToString());
        return OutWindows;
    }
    ...
    for (const FAnimNotifyEvent& NotifyEvent : Montage->Notifies) {
        ...
        if (NotifyStartTime >= SectionStartTime && NotifyStartTime < SectionEndTime) {
            UInputWindowNotifyState* Notify = Cast<UInputWindowNotifyState>(NotifyEvent.NotifyStateClass);
            if (Notify) {
    ...
    Ranges.Sort([](const FWindowTimeRange& A, const FWindowTimeRange& B) { return A.Start < B.Start; });
...
bool UOHCombatUtils::IsOutsideMontageSectionWindow(const FOHMontagePlaybackState& State, float WindowStart,
                                                   float WindowEnd) {
    return !IsInMontageSectionWindow(State, WindowStart, WindowEnd);
}

bool UOHCombatUtils::IsInMontageSegmentWindow(const FOHMontagePlaybackState& State, float SegmentStart,
                                              float SegmentEnd) {
    if (!State.bIsValid || SegmentEnd <= SegmentStart || SegmentStart < 0.f || SegmentEnd > 1.f) {
        return false;
    }
...
float UOHCombatUtils::GetRemainingSectionTime(const FOHMontagePlaybackState& State) {
    if (!State.bIsValid || State.SectionLength <= 0.f) {
        return 0.f;
    }

    const float LocalTime = State.NormalizedTime * State.SectionLength;
    return FMath::Clamp(State.SectionLength - LocalTime, 0.f, State.SectionLength);
}
...
bool UOHCombatUtils::IsStrikeContactValid(FVector Velocity, FVector ContactNormal, float EstimatedForce,
                                          float MinDotThreshold, float MinEstimatedForce) {
    const float Dot = FVector::DotProduct(Velocity.GetSafeNormal(), ContactNormal.GetSafeNormal());
...
float UOHCombatUtils::ComputeContactScore(FVector Velocity, FVector ContactNormal, float EstimatedForce) {
    float Dot = FVector::DotProduct(Velocity.GetSafeNormal(), ContactNormal.GetSafeNormal());
...
void UOHCombatUtils::SortContactsByImpactScore_Raw(TArray<FVector>& Velocities, TArray<FVector>& Normals,
    ...
    if (Count != Normals.Num() || Count != Forces.Num()) {
        UE_LOG(LogTemp, Warning, TEXT("SortContactsByImpactScore_Raw: Mismatched input array lengths"));
        return;
    }
    ...
    OutSortedIndices.Sort([&](int32 A, int32 B) {
        const float ScoreA = ComputeContactScore(Velocities[A], Normals[A], Forces[A]);
        const float ScoreB = ComputeContactScore(Velocities[B], Normals[B], Forces[B]);
        return ScoreA > ScoreB;
    });
}

FVector UOHCombatUtils::ConvertWorldToActorLocal(const AActor* ReferenceActor, const FVector& WorldLocation) {
    return ReferenceActor ? ReferenceActor->GetTransform().InverseTransformPosition(WorldLocation)
                          : FVector::ZeroVector;
}

bool UOHCombatUtils::IsVelocityAlignedWithImpactNormal(const FVector& Velocity, const FVector& Normal,
                                                       float ThresholdDegrees) {
    const float Dot = FVector::DotProduct(Velocity.GetSafeNormal(), Normal.GetSafeNormal());
    const float Angle = FMath::Acos(Dot) * (180.f / PI);
    return Angle >= ThresholdDegrees;
}

void UOHCombatUtils::FilterRedundantHitsByActor(const TArray<AActor*>& HitActors, TArray<int32>& OutUniqueIndices) {
    TSet<AActor*> Seen;
...
void UOHCombatUtils::DrawDebugPath(const UObject* WorldContextObject, const TArray<FVector>& Points, FColor Color,
                                   float Duration, float Thickness, bool bLoop, bool bPersistentLines) {
    if (!WorldContextObject || Points.Num() < 2) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] DrawDebugPath: Invalid input or insufficient points"));
        return;
    }
    ...
    for (int32 i = 1; i < Points.Num(); ++i) {
        DrawDebugLine(World, Points[i - 1], Points[i], Color, bPersistentLines, Duration, 0, Thickness);
    }

    if (bLoop) {
        DrawDebugLine(World, Points.Last(), Points[0], Color, bPersistentLines, Duration, 0, Thickness);
    }
}

void UOHCombatUtils::DrawDebugStrikeCone(const UObject* WorldContextObject, const FVector& Origin,
                                         const FVector& Velocity, float Length, float AngleDegrees, FColor Color,
    ...
    const FVector Edge1 = Forward.RotateAngleAxis(AngleDegrees, Right).GetSafeNormal();
    const FVector Edge2 = Forward.RotateAngleAxis(-AngleDegrees, Right).GetSafeNormal();
    const FVector Edge3 = Forward.RotateAngleAxis(AngleDegrees, Up).GetSafeNormal();
    const FVector Edge4 = Forward.RotateAngleAxis(-AngleDegrees, Up).GetSafeNormal();

    DrawDebugLine(World, Origin, Origin + Edge1 * Length, Color, false, Duration, 0, Thickness);
    DrawDebugLine(World, Origin, Origin + Edge2 * Length, Color, false, Duration, 0, Thickness);
    DrawDebugLine(World, Origin, Origin + Edge3 * Length, Color, false, Duration, 0, Thickness);
    DrawDebugLine(World, Origin, Origin + Edge4 * Length, Color, false, Duration, 0, Thickness);
}

FVector UOHCombatUtils::GetBoneLinearVelocity(const USkeletalMeshComponent* Mesh, FName BoneName, float DeltaTime) {
    if (!Mesh || !Mesh->DoesSocketExist(BoneName) || DeltaTime <= KINDA_SMALL_NUMBER) {
        return FVector::ZeroVector;
    }
...
FVector UOHCombatUtils::ComputeBoneLinearVelocityFromSamples(const FVector& CurrentPos, const FVector& PreviousPos,
                                                             float DeltaTime) {
...
FVector UOHCombatUtils::ComputeBoneVelocityMeshRelative(const USkeletalMeshComponent* Mesh, FName BoneName,
                                                        float DeltaTime) {
    if (!Mesh || !Mesh->DoesSocketExist(BoneName) || DeltaTime <= KINDA_SMALL_NUMBER) {
        return FVector::ZeroVector;
    }
...
FVector UOHCombatUtils::PredictBonePositionFromAnimation(const USkeletalMeshComponent* Mesh, FName BoneName,
                                                         float TimeOffset) {
    ...
    const FTransform CurrentBoneTransform = Mesh->GetSocketTransform(BoneName, RTS_Component);
    ...
    const FVector PredictedWorld =
        Mesh->GetComponentTransform().TransformPosition(RootMotionDelta.TransformPosition(LocalOffset));

    return PredictedWorld;
}

void UOHCombatUtils::DetectContactAlongStrikeArc(const UObject* WorldContextObject, const TArray<FVector>& ArcPoints,
    ...
    for (int32 i = 1; i < ArcPoints.Num(); ++i) {
        ...
        if (SegmentThickness <= 0.f) {
            ...
            World->SweepSingleByChannel(Hit, Start, End, FQuat::Identity, TraceChannel, Sphere);
        }
...
void UOHCombatUtils::SweepCapsuleAlongStrikeArc(const UObject* WorldContextObject, const TArray<FVector>& ArcPoints,
    ...
    for (int32 i = 1; i < ArcPoints.Num(); ++i) {
        ...
        FHitResult Hit;
        World->SweepSingleByChannel(Hit, Start, End, FQuat::Identity, TraceChannel, CapsuleShape);
...
bool UOHCombatUtils::DetectGJKContactFromStrikeArc(const TArray<FVector>& ArcPoints,
    ...
    UOHCollisionUtils::ComputeConvexHullWithIndices(ArcPoints, ArcHull, ArcIndices);
    ...
    bool bGJK = UOHCollisionUtils::GJK_Intersect(ArcHull, TargetConvexPoints, Simplex, SupportA, SupportB,
    ...
    bool bEPA = UOHCollisionUtils::EPA_PenetrationDepth(ArcHull, TargetConvexPoints, Simplex, Normal, Depth,
                                                        MaxIterations, Tolerance);
    ...
    OutContactPoint = UOHCollisionUtils::ComputeAverageContactPoint(SupportA, SupportB);

    return true;
}
static bool SampleStrikeArc_Internal(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh, FName BoneName,
                                     const TArray<float>& SampleTimes, TArray<FVector>& OutPoints) {
    OutPoints.Reset();
    ...
    for (float Time : SampleTimes) {
        ...
        if (UOHCombatUtils::GetBoneTransformAtTime(Sequence, Mesh, BoneName, Time, BoneTransform)) {
...
void UOHCombatUtils::GenerateStrikeArcFromLiveMesh(const USkeletalMeshComponent* Mesh, FName BoneName, int32 NumSamples,
                                                   float SampleInterval, TArray<FVector>& OutArc) {
    OutArc.Reset();

    if (!Mesh || !Mesh->DoesSocketExist(BoneName) || NumSamples < 2 || SampleInterval <= 0.f) {
        return;
    }
    ...
    for (int32 i = 0; i < NumSamples; ++i) {
        // Currently just using live bone position, optionally you can query motion history here
        const FVector Pos = Mesh->GetSocketLocation(BoneName);
        OutArc.Add(Pos);
    }
}

bool UOHCombatUtils::GenerateStrikeArcFromAnimBase(UAnimSequenceBase* AnimBase, USkeletalMeshComponent* Mesh,
                                                   FName BoneName, float StartTime, float EndTime,
                                                   TArray<FVector>& OutPoints, float SampleRate) {
    if (!AnimBase || !Mesh) {
    ...
    UAnimSequence* Sequence = ExtractSequenceFromAnimBaseAtTime(AnimBase, QueryTime);

    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] No valid sequence extracted from AnimBase at time %.2f"), QueryTime);
        return false;
    }

    const float ClampedStart = FMath::Clamp(StartTime, 0.f, Sequence->GetPlayLength());
    const float ClampedEnd = FMath::Clamp(EndTime, ClampedStart, Sequence->GetPlayLength());

    return GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, ClampedStart, ClampedEnd, OutPoints, SampleRate);
}
bool UOHCombatUtils::GenerateStrikeArcFromAnimSequence(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh,
                                                       FName BoneName, float StartTime, float EndTime,
                                                       TArray<FVector>& OutPoints, float SampleRate) {
    if (!Sequence || !Mesh || BoneName.IsNone() || StartTime >= EndTime || SampleRate <= 0.f)
        return false;
    ...
    return SampleStrikeArc_Internal(Sequence, Mesh, BoneName, SampleTimes, OutPoints);
}

bool UOHCombatUtils::GenerateStrikeArcFromAnimSequence_FixedResolution(UAnimSequence* Sequence,
                                                                       USkeletalMeshComponent* Mesh, FName BoneName,
                                                                       int32 NumSamples, float StartTime, float EndTime,
                                                                       TArray<FVector>& OutPoints) {
    if (!Sequence || !Mesh || BoneName.IsNone() || StartTime >= EndTime || NumSamples < 2)
        return false;
...
bool UOHCombatUtils::GenerateStrikeArcFromComposite(UAnimComposite* Composite, USkeletalMeshComponent* Mesh,
                                                    FName BoneName, float StartTime, float EndTime,
                                                    TArray<FVector>& OutPoints, float SampleRate) {
    if (!Composite || !Mesh) {
    ...
    const float QueryTime = (StartTime > 0.f) ? StartTime : 0.01f;
    UAnimSequence* Sequence = ExtractSequenceFromAnimBaseAtTime(Composite, QueryTime);

    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] No valid sequence extracted from Composite at time %.2f"), QueryTime);
        return false;
    }
...
bool UOHCombatUtils::GenerateStrikeArcFromMontage(UAnimMontage* Montage, USkeletalMeshComponent* Mesh, FName BoneName,
    ...
    const float QueryTime = (StartTime > 0.f) ? StartTime : 0.01f;
    UAnimSequence* Sequence = ExtractSequenceFromAnimBaseAtTime(Montage, QueryTime);

    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] No valid sequence extracted from Montage at time %.2f"), QueryTime);
        return false;
    }
...
void UOHCombatUtils::VisualizeBoneMotionArcFromAnim(const UObject* WorldContextObject, UAnimSequenceBase* AnimBase,
                                                    USkeletalMeshComponent* Mesh, FName BoneName, int32 NumSamples,
                                                    FName MontageSectionName, FColor Color, float Duration,
                                                    float Thickness, bool bLoop, bool bDrawVelocity,
                                                    bool bDrawImpactNormals) {
    if (!WorldContextObject || !AnimBase || !Mesh || BoneName.IsNone() || NumSamples < 2) {
        UE_LOG(LogTemp, Warning, TEXT("VisualizeBoneMotionArcFromAnim: Invalid input."));
        return;
    }
    ...
    if (UAnimMontage* Montage = Cast<UAnimMontage>(AnimBase)) {
        if (!MontageSectionName.IsNone()) {
            ...
            if (SectionIndex != INDEX_NONE) {
                ...
                UE_LOG(LogTemp, Warning, TEXT("VisualizeBoneMotionArcFromAnim: Montage section '%s' not found."),
    ...
    if (!UOHCombatUtils::GenerateStrikeArcFromAnimBase(AnimBase, Mesh, BoneName, StartTime, EndTime, ArcPoints,
                                                       NumSamples /
                                                           (EndTime - StartTime))) // Convert NumSamples to SampleRate
    {
        UE_LOG(LogTemp, Warning, TEXT("VisualizeBoneMotionArcFromAnim: Failed to generate arc."));
        return;
    }

    if (ArcPoints.Num() < 2) {
        UE_LOG(LogTemp, Warning, TEXT("VisualizeBoneMotionArcFromAnim: Arc has insufficient points."));
        return;
    }

    // Draw main arc path
    DrawDebugPath(WorldContextObject, ArcPoints, Color, Duration, Thickness, bLoop);
    ...
    if (bDrawVelocity || bDrawImpactNormals) {
        for (int32 i = 1; i < ArcPoints.Num(); ++i) {
            ...
            if (bDrawVelocity) {
                DrawDebugLine(WorldContextObject->GetWorld(), Start, Start + Velocity * 20.f, FColor::Yellow, false,
                              Duration, 0, 1.5f);
            }
            ...
            if (bDrawImpactNormals && i < ArcPoints.Num() - 1) {
                ...
                const FVector Normal = FVector::UpVector ^ Tangent; // crude right-angle estimate

                DrawDebugLine(WorldContextObject->GetWorld(), End, End + Normal * 20.f, FColor::Cyan, false, Duration,
                              0, 1.0f);
            }
        }
    }
...
void UOHCombatUtils::VisualizePredictedArc(const UObject* WorldContextObject, USkeletalMeshComponent* Mesh,
                                           FName BoneName, float PredictionWindow, int32 NumSamples, FColor Color,
                                           float Duration, float Thickness, bool bDrawVelocity,
                                           bool bDrawImpactNormals) {
    if (!WorldContextObject || !Mesh || BoneName.IsNone() || NumSamples < 2 || PredictionWindow <= 0.f) {
        UE_LOG(LogTemp, Warning, TEXT("VisualizePredictedArcFromNow: Invalid input."));
        return;
    }
    ...
    if (!UOHCombatUtils::ExtractActiveSequenceAndTimeFromMesh(Mesh, Sequence, LocalTime) || !Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("VisualizePredictedArcFromNow: Failed to extract active sequence."));
        return;
    }

    // Compute time bounds
    const float StartTime = LocalTime;
    const float EndTime = FMath::Min(StartTime + PredictionWindow, Sequence->GetPlayLength());

    TArray<FVector> ArcPoints;
    if (!GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, StartTime, EndTime, ArcPoints,
                                           NumSamples / PredictionWindow)) {
        UE_LOG(LogTemp, Warning, TEXT("VisualizePredictedArcFromNow: Failed to generate arc."));
        return;
    }
    ...
    for (int32 i = 1; i < ArcPoints.Num(); ++i) {
        ...
        if (bDrawImpactNormals && i < ArcPoints.Num() - 1) {
            ...
            DrawDebugLine(WorldContextObject->GetWorld(), End, End + Normal * 20.f, FColor::Cyan, false, Duration, 0,
                          1.0f);
        }
    }
}

FCombatStrikePredictionResult UOHCombatUtils::PredictStrikeCollision(const UObject* WorldContextObject,
                                                                     ACharacter* Attacker,
                                                                     UOHPhysicsManager* PhysicsManager, FName BoneName,
                                                                     float PredictionTime, int Steps,
                                                                     bool bEnableDebug) {
    ...
    for (TActorIterator<ACharacter> It(World); It; ++It) {
        ...
        float DistSq = FVector::DistSquared(Attacker->GetActorLocation(), Candidate->GetActorLocation());
    ...
    if (UOHPhysicsManager* TargetPM = ClosestTarget->FindComponentByClass<UOHPhysicsManager>()) {
    ...
    if (bEnableDebug) {
        DrawDebugLine(World, BestPredictedPos, BestTargetBoneLoc, FColor::Yellow, false, 2.f, 0, 1.5f);
        DrawDebugSphere(World, BestTargetBoneLoc, 5.f, 8, FColor::Blue, false, 2.f);
        DrawDebugString(World, BestPredictedPos + FVector(0, 0, 12),
                        FString::Printf(TEXT("Target Bone: %s\nDist: %.1f\nTTC: %.2fs"), *BestTargetBone.ToString(),
                                        BestDistance, BestTime),
...
FCombatStrikePredictionResult UOHCombatUtils::AnalyzeStrikeTrajectoryAndTargeting(
    const UObject* WorldContextObject, ACharacter* Attacker, UOHPhysicsManager* PhysicsManager, FName StrikeBone,
    float PredictionTime, int Steps, bool bEnableDebug, FName DebugTag) {
    FCombatStrikePredictionResult Result;
    if (!WorldContextObject || !Attacker || !PhysicsManager || StrikeBone.IsNone())
        return Result;
    ...
    TArray<FName> BoneChain = UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, RootBone, StrikeBone);
    if (BoneChain.Num() == 0)
    ...
    if (ValidBones.Num() == 0) {
        UE_LOG(LogTemp, Warning, TEXT("Strike prediction failed: No valid OHBoneData for attacker."));
        return Result;
    }
    ...
    for (TActorIterator<ACharacter> It(World); It; ++It) {
        ...
        const float DistSq = FVector::DistSquared(Attacker->GetActorLocation(), Candidate->GetActorLocation());
        if (DistSq < ClosestDistSq) {
            ClosestTarget = Candidate;
            ClosestDistSq = DistSq;
        }
    ...
    for (int32 Step = 1; Step <= Steps; ++Step) {
        ...
        const FVector Predicted = AvgStart + AvgVelocity * T + 0.5f * AvgAccel * T * T;
        ...
        for (const FName& Bone : TargetBones) {
            ...
            const FVector BoneLoc = (TargetBoneData && TargetBoneData->IsValid())
    ...
    const float AngleDeg = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(Dot, -1.f, 1.f)));
    const float DirScore = FMath::GetMappedRangeValueClamped(FVector2D(-1, 1), FVector2D(-1, 1), Dot);
    ...
    if (bEnableDebug) {
        DrawDebugLine(World, BestPredictedPos, BestTargetBoneLoc, FColor::Yellow, false, 2.f, 0, 1.5f);
        DrawDebugSphere(World, BestTargetBoneLoc, 6.f, 12, FColor::Blue, false, 2.f);
        DrawDebugDirectionalArrow(World, AvgStart, BestTargetBoneLoc, 150.f, FColor::Red, false, 2.f, 0, 3.0f);
        DrawDebugString(World, BestPredictedPos + FVector(0, 0, 32),
                        FString::Printf(TEXT("Bone: %s\nAngle: %.1f°\nScore: %.2f\nRegion: %s"),
                                        *BestTargetBone.ToString(), AngleDeg, DirScore, *Region.ToString()),
...
FCombatStrikePredictionResult UOHCombatUtils::AnalyzeAdvancedStrikeTrajectory3D(
    const UObject* WorldContextObject, ACharacter* Attacker, UOHPhysicsManager* PhysicsManager, FName StrikeBone,
    float PredictionTime, int Steps, bool bEnableDebug, FName DebugTag, FName OptionalChainStart, int32 MaxChainDepth,
    bool bIsFullBodyAnimation, float HistoricalBlendAlpha) {
    FCombatStrikePredictionResult Result;
    ...
    TArray<FName> BoneChain =
        OptionalChainStart.IsNone()
            ? UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, NAME_None, StrikeBone)
            : UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, OptionalChainStart, StrikeBone);
    ...
    TArray<const FOHBoneData*> ValidBones;
    FVector SumPos, SumVel, SumHistVel, SumAccel, SumForward, SumAngVel, SumAngAccel = FVector::ZeroVector;

    for (const FName& Bone : BoneChain) {
        if (!bIsFullBodyAnimation && (Bone == "pelvis" || Bone.ToString().Contains(TEXT("root"))))
            continue;
        ...
        if (bEnableDebug) {
            DrawDebugSphere(World, BoneData->GetCurrentPosition(), 3.f, 8, FColor::Magenta, false, 2.f);
            DrawDebugString(World, BoneData->GetCurrentPosition() + FVector(0, 0, 10), Bone.ToString(), nullptr,
                            FColor::White, 2.f);
        }
    }
    if (ValidBones.Num() == 0)
    ...
    // --- 3. Pose Deformation ---
    float FullExtension = FVector::Dist(Mesh->GetBoneLocation(BoneChain[0]), Mesh->GetBoneLocation(StrikeBone));
    float MaxChainLength = 0.f;
    for (int32 i = 1; i < BoneChain.Num(); ++i) {
        MaxChainLength += FVector::Dist(Mesh->GetBoneLocation(BoneChain[i - 1]), Mesh->GetBoneLocation(BoneChain[i]));
    }
    ...
    const FVector TangentDir = FVector::CrossProduct(AngularVel, Forward).GetSafeNormal();

    for (int32 Step = 1; Step <= Steps; ++Step) {
        const float T = Step * StepSize;
        const FVector LinearPart = Start + LinearVel * T + 0.5f * LinearAccel * T * T;
        const float AngularSpeed = AngularVel.Size() + AngularAccel.Size() * T;
        const FVector ArcOffset = TangentDir * (ArcRadius * FMath::Sin(AngularSpeed * T));
        ...
        if (bEnableDebug) {
            FLinearColor LinearColor =
                FLinearColor::LerpUsingHSV(FLinearColor::Green, FLinearColor::Red, Step / static_cast<float>(Steps));
            FColor StepColor = LinearColor.ToFColor(true); // Convert to gamma-corrected FColor

            DrawDebugSphere(World, Predicted, 4.f, 10, StepColor, false, 2.f);
            DrawDebugLine(World, LinearPart, Predicted, FColor::Cyan, false, 2.f);
            DrawDebugString(World, Predicted + FVector(0, 0, 12), FString::Printf(TEXT("T: %.2f"), T), nullptr,
                            StepColor, 2.f);
        }
    }
    ...
    // --- 8. Debug ---
    if (bEnableDebug) {
        DrawDebugLine(World, BestPos, BestTargetLoc, FColor::Yellow, false, 2.f, 0, 2.f);
        DrawDebugSphere(World, BestTargetLoc, 7.f, 14, FColor::Red, false, 2.f);
        DrawDebugString(World, BestPos + FVector(0, 0, 30),
                        FString::Printf(TEXT("Target: %s\nRegion: %s\nScore: %.2f\nAngle: %.1f°"),
                                        *BestTargetBone.ToString(), *Region.ToString(), Result.DirectionalityScore,
                                        Result.AngularDeviationDegrees),
...
FCombatStrikePredictionResult UOHCombatUtils::AnalyzeAdvancedStrikeTrajectory2D(
    ...
    for (const FName& Bone : BoneChain) {
        ...
        if (bEnableDebug) {
            DrawDebugSphere(World, Data->GetCurrentPosition(), 3.f, 8, FColor::Purple, false, 2.f);
            DrawDebugString(World, Data->GetCurrentPosition() + FVector(0, 0, 10), Bone.ToString(), nullptr,
                            FColor::White, 2.f);
        }
    }
    if (ValidBones.Num() == 0)
    ...
    for (int32 i = 1; i < BoneChain.Num(); ++i)
        MaxChainLen += FVector::Dist(Mesh->GetBoneLocation(BoneChain[i - 1]), Mesh->GetBoneLocation(BoneChain[i]));
    ...
    for (TActorIterator<ACharacter> It(World); It; ++It) {
        ...
        float DistSq = FVector::DistSquared(Attacker->GetActorLocation(), Other->GetActorLocation());
        if (DistSq < MinDistSq) {
            MinDistSq = DistSq;
            ClosestTarget = Other;
        }
    ...
    const FVector Tangent = FVector::CrossProduct(AngularVel, Forward).GetSafeNormal();

    for (int32 Step = 1; Step <= Steps; ++Step) {
        const float T = Step * StepSize;
        FVector StepPos = Start + LinearVel * T + 0.5f * LinearAccel * T * T;
        StepPos += Tangent * (ArcRadius * FMath::Sin((AngularVel.Size() + AngularAccel.Size() * T) * T));

        // Project X and Z, keep actual Z height
        StepPos = FVector(StepPos.X, Start.Y, StepPos.Z); // maintains height, fixes “on the floor” issue
        ...
        if (bEnableDebug) {
            ...
            DrawDebugString(World, StepPos + FVector(0, 0, 10), FString::Printf(TEXT("T: %.2f"), T), nullptr,
                            FColor::White, 2.f);
        }
    }
    if (BestTargetBone.IsNone())
    ...
    // --- 8. Debug Overlay ---
    if (bEnableDebug) {
        DrawDebugLine(World, BestPredicted, BestTargetLoc, FColor::Yellow, false, 2.f, 0, 2.f);
        DrawDebugSphere(World, BestTargetLoc, 6.f, 14, FColor::Red, false, 2.f);
        DrawDebugString(World, BestPredicted + FVector(0, 0, 30),
                        FString::Printf(TEXT("Target: %s\nRegion: %s\nScore: %.2f\nAngle: %.1f°"),
                                        *BestTargetBone.ToString(), *Region.ToString(), Result.DirectionalityScore,
                                        Result.AngularDeviationDegrees),
                        nullptr, FColor::White, 2.f);
...
FOHBoneMotionSample UOHCombatUtils::SampleBoneMotionFromAnimBase(UAnimSequenceBase* AnimBase,
                                                                 USkeletalMeshComponent* Mesh, FName BoneName,
                                                                 int32 NumSamples, float StartTime, float EndTime,
                                                                 FName MontageSectionName, bool bDrawDebug,
                                                                 FColor DebugColor, float DebugDuration,
                                                                 float DebugThickness) {
    FOHBoneMotionSample Result =
        SampleBoneMotionInternal(AnimBase, Mesh, BoneName, NumSamples, StartTime, EndTime, MontageSectionName);

    if (bDrawDebug && Mesh && Mesh->GetWorld()) {
        DrawDebugPath(Mesh, Result.Points, DebugColor, DebugDuration, DebugThickness);
    }

    return Result;
}

FOHBoneMotionSample UOHCombatUtils::SampleBoneMotionFromAnimBasePure(UAnimSequenceBase* AnimBase,
                                                                     USkeletalMeshComponent* Mesh, FName BoneName,
                                                                     int32 NumSamples, float StartTime, float EndTime,
                                                                     FName MontageSectionName) {
    return SampleBoneMotionInternal(AnimBase, Mesh, BoneName, NumSamples, StartTime, EndTime, MontageSectionName);
}

FOHBoneMotionSample UOHCombatUtils::SampleBoneMotionInternal(UAnimSequenceBase* AnimBase, USkeletalMeshComponent* Mesh,
    ...
    if (!SkelMesh || !SkelMesh->GetSkeleton()) {
        UE_LOG(LogTemp, Warning, TEXT("[SampleBoneMotion] Mesh or skeleton not valid at runtime."));
        return Result;
    }
    ...
    if (FMath::IsNearlyZero(DeltaT)) {
        UE_LOG(LogTemp, Warning, TEXT("[SampleBoneMotion] Insufficient delta time (%.4f)"), DeltaT);
        return Result;
    }
    ...
    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[SampleBoneMotion] Could not resolve sequence at time %.2f in %s"), T0,
               *GetNameSafe(AnimBase));
        return Result;
    }
    ...
    for (int32 i = 0; i < NumSamples; ++i) {
        ...
        FTransform BoneTransform;
        if (UOHCombatUtils::GetBoneTransformAtTime(Sequence, Mesh, BoneName, static_cast<float>(Time), BoneTransform)) {
            const FVector Pos = BoneTransform.GetLocation();
            if (!Pos.ContainsNaN()) {
    ...
    if (Result.NumPoints > 1) {
        UOHCombatUtils::SampleMotionDerivatives(Result.Points, SampleInterval, Result.Velocities, Result.Accelerations);
    }

    return Result;
}

void UOHCombatUtils::ScheduleDeferredBoneArcSample(UObject* WorldContext, UAnimSequenceBase* AnimBase,
                                                   USkeletalMeshComponent* Mesh, FName BoneName, int32 NumSamples,
    ...
    TimerCallback.BindLambda([=]() {
        ...
        UOHCombatUtils::VisualizeBoneMotionArcFromAnim(WorldContext, WeakAnim.Get(), WeakMesh.Get(), BoneName,
                                                       NumSamples, MontageSectionName, DebugColor, Duration, Thickness);
    });
...
bool UOHCombatUtils::GetBoneTransformAtTime(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh, FName BoneName,
                                            double Time, FTransform& OutTransform) {
    OutTransform = FTransform::Identity;

    if (!Sequence || !Mesh || BoneName.IsNone()) {
        UE_LOG(LogTemp, Error, TEXT("[OH] Invalid input to GetBoneTransformAtTime"));
        return false;
    }
    ...
    if (BoneIndex == INDEX_NONE) {
        UE_LOG(LogTemp, Error, TEXT("[OH] Bone '%s' not found in skeleton"), *BoneName.ToString());
        return false;
    }
    ...
    const FCompactPoseBoneIndex CompactIndex = BoneContainer.GetCompactPoseIndexFromSkeletonIndex(BoneIndex);
    if (!CompactIndex.IsValid()) {
        UE_LOG(LogTemp, Error, TEXT("[OH] Invalid compact pose index for bone '%s'"), *BoneName.ToString());
        return false;
    }
...
bool UOHCombatUtils::GetBoneTransformWorldAtTime(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh, FName BoneName,
                                                 double Time, FTransform& OutTransform) {
...
bool UOHCombatUtils::GetBoneTransformFromAnimBase(UAnimSequenceBase* AnimBase, USkeletalMeshComponent* Mesh,
                                                  FName BoneName, double Time, FTransform& OutTransform,
                                                  bool bWorldSpace) {
    UAnimSequence* Sequence = ExtractSequenceFromAnimBaseAtTime(AnimBase, Time);
    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Failed to extract sequence from anim base at %.2f"), Time);
        OutTransform = FTransform::Identity;
        return false;
    }

    if (bWorldSpace) {
        return GetBoneTransformWorldAtTime(Sequence, Mesh, BoneName, Time, OutTransform);
    } else {
        return GetBoneTransformAtTime(Sequence, Mesh, BoneName, Time, OutTransform);
    }
}

bool UOHCombatUtils::GetBoneRotationAtTime(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh, FName BoneName,
                                           double Time, FRotator& OutRotation, bool bWorldSpace) {
    FTransform BoneTransform;
    const bool bSuccess = bWorldSpace ? GetBoneTransformWorldAtTime(Sequence, Mesh, BoneName, Time, BoneTransform)
                                      : GetBoneTransformAtTime(Sequence, Mesh, BoneName, Time, BoneTransform);
...
bool UOHCombatUtils::GetBoneVelocityAtTime(UAnimSequence* Sequence, USkeletalMeshComponent* Mesh, FName BoneName,
                                           double Time, FVector& OutVelocity, float SampleWindow) {
    OutVelocity = FVector::ZeroVector;

    if (!Sequence || !Mesh || BoneName.IsNone()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Invalid input to GetBoneVelocityAtTime"));
        return false;
    }
...
FTransform UOHCombatUtils::GetBoneComponentSpaceTransformAtTime(UAnimSequence* AnimSequence,
                                                                const USkeletalMeshComponent* Mesh, FName BoneName,
                                                                double Time) {
    if (!AnimSequence || !Mesh || !Mesh->GetSkeletalMeshAsset()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] [GetBoneComponentSpaceTransformAtTime] Invalid input."));
    ...
    const int32 BoneIndex = RefSkeleton.FindBoneIndex(BoneName);
    if (BoneIndex == INDEX_NONE) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] [GetBoneComponentSpaceTransformAtTime] Bone '%s' not found in mesh."),
               *BoneName.ToString());
        return FTransform::Identity;
    }
    ...
    if (!BoneAsset) {
        UE_LOG(LogTemp, Error, TEXT("[OH] Failed to resolve valid asset for bone container."));
        return FTransform::Identity;
    }
    ...
    const FCompactPoseBoneIndex CompactIndex = BoneContainer.MakeCompactPoseIndex(MeshPoseBoneIndex);
    if (!Pose.IsValidIndex(CompactIndex)) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Invalid CompactPose index for '%s'."), *BoneName.ToString());
        return FTransform::Identity;
    }
    ...
    while (ParentIndex != INDEX_NONE) {
        ...
        const FCompactPoseBoneIndex ParentCompactIndex = BoneContainer.MakeCompactPoseIndex(ParentMeshIndex);
...
void UOHCombatUtils::ComputeMotionDerivatives(const TArray<FVector>& Points, float SampleDelta,
    ...
    for (int32 i = 0; i < Count; ++i) {
        if (i == 0) {
            ...
            OutAccelerations[i] = (Points[i + 1] - 2.f * Points[i] + Points[i - 1]) / (SampleDelta * SampleDelta);
        }
    }
}

bool UOHCombatUtils::ResolveAnimSequenceAtTimeRecursive(UAnimSequenceBase* AnimBase, float Time,
                                                        UAnimSequence*& OutSequence, float& OutLocalTime) {
    if (!AnimBase) {
        UE_LOG(LogTemp, Warning, TEXT("[ResolveAnimSequenceAtTimeRecursive] Null AnimBase."));
        return false;
    }
        ...
        for (const FAnimSegment& Segment : Segments) {
            ...
            if (Time >= Start && Time <= End) {
                if (UAnimSequenceBase* Nested = Segment.GetAnimReference()) {
                    return ResolveAnimSequenceAtTimeRecursive(Nested, Time - Start, OutSequence, OutLocalTime);
                }
            }
        }
        return false;
    ...
    UE_LOG(LogTemp, Warning, TEXT("[ResolveAnimSequenceAtTimeRecursive] Could not resolve sequence at time %.3f in %s"),
           Time, *GetNameSafe(AnimBase));
    return false;
}

UAnimSequence* UOHCombatUtils::ExtractSequenceFromAnimBaseAtTime(UAnimSequenceBase* AnimBase, float Time) {
    if (!AnimBase)
        return nullptr;
    ...
    const bool bResolved = ResolveAnimSequenceAtTimeRecursive(AnimBase, Time, OutSequence, OutLocalTime);
    if (!bResolved || !OutSequence) {
        UE_LOG(LogTemp, Warning, TEXT("ExtractSequenceFromAnimBaseAtTime: No sequence found at time %.3f in %s"), Time,
               *GetNameSafe(AnimBase));
    }
...
UAnimSequence* UOHCombatUtils::ExtractSequenceFromAnimBase(UAnimSequenceBase* AnimBase) {
    if (!AnimBase)
        return nullptr;
...
UAnimSequence* UOHCombatUtils::ExtractActiveSequenceFromMesh(USkeletalMeshComponent* Mesh) {
    if (!Mesh || !Mesh->GetAnimInstance()) {
        UE_LOG(LogTemp, Warning, TEXT("ExtractActiveSequenceFromMesh: Invalid mesh or anim instance"));
        return nullptr;
    }
    ...
    if (!Montage) {
        UE_LOG(LogTemp, Warning, TEXT("ExtractActiveSequenceFromMesh: No active montage"));
        return nullptr;
    }

    const float PlaybackTime = AnimInstance->Montage_GetPosition(Montage);
    UAnimSequence* Sequence = ExtractSequenceFromAnimBaseAtTime(Montage, PlaybackTime);

    if (!Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("ExtractActiveSequenceFromMesh: Could not extract sequence at time %.2f from %s"),
               PlaybackTime, *Montage->GetName());
    } else {
        UE_LOG(LogTemp, Log, TEXT("ExtractActiveSequenceFromMesh: Resolved '%s' at time %.2f"), *Sequence->GetName(),
               PlaybackTime);
    }
...
UAnimSequence* UOHCombatUtils::ExtractActiveSequenceFromMeshPure(USkeletalMeshComponent* Mesh) {
    if (!Mesh || !Mesh->GetAnimInstance()) {
        return nullptr;
    }
...
bool UOHCombatUtils::ExtractActiveSequenceAndTimeFromMesh(USkeletalMeshComponent* Mesh, UAnimSequence*& OutSequence,
                                                          float& OutLocalTime) {
    OutSequence = nullptr;
    OutLocalTime = 0.f;
    ...
    const float PlaybackTime = AnimInstance->Montage_GetPosition(Montage);
    return ResolveAnimSequenceAtTimeRecursive(Montage, PlaybackTime, OutSequence, OutLocalTime);
}

void UOHCombatUtils::StartTimedBoneArcSampling(UObject* WorldContextObject, USkeletalMeshComponent* Mesh,
                                               FName BoneName, float Duration, int32 NumSamples,
    ...
    World->GetTimerManager().SetTimer(
        ...
        [Sampler, OnComplete, World]() {
            if (!Sampler->Mesh.IsValid() || Sampler->ElapsedTime > Sampler->Duration) {
            ...
            if (Sampler->Mesh->DoesSocketExist(Sampler->BoneName)) {
                const FVector Pos = Sampler->Mesh->GetSocketLocation(Sampler->BoneName);
...
UObject* UOHCombatUtils::GetValidBoneAssetForContainer(const USkeletalMeshComponent* Mesh) {
    if (!Mesh)
        return nullptr;
...
bool UOHCombatUtils::GetBoneWorldSpaceVelocityAtTime(UAnimSequence* Sequence, const USkeletalMeshComponent* Mesh,
                                                     FName BoneName, float Time, FVector& OutVelocity,
                                                     float SampleWindow) {
    OutVelocity = FVector::ZeroVector;

    if (!Sequence || !Mesh || BoneName.IsNone() || SampleWindow <= 0.f) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Invalid input to GetBoneWorldSpaceVelocityAtTime"));
        return false;
    }
    ...
    const FTransform X0 = GetBoneComponentSpaceTransformAtTime(Sequence, Mesh, BoneName, T0);
    const FTransform X1 = GetBoneComponentSpaceTransformAtTime(Sequence, Mesh, BoneName, T1);

    const FVector Pos0 = X0.GetLocation();
    const FVector Pos1 = X1.GetLocation();

    if (Pos0.ContainsNaN() || Pos1.ContainsNaN()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Bone position NaN during velocity sampling"));
        return false;
    }
...
void UOHCombatUtils::SampleMotionDerivatives(const TArray<FVector>& Positions, float SampleInterval,
                                             TArray<FVector>& OutVelocities, TArray<FVector>& OutAccelerations) {
    ...
    for (int32 i = 1; i < OutVelocities.Num(); ++i) {
        const FVector A = (OutVelocities[i] - OutVelocities[i - 1]) / SampleInterval;
...
void UOHCombatUtils::VisualizeMotionDerivatives(const UObject* WorldContextObject, const TArray<FVector>& Points,
                                                const TArray<FVector>& Velocities, const TArray<FVector>& Accelerations,
                                                float Duration, float Scale, float Thickness) {
    if (!WorldContextObject || Points.Num() < 2 || Velocities.Num() != Points.Num() ||
        Accelerations.Num() != Points.Num()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] VisualizeMotionDerivatives: Mismatched or invalid input arrays."));
        return;
    }
    ...
    for (int32 i = 0; i < Points.Num(); ++i) {
        ...
        DrawDebugLine(World, Origin, Origin + V, FColor::Yellow, false, Duration, 0, Thickness);

        // Draw acceleration vector (cyan)
        const FVector A = Accelerations[i] * Scale;
        DrawDebugLine(World, Origin, Origin + A, FColor::Cyan, false, Duration, 0, Thickness * 0.75f);
    }
}

FOHBoneMotionSample UOHCombatUtils::GenerateBoneMotionSample(UAnimSequenceBase* AnimBase, USkeletalMeshComponent* Mesh,
                                                             FName BoneName, int32 NumSamples, FName MontageSectionName,
                                                             float StartTime, float EndTime) {
    FOHBoneMotionSample Sample;
    Sample.BoneName = BoneName;

    if (!AnimBase) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: Null AnimBase"));
        return Sample;
    }

    if (!Mesh || !Mesh->GetSkeletalMeshAsset()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: Invalid SkeletalMeshComponent"));
        return Sample;
    }

    if (BoneName.IsNone()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: BoneName is None"));
        return Sample;
    }

    if (NumSamples < 2) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: NumSamples too low (%d)"), NumSamples);
        return Sample;
    }

    const bool bCallerProvidedRange = !FMath::IsNearlyZero(StartTime) || !FMath::IsNearlyZero(EndTime);
    const float MaxDuration = AnimBase->GetPlayLength();

    if (bCallerProvidedRange) {
        if (StartTime < 0.f || EndTime < 0.f || StartTime >= EndTime || EndTime > MaxDuration) {
            UE_LOG(LogTemp, Warning,
                   TEXT("[OH] GenerateBoneMotionSample: Invalid time range provided (%.2f → %.2f), Max: %.2f"),
                   StartTime, EndTime, MaxDuration);
            return Sample;
        }
    }

    Sample = SampleBoneMotionInternal(AnimBase, Mesh, BoneName, NumSamples, StartTime, EndTime, MontageSectionName);

    if (Sample.Points.Num() < 2) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: Insufficient motion points returned"));
    }

    return Sample;
}

void UOHCombatUtils::TestVisualizeBoneMotionArcFromMesh(USkeletalMeshComponent* Mesh, FName BoneName,
                                                        float PredictDuration, int32 NumSamples, FColor DebugColor,
                                                        float DebugLineDuration, float LineThickness, bool bLoop) {
    if (!Mesh || BoneName.IsNone()) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] TestVisualizeBoneMotionArcFromMesh: Invalid input."));
        return;
    }
    ...
    if (!UOHCombatUtils::ExtractActiveSequenceAndTimeFromMesh(Mesh, Sequence, LocalTime) || !Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] Failed to extract active sequence from mesh."));
        return;
    }
    ...
    const bool bSuccess = UOHCombatUtils::GenerateStrikeArcFromAnimSequence(
        ...
        NumSamples > 2 ? static_cast<float>(NumSamples) / (T1 - T0) : 60.f); // auto sample rate

    if (!bSuccess || ArcPoints.Num() < 2) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] TestVisualize: Failed to generate arc for bone %s."), *BoneName.ToString());
        return;
    }

    UE_LOG(LogTemp, Log, TEXT("[OH] Drawing test arc (%d pts) for bone %s from %.2f → %.2f"), ArcPoints.Num(),
           *BoneName.ToString(), T0, T1);

    UOHCombatUtils::DrawDebugPath(Mesh, ArcPoints, DebugColor, DebugLineDuration, LineThickness, bLoop,
                                  true /* persistent */);
}

void UOHCombatUtils::TestDrawArcFromMesh(USkeletalMeshComponent* Mesh, FName BoneName, float Duration, FColor Color) {
    if (!Mesh || BoneName.IsNone())
        return;
    ...
    if (!ExtractActiveSequenceAndTimeFromMesh(Mesh, Sequence, LocalTime) || !Sequence) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] TestDrawArcFromMesh: Failed to resolve sequence/time."));
        return;
    }
    ...
    TArray<FVector> ArcPoints;
    const bool bSuccess = GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, StartTime, EndTime, ArcPoints);

    if (!bSuccess || ArcPoints.Num() < 2) {
        UE_LOG(LogTemp, Warning, TEXT("[OH] TestDrawArcFromMesh: Arc gen failed or insufficient points."));
        return;
    }
...
void UOHCombatUtils::QuickDrawBoneArcFromMesh(USkeletalMeshComponent* Mesh, FName BoneName, float PredictDuration,
    ...
    if (!ResolveAnimSequenceAtTimeRecursive(Montage, MontageTime, Sequence, LocalTime)) {
        UE_LOG(LogTemp, Error, TEXT("[OH] Could not resolve sequence from montage at %.2f"), MontageTime);
        return;
    }

    const float T0 = FMath::Clamp(LocalTime, 0.f, Sequence->GetPlayLength());
    const float T1 = FMath::Clamp(T0 + PredictDuration, T0 + 0.01f, Sequence->GetPlayLength());

    TArray<FVector> ArcPoints;
    bool bSuccess = GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, T0, T1, ArcPoints);

    if (!bSuccess || ArcPoints.Num() < 2) {
        UE_LOG(LogTemp, Error, TEXT("[OH] ArcPoints failed: %d pts"), ArcPoints.Num());
        for (int32 i = 0; i < ArcPoints.Num(); ++i)
            UE_LOG(LogTemp, Warning, TEXT("Point[%d] = %s"), i, *ArcPoints[i].ToString());
        return;
    }

    UE_LOG(LogTemp, Log, TEXT("[OH] Drawing arc from %.2f to %.2f with %d points"), T0, T1, ArcPoints.Num());
...
void UOHCombatUtils::ScheduleDeferredArcDraw(USkeletalMeshComponent* Mesh, FName BoneName, float PredictDuration,
                                             FColor DebugColor, float Duration, float Thickness) {
    if (!Mesh || BoneName.IsNone()) {
        UE_LOG(LogTemp, Error, TEXT("[OH] ScheduleDeferredArcDraw: Mesh or BoneName invalid"));
        return;
    }

    UWorld* World = Mesh->GetWorld();
    if (!World) {
        UE_LOG(LogTemp, Error, TEXT("[OH] ScheduleDeferredArcDraw: No valid world"));
        return;
    }
    ...
    World->GetTimerManager().SetTimer(
        ...
        [=]() {
            UAnimInstance* AnimInst = Mesh->GetAnimInstance();
            if (!AnimInst) {
                UE_LOG(LogTemp, Error, TEXT("[OH] No anim instance found for deferred arc"));
                return;
            }

            UAnimMontage* Montage = AnimInst->GetCurrentActiveMontage();
            if (!Montage) {
                UE_LOG(LogTemp, Error, TEXT("[OH] No active montage during deferred arc"));
                return;
            }
            ...
            if (!ResolveAnimSequenceAtTimeRecursive(Montage, MontageTime, Sequence, LocalTime) || !Sequence) {
                UE_LOG(LogTemp, Error, TEXT("[OH] Could not resolve sequence from montage"));
                return;
            }
            ...
            TArray<FVector> ArcPoints;
            if (!GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, T0, T1, ArcPoints)) {
                UE_LOG(LogTemp, Warning, TEXT("[OH] Arc generation failed"));
                return;
            }

            if (ArcPoints.Num() >= 2) {
                DrawDebugPath(Mesh, ArcPoints, DebugColor, Duration, Thickness, false, true);
                UE_LOG(LogTemp, Log, TEXT("[OH] Drew arc (%d points) for bone '%s'"), ArcPoints.Num(),
                       *BoneName.ToString());
            } else if (ArcPoints.Num() == 1) {
                DrawDebugSphere(World, ArcPoints[0], 10.f, 12, FColor::Red, true, Duration);
                UE_LOG(LogTemp, Warning, TEXT("[OH] Only one point in arc — drawing fallback sphere"));
            } else {
                UE_LOG(LogTemp, Error, TEXT("[OH] Arc had no points — fallback draw"));
                DrawDebugBox(World, FVector(0, 0, 100), FVector(10, 10, 10), FColor::Magenta, true, Duration);
            }
        },
        0.01f, false);
}

void UOHCombatUtils::GetBoneChainAngularMotion(UOHPhysicsManager* PhysicsManager, const TArray<FName>& BoneChain,
...
FVector UOHCombatUtils::GetBlendedChainVelocity(UOHPhysicsManager* PhysicsManager, const USkeletalMeshComponent* Mesh,
    ...
    TArray<FName> BoneChain = StartBone.IsNone()
                                  ? UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, NAME_None, EndBone)
                                  : UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, StartBone, EndBone);
...
FVector UOHCombatUtils::GetBlendedChainAcceleration(UOHPhysicsManager* PhysicsManager,
                                                    const USkeletalMeshComponent* Mesh, const FName StartBone,
                                                    const FName EndBone, float HistoryWeight, int NumSamples) {
    if (!PhysicsManager || !Mesh || EndBone.IsNone()) {
        return FVector::ZeroVector;
    }
    ...
    for (const FName& Bone : BoneChain) {
        ...
        Accumulated += Data->GetBlendedLinearAcceleration(HistoryWeight, NumSamples);
        ++ValidCount;
    }
...
void UOHCombatUtils::GetBlendedBoneChainMotion(UOHPhysicsManager* PhysicsManager, const TArray<FName>& BoneChain,
    ...
    for (const FName& BoneName : BoneChain) {
        ...
        OutVelocity += BoneData->GetBlendedLinearVelocity(HistoryWeight, NumSamples);
        OutAcceleration += BoneData->GetBlendedLinearAcceleration(HistoryWeight, NumSamples);
...
void UOHCombatUtils::GetBlendedBoneChainMotionData(UOHPhysicsManager* PhysicsManager, const TArray<FName>& BoneChain,
                                                   float HistoryWeight, int NumSamples, FVector& OutVelocity,
                                                   FVector& OutAcceleration, FVector& OutStartLocation,
...
void UOHCombatUtils::GetBlendedBoneChainMotionFromStrikeBone(
    UOHPhysicsManager* PhysicsManager, const USkeletalMeshComponent* Mesh, FName StrikeBone, FName ChainStart,
    int32 MaxDepth, bool bIsFullBodyAnimation, float HistoryWeight, int NumSamples, FVector& OutVelocity,
    FVector& OutAcceleration, FVector& OutStartLocation, FVector& OutForward, FVector& OutAngularVelocity,
    FVector& OutAngularAcceleration, float& OutJitterScore) {
    OutVelocity = FVector::ZeroVector;
    OutAcceleration = FVector::ZeroVector;
    OutStartLocation = FVector::ZeroVector;
    ...
    TArray<FName> BoneChain = ChainStart.IsNone()
                                  ? UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, NAME_None, StrikeBone)
                                  : UOHSkeletalPhysicsUtils::GetBoneChainBetweenByName(Mesh, ChainStart, StrikeBone);
    ...
    for (const FName& BoneName : BoneChain) {
        if (!bIsFullBodyAnimation && (BoneName == "pelvis" || BoneName.ToString().Contains(TEXT("root"))))
            continue;
        ...
        OutAcceleration += Data->GetBlendedLinearAcceleration(HistoryWeight, NumSamples);
        OutAngularVelocity += Data->GetBodyAngularVelocity();
        OutAngularAcceleration += Data->GetAngularAcceleration();
...
void UOHCombatUtils::AddWarpTargetToNearestEnemy(UMotionWarpingComponent* MotionWarpingComponent, float SearchRadius,
    ...
    for (TActorIterator<ACharacter> It(World); It; ++It) {
        ...
        float DistSqr = FVector::DistSquared(OwnerLocation, Candidate->GetActorLocation());
    ...
    if (ClosestTarget) {
        ...
        if (UCapsuleComponent* Capsule = ClosestTarget->GetCapsuleComponent()) {
            DrawDebugCapsule(World, TargetLocation, Capsule->GetScaledCapsuleHalfHeight(),
                             Capsule->GetScaledCapsuleRadius(), FQuat::Identity, FColor::Red, false, 2.0f, 0, 2.0f);
        }
...
float UOHCombatUtils::ComputeImpulseVulnerabilityScore(const FOHPhysicsGraphNode& Graph, FName BoneName,
    ...
    const FVector BoneDirection = (Bone->GetCurrentPosition() - RootLocation).GetSafeNormal();
    const float Dot = FVector::DotProduct(BoneDirection, ImpactNormal);
    return FMath::Clamp((1.f + Dot) * 0.5f, 0.f, 1.f); // Simple vulnerability heuristic
}

float UOHCombatUtils::ComputeImpulseVulnerabilityScore(const FOHBoneData& BoneData, const FVector& ImpactDirection,
                                                       const FVector& RootLocation) {
    if (!BoneData.IsValid()) {
        UE_LOG(LogTemp, Warning, TEXT("[CombatUtils] Invalid bone data in vulnerability score computation."));
        return 0.0f;
    }

    const FVector BoneVector = (BoneData.GetCurrentPosition() - RootLocation).GetSafeNormal();
    const float Dot = FVector::DotProduct(BoneVector, ImpactDirection);
    const float Score = FMath::Clamp((1.0f + Dot) * 0.5f, 0.0f, 1.0f); // -1..1 to 0..1

#if WITH_EDITOR
    UE_LOG(LogTemp, Verbose, TEXT("[CombatUtils] Vulnerability Score: Dot = %.2f → Score = %.2f"), Dot, Score);
#endif

    return Score;
}

FVector UOHCombatUtils::ComputeImpulseVectorForBone(const FOHPhysicsGraphNode& Graph, FName BoneName,
                                                    const FHitResult& Hit, const FVector& RootLocation,
                                                    float BaseStrength, EOHImpulseMode ModeOverride,
                                                    bool bAutoInferMode, EImpulseDirectionMode DirectionMode) {
    const FOHBoneData* Bone = Graph.FindBoneData(BoneName);
    if (!Bone || !Bone->IsValid()) {
        UE_LOG(LogTemp, Warning, TEXT("ComputeImpulseVectorForBone: Invalid bone %s"), *BoneName.ToString());
        return FVector::ZeroVector;
    }
    ...
    case EImpulseDirectionMode::FromBoneToImpactPoint:
        ImpulseDirection = (Hit.ImpactPoint - Bone->GetCurrentPosition()).GetSafeNormal();
        break;
    ...
    float Multiplier = ComputeImpulseVulnerabilityScore(Graph, BoneName, ImpulseDirection, RootLocation);
    ...
    case EOHImpulseMode::Radial:
        return (Bone->GetCurrentPosition() - RootLocation).GetSafeNormal() * Strength;
    ...
    case EOHImpulseMode::Auto:
        if (bAutoInferMode) {
            ...
            return bIsLeaf ? ImpulseDirection * Strength : FVector::UpVector * Strength;
        }
        return ImpulseDirection * Strength;
...
FVector UOHCombatUtils::GetImpulseDirectionFromHitNormal(const FHitResult& Hit) {
    return Hit.bBlockingHit ? -Hit.ImpactNormal.GetSafeNormal() : FVector::ZeroVector;
}

FVector UOHCombatUtils::GetImpulseDirectionFromBoneToImpactPoint(const FOHBoneData& BoneData, const FHitResult& Hit) {
...
FVector UOHCombatUtils::GetImpulseDirectionFromHit(const FOHBoneData& BoneData, const FHitResult& Hit,
...
float UOHCombatUtils::ComputeImpulseStrengthFromNormal(const FOHBoneData& BoneData, const FHitResult& Hit,
...
float UOHCombatUtils::ComputeImpulseStrengthFromImpactVector(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                             float BaseMultiplier) {
    const FVector Dir = GetImpulseDirectionFromBoneToImpactPoint(BoneData, Hit);
    return ComputeImpulseStrengthFromBoneData(BoneData, Dir, BaseMultiplier);
}

float UOHCombatUtils::ComputeImpulseStrengthFromBoneData(const FOHBoneData& BoneData, const FVector& OptionalStrikeDir,
                                                         float BaseMultiplier) {
    if (!BoneData.IsValid() || !BoneData.HasMotionHistory()) {
        UE_LOG(LogTemp, Warning, TEXT("[CombatUtils] Invalid or missing bone data — impulse strength defaulted to 0"));
        return 0.f;
    }

    const FOHMotionSample Sample = BoneData.GetLatestMotionSample();
    if (!Sample.IsValidSample()) {
        UE_LOG(LogTemp, Warning, TEXT("[CombatUtils] No valid motion sample in bone data"));
        return 0.f;
    }
    ...
    if (!OptionalStrikeDir.IsNearlyZero()) {
        ...
        const float Dot = FVector::DotProduct(BoneVelocity.GetSafeNormal(), OptionalStrikeDir.GetSafeNormal());
        DirectionScore = FMath::Clamp(Dot, 0.f, 1.f); // Emphasize forward-aligned strikes
    }

    // Weighted scoring formula
    const float WeightedScore =
        (LinearSpeed * 0.5f) + (AngularSpeed * 0.2f) + (LinearAccelMag * 0.2f) + (AngularAccelMag * 0.1f);

    const float FinalStrength = WeightedScore * DirectionScore * BaseMultiplier;

#if WITH_EDITOR
    UE_LOG(LogTemp, Log,
           TEXT("[CombatUtils] ImpulseStrength: %.2f (Linear: %.2f, Angular: %.2f, Accel: %.2f, DirScore: %.2f)"),
           FinalStrength, LinearSpeed, AngularSpeed, LinearAccelMag, DirectionScore);
#endif

    return FinalStrength;
}

float UOHCombatUtils::ComputeImpulseStrengthFromHit(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                    const USkeletalMeshComponent* SkeletalMesh, float BaseMultiplier) {
    if (!BoneData.IsValid() || !Hit.bBlockingHit || !IsValid(SkeletalMesh)) {
        UE_LOG(LogTemp, Warning, TEXT("[CombatUtils] Invalid input to ComputeImpulseStrengthFromHit"));
        return 0.0f;
    }

    // Anchor point for leverage: assume "pelvis"
    static const FName PelvisBone = FName("pelvis");
    const FVector RootLocation = SkeletalMesh->GetBoneLocation(PelvisBone, EBoneSpaces::WorldSpace);

    const FVector ImpactDir = Hit.ImpactNormal;
    const float VulnerabilityScore = ComputeImpulseVulnerabilityScore(BoneData, ImpactDir, RootLocation);

    const float FinalStrength = BaseMultiplier * VulnerabilityScore;

#if WITH_EDITOR
    UE_LOG(LogTemp, Log, TEXT("[CombatUtils] ImpulseStrength: Base = %.2f | Multiplier = %.2f | Final = %.2f"),
...
FVector UOHCombatUtils::ComputeImpulseVectorFromHit(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                    float BaseMultiplier, EImpulseDirectionMode Mode) {
    const FVector Dir = GetImpulseDirectionFromHit(BoneData, Hit, Mode);
    const float Strength = ComputeImpulseStrengthFromBoneData(BoneData, Dir, BaseMultiplier);
    const FVector FinalImpulse = Dir * Strength;

#if WITH_EDITOR
    UE_LOG(LogTemp, Log, TEXT("[CombatUtils] Impulse Vector (%s): Strength = %.2f | Direction = %s | Result = %s"),
           *UEnum::GetValueAsString(Mode), Strength, *Dir.ToString(), *FinalImpulse.ToString());
#endif

    return FinalImpulse;
}

FVector UOHCombatUtils::ComputeImpulseVectorFromHitNormal(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                          float BaseMultiplier) {
    const FVector Dir = GetImpulseDirectionFromHitNormal(Hit);
    const float Strength = ComputeImpulseStrengthFromBoneData(BoneData, Dir, BaseMultiplier);
    return Dir * Strength;
...
FVector UOHCombatUtils::ComputeImpulseVectorFromImpactVector(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                             float BaseMultiplier) {
    const FVector Dir = GetImpulseDirectionFromBoneToImpactPoint(BoneData, Hit);
    const float Strength = ComputeImpulseStrengthFromBoneData(BoneData, Dir, BaseMultiplier);
    return Dir * Strength;
...
EImpulseDirectionMode UOHCombatUtils::InferBestImpulseDirectionMode(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                                    float AlignmentThreshold) {
    if (!BoneData.IsValid() || !Hit.bBlockingHit)
        return EImpulseDirectionMode::FromHitNormal;

    const FVector BoneVelocity = BoneData.GetBodyLinearVelocity().GetSafeNormal();
    const FVector ToImpactPoint = (Hit.ImpactPoint - BoneData.GetCurrentPosition()).GetSafeNormal();

    const float Dot = FVector::DotProduct(BoneVelocity, ToImpactPoint);
    const bool bIsAligned = Dot > AlignmentThreshold;

#if WITH_EDITOR
    UE_LOG(LogTemp, Log, TEXT("[CombatUtils] Impulse Direction Auto-Detect: Dot = %.2f (%s)"), Dot,
           bIsAligned ? TEXT("BoneToImpact") : TEXT("HitNormal"));
#endif

    return bIsAligned ? EImpulseDirectionMode::FromBoneToImpactPoint : EImpulseDirectionMode::FromHitNormal;
}

FVector UOHCombatUtils::ComputeSelfReactionImpulse(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                   const FVector& RootLocation, float BaseStrength, float Mass) {
    if (!BoneData.IsValid() || !Hit.bBlockingHit) {
        return FVector::ZeroVector;
    }
    ...
    const float VelocityAlignment = FVector::DotProduct(BoneVel.GetSafeNormal(), -Hit.ImpactNormal);
    ...
    const float AngularFactor = FMath::Clamp(AngularMag / 300.f, 0.f, 1.f);        // Tunable
    const float LeverageFactor = FMath::Clamp(DistanceFromRoot / 100.f, 0.f, 2.f); // Tunable

    // 4. Composite multiplier
    const float Combined = (VelocityFactor * 0.4f) + (AngularFactor * 0.3f) + (LeverageFactor * 0.3f);
    const float FinalStrength = BaseStrength * Mass * FMath::Clamp(Combined, 0.2f, 2.0f);

    // 5. Final impulse
    const FVector Impulse = Direction * FinalStrength;

#if WITH_EDITOR
    UE_LOG(LogTemp, Log, TEXT("[SelfImpulse] VelDot=%.2f | Ang=%.2f | Dist=%.2f | Final=%.2f"), VelocityFactor,
           AngularFactor, LeverageFactor, FinalStrength);
#endif

    return Impulse;
}

FVector UOHCombatUtils::ComputeImpulseVectorFromBone(const FOHBoneData& BoneData, const FHitResult& Hit,
                                                     const FVector& RootLocation, float BaseStrength,
                                                     EImpulseDirectionMode DirectionMode, bool bUseAutoInfer) {
    if (!BoneData.IsValid() || !Hit.bBlockingHit)
        return FVector::ZeroVector;

    const FVector Dir = (bUseAutoInfer)
                            ? GetImpulseDirectionFromHit(BoneData, Hit, InferBestImpulseDirectionMode(BoneData, Hit))
                            : GetImpulseDirectionFromHit(BoneData, Hit, DirectionMode);

    const float Strength = ComputeImpulseStrengthFromBoneData(BoneData, Dir, BaseStrength);
    return Dir * Strength;
}

#pragma endregion

AActor* UOHCombatUtils::FindFuzzyTarget(const AActor* SelfActor, const FString& ContextualHint) {
    if (!SelfActor) {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] SelfActor is null!"));
        return nullptr;
    }

    UWorld* World = SelfActor->GetWorld();
    if (!World) {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] Could not get world from SelfActor [%s]"),
               *SelfActor->GetName());
        return nullptr;
    }
    ...
    if (Candidates.Num() == 0) {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] No candidates found in world for [%s]"),
               *SelfActor->GetName());
        return nullptr;
    }
    ...
    for (AActor* Candidate : Candidates) {
        ...
        const FVector ToTarget = (Candidate->GetActorLocation() - SelfLoc).GetSafeNormal();
        ...
        if (!ContextualHint.IsEmpty()) {
            ...
            float FuzzyScore = UOHAlgoUtils::JaroWinklerDistance(TargetName, ContextualHint.ToLower());
    ...
    // --- Use OHSafeMapUtils if you want to process scores more defensively/efficiently ---
    // Example: Get candidate with max score (safe, future-proof, easy to extend)
    ...
    if (!BestCandidate) {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] No valid opponent found for [%s]"),
               *SelfActor->GetName());
    }
...
bool UOHCombatUtils::PredictiveCapsuleGroundTrace(USkeletalMeshComponent* Mesh, FName BoneName, float MaxDistance,
    ...
    bool bHit =
        Mesh->GetWorld()->SweepSingleByChannel(OutHit, PredictedStart, End, FQuat::Identity, ECC_Visibility,
                                               FCollisionShape::MakeCapsule(CapsuleRadius, CapsuleHalfHeight), Params);

    if (bDrawDebug) {
        FColor TraceColor = bHit ? FColor::Green : FColor::Red;
        DrawDebugCapsule(Mesh->GetWorld(), PredictedStart, CapsuleHalfHeight, CapsuleRadius, FQuat::Identity,
                         TraceColor, false, 0.05f, 0, 1.0f);
        DrawDebugLine(Mesh->GetWorld(), PredictedStart, End, TraceColor, false, 0.05f, 0, 1.0f);
        if (bHit) {
            DrawDebugPoint(Mesh->GetWorld(), OutHit.ImpactPoint, 12.f, FColor::Blue, false, 0.1f);
            DrawDebugDirectionalArrow(Mesh->GetWorld(), OutHit.ImpactPoint,
                                      OutHit.ImpactPoint + OutHit.ImpactNormal * 30.f, 16.f, FColor::Yellow, false,
                                      0.1f, 0, 1.5f);
        }
    }
    return bHit;
...
AActor* UOHCombatUtils::FindMostLikelyOpponent(const AActor* SelfActor, const FString& ContextualHint) {
    ...
    for (TActorIterator<AActor> It(World); It; ++It) {
        ...
        // --- Skip hitboxes/child actors if their parent/owner is a Pawn/Character (common in fighting games) ---
        if (Candidate->GetOwner() && Candidate->GetOwner()->IsA(APawn::StaticClass()))
            continue;
        if (Candidate->GetAttachParentActor() && Candidate->GetAttachParentActor()->IsA(APawn::StaticClass()))
            continue;

        // --- Optional: Skip actors that aren't Pawns (or Characters, or your base combat class) ---
        // If you want to *only* ever target pawns/characters, uncomment below:
        // if (!Candidate->IsA(APawn::StaticClass())) continue;
