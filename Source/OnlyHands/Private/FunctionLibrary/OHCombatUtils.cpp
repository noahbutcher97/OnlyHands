	...
	FVector SourceLocation, ERotationContext CurrentState, ERotationContext& NextState, ACharacter*& ClosestCharacter)
{
	...
	else if (CurrentState == ERotationContext::Off ||
			 CurrentState == ERotationContext::TurnOff)
	{
	...
	FVector SourceLocation, ERotationContext CurrentState, ERotationContext& NextState, ACharacter*& ClosestCharacter)
{
	...
	if (NumChars == 0)
	{
		if (CurrentState == ERotationContext::Tracking ||
			CurrentState == ERotationContext::TurnOn)
		{
	...
	{
		if (CurrentState == ERotationContext::Off ||
			CurrentState == ERotationContext::TurnOff)
		{
			NextState = ERotationContext::TurnOn;
		}
...
void UOHCombatUtils::InterpComponentRotation(USceneComponent* Component, float DeltaTime, ERotationContext Context,
                                             const FRotator& OnRotation, const FRotator& OffRotation, const FRotator& TrackingRotation, float InterpSpeed)
{
	if (!Component) return;
...
ERotationContext UOHCombatUtils::DetermineNextRotationContext(
	...
	int32 NumOverlappingCharacters)
{
	...
	{
		if (CurrentContext == ERotationContext::Tracking ||
			CurrentContext == ERotationContext::TurnOn)
		{
	...
	if (CurrentContext == ERotationContext::Off ||
		CurrentContext == ERotationContext::TurnOff)
	{
...
{
	return FMath::Clamp(FVector2D::DotProduct(ProjectVectorTo2D(A.GetSafeNormal()), ProjectVectorTo2D(B.GetSafeNormal())), -1.f, 1.f);
}
...
bool UOHCombatUtils::IsFootApproachingGround(
...
float UOHCombatUtils::GetTorsoLeanAngle(USkeletalMeshComponent* Mesh, FName SpineBone)
{
	...
	float Angle = FMath::RadiansToDegrees(
		FMath::Acos(FVector::DotProduct(SpineForward.GetSafeNormal(), FVector::UpVector)));
...
float UOHCombatUtils::GetShoulderCompressionRatio(USkeletalMeshComponent* Mesh, FName ClavicleLeft, FName ClavicleRight)
{
	if (!Mesh)
	{
		return 0.f;
...
bool UOHCombatUtils::IsArmAlignedWithMovement(
	USkeletalMeshComponent* Mesh,
	FName ClavicleBone,
	FName HandBone,
...
float UOHCombatUtils::GetArmToFacingAngle(
	USkeletalMeshComponent* Mesh,
	FName ClavicleBone,
	FName HandBone)
{
...
float UOHCombatUtils::GetStrikePrepAlignmentScore(
	USkeletalMeshComponent* Mesh,
	FName ClavicleBone,
	FName HandBone,
	FVector ActorVelocity)
...
bool UOHCombatUtils::IsStrikeArcingTowardTarget(
	USkeletalMeshComponent* Mesh,
	FName ClavicleBone,
	FName HandBone,
	AActor* Target,
...
float UOHCombatUtils::GetActorFacingAngleRelativeToCamera(const AActor* TargetActor)
{
	...
	const float Angle = FMath::RadiansToDegrees(FMath::Atan2(
		FVector::CrossProduct(CameraForward, ActorForward).Z,
...
float UOHCombatUtils::GetActorMovementDirectionRelativeToCamera(const AActor* TargetActor)
{
	...
	const float Angle = FMath::RadiansToDegrees(FMath::Atan2(
		FVector::CrossProduct(CameraForward, MoveDirection).Z,
...
bool UOHCombatUtils::GetClosestCharacterToLocationFromActors(const FVector& SourceLocation,
	const TArray<AActor*>& Actors, ACharacter*& ClosestCharacter)
{
...
bool UOHCombatUtils::GetPlayerCameraView(
...
void UOHCombatUtils::DebugDrawActorMovementDirection(const AActor* TargetActor, float Duration, FColor Color)
{
	...
	DrawDebugString(TargetActor->GetWorld(), TargetActor->GetActorLocation() + Offset, Label, nullptr, Color, Duration, false);
}


bool UOHCombatUtils::IsActorWithinRange(
	const AActor* Source,
	const AActor* Target,
	float MaxDistance,
	float Radius)
                                                     ...
                                                     bool bUse8Directions)
{
	...
	float YawAngle = FMath::RadiansToDegrees(FMath::Atan2(
		FVector::CrossProduct(TargetForward, HitDirection).Z,
...
void UOHCombatUtils::DebugDrawHitDirection(
	...
	float ArrowSize)
{
	...
	switch (Dir)
	{
	case EOHHitDirection::Front:       Color = FColor::Green; break;
	case EOHHitDirection::FrontRight:  Color = FColor::Emerald; break;
	case EOHHitDirection::Right:       Color = FColor::Blue; break;
	case EOHHitDirection::BackRight:   Color = FColor::Cyan; break;
	case EOHHitDirection::Back:        Color = FColor::Red; break;
	case EOHHitDirection::BackLeft:    Color = FColor::Magenta; break;
	case EOHHitDirection::Left:        Color = FColor::Yellow; break;
	case EOHHitDirection::FrontLeft:   Color = FColor::Orange; break;
...
FStrikeContactMetrics UOHCombatUtils::ComputeStrikeContactMetrics(
...
FVector UOHCombatUtils::GetBoneVelocitySafe(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	float DeltaTime)
{
...
void UOHCombatUtils::BeginDeferredStrikeSweep(
	FDeferredStrikeSweepState& State,
	USkeletalMeshComponent* Mesh,
	AActor* Owner,
...
void UOHCombatUtils::TickDeferredStrikeSweep(
	FDeferredStrikeSweepState& State,
	float DeltaTime)
{
...
bool UOHCombatUtils::GetActiveMontageBlendWeight(
	USkeletalMeshComponent* Mesh,
...
FOHMontagePlaybackState UOHCombatUtils::GetMontagePlaybackState(USkeletalMeshComponent* Mesh)
{
	...
	if (SectionIndex != INDEX_NONE)
	{
		...
		State.NormalizedTime = (State.SectionLength > 0.f)
			? FMath::Clamp((Position - SectionStart) / State.SectionLength, 0.f, 1.f)
			: 0.f;
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("[MontagePlaybackState] Could not resolve section from playback position %.3f"), Position);
	}
	...
	for (const FSlotAnimationTrack& Slot : Montage->SlotAnimTracks)
	{
		for (const FAnimSegment& Segment : Slot.AnimTrack.AnimSegments)
		{
			...
			if (Position >= SegmentStart && Position <= SegmentEnd)
			{
				if (UAnimSequence* Seq = Cast<UAnimSequence>(Segment.GetAnimReference()))
				{
					...
					State.SegmentNormalizedTime = (State.SegmentLength > 0.f)
		...
		if (State.CurrentAnimSequence) break;
	}
...
FOHRootMotionState UOHCombatUtils::GetPoseRootMotionState(USkeletalMeshComponent* Mesh, float DeltaTime)
{
	...
	const FTransform DeltaTransform = MontageInstance->Montage->ExtractRootMotionFromTrackRange(CurrentTime, FutureTime);
...
FOHMontageSectionWindow UOHCombatUtils::GetAnimSectionTransitionWindow(
	USkeletalMeshComponent* Mesh,
	...
	float CancelWindowEnd)
{
	...
	const float NormalizedTime = (SectionLength > 0.f)
		? FMath::Clamp(LocalTime / SectionLength, 0.f, 1.f)
		: 0.f;
...
void UOHCombatUtils::DrawPredictedRootMotionArc(
	...
	const FOHRootMotionState& RootMotion,
	FColor Color,
	float Duration,
	float Thickness)
{
...
bool UOHCombatUtils::GetMontageSectionTimeRange(
...
bool UOHCombatUtils::GetMontageSectionNormalizedTime(
	UAnimMontage* Montage,
	FName SectionName,
	float PlaybackTime,
	float& OutNormalizedTime)
{
	...
	OutNormalizedTime = (Length > 0.f) 
		? FMath::Clamp((PlaybackTime - Start) / Length, 0.f, 1.f)
		: 0.f;
...
bool UOHCombatUtils::IsInMontageSectionWindow(
	const FOHMontagePlaybackState& PlaybackState,
	float WindowStart,
	float WindowEnd)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[IsInMontageSectionWindow] Invalid window range: Start=%.2f End=%.2f"), WindowStart, WindowEnd);
		return false;
	}
	...
	// UE_LOG(LogTemp, Log, TEXT("[IsInMontageSectionWindow] NormalizedTime=%.3f InWindow=%s"), T, bInWindow ? TEXT("YES") : TEXT("NO"));
...
TArray<FInputWindow> UOHCombatUtils::ExtractInputWindowsFromMontage(UAnimMontage* Montage, FName SectionName)
{
    ...
    if (!Montage) return TArray<FInputWindow>();
...
bool UOHCombatUtils::IsStrikeContactValid(
	FVector Velocity,
...
float UOHCombatUtils::ComputeContactScore(
	FVector Velocity,
	FVector ContactNormal,
	float EstimatedForce)
{
...
FVector UOHCombatUtils::ConvertWorldToActorLocal(const AActor* ReferenceActor, const FVector& WorldLocation)
{
	return ReferenceActor
		       ? ReferenceActor->GetTransform().InverseTransformPosition(WorldLocation)
		       : FVector::ZeroVector;
}
...
void UOHCombatUtils::DrawDebugPath(
	const UObject* WorldContextObject,
...
void UOHCombatUtils::DrawDebugStrikeCone(
	const UObject* WorldContextObject,
...
FVector UOHCombatUtils::GetBoneLinearVelocity(
	...
	float TimeOffset)
{
	...
	const FTransform RootMotionDelta = MontageInstance->Montage->ExtractRootMotionFromTrackRange(
	...
	const FVector PredictedWorld = Mesh->GetComponentTransform().TransformPosition(
...
void UOHCombatUtils::DetectContactAlongStrikeArc(
	const UObject* WorldContextObject,
	const TArray<FVector>& ArcPoints,
...
bool UOHCombatUtils::DetectGJKContactFromStrikeArc(
	const TArray<FVector>& ArcPoints,
	const TArray<FVector>& TargetConvexPoints,
	...
	FVector& OutPenetrationVector)
{
	...
	bool bGJK = UOHCollisionUtils::GJK_Intersect(
		ArcHull,
		TargetConvexPoints,
		Simplex,
		SupportA,
...
static bool SampleStrikeArc_Internal(
	...
	bool bDrawImpactNormals)
{
	...
	if (UAnimMontage* Montage = Cast<UAnimMontage>(AnimBase))
	{
		if (!MontageSectionName.IsNone())
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("VisualizeBoneMotionArcFromAnim: Montage section '%s' not found."), *MontageSectionName.ToString());
			}
		}
		else
		{
	...
	if (bDrawVelocity || bDrawImpactNormals)
	{
		for (int32 i = 1; i < ArcPoints.Num(); ++i)
		{
			...
			if (bDrawVelocity)
			{
				DrawDebugLine(WorldContextObject->GetWorld(), Start, Start + Velocity * 20.f, FColor::Yellow, false, Duration, 0, 1.5f);
			}
			...
			if (bDrawImpactNormals && i < ArcPoints.Num() - 1)
			{
				...
				DrawDebugLine(WorldContextObject->GetWorld(), End, End + Normal * 20.f, FColor::Cyan, false, Duration, 0, 1.0f);
			}
		}
	}
}
	...
	bool bDrawImpactNormals)
{
	...
	TArray<FVector> ArcPoints;
	if (!GenerateStrikeArcFromAnimSequence(Sequence, Mesh, BoneName, StartTime, EndTime, ArcPoints, NumSamples / PredictionWindow))
	{
...
)
{
	...
	if (!WorldContextObject || !Attacker || !PhysicsManager) return Result;

	UWorld* World = WorldContextObject->GetWorld();
	if (!World) return Result;

	const FOHBoneData* BoneData = PhysicsManager->GetBoneData(BoneName);
	if (!BoneData || !BoneData->IsValid()) return Result;
	...
	if (!ClosestTarget) return Result;
	USkeletalMeshComponent* TargetMesh = ClosestTarget->GetMesh();
	if (!TargetMesh) return Result;
	...
	// === Result ===
	if (BestTargetBone.IsNone()) return Result;
	...
	if (bEnableDebug)
	{
		...
		DrawDebugString(World, BestPredictedPos + FVector(0, 0, 12),
			FString::Printf(TEXT("Target Bone: %s\nDist: %.1f\nTTC: %.2fs"), *BestTargetBone.ToString(), BestDistance, BestTime),
...
)
{
	...
	if (!ClosestTarget || !ClosestTarget->GetMesh()) return Result;
	...
	if (BoneStr.Contains("head") || BoneStr.Contains("neck")) Region = "Head";
	else if (BoneStr.Contains("spine") || BoneStr.Contains("pelvis")) Region = "Torso";
	else if (BoneStr.Contains("hand") || BoneStr.Contains("arm")) Region = "Arm";
	else if (BoneStr.Contains("leg") || BoneStr.Contains("foot")) Region = "Leg";
...
FCombatStrikePredictionResult UOHCombatUtils::AnalyzeAdvancedStrikeTrajectory3D(
	...
	bool bEnableDebug,
	FName DebugTag,
	FName OptionalChainStart,
	int32 MaxChainDepth,
	bool bIsFullBodyAnimation,
...
)
{
	...
	UWorld* World = WorldContextObject->GetWorld();
	const USkeletalMeshComponent* Mesh = Attacker->GetMesh();
	if (!World || !Mesh) return Result;

	// --- 1. Bone Chain ---
	TArray<FName> BoneChain = OptionalChainStart.IsNone()
	...
	if (BoneChain.Num() == 0) BoneChain.Add(StrikeBone);
	...
	for (const FName& Bone : BoneChain)
	{
		...
		const FOHBoneData* BoneData = PhysicsManager->GetBoneData(Bone);
		if (!BoneData || !BoneData->IsValid()) continue;
		...
		if (bEnableDebug)
		{
			...
			DrawDebugString(World, BoneData->GetCurrentPosition() + FVector(0,0,10), Bone.ToString(), nullptr, FColor::White, 2.f);
		}
	}
	if (ValidBones.Num() == 0) return Result;
	...
	for (int32 Step = 1; Step <= Steps; ++Step)
	{
		...
		if (bEnableDebug)
		{
			FLinearColor LinearColor = FLinearColor::LerpUsingHSV(FLinearColor::Green, FLinearColor::Red, Step / static_cast<float>(Steps));
			...
			DrawDebugString(World, Predicted + FVector(0, 0, 12), FString::Printf(TEXT("T: %.2f"), T), nullptr, StepColor, 2.f);
		}
	}
	...
	if (BoneStr.Contains("head") || BoneStr.Contains("neck")) { Region = "Head"; ImpactWeight = 2.f; }
	else if (BoneStr.Contains("spine") || BoneStr.Contains("pelvis")) { Region = "Torso"; ImpactWeight = 1.5f; }
	else if (BoneStr.Contains("hand") || BoneStr.Contains("arm")) { Region = "Arm"; ImpactWeight = 0.75f; }
	else if (BoneStr.Contains("leg") || BoneStr.Contains("foot")) { Region = "Leg"; ImpactWeight = 0.75f; }
	...
	if (bEnableDebug)
	{
		...
		DrawDebugString(World, BestPos + FVector(0, 0, 30),
			FString::Printf(TEXT("Target: %s\nRegion: %s\nScore: %.2f\nAngle: %.1f°"),
				*BestTargetBone.ToString(), *Region.ToString(), Result.DirectionalityScore, Result.AngularDeviationDegrees),
...
)
{
	...
	for (const FName& Bone : BoneChain)
	{
		...
		if (bEnableDebug)
		{
			...
			DrawDebugString(World, Data->GetCurrentPosition() + FVector(0, 0, 10), Bone.ToString(), nullptr, FColor::White, 2.f);
		}
	}
	if (ValidBones.Num() == 0) return Result;
	...
	{
		ACharacter* Other = *It;
		if (!Other || Other == Attacker) continue;
	...
	for (int32 Step = 1; Step <= Steps; ++Step)
	{
		...
		if (bEnableDebug)
		{
			...
			DrawDebugString(World, StepPos + FVector(0, 0, 10), FString::Printf(TEXT("T: %.2f"), T), nullptr, FColor::White, 2.f);
		}
	}
	if (BestTargetBone.IsNone()) return Result;
...
{
	FOHBoneMotionSample Result = SampleBoneMotionInternal(
		AnimBase, Mesh, BoneName, NumSamples, StartTime, EndTime, MontageSectionName);
...
FOHBoneMotionSample UOHCombatUtils::SampleBoneMotionFromAnimBasePure(
	...
	FName MontageSectionName)
{
	return SampleBoneMotionInternal(
		AnimBase, Mesh, BoneName, NumSamples, StartTime, EndTime, MontageSectionName);
}
	...
	FName MontageSectionName)
{
	...
	if (!Sequence)
	{
		UE_LOG(LogTemp, Warning, TEXT("[SampleBoneMotion] Could not resolve sequence at time %.2f in %s"),
...
bool UOHCombatUtils::GetBoneTransformAtTime(
	UAnimSequence* Sequence,
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	double Time,
...
bool UOHCombatUtils::GetBoneTransformWorldAtTime(
	UAnimSequence* Sequence,
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	double Time,
...
bool UOHCombatUtils::GetBoneRotationAtTime(
	UAnimSequence* Sequence,
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	double Time,
...
{
	FTransform BoneTransform;
	const bool bSuccess = bWorldSpace
		? GetBoneTransformWorldAtTime(Sequence, Mesh, BoneName, Time, BoneTransform)
...
bool UOHCombatUtils::GetBoneVelocityAtTime(
	UAnimSequence* Sequence,
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	double Time,
	...
	double Time)
{
	...
	if (BoneIndex == INDEX_NONE)
	{
		UE_LOG(LogTemp, Warning, TEXT("[OH] [GetBoneComponentSpaceTransformAtTime] Bone '%s' not found in mesh."), *BoneName.ToString());
		return FTransform::Identity;
	}
...
UAnimSequence* UOHCombatUtils::ExtractSequenceFromAnimBaseAtTime(UAnimSequenceBase* AnimBase, float Time)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ExtractSequenceFromAnimBaseAtTime: No sequence found at time %.3f in %s"),
			Time, *GetNameSafe(AnimBase));
	}
...
void UOHCombatUtils::VisualizeMotionDerivatives(
	...
	const TArray<FVector>& Accelerations,
	float Duration,
	float Scale,
	float Thickness)
{
	if (!WorldContextObject || Points.Num() < 2 || Velocities.Num() != Points.Num() || Accelerations.Num() != Points.Num())
	{
	...
	float EndTime)
{
	...
	if (bCallerProvidedRange)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("[OH] GenerateBoneMotionSample: Invalid time range provided (%.2f → %.2f), Max: %.2f"),
				   StartTime, EndTime, MaxDuration);
			return Sample;
		}
	}

	Sample = SampleBoneMotionInternal(
		AnimBase,
		Mesh,
		BoneName,
		NumSamples,
...
void UOHCombatUtils::TestDrawArcFromMesh(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	float Duration,
	FColor Color)
{
	if (!Mesh || BoneName.IsNone()) return;
...
void UOHCombatUtils::QuickDrawBoneArcFromMesh(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	float PredictDuration,
	FColor Color,
	...
	float Thickness)
{
	...
	UE_LOG(LogTemp, Log, TEXT("[OH] Drawing arc from %.2f to %.2f with %d points"),
		T0, T1, ArcPoints.Num());
...
void UOHCombatUtils::ScheduleDeferredArcDraw(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
	float PredictDuration,
	FColor DebugColor,
	...
	float Thickness)
{
	...
	World->GetTimerManager().SetTimer(DummyTimer, [=]()
	{
		UAnimInstance* AnimInst = Mesh->GetAnimInstance();
		if (!AnimInst)
		{
		...
		if (ArcPoints.Num() >= 2)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("[OH] Drew arc (%d points) for bone '%s'"), ArcPoints.Num(), *BoneName.ToString());
		}
...
FVector UOHCombatUtils::ComputeImpulseVectorForBone(const FOHPhysicsGraphNode& Graph, FName BoneName,
                                                    const FHitResult& Hit, const FVector& RootLocation, float BaseStrength, EOHImpulseMode ModeOverride,
...
FVector UOHCombatUtils::GetImpulseDirectionFromHit(const FOHBoneData& BoneData, const FHitResult& Hit, EImpulseDirectionMode Mode)
{
...
float UOHCombatUtils::ComputeImpulseStrengthFromNormal(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier)
{
...
float UOHCombatUtils::ComputeImpulseStrengthFromImpactVector(const FOHBoneData& BoneData, const FHitResult& Hit, float BaseMultiplier)
{
...
float UOHCombatUtils::ComputeImpulseStrengthFromBoneData(
	const FOHBoneData& BoneData,
	const FVector& OptionalStrikeDir,
...
#if WITH_EDITOR
	UE_LOG(LogTemp, Log, TEXT("[CombatUtils] ImpulseStrength: %.2f (Linear: %.2f, Angular: %.2f, Accel: %.2f, DirScore: %.2f)"),
	...
	float BaseMultiplier)
{
	...
	const float VulnerabilityScore = ComputeImpulseVulnerabilityScore(
		BoneData,
		ImpactDir,
		RootLocation);
...
#if WITH_EDITOR
	UE_LOG(LogTemp, Log, TEXT("[CombatUtils] Impulse Direction Auto-Detect: Dot = %.2f (%s)"),
		Dot,
...
FVector UOHCombatUtils::ComputeSelfReactionImpulse(
	const FOHBoneData& BoneData,
	const FHitResult& Hit,
	const FVector& RootLocation,
	float BaseStrength,
...
AActor* UOHCombatUtils::FindFuzzyTarget(const AActor* SelfActor, const FString& ContextualHint)
{
    ...
    if (!World)
    {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] Could not get world from SelfActor [%s]"), *SelfActor->GetName());
        return nullptr;
    }
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (Candidate == SelfActor) continue; // Don't target self
		if (!IsValid(Candidate)) continue;    // Dead or being GC'd
    ...
    if (Candidates.Num() == 0)
    {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] No candidates found in world for [%s]"), *SelfActor->GetName());
        return nullptr;
    }
    ...
    if (!BestCandidate)
    {
        UE_LOG(LogTemp, Warning, TEXT("[FindMostLikelyOpponent] No valid opponent found for [%s]"), *SelfActor->GetName());
    }
...
bool UOHCombatUtils::PredictiveCapsuleGroundTrace(
	...
	FName BoneName,
	float MaxDistance,
	float TraceForwardBias,
	float CapsuleRadius,
	float CapsuleHalfHeight,
	const FVector& OwnerVelocity,
	FHitResult& OutHit,
	bool bDrawDebug
)
{
	...
	if (bDrawDebug)
	{
		...
		DrawDebugCapsule(Mesh->GetWorld(), PredictedStart, CapsuleHalfHeight, CapsuleRadius, FQuat::Identity, TraceColor, false, 0.05f, 0, 1.0f);
		...
		if (bHit)
		{
			...
			DrawDebugDirectionalArrow(Mesh->GetWorld(), OutHit.ImpactPoint, OutHit.ImpactPoint + OutHit.ImpactNormal * 30.f, 16.f, FColor::Yellow, false, 0.1f, 0, 1.5f);
		}
	}
	return bHit;
}

float UOHCombatUtils::CalculateMassRatio(
    AActor* Attacker,
    AActor* Target,
    bool bVerbose
)
{
    ...
    if (!FMath::IsFinite(AttackerMass) || AttackerMass <= 0.0f)
    {
        if (bVerbose)
        {
            UE_LOG(LogTemp, Warning, TEXT("OHCombatUtils::CalculateMassRatio: Invalid Attacker mass (%.2f), using 1.0f"), AttackerMass);
        }
        AttackerMass = 1.0f;
    }
    if (!FMath::IsFinite(TargetMass) || TargetMass <= 0.0f)
    {
        if (bVerbose)
        {
            UE_LOG(LogTemp, Warning, TEXT("OHCombatUtils::CalculateMassRatio: Invalid Target mass (%.2f), using 1.0f"), TargetMass);
        }
        TargetMass = 1.0f;
    }
    ...
    if (bVerbose)
    {
        UE_LOG(LogTemp, Log, TEXT("OHCombatUtils::CalculateMassRatio: AttackerMass=%.2f, TargetMass=%.2f, Ratio=%.2f"), AttackerMass, TargetMass, Ratio);
    }
...
)
{
	...
	for (const FName& BoneName : BoneNames)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("CalculateChainMass: Missing or invalid body for bone %s on %s"), *BoneName.ToString(), *Actor->GetName());
		}
	}
...
AActor* UOHCombatUtils::FindMostLikelyOpponent(const AActor* SelfActor, const FString& ContextualHint)
{
    ...
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        AActor* Candidate = *It;
        if (!Candidate) continue;
        if (Candidate == SelfActor) continue;
        if (!IsValid(Candidate)) continue;

        // --- Skip hitboxes/child actors if their parent/owner is a Pawn/Character (common in fighting games) ---
        if (Candidate->GetOwner() && Candidate->GetOwner()->IsA(APawn::StaticClass())) continue;
        if (Candidate->GetAttachParentActor() && Candidate->GetAttachParentActor()->IsA(APawn::StaticClass())) continue;
    ...
    for (AActor* Candidate : Candidates)
    {
        ...
        if (Candidate->IsA(APawn::StaticClass())) Score += 100.f; // Strong preference
        if (Candidate->IsA(AOHCombatCharacter::StaticClass())) Score += 200.f; // Top preference
