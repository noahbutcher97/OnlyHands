...
bool UOHCollisionUtils::AABB_Overlap(const FVector &CenterA,
...
FVector UOHCollisionUtils::ComputeAABBDistanceOrPenetration(
    const FVector &CenterA, const FVector &ExtentsA, const FVector &CenterB,
    const FVector &ExtentsB) {
...
FVector UOHCollisionUtils::ClosestPointOnAABB(const FVector &Point,
...
FVector UOHCollisionUtils::SupportAABB(const FVector &Direction,
                                       const FVector &Center,
                                       const FVector &Extents) {
  return Center + FVector(Direction.X >= 0 ? Extents.X : -Extents.X,
...
bool UOHCollisionUtils::SAT_BoxIntersect(const FVector &CenterA,
  ...
  const FVector AxesA[3] = {RotationA.RotateVector(FVector::ForwardVector),
                            RotationA.RotateVector(FVector::RightVector),
                            RotationA.RotateVector(FVector::UpVector)};

  const FVector AxesB[3] = {RotationB.RotateVector(FVector::ForwardVector),
                            RotationB.RotateVector(FVector::RightVector),
                            RotationB.RotateVector(FVector::UpVector)};
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    float rb = ExtentsB.X * AbsR[i][0] + ExtentsB.Y * AbsR[i][1] +
               ExtentsB.Z * AbsR[i][2];
  ...
  for (int i = 0; i < 3; ++i) {
    float ra = ExtentsA.X * AbsR[0][i] + ExtentsA.Y * AbsR[1][i] +
               ExtentsA.Z * AbsR[2][i];
...
#pragma region OBB
bool UOHCollisionUtils::IsPointInOBB(const FVector &Point,
                                     const FVector &BoxCenter,
  ...
  return FMath::Abs(LocalPoint.X) <= BoxExtents.X &&
         FMath::Abs(LocalPoint.Y) <= BoxExtents.Y &&
         FMath::Abs(LocalPoint.Z) <= BoxExtents.Z;
}

FVector UOHCollisionUtils::SupportOBB(const FVector &Direction,
                                      const FVector &BoxCenter,
                                      const FRotator &BoxRotation,
                                      const FVector &BoxExtents) {
...
FVector UOHCollisionUtils::ClosestPointOnOBB(const FVector &Point,
                                             const FVector &BoxCenter,
                                             const FRotator &BoxRotation,
                                             const FVector &BoxExtents) {
  ...
  ClampedLocalPoint.X =
      FMath::Clamp(ClampedLocalPoint.X, -BoxExtents.X, BoxExtents.X);
  ClampedLocalPoint.Y =
      FMath::Clamp(ClampedLocalPoint.Y, -BoxExtents.Y, BoxExtents.Y);
  ClampedLocalPoint.Z =
      FMath::Clamp(ClampedLocalPoint.Z, -BoxExtents.Z, BoxExtents.Z);
...
TArray<FVector>
UOHCollisionUtils::ComputeMinkowskiSum(const TArray<FVector> &ShapeA,
...
TArray<FVector>
UOHCollisionUtils::ComputeMinkowskiDifference(const TArray<FVector> &ShapeA,
                                              const TArray<FVector> &ShapeB) {
  TArray<FVector> Result;
  for (const FVector &A : ShapeA) {
    for (const FVector &B : ShapeB) {
...
bool UOHCollisionUtils::GJK_Intersect(const TArray<FVector> &ShapeA,
                                      const TArray<FVector> &ShapeB,
                                      int32 MaxIterations, float Tolerance) {
...
FVector
UOHCollisionUtils::GetFarthestPointInDirection(const TArray<FVector> &Points,
...
FVector UOHCollisionUtils::TripleCrossProduct(const FVector &A,
                                              const FVector &B,
                                              const FVector &C) {
  return FVector::CrossProduct(FVector::CrossProduct(A, B), C);
}

bool UOHCollisionUtils::ProcessSimplex(TArray<FVector> &Simplex,
...
bool UOHCollisionUtils::EPA_PenetrationDepth(
    const TArray<FVector> &ShapeA, const TArray<FVector> &ShapeB,
  ...
  for (int Iter = 0; Iter < MaxIterations; ++Iter) {
    ...
    for (const auto &Face : Faces) {
      ...
      const FVector &Normal =
          FVector::CrossProduct(Vertices[B] - PA, Vertices[C] - PA)
              .GetSafeNormal();
...
TArray<FVector> UOHCollisionUtils::GenerateBoxHull(const FVector &Center,
                                                   const FVector &Extents,
                                                   const FRotator &Rotation) {
  const FTransform BoxTransform(Rotation, Center);

  const FVector LocalCorners[8] = {FVector(-Extents.X, -Extents.Y, -Extents.Z),
...
TArray<FVector> UOHCollisionUtils::GenerateSphereHull(const FVector &Center,
                                                      float Radius,
                                                      int32 Subdivisions) {
  ...
  for (int32 Lat = 0; Lat <= LatitudeSteps; ++Lat) {
    ...
    for (int32 Lon = 0; Lon < LongitudeSteps; ++Lon) {
      ...
      FVector Point(Radius * SinTheta * CosPhi, Radius * SinTheta * SinPhi,
                    Radius * CosTheta);
...
TArray<FVector> UOHCollisionUtils::GenerateCapsuleHull(
    const FVector &Center, const FRotator &Rotation, float HalfHeight,
    float Radius, int32 HemisphereResolution, int32 RingResolution) {
  TArray<FVector> Points;
  ...
  for (int32 LatStep = 0; LatStep <= HemisphereResolution; ++LatStep) {
    float Theta =
        (PI / 2.0f) * (static_cast<float>(LatStep) / HemisphereResolution);
    ...
    for (int32 LonStep = 0; LonStep < RingResolution; ++LonStep) {
      ...
      FVector Bottom =
          ...
          Rotation.RotateVector(FVector(X * R, Z * R, -Y) * Radius);
      Points.Add(Bottom);
    }
  }
...
TArray<FVector>
UOHCollisionUtils::ComputeConvexHull(const TArray<FVector> &InputPoints) {
  TArray<FVector> Result;
  ComputeConvexHull_Internal(InputPoints, Result);
  return Result;
}

void UOHCollisionUtils::ComputeConvexHull_Internal(
    const TArray<FVector> &InPoints, TArray<FVector> &OutHullVertices,
...
FVector
UOHCollisionUtils::ComputeAverageContactPoint(const TArray<FVector> &SupportA,
...
TArray<FVector> UOHCollisionUtils::GetStaticMeshComponentVertices(
    UStaticMeshComponent *StaticComp, bool bWorldSpace) {
  TArray<FVector> Points;
  if (!StaticComp)
    return Points;
  ...
  const FPositionVertexBuffer &VertexBuffer =
      LODResource.VertexBuffers.PositionVertexBuffer;

  for (uint32 i = 0; i < VertexBuffer.GetNumVertices(); ++i) {
    FVector LocalPos = FVector(VertexBuffer.VertexPosition(i));
    Points.Add(
        bWorldSpace
            ? StaticComp->GetComponentTransform().TransformPosition(LocalPos)
            : LocalPos);
  }
  return Points;
}

TArray<FVector> UOHCollisionUtils::GetSkeletalMeshComponentVertices(
    USkeletalMeshComponent *SkelComp, bool bWorldSpace) {
  TArray<FVector> Points;
  if (!SkelComp)
    return Points;
  ...
  const FSkeletalMeshRenderData *RenderData =
      SkelComp->GetSkeletalMeshRenderData();
  ...
  for (uint32 i = 0; i < VertexBuffer.GetNumVertices(); ++i) {
    ...
    Points.Add(
        bWorldSpace
            ? SkelComp->GetComponentTransform().TransformPosition(LocalPos)
            : LocalPos);
  }
  return Points;
}

TArray<FVector> UOHCollisionUtils::GetSplineMeshComponentVertices(
    USplineMeshComponent *SplineComp, bool bWorldSpace) {
  TArray<FVector> Points;
  if (!SplineComp)
    return Points;
  ...
  for (int32 i = 0; i <= NumSamples; ++i) {
    ...
    FVector Point =
        FMath::CubicInterp(StartPos, StartTangent, EndPos, EndTangent, T);
    Points.Add(
        bWorldSpace
            ? SplineComp->GetComponentTransform().TransformPosition(Point)
            : Point);
  }
  return Points;
}
...
TArray<FVector>
UOHCollisionUtils::GetMeshComponentVertices(UMeshComponent *MeshComp,
                                            bool bWorldSpace) {
...
void UOHCollisionUtils::GetMeshComponentAABB(UMeshComponent *MeshComp,
                                             FVector &OutCenter,
                                             FVector &OutExtents,
                                             bool bWorldSpace) {
...
void UOHCollisionUtils::GetMeshComponentOBB(UMeshComponent *MeshComp,
                                            FVector &OutCenter,
...
void UOHCollisionUtils::DebugDrawSATBox(
  ...
  // Bottom square
  DrawDebugLine(World, WorldCorners[0], WorldCorners[1], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[1], WorldCorners[2], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[2], WorldCorners[3], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[3], WorldCorners[0], Color, false, Duration,
                0, LineThickness);

  // Top square
  DrawDebugLine(World, WorldCorners[4], WorldCorners[5], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[5], WorldCorners[6], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[6], WorldCorners[7], Color, false, Duration,
                0, LineThickness);
  DrawDebugLine(World, WorldCorners[7], WorldCorners[4], Color, false, Duration,
                0, LineThickness);

  // Vertical edges
  for (int i = 0; i < 4; ++i) {
    DrawDebugLine(World, WorldCorners[i], WorldCorners[i + 4], Color, false,
                  Duration, 0, LineThickness);
  }
}

void UOHCollisionUtils::DebugDrawAABB(const UObject *WorldContextObject,
                                      const FVector &Center,
                                      const FVector &Extents,
  ...
  DrawDebugBox(World, Center, Extents, FQuat::Identity, Color, false, Duration,
               0, LineThickness);

  DrawDebugString(World, Center + FVector(0, 0, Extents.Z + 10.f), TEXT("AABB"),
                  nullptr, Color, Duration);
}

void UOHCollisionUtils::DrawDebugOBB(const UObject *WorldContextObject,
                                     const FVector &Center,
                                     const FRotator &Rotation,
                                     const FVector &Extents, const FColor Color,
                                     float Duration, float LineThickness) {
  if (!WorldContextObject) {
    return;
  }
...
void UOHCollisionUtils::DrawDebugSimplex(const UObject *WorldContextObject,
                                         const TArray<FVector> &SimplexPoints,
                                         const FColor Color, float Duration,
                                         float PointSize, float LineThickness) {
  ...
  for (int32 i = 0; i < SimplexPoints.Num(); ++i) {
    DrawDebugSphere(World, SimplexPoints[i], PointSize, 8, Color, false,
                    Duration);
    DrawDebugString(World, SimplexPoints[i] + FVector(0, 0, 10),
                    FString::Printf(TEXT("P%d"), i), nullptr, Color, Duration,
                    false);
  }

  // Draw lines (depends on simplex size)
  if (SimplexPoints.Num() == 2) {
    DrawDebugLine(World, SimplexPoints[0], SimplexPoints[1], Color, false,
                  Duration, 0, LineThickness);
  } else if (SimplexPoints.Num() == 3) {
    DrawDebugLine(World, SimplexPoints[0], SimplexPoints[1], Color, false,
                  Duration, 0, LineThickness);
    DrawDebugLine(World, SimplexPoints[1], SimplexPoints[2], Color, false,
                  Duration, 0, LineThickness);
    DrawDebugLine(World, SimplexPoints[2], SimplexPoints[0], Color, false,
                  Duration, 0, LineThickness);
  } else if (SimplexPoints.Num() == 4) {
    // Tetrahedron: draw all edges
    for (int i = 0; i < 4; ++i) {
      for (int j = i + 1; j < 4; ++j) {
        DrawDebugLine(World, SimplexPoints[i], SimplexPoints[j], Color, false,
                      Duration, 0, LineThickness);
      }
    }
  }
...
void UOHCollisionUtils::DrawDebugPointCloud(const UObject *WorldContextObject,
                                            const TArray<FVector> &Points,
                                            const FColor Color, float PointSize,
                                            float Duration) {
...
void UOHCollisionUtils::DrawDebugConvexHull(const UObject *WorldContextObject,
  ...
  for (int32 i = 0; i < Count; ++i) {
    ...
    DrawDebugString(World, HullPoints[i] + FVector(0, 0, PointSize * 1.5f),
                    FString::Printf(TEXT("H%d"), i), nullptr, Color, Duration);
  }
  ...
  if (Count == 4) {
    // Tetrahedron — 6 edges
    static const int32 Indices[][2] = {{0, 1}, {0, 2}, {0, 3},
                                       {1, 2}, {1, 3}, {2, 3}};

    for (const auto &Pair : Indices) {
      DrawDebugLine(World, HullPoints[Pair[0]], HullPoints[Pair[1]], Color,
                    false, Duration, 0, LineThickness);
    }
  } else if (Count <= 8) {
    // Small hull — draw full mesh
    for (int32 i = 0; i < Count; ++i) {
      for (int32 j = i + 1; j < Count; ++j) {
        DrawDebugLine(World, HullPoints[i], HullPoints[j], Color, false,
                      Duration, 0, LineThickness);
      }
    }
  }
...
void UOHCollisionUtils::DrawDebugConvexMeshFromFaces(
    const UObject *WorldContextObject, const TArray<FVector> &Vertices,
    const TArray<int32> &Indices, const FColor Color, float LineThickness,
    float Duration) {
  ...
  for (int32 i = 0; i + 2 < Indices.Num(); i += 3) {
    ...
    FVector Normal = FVector::CrossProduct(B - A, C - A).GetSafeNormal();
    DrawDebugLine(World, FaceCenter, FaceCenter + Normal * 10.0f, FColor::Cyan,
                  false, Duration, 0, 1.0f);
  }
}

void UOHCollisionUtils::ComputeConvexHullWithIndices(
    const TArray<FVector> &InputPoints, TArray<FVector> &OutVertices,
    ...
    for (int32 j = i + 1; j < Points.Num(); ++j) {
      for (int32 k = j + 1; k < Points.Num(); ++k) {
        ...
        for (int32 p = 0; p < Points.Num(); ++p) {
          ...
          const float Side =
              FVector::DotProduct(PlaneNormal, Points[p]) - PlaneD;
        ...
        TArray<int32>& OutTriangleIndices)
{
        ...
        for (int32 i = 0; i < InputPoints.Num(); ++i)
        {
                ...
                FVector AreaVec = FVector::CrossProduct(B - A, InputPoints[i] -
A); float Area = AreaVec.Size(); if (Area > MaxArea)
                {
                        MaxArea = Area;
                        MaxAreaIdx = i;
                }
        ...
        for (int32 i = 0; i < InputPoints.Num(); ++i)
        {
                ...
                float Dist = FMath::Abs(FVector::DotProduct(InputPoints[i] - A,
Normal)); if (Dist > MaxDist)
                {
                        MaxDist = Dist;
                        MaxDepthIdx = i;
                }
...
*/
void UOHCollisionUtils::DebugConvexIntersectionCheck(
    const UObject *WorldContextObject, const TArray<FVector> &PointsA,
    const TArray<FVector> &PointsB, const FColor ColorA, const FColor ColorB,
    float Duration) {
  ...
  bool bEPA = false;
  if (bOverlaps) {
    // Try EPA using last simplex from GJK (or fall back to best guess
    // tetrahedron) For now, just use first 4 from hull
  ...
  if (bEPA && Depth > 0.f) {
    ...
    DrawDebugLine(World, Midpoint, Midpoint + Normal * Depth, FColor::Red,
                  false, Duration, 0, 3.0f);
    DrawDebugString(World, Midpoint + Normal * Depth * 0.5f,
                    FString::Printf(TEXT("Penetration: %.2f"), Depth), nullptr,
                    FColor::Red, Duration);
  } else {
    DrawDebugString(World, (HullA[0] + HullB[0]) * 0.5f,
                    bOverlaps ? TEXT("Overlap Detected") : TEXT("No Overlap"),
                    nullptr, bOverlaps ? FColor::Green : FColor::White,
                    Duration);
  }
}

void UOHCollisionUtils::DrawConvexHullFromMotionHistory(
    const UObject *WorldContextObject, const TArray<FVector> &HistoryPoints,
  ...
  DrawDebugConvexMeshFromFaces(WorldContextObject, Hull, Indices, HullColor,
                               1.5f, Duration);
}

void UOHCollisionUtils::DrawDebugStrikeArc(
  ...
  const FVector ForwardFlat =
      FVector(Forward.X, Forward.Y, 0.f).GetSafeNormal();
  const float HalfAngleRad = FMath::DegreesToRadians(ArcAngleDegrees * 0.5f);
  const float AngleStep = (2.f * HalfAngleRad) / Segments;

  // Start direction
  const FRotator StartRot =
      ForwardFlat.Rotation().Add(0.f, -ArcAngleDegrees * 0.5f, 0.f);
  FVector LastPoint = Origin + StartRot.Vector() * Radius;

  for (int32 i = 1; i <= Segments; ++i) {
    const float Angle = -HalfAngleRad + i * AngleStep;
    const FVector Dir = ForwardFlat.RotateAngleAxis(
        FMath::RadiansToDegrees(Angle), FVector::UpVector);
    const FVector Point = Origin + Dir * Radius;

    DrawDebugLine(World, LastPoint, Point, Color, bPersistentLines, Duration, 0,
                  1.5f);
    LastPoint = Point;
  }

  // Optional center line
  DrawDebugLine(World, Origin, Origin + ForwardFlat * Radius, Color,
                bPersistentLines, Duration, 0, 2.0f);
}

void UOHCollisionUtils::DrawDebugCone(const UObject *WorldContextObject,
                                      const FVector &Origin,
                                      const FVector &Direction, float Length,
                                      float ConeAngleDegrees,
                                      const FColor Color, int32 Segments,
                                      float Duration, bool bPersistent) {
  if (!WorldContextObject) {
    return;
  }
  UWorld *World = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
  ...
  for (int32 i = 0; i < Segments; ++i) {
    ...
    FVector LocalDir = FMath::Cos(AngleRad) * Forward +
                       FMath::Sin(AngleRad) *
                           (FMath::Cos(Theta) * Right + FMath::Sin(Theta) * Up);
...
void UOHCollisionUtils::DrawDebugTrajectory(const UObject *WorldContextObject,
                                            const FVector &Start,
                                            const FVector &Velocity,
                                            float StepSize, int32 MaxSteps,
                                            const FColor Color, float Duration,
                                            bool bPersistent, FVector Gravity) {
  if (!WorldContextObject) {
    return;
  }
  UWorld *World = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
...
void UOHCollisionUtils::DrawDebugConvexSweep(
    const UObject *WorldContextObject, const TArray<FVector> &ShapePoints,
  ...
  DrawDebugConvexHull(WorldContextObject, ShapePoints, StartColor, 6.f, 1.0f,
                      Duration, true);
  DrawDebugConvexHull(WorldContextObject, OffsetPoints, EndColor, 6.f, 1.0f,
                      Duration, true);
}

void UOHCollisionUtils::DrawDebugConvexFaceNormals(
    const UObject *WorldContextObject, const TArray<FVector> &Vertices,
  ...
  for (int32 i = 0; i < Indices.Num(); i += 3) {
    ...
    DrawDebugLine(World, FaceCenter, FaceCenter + Normal * NormalLength, Color,
...
void UOHCollisionUtils::GenerateConvexHullFromBoxBounds(
    const UObject *WorldContextObject, const AActor *SourceActor,
    const FColor HullColor, float Duration) {
...
void UOHCollisionUtils::GenerateConvexHullFromSockets(
    const UObject *WorldContextObject, USkeletalMeshComponent *MeshComp,
    const TArray<FName> &SocketNames, const FColor HullColor, float Duration) {
...
void UOHCollisionUtils::GenerateConvexHullFromImpactPoints(
    const UObject *WorldContextObject, const TArray<FVector> &ImpactPoints,
    const FColor HullColor, float Duration) {
...
void UOHCollisionUtils::GenerateConvexHullFromMotionTrail(
    const UObject *WorldContextObject, USceneComponent *TargetComponent,
    int32 NumSamples, float SimulatedDuration, const FColor HullColor,
    float Duration) {
  ...
  TArray<FVector> TrailPoints =
      SampleMotionPoints(TargetComponent, NumSamples, SimulatedDuration);
...
void UOHCollisionUtils::GenerateConvexHullFromBoneCluster(
    const UObject *WorldContextObject, USkeletalMeshComponent *MeshComp,
    const TArray<FName> &BoneOrSocketNames, const FColor HullColor,
    float Duration) {
...
void UOHCollisionUtils::GenerateConvexHullFromBodyPartChain(
    const UObject *WorldContextObject, USkeletalMeshComponent *MeshComp,
    EOHBodyPart BodyPart, const FColor HullColor, float Duration) {
  if (!WorldContextObject || !MeshComp) {
    return;
  }

  const EOHSkeletalBone RootBone =
      UOHSkeletalPhysicsUtils::GetRootBoneInBodyPartFromMesh(MeshComp,
                                                             BodyPart);
  const EOHSkeletalBone EndBone =
      UOHSkeletalPhysicsUtils::GetEndBoneInBodyPartFromMesh(MeshComp, BodyPart);
...
void UOHCollisionUtils::RunStrikePredictionDebug(
    const UObject *WorldContextObject, AActor *Attacker, AActor *Target,
  ...
  FVector TargetFuture =
      Target->GetActorLocation() + Target->GetVelocity() * PredictTime;

  DrawDebugStrikeArc(WorldContextObject, Origin, Forward, Radius, ArcAngle,
                     FColor::Red, 2.0f);

  DrawDebugSphere(GEngine->GetWorldFromContextObjectChecked(WorldContextObject),
                  TargetFuture, 10.f, 12, FColor::Green, false, 2.0f);

  bool bInArc =
      IsTargetWithinStrikeArc(Origin, Forward, TargetFuture, ArcAngle, Radius);

  DrawDebugString(GEngine->GetWorldFromContextObjectChecked(WorldContextObject),
                  TargetFuture + FVector(0, 0, 20),
                  bInArc ? TEXT("HIT") : TEXT("MISS"), nullptr,
                  bInArc ? FColor::Green : FColor::White, 2.0f);
}

void UOHCollisionUtils::RunStrikePredictionLive(
    const UObject *WorldContextObject, AActor *Attacker, AActor *Target,
  ...
  const FVector TargetFuture =
      Target->GetActorLocation() + TargetVel * PredictTime;

  if (bShowTrajectory) {
    DrawDebugTrajectory(WorldContextObject, Target->GetActorLocation(),
                        TargetVel, 0.05f, 10, FColor::Blue, 1.5f);
  }
...
bool UOHCollisionUtils::ShouldDodgePredictedStrike(AActor *Attacker,
                                                   AActor *SelfActor,
  ...
  const FVector Origin = Attacker->GetActorLocation();
  const FVector Forward = Attacker->GetActorForwardVector();
  const FVector FuturePos =
      SelfActor->GetActorLocation() + SelfActor->GetVelocity() * PredictTime;

  return IsTargetWithinStrikeArc(Origin, Forward, FuturePos, ArcAngleDegrees,
                                 Radius);
}
...
TArray<FVector>
UOHCollisionUtils::SampleMotionPoints(USceneComponent *TargetComponent,
                                      int32 NumSamples, float DurationSeconds) {
  ...
  const FVector End = Start + TargetComponent->GetForwardVector() *
                                  100.f; // simulate forward arc
...
void UOHCollisionUtils::UpdateMotionHistory(TArray<FVector> &HistoryBuffer,
                                            const FVector &NewSample,
                                            float MaxDistance,
                                            int32 MaxSamples) {
  if (HistoryBuffer.Num() > 0) {
    if (FVector::DistSquared(HistoryBuffer.Last(), NewSample) <
        FMath::Square(1.f)) {
  ...
  if (HistoryBuffer.Num() >= 2) {
    while (FVector::Dist(HistoryBuffer[0], HistoryBuffer.Last()) >
               MaxDistance &&
           HistoryBuffer.Num() > 2) {
      HistoryBuffer.RemoveAt(0);
    }
...
float UOHCollisionUtils::ComputeContactConfidence(const FHitResult &Hit,
                                                  const FOHBoneData &BoneData) {
  if (!Hit.bBlockingHit || !BoneData.IsValid())
    return 0.f;

  const FVector ToImpact =
...
bool UOHCollisionUtils::GetDirectionToClosestCharacter(
    AActor *OriginActor, FVector &OutDirection,
  ...
  UGameplayStatics::GetAllActorsOfClass(OriginActor->GetWorld(),
                                        ACharacter::StaticClass(), FoundActors);
  ...
  for (AActor *Actor : FoundActors) {
    ...
    float DistSq =
        FVector::DistSquared(OriginLocation, Character->GetActorLocation());
...
TArray<ACharacter *>
UOHCollisionUtils::GetCharactersInRange(AActor *OriginActor, float Radius) {
...
bool UOHCollisionUtils::GetClosestCharacterInRange(
    AActor *OriginActor, float Radius, ACharacter *&OutClosestCharacter) {
  OutClosestCharacter = nullptr;
  ...
  for (AActor *Actor : FoundActors) {
    ...
    if (DistSq <= ClosestDistSq) {
      if (!OutClosestCharacter ||
          DistSq <
              FVector::DistSquared(OriginLocation,
                                   OutClosestCharacter->GetActorLocation())) {
        OutClosestCharacter = Character;
...
bool UOHCollisionUtils::GetClosestCharacterInTraceRadius(
    AActor *OriginActor, float Radius, ACharacter *&OutClosestCharacter,
  ...
  bool bHasHit = World->OverlapMultiByChannel(Overlaps, Origin, FQuat::Identity,
                                              TraceChannel, Sphere, Params);
  ...
  for (const FOverlapResult &Result : Overlaps) {
    ...
    const float DistSq =
        FVector::DistSquared(Origin, Character->GetActorLocation());
    if (DistSq <= ClosestDistSq) {
      OutClosestCharacter = Character;
      ClosestDistSq = DistSq;
    }
...
bool UOHCollisionUtils::GetDirectionToClosestCharacterInRadiusTrace(
    AActor *OriginActor, float Radius, FVector &OutDirection,
    ACharacter *&OutClosestCharacter, ECollisionChannel TraceChannel) {
  OutDirection = FVector::ZeroVector;
  OutClosestCharacter = nullptr;
  ...
  if (OutClosestCharacter) {
    OutDirection =
        (OutClosestCharacter->GetActorLocation() - Origin).GetSafeNormal();
    return true;
  }
...
bool UOHCollisionUtils::GetDirectionAndHitToClosestCharacterInRadiusTrace(
    AActor *OriginActor, float Radius, FVector &OutDirection,
    ACharacter *&OutClosestCharacter, FHitResult &OutHit,
    ECollisionChannel OverlapChannel, ECollisionChannel TraceChannel,
    bool bRequireLineOfSight, bool bDrawDebug, FColor DebugColor,
    float DebugDuration) {
  OutDirection = FVector::ZeroVector;
  OutClosestCharacter = nullptr;
  OutHit = FHitResult();
  ...
  const bool bHasOverlap = World->OverlapMultiByChannel(
      Overlaps, Origin, FQuat::Identity, OverlapChannel, Sphere, OverlapParams);

  if (bDrawDebug) {
    DrawDebugSphere(World, Origin, Radius, 16, DebugColor, false,
                    DebugDuration);
  }
  ...
  for (const FOverlapResult &Result : Overlaps) {
    ...
    if (DistSq < ClosestDistSq) {
      ...
      bool bHit = World->LineTraceSingleByChannel(
          TraceHit, Origin, TargetLocation, TraceChannel, TraceParams);

      const bool bLOSOK =
          !bRequireLineOfSight || (bHit && TraceHit.GetActor() == Candidate);

      if (bDrawDebug) {
        DrawDebugLine(World, Origin, TargetLocation,
                      bLOSOK ? DebugColor : FColor::Red, false, DebugDuration,
                      0, 1.5f);
      }
...
bool UOHCollisionUtils::GetDirectionToClosestCharacterUsingAABB(
    AActor *OriginActor, float Radius, FVector &OutDirection,
    ACharacter *&OutClosestCharacter) {
  OutDirection = FVector::ZeroVector;
  OutClosestCharacter = nullptr;
  ...
  FVector OriginExtents =
      FVector(Radius); // Create a cubic volume as AABB around origin

  TArray<AActor *> AllCharacters;
  UGameplayStatics::GetAllActorsOfClass(
      OriginActor->GetWorld(), ACharacter::StaticClass(), AllCharacters);
  ...
  OutDirection =
      (OutClosestCharacter->GetActorLocation() - OriginCenter).GetSafeNormal();
  return true;
}

bool UOHCollisionUtils::GetClosestCharacterUsingOBBIntersection(
    AActor *OriginActor, FVector Extents, FRotator Rotation,
    ACharacter *&OutClosestCharacter) {
  OutClosestCharacter = nullptr;
  if (!OriginActor) {
    return false;
  ...
  for (AActor *Actor : Found) {
    ...
    if (SAT_BoxIntersect(Origin, Rotation, Extents, TargetCenter,
                         TargetRotation, TargetExtents)) {
...
bool UOHCollisionUtils::GetClosestCharacterUsingGJKIntersection(
    const TArray<FVector> &AttackHull, AActor *OriginActor,
    ACharacter *&OutClosestCharacter) {
  OutClosestCharacter = nullptr;
  ...
  for (AActor *Actor : Found) {
    ...
    TArray<FVector> TargetHull = GenerateBoxHull(
        Character->GetMesh()->Bounds.Origin,
        Character->GetMesh()->Bounds.BoxExtent, Character->GetActorRotation());
...
bool UOHCollisionUtils::DetectTargetUsingAABB(AActor *OriginActor, float Radius,
                                              ACharacter *&OutClosestCharacter,
                                              FVector &OutDirection) {
  OutDirection = FVector::ZeroVector;
  OutClosestCharacter = nullptr;
  ...
  UGameplayStatics::GetAllActorsOfClass(OriginActor->GetWorld(),
                                        ACharacter::StaticClass(), AllChars);
  ...
  if (OutClosestCharacter) {
    OutDirection = (OutClosestCharacter->GetActorLocation() - OriginCenter)
                       .GetSafeNormal();
    return true;
  }
...
bool UOHCollisionUtils::DetectTargetUsingOBB(AActor *OriginActor,
  ...
  for (AActor *Actor : Found) {
    ...
    if (SAT_BoxIntersect(OriginCenter, OriginRotation, OriginExtents,
                         TargetCenter, TargetRotation, TargetExtents)) {
...
bool UOHCollisionUtils::DetectTargetUsingGJK(const TArray<FVector> &AttackHull,
                                             AActor *OriginActor,
                                             ACharacter *&OutClosestCharacter,
                                             FVector &OutDirection) {
  OutClosestCharacter = nullptr;
...
bool UOHCollisionUtils::DetectTargetWithEPA(const TArray<FVector> &AttackHull,
                                            AActor *OriginActor,
                                            ACharacter *&OutClosestCharacter,
                                            FVector &OutNormal,
  ...
  for (AActor *Actor : Found) {
    ...
    TArray<FVector> Simplex;
    if (GJK_Intersect(AttackHull, TargetHull) &&
        ProcessSimplex(Simplex, OutNormal)) {
      FVector Normal;
      float Depth = 0.f;
      if (EPA_PenetrationDepth(AttackHull, TargetHull, Simplex, Normal,
                               Depth)) {
        float DistSq =
            FVector::DistSquared(Origin, Character->GetActorLocation());
        if (DistSq < ClosestDistSq) {
...
bool UOHCollisionUtils::DetectTargetFromTrailHull(
    USceneComponent *TrailComponent, float Duration, int32 NumSamples,
  ...
  for (int32 i = 0; i < NumSamples; ++i) {
    ...
    FVector SamplePos =
        TrailComponent
            ->GetComponentLocation(); // In real code: cache transform history
  ...
  return DetectTargetUsingGJK(AttackHull, OriginActor, OutClosestCharacter,
                              OutDirection);
}
...
bool UOHCollisionUtils::ComputeAABBSeparationImpulse(const FVector &CenterA,
                                                     const FVector &ExtentsA,
                                                     const FVector &CenterB,
                                                     const FVector &ExtentsB,
                                                     FVector &OutImpulse) {
...
bool UOHCollisionUtils::ComputeEPAPenetrationCorrection(
    const TArray<FVector> &ShapeA, const TArray<FVector> &ShapeB,
  ...
  return EPA_PenetrationDepth(ShapeA, ShapeB, Simplex, OutCorrectionNormal,
                              OutPenetrationDepth, 30, 0.001f);
}

void UOHCollisionUtils::ApplyImpulseToBone(USkeletalMeshComponent *Mesh,
                                           FName BoneName,
...
void UOHCollisionUtils::ApplySafeAngularTorque(USkeletalMeshComponent *Mesh,
                                               FName BoneName,
...
void UOHCollisionUtils::ApplySafeSpringForce(USkeletalMeshComponent *Mesh,
                                             FName BoneName,
...
void UOHCollisionUtils::ApplyLookAtTorque(USkeletalMeshComponent *Mesh,
                                          FName BoneName,
  ...
  const FVector ToTarget =
      (TargetLocation - BodyTransform.GetLocation()).GetSafeNormal();
...
void UOHCollisionUtils::AdjustPhysicalStiffness(
    UPhysicalAnimationComponent *PhysAnim, FName BoneName, float HitMagnitude,
    float MaxExpectedForce, float MinStiffness, float MaxStiffness) {
  if (!PhysAnim) {
    return;
  }
...
FVector UOHCollisionUtils::PredictFuturePosition(const FVector &StartPosition,
                                                 const FVector &Velocity,
                                                 float Time) {
  return StartPosition + Velocity * Time;
}

float UOHCollisionUtils::PredictTimeToCollision(const FVector &PositionA,
...
bool UOHCollisionUtils::PredictAABBOverlap(
    const FVector &CenterA, const FVector &ExtentsA, const FVector &VelocityA,
...
FVector UOHCollisionUtils::ComputeInterceptPoint(
    const FVector &ChaserPosition, float ChaserSpeed,
...
bool UOHCollisionUtils::PredictConvexSweepOverlap(const TArray<FVector> &ShapeA,
                                                  const FVector &VelocityA,
                                                  const TArray<FVector> &ShapeB,
                                                  float DeltaTime) {
  if (ShapeA.Num() == 0 || ShapeB.Num() == 0) {
...
FVector UOHCollisionUtils::ComputeLeadAimDirection(
    const FVector &Origin, const FVector &TargetLocation,
...
bool UOHCollisionUtils::IsTargetWithinCone(const FVector &Origin,
                                           const FVector &Direction,
                                           const FVector &TargetLocation,
                                           float ConeAngleDegrees,
                                           float MaxDistance) {
  ...
  const float CosAngle =
      FMath::Cos(FMath::DegreesToRadians(ConeAngleDegrees * 0.5f));
  const float Dot =
      FVector::DotProduct(ToTarget.GetSafeNormal(), Direction.GetSafeNormal());

  return Dot >= CosAngle;
}

bool UOHCollisionUtils::IsTargetWithinStrikeArc(const FVector &Origin,
  ...
  const float Dot = FVector::DotProduct(FlatForward.GetSafeNormal(),
                                        FlatToTarget.GetSafeNormal());
...
FStrikePredictionResult
UOHCollisionUtils::EvaluateStrikePrediction(AActor *Attacker, AActor *Target,
                                            float Radius, float ArcAngleDegrees,
                                            float PredictTime) {
  ...
  const FVector FutureTarget =
      Target->GetActorLocation() + VelocityTarget * PredictTime;

  const float TimeToHit = PredictTime;
  const float Distance = FVector::Dist(Origin, FutureTarget);
  const bool bHit = IsTargetWithinStrikeArc(Origin, Forward, FutureTarget,
                                            ArcAngleDegrees, Radius);
...
bool UOHCollisionUtils::SweepBoneChainWithMetrics(
    USkeletalMeshComponent *MeshComp, const TArray<FName> &BoneNames,
  ...
  TArray<AActor *> Found;
  UGameplayStatics::GetAllActorsOfClass(SelfActor->GetWorld(),
                                        ACharacter::StaticClass(), Found);
  ...
  for (AActor *Actor : Found) {
    ...
    TArray<FVector> TargetHull = GenerateBoxHull(
        Bounds.Origin, Bounds.BoxExtent, Target->GetActorRotation());
    ...
    if (GJK_Intersect(ConvexHull, TargetHull)) {
      ...
      if (bUseEPA) {
        ...
        if (ProcessSimplex(Simplex, ContactNormal)) {
          EPA_PenetrationDepth(ConvexHull, TargetHull, Simplex, ContactNormal,
                               PenetrationDepth);
        }
      }
      ...
      const FVector BoneVelocity = UOHCombatUtils::GetBoneVelocitySafe(
          MeshComp, StrikingBone, DeltaTime);
      ...
      const float Dot = FVector::DotProduct(Metrics.ContactNormal,
                                            BoneVelocity.GetSafeNormal());
      Metrics.VelocityDotNormal = Dot;
      Metrics.ImpactAngleDegrees =
          FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(Dot, -1.f, 1.f)));
      Metrics.EstimatedForce =
          BoneVelocity.Size() * PenetrationDepth * FMath::Abs(Dot);
