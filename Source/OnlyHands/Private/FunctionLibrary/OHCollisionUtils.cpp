...
bool UOHCollisionUtils::AABB_Overlap(
...
FVector UOHCollisionUtils::ClosestPointOnAABB(
	const FVector& Point,
...
FVector UOHCollisionUtils::SupportAABB(
...
bool UOHCollisionUtils::SAT_BoxIntersect(
	...
	const FVector& CenterB, const FRotator& RotationB, const FVector& ExtentsB)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			...
			float ra =
				ExtentsA.X * FMath::Abs(FVector::DotProduct(Axis, AxesA[0])) +
				ExtentsA.Y * FMath::Abs(FVector::DotProduct(Axis, AxesA[1])) +
				ExtentsA.Z * FMath::Abs(FVector::DotProduct(Axis, AxesA[2]));

			float rb =
				ExtentsB.X * FMath::Abs(FVector::DotProduct(Axis, AxesB[0])) +
...
#pragma region OBB
bool UOHCollisionUtils::IsPointInOBB(
	const FVector& Point,
	const FVector& BoxCenter,
...
FVector UOHCollisionUtils::SupportOBB(
	const FVector& Direction,
	const FVector& BoxCenter,
	const FRotator& BoxRotation,
	const FVector& BoxExtents)
{
	const FVector Axes[3] = {
		BoxRotation.RotateVector(FVector::ForwardVector),
...
FVector UOHCollisionUtils::ClosestPointOnOBB(
	const FVector& Point,
	const FVector& BoxCenter,
	const FRotator& BoxRotation,
	const FVector& BoxExtents)
...
TArray<FVector> UOHCollisionUtils::ComputeMinkowskiSum(
...
TArray<FVector> UOHCollisionUtils::ComputeMinkowskiDifference(
	const TArray<FVector>& ShapeA,
	const TArray<FVector>& ShapeB)
{
	TArray<FVector> Result;
...
bool UOHCollisionUtils::GJK_Intersect(
	const TArray<FVector>& ShapeA,
	const TArray<FVector>& ShapeB,
	int32 MaxIterations,
	float Tolerance)
...
FVector UOHCollisionUtils::GetFarthestPointInDirection(
...
FVector UOHCollisionUtils::TripleCrossProduct(
	const FVector& A,
	const FVector& B,
	const FVector& C)
{
...
TArray<FVector> UOHCollisionUtils::GenerateBoxHull(
	const FVector& Center,
	const FVector& Extents,
...
TArray<FVector> UOHCollisionUtils::GenerateSphereHull(
	const FVector& Center,
	float Radius,
	int32 Subdivisions)
{
...
FVector UOHCollisionUtils::ComputeAverageContactPoint(
...
TArray<FVector> UOHCollisionUtils::GetSkeletalMeshComponentVertices(USkeletalMeshComponent* SkelComp, bool bWorldSpace)
{
    TArray<FVector> Points;
    if (!SkelComp) return Points;

    USkeletalMesh* SkelMesh = SkelComp->GetSkeletalMeshAsset();
    if (!SkelMesh) return Points;
...
void UOHCollisionUtils::GetMeshComponentAABB(
...
void UOHCollisionUtils::GetMeshComponentOBB(
	UMeshComponent* MeshComp,
	FVector& OutCenter,
...
void UOHCollisionUtils::DebugDrawAABB(
	const UObject* WorldContextObject,
	const FVector& Center,
	const FVector& Extents,
	const FColor Color,
...
void UOHCollisionUtils::DrawDebugOBB(
	const UObject* WorldContextObject,
	const FVector& Center,
	const FRotator& Rotation,
	const FVector& Extents,
...
void UOHCollisionUtils::DrawDebugSimplex(
	const UObject* WorldContextObject,
...
void UOHCollisionUtils::DrawDebugPointCloud(
	const UObject* WorldContextObject,
	const TArray<FVector>& Points,
	const FColor Color,
	float PointSize,
...
void UOHCollisionUtils::DrawDebugConvexHull(
	const UObject* WorldContextObject,
...
void UOHCollisionUtils::DrawDebugCone(
	const UObject* WorldContextObject,
	const FVector& Origin,
	const FVector& Direction,
	float Length,
...
void UOHCollisionUtils::DrawDebugTrajectory(
	const UObject* WorldContextObject,
	const FVector& Start,
	...
	float PredictTime)
{
	...
	DrawDebugString(
		...
		bInArc ? TEXT("HIT") : TEXT("MISS"),
		nullptr, bInArc ? FColor::Green : FColor::White,
		2.0f);
}
...
bool UOHCollisionUtils::ShouldDodgePredictedStrike(
	AActor* Attacker,
	AActor* SelfActor,
	float ArcAngleDegrees,
	float Radius,
...
TArray<FVector> UOHCollisionUtils::SampleMotionPoints(
	USceneComponent* TargetComponent,
	int32 NumSamples,
...
void UOHCollisionUtils::UpdateMotionHistory(
...
bool UOHCollisionUtils::GetClosestCharacterInRange(AActor* OriginActor, float Radius, ACharacter*& OutClosestCharacter)
{
	OutClosestCharacter = nullptr;
	...
	for (AActor* Actor : FoundActors)
	{
		...
		if (DistSq <= ClosestDistSq)
		{
			if (!OutClosestCharacter || DistSq < FVector::DistSquared(OriginLocation,
...
)
{
	...
	bool bHasHit = World->OverlapMultiByChannel(
		Overlaps,
		Origin,
		FQuat::Identity,
		TraceChannel,
                                                                ...
                                                                ACharacter*& OutClosestCharacter)
{
	...
	for (AActor* Actor : Found)
	{
		...
		TArray<FVector> TargetHull = GenerateBoxHull(
			...
			Character->GetMesh()->Bounds.BoxExtent,
...
bool UOHCollisionUtils::DetectTargetUsingAABB(
	AActor* OriginActor,
	float Radius,
	ACharacter*& OutClosestCharacter,
	FVector& OutDirection
...
bool UOHCollisionUtils::DetectTargetUsingOBB(
	AActor* OriginActor,
...
)
{
	...
	for (AActor* Actor : Found)
	{
		...
		if (SAT_BoxIntersect(OriginCenter, OriginRotation, OriginExtents, TargetCenter, TargetRotation, TargetExtents))
		{
...
bool UOHCollisionUtils::DetectTargetUsingGJK(
...
bool UOHCollisionUtils::DetectTargetWithEPA(
	const TArray<FVector>& AttackHull,
	AActor* OriginActor,
	ACharacter*& OutClosestCharacter,
	FVector& OutNormal,
...
bool UOHCollisionUtils::ComputeAABBSeparationImpulse(
...
void UOHCollisionUtils::ApplyImpulseToBone(
...
void UOHCollisionUtils::ApplySafeAngularTorque(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
...
void UOHCollisionUtils::ApplySafeSpringForce(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
...
void UOHCollisionUtils::ApplyLookAtTorque(
	USkeletalMeshComponent* Mesh,
	FName BoneName,
...
FVector UOHCollisionUtils::PredictFuturePosition(
...
float UOHCollisionUtils::PredictTimeToCollision(
...
bool UOHCollisionUtils::PredictConvexSweepOverlap(
	const TArray<FVector>& ShapeA,
	const FVector& VelocityA,
	const TArray<FVector>& ShapeB,
	float DeltaTime)
...
bool UOHCollisionUtils::IsTargetWithinCone(
	const FVector& Origin,
	const FVector& Direction,
	const FVector& TargetLocation,
	float ConeAngleDegrees,
...
bool UOHCollisionUtils::IsTargetWithinStrikeArc(
	const FVector& Origin,
	const FVector& Forward,
	const FVector& TargetLocation,
	float ArcAngleDegrees,
...
FStrikePredictionResult UOHCollisionUtils::EvaluateStrikePrediction(
	AActor* Attacker,
	AActor* Target,
	float Radius,
	float ArcAngleDegrees,
...
void UOHCollisionUtils::ResolveCharacterCollision(
    ...
    float PushStrength)
{
    ...
    FVector Response1 = CalculateElasticCollisionResponse(
        Vel1, Vel2, Mass1, Mass2, CollisionNormal, 0.3f
    );
    
    FVector Response2 = CalculateElasticCollisionResponse(
        Vel2, Vel1, Mass2, Mass1, -CollisionNormal, 0.3f
    );
