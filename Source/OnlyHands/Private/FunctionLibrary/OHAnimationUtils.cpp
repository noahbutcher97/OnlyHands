	...
	const TArray<FName>& BoneChain)
{
	...
	for (int32 i = 1; i < BoneChain.Num(); ++i)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Bone %s or %s not found in skeleton!"),
				   *BoneChain[i - 1].ToString(), *BoneChain[i].ToString());
			return 0.0f;
		}
...
void UOHAnimationUtils::DrawPoseDeviation(UOHPhysicsManager* Manager, FName BoneName, FLinearColor Color, float Thickness)
{
	if (!Manager) return;
...
TArray<FName> UOHAnimationUtils::GetBonesWithHighDeviation(UOHPhysicsManager* Manager, float PositionThreshold, float RotationThreshold)
{
	TArray<FName> Result;
