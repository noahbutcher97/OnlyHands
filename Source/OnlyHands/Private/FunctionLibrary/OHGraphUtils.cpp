...
bool UOHGraphUtils::IsBoneValid(const FOHBoneData &Bone,
...
bool UOHGraphUtils::AddBoneIfNoConflict(TMap<FName, FOHBoneData> &BoneMap,
  ...
  return OHSafeMapUtils::TryInsertIfNoConflict<FName, FOHBoneData>(
      BoneMap, Name, BoneData,
      [](const FOHBoneData &Existing, const FOHBoneData &Incoming) {
        // Conflict is defined as mismatched simulation status
        const bool bMismatchSim =
            Existing.GetHasSimulatedBody() != Incoming.GetHasSimulatedBody();

        // Or: meaningful mass/length discrepancy
        const bool bMassDiff = !FMath::IsNearlyEqual(
            Existing.GetCachedBodyMass(), Incoming.GetCachedBodyMass(), 0.01f);
        const bool bLengthDiff =
            !FMath::IsNearlyEqual(Existing.GetCachedBoneLength(),
                                  Incoming.GetCachedBoneLength(), 0.01f);
...
int32 UOHGraphUtils::InsertAllBonesIfValid(
  ...
  return OHSafeMapUtils::InsertAllValidated<FName, FOHBoneData>(
      TargetMap, SourceMap, [Strictness](const FOHBoneData &Bone) {
...
int32 UOHGraphUtils::InsertAllConstraintsIfValid(
  ...
  for (const FOHConstraintInstanceData &Constraint : SourceList) {
    ...
    const bool bParentValid =
        BoneMap.Contains(Parent) && BoneMap[Parent].GetHasSimulatedBody();
    const bool bChildValid =
        BoneMap.Contains(Child) && BoneMap[Child].GetHasSimulatedBody();
...
TArray<FName>
UOHGraphUtils::GetChildBones(const TMap<FName, FOHBoneData> &BoneMap,
                             FName ParentBone) {
...
TArray<FOHConstraintInstanceData> UOHGraphUtils::GetConstraintsForBone(
  ...
  for (const auto &Constraint : ConstraintList) {
    if (Constraint.GetParentBone() == BoneName ||
...
TArray<FName>
UOHGraphUtils::GetSimulatedBones(const TMap<FName, FOHBoneData> &BoneMap) {
  TArray<FName> Result;
...
void UOHGraphUtils::RemoveInvalidConstraints(
  ...
  ConstraintList.RemoveAll([&](const FOHConstraintInstanceData &C) {
    const bool bValidParent = BoneMap.Contains(C.GetParentBone()) &&
                              BoneMap[C.GetParentBone()].GetHasSimulatedBody();
    const bool bValidChild = BoneMap.Contains(C.GetChildBone()) &&
                             BoneMap[C.GetChildBone()].GetHasSimulatedBody();
...
void UOHGraphUtils::NormalizeBoneMasses(TMap<FName, FOHBoneData> &BoneMap) {
  OHSafeMapUtils::NormalizeMapValues<FName, FOHBoneData>(
      BoneMap, [](const FOHBoneData &Bone) { return Bone.GetCachedBodyMass(); },
      [](FOHBoneData &Bone, float Normalized) {
        Bone.SetCachedBodyMass(Normalized);
      });
}

float UOHGraphUtils::ComputeBoneMapEntropy(
    const TMap<FName, FOHBoneData> &BoneMap) {
  return OHSafeMapUtils::EntropyOfMapKeys(BoneMap);
}

void UOHGraphUtils::RemoveBoneMassOutliers(TMap<FName, FOHBoneData> &BoneMap,
                                           float ThresholdZ) {
...
void UOHGraphUtils::LogGraphStats(
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[%s] BoneCount: %d, Simulated: %d, Constraints: %d"),
         *ContextTag, BoneCount, SimulatedCount, ConstraintCount);
}

bool UOHGraphUtils::ValidateAllChainsStable(
    const TMap<FName, FOHBoneData> &BoneMap,
    const TArray<FOHConstraintInstanceData> &Constraints, int32 MaxDepth) {
  auto ChildMap =
      OHSafeMapUtils::BuildParentToChildrenMap<FOHConstraintInstanceData>(
          Constraints,
  ...
  Recurse = [&](const FName &Bone, int32 Depth) -> bool {
    ...
    if (!BoneData || !BoneData->GetIsSimulating())
      return false;
...
bool UOHGraphUtils::BuildPhysicsGraphFromComponent(
    ...
    const FString &ContextTag, bool bRunAudit) {
  if (!SkeletalMeshComp) {
    UE_LOG(LogTemp, Error, TEXT("[%s] SkeletalMeshComponent is null"),
           *ContextTag);
    return false;
  }
  ...
  if (!PhysAsset) {
    UE_LOG(LogTemp, Error,
           TEXT("[%s] SkeletalMeshComponent has no PhysicsAsset"), *ContextTag);
    return false;
  }

  const USkeletalMesh *SkeletalMesh = SkeletalMeshComp->GetSkeletalMeshAsset();
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Error, TEXT("[%s] SkeletalMesh asset is null"),
           *ContextTag);
    return false;
  }
  ...
  for (const FBodyInstance *BI : SkeletalMeshComp->Bodies) {
    ...
    const FName BoneName =
        BI->BodySetup.IsValid() ? BI->BodySetup->BoneName : NAME_None;
  ...
  for (const UPhysicsConstraintTemplate *Template :
    ...
    FConstraintInstance **FoundPtr =
        SkeletalMeshComp->Constraints.FindByPredicate(
    ...
    Link.SetConstraintTemplate(
        const_cast<UPhysicsConstraintTemplate *>(Template));
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[%s] Built physics graph: %d bones, %d constraints"),
...
bool UOHGraphUtils::RefreshBonesOnlyFromComponent(
  ...
  for (auto &[BoneName, Bone] : BoneMap) {
    ...
    if (!BI || !BI->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Warning, TEXT("[%s] Missing or invalid body: %s"),
             *ContextTag, *BoneName.ToString());
    ...
    float Length = 0.f;
    if (int32 BoneIndex = RefSkeleton.FindBoneIndex(BoneName);
        BoneIndex != INDEX_NONE && BoneIndex != 0) {
      const FTransform ParentRef =
          RefSkeleton.GetRefBonePose()[RefSkeleton.GetParentIndex(BoneIndex)];
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[%s] Bone refresh complete: %d refreshed, %d missing"),
         *ContextTag, Refreshed, Missing);
  return true;
}
...
bool UOHGraphUtils::RefreshConstraintsOnlyFromComponent(
  ...
  if (bRebuildFullConstraintList) {
    ...
    if (!PhysAsset) {
      UE_LOG(LogTemp, Error,
             TEXT("[%s] No PhysicsAsset on SkeletalMeshComponent"),
             *ContextTag);
      return false;
    }
    ...
    for (const UPhysicsConstraintTemplate *Template :
      ...
      FConstraintInstance *FoundCI =
          SkeletalMeshComp->FindConstraintInstance(Entry.GetConstraintName());
      Entry.SetConstraintInstance(FoundCI);
      Entry.SetConstraintTemplate(
          const_cast<UPhysicsConstraintTemplate *>(Template));
    ...
    for (FOHConstraintInstanceData &Entry : Constraints) {
      ...
      if (FoundCI) {
        ...
        UE_LOG(LogTemp, Warning,
               TEXT("[%s] Could not find constraint instance for '%s'"),
...
bool UOHGraphUtils::RefreshGraphFromComponent(
  ...
  if (!SkeletalMeshComp) {
    UE_LOG(LogTemp, Error,
           TEXT("[%s] RefreshGraph failed: SkeletalMeshComponent is null"),
           *ContextTag);
    return false;
  }
  ...
  if (bRefreshBones) {
    const bool bBoneResult = RefreshBonesOnlyFromComponent(
        SkeletalMeshComp, Graph, ContextTag + TEXT("_Bones"),
  ...
  if (bRefreshConstraints) {
    const bool bConstraintResult = RefreshConstraintsOnlyFromComponent(
        SkeletalMeshComp, Graph, ContextTag + TEXT("_Constraints"),
        bFullConstraintRebuild, bRebindOwnerComponent);

    if (!bConstraintResult) {
      UE_LOG(LogTemp, Warning, TEXT("[%s] Constraint refresh failed"),
             *ContextTag);
      bSuccess = false;
    }
  }
...
void UOHGraphUtils::ResetGraph(FOHPhysicsGraphNode &Graph) {
  UE_LOG(LogTemp, Verbose,
         TEXT("ResetGraph: Clearing %d bones, %d constraints"),
         Graph.GetBoneMap().Num(), Graph.GetConstraintLinks().Num());
  Graph.Reset();
}

bool UOHGraphUtils::SafeRebuildGraph(const USkeletalMeshComponent *SkelComp,
                                     FOHPhysicsGraphNode &Graph,
                                     bool bForceResetIfFailed) {
  if (!SkelComp)
    return false;

  // Build directly into the existing graph (the function resets it internally)
  if (BuildPhysicsGraphFromComponent(
          const_cast<USkeletalMeshComponent *>(SkelComp), Graph,
          TEXT("SafeRebuild"), true)) {
    return true;
  }
...
TSet<FName>
UOHGraphUtils::GetSimulatedBoneNames(const FOHPhysicsGraphNode &Graph) {
  TSet<FName> Result;
...
TArray<FName>
UOHGraphUtils::GetAllConstraintNames(const FOHPhysicsGraphNode &Graph) {
  TArray<FName> Result;
...
TSet<FName> UOHGraphUtils::GetAllDescendants(const FOHPhysicsGraphNode &Graph,
                                             FName RootBone) {
...
UOHGraphUtils::BuildAdjacencyMap(const FOHPhysicsGraphNode &Graph) {
  return OHSafeMapUtils::BuildBoneConstraintAdjacencyGraph<
...
TSet<FName> UOHGraphUtils::TraverseGraphBFS(const FOHPhysicsGraphNode &Graph,
                                            FName Start) {
  ...
  // Call traversal with start node, get neighbors, visitor, and max node limit
  // (512)
  OHSafeMapUtils::TraverseWithNodeLimit<FName>(Start, GetChildren, Visitor,
                                               512);
...
TSet<FName> UOHGraphUtils::TraverseGraphBFS(const FOHPhysicsGraphNode &Graph,
  ...
  return OHSafeMapUtils::TraverseWithNodeLimit<FName>(
      ...
      [](const FName &) {}, MaxNodesToVisit);
}

void UOHGraphUtils::TraverseGraphDFS(
    const FOHPhysicsGraphNode &Graph, FName Start,
...
TArray<TSet<FName>>
UOHGraphUtils::ExtractConnectedComponents(const FOHPhysicsGraphNode &Graph) {
...
TMap<FName, FString>
UOHGraphUtils::GetConstraintHealthReport(const FOHPhysicsGraphNode &Graph) {
  ...
  return OHSafeMapUtils::MapToReport<FOHConstraintInstanceData, FName>(
      ...
      [&](const FOHConstraintInstanceData &C) -> FString {
        ...
        float Distance = FVector::Dist(DataA->GetCurrentPosition(),
                                       DataB->GetCurrentPosition());
        ...
        return FString::Printf(TEXT("%sParent: %s | Child: %s | Ratio: %.2f | "
                                    "Dist: %.1f | Stiff: %.2f | Damp: %.2f"),
                               *Flags, *A.ToString(), *B.ToString(), Ratio,
                               Distance, C.GetConeLimitStiffness(),
                               C.GetConeLimitDamping());
      });
}
...
void UOHGraphUtils::PruneInvalidConstraints(FOHPhysicsGraphNode &Graph) {
  OHSafeMapUtils::RemoveIfAnyMissingOrInvalid<FName, FOHBoneData,
                                              FOHConstraintInstanceData>(
      Graph.GetConstraintLinks(), Graph.GetBoneMap(),
      [](const FOHConstraintInstanceData &C) { return C.GetParentBone(); },
      [](const FOHConstraintInstanceData &C) { return C.GetChildBone(); },
...
bool UOHGraphUtils::ValidateConstraintConnectivity(
...
bool UOHGraphUtils::HasMissingBodies(const FOHPhysicsGraphNode &Graph) {
  for (const auto &Pair : Graph.GetBoneMap()) {
    ...
    if (!Bone.GetBodyInstance() ||
        !Bone.GetBodyInstance()->IsValidBodyInstance()) {
      return true;
    }
  }
  return false;
...
void UOHGraphUtils::DrawPhysicsGraphOverlay(
    const FOHPhysicsGraphNode &Graph, const USkeletalMeshComponent *MeshComp,
  ...
  for (const auto &Pair : Graph.GetBoneMap()) {
    ...
    const FVector BoxExtent = FVector(5.f, 5.f, Length * 0.5f) * MeshScale *
                              FMath::LogX(2.0f, Mass + 2.0f);
    const FColor BoxColor =
        Bone.GetHasSimulatedBody() ? FColor::Green : FColor::Red;

    DrawDebugBox(World, Pos, BoxExtent, BoneTransform.GetRotation(), BoxColor,
                 false, Duration, 0, 0.5f);
  }
  ...
  for (const FOHConstraintInstanceData &Constraint :
    ...
    const float Influence =
        CalculateConstraintInfluenceScore(Constraint, Graph, MeshComp);
    ...
    DrawDebugSphere(World, Midpoint, 5.f, 12, SphereColor, false, Duration, 0,
                    1.0f);
  }
}
...
float UOHGraphUtils::CalculateConstraintInfluenceScore(
  ...
  const FVector Center = [&] {
    ...
    return (MeshComp->GetBoneTransform(P).GetLocation() +
            MeshComp->GetBoneTransform(C).GetLocation()) *
           0.5f;
  }();
  ...
  for (const FOHConstraintInstanceData &Other : Graph.GetConstraintLinks()) {
    ...
    if (OP == INDEX_NONE || OC == INDEX_NONE)
      continue;

    const FVector OtherMid = (MeshComp->GetBoneTransform(OP).GetLocation() +
                              MeshComp->GetBoneTransform(OC).GetLocation()) *
                             0.5f;
    if (FVector::DistSquared(Center, OtherMid) < FMath::Square(30.f))
      ++NearbyConstraints;
  }
  const float SpatialCrowding =
      FMath::Clamp(static_cast<float>(NearbyConstraints) / 10.f, 0.f, 1.f);

  // === Topological Density ===
  int32 LinksIn = 0;
  for (const FOHConstraintInstanceData &Link : Graph.GetConstraintLinks()) {
    if (Link.GetParentBone() == Constraint.GetParentBone() ||
        Link.GetChildBone() == Constraint.GetParentBone())
      ++LinksIn;
    if (Link.GetParentBone() == Constraint.GetChildBone() ||
        Link.GetChildBone() == Constraint.GetChildBone())
      ++LinksIn;
  }
  const float TopologicalCrowding =
      FMath::Clamp(static_cast<float>(LinksIn) / 12.f, 0.f, 1.f);
  ...
  if (PData && CData) {
    ...
    VelocityDelta = FVector::Dist(PData->GetBodyLinearVelocity(),
                                  CData->GetBodyLinearVelocity());
    AccelDelta = FVector::Dist(PData->GetLinearAcceleration(),
                               CData->GetLinearAcceleration());
  }
...
static bool HasCycleUtil(const FName &Bone,
                         const TMap<FName, TArray<FName>> &AdjacencyMap,
                         TSet<FName> &Visited, TSet<FName> &RecStack) {
    ...
    if (const TArray<FName> *Neighbors = AdjacencyMap.Find(Bone)) {
      for (const FName &Neighbor : *Neighbors) {
        if (!Visited.Contains(Neighbor) &&
            HasCycleUtil(Neighbor, AdjacencyMap, Visited, RecStack)) {
          return true;
...
bool UOHGraphUtils::DetectCyclesInGraph(
  ...
  for (const FOHConstraintInstanceData &Constraint : Constraints) {
    AdjacencyMap.FindOrAdd(Constraint.GetParentBone())
        .Add(Constraint.GetChildBone());
  }
  ...
  for (const auto &Pair : BoneMap) {
    ...
    if (!Visited.Contains(Bone)) {
      if (HasCycleUtil(Bone, AdjacencyMap, Visited, RecursionStack)) {
        UE_LOG(LogTemp, Warning,
               TEXT("[OHGraphUtils] Cycle detected involving bone: %s"),
               *Bone.ToString());
        return true;
      }
    }
...
void UOHGraphUtils::RefreshConstraintInstances(
    FOHPhysicsGraphNode &Graph, USkeletalMeshComponent *SkeletalMesh) {
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Warning,
           TEXT("RefreshConstraintInstances: SkeletalMeshComponent is null, "
                "skipping."));
    return;
  }
  ...
  for (FOHConstraintInstanceData &Entry : Graph.GetConstraintLinks()) {
    ...
    FConstraintInstance *FoundCI =
        SkeletalMesh->FindConstraintInstance(Entry.GetConstraintName());
...
bool UOHGraphUtils::ValidateGraph(FOHPhysicsGraphNode &Graph,
                                  EValidationStrictness Strictness) {
  // Grab the raw data
  const auto &BoneMap = Graph.GetBoneMap();
  const auto &Constraints = Graph.GetConstraintLinks();
  ...
  TSet<FName> BonesWithIssues =
      OHSafeMapUtils::FindUnconstrainedBones<FOHConstraintInstanceData>(
          BoneMap, Constraints,
          /* ParentKeyFn = */
          [](const FOHConstraintInstanceData &C) { return C.GetParentBone(); },
          /* ChildKeyFn  = */
  ...
  if (Strictness == EValidationStrictness::Strict) {
    ...
    return BonesWithIssues.IsEmpty() && !DetectCyclesInGraph(Graph);
  } else {
...
void UOHGraphUtils::PruneInvalidGraphElements(
    FOHPhysicsGraphNode &Graph, EValidationStrictness Strictness) {
  auto &BoneMap = Graph.GetBoneMap();
  auto &Constraints = Graph.GetConstraintLinks();

  TSet<FName> InvalidBones =
      OHSafeMapUtils::FindUnconstrainedBones<FOHConstraintInstanceData>(
          BoneMap, Constraints,
          [](const FOHConstraintInstanceData &C) { return C.GetParentBone(); },
          [](const FOHConstraintInstanceData &C) { return C.GetChildBone(); });
  ...
  Constraints.RemoveAll([&](const FOHConstraintInstanceData &C) {
    return !BoneMap.Contains(C.GetParentBone()) ||
           !BoneMap.Contains(C.GetChildBone());
  });
}

void UOHGraphUtils::DrawPhysicsGraphOverlay(
    const FOHPhysicsGraphNode &Graph, const USkeletalMeshComponent *MeshComp,
    UWorld *World, float Duration, EPhysicsGraphOverlayMode OverlayMode,
    bool bDrawLabels) {
  if (!MeshComp || !World)
    return;

  const bool bDrawFull = (OverlayMode == EPhysicsGraphOverlayMode::Full ||
                          OverlayMode == EPhysicsGraphOverlayMode::Both);
  const bool bDrawAnomalies =
      (OverlayMode == EPhysicsGraphOverlayMode::Anomalies ||
       OverlayMode == EPhysicsGraphOverlayMode::Both);
    ...
    for (const auto &Pair : Graph.GetBoneMap()) {
      ...
      if (bDrawLabels) {
        FString Label = FString::Printf(TEXT("%s\nM:%.1f L:%.1f"),
                                        *BoneName.ToString(), Mass, Length);
        DrawDebugString(World, Pos + FVector(0, 0, Length), Label, nullptr,
                        BoxColor, Duration, false);
      }
    }
    ...
    for (const FOHConstraintInstanceData &Constraint :
      ...
      if (bDrawLabels) {
        FString Label = FString::Printf(
            TEXT("%s\nI:%.2f"), *Constraint.GetConstraintName().ToString(),
            Influence);
        DrawDebugString(World, Midpoint + FVector(0, 0, 10), Label, nullptr,
                        LineColor, Duration, false);
      }
    }
  }
  ...
  if (bDrawAnomalies) {
    ...
    for (const auto &Pair : Graph.GetBoneMap()) {
      ...
      for (const auto &Constraint : Graph.GetConstraintLinks()) {
        if (Constraint.GetParentBone() == Pair.Key ||
            Constraint.GetChildBone() == Pair.Key) {
          bReferenced = true;
          break;
        }
      }
    ...
    for (FName Orphan : Orphans) {
      ...
      if (BoneIndex != INDEX_NONE) {
        ...
        DrawDebugString(World, Pos + FVector(0, 0, 18), TEXT("ORPHAN"), nullptr,
    ...
    if (Islands.Num() > 1) {
      const TArray<FColor> Palette = {FColor::Blue, FColor::Green, FColor::Cyan,
                                      FColor::Purple, FColor::Yellow};
      for (int32 i = 0; i < Islands.Num(); ++i) {
        ...
        for (const FName &Bone : Islands[i]) {
          ...
          if (BoneIndex != INDEX_NONE) {
            ...
            if (bDrawLabels)
              DrawDebugString(World, Pos + FVector(0, 0, 12),
                              FString::Printf(TEXT("Island %d"), i + 1),
                              nullptr, Color, Duration, false);
          }
        }
      }
    }

    // TODO: Add more anomaly overlays (outliers, invalid constraints, etc) as
    // needed
  }
}

// Overlay state (file-scope or static inside a manager class)
static EPhysicsGraphOverlayMode GPhysicsGraphOverlayMode =
