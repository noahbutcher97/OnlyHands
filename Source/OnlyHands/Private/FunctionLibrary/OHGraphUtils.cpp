...
bool UOHGraphUtils::IsBoneValid(const FOHBoneData& Bone, EValidationStrictness Strictness) {
    ...
    case EValidationStrictness::Strict:
        return Bone.GetBoneName() != NAME_None && Bone.GetHasSimulatedBody() && Bone.GetCachedBodyMass() > 0.f &&
...
bool UOHGraphUtils::AddBoneIfNoConflict(TMap<FName, FOHBoneData>& BoneMap, const FOHBoneData& BoneData) {
    ...
    return OHSafeMapUtils::TryInsertIfNoConflict<FName, FOHBoneData>(
        BoneMap, Name, BoneData, [](const FOHBoneData& Existing, const FOHBoneData& Incoming) {
            ...
            const bool bMassDiff =
                !FMath::IsNearlyEqual(Existing.GetCachedBodyMass(), Incoming.GetCachedBodyMass(), 0.01f);
            const bool bLengthDiff =
                !FMath::IsNearlyEqual(Existing.GetCachedBoneLength(), Incoming.GetCachedBoneLength(), 0.01f);

            return bMismatchSim || bMassDiff || bLengthDiff;
        });
}

int32 UOHGraphUtils::InsertAllBonesIfValid(TMap<FName, FOHBoneData>& TargetMap,
    ...
    return OHSafeMapUtils::InsertAllValidated<FName, FOHBoneData>(
        TargetMap, SourceMap,
        [Strictness](const FOHBoneData& Bone) { return UOHGraphUtils::IsBoneValid(Bone, Strictness); });
}

int32 UOHGraphUtils::InsertAllConstraintsIfValid(TArray<FOHConstraintInstanceData>& TargetList,
...
TArray<FOHConstraintInstanceData>
UOHGraphUtils::GetConstraintsForBone(FName BoneName, const TArray<FOHConstraintInstanceData>& ConstraintList) {
...
void UOHGraphUtils::RemoveInvalidConstraints(TArray<FOHConstraintInstanceData>& ConstraintList,
                                             const TMap<FName, FOHBoneData>& BoneMap) {
    ConstraintList.RemoveAll([&](const FOHConstraintInstanceData& C) {
        const bool bValidParent =
            BoneMap.Contains(C.GetParentBone()) && BoneMap[C.GetParentBone()].GetHasSimulatedBody();
...
void UOHGraphUtils::RemoveBoneMassOutliers(TMap<FName, FOHBoneData>& BoneMap, float ThresholdZ) {
    OHSafeMapUtils::RemoveMapOutliers<FName, FOHBoneData>(
        BoneMap, [](const FOHBoneData& Bone) { return Bone.GetCachedBodyMass(); }, ThresholdZ);
}

void UOHGraphUtils::LogGraphStats(const TMap<FName, FOHBoneData>& BoneMap,
                                  const TArray<FOHConstraintInstanceData>& Constraints, const FString& ContextTag) {
    ...
    UE_LOG(LogTemp, Log, TEXT("[%s] BoneCount: %d, Simulated: %d, Constraints: %d"), *ContextTag, BoneCount,
           SimulatedCount, ConstraintCount);
}

bool UOHGraphUtils::ValidateAllChainsStable(const TMap<FName, FOHBoneData>& BoneMap,
    ...
    auto ChildMap = OHSafeMapUtils::BuildParentToChildrenMap<FOHConstraintInstanceData>(
        Constraints, [](const FOHConstraintInstanceData& C) { return C.GetParentBone(); },
...
// ========================================================
bool UOHGraphUtils::BuildPhysicsGraphFromComponent(USkeletalMeshComponent* SkeletalMeshComp,
                                                   FOHPhysicsGraphNode& OutGraph, const FString& ContextTag,
                                                   bool bRunAudit) {
    if (!SkeletalMeshComp) {
    ...
    for (const FBodyInstance* BI : SkeletalMeshComp->Bodies) {
        ...
        const FName BoneName = BI->BodySetup.IsValid() ? BI->BodySetup->BoneName : NAME_None;
    ...
    for (const UPhysicsConstraintTemplate* Template : PhysAsset->ConstraintSetup) {
        ...
        FConstraintInstance** FoundPtr = SkeletalMeshComp->Constraints.FindByPredicate(
            [Link](const FConstraintInstance* CI) { return CI && CI->JointName == Link.GetConstraintName(); });
    ...
    UE_LOG(LogTemp, Log, TEXT("[%s] Built physics graph: %d bones, %d constraints"), *ContextTag, BoneMap.Num(),
           Constraints.Num());
...
bool UOHGraphUtils::RefreshBonesOnlyFromComponent(USkeletalMeshComponent* SkeletalMeshComp, FOHPhysicsGraphNode& Graph,
...
bool UOHGraphUtils::RefreshConstraintsOnlyFromComponent(USkeletalMeshComponent* SkeletalMeshComp,
    ...
    if (bRebuildFullConstraintList) {
        ...
        for (FOHConstraintInstanceData& Entry : Constraints) {
            ...
            if (FoundCI) {
                ...
                UE_LOG(LogTemp, Warning, TEXT("[%s] Could not find constraint instance for '%s'"), *ContextTag,
                       *Entry.GetConstraintName().ToString());
            }
        }
    }
    ...
    UE_LOG(LogTemp, Log, TEXT("[%s] Refreshed %d constraints (%s full rebuild)"), *ContextTag, ReboundCount,
           bRebuildFullConstraintList ? TEXT("with") : TEXT("without"));

    return true;
}
bool UOHGraphUtils::RefreshGraphFromComponent(USkeletalMeshComponent* SkeletalMeshComp, FOHPhysicsGraphNode& Graph,
    ...
    if (bRefreshBones) {
        const bool bBoneResult =
            RefreshBonesOnlyFromComponent(SkeletalMeshComp, Graph, ContextTag + TEXT("_Bones"), bRebindOwnerComponent);
    ...
    if (bRefreshConstraints) {
        const bool bConstraintResult = RefreshConstraintsOnlyFromComponent(
            SkeletalMeshComp, Graph, ContextTag + TEXT("_Constraints"), bFullConstraintRebuild, bRebindOwnerComponent);
...
void UOHGraphUtils::ResetGraph(FOHPhysicsGraphNode& Graph) {
    UE_LOG(LogTemp, Verbose, TEXT("ResetGraph: Clearing %d bones, %d constraints"), Graph.GetBoneMap().Num(),
           Graph.GetConstraintLinks().Num());
    Graph.Reset();
}

bool UOHGraphUtils::SafeRebuildGraph(const USkeletalMeshComponent* SkelComp, FOHPhysicsGraphNode& Graph,
                                     bool bForceResetIfFailed) {
    if (!SkelComp)
        return false;

    // Build directly into the existing graph (the function resets it internally)
    if (BuildPhysicsGraphFromComponent(const_cast<USkeletalMeshComponent*>(SkelComp), Graph, TEXT("SafeRebuild"),
                                       true)) {
        return true;
    }
...
TSet<FName> UOHGraphUtils::GetAllDescendants(const FOHPhysicsGraphNode& Graph, FName RootBone) {
    return OHSafeMapUtils::TraverseWithNodeLimit<FName>(
        RootBone, [&](const FName& Node) -> TArray<FName> { return Graph.GetChildrenOfBone(Node); },
        [](const FName&) {},
        /*MaxNodesToVisit=*/-1);
}
...
TMap<FName, TArray<FName>> UOHGraphUtils::BuildAdjacencyMap(const FOHPhysicsGraphNode& Graph) {
    return OHSafeMapUtils::BuildBoneConstraintAdjacencyGraph<FOHConstraintInstanceData>(
        Graph.GetConstraintLinks(), [](const FOHConstraintInstanceData& C) { return C.GetParentBone(); },
        [](const FOHConstraintInstanceData& C) { return C.GetChildBone(); });
}
...
TSet<FName> UOHGraphUtils::TraverseGraphBFS(const FOHPhysicsGraphNode& Graph, FName Start) {
    ...
    auto Visitor = [&](const FName& VisitedBone) { VisitedNodes.Add(VisitedBone); };

    // Call traversal with start node, get neighbors, visitor, and max node limit (512)
    OHSafeMapUtils::TraverseWithNodeLimit<FName>(Start, GetChildren, Visitor, 512);

    return VisitedNodes;
}

TSet<FName> UOHGraphUtils::TraverseGraphBFS(const FOHPhysicsGraphNode& Graph, FName Start, int32 MaxNodesToVisit) {
    return OHSafeMapUtils::TraverseWithNodeLimit<FName>(
        Start, [&](const FName& Node) -> TArray<FName> { return Graph.GetChildrenOfBone(Node); }, [](const FName&) {},
        MaxNodesToVisit);
}

void UOHGraphUtils::TraverseGraphDFS(const FOHPhysicsGraphNode& Graph, FName Start,
...
TMap<FName, FString> UOHGraphUtils::GetConstraintHealthReport(const FOHPhysicsGraphNode& Graph) {
    ...
    return OHSafeMapUtils::MapToReport<FOHConstraintInstanceData, FName>(
        Constraints, [](const FOHConstraintInstanceData& C) { return C.GetConstraintName(); },
        [&](const FOHConstraintInstanceData& C) -> FString {
            ...
            return FString::Printf(
                TEXT("%sParent: %s | Child: %s | Ratio: %.2f | Dist: %.1f | Stiff: %.2f | Damp: %.2f"), *Flags,
                *A.ToString(), *B.ToString(), Ratio, Distance, C.GetConeLimitStiffness(), C.GetConeLimitDamping());
        });
}
...
void UOHGraphUtils::PruneInvalidConstraints(FOHPhysicsGraphNode& Graph) {
    OHSafeMapUtils::RemoveIfAnyMissingOrInvalid<FName, FOHBoneData, FOHConstraintInstanceData>(
        ...
        [](const FOHConstraintInstanceData& C) { return C.GetConstraintInstance() != nullptr; });
}
...
void UOHGraphUtils::DrawPhysicsGraphOverlay(const FOHPhysicsGraphNode& Graph, const USkeletalMeshComponent* MeshComp,
...
float UOHGraphUtils::CalculateConstraintInfluenceScore(const FOHConstraintInstanceData& Constraint,
    ...
    for (const FOHConstraintInstanceData& Other : Graph.GetConstraintLinks()) {
        ...
        const FVector OtherMid =
            (MeshComp->GetBoneTransform(OP).GetLocation() + MeshComp->GetBoneTransform(OC).GetLocation()) * 0.5f;
    ...
    for (const FOHConstraintInstanceData& Link : Graph.GetConstraintLinks()) {
        if (Link.GetParentBone() == Constraint.GetParentBone() || Link.GetChildBone() == Constraint.GetParentBone())
            ++LinksIn;
        if (Link.GetParentBone() == Constraint.GetChildBone() || Link.GetChildBone() == Constraint.GetChildBone())
            ++LinksIn;
    }
...
static bool HasCycleUtil(const FName& Bone, const TMap<FName, TArray<FName>>& AdjacencyMap, TSet<FName>& Visited,
                         TSet<FName>& RecStack) {
...
bool UOHGraphUtils::DetectCyclesInGraph(const TMap<FName, FOHBoneData>& BoneMap,
...
void UOHGraphUtils::RefreshConstraintInstances(FOHPhysicsGraphNode& Graph, USkeletalMeshComponent* SkeletalMesh) {
    if (!SkeletalMesh) {
    ...
    UE_LOG(
        LogTemp, Verbose, TEXT("RefreshConstraintInstances: Rebound %d/%d constraints."),
...
bool UOHGraphUtils::ValidateGraph(FOHPhysicsGraphNode& Graph, EValidationStrictness Strictness) {
    // Grab the raw data
    const auto& BoneMap = Graph.GetBoneMap();
    const auto& Constraints = Graph.GetConstraintLinks();
...
void UOHGraphUtils::PruneInvalidGraphElements(FOHPhysicsGraphNode& Graph, EValidationStrictness Strictness) {
    ...
    TSet<FName> InvalidBones = OHSafeMapUtils::FindUnconstrainedBones<FOHConstraintInstanceData>(
        BoneMap, Constraints, [](const FOHConstraintInstanceData& C) { return C.GetParentBone(); },
        [](const FOHConstraintInstanceData& C) { return C.GetChildBone(); });
...
void UOHGraphUtils::DrawPhysicsGraphOverlay(const FOHPhysicsGraphNode& Graph, const USkeletalMeshComponent* MeshComp,
    ...
    const bool bDrawFull =
        (OverlayMode == EPhysicsGraphOverlayMode::Full || OverlayMode == EPhysicsGraphOverlayMode::Both);
    const bool bDrawAnomalies =
        (OverlayMode == EPhysicsGraphOverlayMode::Anomalies || OverlayMode == EPhysicsGraphOverlayMode::Both);
    ...
    if (bDrawAnomalies) {
        ...
        for (FName Orphan : Orphans) {
            ...
            if (BoneIndex != INDEX_NONE) {
                ...
                DrawDebugString(World, Pos + FVector(0, 0, 18), TEXT("ORPHAN"), nullptr, FColor::Magenta, Duration,
                                false);
            }
        }
        ...
        if (Islands.Num() > 1) {
            const TArray<FColor> Palette = {FColor::Blue, FColor::Green, FColor::Cyan, FColor::Purple, FColor::Yellow};
            for (int32 i = 0; i < Islands.Num(); ++i) {
                ...
                for (const FName& Bone : Islands[i]) {
                    ...
                    if (BoneIndex != INDEX_NONE) {
                        ...
                        if (bDrawLabels)
                            DrawDebugString(World, Pos + FVector(0, 0, 12), FString::Printf(TEXT("Island %d"), i + 1),
