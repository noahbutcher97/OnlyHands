...
/**
 * A hardcoded strategy that determines physics targets based on body part
 * heuristics.
 */
class FOHAnatomicalHeuristicStrategy : public IOHPhysicsBehaviorStrategy {
public:
  virtual FOHResolvedPhysicsTargets
  Resolve(const FBoneState &State, EOHSkeletalBone Bone) const override {
    ...
    } else if (Part == EOHBodyPart::Leg_Left ||
               Part == EOHBodyPart::Leg_Right) {
  ...
  virtual FString GetStrategyName() const override {
    return TEXT("AnatomicalHeuristic");
  }
};