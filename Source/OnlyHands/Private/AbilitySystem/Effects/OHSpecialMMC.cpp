...
UOHSpecialMMC::UOHSpecialMMC() {
  SpecialBarDef.AttributeToCapture =
      UOHCombatAttributeSet::GetSpecialBarAttribute();
  SpecialBarDef.AttributeSource = EGameplayEffectAttributeCaptureSource::Source;
  SpecialBarDef.bSnapshot = true;

  MaxSpecialBarDef.AttributeToCapture =
      UOHCombatAttributeSet::GetMaxSpecialBarAttribute();
  MaxSpecialBarDef.AttributeSource =
...
float UOHSpecialMMC::CalculateBaseMagnitude_Implementation(
  ...
  GetCapturedAttributeMagnitude(SpecialBarDef, Spec, EvaluationParameters,
                                SpecialBar);

  float MaxSpecialBar = 0.f;
  GetCapturedAttributeMagnitude(MaxSpecialBarDef, Spec, EvaluationParameters,
                                MaxSpecialBar);

  float Special = 0.f;
  GetCapturedAttributeMagnitude(SpecialDef, Spec, EvaluationParameters,
                                Special);

  if (MaxSpecialBar == 0.f)
    MaxSpecialBar =
        10000.f; // dont divide by zero, large denominator = small number.
  if (Special == 0.f)
    Special = 1.f;
