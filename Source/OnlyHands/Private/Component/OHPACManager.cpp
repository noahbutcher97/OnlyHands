...
void UOHPACManager::FinalizeBlendByID(FName BoneName, int32 BlendID)
{
    ...
    for (const FName& ChainBone : Chain)
    {
        ...
        SafeLog(FString::Printf(TEXT("Finalized blend ID %d for bone %s (remaining ref count: %d)"), 
            BlendID, *ChainBone.ToString(), RefCount));
    ...
    FName ReactionTag)
{
    ...
    {
        if (!StartChainPhysicalAnimation_Filtered(
             BoneName,
             CustomProfile,
             true, // bUseNativePropagation
             true, // bEnableCollision
             [this](FName Bone){
                 return SimulatableBones.Contains(Bone) && !SimulationExclusions.Contains(Bone);
             }))
        {
    ...
    SafeLog(FString::Printf(TEXT("Started hit reaction (ID: %d) on bone: %s (total reactions: %d)"),
        BlendState.BlendID, *BoneName.ToString(), BoneBlends.Num()));
}
#pragma endregion
...
{
    if (!SkeletalMesh || !IsBoneValidForSimulation(BoneName)) return false;
...
bool UOHPACManager::StartBonePhysicalAnimation(FName BoneName, const FPhysicalAnimationData& Profile, bool bEnableCollision)
{
    if (!IsBoneValidForSimulation(BoneName)) return false;
    if (!PhysicalAnimationComponent || !SkeletalMesh) return false;
...
bool UOHPACManager::StartChainPhysicalAnimation(FName RootBone, const FPhysicalAnimationData& Profile, bool bUseNativePropagation, bool bEnableCollision)
{
    // Default: no filtering
    return StartChainPhysicalAnimation_Filtered(RootBone, Profile, bUseNativePropagation, bEnableCollision, [](FName){ return true; });
}

bool UOHPACManager::StartChainPhysicalAnimation_Filtered(FName RootBone, const FPhysicalAnimationData& Profile, bool bUseNativePropagation, bool bEnableCollision, TFunctionRef<bool(FName)> BoneFilter)
{
    if (!IsBoneValidForSimulation(RootBone)) return false;
    if (!PhysicalAnimationComponent || !SkeletalMesh) return false;
...
void UOHPACManager::StopChainPhysicalAnimation_Filtered(FName RootBone, bool bUseNativePropagation, TFunctionRef<bool(FName)> BoneFilter)
{
    if (!PhysicalAnimationComponent || !SkeletalMesh) return;
...
FBodyInstance* UOHPACManager::GetBodyInstanceDirect(FName BoneName)
{
    if (FBodyInstance** Found = BodyInstanceCache.Find(BoneName))
    {
        ...
        else
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Removed invalid cached body for bone: %s"), *BoneName.ToString());
        }
    }
...
float UOHPACManager::GetBlendAlpha(FName BoneName) const
{
    ...
    if (!BoneBlends || BoneBlends->Num() == 0) return 0.f;
...
bool UOHPACManager::IsBoneNamePatternValid(FName BoneName)
{
    ...
    static const TArray<FString> DisallowedSubstrings = {
        TEXT("twist"), TEXT("ik"), TEXT("attach"), TEXT("helper")
    };
...
TArray<FName> UOHPACManager::GetBoneChain(FName RootBone, int32 MaxDepth) const
{
    ...
    if (bUsingFallback)
    {
        UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] BoneChildrenMap missing/corrupt, used RefSkeleton fallback in GetBoneChain! Consider rebuilding BoneChildrenMap."));
    }
...
bool UOHPACManager::ValidateBodyInstances(TArray<FName>& OutMissingBones, TArray<FName>& OutInstancesWithoutBodies) const
{
    ...
    for (const FName& BoneName : TrackedBones)
    {
        if (!AssetBodyNames.Contains(BoneName))
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Tracked bone missing in PhysicsAsset: %s"), *BoneName.ToString());
        }
    }
    ...
    for (const FName& AssetName : AssetBodyNames)
    {
        if (!RuntimeBodyNames.Contains(AssetName))
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Body in PhysicsAsset but NOT present at runtime: %s"), *AssetName.ToString());
        }
    }
    ...
    for (const FName& InstanceName : RuntimeBodyNames)
    {
        if (!AssetBodyNames.Contains(InstanceName))
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Body present at runtime but NOT in PhysicsAsset: %s"), *InstanceName.ToString());
        }
    }
...
) const
{
    ...
    for (const auto& AssetPair : AssetConstraintMap)
    {
        ...
        if (!RuntimeInstancePtr)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Constraint in PhysicsAsset but NOT present at runtime: %s"), *ConstraintName.ToString());
            continue;
        }
            ...
            RuntimeInstance->ConstraintBone2 != AssetInstance->ConstraintBone2)
        {
            ...
            UE_LOG(LogTemp, Error, TEXT("[OHPACManager] Constraint binding mismatch: %s. Asset (Parent: %s, Child: %s), Runtime (Parent: %s, Child: %s)"),
                *ConstraintName.ToString(),
                *AssetInstance->ConstraintBone1.ToString(), *AssetInstance->ConstraintBone2.ToString(),
                *RuntimeInstance->ConstraintBone1.ToString(), *RuntimeInstance->ConstraintBone2.ToString());
        }
    }
    ...
    for (const auto& RuntimePair : RuntimeConstraintMap)
    {
        ...
        if (!AssetConstraintMap.Contains(ConstraintName))
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Constraint present at runtime but NOT in PhysicsAsset: %s"), *ConstraintName.ToString());
        }
    }
...
) const
{
    ...
    if (!bValid)
    {
        UE_LOG(LogTemp, Error, TEXT("[OHPACManager] Core component validation failed. Skipping PhysicsAsset validation."));
        return false;
    }
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] %d tracked bones missing bodies (will be excluded from simulation)"), 
            OutMissingBones.Num());
...
bool UOHPACManager::IsSkeletalMeshBindingValid(bool bAutoFix, bool bLog) const
{
    ...
    if (!bMatch && bAutoFix && PhysicalAnimationComponent && BoundMesh)
    {
        ...
        if (bLog)
        {
            UE_LOG(LogTemp, Warning, TEXT("[OHPACManager] Auto-fixed PhysicalAnimationComponent SkeletalMesh binding: BoundMesh=%s, PACMesh=%s, Match=%d"),
    ...
    for (const FName& BoneName : SimulatableBones)
    {
        ...
        else
        {
            ...
            BoneMsg += FString::Printf(TEXT("  -- Valid. IsSimPhysics: %d, BoneIdx: %d, Owner: %s"),
                (int)bSim, SkeletalMesh->GetBoneIndex(BoneName), *SkeletalMesh->GetName());
        }
