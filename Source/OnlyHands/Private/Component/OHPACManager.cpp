...
MismatchedConstraints; if (!ValidateSetup(MissingBones, InstancesWithoutBodies, MissingConstraints,
RuntimeConstraintsNotInAsset, MismatchedConstraints))
    {
...
void UOHPACManager::FinalizeBlendByID(FName BoneName, int32 BlendID) {
    ...
    if (bVerboseLogging) {
        UE_LOG(LogTemp, Warning, TEXT("🏁 Finalized blend ID %d for %s (%d blends remaining)"), BlendID,
               *BoneName.ToString(), BlendArray->Num());
    }
...
bool UOHPACManager::TryActivateSimForBone(FName BoneName, const FPhysicalAnimationData& Profile) {
    ...
    if (RefCount == 1) {
        ...
        if (PhysicalAnimationComponent) {
            PhysicalAnimationComponent->ApplyPhysicalAnimationSettings(BoneName, Profile);
            if (bVerboseLogging) {
                UE_LOG(LogTemp, Warning, TEXT("🔄 Updated PAC profile for %s (RefCount: %d)"), *BoneName.ToString(),
                       RefCount);
            }
            return true;
        } else {
    ...
    for (const FName& BoneName : SimulatableBones) {
        ...
        if (!Body) {
            ...
            if (bSim)
                SimulatingCount++;
            BoneMsg +=
                FString::Printf(TEXT("  -- Valid. IsSimPhysics: %d, BoneIdx: %d, Owner: %s"), static_cast<int>(bSim),
