...
FVector FOHConstraintData::GetSwingStrain(USkeletalMeshComponent* SkelMesh) const {
    ...
    return FVector(Swing1Limit > 0.f ? Swing1 / Swing1Limit : 0.f, Swing2Limit > 0.f ? Swing2 / Swing2Limit : 0.f, 0.f);
}
...
void UOHPACManager::BeginPlay() {
    ...
    if (bVerifyPhysicsState) {
        GetWorld()->GetTimerManager().SetTimer(PhysicsVerificationTimer, this,
                                               &UOHPACManager::VerifyPhysicsStateInternal, PhysicsVerificationInterval,
                                               true);
    }
...
void UOHPACManager::SetupPhysicsAsset() {
    ...
    if (!PhysicsAsset && DefaultPhysicsAsset) {
        ...
        SafeLog(FString::Printf(TEXT("No PhysicsAsset found on SkeletalMesh '%s', using default asset"),
                                SkeletalMesh->GetSkeletalMeshAsset() ? *SkeletalMesh->GetSkeletalMeshAsset()->GetName()
                                                                     : TEXT("None")),
                true);
    }
    ...
    if (!bHasExpectedBones) {
        SafeLog(FString::Printf(TEXT("WARNING: Physics asset '%s' only has %d arm bones (expected at least 6)"),
                                *PhysicsAsset->GetName(), ArmBoneCount),
                true);
...
void UOHPACManager::InitializePACManager() {
    ...
    if (CoreBones.Num() == 0) {
        CoreBones = {TEXT("spine_01"), TEXT("spine_02"), TEXT("spine_03"), TEXT("neck_01"), TEXT("head"),
                     TEXT("pelvis"),   TEXT("hips"),     TEXT("chest"),    TEXT("ribcage")};
    }
    ...
    if (SkeletalMesh->GetSkeletalMeshAsset()) {
        SkeletalMesh->GetSkeletalMeshAsset()->GetOnMeshChanged().AddUObject(this,
                                                                            &UOHPACManager::OnSkeletalMeshChanged);
    }
#endif
...
void UOHPACManager::StartInitializationRetry() {
    if (GetWorld() && InitRetryTotalDurationSeconds > 0.0f) {
        ...
        GetWorld()->GetTimerManager().SetTimer(
            ...
            InitRetryTotalDurationSeconds, false);
    }
}
...
void UOHPACManager::InitializeBoneLists() {
    ...
    SafeLog(FString::Printf(TEXT("Initialized with %d tracked bones, %d simulatable bones"), TrackedBones.Num(),
...
void UOHPACManager::InitializeBoneCategoryDefinitions() {
    ...
    BoneCategoryDefinitions.Add(EOHBoneCategory::UpperBodyBones,
                                {TEXT("spine_01"), TEXT("spine_02"), TEXT("spine_03"), TEXT("clavicle_l"),
                                 TEXT("clavicle_r"), TEXT("upperarm_l"), TEXT("upperarm_r"), TEXT("lowerarm_l"),
                                 TEXT("lowerarm_r"), TEXT("hand_l"), TEXT("hand_r")});

    // Lower Body
    BoneCategoryDefinitions.Add(EOHBoneCategory::LowerBodyBones,
                                {TEXT("pelvis"), TEXT("spine_01"), TEXT("thigh_l"), TEXT("thigh_r"), TEXT("calf_l"),
                                 TEXT("calf_r"), TEXT("foot_l"), TEXT("foot_r")});
    ...
    // Head
    BoneCategoryDefinitions.Add(EOHBoneCategory::HeadBones, {TEXT("neck_01"), TEXT("head")});
    ...
    BoneCategoryDefinitions.Add(EOHBoneCategory::ArmBones,
                                {TEXT("clavicle_l"), TEXT("clavicle_r"), TEXT("upperarm_l"), TEXT("upperarm_r"),
                                 TEXT("lowerarm_l"), TEXT("lowerarm_r"), TEXT("hand_l"), TEXT("hand_r")});

    // Clavicles
    BoneCategoryDefinitions.Add(EOHBoneCategory::ClavicleBones, {TEXT("clavicle_l"), TEXT("clavicle_r")});

    // Upper Arms
    BoneCategoryDefinitions.Add(EOHBoneCategory::UpperArmBones, {TEXT("upperarm_l"), TEXT("upperarm_r")});

    // Lower Arms
    BoneCategoryDefinitions.Add(EOHBoneCategory::LowerArmBones, {TEXT("lowerarm_l"), TEXT("lowerarm_r")});

    // Hands
    BoneCategoryDefinitions.Add(EOHBoneCategory::HandBones, {TEXT("hand_l"), TEXT("hand_r")});

    // Left Leg
    BoneCategoryDefinitions.Add(EOHBoneCategory::LeftLegBones, {TEXT("thigh_l"), TEXT("calf_l"), TEXT("foot_l")});

    // Right Leg
    BoneCategoryDefinitions.Add(EOHBoneCategory::RightLegBones, {TEXT("thigh_r"), TEXT("calf_r"), TEXT("foot_r")});

    // All Legs
    BoneCategoryDefinitions.Add(EOHBoneCategory::LegBones, {TEXT("thigh_l"), TEXT("thigh_r"), TEXT("calf_l"),
                                                            TEXT("calf_r"), TEXT("foot_l"), TEXT("foot_r")});

    // Thighs
    BoneCategoryDefinitions.Add(EOHBoneCategory::ThighBones, {TEXT("thigh_l"), TEXT("thigh_r")});

    // Calves
    BoneCategoryDefinitions.Add(EOHBoneCategory::CalfBones, {TEXT("calf_l"), TEXT("calf_r")});

    // Feet
    BoneCategoryDefinitions.Add(EOHBoneCategory::FootBones, {TEXT("foot_l"), TEXT("foot_r")});
...
bool UOHPACManager::SetBonePhysicalAnimation_Internal(FName BoneName, bool bEnable,
                                                      const FPhysicalAnimationData& Profile, bool bAffectBelow,
                                                      const TArray<FName>& FilterBones, bool bEnableCollision,
                                                      bool bWakeBody, bool bEnableNativeForcePropagation) {
    if (!SkeletalMesh || !PhysicalAnimationComponent) {
        SafeLog(
            FString::Printf(
                TEXT("SetBonePhysicalAnimation_Internal: SkeletalMesh or PhysicalAnimationComponent is missing for %s"),
                *BoneName.ToString()),
            true);
        return false;
    }
    ...
    if (!Body || !Body->IsValidBodyInstance()) {
        SafeLog(
            FString::Printf(
                TEXT("SetBonePhysicalAnimation_Internal: Invalid or null BodyInstance for %s, attempting refresh..."),
                *BoneName.ToString()),
            true);
    ...
    if (bEnable) {
        ...
        SafeLog(FString::Printf(TEXT("Applied PAC profile to %s (Strength: %.1f)"), *BoneName.ToString(),
...
bool UOHPACManager::SetBonePhysicalAnimation(FName BoneName, bool bEnable, const FPhysicalAnimationData& Profile,
                                             bool bIncludeChain, bool bEnableCollision, bool bWakeBody) {
    return SetBonePhysicalAnimation_Internal(BoneName, bEnable, Profile, bIncludeChain, TArray<FName>(),
                                             bEnableCollision, bWakeBody);
}

bool UOHPACManager::SetBonePhysicalAnimationWithFilter(FName BoneName, bool bEnable,
                                                       const FPhysicalAnimationData& Profile, bool bIncludeChain,
                                                       const TArray<FName>& FilterBones, bool bEnableCollision,
                                                       bool bWakeBody) {
    return SetBonePhysicalAnimation_Internal(BoneName, bEnable, Profile, bIncludeChain, FilterBones, bEnableCollision,
                                             bWakeBody);
}
...
bool UOHPACManager::EnablePhysicsForBone(FName BoneName, const FPhysicalAnimationData& Profile, bool bEnableCollision,
                                         bool bWakeBody,
                                         bool bEnableDrivePropagation) // NEW PARAMETER with default = true
{
    ...
    // Log state change
    if (bLogStateChanges) {
        SafeLog(FString::Printf(TEXT("Enabling physics for bone %s (Drive Propagation: %s)"), *BoneName.ToString(),
    ...
    if (bEnableDrivePropagation) {
        ...
        SafeLog(
            FString::Printf(TEXT("Applied physics settings BELOW %s with force propagation"), *BoneName.ToString()));
    } else {
        // Single bone only
        PhysicalAnimationComponent->ApplyPhysicalAnimationSettings(BoneName, Profile);

        SafeLog(FString::Printf(TEXT("Applied physics settings to %s only"), *BoneName.ToString()));
    }
    ...
    if (!Body->IsInstanceSimulatingPhysics()) {
        SafeLog(FString::Printf(TEXT("Failed to enable simulation for %s, retrying..."), *BoneName.ToString()), true);

        // Force recreation of physics state
        Body->TermBody();
        Body->InitBody(Body->GetBodySetup(), SkeletalMesh->GetComponentTransform(), SkeletalMesh,
                       SkeletalMesh->GetWorld()->GetPhysicsScene());
...
bool UOHPACManager::VerifyBonePhysicsSetup(FName BoneName) {
    if (!SkeletalMesh || !PhysicalAnimationComponent) {
        SafeLog(FString::Printf(TEXT("VerifyBonePhysicsSetup: Missing components for bone %s"), *BoneName.ToString()),
                true);
        return false;
    }
    ...
    if (BoneIndex == INDEX_NONE) {
        SafeLog(FString::Printf(TEXT("VerifyBonePhysicsSetup: Bone %s not found in skeleton"), *BoneName.ToString()),
                true);
        return false;
    }
    ...
    FBodyInstance* Body = GetBodyInstanceDirect(BoneName);
    if (!Body) {
        SafeLog(FString::Printf(TEXT("VerifyBonePhysicsSetup: No body instance for bone %s"), *BoneName.ToString()),
                true);
        return false;
    }
    ...
    if (!Body->IsValidBodyInstance()) {
        SafeLog(
            FString::Printf(TEXT("VerifyBonePhysicsSetup: Invalid body instance for bone %s"), *BoneName.ToString()),
            true);
    ...
    if (CachedPhysicsAsset) {
        ...
        if (!bFoundInAsset) {
            SafeLog(FString::Printf(TEXT("VerifyBonePhysicsSetup: Bone %s not in physics asset"), *BoneName.ToString()),
                    true);
            return false;
        }
    }
                    ...
                    for (const FOHActiveBlend& Blend : *ActiveBlendsForBone) {
                        if (!Blend.IsComplete()) {
                            FPhysicalAnimationData BlendedProfile = UOHSkeletalPhysicsUtils::LerpProfiles(
                                Blend.PhysicsProfile_Start, Blend.PhysicsProfile_Target,
                                Blend.PhysicsBlendWeight_Current);
                            StrongestProfile =
                                UOHSkeletalPhysicsUtils::GetStrongerProfile(StrongestProfile, BlendedProfile);
                        }
                    }
...
void UOHPACManager::VerifyPhysicsStateInternal() {
    ...
    for (const auto& Pair : ActiveBlends) {
        ...
        if (!Body->IsValidBodyInstance() && bShouldSimulate) {
            IssuesFound++;
            SafeLog(FString::Printf(TEXT("Invalid body instance for simulating bone %s"), *BoneName.ToString()), true);
    ...
    if (IssuesFound > 0) {
        SafeLog(FString::Printf(TEXT("Physics verification: Found %d issues, fixed %d"), IssuesFound, IssuesFixed));
        ...
        if (IssuesFixed > 0 && bAutoFixStaleBodies) {
            ...
            SkeletalMesh->UpdateKinematicBonesToAnim(SkeletalMesh->GetEditableComponentSpaceTransforms(),
                                                     ETeleportType::None, true);
        }
    }
}

void UOHPACManager::EnableBonePhysicsSimple(FName BoneName, EOHPhysicsStrength Strength, bool bIncludeChain) {
...
void UOHPACManager::DisableBonePhysicsSimple(FName BoneName, bool bIncludeChain) {
...
void UOHPACManager::SetCategorySimulation(EOHBoneCategory Category, bool bEnable, const FPhysicalAnimationData& Profile,
    ...
    SafeLog(FString::Printf(TEXT("Setting simulation for category %s (%d bones): %s"),
                            *UEnum::GetValueAsString(Category), CategoryBones.Num(),
...
bool UOHPACManager::VerifyComponentSetup(FString& OutReport) {
    ...
    if (!SkeletalMesh) {
        ...
        OutReport +=
            FString::Printf(TEXT("  - Tick Enabled: %s\n"),
                            SkeletalMesh->PrimaryComponentTick.IsTickFunctionEnabled() ? TEXT("Yes") : TEXT("No"));
        OutReport +=
            FString::Printf(TEXT("  - Blend Physics: %s\n"), SkeletalMesh->bBlendPhysics ? TEXT("Yes") : TEXT("No"));
    }
...
void UOHPACManager::EnsurePhysicsTickEnabled() {
    ...
    SafeLog(FString::Printf(TEXT("  - Blend Physics: %s"), SkeletalMesh->bBlendPhysics ? TEXT("Yes") : TEXT("No")));
...
float UOHPACManager::CalculateCombinedBlendWeight(const TArray<FOHActiveBlend>& Blends,
    ...
    if (Blends.Num() == 1) {
        ...
        OutCombinedProfile = UOHSkeletalPhysicsUtils::LerpProfiles(
            Blend.PhysicsProfile_Start, Blend.PhysicsProfile_Target, Blend.PhysicsBlendWeight_Current);
    ...
    if (bEnabled) {
        ...
        if (Params.bAffectChain) {
            ...
            if (Params.FilterBones.Num() > 0) {
                ChainBones = ChainBones.FilterByPredicate(
                    [&Params](const FName& Bone) { return Params.FilterBones.Contains(Bone); });
            }
        } else {
        ...
        for (const FName& ChainBone : ChainBones) {
            ...
            bool bHasProvidedProfile =
                (Params.Profile.OrientationStrength > 0.0f || Params.Profile.PositionStrength > 0.0f);
            ...
            if (Params.bIsPermanent) {
                ...
                SafeLog(FString::Printf(TEXT("Updated baseline for %s: weight=%.3f"), *ChainBone.ToString(),
                                        Baseline.DefaultWeight));
            }
            ...
            if (!IsBoneSimulating(ChainBone) && NewBlend.PhysicsBlendWeight_Target > 0.001f) {
                SetBonePhysicalAnimation_Internal(ChainBone, true, NewBlend.PhysicsProfile_Target, false,
                                                  Params.FilterBones, Params.bEnableCollision, true,
        ...
        SafeLog(FString::Printf(TEXT("Set physics blend for %s: Target=%.3f, Permanent=%s, ChainCount=%d"),
                                *BoneName.ToString(), Params.TargetAlpha,
                                Params.bIsPermanent ? TEXT("Yes") : TEXT("No"), ChainBones.Num()));
    } else {
        // DISABLE BLEND
        ...
        for (const FName& ChainBone : ChainBones) {
            ...
            if (CurrentWeight > 0.005f) {
                ...
                SafeLog(FString::Printf(TEXT("Started disable blend for %s from weight %.3f"), *ChainBone.ToString(),
                                        CurrentWeight));
            } else {
            ...
            if (Blend.IsComplete() || Blend.bForceComplete) {
                ...
                if (Blend.bReturnToBaseline && !Blend.bIsBlendOutOnly) {
                    ...
                    if (Baseline && Baseline->bHasBaseline && Baseline->DefaultWeight > 0.001f) {
                        ...
                        SafeLog(FString::Printf(TEXT("Returning %s to baseline weight %.3f"), *BoneName.ToString(),
                                                Baseline->DefaultWeight));
    ...
    for (auto& Pair : ActiveBlends) {
        ...
        if (FBodyInstance* Body = GetBodyInstanceDirect(BoneName)) {
            if (Blends.Num() > 0) {
                ...
                SafeLog(FString::Printf(TEXT("Disabling physics for %s (no active blends)"), *BoneName.ToString()));
...
void UOHPACManager::UpdateBlendState(FOHActiveBlend& Blend, float DeltaTime) {
    ...
    if (Blend.StartTime <= 0.0f) {
        ...
        SafeLog(FString::Printf(TEXT("Blend initialized for %s: StartWeight=%.3f, TargetWeight=%.3f, IsPermanent=%s"),
                                *Blend.BoneName.ToString(), Blend.PhysicsBlendWeight_Start,
                                Blend.PhysicsBlendWeight_Target, Blend.bIsPermanent ? TEXT("YES") : TEXT("NO")));
    }
...
void UOHPACManager::ApplyBlendToBody(FName BoneName, const TArray<FOHActiveBlend>& Blends, float CombinedWeight,
    ...
    if (!bIsSimulating && bShouldSimulate) {
        ...
        if (bIsRootOfPropagation) {
            PhysicalAnimationComponent->ApplyPhysicalAnimationSettingsBelow(BoneName, CombinedProfile);
            SafeLog(FString::Printf(TEXT("Applied profile BELOW %s (native propagation)"), *BoneName.ToString()));
        } else if (!bUsedNativePropagation) {
            PhysicalAnimationComponent->ApplyPhysicalAnimationSettings(BoneName, CombinedProfile);
            SafeLog(FString::Printf(TEXT("Applied profile to %s (direct)"), *BoneName.ToString()));
        }
    ...
    if (bIsSimulating) {
        ...
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("Weight %.3f on %s (Updated Profile: %s)"), CombinedWeight,
                                    *BoneName.ToString(), bNeedsProfileUpdate ? TEXT("YES") : TEXT("NO")));
        }
        ...
        if (Blends.Num() > 0) {
            ...
            if (Blend.IsComplete()) {
                ...
                if (Blend.IsCompleteAtTime(WorldTime)) {
                    ...
                    SafeLog(FString::Printf(TEXT("Removing stuck blend for %s (elapsed: %.2f)"), *BoneName.ToString(),
                                            Blend.ElapsedTime));
                    bShouldRemove = true;
                }
            }
...
void UOHPACManager::UpdateMotionTracking(float DeltaTime) {
    ...
    for (const FName& BoneName : SimulatableBones) {
        ...
        MotionData->AddMotionSample(BoneTransform.GetLocation(), // WorldPos
...
void UOHPACManager::SetBoneBaseline(FName BoneName, float Weight, const FPhysicalAnimationData& Profile) {
    ...
    SafeLog(FString::Printf(TEXT("Manually set baseline for %s: weight=%.3f"), *BoneName.ToString(), Weight));
}
...
TArray<FName> UOHPACManager::GetBoneChain(FName RootBone, int32 MaxDepth) const {
    ...
    if (RootIndex == INDEX_NONE) {
        SafeLog(FString::Printf(TEXT("GetBoneChain: Bone %s not found in skeleton"), *RootBone.ToString()), true);
        return Chain;
    }
    ...
    while (!BonesToProcess.IsEmpty()) {
        ...
        for (int32 ChildIndex : Children) {
            ...
            if (SimulatableBones.Contains(ChildBoneName) || TrackedBones.Contains(ChildBoneName)) {
                ...
                SafeLog(
                    FString::Printf(TEXT("GetBoneChain: Skipping non-simulatable bone %s"), *ChildBoneName.ToString()));
            }
        }
    }

    if (bVerboseLogging) {
        SafeLog(FString::Printf(TEXT("GetBoneChain: Found %d bones in chain from %s (MaxDepth=%d)"), Chain.Num(),
...
FBodyInstance* UOHPACManager::GetBodyInstanceDirect(FName BoneName) const {
    ...
    // Validate
    if (!Body || !Body->IsValidBodyInstance()) {
        SafeLog(FString::Printf(TEXT("GetBodyInstanceDirect: Bone %s not found or invalid"), *BoneName.ToString()),
                true);
        return nullptr;
    }
...
void UOHPACManager::BuildConstraintData() {
    ...
    for (const auto& Pair : ConstraintLookupMap) {
        ...
        if (Lookup.AsChild.Num() > 1) {
            ...
            SafeLog(
                FString::Printf(TEXT("Bone %s has %d parent constraints"), *Pair.Key.ToString(), Lookup.AsChild.Num()),
                false);
        }
    }

    SafeLog(FString::Printf(TEXT("Built constraint data: %d total constraints, %d bones tracked, %d PAC constraints, "
                                 "%d bones with multiple constraints"),
...
bool UOHPACManager::HasPhysicalAnimationDrives(const FConstraintInstance* Constraint) {
    ...
    if (Profile.AngularDrive.AngularDriveMode == EAngularDriveMode::SLERP) {
        if (Profile.AngularDrive.SlerpDrive.bEnablePositionDrive ||
            Profile.AngularDrive.SlerpDrive.bEnableVelocityDrive) {
            bHasAngularDrive =
                Profile.AngularDrive.SlerpDrive.Stiffness > 0.0f || Profile.AngularDrive.SlerpDrive.Damping > 0.0f;
        }
    } else if (Profile.AngularDrive.AngularDriveMode == EAngularDriveMode::TwistAndSwing) {
        if (Profile.AngularDrive.SwingDrive.bEnablePositionDrive ||
            Profile.AngularDrive.SwingDrive.bEnableVelocityDrive) {
            bHasAngularDrive =
                Profile.AngularDrive.SwingDrive.Stiffness > 0.0f || Profile.AngularDrive.SwingDrive.Damping > 0.0f;
        }
    }
...
FOHConstraintDriveData UOHPACManager::ExtractConstraintDriveData(const FConstraintInstance* Constraint) {
    ...
    DriveData.bLinearXDriveEnabled =
        Profile.LinearDrive.XDrive.bEnablePositionDrive || Profile.LinearDrive.XDrive.bEnableVelocityDrive;
    DriveData.bLinearYDriveEnabled =
        Profile.LinearDrive.YDrive.bEnablePositionDrive || Profile.LinearDrive.YDrive.bEnableVelocityDrive;
    DriveData.bLinearZDriveEnabled =
        Profile.LinearDrive.ZDrive.bEnablePositionDrive || Profile.LinearDrive.ZDrive.bEnableVelocityDrive;
    ...
    if (Profile.AngularDrive.AngularDriveMode == EAngularDriveMode::SLERP) {
        ...
        DriveData.bAngularSlerpDriveEnabled = Profile.AngularDrive.SlerpDrive.bEnablePositionDrive ||
                                              Profile.AngularDrive.SlerpDrive.bEnableVelocityDrive;
        ...
        DriveData.bAngularSwingDriveEnabled = Profile.AngularDrive.SwingDrive.bEnablePositionDrive ||
                                              Profile.AngularDrive.SwingDrive.bEnableVelocityDrive;
        DriveData.bAngularTwistDriveEnabled = Profile.AngularDrive.TwistDrive.bEnablePositionDrive ||
                                              Profile.AngularDrive.TwistDrive.bEnableVelocityDrive;
...
bool UOHPACManager::ValidateSetup(TArray<FName>& OutMissingBones, TArray<FName>& OutInstancesWithoutBodies,
    ...
    if (!ValidateConstraintInstances(OutMissingConstraints, OutRuntimeConstraintsNotInAsset,
...
bool UOHPACManager::ValidatePhysicsAsset(TArray<FName>& OutMissingBones, TArray<FName>& OutInstancesWithoutBodies,
                                         ...
                                         TArray<FName>& OutRuntimeConstraintsNotInAsset,
                                         TArray<FName>& OutMismatchedConstraints) const {
    return ValidateSetup(OutMissingBones, OutInstancesWithoutBodies, OutMissingConstraints,
...
void UOHPACManager::DrawDebugOverlay() const {
    ...
    for (const FName& BoneName : SimulatableBones) {
        ...
        if (bShowBlendWeightText) {
            ...
            if (BlendWeight > 0.01f) {
                ...
                DrawDebugString(GetWorld(), BoneLocation + FVector(0, 0, 15), WeightText, nullptr, FColor::White, 0.0f,
                                true);
            }
        }
        ...
        if (bShowPACProfileValues && IsBoneDrivenByPhysicalAnimation(BoneName)) {
            ...
            FString ProfileText = FString::Printf(TEXT("P:%.0f O:%.0f"), CurrentProfile.PositionStrength,
                                                  CurrentProfile.OrientationStrength);
            DrawDebugString(GetWorld(), BoneLocation + FVector(0, 0, 25), ProfileText, nullptr, FColor::Cyan, 0.0f,
                            true);
        }
        ...
        if (Body->IsInstanceSimulatingPhysics()) {
            ...
            if (!Velocity.IsNearlyZero()) {
                DrawDebugLine(GetWorld(), BoneLocation, BoneLocation + Velocity * 0.1f, FColor::Cyan, false, -1.0f, 0,
                              2.0f);
            }
        }
        ...
        if (const FOHConstraintData* ConstraintData = GetConstraintData(BoneName)) {
            if (ConstraintData->CurrentStrain > 0.5f) {
                ...
                DrawDebugString(GetWorld(), BoneLocation + FVector(0, 0, 10),
                                FString::Printf(TEXT("Strain: %.2f"), ConstraintData->CurrentStrain), nullptr,
                                StrainColor, 0.0f, true);
            }
        }
    }
    ...
    for (const auto& Pair : ActiveBlends) {
        for (const FOHActiveBlend& Blend : Pair.Value) {
            if (!Blend.IsComplete()) {
                FString BlendInfo = FString::Printf(TEXT("%s: %s (%.1f%%)"), *Blend.BoneName.ToString(),
                                                    *UEnum::GetValueAsString(Blend.CurrentPhase),
                                                    Blend.PhysicsBlendWeight_Current * 100.0f);

                DrawDebugString(GetWorld(), FVector::ZeroVector, BlendInfo, nullptr, FColor::White, 0.0f, true, 1.0f);
                YOffset += 20;
            }
        }
    }
...
void UOHPACManager::LogBlendState(FName BoneName) const {
    ...
    for (int32 i = 0; i < Blends->Num(); i++) {
        ...
        UE_LOG(LogPACManager, Warning,
               TEXT("Blend[%d] - CurrentPhase: %s, Alpha: %.4f, PhysicsBlendWeight_Start: %.4f, "
                    "PhysicsBlendWeight_Target: %.4f, ElapsedTime: %.2f"),
               i, *UEnum::GetValueAsString(Blend.CurrentPhase), Blend.PhysicsBlendWeight_Current,
               Blend.PhysicsBlendWeight_Start, Blend.PhysicsBlendWeight_Target, Blend.ElapsedTime);
    }
}
        ...
        for (const FOHActiveBlend& Blend : Blends) {
            if (!Blend.IsComplete()) {
                UE_LOG(LogPACManager, Warning, TEXT("  - CurrentPhase: %s, Alpha: %.2f, Tag: %s"),
                       *UEnum::GetValueAsString(Blend.CurrentPhase), Blend.PhysicsBlendWeight_Current,
...
void UOHPACManager::DebugBodyPhysicsStates() {
    ...
    for (const FName& BoneName : SimulatableBones) {
        ...
        UE_LOG(LogPACManager, Warning, TEXT("%s: Sim=%s, PAC=%s, BlendWeight=%.2f"), *BoneName.ToString(),
               bSimulating ? TEXT("Yes") : TEXT("No"), bHasPAC ? TEXT("Yes") : TEXT("No"), BlendWeight);
    }
}
...
FVector UOHPACManager::GetBoneVelocity(FName BoneName, EOHReferenceSpace Space, float SmoothingAlpha) const {
    ...
    if (!MotionData) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("GetBoneVelocity: No motion data found for bone '%s'"), *BoneName.ToString()),
                    false);
        }
        return FVector::ZeroVector;
    }
    ...
    if (!MotionData->GetLatestSample()) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("GetBoneVelocity: No samples available for bone '%s'"), *BoneName.ToString()),
                    false);
        }
        return FVector::ZeroVector;
    }
    ...
    if (ResultVelocity.Size() > MaxReasonableVelocity) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("GetBoneVelocity: Clamping excessive velocity %.1f for bone '%s'"),
                                    ResultVelocity.Size(), *BoneName.ToString()),
                    true);
        }
...
FVector UOHPACManager::GetBoneAcceleration(FName BoneName, EOHReferenceSpace Space, float SmoothingAlpha) const {
    ...
    if (!MotionData) {
        if (bVerboseLogging) {
            SafeLog(
                FString::Printf(TEXT("GetBoneAcceleration: No motion data found for bone '%s'"), *BoneName.ToString()),
                false);
        }
        return FVector::ZeroVector;
    }
    ...
    if (!MotionData->GetLatestSample()) {
        if (bVerboseLogging) {
            SafeLog(
                FString::Printf(TEXT("GetBoneAcceleration: No samples available for bone '%s'"), *BoneName.ToString()),
                false);
        }
        return FVector::ZeroVector;
    }
    ...
    if (SmoothingAlpha > KINDA_SMALL_NUMBER) {
        ...
        if (MotionData->GetHistoryDepth() >= 2) {
            ...
            FVector RawAcceleration = (Space == EOHReferenceSpace::LocalSpace) ? MotionData->GetAcceleration(true)
                                                                               : MotionData->GetAcceleration(false);
            ...
            if (FVector* PrevSmoothed = PreviousSmoothedAcceleration.Find(BoneName)) {
            ...
            ResultAcceleration = (Space == EOHReferenceSpace::LocalSpace) ? MotionData->GetAcceleration(true)
                                                                          : MotionData->GetAcceleration(false);
        }
    } else {
        ...
        default:
            if (bVerboseLogging) {
                SafeLog(
                    FString::Printf(TEXT("GetBoneAcceleration: Unknown reference space %d, defaulting to WorldSpace"),
                                    static_cast<int32>(Space)),
                    true);
            }
            ResultAcceleration = MotionData->GetAcceleration(false);
    ...
    if (!ResultAcceleration.ContainsNaN()) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(
                        TEXT("GetBoneAcceleration: Non-finite acceleration detected for bone '%s', returning zero"),
                        *BoneName.ToString()),
                    true);
        }
        return FVector::ZeroVector;
    ...
    if (ResultAcceleration.Size() > MaxReasonableAcceleration) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("GetBoneAcceleration: Clamping excessive acceleration %.1f for bone '%s'"),
                                    ResultAcceleration.Size(), *BoneName.ToString()),
                    true);
        }
...
void UOHPACManager::ProcessImpactPushback(const FVector& ImpactForce, const FVector& ImpactLocation,
                                          FName ImpactedBone) {
    ...
    if (ImpactMagnitude < MinPushbackThreshold) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("ProcessImpactPushback: Impact force %.1f below threshold %.1f"),
                                    ImpactMagnitude, MinPushbackThreshold),
                    false);
        }
        return;
    }
    ...
    if (!CharacterRelativePushback.ContainsNaN() && CharacterRelativePushback.Size() > KINDA_SMALL_NUMBER) {
        ...
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("ProcessImpactPushback: Applied unified pushback - WorldDir=%s, "
                                         "CharRelative=(F:%.2f,R:%.2f), Magnitude=%.1f"),
                                    *PushbackDirection.ToString(), CharacterRelativePushback.X,
                                    CharacterRelativePushback.Y, PushbackMagnitude),
                    false);
        }
    } else {
    ...
    if (bVerboseLogging) {
        if (FMath::Abs(HorizontalPushback.Y) > FMath::Abs(HorizontalPushback.X) * 3.0f) {
            SafeLog(FString::Printf(TEXT("WARNING: Unusual Y-axis dominance in pushback: X=%.2f Y=%.2f"),
                                    HorizontalPushback.X, HorizontalPushback.Y),
                    true);
        }
    }
    ...
    if (FBodyInstance* Body = GetBodyInstanceDirect(BoneName)) {
        if (Body->IsInstanceSimulatingPhysics()) {
            ...
            if (bVerboseLogging && Impulse.Size() > 100.0f) {
                SafeLog(FString::Printf(TEXT("Applied impulse %.1f to %s"), Impulse.Size(), *BoneName.ToString()));
            }
        }
    }
}

FVector2D UOHPACManager::CalculateMovementFromImpact(const FHitResult& Hit, const FVector& RelativeVelocity,
...
void UOHPACManager::BlendPhysicalAnimationStrength(float Target, float BlendDuration) {
    PACStrengthBlendState.StartBlend(GetPhysicalAnimationStrengthMultiplier(), Target, BlendDuration,
...
void UOHPACManager::ApplyImpactFlinch(float ImpactMagnitude) {
    ...
    if (!FMath::IsFinite(ImpactMagnitude) || ImpactMagnitude < MinImpactFlinchThreshold) {
        if (bVerboseLogging) {
            SafeLog(FString::Printf(TEXT("ApplyImpactFlinch: Impact magnitude %.1f below threshold %.1f"),
                                    ImpactMagnitude, MinImpactFlinchThreshold),
                    false);
        }
        return;
    }
    ...
    float FlinchIntensity = FMath::GetMappedRangeValueClamped(
        FVector2D(MinImpactFlinchThreshold, MaxImpactFlinchThreshold), FVector2D(0.1f, 1.0f), ImpactMagnitude);
    ...
    float TargetStrength =
        FMath::Clamp(MaxPhysicalAnimationStrength * (1.0f - FlinchIntensity * FlinchStrengthMultiplier),
                     MinPhysicalAnimationStrength, CurrentStrength);

    // === ENHANCED BLEND DURATION CALCULATION ===
    float BlendDuration = FMath::GetMappedRangeValueClamped(
        FVector2D(0.1f, 1.0f), FVector2D(FlinchMinDuration, FlinchMaxDuration), FlinchIntensity);
    ...
    for (const FName& BoneName : FlinchBones) {
        ...
        FVector RandomComponent = FVector(FMath::RandRange(-1.0f, 1.0f), FMath::RandRange(-1.0f, 1.0f),
        ...
        if (LastImpactDirection.Size() > KINDA_SMALL_NUMBER) {
            ...
            FlinchDirection = UOHLocomotionUtils::BlendDirectionalInput(RandomComponent, AwayFromImpact, 0.6f);
        } else {
    ...
    GetWorld()->GetTimerManager().SetTimer(FlinchRecoveryTimer, this,
                                           &UOHPACManager::RestorePhysicalAnimationStrengthSmooth,
                                           BlendDuration + FlinchRecoveryDelay, false);

    if (bVerboseLogging) {
        SafeLog(FString::Printf(TEXT("Impact Flinch Applied: Magnitude=%.1f, Intensity=%.2f, TargetStrength=%.2f, "
                                     "Duration=%.2f, Bones=%d"),
                                ImpactMagnitude, FlinchIntensity, TargetStrength, BlendDuration, FlinchBones.Num()),
                false);
    }
...
void UOHPACManager::RestorePhysicalAnimationStrengthSmooth() {
    ...
    if (UOHMovementComponent* MovementComp = GetMovementComponent()) {
        ...
        if (MovementComp->bCombatPushbackActive) {
            ...
            if (bVerboseLogging) {
                UE_LOG(LogPACManager, VeryVerbose, TEXT("Delaying restoration due to active combat pushback"));
            }
        }
    }
    ...
    if (bVerboseLogging) {
        SafeLog(FString::Printf(TEXT("PAC Strength Restoration: Target=%.2f, Duration=%.2f, Combat=%s"), TargetStrength,
                                BlendDuration, MyCombatState.bIsAttacking ? TEXT("Active") : TEXT("Inactive")),
                false);
    }
}
...
EOHShapeType UOHPACManager::DetermineBoneShape(FName BoneName) {
    ...
    if (BoneStr.Contains("spine") || BoneStr.Contains("chest") || BoneStr.Contains("pelvis") ||
        BoneStr.Contains("abdomen")) {
        return EOHShapeType::Human_Torso;
    }
    if (BoneStr.Contains("arm") || BoneStr.Contains("leg") || BoneStr.Contains("thigh") || BoneStr.Contains("calf")) {
...
FVector UOHPACManager::GetBoneBounds(FName BoneName) const {
    ...
    if (BoneIndex != INDEX_NONE) {
        ...
        int32 ParentIndex = SkeletalMesh->GetBoneIndex(SkeletalMesh->GetParentBone(BoneName));

        if (ParentIndex != INDEX_NONE) {
            FVector BonePos = SkeletalMesh->GetBoneLocation(BoneName);
            FVector ParentPos = SkeletalMesh->GetBoneLocation(SkeletalMesh->GetParentBone(BoneName));
...
void UOHPACManager::InitializeCombatChains() {
    ...
    for (const FChainDefinition& ChainDef : ChainDefinitions) {
        ...
        for (const FName& BoneName : ChainDef.ChainBones) {
            int32 BoneIndex = SkeletalMesh->GetBoneIndex(BoneName);
            if (BoneIndex == INDEX_NONE) {
                SafeLog(FString::Printf(TEXT("InitializeCombatChains: Bone %s not found"), *BoneName.ToString()), true);
        ...
        if (!bAllBonesValid) {
            SafeLog(FString::Printf(TEXT("Skipping chain %s due to missing bones"), *ChainDef.RootBone.ToString()),
                    true);
            continue;
        }
        ...
        for (int32 i = 0; i < NumBones; i++) {
            ...
            InitialSample.LocalLinearVelocity =
                InitialSample.ComponentTransform.InverseTransformVector(InitialSample.WorldLinearVelocity);
            ...
            MotionData.AddMotionSample(InitialSample.WorldPosition, InitialSample.WorldRotation, ComponentVelocity,
                                       InitialSample.ReferenceVelocity, InitialSample.ComponentTransform,
                                       InitialSample.DeltaTime, InitialSample.TimeStamp);
        ...
        SafeLog(FString::Printf(TEXT("Combat chain initialized: %s with %d bones"), *ChainDef.RootBone.ToString(),
                                NumBones));
    }
    ...
    SafeLog(FString::Printf(TEXT("InitializeCombatChains complete: %d chains initialized"), CombatChains.Num()));
}

void UOHPACManager::UpdateChainMotionData(FOHCombatChainData& Chain, float DeltaTime) {
    if (Chain.ChainBones.Num() == 0 || !SkeletalMesh || !GetWorld())
        return;
    ...
    FVector ComponentVelocity = FVector::ZeroVector;
    FTransform ComponentTransform = SkeletalMesh->GetComponentTransform();
    FTransform ReferenceFrameTransform = FTransform::Identity; // FIXED: Changed to FTransform
    ...
    // === CRITICAL: ACQUIRE REFERENCE FRAME TRANSFORM BASED ON CHAIN CONFIGURATION ===
    if (!Chain.MotionReferenceFrameBone.IsNone()) {
        int32 RefBoneIndex = SkeletalMesh->GetBoneIndex(Chain.MotionReferenceFrameBone);
        if (RefBoneIndex != INDEX_NONE) {
            // Get the reference bone's transform, not velocity
            ReferenceFrameTransform = SkeletalMesh->GetBoneTransform(RefBoneIndex);
        }
    }
    ...
    for (int32 i = 0; i < Chain.ChainBones.Num(); i++) {
        ...
        if (const FOHBoneMotionData* MainMotionData = BoneMotionMap.Find(BoneName)) {
            ...
            if (MainMotionData->GetHistoryDepth() > 0 && MainMotionData->GetLatestSample()) {
                *ChainMotionData = *MainMotionData;
                ...
                if (FOHMotionFrameSample* MutableSample =
                        const_cast<FOHMotionFrameSample*>(ChainMotionData->GetLatestSample())) {
                    // FIXED: Pass two transforms, not transform and velocity
                    MutableSample->RecalculateReferenceSpaces(ComponentTransform, ReferenceFrameTransform);
        ...
        // === PRIORITY 2: ENHANCED MANUAL MOTION TRACKING WITH MULTI-SPACE CALCULATION ===
            ...
            if (!WorldPos.ContainsNaN() && !WorldRot.ContainsNaN() && !WorldVelocity.ContainsNaN() &&
                !AngularVelocity.ContainsNaN()) {
                // Get reference frame velocity for the motion sample
                ...
                ChainMotionData->AddMotionSample(
                    WorldPos, WorldRot, ComponentVelocity,
                    ReferenceFrameVelocity, // This is velocity for the AddMotionSample method
                    ComponentTransform, DeltaTime, CurrentTime);
...
void UOHPACManager::UpdateCombatChainStates(float DeltaTime) {
    if (!SkeletalMesh || !bEnableCollisionImpactSystem || CombatChains.Num() == 0)
        return;

    float CurrentTime = GetWorld()->GetTimeSeconds();

    // Get component transform once
    FTransform ComponentTransform = SkeletalMesh->GetComponentTransform();

    // Process each combat chain
    for (auto& ChainPair : CombatChains) {
        FOHCombatChainData& Chain = ChainPair.Value;

        // Get reference transform for this chain
        FTransform ReferenceTransform = FTransform::Identity;
        if (!Chain.MotionReferenceFrameBone.IsNone()) {
            int32 RefBoneIndex = SkeletalMesh->GetBoneIndex(Chain.MotionReferenceFrameBone);
            if (RefBoneIndex != INDEX_NONE) {
                ReferenceTransform = SkeletalMesh->GetBoneTransform(RefBoneIndex);
            }
        }

        // Recalculate reference spaces
        Chain.RecalculateReferenceSpaces(ComponentTransform, ReferenceTransform);
        ...
        // Update trajectory if attacking
        if (Chain.AttackConfidence > MinAttackConfidenceThreshold && Chain.HasSignificantMotion()) {
            ...
            Chain.ClearPredictedTrajectory();
            Chain.ClearBezierControlPoints();
        ...
        if (Chain.bIsAttacking) {
            ...
            if (bVerboseLogging && Chain.TimeInAttack < DeltaTime * 2.0f) {
                UE_LOG(LogPACManager, Warning,
                       TEXT("Chain Attack Initiated [%s] - Space: %s, Confidence: %.2f, Speed: %.1f, Energy: %.1f"),
                       *Chain.RootBone.ToString(), *UEnum::GetValueAsString(Chain.MotionAnalysisSpace),
                       Chain.AttackConfidence, Chain.GetEffectiveMotionSpeed(), Chain.ChainKineticEnergy);
            }
        } else {
...
// ============================================================================
// PART 2: COMPLETE UpdateChainTrajectory WITH ALL PREDICTION MODELS
// ============================================================================

void UOHPACManager::UpdateChainTrajectory(FOHCombatChainData& Chain, float DeltaTime) {
    // Clear trajectory using accessor methods
    Chain.ClearPredictedTrajectory();
    Chain.ClearBezierControlPoints();
    Chain.TrajectoryConfidence = 0.0f;
    ...
    // === VALIDATION WITH REFERENCE SPACE CONTEXT ===
    if (!MotionData) {
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, VeryVerbose,
    ...
    if (!Chain.HasSignificantMotion()) {
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, VeryVerbose,
                   TEXT("Trajectory prediction skipped for chain '%s': No significant motion"),
    ...
    if (MotionQuality < PredictionQualityThreshold) {
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, VeryVerbose,
                   TEXT("Trajectory prediction skipped for chain '%s': Low motion quality (%.2f < %.2f)"),
                   *Chain.RootBone.ToString(), MotionQuality, PredictionQualityThreshold);
        }
        return;
    }

    // === PREDICTION PARAMETERS WITH REFERENCE SPACE OPTIMIZATION ===
    const float PredictionTime = FMath::GetMappedRangeValueClamped(FVector2D(0.3f, 1.0f), // Confidence range
                                                                   FVector2D(0.2f, 0.4f), // Prediction time range
                                                                   Chain.AttackConfidence);
    const int32 NumSamples = 20;

    // === MULTI-MODEL PREDICTION WITH INTELLIGENT SELECTION ===
    TArray<TArray<FVector>> ModelPredictions;
    TArray<float> ModelWeights;
    TArray<FString> ModelNames;

    // === MODEL 1: KALMAN FILTER WITH REFERENCE SPACE VALIDATION ===
    if (MotionData->GetHistoryDepth() >= 5 && Chain.MotionCoherence > 0.3f) {
        ...
        if (!KalmanState.Position.ContainsNaN() && !KalmanState.Velocity.ContainsNaN() &&
            ...
            for (int32 i = 0; i <= NumSamples; i++) {
                if (i > 0) {
                    // Predict ahead and update Kalman filter
                    float PredictAheadTime = i * TimeStep;
                    FVector PredictedMeasurement =
                        MotionData->PredictFuturePosition(PredictAheadTime, Chain.MotionAnalysisSpace, true);

                    if (!PredictedMeasurement.ContainsNaN() && PredictedMeasurement.Size() < 100000.0f) {
                        KalmanState = UOHAlgoUtils::PredictKalmanState(KalmanState, PredictedMeasurement, TimeStep);
                    } else {
            ...
            if (bKalmanSuccess && KalmanPath.Num() > 0) {
                ModelPredictions.Add(KalmanPath);
                float KalmanWeight = 0.5f * Chain.MotionCoherence * MotionQuality;
                ModelWeights.Add(KalmanWeight);

                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Kalman prediction success: %d samples, weight=%.3f"),
                           KalmanPath.Num(), KalmanWeight);
                }
            }
        }
    }

    // === MODEL 2: BEZIER CURVE PREDICTION ===
    if (MotionData->GetHistoryDepth() >= 3) {
        TArray<FVector> BezierPath = MotionData->GetBezierPredictionPath(PredictionTime, true, NumSamples);

        if (BezierPath.Num() > 0) {
            bool bValidPath = true;
            for (const FVector& Point : BezierPath) {
            ...
            if (bValidPath) {
                ...
                ModelNames.Add(TEXT("Bezier"));

                // Weight based on trajectory smoothness
                float TrajectoryDeviation = UOHAlgoUtils::GetTrajectoryDeviation(*MotionData, 5);
                float BezierWeight = FMath::Clamp(1.0f - (TrajectoryDeviation / 100.0f), 0.1f, 1.0f) * 0.4f;
                ModelWeights.Add(BezierWeight);

                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Bezier prediction: deviation=%.1f, weight=%.3f"),
                           TrajectoryDeviation, BezierWeight);
                }
            }
        }
    }

    // === MODEL 3: QUADRATIC PHYSICS PREDICTION ===
    if (MotionData->GetHistoryDepth() >= 2) {
        if (const FOHMotionFrameSample* Latest = MotionData->GetLatestSample()) {
            TArray<FVector> QuadraticPath;
            for (int32 i = 0; i <= NumSamples; i++) {
                float Time = static_cast<float>(i) / static_cast<float>(NumSamples) * PredictionTime;
                FVector PredictedPos = UOHAlgoUtils::PredictQuadratic(*Latest, Time);

                if (!PredictedPos.ContainsNaN() && PredictedPos.Size() < 100000.0f) {
                    QuadraticPath.Add(PredictedPos);
                }
            }

            if (QuadraticPath.Num() > 0) {
                ModelPredictions.Add(QuadraticPath);
                ModelNames.Add(TEXT("Quadratic"));

                // Weight based on acceleration consistency
                float AccelMag = Latest->WorldLinearAcceleration.Size();
                float QuadraticWeight = FMath::Clamp(AccelMag / 5000.0f, 0.1f, 0.5f) * 0.3f;
                ModelWeights.Add(QuadraticWeight);
            }
        }
    }

    // === MODEL 4: DAMPED PREDICTION FOR DECELERATION ===
    if (Chain.AttackJerkMagnitude < 0.0f) // Negative jerk indicates deceleration
    {
        if (const FOHMotionFrameSample* Latest = MotionData->GetLatestSample()) {
            TArray<FVector> DampedPath;
            float DampingRatio = FMath::Clamp(FMath::Abs(Chain.AttackJerkMagnitude) / 10000.0f, 0.1f, 0.9f);

            for (int32 i = 0; i <= NumSamples; i++) {
                float Time = static_cast<float>(i) / static_cast<float>(NumSamples) * PredictionTime;
                FVector PredictedPos = UOHAlgoUtils::PredictDamped(*Latest, Time, DampingRatio);

                if (!PredictedPos.ContainsNaN() && PredictedPos.Size() < 100000.0f) {
                    DampedPath.Add(PredictedPos);
                }
            }

            if (DampedPath.Num() > 0) {
                ModelPredictions.Add(DampedPath);
                ModelNames.Add(TEXT("Damped"));
                ModelWeights.Add(0.3f);
            }
        }
    }
    ...
    if (ModelPredictions.Num() > 0) {
        TArray<FVector> CombinedPath;
        CombinedPath.SetNum(NumSamples + 1);

        // Normalize weights
        float TotalWeight = 0.0f;
        for (float Weight : ModelWeights) {
            TotalWeight += Weight;
        }

        if (TotalWeight > 0.0f) {
            for (int32 i = 0; i < ModelWeights.Num(); i++) {
                ModelWeights[i] /= TotalWeight;
            }
        }

        // Combine predictions
        for (int32 SampleIdx = 0; SampleIdx <= NumSamples; SampleIdx++) {
            FVector CombinedPos = FVector::ZeroVector;
            float ValidWeight = 0.0f;

            for (int32 ModelIdx = 0; ModelIdx < ModelPredictions.Num(); ModelIdx++) {
                if (SampleIdx < ModelPredictions[ModelIdx].Num()) {
                    CombinedPos += ModelPredictions[ModelIdx][SampleIdx] * ModelWeights[ModelIdx];
                    ValidWeight += ModelWeights[ModelIdx];
                }
            }

            if (ValidWeight > 0.0f) {
                CombinedPath[SampleIdx] = CombinedPos / ValidWeight;
            } else {
                CombinedPath[SampleIdx] = MotionData->GetCurrentPosition();
            }
        }

        // Set the combined trajectory
        Chain.SetPredictedTrajectory(CombinedPath);

        // Calculate confidence based on model agreement
        float AgreementScore = 1.0f;
        if (ModelPredictions.Num() > 1) {
            float Variance = 0.0f;
            for (int32 i = 0; i < ModelPredictions.Num(); i++) {
                if (ModelPredictions[i].Num() > 0) {
                    float Distance = FVector::Dist(ModelPredictions[i].Last(), CombinedPath.Last());
                    Variance += Distance * Distance;
                }
            }
            Variance /= ModelPredictions.Num();
            AgreementScore = FMath::Clamp(1.0f - (Variance / 10000.0f), 0.0f, 1.0f);
        }

        Chain.TrajectoryConfidence = AgreementScore * MotionQuality * Chain.MotionCoherence;

        // Calculate Bezier control points from the combined path
        TArray<FVector> ControlPoints;
        if (CombinedPath.Num() >= 4) {
            ControlPoints.Add(CombinedPath[0]);
            ControlPoints.Add(CombinedPath[CombinedPath.Num() / 3]);
            ControlPoints.Add(CombinedPath[2 * CombinedPath.Num() / 3]);
            ControlPoints.Add(CombinedPath.Last());
        }
        Chain.SetBezierControlPoints(ControlPoints);

        if (bVerboseLogging) {
            FString ModelList;
            for (int32 i = 0; i < ModelNames.Num(); i++) {
                ModelList += FString::Printf(TEXT("%s(%.2f)"), *ModelNames[i], ModelWeights[i]);
                if (i < ModelNames.Num() - 1)
                    ModelList += TEXT(", ");
            }

            UE_LOG(LogPACManager, Log,
                   TEXT("Trajectory Prediction Complete [%s]: Models=[%s], Confidence=%.3f, Agreement=%.3f"),
                   *Chain.RootBone.ToString(), *ModelList, Chain.TrajectoryConfidence, AgreementScore);
        }
    } else {
        ...
        if (!CurrentPos.ContainsNaN() && !Velocity.ContainsNaN() && !Acceleration.ContainsNaN()) {
            ...
            SimplePath.SetNum(NumSamples + 1);

            for (int32 i = 0; i <= NumSamples; i++) {
                float Time = static_cast<float>(i) / static_cast<float>(NumSamples) * PredictionTime;
                FVector PredictedPos = CurrentPos + Velocity * Time + 0.5f * Acceleration * Time * Time;
                SimplePath[i] = PredictedPos;
            }

            Chain.SetPredictedTrajectory(SimplePath);
            Chain.TrajectoryConfidence = 0.4f * Chain.MotionCoherence;

            if (bVerboseLogging) {
                UE_LOG(LogPACManager, Log, TEXT("Fallback trajectory for chain '%s': %d samples, confidence=%.3f"),
                       *Chain.RootBone.ToString(), SimplePath.Num(), Chain.TrajectoryConfidence);
            }
        }
    }
}
...
void UOHPACManager::AnalyzeChainAttackPattern(FOHCombatChainData& Chain, float DeltaTime) {
    ...
    // Track raw attack direction from primary striking bone
    FVector RawAttackDirection = FVector::ZeroVector;
    ...
    for (const FName& BoneName : Chain.ChainBones) {
        ...
        if (!MotionData || MotionData->GetHistoryDepth() < 3)
            continue;
        ...
        FVector VelocityAnalysis = MotionData->GetVelocity(Chain.MotionAnalysisSpace);
        ...
        if (VelocityWorld.ContainsNaN() || VelocityLocal.ContainsNaN() || VelocityComponent.ContainsNaN() ||
            VelocityAnalysis.ContainsNaN()) {
            continue;
        }

        // Capture raw direction from primary striking bone
        if (BoneName == Chain.GetPrimaryStrikingBone() && !VelocityAnalysis.IsNearlyZero()) {
            RawAttackDirection = VelocityAnalysis.GetSafeNormal();
        }
    ...
    auto CalculateCoherence = [](const TArray<FVector>& Directions,
                                 const TArray<float>& Speeds) -> TPair<float, FVector> {
        if (Directions.Num() < 2)
            return TPair<float, FVector>(0.0f, FVector::ZeroVector);
    ...
    // === CRITICAL: UPDATE BOTH ATTACK DIRECTION FIELDS ===

    // Set raw attack direction (instantaneous)
    if (!RawAttackDirection.IsNearlyZero()) {
        Chain.AttackDirection = RawAttackDirection;
    }

    // Set principal attack direction (coherence-weighted)
    ...
    if (BoneSpeedsWorld.Num() > 0) {
        ...
        if (BoneSpeedsComponent.Num() > 0) {
            ...
            for (float Speed : BoneSpeedsComponent)
                AvgComponentSpeed += Speed;
    ...
    // Check for isolated limb motion
    else if (LocalToWorldRatio > 1.8f || ComponentToWorldRatio > 1.8f) {
        DetectedPattern = EAttackPattern::IsolatedLimb;
        PatternConfidence = FMath::Clamp(
            CoherenceScore * 0.6f + FMath::Clamp(Chain.AttackJerkMagnitude / 12000.0f, 0.0f, 1.0f) * 0.4f, 0.0f, 1.0f);
    }
    // Check for curved attacks using principal direction
        ...
        PatternConfidence = CoherenceScore * 0.3f + FMath::Clamp(Chain.AttackCurvature / 0.5f, 0.0f, 1.0f) * 0.4f +
    ...
    if (!FMath::IsNaN(PatternConfidence)) {
        Chain.AttackConfidence = FMath::FInterpTo(Chain.AttackConfidence, PatternConfidence, DeltaTime, 5.0f);
    }

    // === ENHANCED: ATTACK STATE DETERMINATION WITH CHAIN THRESHOLD ===
    Chain.bIsAttacking =
        (Chain.AttackConfidence > MinAttackConfidenceThreshold) || (bLikelyStrike && Chain.HasSignificantMotion());

    // === COMPREHENSIVE DEBUG LOGGING WITH MULTI-SPACE METRICS ===
    if (bVerboseLogging && Chain.bIsAttacking) {
        const TCHAR* PatternNames[] = {TEXT("None"),      TEXT("Straight"),     TEXT("Hook"),       TEXT("Uppercut"),
                                       TEXT("Defensive"), TEXT("IsolatedLimb"), TEXT("Coordinated")};

        float DirectionDeviation = Chain.GetAttackDirectionDeviation();

        UE_LOG(LogPACManager, Warning,
               TEXT("Attack Pattern [%s]: Type=%s, Conf=%.2f, Coh=[A:%.2f,W:%.2f,L:%.2f,C:%.2f], ")
                   TEXT("Jerk=%.0f, Curve=%.2f, DirDev=%.1f°, Stable=%s"),
               *Chain.RootBone.ToString(), PatternNames[static_cast<uint8>(DetectedPattern)], Chain.AttackConfidence,
               CoherenceScore, WorldCoherence, LocalCoherence, ComponentCoherence, Chain.AttackJerkMagnitude,
               Chain.AttackCurvature, DirectionDeviation, Chain.HasStableAttackDirection() ? TEXT("YES") : TEXT("NO"));
    }
}

void UOHPACManager::CheckChainVsAllBodies(const FOHCombatChainData& AttackingChain, UOHPACManager* DefenderManager) {
    if (!DefenderManager || AttackingChain.GetPredictedTrajectory().Num() < 2)
        return; // FIXED: Use getter

    // Time parameters
    const float TotalPredictTime = 0.3f;
    const float TimeStep = TotalPredictTime / (AttackingChain.GetPredictedTrajectory().Num() - 1); // FIXED: Use getter

    // Gather all candidates along trajectory
    TArray<FCombatTargetCandidate> AllCandidates;

    for (int32 i = 0; i < AttackingChain.GetPredictedTrajectory().Num() - 1; i++) // FIXED: Use getter
    {
        FVector TrajectoryStart = AttackingChain.GetPredictedTrajectory()[i];   // FIXED: Use getter
        FVector TrajectoryEnd = AttackingChain.GetPredictedTrajectory()[i + 1]; // FIXED: Use getter
        ...
        for (const FName& DefenderBone : DefenderManager->GetSimulatableBones()) {
            ...
            FVector DefenderFuturePos =
                DefenderMotion->PredictFuturePosition(CurrentTime, EOHReferenceSpace::WorldSpace, true);

            // High quality motion gets bezier prediction
            if (DefenderMotion->GetMotionQuality() > 0.7f) {
                TArray<FVector> DefenderBezier =
                    UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(*DefenderMotion, CurrentTime);

                if (DefenderBezier.Num() >= 3) {
                    float t = FMath::Clamp(CurrentTime / TotalPredictTime, 0.0f, 1.0f);
                    DefenderFuturePos =
                        UOHAlgoUtils::SampleBezierQuadratic(DefenderBezier[0], DefenderBezier[1], DefenderBezier[2], t);
                }
            }

            // Check distance to trajectory segment
            FVector ClosestPoint = FMath::ClosestPointOnSegment(DefenderFuturePos, TrajectoryStart, TrajectoryEnd);
            ...
            if (Distance <= CheckRadius) {
                ...
                if (Candidate.bIsExtremity) {
                    Candidate.bIsBlocking =
                        IsExtremityProtectingCore(DefenderBone, DefenderManager,
                                                  AttackingChain.AttackDirection // This field should exist
                        );
                }
    ...
    if (AllCandidates.Num() > 0) {
        FCombatTargetCandidate BestTarget = SelectBestTargetFromChain(AttackingChain, DefenderManager, AllCandidates);
        // Process the hit
        ProcessChainHit(AttackingChain, DefenderManager, BestTarget);
    }
}

bool UOHPACManager::ProcessChainHit(const FOHCombatChainData& AttackingChain, UOHPACManager* DefenderManager,
    ...
    if (!Target.ImpactDirection.IsNearlyZero() && Target.ImpactDirection.IsNormalized()) {
        ...
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, VeryVerbose, TEXT("Using EPA-derived impact direction: %s"),
    ...
    // Priority 4: Spatial relationship fallback (geometric)
        ...
        if (!AttackerToTarget.IsNearlyZero()) {
            ...
            if (bVerboseLogging) {
                UE_LOG(LogPACManager, Warning, TEXT("Using geometric fallback direction: %s"),
                       *ImpactDirection.ToString());
            }
        } else {
            ...
            if (AActor* AttackerActor = GetOwner()) {
                ...
                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, Warning, TEXT("Using attacker forward as final fallback: %s"),
                           *ImpactDirection.ToString());
                }
            } else {
                if (bVerboseLogging) {
        ...
        if (UOHMovementComponent* DefenderMovement = DefenderManager->GetMovementComponent()) {
            ...
            if (AActor* AttackerActor = GetOwner()) {
                if (AActor* DefenderActor = DefenderManager->GetOwner()) {
                    FVector AttackerToDefender =
                        (DefenderActor->GetActorLocation() - AttackerActor->GetActorLocation()).GetSafeNormal();
                    ...
                    if (DirectionAlignment < 0.0f) {
                        ...
                        if (bVerboseLogging) {
                            UE_LOG(LogPACManager, Warning,
                                   TEXT("Corrected push direction - was pointing toward attacker"));
                        }
                    }
                }
            }
    ...
    if (bApplySelfPushback && GetMovementComponent()) {
        ...
        if (const FOHBoneMotionData* StrikingMotionData = GetBoneMotionData(StrikingBone)) {
            ...
            if (AttackSpeed > MinAttackSpeed * 0.5f) {
                ...
                bool bIsCommittedAttack =
                    (AttackSpeed > MinAttackSpeed && ForwardAlignment > 0.7f && AttackingChain.AttackConfidence > 0.8f);
                ...
                if (bIsCommittedAttack) {
                    ...
                    FVector AttackerPushDirection = -StrikingMotionData->GetVelocity().GetSafeNormal();
                    ...
                    if (bVerboseLogging) {
                        UE_LOG(LogPACManager, Log, TEXT("Applied reduced self-pushback %.1f (committed attack)"),
                               AttackerPushMagnitude);
                    }
                } else {
                    // Normal self-pushback for glancing or weak strikes
                    FVector AttackerPushDirection = -ImpactDirection;
                    float AttackerPushMagnitude =
                        FinalForce * SelfPushbackMultiplier * (1.0f - ForwardAlignment * 0.5f);
    ...
    // === EVENT BROADCASTING ===
    OnPhysicsBodyHitEvent.Broadcast(SkeletalMesh, StrikingBone, DefenderManager->SkeletalMesh, Target.BoneName,
                                    AttackingChain.ChainMomentum / FMath::Max(AttackingChain.ChainTotalMass, 1.0f),
                                    FHitResult());

    if (bVerboseLogging) {
        SafeLog(FString::Printf(TEXT("Combat Hit Complete: %s->%s, Dir=%s, Force=%.1f, Penetration=%.2f"),
                                *StrikingBone.ToString(), *Target.BoneName.ToString(), *ImpactDirection.ToString(),
                                FinalForce, Target.PenetrationDepth),
                false);
    }
...
void UOHPACManager::CheckChainCombatCollisions(float DeltaTime) {
    ...
    for (AActor* Actor : FoundCharacters) {
        if (Actor == MyActor || !IsValid(Actor))
            continue;
    ...
    for (auto& ChainPair : CombatChains) {
        ...
        if (!AttackingChain.bIsAttacking || AttackingChain.AttackConfidence < MinAttackConfidenceThreshold ||
        ...
        for (const FName& BoneName : AttackingChain.ChainBones) {
            if (const FOHBoneMotionData* MotionData = GetBoneMotionData(BoneName)) {
                if (MotionData->GetSpeed(EOHReferenceSpace::WorldSpace) >
                    MinAttackSpeed * 0.7f) // Slightly lower threshold for detection
                {
                    // Current position
                    ...
                    for (int32 i = 0; i < 4; i++) {
                        ...
                        FVector RadialOffset =
                            FVector(FMath::Cos(Angle) * BoneRadius, FMath::Sin(Angle) * BoneRadius, 0.0f);
        ...
        for (ACharacter* OpponentCharacter : PotentialOpponents) {
            ...
            for (const FName& OpponentBone : OpponentBones) {
                ...
                for (const FVector& HullPoint : AttackHullPoints) {
                    ...
                    float EffectiveStrikeRadius =
                        AttackingChain.EffectiveStrikeRadius + EstimateBoneRadius(OpponentBone);
                ...
                if (bWithinStrikeRange) {
                    ...
                    Candidate.PenetrationDepth = FMath::Max(
                        0.0f, (AttackingChain.EffectiveStrikeRadius + EstimateBoneRadius(OpponentBone)) - MinDistance);
            ...
            if (AllCandidates.Num() > 0) {
                ...
                if (bHitProcessed && bVerboseLogging) {
                    ...
                    float AttackAngle =
                        FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(AttackerForward, AttackerToDefender)));

                    UE_LOG(
                        LogPACManager, Warning,
                        TEXT("Combat Hit Detected: Chain=%s->%s, Bone=%s, Angle=%.1f°, Penetration=%.2f, Direction=%s"),
                        *AttackingChain.RootBone.ToString(), *OpponentCharacter->GetName(),
                        *BestTarget.BoneName.ToString(), AttackAngle, BestTarget.PenetrationDepth,
...
FName UOHPACManager::DetermineBestTargetBone(UOHPACManager* TargetManager, const FVector& PenetrationDirection,
    ...
    for (const FName& BoneName : CandidateBones) {
        ...
        float DirectionalAlignment = UOHCombatUtils::ComputeDirectionalAlignment2D(PenetrationDirection, ToBone);
...
FOHCombatAnalysis UOHPACManager::AnalyzeCombatState() const {
    ...
    for (const auto& ChainPair : CombatChains) {
        ...
        if (!Chain.bIsAttacking || Chain.AttackConfidence < 0.1f) {
            if (bVerboseLogging && Chain.AttackConfidence > 0.05f) {
                UE_LOG(LogPACManager, VeryVerbose, TEXT("Skipping inactive chain '%s': Attacking=%s, Confidence=%.3f"),
                       *Chain.RootBone.ToString(), Chain.bIsAttacking ? TEXT("True") : TEXT("False"),
                       Chain.AttackConfidence);
            }
            continue;
        }
        ...
        for (const FName& BoneName : Chain.ChainBones) {
            if (const FOHBoneMotionData* MotionData = Chain.GetBoneMotionData(BoneName)) {
                ...
                FVector BoneJerk =
                    MotionData->CalculateJerk(Chain.MotionAnalysisSpace == EOHReferenceSpace::LocalSpace);
        ...
        if (FMath::IsFinite(Chain.ChainMotionQuality) && Chain.ChainMotionQuality >= 0.0f &&
            Chain.ChainMotionQuality <= 1.0f) {
        ...
        if (FMath::IsFinite(Chain.TrajectoryConfidence) && Chain.TrajectoryConfidence >= 0.0f &&
            Chain.TrajectoryConfidence <= 1.0f) {
    ...
    Analysis.bIsCommittedAttack = Analysis.bIsAttacking && Analysis.AttackConfidence > 0.7f &&
                                  Analysis.TimeInAttack > 0.2f && Analysis.PeakJerkMagnitude > 8000.0f &&
                                  Analysis.EffectiveMotionSpeed > MinAttackSpeed * 1.5f;

    Analysis.bIsFeintOrGlancing =
        Analysis.bIsAttacking && !Analysis.bIsCommittedAttack &&
        (Analysis.AttackCurvature > 0.3f || Analysis.MotionConsistencyScore < 0.6f || Analysis.TimeInAttack < 0.15f);

    // === SPATIAL ANALYSIS ===
    Analysis.ChainCenterOfMass =
        ActiveChainCount > 0 ? (AccumulatedCenterOfMass / ActiveChainCount) : FVector::ZeroVector;
    ...
    if (Analysis.bIsAttacking) {
        ...
        Analysis.AttackIntensity =
            (SpeedFactor * 0.25f + HighestConfidence * 0.20f + EnergyFactor * 0.20f + JerkFactor * 0.15f +
             QualityFactor * 0.10f + CoordinationFactor * 0.05f + DirectionalFactor * 0.05f) *
            100.0f;
    }
    ...
    if (Analysis.AttackPrincipalDirection.ContainsNaN()) {
        ...
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, Warning,
                   TEXT("AnalyzeCombatState: Invalid AttackPrincipalDirection detected and corrected"));
        }
    }
    if (Analysis.ChainCenterOfMass.ContainsNaN()) {
        ...
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, Warning,
                   TEXT("AnalyzeCombatState: Invalid ChainCenterOfMass detected and corrected"));
        }
    }

    // === COMPREHENSIVE DEBUG LOGGING ===
    if (bVerboseLogging && Analysis.bIsAttacking) {
        UE_LOG(LogPACManager, Log, TEXT("=== ENHANCED COMBAT ANALYSIS ==="));
        UE_LOG(LogPACManager, Log, TEXT("Classification: %s | Confidence: %.2f | Primary Space: %s"),
               Analysis.bIsCommittedAttack ? TEXT("COMMITTED")
                                           : (Analysis.bIsFeintOrGlancing ? TEXT("FEINT") : TEXT("STANDARD")),
               Analysis.AttackConfidence, *UEnum::GetValueAsString(Analysis.PrimaryMotionSpace));
        UE_LOG(LogPACManager, Log,
               TEXT("Multi-Space Speeds: [World:%.1f, Local:%.1f, Component:%.1f] | Effective:%.1f"),
               Analysis.MaxChainSpeedWorld, Analysis.MaxChainSpeedLocal, Analysis.MaxChainSpeedComponent,
               Analysis.EffectiveMotionSpeed);
        UE_LOG(LogPACManager, Log, TEXT("Kinematics: PeakAccel=%.0f | PeakJerk=%.0f | Energy=%.1f | Momentum=%.1f"),
               Analysis.PeakAcceleration, Analysis.PeakJerkMagnitude, Analysis.TotalKineticEnergy,
               Analysis.MomentumMagnitude);
        UE_LOG(LogPACManager, Log,
               TEXT("Quality Metrics: Motion=%.2f | Consistency=%.2f | Coordination=%.2f | Trajectory=%.2f"),
               Analysis.ChainMotionQuality, Analysis.MotionConsistencyScore, Analysis.CoordinationFactor,
               Analysis.TrajectoryConfidence);
        UE_LOG(LogPACManager, Log,
               TEXT("Attack Profile: Intensity=%.1f | Curvature=%.2f | Time=%.2fs | ActiveBones=%d | ActiveChains=%d"),
               Analysis.AttackIntensity, Analysis.AttackCurvature, Analysis.TimeInAttack, Analysis.ActiveBoneCount,
...
FOHCombatAnalysis UOHPACManager::AnalyzeCharacterCombatState(ACharacter* Character) {
    ...
    if (!PACManager) {
        UE_LOG(LogPACManager, VeryVerbose, TEXT("AnalyzeCharacterCombatState: No PAC Manager found on character '%s'"),
               *Character->GetName());
        return Analysis;
    }
    ...
    if (!PACManager->bEnableCollisionImpactSystem || !PACManager->bIsInitialized) {
        UE_LOG(LogPACManager, VeryVerbose,
               TEXT("AnalyzeCharacterCombatState: PAC Manager not enabled or initialized on character '%s'"),
               *Character->GetName());
        return Analysis;
    }
    ...
    if (Analysis.bIsAttacking) {
        ...
        if (UCharacterMovementComponent* CharMove = Character->GetCharacterMovement()) {
            ...
            if (!CharacterVelocity.ContainsNaN() && !AttackDirection.ContainsNaN() &&
                ...
                float MomentumAlignment =
                    FVector::DotProduct(CharacterVelocity.GetSafeNormal(), AttackDirection.GetSafeNormal());
                ...
                if (FMath::IsFinite(MomentumAlignment) && FMath::Abs(MomentumAlignment) <= 1.0f) {
                    ...
                    if (MomentumAlignment > 0.5f) // Forward momentum
                    {
                        ...
                        if (FMath::IsFinite(VelocityMagnitude) && VelocityMagnitude > 0.0f &&
                            VelocityMagnitude < 5000.0f) {
                            ...
                            if (PACManager->bVerboseLogging) {
                                UE_LOG(LogPACManager, VeryVerbose,
                                       TEXT("Character momentum contribution: Alignment=%.2f, Velocity=%.1f, "
                                            "Contribution=%.1f"),
                    ...
                    } else if (MomentumAlignment < -0.3f) // Retreating
                    {
                        ...
                        if (PACManager->bVerboseLogging) {
                            UE_LOG(LogPACManager, VeryVerbose,
                                   TEXT("Retreating motion detected: Alignment=%.2f, marking as feint"),
                                   MomentumAlignment);
                        }
                    }
                }
            ...
            if (!CurrentAcceleration.ContainsNaN() && CurrentAcceleration.Size() < 50000.0f) {
                ...
                if (bIsAccelerating && !CharacterVelocity.ContainsNaN()) {
                    float MomentumAlignment = FVector::DotProduct(CharacterVelocity.GetSafeNormal(),
                ...
                if (FMath::IsFinite(CharacterSpeed) && CharacterSpeed >= 0.0f) {
                    if (CharacterSpeed > CharMove->MaxWalkSpeed * 0.8f) // Sprint speed
                    {
                        ...
                        if (!Analysis.AttackDirection.IsZero()) {
                            float SprintAlignment = FVector::DotProduct(CharacterVelocity.GetSafeNormal(),
                                                                        Analysis.AttackDirection.GetSafeNormal());
                    ...
                    } else if (CharacterSpeed < 50.0f) // Near stationary
                    {
                        ...
                        float LocalSpeedBonus =
                            FMath::Max(Analysis.MaxChainSpeedLocal - Analysis.MaxChainSpeedWorld, 0.0f);
        ...
        if (UOHMovementComponent* OHMovement = Character->FindComponentByClass<UOHMovementComponent>()) {
            ...
            if (OHMovement->bCombatPushbackActive) {
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Combat pushback active - reducing attack metrics"));
                }
            }
            ...
            if (!OHMovement->AccumulatedPhysicsImpulse.ContainsNaN() &&
                ...
                if (FMath::IsFinite(ImpulseSize) && ImpulseSize > 200.0f) {
                    ...
                    if (PACManager->bVerboseLogging) {
                        UE_LOG(LogPACManager, VeryVerbose, TEXT("Physics impulse contribution: Size=%.1f, Bonus=%.2f"),
        ...
        if (USkeletalMeshComponent* CharacterMesh = Character->GetMesh()) {
            ...
            if (!MeshAngularVelocity.ContainsNaN() && MeshAngularVelocity.Size() < 36000.0f) // 36000 deg/s max
            {
                ...
                if (FMath::IsFinite(AngularSpeed) && AngularSpeed > 50.0f && AngularSpeed < 3600.0f) {
                    ...
                    if (PACManager->bVerboseLogging) {
                        UE_LOG(LogPACManager, VeryVerbose,
                               TEXT("Angular velocity contribution: Speed=%.1f deg/s, Curvature=%.3f"), AngularSpeed,
        ...
        bool bValidSpeedMetrics = FMath::IsFinite(Analysis.MaxChainSpeedLocal) &&
                                  ...
                                  Analysis.MaxChainSpeedLocal >= 0.0f && Analysis.MaxChainSpeedWorld >= 0.0f &&
                                  Analysis.MaxChainSpeedComponent >= 0.0f && Analysis.MaxChainSpeedLocal < 20000.0f &&
                                  Analysis.MaxChainSpeedWorld < 20000.0f && Analysis.MaxChainSpeedComponent < 20000.0f;
        ...
        if (bValidSpeedMetrics) {
            ...
            if (Analysis.MaxChainSpeedLocal > Analysis.MaxChainSpeedWorld * 1.5f) {
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Isolated limb motion: Local=%.1f, World=%.1f, Ratio=%.2f"),
            ...
            } else if (Analysis.MaxChainSpeedComponent >
                       FMath::Max(Analysis.MaxChainSpeedWorld, Analysis.MaxChainSpeedLocal)) {
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Component space dominant: Component=%.1f"),
        ...
        if (FMath::IsFinite(Analysis.TimeInAttack) && FMath::IsFinite(Analysis.PeakJerkMagnitude) &&
            ...
            if (Analysis.TimeInAttack > 0.5f) {
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Sustained attack: Duration=%.2fs"), Analysis.TimeInAttack);
                }
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Explosive attack: Duration=%.3fs, Jerk=%.0f"),
            ...
            if ((bLowIntensity && bLowConfidence) || (bHighCurvature && bInconsistentMotion)) {
                ...
                if (PACManager->bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose,
                           ...
                           bLowIntensity ? TEXT("Yes") : TEXT("No"), bLowConfidence ? TEXT("Yes") : TEXT("No"),
                           bHighCurvature ? TEXT("Yes") : TEXT("No"), bInconsistentMotion ? TEXT("Yes") : TEXT("No"));
                }
            }
        }
    }
    ...
    if (PACManager->bVerboseLogging && Analysis.bIsAttacking) {
        UE_LOG(LogPACManager, Log, TEXT("=== CHARACTER COMBAT ANALYSIS [%s] ==="), *Character->GetName());
        UE_LOG(LogPACManager, Log, TEXT("Classification: %s | Confidence: %.2f | Primary Space: %s"),
               Analysis.bIsCommittedAttack ? TEXT("COMMITTED")
                                           : (Analysis.bIsFeintOrGlancing ? TEXT("FEINT") : TEXT("STANDARD")),
               Analysis.AttackConfidence, *UEnum::GetValueAsString(Analysis.PrimaryMotionSpace));
        UE_LOG(LogPACManager, Log,
               ...
               Analysis.MaxChainSpeedWorld, Analysis.MaxChainSpeedLocal, Analysis.MaxChainSpeedComponent,
               Analysis.EffectiveMotionSpeed);
        UE_LOG(LogPACManager, Log, TEXT("Character Integration: Intensity=%.1f | Impact=%.1f | ActiveChains=%d"),
               Analysis.AttackIntensity, Analysis.ImpactPotential, Analysis.ActiveChains.Num());
    }
...
float UOHPACManager::CalculateMovementAlignmentBonus(const FVector& CharacterVelocity, const FVector& AttackDirection) {
    if (CharacterVelocity.IsZero() || AttackDirection.IsZero())
        return 0.0f;
...
bool UOHPACManager::IsExtremityBone(FName BoneName) {
    ...
    return BoneStr.Contains("hand") || BoneStr.Contains("lowerarm") || BoneStr.Contains("forearm") ||
           BoneStr.Contains("upperarm") || BoneStr.Contains("foot") || BoneStr.Contains("calf") ||
           BoneStr.Contains("shin") || BoneStr.Contains("thigh");
}

bool UOHPACManager::IsExtremityProtectingCore(FName ExtremityBone, UOHPACManager* DefenderManager,
    ...
    for (const FName& CoreBone : CoreBones) {
        ...
        if (AlignmentWithAttack > 0.5f) // Core is somewhat aligned with attack direction
        {
            ...
            if (ExtremityProjection < CoreProjection) {
                ...
                FVector ClosestPointOnAttackLine =
                    ExtremityPos + AttackDirection * FVector::DotProduct(CorePos - ExtremityPos, AttackDirection);
                ...
                if (DistanceFromLine < BlockingRadius) {
                    ...
                    if (bVerboseLogging) {
                        UE_LOG(LogPACManager, Log, TEXT("Extremity %s is protecting core %s (Distance: %.1f)"),
                               *ExtremityBone.ToString(), *CoreBone.ToString(), DistanceFromLine);
                    }
...
FCombatTargetCandidate UOHPACManager::SelectBestTargetFromChain(const FOHCombatChainData& AttackingChain,
    ...
    for (int32 GroupIdx = 0; GroupIdx < TimeGroups.Num(); GroupIdx++) {
        ...
        for (const FCombatTargetCandidate& Candidate : Group) {
            ...
            if (Candidate.bIsExtremity && bUsePunchThroughTargeting) {
                ...
                float PenetrationScore = CalculatePenetrationScore(Candidate, AttackingChain, TimeGroups, GroupIdx);
                ...
                if (PenetrationScore > 0.5f) {
                    ...
                    if (bVerboseLogging) {
                        UE_LOG(LogPACManager, Warning, TEXT("Extremity %s has high penetration score: %.2f"),
                               *Candidate.BoneName.ToString(), PenetrationScore);
                    }
                }
            }
                ...
                for (const FCombatTargetCandidate& FutureCandidate : TimeGroups[FutureIdx]) {
                    if (FutureCandidate.bIsCore) {
                        ...
                        if (TimeDiff < 0.2f && IsCoreInTrajectory(FutureCandidate, AttackingChain)) {
                            float CoreScore = CalculateTargetScore(FutureCandidate, AttackingChain, DefenderManager);
            ...
            if (bFoundCore && BestCoreScore > GroupBestScore * 1.5f) {
                ...
                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, Warning,
                           ...
                           *GroupBest.BoneName.ToString(), GroupBestScore, *BestCoreAhead.BoneName.ToString(),
                           BestCoreScore);
                }
...
float UOHPACManager::CalculateTargetScore(const FCombatTargetCandidate& Candidate,
                                          const FOHCombatChainData& AttackingChain, UOHPACManager* DefenderManager) {
    ...
    float MomentumFactor = FMath::Clamp(AttackingChain.ChainMomentum.Size() / 2000.0f, 0.5f, 2.0f);
...
float UOHPACManager::CalculatePenetrationScore(const FCombatTargetCandidate& ExtremityTarget,
    ...
    if (AttackingChain.GetPredictedTrajectory().Num() > 0) // FIXED: Use getter
    {
        // Find trajectory point at extremity impact time
        int32 ImpactIdx =
            FMath::Clamp(static_cast<int32>(ExtremityTarget.TimeToImpact *
                                            AttackingChain.GetPredictedTrajectory().Num() / 0.3f), // FIXED: Use getter
                         0,
                         AttackingChain.GetPredictedTrajectory().Num() - 1 // FIXED: Use getter
            );

        // Check how much trajectory continues past impact
        float RemainingTrajectory =
            static_cast<float>(AttackingChain.GetPredictedTrajectory().Num() - ImpactIdx) / // FIXED: Use getter
            static_cast<float>(AttackingChain.GetPredictedTrajectory().Num());              // FIXED: Use getter
...
// In IsCoreInTrajectory method:
bool UOHPACManager::IsCoreInTrajectory(const FCombatTargetCandidate& CoreTarget,
                                       const FOHCombatChainData& AttackingChain) {
    if (AttackingChain.GetPredictedTrajectory().Num() < 2) // FIXED: Use getter
    {
        return false;
    }
    ...
    for (int32 i = 0; i < AttackingChain.GetPredictedTrajectory().Num() - 1; i++) // FIXED: Use getter
    {
        FVector SegmentStart = AttackingChain.GetPredictedTrajectory()[i];   // FIXED: Use getter
        FVector SegmentEnd = AttackingChain.GetPredictedTrajectory()[i + 1]; // FIXED: Use getter

        FVector ClosestPoint = FMath::ClosestPointOnSegment(CoreTarget.Position, SegmentStart, SegmentEnd);
...
void UOHPACManager::ApplyCoreImpact(UOHPACManager* DefenderManager, FName CoreBone, const FVector& Direction,
                                    float Force) {
...
void UOHPACManager::ApplyExtremityImpact(UOHPACManager* DefenderManager, FName ExtremityBone, const FVector& Direction,
                                         float Force) {
...
void UOHPACManager::ApplyImpulseToChain(FName RootBone, const FVector& Impulse, float Falloff, bool bVelChange) {
    ...
    if (ChainBones.Num() == 0) {
        if (bVerboseLogging) {
            SafeLog(
                FString::Printf(TEXT("ApplyImpulseToChain: No chain found for root bone '%s'"), *RootBone.ToString()),
                true);
        }
        return;
    }
    ...
    for (int32 i = 0; i < ChainBones.Num(); i++) {
        ...
        float DistanceRatio = (TotalChainLength > KINDA_SMALL_NUMBER) ? (AccumulatedDistance / TotalChainLength)
                                                                      : (static_cast<float>(i) / ChainBones.Num());
        ...
        if (const FOHBoneMotionData* MotionData = GetBoneMotionData(BoneName)) {
            ...
            if (CurrentSpeed > 300.0f) {
                float MotionReduction =
                    FMath::GetMappedRangeValueClamped(FVector2D(300.0f, 1000.0f), FVector2D(1.0f, 0.5f), CurrentSpeed);
    ...
    if (bVerboseLogging) {
        SafeLog(FString::Printf(
                    ...
                    *RootBone.ToString(), ChainBones.Num(), BonesAffected, TotalChainLength, ValidatedFalloff),
                false);
    }
}
...
void UOHPACManager::ApplyRadialImpulseToTarget(UOHPACManager* TargetManager, const FVector& ImpactPoint,
                                               float BaseForce) {
    // === COMPREHENSIVE VALIDATION USING UTILITY PATTERNS ===
    ...
    for (const FName& BoneName : ValidTargetBones) {
        ...
        FVector ClosestPoint = UOHCollisionUtils::ClosestPointOnAABB(ImpactPoint, BoneLocation, BoneBounds * 0.5f);
        ...
        UOHCollisionUtils::ApplyImpulseToBone(TargetManager->SkeletalMesh, BoneName, RadialImpulse,
    ...
    if (bVerboseLogging) {
        SafeLog(FString::Printf(
                    ...
                    *ImpactPoint.ToString(), BaseForce, MassRatio, BonesAffected, ValidTargetBones.Num()),
                false);
    }
}
#pragma endregion
