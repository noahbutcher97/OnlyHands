...
void UOHPACManager::CachePhysicsAssetData()
{
	...
	for (FBodyInstance* BodyInstance : SkeletalMeshComponent->Bodies)
	{
		...
		{
			FName BoneName = BodyInstance->BodySetup.IsValid() ? BodyInstance->BodySetup->BoneName : NAME_None;
	...
	for (FConstraintInstance* ConstraintInstance : SkeletalMeshComponent->Constraints)
	{
		if (ConstraintInstance)
		{
			...
			if (Body1) BodyInstanceToConstraintInstances.FindOrAdd(Body1).Add(ConstraintInstance);
			if (Body2) BodyInstanceToConstraintInstances.FindOrAdd(Body2).Add(ConstraintInstance);
		}
	}
}
...
{
	// Smart: Only recache if mesh/asset/component has changed, otherwise update live constraint/body instance pointers
	// This could be extended to be even smarter with property diff checks
	CachePhysicsAssetData();
}

void UOHPACManager::VisualizePhysicsSnapshot(bool bDrawBodies, bool bDrawConstraints, bool bDrawBoneNames, float Duration) const
{
	...
	if (bDrawBodies)
	{
		for (const TPair<FName, FBodyInstance*>& Pair : BoneNameToBodyInstance)
		{
			...
			DrawDebugSphere(GetWorld(), Location, BodyRadius, 8, BodyColor, false, Duration);
			if (bDrawBoneNames)
			{
				DrawDebugString(GetWorld(), Location + FVector(0,0,10), BoneName.ToString(), nullptr, BoneNameColor, Duration, false);
			}
		}
	}
	...
	if (bDrawConstraints)
	{
		for (const TPair<FName, FConstraintInstance*>& Pair : ConstraintNameToConstraintInstance)
		{
			...
			DrawDebugLine(GetWorld(), Pos1, Pos2, LineColor, false, Duration, 0, 1.5f);
			DrawDebugSphere(GetWorld(), (Pos1+Pos2)*0.5f, ConstraintRadius, 8, ConstraintColor, false, Duration);
			if (bDrawBoneNames)
			{
				FString ConStr = FString::Printf(TEXT("%s-%s"), *Bone1.ToString(), *Bone2.ToString());
				DrawDebugString(GetWorld(), (Pos1+Pos2)*0.5f + FVector(0,0,10), ConStr, nullptr, ConstraintColor, Duration, false);
			}
		}
	}
}
