    ...
    for (int32 i = 0; i < Chain.ChainBones.Num(); i++) {
        ...
        // === PRIORITY 2: ENHANCED MANUAL MOTION TRACKING WITH MULTI-SPACE CALCULATION ===
            ...
            if (!WorldPos.ContainsNaN() && !WorldRot.ContainsNaN() && !WorldVelocity.ContainsNaN() &&
                ...
                ChainMotionData->AddMotionSample(WorldPos, WorldRot, ComponentVelocity,
...
void UOHPACManager::UpdateChainTrajectory(FOHCombatChainData& Chain, float DeltaTime) {
    ...
    if (!Chain.HasSignificantMotion()) {
        if (bVerboseLogging) {
            UE_LOG(LogPACManager, VeryVerbose,
                   TEXT("Trajectory prediction skipped for chain '%s': No significant motion (Speed=%.1f < "
                        "Threshold=%.1f)"),
    ...
    if (bVerboseLogging) {
        UE_LOG(
            LogPACManager, VeryVerbose,
            TEXT(
                "Starting trajectory prediction for chain '%s': PredictTime=%.2fs, Samples=%d, Quality=%.2f, Space=%s"),
    ...
    if (MotionData->GetHistoryDepth() >= 5 && Chain.MotionCoherence > 0.3f) {
        ...
        if (!KalmanState.Position.ContainsNaN() && !KalmanState.Velocity.ContainsNaN() &&
            ...
            if (bKalmanSuccess && KalmanPath.Num() == NumSamples + 1) {
                ...
                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Kalman model added: Weight=%.3f, Samples=%d"),
                           KalmanWeight, KalmanPath.Num());
                }
            } else if (bVerboseLogging) {
                UE_LOG(LogPACManager, VeryVerbose, TEXT("Kalman model failed: Success=%s, PathSamples=%d"),
            ...
            UE_LOG(
                LogPACManager, VeryVerbose,
                TEXT(
                    "Kalman model skipped: Invalid initial state - Pos NaN=%s, Vel NaN=%s, PosSize=%.1f, VelSize=%.1f"),
                KalmanState.Position.ContainsNaN() ? TEXT("True") : TEXT("False"),
                KalmanState.Velocity.ContainsNaN() ? TEXT("True") : TEXT("False"), KalmanState.Position.Size(),
                KalmanState.Velocity.Size());
        }
    } else if (bVerboseLogging) {
        UE_LOG(LogPACManager, VeryVerbose,
        ...
        if (!PhysicsState.Position.ContainsNaN() && !PhysicsState.Velocity.ContainsNaN() &&
            ...
            for (int32 i = 0; i <= NumSamples; i++) {
                if (!PhysicsState.Position.ContainsNaN() && PhysicsState.Position.Size() < 100000.0f) {
                    ...
                    if (bVerboseLogging) {
                        UE_LOG(LogPACManager, VeryVerbose, TEXT("RK4 position invalid at step %d: NaN=%s, Size=%.1f"),
                               i, PhysicsState.Position.ContainsNaN() ? TEXT("True") : TEXT("False"),
                ...
                if (i < NumSamples) {
                    ...
                    if (!PhysicsState.Velocity.IsNearlyZero(1.0f)) // Small threshold for nearly zero
                    {
                        ...
                        if (VelocityMagnitude > 0.0f && Chain.ChainTotalMass > 0.0f) {
                            DragAcceleration = -PhysicsState.Velocity.GetSafeNormal() * DragCoeff * VelocityMagnitude *
                                               VelocityMagnitude / Chain.ChainTotalMass;
                        }
                    }

                    // Validate drag acceleration
                    if (!DragAcceleration.ContainsNaN() && DragAcceleration.Size() < 50000.0f) {
                        PhysicsState = UOHAlgoUtils::IntegrateRK4State(PhysicsState, DragAcceleration, TimeStep);
                    } else {
                        if (bVerboseLogging) {
                            UE_LOG(LogPACManager, VeryVerbose,
            ...
            if (bRK4Success && RK4Path.Num() == NumSamples + 1) {
                ...
                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose,
                           TEXT("RK4 model added: Weight=%.3f, DragCoeff=%.6f, Energy=%.1f"), RK4Weight, DragCoeff,
                           Chain.ChainKineticEnergy);
                }
            } else if (bVerboseLogging) {
                UE_LOG(LogPACManager, VeryVerbose, TEXT("RK4 model failed: Success=%s, PathSamples=%d"),
            ...
            UE_LOG(
                ...
                PhysicsState.Velocity.ContainsNaN() ? TEXT("True") : TEXT("False"), PhysicsState.Position.Size(),
                PhysicsState.Velocity.Size());
        }
    } else if (bVerboseLogging) {
        UE_LOG(LogPACManager, VeryVerbose,
    ...
    if (Curvature > 0.1f && !FMath::IsNaN(Curvature) && FMath::IsFinite(Curvature)) {
        if (Curvature > 0.25f) {
            ...
            Chain.BezierControlPoints =
                UOHAlgoUtils::GetCubicBezierControlPointsFromBoneData(*MotionData, PredictionTime);
                ...
                if (bValidControlPoints) {
                    ...
                    for (int32 i = 0; i <= NumSamples; i++) {
                        ...
                        FVector Point = UOHAlgoUtils::SampleBezierCubic(
                            Chain.BezierControlPoints[0], Chain.BezierControlPoints[1], Chain.BezierControlPoints[2],
                            Chain.BezierControlPoints[3], t);
                    ...
                    if (bBezierSuccess && BezierPath.Num() == NumSamples + 1) {
                        ...
                        if (bVerboseLogging) {
                            UE_LOG(LogPACManager, VeryVerbose,
                                   TEXT("Cubic Bezier model added: Weight=%.3f, Curvature=%.3f"), BezierWeight,
                                   Curvature);
                        }
                    }
                }
            ...
            Chain.BezierControlPoints =
                UOHAlgoUtils::GetQuadraticBezierControlPointsFromBoneData(*MotionData, PredictionTime);
            ...
            if (Chain.BezierControlPoints.Num() >= 3) {
                ...
                if (bValidControlPoints) {
                    ...
                    for (int32 i = 0; i <= NumSamples; i++) {
                        float t = static_cast<float>(i) / static_cast<float>(NumSamples);
                        FVector Point = UOHAlgoUtils::SampleBezierQuadratic(Chain.BezierControlPoints[0],
                                                                            Chain.BezierControlPoints[1],
                                                                            Chain.BezierControlPoints[2], t);
                    ...
                    if (bBezierSuccess && BezierPath.Num() == NumSamples + 1) {
                        ...
                        if (bVerboseLogging) {
                            UE_LOG(LogPACManager, VeryVerbose,
                                   TEXT("Quadratic Bezier model added: Weight=%.3f, Curvature=%.3f"), BezierWeight,
                                   Curvature);
                        }
                    }
                }
        ...
        UE_LOG(LogPACManager, VeryVerbose, TEXT("Bezier model skipped: Low curvature (%.3f ≤ 0.1)"), Curvature);
    }
    ...
    if (Chain.AttackConfidence < 0.7f && Chain.AttackConfidence > 0.3f) {
        ...
        if (CurrentSamplePtr && !CurrentSamplePtr->WorldPosition.ContainsNaN()) {
            ...
            if (!CurrentPos.ContainsNaN() && !AttackVelocity.ContainsNaN() && CurrentPos.Size() < 100000.0f &&
                AttackVelocity.Size() < 10000.0f) {
                FVector TargetPos =
                    CurrentPos + AttackVelocity.GetSafeNormal() * (PunchThroughDistance * Chain.AttackConfidence);
                    ...
                    for (int32 i = 0; i <= NumSamples; i++) {
                        ...
                        if (i < NumSamples) {
                            SpringSample = UOHAlgoUtils::IntegrateCriticallyDampedSpringMotionSample(
                                SpringSample, TargetPos, SpringFreq, TimeStep);
                        }
                    }
        ...
        UE_LOG(LogPACManager, VeryVerbose, TEXT("Spring model skipped: AttackConfidence=%.2f (need 0.3-0.7)"),
               Chain.AttackConfidence);
    }
    ...
    if (ModelPredictions.Num() > 0) {
        ...
        for (int32 i = 0; i < ModelWeights.Num(); i++) {
            if (!FMath::IsNaN(ModelWeights[i]) && FMath::IsFinite(ModelWeights[i]) && ModelWeights[i] > 0.0f) {
                ...
                if (bVerboseLogging) {
                    UE_LOG(LogPACManager, VeryVerbose, TEXT("Model weight %d invalidated: NaN or non-finite"), i);
                }
            }
        }
        ...
        if (TotalWeight > KINDA_SMALL_NUMBER) {
            ...
            if (Chain.PredictedTrajectory.Num() > 3 && ValidBlendedPoints > (NumSamples * 0.8f)) {
                ...
                for (int32 i = 1; i < Chain.PredictedTrajectory.Num() - 1; i++) {
                    FVector SmoothedPoint =
                        (Chain.PredictedTrajectory[i - 1] * 0.25f + Chain.PredictedTrajectory[i] * 0.5f +
            ...
            Chain.TrajectoryConfidence = FMath::Clamp(ConsensusConfidence * ValidationConfidence, 0.0f, 1.0f);
        } else {
            if (bVerboseLogging) {
                UE_LOG(LogPACManager, Warning, TEXT("All model weights invalid for chain '%s': TotalWeight=%.6f"),
        ...
        if (bVerboseLogging) {
            ...
            UE_LOG(LogPACManager, Log,
                   ...
                   *Chain.RootBone.ToString(), *ModelList, Chain.TrajectoryConfidence,
                   *UEnum::GetValueAsString(Chain.MotionAnalysisSpace), Chain.PredictedTrajectory.Num());
        }
    } else if (bVerboseLogging) {
        UE_LOG(LogPACManager, VeryVerbose, TEXT("No valid prediction models for chain '%s'"),
    ...
    if (Chain.PredictedTrajectory.Num() == 0) {
        ...
        if (!CurrentPos.ContainsNaN() && !Velocity.ContainsNaN() && !Acceleration.ContainsNaN() &&
            ...
            if (bVerboseLogging) {
                UE_LOG(LogPACManager, Log, TEXT("Fallback trajectory prediction used for chain '%s': Confidence=%.3f"),
...
void UOHPACManager::AnalyzeChainAttackPattern(FOHCombatChainData& Chain, float DeltaTime) {
    ...
    if (Chain.bIsAttacking && !Chain.AttackPrincipalDirection.ContainsNaN() &&
        !Chain.AttackPrincipalDirection.IsZero()) {
    ...
    if (bVerboseLogging && Chain.bIsAttacking) {
        ...
        UE_LOG(LogPACManager, Warning,
               TEXT("Attack Pattern [%s]: Type=%s, Conf=%.2f, Coh=[A:%.2f,W:%.2f,L:%.2f,C:%.2f], Jerk=%.0f, "
                    "Curve=%.2f, Space=%s, Ratios=[L:%.1f,C:%.1f]"),
               *Chain.RootBone.ToString(), PatternNames[static_cast<uint8>(DetectedPattern)], Chain.AttackConfidence,
               ...
               Chain.AttackCurvature, *UEnum::GetValueAsString(Chain.MotionAnalysisSpace), LocalToWorldRatio,
...
void UOHPACManager::CheckChainVsAllBodies(const FOHCombatChainData& AttackingChain, UOHPACManager* DefenderManager) {
    if (!DefenderManager || AttackingChain.PredictedTrajectory.Num() < 2)
        return;
            ...
            if (Distance <= CheckRadius) {
                ...
                if (Candidate.bIsExtremity) {
                    Candidate.bIsBlocking =
                        IsExtremityProtectingCore(DefenderBone, DefenderManager, AttackingChain.AttackDirection);
                }
...
float UOHPACManager::CalculatePenetrationScore(const FCombatTargetCandidate& ExtremityTarget,
    ...
    if (AttackingChain.PredictedTrajectory.Num() > 0) {
        ...
        int32 ImpactIdx = FMath::Clamp(
            static_cast<int32>(ExtremityTarget.TimeToImpact * AttackingChain.PredictedTrajectory.Num() / 0.3f), 0,
