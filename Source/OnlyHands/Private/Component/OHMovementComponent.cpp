    ...
    if (bUseMomentumBasedMovement)
    {
        ...
        if (bHasInput && bUseInputMagnitudeForSpeed)
        {
            float InputScale = FMath::GetMappedRangeValueClamped(
                FVector2D(JoystickDeadzone, 1.0f),
                FVector2D(0.3f, 1.0f),
                BufferedInputAmount
            );
	...
	if (bDebugMovement)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				static_cast<uint64>((PTRINT)this)+11111,
				0.05f,
				FColor::Yellow,
...
void UOHMovementComponent::OnReceiveMoveRight(float RightValue)
{
	...
	if (bDebugMovement)
	{
		...
		{
			GEngine->AddOnScreenDebugMessage(
				static_cast<uint64>((PTRINT)this)+22222,
				0.05f,
				FColor::Orange,
...
void UOHMovementComponent::OnReceiveMoveVector(float ForwardValue, float RightValue)
{
	...
	if (bDebugMovement)
	{
		...
		{
			GEngine->AddOnScreenDebugMessage(
				static_cast<uint64>((PTRINT)this)+33333,
				0.05f,
				FColor::Red,
