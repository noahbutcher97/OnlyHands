...
void UOHMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    if (MovementConfig.bEnableDebugOutput && GEngine)
    {
        ...
        if (bAcceptPhysicsInput)
        {
            ...
            {
                FString DebugText = FString::Printf(TEXT("PhysicsInput: %.2f, %.2f"), 
                    PhysicsInputVector.X, PhysicsInputVector.Y);
            ...
            {
                FString ImpulseText = FString::Printf(TEXT("PhysicsImpulse: %s"), 
            ...
            {
                FString EffectiveText = FString::Printf(TEXT("Effective: %.2f, %.2f"), 
                    EffectiveInput.X, EffectiveInput.Y);
    ...
    if (bVerboseLogging)
    {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("Physics Input Active: %.3f, %.3f"), 
                PhysicsInputVector.X, PhysicsInputVector.Y);
        }
        ...
        if (!EffectiveInput.IsNearlyZero())
        {
            UE_LOG(LogTemp, Warning, TEXT("Effective Input: %.3f, %.3f"), 
                EffectiveInput.X, EffectiveInput.Y);
        }
    }
}
...
void UOHMovementComponent::ApplyMovementInput()
{
    ...
    if (!MoveVector.IsNearlyZero())
    {
        ...
        UOHLocomotionUtils::ApplyWeightedMovementInput(
            Character,
            MoveVector,
        ...
        if (bVerboseLogging)
        {
            UE_LOG(LogTemp, VeryVerbose, 
                TEXT("Movement Applied: Input=(X:%.2f,Y:%.2f), Move=%s, Weight=%.2f"),
                EffectiveInput.X, EffectiveInput.Y, 
                *MoveVector.ToString(),
                MovementConfig.GetMomentumBasedMovementWeight());
        }
...
void UOHMovementComponent::PrintDebug_MovementInput()
{
	...
	if (MovementConfig.bEnableDebugOutput && GetOwner())
		...
		DrawDebugLine(GetWorld(), Start + FVector(0,0,10), PhysicsEnd + FVector(0,0,10), 
...
void UOHMovementComponent::PrintDebug_MovementData()
{
	...
	// Player input
	GEngine->AddOnScreenDebugMessage(MessageKey++, 0.f, FColor::Green, 
		FString::Printf(TEXT("Player Input: (%.2f, %.2f)"), 
			MovementInputVector.X, MovementInputVector.Y));
	...
	GEngine->AddOnScreenDebugMessage(MessageKey++, 0.f, FColor::Orange, 
		FString::Printf(TEXT("Physics Impulse: %s"), 
			*AccumulatedPhysicsImpulse.ToString()));
	
	// Effective movement
	GEngine->AddOnScreenDebugMessage(MessageKey++, 0.f, FColor::Cyan, 
		FString::Printf(TEXT("Actual Velocity: %s Speed: %.1f"), 
			*ActualVelocity.ToString(), ActualVelocity.Size()));
	// Debug output for tuning
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow,
					FString::Printf(TEXT("Gait Speeds - Walk: %.0f (Thresh: %.0f) | Run: %.0f (Thresh: %.0f) | Sprint: %.0f (Thresh: %.0f)"),
						MovementConfig.GetMaxWalkSpeed(), MovementConfig.GetWalkSpeedThreshold(),
						MovementConfig.GetMaxRunSpeed(), MovementConfig.GetRunSpeedThreshold(),
						MovementConfig.GetMaxSprintSpeed(), MovementConfig.GetSprintSpeedThreshold()));

	GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Green, FString::Printf(TEXT("Momentum: %.0f | Dir: %.2fx | Rot: %.2fx | Angle: %.0f°"), CurrentMomentumSpeed,
...
void UOHMovementComponent::ApplyRotation(const FRotator& TargetRotation, bool bBlend, float BlendSpeed, float DeltaTime)
{
...
FVector UOHMovementComponent::CalculateMovementInputVector() const
{
	...
	FRotator ControlRotation = CalculateControlRotation();
	float FacingBlend = MovementConfig.bUseRotationBlendedMovementInput ? 
					   MovementConfig.RotationBlendedMovementWeight : 0.0f;
    
	return UOHLocomotionUtils::CalculateBlendedMovementInputVector(
		MoveVector, // Now using EFFECTIVE input
		ControlRotation, 
...
{
	if (!TargetActor) return GetOwner()->GetActorRotation();
	return UOHLocomotionUtils::CalculateBlendedHybridFacing(
		GetOwner()->GetActorRotation(),
		GetOwner()->GetActorLocation(),
		TargetActor->GetActorLocation(),
		MovementConfig.DeadzoneAngle,
		MovementConfig.MaxOffsetAngle,
		MovementConfig.PullStrength,
		MovementConfig.HardSnapStrength,
		DeltaTime
	);
}
...
void UOHMovementComponent::UpdateInputBuffers(float DeltaTime)
{
	...
	{
		BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, CurrentInputAmount, DeltaTime, MovementConfig.GetInputBufferRiseRate());
	}
	else
	{
		BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, CurrentInputAmount, DeltaTime, MovementConfig.GetInputBufferDecay());
	}
...
void UOHMovementComponent::UpdateDynamicMovementSettings(EOHGait AllowedGait)
{
	...
	if (MovementConfig.bUseMomentumBasedMovement)
	{
		...
		if (bHasInput)
		{
			...
			CurrentMomentumSpeed = FMath::FInterpTo(
		...
		else
		{
			...
			CurrentMomentumSpeed = FMath::FInterpTo(
				...
				0.f,
				DeltaTime,
				MovementConfig.GetNoInputSpeedDecayRate()
			);
		}
        ...
        if (bHasInput && MovementConfig.bUseInputMagnitudeForSpeed)
        {
            float InputScale = FMath::GetMappedRangeValueClamped(
                FVector2D(MovementConfig.GetJoystickDeadzone(), 1.0f),
                FVector2D(0.3f, 1.0f),
                BufferedInputAmount
            );
        ...
        if (!LastMovementDirection.IsNearlyZero())
        {
            ...
            if (DirectionChangeAngle > 90.f)
            {
                float ChangeMultiplier = FMath::GetMappedRangeValueClamped(
                    ...
                    FVector2D(1.0f, MovementConfig.GetSpeedRetentionOnDirectionChange()),
...
void UOHMovementComponent::HandlePACPushback(FName BoneName, const FVector2D& PushbackVector, float ImpactForce)
{
    ...
    if (Character->GetCharacterMovement())
    {
        ...
        if (bVerboseLogging)
        {
            UE_LOG(LogTemp, Warning, TEXT("Applied immediate velocity: %s"), 
                *ImmediateVelocity.ToString());
        }
    }
    ...
    if (bVerboseLogging)
    {
        UE_LOG(LogTemp, Warning, TEXT("HandlePACPushback: CharRelative=(%.2f,%.2f) -> World=%s, Total=%s"), 
            PushbackVector.X, PushbackVector.Y,
            *WorldPush.ToString(), 
            *AccumulatedPhysicsImpulse.ToString());
    }
...
void UOHMovementComponent::OnReceiveMoveForward(float ForwardValue)
{
	...
	if (MovementConfig.bEnableDebugOutput && GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			static_cast<uint64>(reinterpret_cast<PTRINT>(this))+11111,
			0.05f,
			FColor::Yellow,
...
void UOHMovementComponent::OnReceiveMoveRight(float RightValue)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(
			static_cast<uint64>(reinterpret_cast<PTRINT>(this))+22222,
			0.05f,
			FColor::Orange,
...
void UOHMovementComponent::OnReceiveMoveVector(float ForwardValue, float RightValue)
{
	...
	if (MovementConfig.bEnableDebugOutput)
	{
		...
		{
			GEngine->AddOnScreenDebugMessage(
				static_cast<uint64>(reinterpret_cast<PTRINT>(this))+33333,
				0.05f,
				FColor::Red,
	...
	if (MovementConfig.bEnableDebugOutput)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				0.05f,
				FColor::Green,
				FString::Printf(TEXT("MC: SetMovementInputVector F=%.2f R=%.2f"), CurrentForwardInput, CurrentRightInput)
			);
		}
	}
...
EOHGait UOHMovementComponent::CalculateDesiredGait(float DeltaTime)
{
    ...
    {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, IntentMagnitude, DeltaTime, MovementConfig.GetInputBufferRiseRate());
    }
    else
    {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, IntentMagnitude, DeltaTime, MovementConfig.GetInputBufferDecay());
    }
    ...
    if (MovementConfig.bEnableDebugOutput && GEngine)
    {
        GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Cyan,
            FString::Printf(TEXT("VJoy: Raw:%.2f Buff:%.2f Intent:%s Speed:%.0f SpeedOK:%s"),
                InputAmount, BufferedInputAmount,
                IntentGait == EOHGait::Sprinting ? TEXT("SPRINT") : 
                IntentGait == EOHGait::Running ? TEXT("RUN") : TEXT("WALK"),
                BufferedSpeed,
                bSpeedSupportsIntent ? TEXT("YES") : TEXT("NO")));
    }

    // Determine final gait with timing requirements
    if (IntentGait == EOHGait::Sprinting && BufferedTimeAboveSprintThreshold >= MovementConfig.GetHoldTimeAboveSprintThreshold())
    {
        return EOHGait::Sprinting;
    }
    else if (IntentGait >= EOHGait::Running && BufferedTimeAboveRunThreshold >= MovementConfig.GetHoldTimeAboveRunThreshold())
    {
...
void UOHMovementComponent::UpdateFootData(FOHFootStanceData& FootData, const FName& BoneName, bool bIsLeftFoot, float DeltaTime)
{
    ACharacter* OwnerChar = Cast<ACharacter>(GetOwner());
    ...
    if (FootData.bIsGrounded)
    {
        ...
        FootData.ChainData.SurfaceAngle = FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(GroundHit.Normal, FVector::UpVector)));
        FootData.ChainData.SurfaceType = GroundHit.PhysMaterial.IsValid() ? GroundHit.PhysMaterial->SurfaceType : TEnumAsByte<EPhysicalSurface>(EPhysicalSurface::SurfaceType_Default);
    }
...
void UOHMovementComponent::CalculateWeightDistribution()
{
	...
	if (TotalWeight > SMALL_NUMBER)
	{
		StanceState.CenterOfGravity = (LeftFootData.WorldPosition * LeftFootData.WeightBearing + 
									   RightFootData.WorldPosition * RightFootData.WeightBearing) / TotalWeight;
...
{
	ACharacter* OwnerChar = Cast<ACharacter>(GetOwner());
	if (!OwnerChar) return FootTraceDistance;
	...
	if (GetWorld()->LineTraceSingleByChannel(OutHit, TraceStart, TraceEnd, ECC_Visibility, QueryParams))
	{
		...
		FOHFootStanceData FootData = OutHit.Component == OwnerChar->GetMesh() ? 
			(OutHit.BoneName == LeftFootBoneName ? LeftFootData : RightFootData) : LeftFootData; // Fallback
		FootData.ChainData.SurfaceNormal = OutHit.Normal;
		FootData.ChainData.SurfaceAngle = FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(OutHit.Normal, FVector::UpVector)));
...
void UOHMovementComponent::UpdateKinematicChain(FOHKinematicChainData& ChainData, bool bIsLeftFoot, USkeletalMeshComponent* Mesh)
{
    // Get all transforms
    ...
    {
        float CosAngle = (ThighLength * ThighLength + ShinLength * ShinLength - ChainData.LegLength * ChainData.LegLength) 
...
void UOHMovementComponent::UpdateSlidingDetection(FOHFootStanceData& FootData, float DeltaTime)
{
    ...
    if (!FootData.bIsGrounded || FootData.CurrentPhase == EOHFootPhase::Swing || FootData.CurrentPhase == EOHFootPhase::Lift)
    {
    ...
    if (HorizontalSpeed > FootSlideThreshold && FootData.ChainData.TimeSincePlanted > 0.1f)
    {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("Foot sliding detected! Distance: %.1fcm, Speed: %.1fcm/s"), 
                SlideDistance, HorizontalSpeed);
        }
    }
    else
    {
...
void UOHMovementComponent::UpdateClippingDetection(FOHFootStanceData& FootData, FOHFootStanceData& OtherFoot)
{
    ...
    float KneeDistance = FVector::Dist(FootData.ChainData.KneeTransform.GetLocation(), 
                                      OtherFoot.ChainData.KneeTransform.GetLocation());
    if (KneeDistance < KneeCollisionRadius * 2.0f)
    {
        FootData.ChainData.bIsClipping = true;
        FootData.ChainData.ClipDepth = FMath::Max(FootData.ChainData.ClipDepth, 
                                                  KneeCollisionRadius * 2.0f - KneeDistance);
        FootData.ChainData.ClippingBones.Add(TEXT("Knees"));
    }
    
    // Check if legs are crossing (knees on wrong sides)
    FVector HipCenter = (FootData.ChainData.HipTransform.GetLocation() + 
                        OtherFoot.ChainData.HipTransform.GetLocation()) * 0.5f;
...
EOHFootPhase UOHMovementComponent::CalculatePhaseFromKinematics(const FOHFootStanceData& FootData, float DeltaTime)
{
    ...
    if (!FootData.bIsGrounded && FootData.GroundDistance < 30.0f && 
        Chain.LegCompression < 0.2f && FootData.Velocity.Z < 0.0f)
    {
        return EOHFootPhase::Reach;
    }
...
void UOHMovementComponent::DrawDebug_MovementData(float DeltaTime)
{
	...
	if (bAcceptPhysicsInput)
	{
		...
		if (!AccumulatedPhysicsImpulse.IsNearlyZero())
		{
			DrawDebugLine(GetWorld(), CharLocation, 
				CharLocation + AccumulatedPhysicsImpulse.GetClampedToMaxSize(200.0f), 
				FColor::Orange, false, -1, 0, 5.0f);
        
			DrawDebugString(GetWorld(), CharLocation + FVector(0,0,100), 
				FString::Printf(TEXT("Impulse: %.1f"), AccumulatedPhysicsImpulse.Size()),
		...
		if (Character->GetCharacterMovement())
		{
			...
			if (!Velocity.IsNearlyZero())
			{
				DrawDebugLine(GetWorld(), CharLocation + FVector(0,0,50), 
					CharLocation + FVector(0,0,50) + Velocity * 0.1f, 
					FColor::Cyan, false, -1, 0, 3.0f);
			}
		}
	}
    ...
    DrawDebugLine(GetWorld(), CharLocation, CharLocation + PlayerInputWorld * 200.f, 
    ...
    if (!AccumulatedPhysicsImpulse.IsNearlyZero())
    {
        DrawDebugLine(GetWorld(), CharLocation + FVector(0,0,10), 
            CharLocation + FVector(0,0,10) + AccumulatedPhysicsImpulse * DebugScale, 
            FColor::Orange, false, -1, 0, 5.0f);
    ...
    DrawDebugLine(GetWorld(), CharLocation + FVector(0,0,20), 
        CharLocation + FVector(0,0,20) + ActualVelocity * DebugScale, 
        FColor::Cyan, false, -1, 0, 4.0f);
    
    // 3. Proximity Spacing Visualization
    if (bEnableProximitySpacing && TargetActor)
    {
        // Personal space bubble
        DrawDebugSphere(GetWorld(), CharLocation, PersonalSpaceRadius, 
            24, FColor::Red, false, -1, 0, 1.0f);
        
        // Optimal spacing ring
        DrawDebugCircle(GetWorld(), CharLocation, BaseSpacingDistance, 32,
            FColor::Green, false, -1, SDPG_World, 2.0f, FVector(0,0,1));
        ...
        FColor DistanceColor = Distance < PersonalSpaceRadius ? FColor::Red :
                              Distance < BaseSpacingDistance ? FColor::Yellow : FColor::Green;
        ...
        DrawDebugString(GetWorld(), MidPoint, 
            ...
            nullptr, DistanceColor, 0.0f, true, 1.2f);
    }
}
...
void UOHMovementComponent::DrawDebug_KinematicChainData(const FOHFootStanceData& FootData, bool bIsLeft, USkeletalMeshComponent* Mesh) const
{
#if WITH_EDITOR
    const FOHKinematicChainData& Chain = FootData.ChainData;
    ...
    DrawDebugDirectionalArrow(GetWorld(), KneePos, KneePos + KneeForward, 
        10.0f, FColor::Yellow, false, DebugDrawDuration, 0, 1.5f);
    
    // Knee angle text
    DrawDebugString(GetWorld(), KneePos + FVector(0, 0, 10), 
        FString::Printf(TEXT("%.0f°"), Chain.KneeAngle),
        nullptr, FColor::Yellow, DebugDrawDuration, true, 0.8f);
    ...
    if (Chain.LegCompression > 0.1f)
    {
        ...
        for (int32 i = 0; i < Coils; i++)
        {
            ...
            FVector Offset1 = FVector(FMath::Cos(FMath::DegreesToRadians(Angle1)), 
                                      FMath::Sin(FMath::DegreesToRadians(Angle1)), 0) * SpringRadius;
            FVector Offset2 = FVector(FMath::Cos(FMath::DegreesToRadians(Angle2)), 
                                      FMath::Sin(FMath::DegreesToRadians(Angle2)), 0) * SpringRadius;
            
            DrawDebugLine(GetWorld(), P1 + Offset1, P2 + Offset2, 
...
void UOHMovementComponent::DrawDebug_SlidingIndicator(const FOHFootStanceData& FootData, const FColor& FootColor) const
{
#if WITH_EDITOR
    // Sliding trail
    if (FootData.ChainData.AccumulatedSlideDistance > 1.0f)
    {
        DrawDebugLine(GetWorld(), FootData.ChainData.PlantedPosition, FootData.WorldPosition,
            FColor::Orange, false, DebugDrawDuration, 0, 3.0f);
        
        // Warning circle at planted position
        DrawDebugCircle(GetWorld(), FootData.ChainData.PlantedPosition, 
            FootData.ChainData.AccumulatedSlideDistance, 16,
    ...
    if (FootData.ChainData.SlideVelocity > 0.1f)
    {
        ...
        DrawDebugDirectionalArrow(GetWorld(), FootData.WorldPosition, 
            FootData.WorldPosition + SlideArrow,
            15.0f, FColor::Orange, false, DebugDrawDuration, 0, 3.0f);
        
        // Slide info
        DrawDebugString(GetWorld(), FootData.WorldPosition + FVector(0, 0, 30),
            FString::Printf(TEXT("SLIDE: %.1fcm @ %.0fcm/s"), 
                FootData.ChainData.AccumulatedSlideDistance,
...
void UOHMovementComponent::DrawDebug_ClippingWarning() const
{
    ...
    if (LeftFootData.ChainData.bIsClipping && RightFootData.ChainData.bIsClipping)
    {
        ...
        DrawDebugString(GetWorld(), MidPoint + FVector(0, 0, 40),
            FString::Printf(TEXT("CLIPPING: %.1fcm"), ClipDepth),
        ...
        if (LeftFootData.ChainData.ClippingBones.Contains(TEXT("LegsCrossed")))
        {
            DrawDebugLine(GetWorld(), LeftFootData.ChainData.KneeTransform.GetLocation(),
                RightFootData.ChainData.KneeTransform.GetLocation(),
                FColor::Red, false, DebugDrawDuration, 0, 4.0f);
            
            DrawDebugLine(GetWorld(), LeftFootData.WorldPosition, RightFootData.WorldPosition,
                FColor::Red, false, DebugDrawDuration, 0, 4.0f);
        }
    }
#endif
...
void UOHMovementComponent::DrawDebug_CompressionBars() const
{
	...
	FColor LeftBarColor = FMath::Lerp(
		FLinearColor(FColor::Green),
		FLinearColor(FColor::Red),
		LeftFootData.ChainData.LegCompression
	).ToFColor(true);
    
	DrawDebugBox(GetWorld(), BarBasePos + FVector(0, -20, LeftBarHeight * 0.5f),
		FVector(10, 5, LeftBarHeight * 0.5f), LeftBarColor, false, DebugDrawDuration);
    
	// Right leg compression bar
	float RightBarHeight = RightFootData.ChainData.LegCompression * 100.0f;
	FColor RightBarColor = FMath::Lerp(
		FLinearColor(FColor::Green),
		FLinearColor(FColor::Red),
		RightFootData.ChainData.LegCompression
	).ToFColor(true);
    
	DrawDebugBox(GetWorld(), BarBasePos + FVector(0, 20, RightBarHeight * 0.5f),
		FVector(10, 5, RightBarHeight * 0.5f), RightBarColor, false, DebugDrawDuration);
    
	// Labels
	DrawDebugString(GetWorld(), BarBasePos + FVector(0, -20, -10),
		FString::Printf(TEXT("L: %.0f%%"), LeftFootData.ChainData.LegCompression * 100.0f),
		nullptr, FColor::White, DebugDrawDuration, true, 0.8f);
    
	DrawDebugString(GetWorld(), BarBasePos + FVector(0, 20, -10),
		FString::Printf(TEXT("R: %.0f%%"), RightFootData.ChainData.LegCompression * 100.0f),
		nullptr, FColor::White, DebugDrawDuration, true, 0.8f);
#endif
}
...
void UOHMovementComponent::DrawDebug_BalanceAndWeightDistribution() const
{
    ...
    DrawDebugDirectionalArrow(GetWorld(), StanceState.CenterOfGravity, WeightArrowEnd, 
        30.0f, FColor::Yellow, false, DebugDrawDuration, 0, 3.0f);
    
    // Balance triangle between feet and COG
    TArray<FVector> BalanceTriangle = {LeftFootPos, RightFootPos, StanceState.CenterOfGravity};
    for (int32 i = 0; i < 3; i++)
    {
        DrawDebugLine(GetWorld(), BalanceTriangle[i], BalanceTriangle[(i+1)%3], 
            COGColor, false, DebugDrawDuration, 0, 1.0f);
    }
    ...
    DrawDebugLine(GetWorld(), StanceState.CenterOfGravity, COGGroundProj, 
        FColor::Yellow, false, DebugDrawDuration, 0, 1.0f);
    ...
    DrawDebugString(GetWorld(), COGGroundProj + FVector(0, 0, 20), 
        FString::Printf(TEXT("Balance: %.0f%%"), BalancePercentage),
        nullptr, COGColor, DebugDrawDuration);
#endif
}
...
void UOHMovementComponent::DrawDebug_EnvironmentInfo(const FOHFootStanceData& FootData) const
{
    ...
    DrawDebugDirectionalArrow(GetWorld(), FootData.WorldPosition, NormalEnd,
        10.0f, FColor::Cyan, false, DebugDrawDuration, 0, 1.0f);
    ...
    if (FootData.ChainData.SurfaceAngle > 5.0f)
    {
        DrawDebugString(GetWorld(), FootData.WorldPosition + FVector(0, 0, -10),
            FString::Printf(TEXT("%.0f°"), FootData.ChainData.SurfaceAngle),
            nullptr, FColor::Cyan, DebugDrawDuration, true, 0.8f);
    }
    
    // Edge warning
    if (FootData.ChainData.bOnEdge)
    {
        DrawDebugCircle(GetWorld(), FootData.WorldPosition, 30.0f, 32,
            FColor::Yellow, false, DebugDrawDuration, SDPG_World, 3.0f, FVector(0,0,1));
        
        DrawDebugString(GetWorld(), FootData.WorldPosition + FVector(0, 0, 40),
            TEXT("EDGE!"), nullptr, FColor::Yellow, DebugDrawDuration, false, 1.2f);
    }
#endif
}
...
void UOHMovementComponent::DrawDebug_FootPhases() const
{
	...
	if (ACharacter* OwnerChar = Cast<ACharacter>(GetOwner()))
	{
		...
		float LeftRadius = FMath::Lerp(3.0f, 12.0f, 
			LeftFootData.WeightBearing * (1.0f + LeftFootData.ChainData.LegCompression));
		float RightRadius = FMath::Lerp(3.0f, 12.0f, 
			RightFootData.WeightBearing * (1.0f + RightFootData.ChainData.LegCompression));
...
void UOHMovementComponent::PrintDebug_StanceData() const
{
	...
	DrawDebugString(GetWorld(), TextPos,
		FString::Printf(TEXT("Leg Length - L: %.0f/%.0f R: %.0f/%.0f"),
			LeftFootData.ChainData.LegLength, LeftFootData.ChainData.RestLegLength,
			RightFootData.ChainData.LegLength, RightFootData.ChainData.RestLegLength),
	...
	if (LeftFootData.ChainData.bIsClipping || RightFootData.ChainData.bIsClipping)
	{
		...
		if (LeftFootData.ChainData.ClippingBones.Num() > 0)
		{
			...
			ClipInfo += FString::JoinBy(
				LeftFootData.ChainData.ClippingBones,
				TEXT(", "),
				[](const FName& Name) { return Name.ToString(); }
			);
		}

		DrawDebugString(GetWorld(), TextPos - FVector(0, 0, 40),
			ClipInfo, nullptr, FColor::Red, DebugDrawDuration, false, 1.2f);
	}
#endif
}
...
bool UOHMovementComponent::ValidateKinematicSetup() const
{
    ...
    TArray<TPair<FName, FName>> RequiredBones = {
        {TEXT("LeftFoot"), LeftFootBoneName},
        {TEXT("RightFoot"), RightFootBoneName},
        {TEXT("LeftAnkle"), LeftAnkleBoneName},
        {TEXT("RightAnkle"), RightAnkleBoneName},
        {TEXT("LeftKnee"), LeftKneeBoneName},
        {TEXT("RightKnee"), RightKneeBoneName},
        {TEXT("LeftHip"), LeftHipBoneName},
        {TEXT("RightHip"), RightHipBoneName},
    ...
    if (!bAllValid)
    {
        ...
        for (int32 i = 0; i < RefSkeleton.GetNum(); i++)
        {
            ...
            if (BoneName.Contains("foot") || BoneName.Contains("knee") || 
                BoneName.Contains("hip") || BoneName.Contains("ankle") ||
                BoneName.Contains("thigh") || BoneName.Contains("calf") ||
...
void UOHMovementComponent::ApplyPhysicsImpulse(const FVector& WorldImpulse, bool bIsCombatHit)
{
    ...
    float MaxImpulse = bIsCombatHit ? 
        (PhysicsImpulseStrength * 3000.0f) : 
        (PhysicsImpulseStrength * 500.0f);
    ...
    if (bIsCombatHit)
    {
        ...
        if (Character->GetCharacterMovement())
        {
            ...
            float ImmediateVelocity = FMath::GetMappedRangeValueClamped(
                FVector2D(100.0f, 2000.0f),
                FVector2D(50.0f, 400.0f),
                ImpulseMagnitude
            );
    ...
    if (bVerboseLogging)
    {
        UE_LOG(LogTemp, Warning, TEXT("ApplyPhysicsImpulse: %s, Combat=%s, Total=%s"), 
            *WorldImpulse.ToString(), 
            bIsCombatHit ? TEXT("Yes") : TEXT("No"),
            *AccumulatedPhysicsImpulse.ToString());
    }
}
...
FVector2D UOHMovementComponent::GetEffectiveMovementInput() const
{
	...
	if (bCombatPushbackActive)
	{
		...
		{
			float ControlReduction = FMath::GetMappedRangeValueClamped(
				FVector2D(0.0f, 0.3f),
				FVector2D(0.1f, 0.5f),  // 10% to 50% control
				TimeSinceCombat
			);
...
{
    if (!bEnableProximitySpacing) return;
    
    // === PERFORMANCE OPTIMIZATION: STAGGERED UPDATES ===
    ProximityUpdateTimer += DeltaTime;
    if (ProximityUpdateTimer < ProximityUpdateInterval) return;
    ...
    if (Distance > PersonalSpaceRadius * 2.5f) return;
    ...
    if (bEitherInActiveCombat)
    {
        if (bVerboseLogging)
        {
            UE_LOG(LogTemp, Warning, TEXT("Proximity: Deferring to combat impulses - My Energy: %.1f, Opponent Energy: %.1f"), 
    ...
    if (MyCombatState.AttackConfidence > 0.3f || OpponentCombatState.AttackConfidence > 0.3f)
    {
        ...
        BaseSpacing *= FMath::Lerp(1.0f, 1.4f, FMath::Max(MyCombatState.AttackConfidence, OpponentCombatState.AttackConfidence));
    }
    ...
    if (Distance < BaseSpacing)
    {
        ...
        FVector BlendedDirection = UOHLocomotionUtils::BlendDirectionalInput(
        ...
        if (bVerboseLogging)
        {
            UE_LOG(LogTemp, Warning, 
                TEXT("Proximity Spacing: Dist=%.1f/%.1f, Penetration=%.1f%%, Push=%s, MyCombat=%.2f, OpponentCombat=%.2f"), 
...
void UOHMovementComponent::ProcessPhysicsImpulses(float DeltaTime)
{
    ...
    if (bIsCombatImpact && ImpulseMagnitude > 200.0f)
    {
        ...
        float MovementSpeed = FMath::GetMappedRangeValueClamped(
            FVector2D(200.0f, 2000.0f),
            FVector2D(CharMove->MaxWalkSpeed * 0.6f, CharMove->MaxWalkSpeed * 1.8f),
            ImpulseMagnitude
        );
        
        // Apply time-based decay for natural recovery
        float RecoveryFactor = FMath::GetMappedRangeValueClamped(
            FVector2D(0.0f, 0.8f),
            FVector2D(1.0f, 0.2f),
            TimeSinceCombat
        );
        ...
        if (MovementSpeed > 50.0f)
        {
            ...
            {
                UE_LOG(LogTemp, Warning, 
                    TEXT("Applied direct movement: Dir=%s, Scale=%.2f, Speed=%.1f"),
        ...
        float BlendInputScale = FMath::GetMappedRangeValueClamped(
            FVector2D(0.3f, 0.8f),
            FVector2D(0.6f, 0.1f),
            TimeSinceCombat
        );
    ...
    else
    {
        ...
        FVector LocalImpulse = Character->GetActorTransform().InverseTransformVector(AccumulatedPhysicsImpulse);
        float ProximityInputScale = FMath::GetMappedRangeValueClamped(
            FVector2D(10.0f, 300.0f),
            FVector2D(0.05f, 0.3f),
            ImpulseMagnitude
        );
...
bool UOHMovementComponent::IsCharacterInCombat(
	ACharacter* Character, 
	float& OutAttackConfidence, 
	float& OutChainSpeed)
{
	...
	bool bInCombat = CombatAnalysis.bIsAttacking && 
					CombatAnalysis.AttackConfidence > 0.3f && 
					CombatAnalysis.MaxChainSpeed > 200.0f; // Configurable threshold
    
	if (bVerboseLogging && bInCombat)
	{
		UE_LOG(LogTemp, Warning, 
			TEXT("Combat Detected: %s - Confidence=%.2f, Speed=%.1f, Energy=%.1f, Quality=%.2f"),
			*Character->GetName(),
			CombatAnalysis.AttackConfidence,
			CombatAnalysis.MaxChainSpeed,
			CombatAnalysis.TotalKineticEnergy,
			CombatAnalysis.ChainMotionQuality);
	}
