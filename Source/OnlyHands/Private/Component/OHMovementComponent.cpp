...
UOHMovementComponent::UOHMovementComponent() {
    ...
    InputIntentMagnitudeThreshold = 0.25f; // Reduced from 0.3 for easier triggering
    PivotThreshold = 45.f;
...
void UOHMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType,
    ...
    if (bDebugMovement && GEngine) {
        // Main status
        GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::White, FString::Printf(TEXT("=== MOVEMENT DEBUG ===")));

        // Input status
        GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Yellow,
                                         FString::Printf(TEXT("Input: Vec(%.2f,%.2f) Mag:%.2f Buffered:%.2f Intent:%s"),
                                                         MovementInputVector.X, MovementInputVector.Y,
                                                         InputIntentMagnitude, BufferedInputAmount,
                                                         bLastInputIntentValid ? TEXT("VALID") : TEXT("INVALID")));

        // Speed status
        GEngine->AddOnScreenDebugMessage(
            -1, 0.f, FColor::Cyan,
            FString::Printf(TEXT("Speed: Current:%.0f Buffered:%.0f"), LastCalculatedSpeed, BufferedSpeed));

        // Gait status
        const TCHAR* GaitNames[] = {TEXT("Walking"), TEXT("Running"), TEXT("Sprinting")};
        GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Green,
                                         FString::Printf(TEXT("Gait: %s (Alpha:%.2f) RunTimer:%.2f SprintTimer:%.2f"),
                                                         GaitNames[static_cast<int32>(CurrentGait)], GaitBlendAlpha,
                                                         BufferedTimeAboveRunThreshold,
                                                         BufferedTimeAboveSprintThreshold));
...
void UOHMovementComponent::ApplyMovementInput() {
    ...
    if (!MoveVector.IsNearlyZero()) {
        if (ACharacter* OwnerCharacter = Cast<ACharacter>(GetOwner())) {
            ...
            if (!LastMovementDirection.IsNearlyZero()) {
                float DirChange = FVector::DotProduct(MoveVector, LastMovementDirection);
    ...
    if (bDebugMovement) {
        if (AActor* Owner = GetOwner()) {
            ...
            DrawDebugLine(GetWorld(), Start, FacingEnd, FColor::Blue, false, -1, 0, 2.0f);

            // Draw momentum direction
            if (!LastMovementDirection.IsNearlyZero()) {
                FVector MomentumEnd = Start + LastMovementDirection * 100.f * (CurrentMomentumSpeed / SprintMaxSpeed);
                DrawDebugLine(GetWorld(), Start, MomentumEnd, FColor::Yellow, false, -1, 0, 2.0f);
            }
        }
    }
#endif
...
FVector UOHMovementComponent::CalculateMovementInputVector() const {
    ...
    FRotator ControlRotation = CalculateControlRotation(); // Your custom function!
...
FRotator UOHMovementComponent::CalculateControlRotation() const {
    ...
    if (UOHCombatUtils::GetPlayerCameraView(GetOwner(), CameraLocation, CameraRotation)) {
...
void UOHMovementComponent::UpdateInputBuffers(float DeltaTime) {
    ...
    if (CurrentInputAmount > BufferedInputAmount) {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, CurrentInputAmount, DeltaTime, InputBufferRiseRate);
    } else {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, CurrentInputAmount, DeltaTime, InputBufferDecay);
    }

    // Smooth buffered speed (always rise rate for smoothing)
    BufferedSpeed = FMath::FInterpTo(BufferedSpeed, CurrentSpeed, DeltaTime, InputBufferRiseRate);
...
void UOHMovementComponent::UpdateDynamicMovementSettings(EOHGait AllowedGait) {
    ...
    if (bUseMomentumBasedMovement) {
        ...
        if (bHasInput) {
            ...
            CurrentMomentumSpeed =
                FMath::FInterpTo(CurrentMomentumSpeed, BaseTargetSpeed, DeltaTime, InputSpeedBuildRate);
        } else {
            // Decay speed when no input
            CurrentMomentumSpeed = FMath::FInterpTo(CurrentMomentumSpeed, 0.f, DeltaTime, NoInputSpeedDecayRate);
        }
        ...
        if (bHasInput && bUseInputMagnitudeForSpeed) {
            float InputScale = FMath::GetMappedRangeValueClamped(FVector2D(JoystickDeadzone, 1.0f),
    ...
    float WeightMultiplier = FMath::Lerp(1.5f, 0.5f, MovementWeight);   // Heavy = slower accel
    float FrictionMultiplier = FMath::Lerp(0.7f, 1.3f, MovementWeight); // Heavy = more friction
    ...
    if (bDebugMovement && GEngine) {
        GEngine->AddOnScreenDebugMessage(
            -1, 0.0f, FColor::Green,
            FString::Printf(TEXT("Momentum: %.0f | Dir: %.2fx | Rot: %.2fx | Angle: %.0f°"), CurrentMomentumSpeed,
                            DirectionalSpeedMultiplier, RotationalSpeedMultiplier, CurrentMovementAngle));
    }
}
...
void UOHMovementComponent::CalculateDirectionalSpeedModifiers(ACharacter* OwnerChar) {
    ...
    if (!MovementDirection.IsNearlyZero()) {
        ...
        if (DirectionalSpeedCurve) {
            DirectionalSpeedMultiplier *= DirectionalSpeedCurve->GetFloatValue(CurrentMovementAngle);
        }
        ...
        if (!LastMovementDirection.IsNearlyZero()) {
            ...
            if (DirectionChangeAngle > 90.f) {
                float ChangeMultiplier = FMath::GetMappedRangeValueClamped(
                    FVector2D(90.f, 180.f), FVector2D(1.0f, SpeedRetentionOnDirectionChange), DirectionChangeAngle);
                DirectionalSpeedMultiplier *= ChangeMultiplier;
            }
        }

        // Smooth the direction change
        LastMovementDirection = FMath::VInterpTo(LastMovementDirection, MovementDirection,
                                                 GetWorld()->GetDeltaSeconds(), DirectionChangeSharpness);
    } else {
    ...
    if (RotationalSpeedCurve) {
        RotationalSpeedMultiplier *= RotationalSpeedCurve->GetFloatValue(CurrentAngularVelocity);
    }
}
...
void UOHMovementComponent::OnReceiveMoveForward(float ForwardValue) {
    ...
    if (bDebugMovement) {
        if (GEngine) {
            GEngine->AddOnScreenDebugMessage(static_cast<uint64>((PTRINT)this) + 11111, 0.05f, FColor::Yellow,
...
void UOHMovementComponent::OnReceiveMoveRight(float RightValue) {
    ...
    if (bDebugMovement) {
        if (GEngine) {
            GEngine->AddOnScreenDebugMessage(static_cast<uint64>((PTRINT)this) + 22222, 0.05f, FColor::Orange,
...
void UOHMovementComponent::SetMovementInputVector(const FVector2D& NewInput) {
    ...
    if (bDebugMovement) {
        if (GEngine) {
            GEngine->AddOnScreenDebugMessage(static_cast<uint64>((PTRINT)this) + 33333, 0.05f, FColor::Green,
...
EOHGait UOHMovementComponent::CalculateDesiredGait(float DeltaTime) {
    ...
    if (IntentMagnitude > BufferedInputAmount) {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, IntentMagnitude, DeltaTime, InputBufferRiseRate);
    } else {
        BufferedInputAmount = FMath::FInterpTo(BufferedInputAmount, IntentMagnitude, DeltaTime, InputBufferDecay);
    }

    // Smooth speed always
    BufferedSpeed = FMath::FInterpTo(BufferedSpeed, Speed, DeltaTime, InputBufferRiseRate);
    ...
    } else if (IntentGait == EOHGait::Running && BufferedSpeed > WalkSpeedThreshold) {
        BufferedTimeAboveRunThreshold += DeltaTime;
        BufferedTimeAboveSprintThreshold = 0.f;
    } else {
        BufferedTimeAboveRunThreshold = 0.f;
    ...
    if (IntentGait == EOHGait::Sprinting && BufferedTimeAboveSprintThreshold >= HoldTimeAboveSprintThreshold) {
        return EOHGait::Sprinting;
    } else if (IntentGait >= EOHGait::Running && BufferedTimeAboveRunThreshold >= HoldTimeAboveRunThreshold) {
...
void UOHMovementComponent::InterpolateGait(EOHGait DesiredGait, float DeltaTime) {
    if (CurrentGait != DesiredGait) {
        ...
        GaitBlendAlpha = FMath::FInterpTo(GaitBlendAlpha, 0.f, DeltaTime, GaitInterpSpeed);
        ...
        GaitBlendAlpha = FMath::FInterpTo(GaitBlendAlpha, 1.f, DeltaTime, GaitInterpSpeed);
    }
}
