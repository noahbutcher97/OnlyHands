    ...
    UObject* WorldContext)
{
    ...
    for (int32 BoneIdx = 0; BoneIdx < RefSkel.GetNum(); ++BoneIdx)
    {
        ...
        if (ParentIdx != INDEX_NONE)
        {
            ...
            if (UOHPhysicsComponent::EnsureBodiesAndConstraint_Static(BoneName, ParentName, SkeletalMeshComponent, PhysicsAsset))
                ++ParentConstraintsCreated;
        }
    }

    // 4. On-screen summary
    FString LogMsg = FString::Printf(TEXT("[OHPhysicsComponent] Batch Repair Complete: %d bodies, %d asset constraints, %d parent constraints created."),
...
FBodyInstance* UOHPhysicsComponent::EnsureBodyForBone_Static(FName BoneName, USkeletalMeshComponent* SkeletalMeshComponent, UPhysicsAsset* PhysicsAsset)
{
#if WITH_EDITOR
	...
	if (BodyIdx == INDEX_NONE)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] (Static) Created new body for bone: %s in PhysicsAsset."), *BoneName.ToString());
		//BodyIdx = NewBodyIdx;
	...
	if (!Body || !Body->IsValidBodyInstance())
	{
		...
		if (!Body || !Body->IsValidBodyInstance())
		{
			UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] (Static) Failed to create runtime BodyInstance for bone: %s"), *BoneName.ToString());
			return nullptr;
		}
	}
	return Body;
...
FConstraintInstance* UOHPhysicsComponent::EnsureConstraintBetweenBones_Static(FName Bone1, FName Bone2, USkeletalMeshComponent* SkeletalMeshComponent, UPhysicsAsset* PhysicsAsset)
{
#if WITH_EDITOR
      ...
      /*  int32 NewIdx = FPhysicsAssetUtils::CreateNewConstraint(PhysicsAsset, FName(*ConstraintName));
		if (NewIdx != INDEX_NONE)
		{
           ...
           NewConstraint->DefaultInstance.ConstraintBone2 = Bone2;
            UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] (Static) Created new constraint between %s and %s."), *Bone1.ToString(), *Bone2.ToString());
			FoundIdx = NewIdx;
        }*/
    }
#endif
    ...
    if (!RuntimeConstraint)
    {
        ...
        if (!RuntimeConstraint)
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] (Static) Failed to create runtime ConstraintInstance between %s and %s!"), *Bone1.ToString(), *Bone2.ToString());
        }
    }
    return RuntimeConstraint;
}

FConstraintInstance* UOHPhysicsComponent::EnsureBodiesAndConstraint_Static(FName Bone1, FName Bone2, USkeletalMeshComponent* SkeletalMeshComponent, UPhysicsAsset* PhysicsAsset)
{
	...
	FConstraintInstance* Constraint = EnsureConstraintBetweenBones_Static(Bone1, Bone2, SkeletalMeshComponent, PhysicsAsset);
	return Constraint;
}
...
void UOHPhysicsComponent::ValidateRuntimePhysicsAgainstTemplate()
{
    ...
    for (const USkeletalBodySetup* BodySetup : PhysicsAsset->SkeletalBodySetups)
    {
        ...
        if (!Body || !Body->IsValidBodyInstance())
        {
            UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] MISSING OR INVALID BODY for bone: %s on %s."), *BoneName.ToString(), *GetName());
        }
    }
    ...
    for (const UPhysicsConstraintTemplate* ConstraintTemplate : PhysicsAsset->ConstraintSetup)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Constraint [%s]: Bone1 (%s) NOT found in skeletal mesh!"), *JointName.ToString(), *Bone1.ToString());
            continue;
        }
        if (Bone2Idx == INDEX_NONE)
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Constraint [%s]: Bone2 (%s) NOT found in skeletal mesh!"), *JointName.ToString(), *Bone2.ToString());
            continue;
        }
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Constraint [%s]: Bone1 (%s) has NO valid BodyInstance!"), *JointName.ToString(), *Bone1.ToString());
            continue;
        }
        if (!Body2 || !Body2->IsValidBodyInstance())
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Constraint [%s]: Bone2 (%s) has NO valid BodyInstance!"), *JointName.ToString(), *Bone2.ToString());
            continue;
        }
        ...
        if (!bFoundConstraint)
        {
            UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] Constraint [%s] between %s <-> %s: NO matching runtime constraint found!"), *JointName.ToString(), *Bone1.ToString(), *Bone2.ToString());
    ...
    for (const FConstraintInstance* RuntimeConstraint : SkeletalMeshComponent->Constraints)
    {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] RUNTIME constraint between %s <-> %s does NOT exist in PhysicsAsset!"), 
...
FBodyInstance* UOHPhysicsComponent::EnsureBodyForBone(FName BoneName)
{
	...
	//	int32 NewBodyIdx = FPhysicsAssetUtils::CreateNewBody(PhysicsAsset, BoneName, Params);
		PhysicsAsset->UpdateBodySetupIndexMap();
		PhysicsAsset->UpdateBoundsBodiesArray();
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] Created new body for bone: %s in PhysicsAsset."), *BoneName.ToString());
	//	BodyIdx = NewBodyIdx;
#endif
	}
	...
	if (!Body || !Body->IsValidBodyInstance())
	{
		...
		if (!Body || !Body->IsValidBodyInstance())
		{
			UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Failed to create runtime BodyInstance for bone: %s"), *BoneName.ToString());
			return nullptr;
		}
	}
	return Body;
...
FConstraintInstance* UOHPhysicsComponent::EnsureConstraintBetweenBones(FName Bone1, FName Bone2)
{
    ...
    if (FoundIdx == INDEX_NONE)
    {
       ...
       /* int32 NewIdx = FPhysicsAssetUtils::CreateNewConstraint(PhysicsAsset, FName(*ConstraintName));
        if (NewIdx != INDEX_NONE)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] Created new constraint between %s and %s."), *Bone1.ToString(), *Bone2.ToString());
            FoundIdx = NewIdx;
        }*/
#endif
    }
    ...
    if (!RuntimeConstraint)
    {
        ...
        if (!RuntimeConstraint)
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsComponent] Failed to create runtime ConstraintInstance between %s and %s!"), *Bone1.ToString(), *Bone2.ToString());
        }
    }
    return RuntimeConstraint;
}
...
void UOHPhysicsComponent::AutoPopulateChainsForCurrentMesh()
{
	...
	UE_LOG(LogTemp, Log, TEXT("AutoPopulate: Matches UE5: %d, UE4: %d. Choosing %s Mannequin config."), UE5Match, UE4Match, *ChosenType);
...
void UOHPhysicsComponent::CreateBoneDriveInstance(FPhysicalAnimationBoneDriveInstance& BoneDrive, const FPhysicalAnimationDriveSettings& DriveSettings, const FTransform& InitialTargetTM)
{
    if (!SkeletalMeshComponent) return;
...
void UOHPhysicsComponent::DrawDebugInfo()
{
    ...
    static const TArray<FColor> ChainColors = {
        FColor::Red, FColor::Green, FColor::Blue, FColor::Cyan, FColor::Magenta, FColor::Yellow, FColor::Orange, FColor::Turquoise
    };
    ...
    for (const auto& Chain : RuntimeChains)
    {
        ...
        for (const auto& BoneDrive : Chain.BoneDrives)
        {
            ...
            if (BoneDrive.TargetActor)
            {
                ...
                FString Info = FString::Printf(TEXT("%s | Blend: %.2f | Error: %.1f"), *Chain.Settings.ChainName.ToString(), BoneDrive.BlendAlpha, Error);
