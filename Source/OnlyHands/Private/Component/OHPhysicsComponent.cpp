...
void UOHPhysicsComponent::SetupPhysicsAnimationSystem() {
  ...
  if (!SkeletalMeshComponent) {
    UE_LOG(LogTemp, Error,
           TEXT("[OHPhysicsComponent] No SkeletalMeshComponent found on %s!"),
           *GetName());
    return;
  }
  ...
  if (!PhysicsAsset) {
    if (DefaultPhysicsAsset) {
      ...
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsComponent] Used DefaultPhysicsAsset for %s."),
             *GetName());
    }
    /*#if WITH_EDITOR
                    else
                    {
                            ...
                            PhysicsAsset =
    GeneratePhysicsAssetForMesh(SkeletalMeshComponent->GetSkeletalMeshAsset());
                            if (PhysicsAsset)
                            {
                                    SkeletalMeshComponent->SetPhysicsAsset(PhysicsAsset,
    false); UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] Auto-generated
    PhysicsAsset for %s."), *GetName());
                            }
                    }
    #endif*/
  }
  if (!PhysicsAsset) {
    UE_LOG(LogTemp, Error,
           TEXT("[OHPhysicsComponent] No valid PhysicsAsset for %s! Setup "
                "aborted."),
           *GetName());
    return;
  }
  ...
  if (Chains.Num() == 0) {
    ...
    UE_LOG(LogTemp, Log,
           TEXT("[OHPhysicsComponent] Auto-populated chains for %s."),
           *GetName());
  }
...
void UOHPhysicsComponent::FixAllMissingPhysicsForComponent(
    USkeletalMeshComponent *SkeletalMeshComponent, UPhysicsAsset *PhysicsAsset,
  ...
  for (const USkeletalBodySetup *BodySetup : PhysicsAsset->SkeletalBodySetups) {
    ...
    if (!Body || !Body->IsValidBodyInstance()) {
      if (UOHPhysicsComponent::EnsureBodyForBone_Static(
              BoneName, SkeletalMeshComponent, PhysicsAsset))
        ++BodiesCreated;
    }
  }
  ...
  for (const UPhysicsConstraintTemplate *ConstraintTemplate :
    ...
    if (UOHPhysicsComponent::EnsureBodiesAndConstraint_Static(
            Bone1, Bone2, SkeletalMeshComponent, PhysicsAsset))
      ++ConstraintsCreated;
  }
  ...
  for (int32 BoneIdx = 0; BoneIdx < RefSkel.GetNum(); ++BoneIdx) {
    ...
    if (ParentIdx != INDEX_NONE) {
      ...
      if (UOHPhysicsComponent::EnsureBodiesAndConstraint_Static(
              BoneName, ParentName, SkeletalMeshComponent, PhysicsAsset))
  ...
  FString LogMsg = FString::Printf(
      TEXT("[OHPhysicsComponent] Batch Repair Complete: %d bodies, %d asset "
...
FBodyInstance *UOHPhysicsComponent::EnsureBodyForBone_Static(
    FName BoneName, USkeletalMeshComponent *SkeletalMeshComponent,
    UPhysicsAsset *PhysicsAsset) {
#if WITH_EDITOR
  int32 BodyIdx = PhysicsAsset->FindBodyIndex(BoneName);
  if (BodyIdx == INDEX_NONE) {
    FPhysAssetCreateParams Params;
    // int32 NewBodyIdx = FPhysicsAssetUtils::CreateNewBody(PhysicsAsset,
    // BoneName, Params);
    PhysicsAsset->UpdateBodySetupIndexMap();
    PhysicsAsset->UpdateBoundsBodiesArray();
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsComponent] (Static) Created new body for bone: %s "
                "in PhysicsAsset."),
           *BoneName.ToString());
    // BodyIdx = NewBodyIdx;
  }
  ...
  if (!Body || !Body->IsValidBodyInstance()) {
    ...
    if (!Body || !Body->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] (Static) Failed to create runtime "
                  "BodyInstance for bone: %s"),
             *BoneName.ToString());
      return nullptr;
    }
  }
...
FConstraintInstance *UOHPhysicsComponent::EnsureConstraintBetweenBones_Static(
    FName Bone1, FName Bone2, USkeletalMeshComponent *SkeletalMeshComponent,
    UPhysicsAsset *PhysicsAsset) {
#if WITH_EDITOR
  int32 FoundIdx = INDEX_NONE;
  for (int32 Idx = 0; Idx < PhysicsAsset->ConstraintSetup.Num(); ++Idx) {
    const UPhysicsConstraintTemplate *Constraint =
        PhysicsAsset->ConstraintSetup[Idx];
    const FConstraintInstance &AssetInst = Constraint->DefaultInstance;
    if ((AssetInst.ConstraintBone1 == Bone1 &&
         AssetInst.ConstraintBone2 == Bone2) ||
        (AssetInst.ConstraintBone1 == Bone2 &&
         AssetInst.ConstraintBone2 == Bone1)) {
      FoundIdx = Idx;
      break;
    }
  }
  if (FoundIdx == INDEX_NONE) {
    FString ConstraintName = FString::Printf(
        TEXT("Constraint_%s_%s"), *Bone1.ToString(), *Bone2.ToString());
    /*  int32 NewIdx = FPhysicsAssetUtils::CreateNewConstraint(PhysicsAsset,
      FName(*ConstraintName)); if (NewIdx != INDEX_NONE)
              {
                      UPhysicsConstraintTemplate* NewConstraint =
      PhysicsAsset->ConstraintSetup[NewIdx];
         NewConstraint->DefaultInstance.ConstraintBone1 = Bone1;
         NewConstraint->DefaultInstance.ConstraintBone2 = Bone2;
          UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] (Static) Created
      new constraint between %s and %s."), *Bone1.ToString(),
      *Bone2.ToString()); FoundIdx = NewIdx;
      }*/
  }
#endif
  ...
  if (!RuntimeConstraint) {
    ...
    if (!RuntimeConstraint) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] (Static) Failed to create runtime "
                  "ConstraintInstance between %s and %s!"),
...
FConstraintInstance *UOHPhysicsComponent::EnsureBodiesAndConstraint_Static(
    ...
    UPhysicsAsset *PhysicsAsset) {
  FBodyInstance *BodyA =
      EnsureBodyForBone_Static(Bone1, SkeletalMeshComponent, PhysicsAsset);
  FBodyInstance *BodyB =
      EnsureBodyForBone_Static(Bone2, SkeletalMeshComponent, PhysicsAsset);
  if (!BodyA || !BodyB)
    return nullptr;
  FConstraintInstance *Constraint = EnsureConstraintBetweenBones_Static(
      Bone1, Bone2, SkeletalMeshComponent, PhysicsAsset);
  return Constraint;
}
...
void UOHPhysicsComponent::ValidateRuntimePhysicsAgainstTemplate() {
  ...
  for (const USkeletalBodySetup *BodySetup : PhysicsAsset->SkeletalBodySetups) {
    ...
    if (!Body || !Body->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsComponent] MISSING OR INVALID BODY for bone: %s "
                  "on %s."),
  ...
  for (const UPhysicsConstraintTemplate *ConstraintTemplate :
    ...
    if (Bone1Idx == INDEX_NONE) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Constraint [%s]: Bone1 (%s) NOT found "
                  "in skeletal mesh!"),
    ...
    if (Bone2Idx == INDEX_NONE) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Constraint [%s]: Bone2 (%s) NOT found "
                  "in skeletal mesh!"),
             *JointName.ToString(), *Bone2.ToString());
      continue;
    }

    // 2. Are both bones present in the physics simulation (i.e., have valid
    // BodyInstances)?
    ...
    if (!Body1 || !Body1->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Constraint [%s]: Bone1 (%s) has NO "
                  "valid BodyInstance!"),
             *JointName.ToString(), *Bone1.ToString());
      continue;
    }
    if (!Body2 || !Body2->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Constraint [%s]: Bone2 (%s) has NO "
                  "valid BodyInstance!"),
             *JointName.ToString(), *Bone2.ToString());
      continue;
    }

    // 3. (Optional) Check if the runtime constraint exists in the
    // SkeletalMeshComponent
    bool bFoundConstraint = false;
    for (const FConstraintInstance *RuntimeConstraint :
         SkeletalMeshComponent->Constraints) {
      if (!RuntimeConstraint)
        continue;
      if ((RuntimeConstraint->ConstraintBone1 == Bone1 &&
           RuntimeConstraint->ConstraintBone2 == Bone2) ||
          (RuntimeConstraint->ConstraintBone1 == Bone2 &&
           RuntimeConstraint->ConstraintBone2 == Bone1)) {
    ...
    if (!bFoundConstraint) {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsComponent] Constraint [%s] between %s <-> %s: NO "
                  "matching runtime constraint found!"),
             *JointName.ToString(), *Bone1.ToString(), *Bone2.ToString());
      // Optionally, call a helper to create/repair the constraint!
    }
  }

  // 4. (Optional) Check for any runtime bodies or constraints NOT present in
  // the PhysicsAsset (out-of-sync issues)
  for (const FConstraintInstance *RuntimeConstraint :
       SkeletalMeshComponent->Constraints) {
    if (!RuntimeConstraint)
      continue;
    ...
    for (const UPhysicsConstraintTemplate *ConstraintTemplate :
         PhysicsAsset->ConstraintSetup) {
      const FConstraintInstance &AssetInst =
    ...
    if (!bAssetHasConstraint) {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsComponent] RUNTIME constraint between %s <-> %s "
                  "does NOT exist in PhysicsAsset!"),
             *RuntimeConstraint->ConstraintBone1.ToString(),
             *RuntimeConstraint->ConstraintBone2.ToString());
    }
  }
}
...
FBodyInstance *UOHPhysicsComponent::EnsureBodyForBone(FName BoneName) {
  ...
  if (BodyIdx == INDEX_NONE) {
    ...
    PhysicsAsset->UpdateBoundsBodiesArray();
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsComponent] Created new body for bone: %s in "
                "PhysicsAsset."),
           *BoneName.ToString());
    //	BodyIdx = NewBodyIdx;
#endif
  ...
  FBodyInstance *Body = SkeletalMeshComponent->GetBodyInstance(BoneName);
  if (!Body || !Body->IsValidBodyInstance()) {
    // The asset now has a body, but it might not be instanced yet (after
    // reinit)
    SkeletalMeshComponent->RecreatePhysicsState();
    Body = SkeletalMeshComponent->GetBodyInstance(BoneName);
    if (!Body || !Body->IsValidBodyInstance()) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Failed to create runtime BodyInstance "
                  "for bone: %s"),
             *BoneName.ToString());
      return nullptr;
    }
...
FConstraintInstance *
UOHPhysicsComponent::EnsureConstraintBetweenBones(FName Bone1, FName Bone2) {
  if (!PhysicsAsset || !SkeletalMeshComponent)
    return nullptr;
  ...
  if (FoundIdx == INDEX_NONE) {
    ...
    /* int32 NewIdx = FPhysicsAssetUtils::CreateNewConstraint(PhysicsAsset,
     ...
     PhysicsAsset->ConstraintSetup[NewIdx];
         ...
         UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsComponent] Created new
     constraint between %s and %s."), *Bone1.ToString(), *Bone2.ToString());
         FoundIdx = NewIdx;
     }*/
#endif
  }
  ...
  if (!RuntimeConstraint) {
    ...
    if (!RuntimeConstraint) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsComponent] Failed to create runtime "
                  "ConstraintInstance between %s and %s!"),
             *Bone1.ToString(), *Bone2.ToString());
    }
  }
...
FConstraintInstance *
UOHPhysicsComponent::EnsureBodiesAndConstraint(FName Bone1, FName Bone2) {
  // Ensure both bodies
...
void UOHPhysicsComponent::SetupEditorVisualization() {
  ...
  for (const USkeletalBodySetup *BodySetup : PhysicsAsset->SkeletalBodySetups) {
    ...
    DrawDebugBox(World, Loc, FVector(2, 2, 2), TM.GetRotation(), FColor::Cyan,
                 false, 15.f, 0, 0.75f);
    DrawDebugString(World, Loc, BodySetup->BoneName.ToString(), nullptr,
                    FColor::Cyan, 15.f, true);
  }
  ...
  for (const UPhysicsConstraintTemplate *ConstraintTemplate :
    ...
    DrawDebugDirectionalArrow(World, A, B, 12.0f, FColor::Yellow, false, 15.f,
                              0, 2.f);
  }
}
...
void UOHPhysicsComponent::InitializeRuntimeChains() {
  ...
  for (const auto &ChainSetting : Chains) {
    ...
    TArray<int32> BoneIndices =
        GetBoneIndicesInChain(ChainSetting.StartBone, ChainSetting.EndBone);
...
// --------- PER-TICK UPDATE ---------
void UOHPhysicsComponent::TickComponent(
...
void UOHPhysicsComponent::UpdatePhysicalAnimation(float DeltaTime) {
  ...
  for (FPhysicalAnimationChainRuntime &Chain : RuntimeChains) {
    ...
    Chain.RuntimeBlendAlpha =
        FMath::FInterpTo(Chain.RuntimeBlendAlpha, TargetAlpha, DeltaTime, 6.f);
    ...
    for (FPhysicalAnimationBoneDriveInstance &BoneDrive : Chain.BoneDrives) {
      ...
      if (BoneDrive.bActive && !BoneDrive.ConstraintInstance) {
        const int32 BoneIndex =
            SkeletalMeshComponent->GetBoneIndex(BoneDrive.BoneName);
        const FTransform AnimTM =
            SkeletalMeshComponent->GetBoneTransform(BoneIndex);

        CreateBoneDriveInstance(BoneDrive, BoneDrive.CurrentDriveSettings,
                                AnimTM);
      ...
      UpdateBoneDriveInstance(BoneDrive, AnimTM, Chain.Settings.DriveSettings,
                              BoneDrive.BlendAlpha);
    }
  }
}
...
void UOHPhysicsComponent::SetChainBlendAlpha(FName ChainName,
                                             float BlendAlpha) {
  for (auto &Chain : RuntimeChains) {
    if (Chain.Settings.ChainName == ChainName) {
...
void UOHPhysicsComponent::SetChainDriveSettings(
    FName ChainName, const FPhysicalAnimationDriveSettings &NewSettings) {
  for (auto &Chain : RuntimeChains) {
    if (Chain.Settings.ChainName == ChainName) {
...
void UOHPhysicsComponent::AutoPopulateDefaultUE5Chains() {
  ...
  // UE5 Mannequin (Manny/Quinn) - different bone names, especially for
  // arms/legs
  ...
  // If you want to support fingers, toes, etc, just add more chains here using
  // their actual bone names.
...
void UOHPhysicsComponent::AutoPopulateChainsForCurrentMesh() {
  ...
  const TArray<FString> UE5SignatureBones = {
      TEXT("spine_05"),   TEXT("clavicle_l"), TEXT("upperarm_l"),
      TEXT("lowerarm_l"), TEXT("hand_l"),     TEXT("thigh_l"),
      TEXT("calf_l"),     TEXT("foot_l"),     TEXT("neck_01"),
      TEXT("head")};
  const TArray<FString> UE4SignatureBones = {
      TEXT("spine_03"),   TEXT("clavicle_l"), TEXT("upperarm_l"),
      TEXT("lowerarm_l"), TEXT("hand_l"),     TEXT("thigh_l"),
      TEXT("calf_l"),     TEXT("foot_l"),     TEXT("neck_01"),
      TEXT("head")};
  ...
  FString ChosenType =
      (UE5Match > 6) ? TEXT("UE5") : (UE4Match > 6 ? TEXT("UE4") : TEXT("UE4"));
  UE_LOG(LogTemp, Log,
         TEXT("AutoPopulate: Matches UE5: %d, UE4: %d. Choosing %s Mannequin "
              "config."),
         UE5Match, UE4Match, *ChosenType);
...
// --------- BONE INDEX UTILITY ---------
TArray<int32> UOHPhysicsComponent::GetBoneIndicesInChain(FName StartBone,
                                                         FName EndBone) const {
...
void UOHPhysicsComponent::CreateBoneDriveInstance(
    FPhysicalAnimationBoneDriveInstance &BoneDrive,
  ...
  FBodyInstance *Body =
      SkeletalMeshComponent->GetBodyInstance(BoneDrive.BoneName);
  ...
  FPhysicsActorHandle TargetActor;
  {
    ...
    auto Sphere =
        MakeUnique<Chaos::FImplicitSphere3>(FVector::ZeroVector, 0.01f);
  ...
  Profile.LinearDrive.XDrive.bEnablePositionDrive =
      !DriveSettings.bIsLocalSimulation;
  Profile.LinearDrive.XDrive.bEnableVelocityDrive =
      !DriveSettings.bIsLocalSimulation;
  Profile.LinearDrive.YDrive.bEnablePositionDrive =
      !DriveSettings.bIsLocalSimulation;
  Profile.LinearDrive.YDrive.bEnableVelocityDrive =
      !DriveSettings.bIsLocalSimulation;
  Profile.LinearDrive.ZDrive.bEnablePositionDrive =
      !DriveSettings.bIsLocalSimulation;
  Profile.LinearDrive.ZDrive.bEnableVelocityDrive =
      !DriveSettings.bIsLocalSimulation;
  ...
  ConstraintInstance->SetRefFrame(EConstraintFrame::Frame1,
                                  FTransform::Identity);
  ConstraintInstance->SetRefFrame(EConstraintFrame::Frame2,
                                  FTransform::Identity);

  // --- 3. Init constraint (attach simulated bone to target actor) ---
  ConstraintInstance->InitConstraint_AssumesLocked(Body->ActorHandle,
                                                   TargetActor, 1.f);
...
void UOHPhysicsComponent::DestroyBoneDriveInstance(
    FPhysicalAnimationBoneDriveInstance &BoneDrive) {
  ...
  if (BoneDrive.TargetActor) {
    if (FChaosScene *PhysScene =
            FChaosEngineInterface::GetCurrentScene(BoneDrive.TargetActor)) {
...
void UOHPhysicsComponent::UpdateBoneDriveInstance(
    FPhysicalAnimationBoneDriveInstance &BoneDrive, const FTransform &TargetTM,
  ...
  FPhysicsInterface::SetKinematicTarget_AssumesLocked(BoneDrive.TargetActor,
                                                      TargetTM);
  ...
  const float AngularVelocityStrength =
      DriveSettings.AngularVelocityStrength * Blend;
  ...
  BoneDrive.ConstraintInstance->SetAngularDriveParams(
      OrientationStrength, AngularVelocityStrength, MaxAngularForce);
  ...
  if (DriveSettings.bIsLocalSimulation) {
    ...
    BoneDrive.ConstraintInstance->SetLinearDriveParams(
        PositionStrength, VelocityStrength, MaxLinearForce);
  }
}

bool UOHPhysicsComponent::IsChainStable(
    const FPhysicalAnimationChainRuntime &Chain) const {
  for (const FPhysicalAnimationBoneDriveInstance &BoneDrive :
       Chain.BoneDrives) {
    FBodyInstance *Body =
        SkeletalMeshComponent->GetBodyInstance(BoneDrive.BoneName);
    if (!Body)
    ...
    const FVector PosErr =
        Body->GetUnrealWorldTransform().GetLocation() - AnimTM.GetLocation();
...
/*
TArray<int32> UOHPhysicsComponent::GetBoneIndicesInChain(FName StartBone, FName
EndBone) const
{
        TArray<int32> Indices;
...
SkeletalMeshComponent->GetSkeletalMeshAsset()->GetRefSkeleton();
        ...
        while (CurrentIdx != INDEX_NONE)
        {
                ...
                if (CurrentIdx == EndIdx)
                        break;
                CurrentIdx = RefSkeleton.GetChildBoneIndex(CurrentIdx, 0); //
May need more logic for complex branches
        }
...
void UOHPhysicsComponent::EnablePhysicsOnChain(FName ChainName, bool bEnable) {
  ...
  for (FPhysicalAnimationChainSettings &Chain : Chains) {
    if (Chain.ChainName == ChainName) {
      ...
      TArray<int32> BoneIndices =
          GetBoneIndicesInChain(Chain.StartBone, Chain.EndBone);
      ...
      UE_LOG(
          LogTemp, Log, TEXT("[OHPhysicsComponent] Physics %s for chain '%s'"),
...
void UOHPhysicsComponent::SetDriveProfileForChain(
    FName ChainName, UOHPhysicalAnimationProfileDataAsset *Profile) {
  if (!Profile)
    return;
  if (!SkeletalMeshComponent)
    return;
  ...
  for (FPhysicalAnimationChainSettings &Chain : Chains) {
    if (Chain.ChainName == ChainName) {
      ...
      UE_LOG(LogTemp, Log,
             TEXT("[OHPhysicsComponent] Applied profile '%s' to chain '%s'"),
...
void UOHPhysicsComponent::DrawDebugInfo() {
  ...
  static const TArray<FColor> ChainColors = {
      FColor::Red,     FColor::Green,  FColor::Blue,   FColor::Cyan,
  ...
  for (const auto &Chain : RuntimeChains) {
    ...
    for (const auto &BoneDrive : Chain.BoneDrives) {
      ...
      DrawDebugSphere(World, BoneLoc, 2.0f, 12, ChainColor, false, -1.f, 0,
                      1.5f);

      // If you have target kinematic actors, draw their positions and error
      // lines
      if (BoneDrive.TargetActor) {
        // Note: This is the world location of your kinematic target actor
        // (animation pose)
        FVector TargetLoc = BoneDrive.TargetActor->GetGameThreadAPI().X();

        DrawDebugSphere(World, TargetLoc, 2.5f, 12, FColor::White, false, -1.f,
                        0, 1.0f);
        DrawDebugLine(World, BoneLoc, TargetLoc, FColor::Purple, false, -1.f, 0,
                      0.5f);
        ...
        FString Info = FString::Printf(TEXT("%s | Blend: %.2f | Error: %.1f"),
                                       *Chain.Settings.ChainName.ToString(),
                                       BoneDrive.BlendAlpha, Error);
        DrawDebugString(World, Mid, Info, nullptr, FColor::White, 0.f, true);
      } else {
        // Show bone name if no target
        DrawDebugString(World, BoneLoc, BoneDrive.BoneName.ToString(), nullptr,
                        ChainColor, 0.f, true);
      }
    }
  }
...
void UOHPhysicsComponent::PreviewChainsInEditor() {
  ...
  for (const FPhysicalAnimationChainSettings &Chain : Chains) {
    ...
    for (int32 i = 0; i < BoneIndices.Num(); ++i) {
      ...
      DrawDebugSphere(World, BoneLoc, 2.0f, 12, ChainColor, false, 10.0f, 0,
                      1.0f);

      if (i > 0) {
        DrawDebugLine(World, LastLoc, BoneLoc, ChainColor, false, 10.0f, 0,
                      0.75f);
      }
      LastLoc = BoneLoc;
    }
    ...
    if (BoneIndices.Num() >= 2) {
      FVector StartLoc =
          SkeletalMeshComponent->GetBoneTransform(BoneIndices[0]).GetLocation();
      FVector EndLoc =
          SkeletalMeshComponent->GetBoneTransform(BoneIndices.Last())
              .GetLocation();
      FVector Mid = (StartLoc + EndLoc) * 0.5f;
      DrawDebugString(World, Mid, Chain.ChainName.ToString(), nullptr,
                      ChainColor, 10.0f, true);
    }
  }
#endif
}
    ...
    if (ParentIdx == INDEX_NONE || !BodyBoneIndices.Contains(ParentIdx)) {
      ...
      while (true) {
        ...
        for (int32 ChildIdx = 0; ChildIdx < RefSkel.GetNum(); ++ChildIdx) {
          if (RefSkel.GetParentIndex(ChildIdx) == CurrentIdx &&
              BodyBoneIndices.Contains(ChildIdx)) {
      ...
      if (ChainIndices.Num() > 1) {
        ...
        FString AutoName = FString::Printf(
            TEXT("Chain_%s_%s"), *StartBone.ToString(), *EndBone.ToString());

        // Optional: Try to detect limb type by name for default drive
        FPhysicalAnimationDriveSettings Defaults =
            GetDefaultDriveForChain(StartBone, EndBone);

        Chains.Add(FPhysicalAnimationChainSettings{
            FName(AutoName), StartBone, EndBone, Defaults, 1.f, true});
      }
    }
  }
}
...
void UOHPhysicsComponent::VisualizeChainsInEditor() {
  ...
  for (const FPhysicalAnimationChainSettings &Chain : Chains) {
    ...
    FVector LastLoc =
        SkeletalMeshComponent->GetBoneTransform(CurrIdx).GetLocation();

    // Always show chain name at midpoint
    FVector EndLoc =
        SkeletalMeshComponent->GetBoneTransform(EndIdx).GetLocation();
    FVector Mid = (LastLoc + EndLoc) * 0.5f;
    DrawDebugString(World, Mid, Chain.ChainName.ToString(), nullptr, ChainColor,
                    10.f, true);
    ...
    while (true) {
      ...
      FVector NextLoc =
          SkeletalMeshComponent->GetBoneTransform(NextIdx).GetLocation();
      DrawDebugDirectionalArrow(World, LastLoc, NextLoc, 10.0f, ChainColor,
                                false, 10.f, 0, 2.f);
      DrawDebugSphere(World, LastLoc, 2.5f, 12, ChainColor, false, 10.f, 0,
                      0.75f);
      LastLoc = NextLoc;
      CurrIdx = NextIdx;
    }
    ...
    if (CurrIdx != EndIdx) {
      FVector FinalLoc =
          SkeletalMeshComponent->GetBoneTransform(EndIdx).GetLocation();
      DrawDebugDirectionalArrow(World, LastLoc, FinalLoc, 10.0f, ChainColor,
                                false, 10.f, 0, 2.f);
      DrawDebugSphere(World, FinalLoc, 3.f, 14, ChainColor, false, 10.f, 0,
                      1.0f);
    }
  }
#endif
...
FPhysicalAnimationDriveSettings
UOHPhysicsComponent::GetDefaultDriveForChain(FName StartBone, FName EndBone) {
  // Example: set strong for spine, soft for arms/legs, etc. Customize as
  // needed!
  ...
  if (Name.Contains(TEXT("upperarm")) || Name.Contains(TEXT("forearm")) ||
