...
UOHPhysicsManager::UOHPhysicsManager() : PhysicsGraph() {
  ...
  TrackedBoneDefinitions = {
      "root",       "pelvis",     "spine_01",   "spine_02",   "spine_03",
      "neck_01",    "head",       "clavicle_l", "clavicle_r", "upperarm_l",
      "upperarm_r", "lowerarm_l", "lowerarm_r", "hand_l",     "hand_r",
      "thigh_l",    "thigh_r",    "calf_l",     "calf_r",     "foot_l",
      "foot_r",     "ball_l",     "ball_r"};
...
void UOHPhysicsManager::BeginPlay() {
  ...
  GetWorld()->GetTimerManager().SetTimer(
      TimerHandle, this, &UOHPhysicsManager::InitializePhysicsManager,
      StartTickDelay, false);
}

void UOHPhysicsManager::TickComponent(
  ...
  if (UOHGraphUtils::GetPhysicsGraphOverlayMode() !=
      EPhysicsGraphOverlayMode::None) {
...
void UOHPhysicsManager::InitializePhysicsManager() {
  ...
  SkeletalMesh =
      GetOwner() ? GetOwner()->FindComponentByClass<USkeletalMeshComponent>()
                 : nullptr;
  if (!SkeletalMesh || !IsValid(SkeletalMesh) ||
      !SkeletalMesh->IsRegistered()) {
    UE_LOG(LogTemp, Error,
           TEXT("[OHPhysicsManager] Initialization aborted — SkeletalMesh is "
                "missing or invalid."));
    return;
  }
  ...
  if (!CachedPhysicsAsset) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] SkeletalMesh has no PhysicsAsset — "
                "simulation features will be limited."));
  }
  ...
  if (World && World->IsGameWorld() && SkeletalMesh->IsRegistered()) {
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Skipping RecreatePhysicsState — invalid or "
                "editor-only world."));
  }

  // === STEP 7: Debug logging ===
  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] Init complete: %d Tracked, %d Simulatable"),
         TrackedBoneDataMap.Num(), SimulatableBones.Num());

  UOHGraphUtils::BuildPhysicsGraphFromComponent(SkeletalMesh, PhysicsGraph,
                                                "INIT", true);
  ...
  if (!PhysicsGraph.ValidateAllConstraintGeometry()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Constraint validation returned failure! "
...
void UOHPhysicsManager::InitializePhysicalAnimationSystem() {
  ...
  if (!PhysicalAnimationComponent) {
    ...
    PhysicalAnimationComponent = NewObject<UPhysicalAnimationComponent>(
        GetOwner(), UPhysicalAnimationComponent::StaticClass(),
        TEXT("AutoPhysicalAnim"));
...
void UOHPhysicsManager::InitializeTrackedBoneData() {
  ...
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] InitializeTrackedBoneData aborted — "
  ...
  for (const FName &TrackedBone : TrackedBoneDefinitions) {
    ...
    if (ValidatedBoneMapping.Contains(TrackedBone)) {
      ...
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsManager] Using suggested bone '%s' for '%s'"),
    ...
    BoneData.SetIsSimulating(
        false); // may be updated later during SimulatableBone initialization

    // Optional: Prime motion history with an initial zero sample
    FOHMotionSample InitialSample = FOHMotionSample::CreateFromState(
        BoneTransform, FVector::ZeroVector, FVector::ZeroVector,
...
void UOHPhysicsManager::InitializeSimulatableBones() {
  ...
  if (!UsePhysicsAsset && SkeletalMesh) {
    ...
    if (UsePhysicsAsset) {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsManager] CachedPhysicsAsset was null — fallback "
                  "to SkeletalMesh asset."));
    }
  }

  if (!UsePhysicsAsset) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Skipping simulatable bone setup — no valid "
                "PhysicsAsset."));
    return;
  }
...
void UOHPhysicsManager::InitializeAllPhysicalAnimationProfiles(
    const FPhysicalAnimationData &Profile) {
  if (!SkeletalMesh || !PhysicalAnimationComponent) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] InitializeAllPhysicalAnimationProfiles "
                "aborted — missing components."));
    return;
  }

  for (const FName &Bone : SimulatableBones) {
    ApplyUnifiedPhysicsProfileToChain(Bone, Profile, 5.0f, 10.0f,
                                      bEnableRuntimeTuningEvaluation);

    UE_LOG(LogTemp, Log,
           TEXT("[OHPhysicsManager] Applied PAC zero profile to Bone: %s"),
           *Bone.ToString());
  }

  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] Physical animation profiles applied to %d "
              "bones."),
...
void UOHPhysicsManager::InitializeBoneLinksFromPhysicsAsset() {
  if (!CachedPhysicsAsset) {
    UE_LOG(
        LogTemp, Warning,
        TEXT("InitializeBoneLinksFromPhysicsAsset: No physics asset found."));
    return;
  }
  ...
  for (const UPhysicsConstraintTemplate *Constraint :
       CachedPhysicsAsset->ConstraintSetup) {
    if (!Constraint)
      continue;
    ...
    const float Damping = Constraint->DefaultInstance.ProfileInstance
                              .AngularDrive.SlerpDrive.Damping;
    ...
    ParentLink.DampingMultiplier =
        FMath::Clamp(Damping / 50.f, 0.1f, 2.f); // Normalize to range
    ...
    if (!ChildLink.Parent.IsNone() && ChildLink.Parent != BoneA) {
      UE_LOG(LogTemp, Warning,
             TEXT("Bone %s already has parent %s, now also %s — check "
                  "constraint structure."),
             *BoneB.ToString(), *ChildLink.Parent.ToString(),
             *BoneA.ToString());
    }
  ...
  UE_LOG(LogTemp, Log,
         TEXT("BoneLinkMap initialized with %d entries from PhysicsAsset."),
         BoneLinkMap.Num());
}
...
void UOHPhysicsManager::ResetPhysicsManager() {
  if (!SkeletalMesh || !IsValid(SkeletalMesh) ||
      !SkeletalMesh->IsRegistered()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Reset aborted: SkeletalMesh is invalid or "
                "unregistered."));
    return;
  }
  ...
  if (SkeletalMesh->GetWorld() && SkeletalMesh->GetWorld()->IsGameWorld()) {
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Skipping RecreatePhysicsState: Not in game "
                "world or world is invalid."));
  }
  ...
  if (IsValid(PhysicalAnimationComponent)) {
    ...
    for (const FName &BoneName : BoneNames) {
      PhysicalAnimationComponent->ApplyPhysicalAnimationProfileBelow(
          BoneName, NAME_None, false, false);
    }
  }
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] Full physics reset and reinit complete."));
}
...
void UOHPhysicsManager::ClearPhysicsState() {
  if (!SkeletalMesh || !IsValid(SkeletalMesh) ||
      !SkeletalMesh->IsRegistered()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] ClearPhysicsState aborted: SkeletalMesh is "
...
void UOHPhysicsManager::ForceReinitializePhysics(bool bLogResults) {
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] ForceReinitializePhysics aborted — "
                "SkeletalMesh is null."));
    return;
  }
  ...
  if (bLogResults) {
    UE_LOG(LogTemp, Log,
           TEXT("[OHPhysicsManager] ForceReinitializePhysics completed in %.2f "
                "ms. Bones: %d tracked, %d simulatable."),
...
USkeletalMeshComponent *UOHPhysicsManager::GetSkeletalMeshComponent() const {
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] GetSkeletalMeshComponent: Missing SkeletalMesh"));
    return nullptr;
  }
  return SkeletalMesh;
}
...
void UOHPhysicsManager::UpdateTrackedBoneKinematics(float DeltaTime) {
  ...
  const float TimeStamp =
      GetCurrentMotionTime(); // Supports anim vs phys tick domain
  ...
  for (const FName &BoneName : TrackedBoneDefinitions) {
    ...
    // Directly pass the timestamp to UpdateKinematics (which calls
    // AddMotionSample internally)
...
void UOHPhysicsManager::PlayPhysicsHitReaction(
    FName BoneName, EOHPhysicsProfile Profile, float BlendIn, float Hold,
    float BlendOut, FVector OptionalImpulseDir, float OptionalImpulseStrength,
    FName ReactionTag) {
  const FPhysicalAnimationData *ProfileData = PhysicsProfiles.Find(Profile);
  if (!ProfileData) {
    UE_LOG(LogTemp, Warning, TEXT("[OH] No PAC profile found for enum %d"),
           static_cast<uint8>(Profile));
    return;
  }

  PlayPhysicsHitReaction_Internal(BoneName, *ProfileData, BlendIn, Hold,
                                  BlendOut, OptionalImpulseDir,
                                  OptionalImpulseStrength, ReactionTag);
}

void UOHPhysicsManager::PlayPhysicsHitReactionCustom(
    FName BoneName, const FPhysicalAnimationData &CustomProfile, float BlendIn,
    float Hold, float BlendOut, FVector OptionalImpulseDir,
    float OptionalImpulseStrength, FName ReactionTag) {
  PlayPhysicsHitReaction_Internal(BoneName, CustomProfile, BlendIn, Hold,
                                  BlendOut, OptionalImpulseDir,
                                  OptionalImpulseStrength, ReactionTag);
}

void UOHPhysicsManager::PlayPhysicsHitReaction_Internal(
    FName &BoneName, const FPhysicalAnimationData &ProfileData, float BlendIn,
    float Hold, float BlendOut, FVector OptionalImpulseDir,
    float OptionalImpulseStrength, FName ReactionTag,
    TArray<FName> *OutAffectedBones /* optional */) {
  if (!SkeletalMesh || !PhysicalAnimationComponent) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] HitReaction failed: Missing SkeletalMesh or PAC"));
    return;
  }
  if (!SkeletalMesh->DoesSocketExist(BoneName)) {
    if (BoneSuggestions.Contains(BoneName)) {
      ...
      UE_LOG(LogTemp, Log,
             TEXT("[OH] Using suggested bone '%s' for hit reaction on '%s'"),
             *BoneName.ToString(), *OriginalBone.ToString());
    } else {
      UE_LOG(LogTemp, Warning,
             TEXT("[OH] HitReaction failed: Bone '%s' not found"),
             *BoneName.ToString());
      return;
    }
  }
  ...
  for (const FName &Bone : BoneNames) {
    ...
    if (!ExistingPAC || !ArePACProfilesEqual(*ExistingPAC, ProfileData) ||
        !IsBoneBlending(Bone)) {
      const FPhysicalAnimationData ScaledProfile =
          GetScaledPACProfile(Bone, ProfileData);
  ...
  if (!OptionalImpulseDir.IsNearlyZero() && OptionalImpulseStrength > 0.f) {
    if (FBodyInstance *Body = SkeletalMesh->GetBodyInstance(BoneName)) {
      if (Body->IsValidBodyInstance() && Body->IsInstanceSimulatingPhysics()) {
        const FVector SafeImpulse =
            OptionalImpulseDir.GetSafeNormal() * OptionalImpulseStrength;
  ...
  const FActivePhysicsBlend Blend = CreatePhysicsBlendState(
      BoneName, StartAlpha, BlendIn, Hold, BlendOut, ReactionTag);

  ActiveBlends.Add(BoneName, Blend);

  UE_LOG(LogTemp, Log,
         TEXT("[OH] Hit reaction: %s | In=%.2f Hold=%.2f Out=%.2f | Alpha=%.2f "
              "| Bones=%d"),
         *BoneName.ToString(), BlendIn, Hold, BlendOut, StartAlpha,
         AffectedBones.Num());
...
void UOHPhysicsManager::ApplyImpulseToBone(FName Bone, const FVector &Direction,
                                           float Magnitude) {
  if (!SkeletalMesh || !SkeletalMesh->IsSimulatingPhysics(Bone)) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Skipping impulse: Bone %s not simulating."),
           *Bone.ToString());
    return;
  }
  ...
  UE_LOG(LogTemp, Verbose,
         TEXT("[OHPhysicsManager] Applied impulse to %s: %s (raw %.2f, scaled "
              "%.2f)"),
         *Bone.ToString(), *ScaledImpulse.ToString(), Magnitude,
         ScaledImpulse.Size());
}

void UOHPhysicsManager::PropagateImpulse(FName OriginBone, FVector Impulse,
                                         float Strength,
                                         EOHPhysicsProfile Profile,
                                         int32 Depth) {
...
/*void UOHPhysicsManager::ApplySimSettingsToBone(const FName& Bone, const
FPhysicalAnimationData& BaseProfile)
{
        ...
        ComputePhysicsTweaksForBone(Bone, PACMultiplier, LinearDamping,
AngularDamping);
        ...
        // Apply PAC
        PhysicalAnimationComponent->ApplyPhysicalAnimationSettings(Bone,
ScaledProfile); ActiveBonePACData.Add(Bone, ScaledProfile);
...
PhysicsAsset->FindConstraintIndex(Bone); if (ConstraintIndex != INDEX_NONE)
                {
...
PhysicsAsset->ConstraintSetup[ConstraintIndex]->DefaultInstance;
                                FConstraintProfileProperties Profile =
Constraint.ProfileInstance;

                                ComputeOptimalConstraintSettings(&Constraint,
Profile, Bone); Constraint.ProfileInstance = Profile;

                                UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager]
[%s] Applied PAC x%.2f | Damping L %.2f A %.2f | Constraint tuned to %s"),
                                        *GetOwner()->GetName(), PACMultiplier,
LinearDamping, AngularDamping, *Bone.ToString()); return;
                        }
                }
        }

        // Fallback log if constraint not applied
        UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f |
Damping L %.2f A %.2f to %s (No constraint)"), *GetOwner()->GetName(),
PACMultiplier, LinearDamping, AngularDamping, *Bone.ToString());
}*/

void UOHPhysicsManager::ApplySimSettingsToBone(
    const FName &Bone, const FPhysicalAnimationData &BaseProfile) {
  if (!PhysicalAnimationComponent || !SkeletalMesh)
    return;
  ...
  if (FBodyInstance *Body = SkeletalMesh->GetBodyInstance(Bone)) {
    Body->SetInstanceSimulatePhysics(
        true); // Make sure physics is actually simulating
    Body->LinearDamping = LinearDamping;
    Body->AngularDamping = AngularDamping;
  ...
  if (UPhysicsAsset *PhysicsAsset = SkeletalMesh->GetPhysicsAsset()) {
    ...
    if (ConstraintIndex != INDEX_NONE &&
      ...
      FConstraintInstance &TemplateConstraint =
          PhysicsAsset->ConstraintSetup[ConstraintIndex]->DefaultInstance;
      ...
      if (FConstraintInstance *RuntimeCI = SkeletalMesh->FindConstraintInstance(
              TemplateConstraint.JointName)) {
        ...
        RuntimeCI->SetAngularDriveParams(Profile.ConeLimit.Stiffness,
                                         Profile.ConeLimit.Damping,
      ...
      UE_LOG(LogTemp, Log,
             TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f | Damping L %.2f "
                  "A %.2f | Constraint tuned"),
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f | Damping L %.2f A "
              "%.2f (No constraint)"),
         *Bone.ToString(), PACMultiplier, LinearDamping, AngularDamping);
}

void UOHPhysicsManager::ActivateSimulationForChain(
    FName RootBone, const FPhysicalAnimationData &ProfileData,
    float StartAlpha) {
  if (!SkeletalMesh || !PhysicalAnimationComponent)
    return;
...
bool UOHPhysicsManager::TryActivateSimForBone(FName BoneName,
                                              float BlendAlpha) {
...
bool UOHPhysicsManager::ActivatePhysicsStateForBone(FName BoneName,
                                                    float BlendAlpha) {
  if (!SkeletalMesh)
    return false;
  ...
  if (!Body || !Body->IsValidBodyInstance()) {
    UE_LOG(LogTemp, Warning, TEXT("[OH] Bone '%s' has invalid body instance"),
           *BoneName.ToString());
    return false;
  }
...
void UOHPhysicsManager::ClearPhysicsStateForBone(FName Bone) {
  if (!SkeletalMesh || !PhysicalAnimationComponent) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] ClearPhysicsStateForBone: Missing skeletal mesh or PAC"));
    return;
  }

  // Ensure bone exists in skeleton
  if (SkeletalMesh->GetBoneIndex(Bone) == INDEX_NONE) {
    UE_LOG(LogTemp, Warning, TEXT("[OH] Bone '%s' not found in skeleton"),
           *Bone.ToString());
    return;
  }
  ...
  if (!Body || !Body->IsValidBodyInstance()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] Bone '%s' has invalid or missing body instance"),
           *Bone.ToString());
    return;
  }
  ...
  if (Body->GetBodyMass() <= KINDA_SMALL_NUMBER) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] Bone '%s' has near-zero mass; skipping clear"),
           *Bone.ToString());
    return;
  }
  ...
  PhysicalAnimationComponent->ApplyPhysicalAnimationSettings(
      Bone, FPhysicalAnimationData());

  // Cleanup internal tracking
  ActiveBonePACData.Remove(Bone);
  BoneSimBlendRefCount.Remove(
      Bone); // Defensive, though handled in TryDeactivate

#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
  if (Body->PhysicsBlendWeight > 0.01f) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OH] Bone '%s' still has non-zero blend weight after clear"),
           *Bone.ToString());
  }
#endif
}
...
void UOHPhysicsManager::ClearAllSimulatedBones() {
  ...
  for (const FName &Bone : BoneNames) {
    if (BoneSimBlendRefCount.Contains(Bone) ||
        ActiveBonePACData.Contains(Bone)) {
...
void UOHPhysicsManager::ApplyUnifiedPhysicsProfile(
    FName BoneName, const FPhysicalAnimationData &Profile,
    float CustomLinearDamping, float CustomAngularDamping, bool bVerboseLog) {
  if (!SkeletalMesh || !PhysicalAnimationComponent) {
    UE_LOG(LogTemp, Warning,
           TEXT("[ApplyUnifiedPhysicsProfile] Missing SkeletalMesh or PAC"));
    return;
  }
  ...
  if (bVerboseLog) {
    UE_LOG(LogTemp, Log,
           TEXT("[ApplyUnifiedPhysicsProfile] PAC applied to bone: %s"),
           *BoneName.ToString());
  }
  ...
  if (FBodyInstance *Body = SkeletalMesh->GetBodyInstance(BoneName)) {
    ...
    if (bVerboseLog) {
      UE_LOG(LogTemp, Log,
             TEXT("  - Damping set [Linear: %.2f | Angular: %.2f]"),
             CustomLinearDamping, CustomAngularDamping);
    }
  }

  // === Apply Constraint Tuning ===
  if (UPhysicsAsset *PhysicsAsset = SkeletalMesh->GetPhysicsAsset()) {
    for (const UPhysicsConstraintTemplate *Template :
      ...
      if (ConstraintBone == BoneName) {
        if (FConstraintInstance *Instance =
                SkeletalMesh->FindConstraintInstance(
                    Template->DefaultInstance.JointName)) {
          ...
          Instance->SetAngularDriveParams(Stiffness, Damping,
                                          0.f); // Unlimited force
          ...
          if (bVerboseLog) {
            UE_LOG(LogTemp, Log,
                   TEXT("  - Constraint '%s' updated [Stiffness: %.1f | "
                        "Damping: %.1f]"),
...
void UOHPhysicsManager::ApplyUnifiedPhysicsProfileToChain(
    FName RootBone, const FPhysicalAnimationData &Profile,
    float CustomLinearDamping, float CustomAngularDamping, bool bVerboseLog) {
  if (!SkeletalMesh) {
    UE_LOG(LogTemp, Warning,
           TEXT("[ApplyUnifiedPhysicsProfileToChain] SkeletalMesh is null"));
    return;
  }
  ...
  while (!BoneQueue.IsEmpty()) {
    ...
    ApplyUnifiedPhysicsProfile(CurrentBone, Profile, CustomLinearDamping,
                               CustomAngularDamping, bVerboseLog);
    ...
    const FReferenceSkeleton &RefSkel =
        SkeletalMesh->GetSkeletalMeshAsset()->GetRefSkeleton();
...
bool UOHPhysicsManager::IsBoneValidForChain(FName Bone, FName RootBone) const {
  if (!SkeletalMesh)
    return false;
  return IsBoneMassValid(Bone) && IsBoneNamePatternValid(Bone) &&
         IsBoneInChain(Bone, RootBone);
}
...
bool UOHPhysicsManager::IsBoneMassValid(FName BoneName) const {
  ...
  if (FBodyInstance *Body = SkeletalMesh->GetBodyInstance(BoneName)) {
    return Body->IsValidBodyInstance() &&
           Body->GetBodyMass() > KINDA_SMALL_NUMBER;
  }
...
bool UOHPhysicsManager::IsBoneNamePatternValid(FName BoneName) {
  ...
  if (BoneNameStr.Contains(TEXT("twist")) || BoneNameStr.Contains(TEXT("ik")) ||
      BoneNameStr.Contains(TEXT("attach")) ||
      BoneNameStr.Contains(TEXT("helper")) ||
      BoneNameStr.StartsWith(TEXT("vb_")) ||
      BoneNameStr.EndsWith(TEXT("_dummy"))) {
...
bool UOHPhysicsManager::HasPhysicsBody(const FName &BoneName) const {
  UPhysicsAsset *Asset =
      CachedPhysicsAsset
          ? CachedPhysicsAsset
          : (SkeletalMesh ? SkeletalMesh->GetPhysicsAsset() : nullptr);
...
void UOHPhysicsManager::DrawDebugPACValues() const {
  ...
  for (const FName &Bone : SimulatableBones) {
    ...
    const FString DebugText = FString::Printf(
        TEXT(
            "%s\nOri: %.1f Pos: %.1f\nVel: %.1f AngVel: %.1f\nMaxF: %.0f/%.0f"),
        *Bone.ToString(), Profile->OrientationStrength,
        Profile->PositionStrength, Profile->VelocityStrength,
        Profile->AngularVelocityStrength, Profile->MaxLinearForce,
        Profile->MaxAngularForce);

    FColor StrengthColor = FColor::Green;

    const float TotalStrength =
        Profile->OrientationStrength + Profile->PositionStrength +
    ...
    DrawDebugString(GetWorld(), BoneLocation + Offset, DebugText, nullptr,
                    StrengthColor, 0.f, false);
  }
}
...
void UOHPhysicsManager::LogSimulatableBones() const {
  ...
  UE_LOG(LogTemp, VeryVerbose,
         TEXT("[OHPhysicsManager] [%s] LogSimulatableBones called on frame %d, "
              "list size: %d"),
  ...
  if (SimulatableBones.Num() == 0) {
    if (!bWasEmptyLastTick) {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsManager] [%s] No bones in SimulatableBones list."),
             *OwnerName);
  ...
  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] [%s] SimulatableBones (%d): [%s]"),
         *OwnerName, SimulatableBones.Num(), *BoneList);
}
  ...
  if (MissingBones.Num() > 0) {
    ...
    for (const FName &Missing : MissingBones) {
      ...
      if (BoneSuggestions.Contains(Missing)) {
        Report += FString::Printf(TEXT(" → Try '%s'"),
                                  *BoneSuggestions[Missing].ToString());
      }
      Report += TEXT("\n");
    }
    Report += TEXT("\n");
...
void UOHPhysicsManager::RebuildDirectPointerMapsForTesting() {
  ...
  for (const UPhysicsConstraintTemplate *ConstraintTemplate :
       CachedPhysicsAsset->ConstraintSetup) {
    if (ConstraintTemplate) {
      ...
      if (!Bone1.IsNone())
        DirectConstraintTemplateMap.Add(
            Bone1,
            const_cast<UPhysicsConstraintTemplate *>(ConstraintTemplate));
      if (!Bone2.IsNone())
        DirectConstraintTemplateMap.Add(
            Bone2,
            const_cast<UPhysicsConstraintTemplate *>(ConstraintTemplate));
      ...
      if (FConstraintInstance *RuntimeInstance =
              SkeletalMesh->FindConstraintInstance(JointName)) {
        if (!Bone1.IsNone())
...
void UOHPhysicsManager::PerformSystemComparison() {
  ...
  for (const auto &Pair : TrackedBoneDataMap) {
    ...
    if (const FOHBoneData *GraphBoneData =
            PhysicsGraph.GetBoneMap().Find(BoneName)) {
  ...
  for (const FOHConstraintInstanceData &ConstraintData :
       PhysicsGraph.GetConstraintLinks()) {
    CurrentFrameDebugData.GraphTraversalCount++;

    // Test graph-based constraint instance access
    if (FConstraintInstance *GraphConstraintInstance =
            ConstraintData.GetConstraintInstance()) {
      ...
      // Meaningfully use values (prevents both optimization and unused
      // warnings)
      CurrentFrameDebugData.GraphMemoryFootprint +=
  ...
  // Calculate memory footprints using proper methods (this will overwrite the
  ...
  FString OverviewText = FString::Printf(
      TEXT("=== ONLYHANDS PHYSICS MANAGER DEBUG ===\n") TEXT(
          ...
          TEXT("Tracked Bones: %d | Simulatable: %d\n") TEXT(
              "Active Blends: %d | Sim Ref Count Bones: %d\n")
              TEXT("Physics Graph Nodes: %d | Constraint Links: %d\n") TEXT(
                  "Graph Constraint Instances: %d Active, %d Inactive\n")
                  TEXT(
                      "Direct Body Map: %d | Direct Constraint Templates: %d\n")
      ...
      TrackedBoneDataMap.Num(), SimulatableBones.Num(), ActiveBlends.Num(),
      BoneSimBlendRefCount.Num(), PhysicsGraph.GetBoneMap().Num(),
      PhysicsGraph.GetConstraintLinks().Num(),
      CurrentFrameDebugData.ActiveConstraintCount,
      CurrentFrameDebugData.InactiveConstraintCount,
      DirectBodyInstanceMap.Num(), DirectConstraintTemplateMap.Num(),
      DirectConstraintInstanceMap.Num());

  GEngine->AddOnScreenDebugMessage(1001, DebugDisplayDuration, FColor::White,
                                   OverviewText, true,
...
void UOHPhysicsManager::DisplayPhysicsGraphAnalysis() const {
  ...
  FString AnalysisText =
      FString::Printf(TEXT("=== PHYSICS GRAPH ANALYSIS ===\n")
                      ...
                      GraphNodes, GraphMemoryKB, GraphConstraints,
                      CurrentFrameDebugData.GraphTraversalCount,
  ...
  GEngine->AddOnScreenDebugMessage(
      1002, DebugDisplayDuration,
      GraphIssues.Num() > 0 ? FColor::Yellow : FColor::Green, AnalysisText,
...
void UOHPhysicsManager::DisplayDirectPointerComparison() const {
  ...
  float PerformanceRatio =
      CurrentFrameDebugData.DirectPointerAccessTime > 0.0f
          ? CurrentFrameDebugData.PhysicsGraphAccessTime /
                CurrentFrameDebugData.DirectPointerAccessTime
          : 0.0f;

  bool bDirectIsFaster = CurrentFrameDebugData.DirectPointerAccessTime <
                         CurrentFrameDebugData.PhysicsGraphAccessTime;
  ...
  int32 GraphConstraintOps = CurrentFrameDebugData.ActiveConstraintCount +
                             CurrentFrameDebugData.InactiveConstraintCount;
  int32 DirectConstraintOps = DirectConstraintInstanceMap.Num();
  float ConstraintEfficiencyRatio =
      DirectConstraintOps > 0 ? static_cast<float>(GraphConstraintOps) /
                                    static_cast<float>(DirectConstraintOps)
                              : 0.0f;

  FString ComparisonText = FString::Printf(
      TEXT("=== DIRECT POINTER vs GRAPH COMPARISON ===\n") TEXT(
          "Direct Memory: %d KB | Graph Memory: %d KB\n")
          TEXT("Direct Lookups: %d | Graph Traversals: %d\n") TEXT(
              "Direct Access Time: %.4f ms | Graph Access Time: %.4f ms\n")
              TEXT("Performance Ratio (Graph/Direct): %.2fx\n") TEXT(
                  "Constraint Efficiency (Graph/Direct Ops): %.2fx\n")
      ...
      CurrentFrameDebugData.PhysicsGraphAccessTime * 1000.0f, PerformanceRatio,
      ConstraintEfficiencyRatio, GraphConstraintOps, DirectConstraintOps,
  ...
  if (bDirectIsFaster && bDirectUsesLessMemory && DirectIssues.Num() == 0 &&
      ConstraintEfficiencyRatio > 1.5f) {
  ...
  } else if (bDirectIsFaster && bDirectUsesLessMemory &&
             DirectIssues.Num() == 0) {
    ...
    ComparisonText += FString::Printf(
        ...
        FMath::Max(PerformanceRatio, static_cast<float>(GraphMemoryKB) /
                                         static_cast<float>(DirectMemoryKB)));
  } else if (ConstraintEfficiencyRatio > 2.0f) {
    ComparisonText +=
        TEXT("RECOMMENDATION: Graph constraint system is inefficient\n");
  ...
  FColor DisplayColor =
      (bDirectIsFaster && bDirectUsesLessMemory) ? FColor::Cyan : FColor::White;

  GEngine->AddOnScreenDebugMessage(1003, DebugDisplayDuration, DisplayColor,
                                   ComparisonText, true,
                                   FVector2D(DebugTextScale, DebugTextScale));
}
...
void UOHPhysicsManager::DisplayPerformanceMetrics() const {
  ...
  for (int32 i = DebugHistory.Num() - SampleCount; i < DebugHistory.Num();
       ++i) {
  ...
  FString MetricsText = FString::Printf(
      ...
      AvgGraphTime * 1000.0f, AvgDirectTime * 1000.0f, AvgGraphTraversals,
      AvgDirectLookups,
      AvgGraphTime > 0.0f ? AvgGraphTraversals / (AvgGraphTime * 1000.0f)
                          : 0.0f,
      AvgDirectTime > 0.0f ? AvgDirectLookups / (AvgDirectTime * 1000.0f)
                           : 0.0f);

  GEngine->AddOnScreenDebugMessage(1004, DebugDisplayDuration, FColor::Magenta,
                                   MetricsText, true,
                                   FVector2D(DebugTextScale, DebugTextScale));
}
...
void UOHPhysicsManager::EndGraphOperationTiming() const {
  CurrentFrameDebugData.PhysicsGraphAccessTime =
      FPlatformTime::Seconds() - GraphOperationStartTime;
}
...
void UOHPhysicsManager::EndDirectOperationTiming() const {
  CurrentFrameDebugData.DirectPointerAccessTime =
      FPlatformTime::Seconds() - DirectOperationStartTime;
}
...
TArray<FString> UOHPhysicsManager::GetDirectPointerIssues() const {
  ...
  if (StaleTemplateReferences > 0)
    Issues.Add(FString::Printf(TEXT("StaleTemplateRefs(%d)"),
                               StaleTemplateReferences));
  ...
  if (NullConstraintInstances > 0)
    Issues.Add(FString::Printf(TEXT("NullConstraintInstances(%d)"),
                               NullConstraintInstances));

  // Check for mismatched template vs instance counts
  int32 TemplateMismatch = FMath::Abs(DirectConstraintTemplateMap.Num() -
                                      DirectConstraintInstanceMap.Num());
  if (TemplateMismatch > 0)
    Issues.Add(FString::Printf(TEXT("TemplateInstanceMismatch(%d)"),
                               TemplateMismatch));
...
TArray<FString> UOHPhysicsManager::GetGraphInconsistencies() const {
  ...
  if (MissingConstraints > 0)
    Issues.Add(
        FString::Printf(TEXT("MissingConstraints(%d)"), MissingConstraints));

  // Check for bones without valid body instances (renamed to avoid class member
  // collision)
  int32 InvalidBodyCount = 0;
  for (const auto &Pair : PhysicsGraph.GetBoneMap()) {
    const FOHBoneData &BoneData = Pair.Value;
    if (!BoneData.GetBodyInstance() ||
        !BoneData.GetBodyInstance()->IsValidBodyInstance()) {
      InvalidBodyCount++;
    }
  }
...
void UOHPhysicsManager::DisplayBoneDetails() const {
  ...
  if (DebugSpecificBones.Num() > 0) {
    for (const FName &BoneName : DebugSpecificBones) {
      // Graph bone analysis
      if (const FOHBoneData *GraphBone =
              PhysicsGraph.GetBoneMap().Find(BoneName)) {
        ...
        BoneText += FString::Printf(
            ...
            *BoneName.ToString(), Mass,
            bIsSimulating ? TEXT("Yes") : TEXT("No"), MotionSamples,
      ...
      if (DirectBody && *DirectBody) {
        ...
        BoneText +=
            FString::Printf(TEXT("  Direct: Mass=%.2f | Sim=%s\n"), DirectMass,
    ...
    BoneText += FString::Printf(
        TEXT("Graph Bones: %d | Direct Bones: %d\n")
            TEXT("Simulating: %d | With Motion History: %d\n") TEXT(
                "Use DebugSpecificBones array for detailed bone analysis\n"),
        GraphBones, DirectBones, SimulatingBones, BonesWithMotionHistory);
  }

  GEngine->AddOnScreenDebugMessage(1005, DebugDisplayDuration, FColor::Cyan,
                                   BoneText, true,
                                   FVector2D(DebugTextScale, DebugTextScale));
}
...
void UOHPhysicsManager::DisplayConstraintDetails() const {
  ...
  for (const FOHConstraintInstanceData &ConstraintData :
    ...
    if (FConstraintInstance *Instance =
      ...
      const FConstraintRuntimeState &RuntimeState =
          ConstraintData.GetRuntimeState();
      ...
      if (FMath::Abs(GraphStrain - DirectStrain) > 50.0f &&
          bEnableDebugDisplay) {
        UE_LOG(LogTemp, Warning,
               TEXT("Constraint strain mismatch: Graph=%.1f Direct=%.1f"),
  ...
  float GraphAvgStrain =
      GraphValidInstances > 0 ? GraphTotalStrain / GraphValidInstances : 0.0f;
  float DirectAvgStrain = DirectValidInstances > 0
                              ? DirectTotalStrain / DirectValidInstances
                              : 0.0f;
  ...
  ConstraintText += FString::Printf(
      ...
      GraphValidInstances > 0 ? (GraphValidInstances * 100.0f /
                                 (GraphValidInstances + GraphNullInstances))
                              : 0.0f,
      DirectValidInstances > 0 ? (DirectValidInstances * 100.0f /
                                  (DirectValidInstances + DirectNullInstances))
                               : 0.0f,
  ...
  if (GraphNullInstances > DirectNullInstances) {
    ConstraintText +=
        TEXT("WARNING: Graph has more null constraint instances\n");
  }

  if (FMath::Abs(GraphAvgStrain - DirectAvgStrain) > 10.0f) {
    ConstraintText +=
        TEXT("WARNING: Significant strain calculation differences\n");
  }

  FColor ConstraintColor =
      (GraphNullInstances > 0) ? FColor::Yellow : FColor::Green;

  GEngine->AddOnScreenDebugMessage(1006, DebugDisplayDuration, ConstraintColor,
                                   ConstraintText, true,
                                   FVector2D(DebugTextScale, DebugTextScale));
}
...
bool UOHPhysicsManager::ValidateConstraintInstanceConsistency() const {
  ...
  for (const FOHConstraintInstanceData &ConstraintData :
    ...
    if (FConstraintInstance *const *Found1 =
            DirectConstraintInstanceMap.Find(ParentBone))
      DirectInstance1 = *Found1;
    if (FConstraintInstance *const *Found2 =
            DirectConstraintInstanceMap.Find(ChildBone))
    ...
    if (GraphInstance && (!DirectInstance1 && !DirectInstance2)) {
      bConsistent = false;
      UE_LOG(LogTemp, Warning,
             TEXT("Graph has constraint instance for %s-%s but direct system "
                  "doesn't"),
             *ParentBone.ToString(), *ChildBone.ToString());
    } else if (!GraphInstance && (DirectInstance1 || DirectInstance2)) {
      bConsistent = false;
      UE_LOG(LogTemp, Warning,
             TEXT("Direct system has constraint instance for %s-%s but graph "
                  "doesn't"),
             *ParentBone.ToString(), *ChildBone.ToString());
    }
  }
...
int32 UOHPhysicsManager::CalculateGraphMemoryFootprint() const {
  ...
  for (const auto &Pair : PhysicsGraph.GetBoneMap()) {
    ...
    TotalMemory += sizeof(TArray<FName>) +
                   (sizeof(FName) * 4); // Estimate avg 4 children per bone
  }
  ...
  TotalMemory += ConstraintCount * (sizeof(FOHConstraintInstanceData) +
                                    sizeof(FConstraintRuntimeState));
  ...
  TotalMemory +=
      PhysicsGraph.GetBoneMap().Num() * 16; // Hash overhead per entry
  ...
  TotalMemory += PhysicsGraph.GetBoneMap().Num() *
                 (sizeof(float) * 4); // Mass, length, and other cached values
  ...
  if (bEnableDebugDisplay) {
    ...
    TotalMemory +=
        PhysicsGraph.GetBoneMap().Num() * 32; // Debug overhead per bone
  }
...
void UOHPhysicsManager::ToggleDebugMode(bool bNewEnabled) {
  ...
  if (bNewEnabled) {
    ...
    UE_LOG(
        LogTemp, Log,
        TEXT("  - Monitoring both constraint templates AND runtime instances"));
    UE_LOG(LogTemp, Log,
           TEXT("  - Tracking graph vs direct pointer performance"));
    UE_LOG(LogTemp, Log,
           TEXT("  - Validating constraint instance consistency"));
  } else {
...
void UOHPhysicsManager::DrawSimBlendDebugOverlay() const {
  ...
  for (const TPair<FName, FActivePhysicsBlend> &Elem : ActiveBlends) {
    ...
    for (const FName &Bone : BoneNames) {
      if (!IsBoneValidForChain(Bone,
                               RootBone)) // ✅ NEW: skip non-simulatable bones
        continue;

      const FTransform BoneTransform = SkeletalMesh->GetSocketTransform(
          Bone, ERelativeTransformSpace::RTS_World);
      ...
      DrawDebugSphere(GetWorld(), Location, 4.0f, 6, FinalColor, false, -1.f, 0,
                      0.75f);

      const FString Label = FString::Printf(TEXT("%s (%.2f)"), *Bone.ToString(),
                                            Blend.BlendAlpha);
      DrawDebugString(GetWorld(), Location + FVector(0, 0, 6.f), Label, nullptr,
                      FinalColor, 0.0f, true);
    }
  }
}
...
void UOHPhysicsManager::DrawTrackedBoneKinematicsDebug() const {
  ...
  for (const auto &Pair : TrackedBoneDataMap) {
    ...
    // Label
    DrawDebugString(GetWorld(), Pos + FVector(0, 0, 15), *Bone.ToString(),
                    nullptr, FColor::White, 0.f, true);

    // Velocity (Blue)
    DrawDebugLine(GetWorld(), Pos, Pos + Vel * DebugVelocityScale, FColor::Blue,
                  false, 0.f, 0, 1.5f);

    // Acceleration (Green, highlight if large)
    const float AccMag = Acc.Size();
    const FColor AccColor = (AccMag > AccelerationAlertThreshold)
                                ? FColor(150, 255, 150)
                                : FColor::Green;

    DrawDebugLine(GetWorld(), Pos, Pos + Acc * DebugAccelerationScale, AccColor,
                  false, 0.f, 0, 1.5f);

    // Angular Velocity (Red, highlight if large)
    const float AngMag = AngVel.Size();
    const FColor AngColor = (AngMag > AngularVelocityAlertThreshold)
                                ? FColor(255, 100, 100)
                                : FColor::Red;

    DrawDebugLine(GetWorld(), Pos, Pos + AngVel * DebugAngularVelocityScale,
                  AngColor, false, 0.f, 0, 1.5f);
  }
}

void UOHPhysicsManager::PrintBoneTrackingInfo() const {
  UE_LOG(LogTemp, Log, TEXT("---- Tracked Bone Definitions (%d) ----"),
         TrackedBoneDefinitions.Num());
  for (const FName &Bone : TrackedBoneDefinitions) {
    UE_LOG(LogTemp, Log, TEXT("Tracked: %s"), *Bone.ToString());
  }

  UE_LOG(LogTemp, Log, TEXT("---- Sim Exclusions (%d) ----"),
         SimExclusionBoneSet.Num());
  for (const FName &Bone : SimExclusionBoneSet) {
    UE_LOG(LogTemp, Log, TEXT("Excluded: %s"), *Bone.ToString());
  }

  UE_LOG(LogTemp, Log, TEXT("---- Simulatable Bones (%d) ----"),
         SimulatableBones.Num());
  for (const FName &Bone : SimulatableBones) {
...
const TMap<FName, FOHBoneData> &UOHPhysicsManager::GetTrackedBoneData() const {
  ...
  if (!this) {
    UE_LOG(
        LogTemp, Error,
        TEXT("UOHPhysicsManager::GetTrackedBoneData called on null instance."));
    return EmptyMap;
  }

  if (!GetOwner()) {
    UE_LOG(LogTemp, Warning,
           TEXT("UOHPhysicsManager::GetTrackedBoneData: Manager has no owning "
                "actor."));
    return EmptyMap;
  }
  ...
  if (TrackedBoneDataMap.Num() == 0) {
    UE_LOG(LogTemp, Warning,
           TEXT("UOHPhysicsManager::GetTrackedBoneData: TrackedBoneDataMap is "
                "empty. Tracking might not be initialized."));
  }
#endif
  ...
  for (auto &Pair : TrackedBoneDataMap) {
    ...
    if (Suggestions.Num() > 0) {
      ...
      const FTransform BoneTransform =
          SkeletalMesh->GetSocketTransform(Bone, RTS_World);
      ...
      for (const FString &Suggestion : Suggestions) {
        ...
        DrawDebugString(GetWorld(), SpherePos + LabelOffset, Suggestion,
                        nullptr, FinalColor, EvaluationInterval, false, 1.5f);
      ...
      UE_LOG(
          LogTemp, Warning,
          TEXT("[Tuning] %s | Instability: %.2f | Suggestions: %s | Notes: %s"),
          *Bone.ToString(), InstabilityScore,
          *FString::Join(Suggestions, TEXT(", ")), *NoteSummary);
    }
  }

  if (!bAnyBoneSimulating) {
    UE_LOG(LogTemp, Verbose,
           TEXT("[Tuning] No bones simulating; skipping suggestions."));
  } else if (SuggestionCount == 0) {
    UE_LOG(LogTemp, Verbose,
           TEXT("[Tuning] Bones simulating but no instability detected."));
  }
}
...
bool UOHPhysicsManager::IsBoneSimulating(FName BoneName) const {
  const FOHBoneData *BoneData =
      OHSafeMapUtils::GetReference(PhysicsGraph.GetBoneMap(), BoneName);
  return BoneData && BoneData->GetIsSimulating();
}

FOHBoneData *UOHPhysicsManager::GetBoneDataChecked(FName BoneName) {
  return OHSafeMapUtils::GetMutableReference(PhysicsGraph.GetBoneMap(),
                                             BoneName);
}

void UOHPhysicsManager::ComputePhysicsTweaksForBone(
    const FName &Bone, float &OutPACMultiplier, float &OutLinearDamping,
  ...
  if (ParentIndex != INDEX_NONE) {
    const FVector BoneLoc =
        SkeletalMesh->GetBoneLocation(Bone, EBoneSpaces::ComponentSpace);
    const FName ParentBoneName = RefSkeleton.GetBoneName(ParentIndex);
    const FVector ParentLoc = SkeletalMesh->GetBoneLocation(
        ParentBoneName, EBoneSpaces::ComponentSpace);
  ...
  const float Responsiveness =
      (Length * 0.3f + Mass * 1.0f) * (1.0f + Depth * 0.05f);
...
void UOHPhysicsManager::UpdateConstraintRuntimeStates(float DeltaTime) {
  ...
  for (FOHConstraintInstanceData &Constraint :
    ...
    if (!RS.GetConstraintInstance() && Constraint.GetOwnerComponent()) {
      RS.SetConstraintInstance(
          SkeletalMesh->FindConstraintInstance(Constraint.GetConstraintName()));
    }
    ...
    FBodyInstance *BI =
        SkeletalMesh->GetBodyInstance(Constraint.GetChildBone());
    ...
    RS.SetJitterMetric(
        FMath::Lerp(RS.GetJitterMetric(), DeltaStrain, JitterSmoothingAlpha));
    ...
    Smoothed.SetPositionStrength(FMath::Lerp(Smoothed.GetPositionStrength(),
                                             LinSpring, DriveSmoothingAlpha));
    Smoothed.SetVelocityStrength(FMath::Lerp(Smoothed.GetVelocityStrength(),
                                             LinDamp, DriveSmoothingAlpha));
    Smoothed.SetOrientationStrength(FMath::Lerp(
        Smoothed.GetOrientationStrength(), AngSpring, DriveSmoothingAlpha));
    Smoothed.SetAngularVelocityStrength(FMath::Lerp(
        Smoothed.GetAngularVelocityStrength(), AngDamp, DriveSmoothingAlpha));
    ...
    CI->SetLinearDriveParams(Smoothed.GetPositionStrength(),
                             Smoothed.GetVelocityStrength(), 0.f);

    // -- Angular drives in Twist & Swing mode
    CI->SetOrientationDriveTwistAndSwing(/*bEnableTwist=*/true,
                                         /*bEnableSwing=*/true);
    CI->SetAngularVelocityDriveTwistAndSwing(/*bEnableTwist=*/true,
                                             /*bEnableSwing=*/true);
    CI->SetAngularDriveParams(Smoothed.GetOrientationStrength(),
                              Smoothed.GetAngularVelocityStrength(), 0.f);
  }
}

void UOHPhysicsManager::ComputeOptimalConstraintSettings(
  ...
  // === Optional: debug log ===
  UE_LOG(LogTemp, Log,
         TEXT("[OHPhysicsManager] [%s] Constraint Profile | Swing=%.1f "
              "Twist=%.1f | MassRatio=%.2f | Length=%.1f"),
         *BoneName.ToString(), SwingLimit, TwistLimit, MassRatio, Length);
}

FPhysicalAnimationData UOHPhysicsManager::GetScaledPACProfile(
    const FName &Bone, const FPhysicalAnimationData &BaseProfile) const {
  FPhysicalAnimationData ScaledProfile = BaseProfile;
...
UOHPhysicsManager::GetPhysicalAnimationData(FName BoneName,
                                            EDriveAccessMode AccessMode) const {
  FPhysicalAnimationData Result;
  Result.BodyName = BoneName;

  // 1) Find the constraint where this is the child bone
  const FOHConstraintInstanceData *Data =
      PhysicsGraph.FindConstraintByChildBone(BoneName);
  if (!Data) {
    return Result;
  }
  ...
  if (AccessMode == EDriveAccessMode::Live) {
    const FConstraintRuntimeReadback &RB =
        Data->GetRuntimeState().GetLastReadback();
    ...
    if (const UPhysicsConstraintTemplate *Tem = Data->GetConstraintTemplate()) {
      ...
      Result.OrientationStrength =
          DI.ProfileInstance.AngularDrive.SlerpDrive.Stiffness;
      Result.AngularVelocityStrength =
          DI.ProfileInstance.AngularDrive.SlerpDrive.Damping;
  ...
  if (const UPhysicsConstraintTemplate *Tem = Data->GetConstraintTemplate()) {
    const FConstraintInstance &DI = Tem->DefaultInstance;
    Result.MaxAngularForce =
        DI.ProfileInstance.AngularDrive.SlerpDrive.MaxForce;
...
void UOHPhysicsManager::SetPhysicalAnimationData(
    FName BoneName, const FPhysicalAnimationData &Data) {
  // 1) Find the constraint in our runtime graph (child‐bone lookup)
  FOHConstraintInstanceData *ConstraintData =
      PhysicsGraph.FindConstraintByChildBone(BoneName);
  if (!ConstraintData) {
    UE_LOG(LogTemp, Warning,
           TEXT("SetPhysicalAnimationData: no constraint found for bone %s"),
           *BoneName.ToString());
    return;
  }
  ...
  FConstraintInstance *CI =
      ConstraintData->GetRuntimeState().GetConstraintInstance();
  if (!CI) {
    UE_LOG(LogTemp, Warning,
           TEXT("SetPhysicalAnimationData: constraint instance is null for "
                "bone %s"),
           *BoneName.ToString());
    return;
  }
  ...
  CI->SetAngularVelocityDriveTwistAndSwing(/*bEnableTwist=*/true,
                                           /*bEnableSwing=*/true);
  CI->SetAngularDriveParams(Data.OrientationStrength,
                            Data.AngularVelocityStrength,
                            /*Limit=*/0.f);
...
bool UOHPhysicsManager::ArePACProfilesEqual(const FPhysicalAnimationData &A,
                                            const FPhysicalAnimationData &B) {
  constexpr float Tolerance = KINDA_SMALL_NUMBER;

  return FMath::IsNearlyEqual(A.OrientationStrength, B.OrientationStrength,
                              Tolerance) &&
         FMath::IsNearlyEqual(A.PositionStrength, B.PositionStrength,
                              Tolerance) &&
         FMath::IsNearlyEqual(A.VelocityStrength, B.VelocityStrength,
                              Tolerance) &&
         FMath::IsNearlyEqual(A.AngularVelocityStrength,
                              B.AngularVelocityStrength, Tolerance) &&
...
FActivePhysicsBlend
UOHPhysicsManager::CreatePhysicsBlendState(FName RootBone, float StartAlpha,
                                           float BlendIn, float Hold,
...
void UOHPhysicsManager::UpdateBlendState(FActivePhysicsBlend &Blend,
                                         float DeltaTime) {
  if (Blend.IsPaused())
    return;
  ...
  case EOHBlendPhase::BlendIn:
    ...
    {
      float Progress =
          Blend.Elapsed / FMath::Max(Blend.BlendInDuration, KINDA_SMALL_NUMBER);
  ...
  case EOHBlendPhase::BlendOut:
    ...
    {
      float Progress = Blend.Elapsed /
                       FMath::Max(Blend.BlendOutDuration, KINDA_SMALL_NUMBER);
...
void UOHPhysicsManager::ResumeBlend(FName Bone) {
  if (FActivePhysicsBlend *Blend = ActiveBlends.Find(Bone)) {
    if (Blend->PauseCount > 0) {
      Blend->PauseCount--;
      if (Blend->PauseCount == 0) {
        UE_LOG(LogTemp, Log, TEXT("[OH] Resumed blend on %s"),
               *Bone.ToString());
      }
    }
  }
...
void UOHPhysicsManager::ForceUnpause(FName Bone) {
  if (FActivePhysicsBlend *Blend = ActiveBlends.Find(Bone)) {
    if (Blend->PauseCount > 0) {
      Blend->PauseCount = 0;
      UE_LOG(LogTemp, Log, TEXT("[OH] Force-unpaused blend on %s"),
             *Bone.ToString());
    }
  }
}
...
EOHBlendPhase UOHPhysicsManager::GetCurrentBlendPhase(FName BoneName) const {
  ...
  return Blend ? Blend->Phase
               : EOHBlendPhase::BlendOut; // default = not blending / fallback
                                          // state
}
...
float UOHPhysicsManager::GetPhaseDuration(FName BoneName,
                                          EOHBlendPhase Phase) const {
  const FActivePhysicsBlend *Blend = ActiveBlends.Find(BoneName);
  if (!Blend)
    return 0.f;
...
bool UOHPhysicsManager::IsBlendComplete(const FActivePhysicsBlend &Blend) {
  return Blend.Phase == EOHBlendPhase::BlendOut &&
         Blend.Elapsed >= Blend.BlendOutDuration;
}
...
void UOHPhysicsManager::ApplyDamageToBone(FName Bone, float Damage) {
  if (FOHBodyPartStatus *Status = BoneStatusMap.Find(Bone)) {
    ...
    if (Status->IsDestroyed()) {
      FinalizeBlend(Bone);
      UE_LOG(LogTemp, Warning, TEXT("[OH] Bone %s destroyed"),
             *Bone.ToString());
    }
  }
}
...
TArray<FName>
UOHPhysicsManager::GetSkeletalBonesInBodyPart(EOHBodyPart Part) const {
...
bool UOHPhysicsManager::ValidateBoneMappings(bool bLogResults) {
  if (!SkeletalMesh || !SkeletalMesh->GetSkeletalMeshAsset()) {
    UE_LOG(LogTemp, Error,
           TEXT("[OHPhysicsManager] ValidateBoneMappings: No skeletal mesh"));
    return false;
  }
  ...
  for (const FName &ExpectedBone : TrackedBoneDefinitions) {
    ...
    Data.MatchResult = UOHAlgoUtils::FindBestNameMatchAutoStrategy(
        ExpectedBone, ActualBoneNames);
    ...
    Data.LogicalBone =
        UOHSkeletalPhysicsUtils::ResolveSkeletalBoneFromNameSmart(
            Data.ResolvedBone, SkeletalMesh, 0.75f);
    Data.BodyPart =
        UOHSkeletalPhysicsUtils::GetBodyPartFromBone(Data.LogicalBone);
    Data.BodyZone =
        UOHSkeletalPhysicsUtils::GetBodyZoneFromBone(Data.LogicalBone);
    Data.FunctionalGroup =
        UOHSkeletalPhysicsUtils::GetFunctionalGroupFromBone(Data.LogicalBone);

    ResolvedBoneData.Add(ExpectedBone, Data);

    if (bLogResults && !Data.bResolved && Data.MatchResult.Candidate != "") {
      UE_LOG(LogTemp, Warning,
             TEXT("[OHPhysicsManager] Bone '%s' not found. Did you mean '%s'? "
                  "(Score: %.2f)"),
             *ExpectedBone.ToString(), *Data.MatchResult.Candidate,
             Data.MatchResult.Score);
    }
  }
  ...
  if (bLogResults) {
    UE_LOG(LogTemp, Log,
           TEXT("[OHPhysicsManager] Validation complete: %d/%d bones valid, %d "
                "simulatable"),
...
void UOHPhysicsManager::ValidateAndRepairGraph() {
  if (!SkeletalMesh || !CachedPhysicsAsset) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] ValidateAndRepairGraph aborted — missing "
                "components."));
    return;
  }
  ...
  if (InvalidBodies.Num() > 0 || InvalidConstraints.Num() > 0) {
    UE_LOG(LogTemp, Warning,
           TEXT("[OHPhysicsManager] Found %d invalid bodies and %d invalid "
                "constraints."),
    ...
    if (InvalidBodies.Num() > 0 || InvalidConstraints.Num() > 0) {
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsManager] Repair incomplete. Consider full physics "
                  "reset."));
...
void UOHPhysicsManager::PerformValidationPass() {
  for (auto &Pair : TrackedBoneDataMap) {
    ...
    FConstraintInstance *ConstraintInstance =
        BoneData.GetParentConstraintInstance();
...
void UOHPhysicsManager::PerformRepairPass() {
  for (const FName &BoneName : InvalidBodies) {
    if (TryRecreateBodyInstance(BoneName)) {
      ...
      UE_LOG(LogTemp, Error,
             TEXT("[OHPhysicsManager] Failed to recreate body instance for "
                  "bone %s"),
             *BoneName.ToString());
    }
  }
  ...
  for (const FName &BoneName : InvalidConstraints) {
    ...
    if (TryRecreateConstraint(ParentBone, BoneName)) {
      ...
      UE_LOG(
          LogTemp, Error,
          TEXT("[OHPhysicsManager] Failed to recreate constraint for bone %s"),
          *BoneName.ToString());
    }
  }
}
...
bool UOHPhysicsManager::TryRecreateBodyInstance(const FName &BoneName) {
  ...
  // Recreate physics state for the skeletal mesh — this refreshes all bodies
  // and constraints
  SkeletalMesh->RecreatePhysicsState();
...
bool UOHPhysicsManager::TryRecreateConstraint(const FName &ParentBone,
  ...
  FConstraintInstance *ConstraintInstance =
      ChildBoneData->GetParentConstraintInstance();
...
bool UOHPhysicsManager::IsConstraintInstanceValid(
    const FConstraintInstance *ConstraintInstance) {
