...
UOHPhysicsManager::UOHPhysicsManager(): PhysicsGraph()
{
	...
	SimExclusionBoneSet = {
		"root", "pelvis", "neck_01", "ball_l", "ball_r"
	};
}
...
void UOHPhysicsManager::BeginPlay()
{
	...
	GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &UOHPhysicsManager::InitializePhysicsManager, StartTickDelay, false);
}
...
void UOHPhysicsManager::InitializePhysicsManager()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] SkeletalMesh has no PhysicsAsset — simulation features will be limited."));
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] Skipping RecreatePhysicsState — invalid or editor-only world."));
	}

	// === STEP 7: Debug logging ===
	UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] Init complete: %d Tracked, %d Simulatable"),
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] Constraint validation returned failure! Check individual logs for details."));
	}
...
void UOHPhysicsManager::InitializeSimulatableBones()
{
	...
	if (!UsePhysicsAsset && SkeletalMesh)
	{
		...
		if (UsePhysicsAsset)
		{
			UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] CachedPhysicsAsset was null — fallback to SkeletalMesh asset."));
		}
	}
...
void UOHPhysicsManager::InitializeAllPhysicalAnimationProfiles(const FPhysicalAnimationData& Profile)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] InitializeAllPhysicalAnimationProfiles aborted — missing components."));
		return;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] Physical animation profiles applied to %d bones."), SimulatableBones.Num());
}
...
void UOHPhysicsManager::ResetPhysicsManager()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] Skipping RecreatePhysicsState: Not in game world or world is invalid."));
	}
...
void UOHPhysicsManager::ClearPhysicsState()
{
	if (!SkeletalMesh || !IsValid(SkeletalMesh) || !SkeletalMesh->IsRegistered())
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] ClearPhysicsState aborted: SkeletalMesh is invalid or unregistered."));
		return;
	}
...
void UOHPhysicsManager::ForceReinitializePhysics(bool bLogResults)
{
	...
	if (bLogResults)
	{
		UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] ForceReinitializePhysics completed in %.2f ms. Bones: %d tracked, %d simulatable."),
			ElapsedMS,
			TrackedBoneDataMap.Num(),
	...
	case EOHMotionTimeDomain::AnimationTime:
		{
			...
			{
				return SkeletalMesh->GetAnimInstance()->GetWorld()->GetTimeSeconds(); // Can be replaced with actual animation time
			}
...
void UOHPhysicsManager::PlayPhysicsHitReaction(
	...
	float Hold,
	float BlendOut,
	FVector OptionalImpulseDir,
	float OptionalImpulseStrength,
	FName ReactionTag)
{
	...
	PlayPhysicsHitReaction_Internal(
		BoneName,
		*ProfileData,
		BlendIn,
		Hold,
...
void UOHPhysicsManager::PlayPhysicsHitReaction_Internal(
	FName& BoneName,
	const FPhysicalAnimationData& ProfileData,
	float BlendIn,
	float Hold,
	float BlendOut,
	FVector OptionalImpulseDir,
	...
	TArray<FName>* OutAffectedBones /* optional */)
{
	...
	if (!SkeletalMesh->DoesSocketExist(BoneName))
	{
		if (BoneSuggestions.Contains(BoneName))
		{
			...
			UE_LOG(LogTemp, Log, TEXT("[OH] Using suggested bone '%s' for hit reaction on '%s'"),
				*BoneName.ToString(), *OriginalBone.ToString());
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("[OH] HitReaction failed: Bone '%s' not found"),
				*BoneName.ToString());
			return;
		}
	}
...
void UOHPhysicsManager::ApplyImpulseToBone(FName Bone, const FVector& Direction, float Magnitude)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[OHPhysicsManager] Skipping impulse: Bone %s not simulating."), *Bone.ToString());
		return;
	}
...
void UOHPhysicsManager::PropagateImpulse(FName OriginBone, FVector Impulse, float Strength, EOHPhysicsProfile Profile, int32 Depth)
{
	if (Depth <= 0 || Strength <= KINDA_SMALL_NUMBER) return;

	if (!BoneLinkMap.Contains(OriginBone)) return;
...
/*void UOHPhysicsManager::ApplySimSettingsToBone(const FName& Bone, const FPhysicalAnimationData& BaseProfile)
{
	...
	if (UPhysicsAsset* PhysicsAsset = SkeletalMesh->GetPhysicsAsset())
	{
		...
		if (ConstraintIndex != INDEX_NONE)
		{
				...
				PhysicsAsset->ConstraintSetup[ConstraintIndex] != nullptr)
			{
				...
				UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f | Damping L %.2f A %.2f | Constraint tuned to %s"),
					*GetOwner()->GetName(), PACMultiplier, LinearDamping, AngularDamping, *Bone.ToString());
				return;
			}
		}
	}
	...
	UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f | Damping L %.2f A %.2f to %s (No constraint)"),
		*GetOwner()->GetName(), PACMultiplier, LinearDamping, AngularDamping, *Bone.ToString());
}*/
	...
	if (UPhysicsAsset* PhysicsAsset = SkeletalMesh->GetPhysicsAsset())
	{
			...
			PhysicsAsset->ConstraintSetup[ConstraintIndex] != nullptr)
		{
			...
			if (FConstraintInstance* RuntimeCI = SkeletalMesh->FindConstraintInstance(TemplateConstraint.JointName))
			{
				...
				RuntimeCI->SetAngularDriveParams(
					Profile.ConeLimit.Stiffness,
			...
			UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] [%s] Applied PAC x%.2f | Damping L %.2f A %.2f | Constraint tuned"),
...
void UOHPhysicsManager::ActivateSimulationForChain(FName RootBone, const FPhysicalAnimationData& ProfileData, float StartAlpha)
{
...
{
	if (!SkeletalMesh || !IsBoneValidForChain(BoneName, BoneName)) return false;
...
void UOHPhysicsManager::ApplyUnifiedPhysicsProfile(FName BoneName, const FPhysicalAnimationData& Profile, float CustomLinearDamping, float CustomAngularDamping, bool bVerboseLog)
{
	if (!SkeletalMesh || !PhysicalAnimationComponent)
	{
	...
	if (FBodyInstance* Body = SkeletalMesh->GetBodyInstance(BoneName))
	{
		...
		if (bVerboseLog)
		{
			UE_LOG(LogTemp, Log, TEXT("  - Damping set [Linear: %.2f | Angular: %.2f]"), CustomLinearDamping, CustomAngularDamping);
		}
	}
		...
		for (const UPhysicsConstraintTemplate* Template : PhysicsAsset->ConstraintSetup)
		{
			...
			if (ConstraintBone == BoneName)
			{
				if (FConstraintInstance* Instance = SkeletalMesh->FindConstraintInstance(Template->DefaultInstance.JointName))
				{
					...
					if (bVerboseLog)
					{
						UE_LOG(LogTemp, Log, TEXT("  - Constraint '%s' updated [Stiffness: %.1f | Damping: %.1f]"), *Instance->JointName.ToString(), Stiffness, Damping);
					}
				}
			}
		}
...
void UOHPhysicsManager::ApplyUnifiedPhysicsProfileToChain(FName RootBone, const FPhysicalAnimationData& Profile, float CustomLinearDamping, float CustomAngularDamping, bool bVerboseLog)
{
	if (!SkeletalMesh)
	{
...
bool UOHPhysicsManager::IsBoneNamePatternValid(FName BoneName)
{
	...
	if (BoneNameStr.Contains(TEXT("twist")) ||
		BoneNameStr.Contains(TEXT("ik")) ||
...
{
	UPhysicsAsset* Asset = CachedPhysicsAsset ? CachedPhysicsAsset : (SkeletalMesh ? SkeletalMesh->GetPhysicsAsset() : nullptr);
...
void UOHPhysicsManager::LogSimulatableBones() const
{
	...
	UE_LOG(LogTemp, Log, TEXT("[OHPhysicsManager] [%s] SimulatableBones (%d): [%s]"),
...
void UOHPhysicsManager::EditorValidateSkeletonCompatibility()
{
	...
	Report += FString::Printf(TEXT("Valid Bones: %d / %d\n\n"), 
...
{
	if (!SkeletalMesh || !GetWorld()) return;
...
{
	if (!bShowTrackedBoneKinematics || !SkeletalMesh) return;
...
const TMap<FName, FOHBoneData>& UOHPhysicsManager::GetTrackedBoneData() const
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UOHPhysicsManager::GetTrackedBoneData: TrackedBoneDataMap is empty. Tracking might not be initialized."));
	}
#endif
...
{
	if (Suggestion == "IncreaseAngularDamping") return FColor::Cyan;
	if (Suggestion == "IncreaseLinearDamping") return FColor::Green;
	if (Suggestion == "IncreasePositionStrength") return FColor::Yellow;
	if (Suggestion == "IncreaseOrientationStrength") return FColor::Orange;
	if (Suggestion == "TightenConstraintLimits") return FColor::Red;
	if (Suggestion == "StabilizeConstraint") return FColor::Purple;
	if (Suggestion == "FreezeBoneTemporarily") return FColor::Magenta;
...
) const
{
	...
	if (!SkeletalMesh || !SkeletalMesh->GetSkeletalMeshAsset()) return;
...
void UOHPhysicsManager::UpdateConstraintRuntimeStates(float DeltaTime)
{
    ...
    for (FOHConstraintInstanceData& Constraint : PhysicsGraph.GetConstraintLinks())
    {
        ...
        Smoothed.SetPositionStrength(
            FMath::Lerp(Smoothed.GetPositionStrength(), LinSpring, DriveSmoothingAlpha)
        );
        Smoothed.SetVelocityStrength(
            FMath::Lerp(Smoothed.GetVelocityStrength(), LinDamp, DriveSmoothingAlpha)
        );
        Smoothed.SetOrientationStrength(
            FMath::Lerp(Smoothed.GetOrientationStrength(), AngSpring, DriveSmoothingAlpha)
        );
        ...
        CI->SetLinearDriveParams(
            Smoothed.GetPositionStrength(),
            Smoothed.GetVelocityStrength(),
            0.f
        );
        ...
        CI->SetAngularDriveParams(
            Smoothed.GetOrientationStrength(),
            Smoothed.GetAngularVelocityStrength(),
            0.f
        );
    }
}
...
{
	if (!SkeletalMesh || !ConstraintInstance) return;
...
FPhysicalAnimationData UOHPhysicsManager::GetScaledPACProfile(const FName& Bone, const FPhysicalAnimationData& BaseProfile) const
{
	FPhysicalAnimationData ScaledProfile = BaseProfile;
...
FPhysicalAnimationData UOHPhysicsManager::GetPhysicalAnimationData(
    FName BoneName,
    EDriveAccessMode AccessMode
) const
{
...
{ 
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("SetPhysicalAnimationData: constraint instance is null for bone %s"),
			*BoneName.ToString());
		return;
	}
...
void UOHPhysicsManager::ProcessBlendPhases(float DeltaTime)
{
	...
	for (auto& Elem : ActiveBlends)
	{
		...
		for (const FName& Bone : BoneNames)
		{
			if (!IsBoneValidForChain(Bone, Blend.RootBone)) continue;
...
bool UOHPhysicsManager::ValidateBoneMappings(bool bLogResults)
{
	...
	for (const FName& ExpectedBone : TrackedBoneDefinitions)
	{
		...
		Data.LogicalBone = UOHSkeletalPhysicsUtils::ResolveSkeletalBoneFromNameSmart(Data.ResolvedBone, SkeletalMesh, 0.75f);
	...
	for (const TPair<FName, FOHResolvedBoneData>& Pair : ResolvedBoneData)
	{
		...
		if (Pair.Value.bResolved &&
			!SimExclusionBoneSet.Contains(ActualBone) &&
...
void UOHPhysicsManager::PerformRepairPass()
{
    for (const FName& BoneName : InvalidBodies)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsManager] Failed to recreate body instance for bone %s"), *BoneName.ToString());
        }
    }
    ...
    for (const FName& BoneName : InvalidConstraints)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("[OHPhysicsManager] Failed to recreate constraint for bone %s"), *BoneName.ToString());
        }
    }
}
