...
void AOHPhysicsHandler::BeginPlay() { Super::BeginPlay(); }
...
void AOHPhysicsHandler::SetupPlayerInputComponent(
    UInputComponent *PlayerInputComponent) {
  Super::SetupPlayerInputComponent(PlayerInputComponent);

  // Movement
  PlayerInputComponent->BindAxis("MoveForward", this,
                                 &AOHPhysicsHandler::MoveForward);
  PlayerInputComponent->BindAxis("MoveRight", this,
                                 &AOHPhysicsHandler::MoveRight);
  ...
  PlayerInputComponent->BindAxis("Turn", this,
                                 &AOHPhysicsHandler::CameraLookYaw);
  PlayerInputComponent->BindAxis("LookUp", this,
                                 &AOHPhysicsHandler::CameraLookPitch);

  // Grabbing
  PlayerInputComponent->BindKey(EKeys::LeftMouseButton, IE_Pressed, this,
                                &AOHPhysicsHandler::Grab);
  PlayerInputComponent->BindKey(EKeys::LeftMouseButton, IE_Released, this,
                                &AOHPhysicsHandler::Release);

  // Camera look mode (RMB)
  PlayerInputComponent->BindKey(EKeys::RightMouseButton, IE_Pressed, this,
                                &AOHPhysicsHandler::StartCameraLook);
  PlayerInputComponent->BindKey(EKeys::RightMouseButton, IE_Released, this,
                                &AOHPhysicsHandler::StopCameraLook);

  PlayerInputComponent->BindAxis("MouseMoveX", this,
                                 &AOHPhysicsHandler::OnGrabMoveX);
  PlayerInputComponent->BindAxis("MouseMoveY", this,
                                 &AOHPhysicsHandler::OnGrabMoveY);
}
...
void AOHPhysicsHandler::StartCameraLook() {
  ...
  if (GEngine)
    GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Green,
                                     TEXT("Camera Look Mode: ON (RMB Down)"));
}
...
void AOHPhysicsHandler::StopCameraLook() {
  ...
  if (GEngine)
    GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red,
                                     TEXT("Camera Look Mode: OFF (RMB Up)"));
}

void AOHPhysicsHandler::CameraLookYaw(float AxisValue) {
  if (GEngine) {
    FString Msg =
        FString::Printf(TEXT("CameraLookYaw: %f | RMB active: %d"), AxisValue,
                        static_cast<int>(bIsCameraLookActive));
...
void AOHPhysicsHandler::CameraLookPitch(float AxisValue) {
  if (GEngine) {
    FString Msg =
        FString::Printf(TEXT("CameraLookPitch: %f | RMB active: %d"), AxisValue,
                        static_cast<int>(bIsCameraLookActive));
    GEngine->AddOnScreenDebugMessage(-1, 0.02f, FColor::Cyan, Msg);
  }
  ...
  if (bIsGrabbingWithMouse && FMath::Abs(AxisValue) > KINDA_SMALL_NUMBER) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(
          -1, 0.01f, FColor::Red,
          FString::Printf(TEXT("MouseMoveX: %f"), AxisValue));
...
void AOHPhysicsHandler::OnGrabMoveY(float AxisValue) {
  if (bIsGrabbingWithMouse && FMath::Abs(AxisValue) > KINDA_SMALL_NUMBER) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(
          -1, 0.01f, FColor::Green,
          FString::Printf(TEXT("MouseMoveY: %f"), AxisValue));
  ...
  if (bDidGrab) {
    ...
    if (APlayerController *PC = Cast<APlayerController>(GetController())) {
      ...
      if (PhysicsHandle && PhysicsHandle->GrabbedComponent) {
        GrabStartWorldPos =
            PhysicsHandle->GrabbedComponent->GetComponentLocation();
      }
...
void AOHPhysicsHandler::MoveGrabbedComponent() {
  if (PhysicsHandle && PhysicsHandle->GrabbedComponent &&
    ...
    PC->DeprojectScreenPositionToWorld(MousePos.X, MousePos.Y, WorldOrigin,
                                       WorldDir);
    ...
    CurrentGrabTargetLocation =
        FMath::VInterpTo(CurrentGrabTargetLocation, TargetLoc,
                         GetWorld()->GetDeltaSeconds(), 20.0f);
...
void AOHPhysicsHandler::UpdateGrabbedTargetFromScreen() {
  ...
  if (APlayerController *PC = Cast<APlayerController>(GetController())) {
    if (PC->DeprojectScreenPositionToWorld(NewScreenPos.X, NewScreenPos.Y,
...
void AOHPhysicsHandler::UpdateHighlight() {
  if (!GetWorld() || !GetController() ||
      (PhysicsHandle && PhysicsHandle->GrabbedComponent)) {
  ...
  // Use your robust helper to find the closest simulating component, even if
  // the hit is a non-sim root
  ...
  if (HitResult.GetActor() && FindClosestSimulatingComponent(
                                  HitResult.GetActor(), HitResult.ImpactPoint,
                                  GrabDistance, // Or some "highlight" distance
                                  ClosestSimComp, ClosestPoint, ClosestDist)) {
...
bool AOHPhysicsHandler::TryGrabUnderCursor(
  ...
  if (!PhysicsHandle) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red,
                                       TEXT("No PhysicsHandle component!"));
    return false;
  }
  ...
  if (!PC) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red,
                                       TEXT("No PlayerController found."));
    return false;
  }
  ...
  if (!HitResult.GetActor()) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red,
                                       TEXT("No actor under cursor!"));
    return false;
  }

  float ActualDistance =
      FVector::Dist(Camera->GetComponentLocation(), HitResult.ImpactPoint);
  if (ActualDistance > MaxGrabDistance) {
    FString Msg = FString::Printf(TEXT("Hit '%s' but too far: %.0f / %.0f"),
                                  *HitResult.GetActor()->GetName(),
                                  ActualDistance, MaxGrabDistance);
    if (GEngine)
  ...
  bool bFoundComponent = FindClosestSimulatingComponent(
      HitResult.GetActor(), HitResult.ImpactPoint, MaxGrabDistance,
      ClosestComponent, ClosestCompPoint, ClosestCompDist);

  if (!bFoundComponent || !ClosestComponent) {
    FString Msg = FString::Printf(
        TEXT("'%s' has no simulating physics component near click."),
        *HitResult.GetActor()->GetName());
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, Msg);
    return false;
  ...
  if (USkeletalMeshComponent *SkelMesh =
          Cast<USkeletalMeshComponent>(ClosestComponent)) {
    ...
    if (FindClosestSimulatingBodyInstance(
            SkelMesh, HitResult.ImpactPoint, MaxGrabDistance, ClosestBody,
            ClosestBone, ClosestCOM, ClosestBodyDist)) {
      // Grab bone at COM
      PhysicsHandle->GrabComponentAtLocationWithRotation(
          SkelMesh, ClosestBone, ClosestCOM, SkelMesh->GetComponentRotation());
      ...
      FString Msg =
          FString::Printf(TEXT("'%s' has no simulating bone near click."),
                          *SkelMesh->GetName());
      if (GEngine)
        GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, Msg);
      return false;
  ...
  PhysicsHandle->GrabComponentAtLocationWithRotation(
      ClosestComponent, NAME_None, ClosestCompPoint,
      ClosestComponent->GetComponentRotation());

  FString Msg = FString::Printf(TEXT("Grabbed component '%s' (%.1f units)"),
...
// 1. Closest Actor
AActor *AOHPhysicsHandler::FindClosestActor(const TArray<AActor *> &Actors,
...
bool AOHPhysicsHandler::FindClosestSimulatingComponent(
    AActor *Actor, const FVector &ClickLocation, float MaxDistance,
...
bool AOHPhysicsHandler::FindClosestSimulatingBodyInstance(
    USkeletalMeshComponent *SkelMesh, const FVector &ClickLocation,
  ...
  for (const FBodyInstance *Body : SkelMesh->Bodies) {
    ...
    FName Bone =
        Body->BodySetup.IsValid() ? Body->BodySetup->BoneName : NAME_None;
    FVector COM = Body->GetCOMPosition();
    float Dist = FVector::Dist(COM, ClickLocation);

    // Track closest simulating
    if (Body->IsInstanceSimulatingPhysics() && Dist < ClosestDist &&
        Dist <= MaxDistance) {
      ClosestSimBody = Body;
      ClosestDist = Dist;
      ClosestBone = Bone;
  ...
  if (ClosestSimBody) {
    ...
    if (GEngine) {
      FString Msg =
          FString::Printf(TEXT("Simulating body found: %s (dist=%.2f)"),
    ...
    if (GEngine) {
      FString Msg = FString::Printf(
          TEXT("No Simulating bodies but closest bone is: %s (dist=%.2f)"),
          *OutBoneName.ToString(), OutBodyDistance);
...
bool AOHPhysicsHandler::IsComponentOrAnyBodySimulatingPhysics(
