...
AOHCombatCharacter::AOHCombatCharacter()
{
	...
	// Mixed mode means we only replicated the GEs to ourselves, not the GEs to simulated proxies. If another GDPlayerState (Hero) receives a GE,
	// the Server won't tell us about it. Attributes, GameplayTags, and GameplayCues will still replicate to us.
...
void AOHCombatCharacter::EnsureMovementComponentExists()
{
	if (!OHMovementComponent)
	{
		...
		if (!OHMovementComponent)
		{
			OHMovementComponent = NewObject<UOHMovementComponent>(this, UOHMovementComponent::StaticClass(), TEXT("OHMovementComponent"));
...
void AOHCombatCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PreviousCustomMode)
{
	...
	if (OHMovementComponent)
	{
		OHMovementComponent->HandleMovementModeChanged(GetCharacterMovement()->MovementMode, GetCharacterMovement()->CustomMovementMode);
	}
