...
AOHCombatCharacter::AOHCombatCharacter() {
  ...
  // Mixed mode means we only replicated the GEs to ourselves, not the GEs to
  // simulated proxies. If another GDPlayerState (Hero) receives a GE, the
  // Server won't tell us about it. Attributes, GameplayTags, and GameplayCues
...
void AOHCombatCharacter::BeginPlay() { Super::BeginPlay(); }

// Called every frame
void AOHCombatCharacter::Tick(float DeltaTime) { Super::Tick(DeltaTime); }

// Called to bind functionality to input
void AOHCombatCharacter::SetupPlayerInputComponent(
...
auto AOHCombatCharacter::PossessedBy(AController *NewController) -> void {
  ...
  if (AOHPlayerController *PC = Cast<AOHPlayerController>(NewController)) {
    if (OHMovementComponent) {
      ...
      PC->OnMoveForwardInput.RemoveDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveForward);
      PC->OnMoveRightInput.RemoveDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveRight);
      PC->OnMoveInputVector.RemoveDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveVector);

      // Bind new
      PC->OnMoveForwardInput.AddDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveForward);
      PC->OnMoveRightInput.AddDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveRight);
      PC->OnMoveInputVector.AddDynamic(
          OHMovementComponent, &UOHMovementComponent::OnReceiveMoveVector);
    }
  }
...
void AOHCombatCharacter::EnsureMovementComponentExists() {
  if (!OHMovementComponent) {
    ...
    if (!OHMovementComponent) {
      OHMovementComponent = NewObject<UOHMovementComponent>(
          this, UOHMovementComponent::StaticClass(),
          TEXT("OHMovementComponent"));
...
void AOHCombatCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode,
                                               uint8 PreviousCustomMode) {
  Super::OnMovementModeChanged(PrevMovementMode, PreviousCustomMode);

  // Notify movement component
  if (OHMovementComponent) {
    OHMovementComponent->HandleMovementModeChanged(
