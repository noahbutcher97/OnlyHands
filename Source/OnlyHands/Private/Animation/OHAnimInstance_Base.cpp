...
static FVector CriticallyDampedSmoothing(const FVector &Current,
                                         const FVector &Target, float DeltaTime,
                                         float Spring = 80.f,
                                         float Damping = 1.2f) {
...
bool UOHAnimInstance_Base::CapsuleGroundTrace(
    const USkeletalMeshComponent *Mesh, const FName &BoneName,
    float MaxDistance, float ForwardBias, float CapsuleRadius,
    float CapsuleHalfHeight, const FVector &Velocity, FHitResult &OutHit,
    bool bDrawDebug) {
  if (!Mesh)
    return false;
  ...
  if (bDrawDebug) {
    ...
    DrawDebugCapsule(Mesh->GetWorld(), PredictedStart, CapsuleHalfHeight,
                     CapsuleRadius, FQuat::Identity, TraceColor, false, 0.08f,
                     0, 1.2f);
    ...
    if (bHit) {
      ...
      DrawDebugDirectionalArrow(Mesh->GetWorld(), OutHit.ImpactPoint,
                                ...
                                16.f, FColor::Yellow, false, 0.12f, 0, 1.5f);
    }
  }
  return bHit;
}

bool UOHAnimInstance_Base::SphereGroundTrace(
    const USkeletalMeshComponent *Mesh, const FName &BoneName,
    float MaxDistance, float ForwardBias, float SphereRadius,
    const FVector &Velocity, FHitResult &OutHit, bool bDrawDebug) {
  if (!Mesh)
    return false;
  FVector Start = Mesh->GetBoneLocation(BoneName) +
                  (Velocity.IsNearlyZero() ? FVector::ZeroVector
  ...
  if (bDrawDebug) {
    FColor TraceColor = bHit ? FColor::Green : FColor::Red;
    DrawDebugSphere(Mesh->GetWorld(), bHit ? OutHit.ImpactPoint : End,
                    SphereRadius, 8, TraceColor, false, 0.12f);
    DrawDebugLine(Mesh->GetWorld(), Start, End, TraceColor, false, 0.08f, 0,
                  1.2f);
  }
  return bHit;
}

bool UOHAnimInstance_Base::SafeDoubleTrace(
    const USkeletalMeshComponent *Mesh, FName Bone, float MaxDist, float Bias,
    const float Radius, const FVector &Velocity, FHitResult &OutMain,
    FHitResult &OutBackup, float BackupDepth) {
  bool bMain = SphereGroundTrace(Mesh, Bone, MaxDist, Bias, Radius, Velocity,
                                 OutMain, false);
  bool bBackup = SphereGroundTrace(Mesh, Bone, MaxDist + BackupDepth, Bias,
                                   Radius, Velocity, OutBackup, false);

  // Use backup if the main misses but backup is close and lower
  if (!bMain && bBackup &&
      FMath::Abs(OutMain.ImpactPoint.Z - OutBackup.ImpactPoint.Z) < 10.f) {
    OutMain = OutBackup;
    bMain = true;
  }
  return bMain;
...
FVector UOHAnimInstance_Base::CatmullRomInterp(const TArray<FVector> &P,
                                               float T) {
  {
    if (P.Num() < 3)
      return (P.Num() == 2) ? FMath::Lerp(P[0], P[1], T)
                            : (P.Num() ? P[0] : FVector::ZeroVector);
    const FVector &P0 = P[0], &P1 = P[1], &P2 = P[2];
    float t2 = T * T;
    float t3 = t2 * T;
    return 0.5f *
           ((2 * P1) + (-P0 + P2) * T + (2 * P0 - 5 * P1 + 4 * P2 - P0) * t2 +
            (-P0 + 3 * P1 - 3 * P2 + P0) * t3);
  }
}

float UOHAnimInstance_Base::FindHighestNearbyFloorZ(const FVector &Center,
                                                    UWorld *World, float Radius,
                                                    float ProbeDepth) {
  float MaxZ = Center.Z - ProbeDepth;
  FHitResult Hit;
  for (int32 i = 0; i < 8; ++i) {
    FVector Offset = FVector(FMath::Cos(i * PI / 4) * Radius,
                             FMath::Sin(i * PI / 4) * Radius, 0.f);
...
EFootPhase
UOHAnimInstance_Base::DetectFootPhase(const FFootTraceBuffer &Buffer) const {
  ...
  if (HorizontalSpeed > PhaseSwingHorizontalSpeed &&
      VerticalSpeed > PhaseSwingVerticalSpeed)
    return EFootPhase::Swing;
  if (FMath::Abs(VerticalSpeed) < PhasePlantVerticalSpeed &&
      HorizontalSpeed < PhasePlantHorizontalSpeed)
...
float UOHAnimInstance_Base::GetHeelToeBlendAlpha(
    EFootPhase Phase, const FFootTraceBuffer &Buffer) {
  switch (Phase) {
  case EFootPhase::Swing:
    return 1.f;
  case EFootPhase::Lift:
    return 0.7f;
  case EFootPhase::Plant:
    return 0.f;
  default:
    return 0.3f;
  }
...
FVector UOHAnimInstance_Base::CriticallyDampedSmoothing(const FVector &Current,
...
void UOHAnimInstance_Base::NativeUpdateAnimation(float DeltaSeconds) {
  ...
  if (!bMeasuredFootLength) {
    ...
    if (IKFootSphereRadius <= 0.f)
      IKFootSphereRadius = FMath::Clamp(AutoFootLength * 0.28f, 4.5f, 10.f);
    if (IKFootCapsuleHalfHeight <= 0.f)
      IKFootCapsuleHalfHeight =
          FMath::Clamp(AutoFootLength * 0.48f, 7.0f, 16.f);
  ...
  FVector MoveDir = CharVel.IsNearlyZero() ? OwnerPawn->GetActorForwardVector()
                                           : CharVel.GetSafeNormal();
  ...
  // --- 6. Update phase buffers with time/position/speed for stride-aware
  // blending ---
  ...
  float BiasLeft = (LeftPhase == EFootPhase::Plant) ? IKFootForwardBias
                                                    : IKFootForwardBias * 2.f;
  float BiasRight = (RightPhase == EFootPhase::Plant) ? IKFootForwardBias
                                                      : IKFootForwardBias * 2.f;
  if (Speed < IdleFreezeSpeed)
    BiasLeft = BiasRight = FMath::Min(IKFootForwardBias, 12.f);
  ...
  bool bLAnkleHit = SafeDoubleTrace(Mesh, FName("foot_l"), IKFootProbeDepth,
                                    BiasLeft, IKFootSphereRadius, CharVel,
                                    LAnkleHit, LAnkleBackup, FloorProbeDepth);
  bool bLToeHit = SafeDoubleTrace(Mesh, FName("ball_l"), IKFootProbeDepth,
                                  BiasLeft, IKFootSphereRadius, CharVel,
                                  LToeHit, LToeBackup, FloorProbeDepth);
  bool bRAnkleHit = SafeDoubleTrace(Mesh, FName("foot_r"), IKFootProbeDepth,
                                    BiasRight, IKFootSphereRadius, CharVel,
                                    RAnkleHit, RAnkleBackup, FloorProbeDepth);
  bool bRToeHit = SafeDoubleTrace(Mesh, FName("ball_r"), IKFootProbeDepth,
                                  BiasRight, IKFootSphereRadius, CharVel,
                                  RToeHit, RToeBackup, FloorProbeDepth);
  ...
  if (LFootZBufValidSamples < FootZBufferSize)
    LFootZBufValidSamples++;
  ...
  if (RFootZBufValidSamples < FootZBufferSize)
    RFootZBufValidSamples++;
  RFootZBufIndex = (RFootZBufIndex + 1) % FootZBufferSize;
  float MedianRFootZ = Median(RFootZBuffer, RFootZBufValidSamples);

  if (bLFootFirstUpdate || LFootZBufValidSamples < 2) {
    SmoothedLFootGroundZ = MedianLFootZ;
    bLFootFirstUpdate = false;
  } else {
    SmoothedLFootGroundZ = FMath::FInterpTo(SmoothedLFootGroundZ, MedianLFootZ,
                                            DeltaSeconds, OffsetInterpSpeed);
  }
  if (bRFootFirstUpdate || RFootZBufValidSamples < 2) {
    SmoothedRFootGroundZ = MedianRFootZ;
    bRFootFirstUpdate = false;
  } else {
    SmoothedRFootGroundZ = FMath::FInterpTo(SmoothedRFootGroundZ, MedianRFootZ,
                                            DeltaSeconds, OffsetInterpSpeed);
  }
  ...
  if (LeftEffectorSpline.Num() < SplineMax) {
    for (int i = LeftEffectorSpline.Num(); i < SplineMax; ++i)
      LeftEffectorSpline.Add(LTargetWS);
  }
  if (RightEffectorSpline.Num() < SplineMax) {
    for (int i = RightEffectorSpline.Num(); i < SplineMax; ++i)
      RightEffectorSpline.Add(RTargetWS);
  }
  LeftEffectorSpline.RemoveAt(0);
  LeftEffectorSpline.Add(LTargetWS);
  RightEffectorSpline.RemoveAt(0);
  RightEffectorSpline.Add(RTargetWS);
  ...
  float AdaptiveSmoothL = FMath::Lerp(
      AdaptiveSmoothingMin, AdaptiveSmoothingMax, 1.f - PhaseConfidenceL);
  float AdaptiveSmoothR = FMath::Lerp(
      AdaptiveSmoothingMin, AdaptiveSmoothingMax, 1.f - PhaseConfidenceR);
  float AdaptiveInterpSpeedL = FMath::Lerp(
      AdaptiveInterpSpeedMin, AdaptiveInterpSpeedMax, 1.f - PhaseConfidenceL);
  float AdaptiveInterpSpeedR = FMath::Lerp(
      AdaptiveInterpSpeedMin, AdaptiveInterpSpeedMax, 1.f - PhaseConfidenceR);

  // --- 17. Sticky plant blend/fade (supports plant fade timing) ---
  if (PredictedLeftPhase == EFootPhase::Plant) {
    LeftPlantAlpha = 1.f;
    LastLeftPlantTime = TimeNow;
  } else
    LeftPlantAlpha =
        FMath::FInterpTo(LeftPlantAlpha, 0.f, DeltaSeconds, PlantFadeSpeed);
  if (PredictedRightPhase == EFootPhase::Plant) {
    RightPlantAlpha = 1.f;
    LastRightPlantTime = TimeNow;
  } else
    RightPlantAlpha =
        FMath::FInterpTo(RightPlantAlpha, 0.f, DeltaSeconds, PlantFadeSpeed);

  // --- 18. Plant lock/hysteresis ---
  if (LeftPhase == EFootPhase::Plant && bLAnkleHit) {
    LastValidLeftImpact = LAnkleHit.ImpactPoint;
    LastValidLeftNormal =
        bLToeHit ? ((LAnkleHit.ImpactNormal + LToeHit.ImpactNormal) * 0.5f)
                       .GetSafeNormal()
                 : LAnkleHit.ImpactNormal;
    LeftFootLockFrames = LockHysteresisFrames;
  } else if (LeftFootLockFrames > 0) {
    LeftFootLockFrames--;
  } else {
  ...
  if (RightPhase == EFootPhase::Plant && bRAnkleHit) {
    ...
    LastValidRightNormal =
        bRToeHit ? ((RAnkleHit.ImpactNormal + RToeHit.ImpactNormal) * 0.5f)
                       .GetSafeNormal()
                 : RAnkleHit.ImpactNormal;
    RightFootLockFrames = LockHysteresisFrames;
  } else if (RightFootLockFrames > 0) {
    RightFootLockFrames--;
  } else {
  ...
  // --- 19. Ghost step/stride anticipation (lookahead for both feet, using
  // respective PredictPhaseTime) ---
  NewLockedLEffector = Mesh->GetComponentTransform().InverseTransformPosition(
      LastValidLeftImpact);
  NewLockedREffector = Mesh->GetComponentTransform().InverseTransformPosition(
      LastValidRightImpact);
  FRotator NewLockedLRot = LastValidLeftNormal.Rotation();
  FRotator NewLockedRRot = LastValidRightNormal.Rotation();

  float GhostStepLookaheadTimeL =
      FMath::Clamp(PredictPhaseTimeL * 0.4f, 0.06f, 0.14f);
  float GhostStepLookaheadTimeR =
      FMath::Clamp(PredictPhaseTimeR * 0.4f, 0.06f, 0.14f);
  float GhostStepBlend = 0.28f;
  FVector PredictedStep_L =
      LeftAnkleWS + AvgStrideVelL * GhostStepLookaheadTimeL;
  FVector PredictedStep_R =
      RightAnkleWS + AvgStrideVelR * GhostStepLookaheadTimeR;
  if (LeftPhase == EFootPhase::Swing || LeftPhase == EFootPhase::Lift)
    NewLockedLEffector = FMath::Lerp(
        NewLockedLEffector,
        Mesh->GetComponentTransform().InverseTransformPosition(PredictedStep_L),
        GhostStepBlend);
  if (RightPhase == EFootPhase::Swing || RightPhase == EFootPhase::Lift)
    NewLockedREffector = FMath::Lerp(
        NewLockedREffector,
        Mesh->GetComponentTransform().InverseTransformPosition(PredictedStep_R),
        GhostStepBlend);

  // --- 20. Biomechanical pivot/side-step logic (context from PelvisWorld and
  // MoveDir) ---
  ...
  float PelvisSpeed = (PelvisWorld - LastPelvisWorld).Size() /
                      FMath::Max(DeltaSeconds, 0.0001f);
  if (PelvisSpeed > 1.f) {
    float SideAngle = FMath::RadiansToDegrees(
        acosf(FVector::DotProduct(MoveDir, PelvisMoveDir)));
    if (SideAngle > SideStepAngleThreshold) {
      float SideStepBlend = 0.34f;
      FVector OutDir = FVector::CrossProduct(PelvisMoveDir, FVector::UpVector)
                           .GetSafeNormal();
      NewLockedLEffector +=
          OutDir * FMath::Sign(FVector::DotProduct(AvgStrideVelL, OutDir)) *
          SideStepBlend * AutoFootLength;
      NewLockedREffector +=
          OutDir * FMath::Sign(FVector::DotProduct(AvgStrideVelR, OutDir)) *
          SideStepBlend * AutoFootLength;
    }
  }
  ...
  // --- 21. CLAMP EFFECTOR RANGE TO PREVENT ROOT/PELVIS TILT (DROP-IN SAFE) ---
  {
    ...
    FVector PelvisCS =
        Mesh->GetComponentTransform().InverseTransformPosition(PelvisWorld);
  ...
  float LHFloorZ = FindHighestNearbyFloorZ(LSplineEffector, World,
                                           FloorPlaneRadius, FloorProbeDepth);
  float RHFloorZ = FindHighestNearbyFloorZ(RSplineEffector, World,
                                           FloorPlaneRadius, FloorProbeDepth);
  if (FMath::Abs(LSplineEffector.Z - LHFloorZ) > MaxStairHeight)
    LSplineEffector.Z =
        FMath::FInterpTo(LSplineEffector.Z, LHFloorZ, DeltaSeconds, 6.f);
  if (FMath::Abs(RSplineEffector.Z - RHFloorZ) > MaxStairHeight)
    RSplineEffector.Z =
        FMath::FInterpTo(RSplineEffector.Z, RHFloorZ, DeltaSeconds, 6.f);

  // --- 23. Physically-inspired critically-damped smoothing ---
  DampedLeftEffector =
      CriticallyDampedSmoothing(DampedLeftEffector, NewLockedLEffector,
                                DeltaSeconds, 80.f, AdaptiveInterpSpeedL);
  DampedRightEffector =
      CriticallyDampedSmoothing(DampedRightEffector, NewLockedREffector,
                                DeltaSeconds, 80.f, AdaptiveInterpSpeedR);
  ...
  NewLockedLRot.Pitch = FMath::Clamp(
      NewLockedLRot.Pitch, -AnklePitchLimits.Pitch, AnklePitchLimits.Pitch);
  NewLockedLRot.Yaw =
      FMath::Clamp(NewLockedLRot.Yaw, -AnkleYawLimits.Yaw, AnkleYawLimits.Yaw);
  NewLockedLRot.Roll = FMath::Clamp(NewLockedLRot.Roll, -AnkleRollLimits.Roll,
                                    AnkleRollLimits.Roll);

  NewLockedRRot.Pitch = FMath::Clamp(
      NewLockedRRot.Pitch, -AnklePitchLimits.Pitch, AnklePitchLimits.Pitch);
  NewLockedRRot.Yaw =
      FMath::Clamp(NewLockedRRot.Yaw, -AnkleYawLimits.Yaw, AnkleYawLimits.Yaw);
  NewLockedRRot.Roll = FMath::Clamp(NewLockedRRot.Roll, -AnkleRollLimits.Roll,
                                    AnkleRollLimits.Roll);

  LeftFootIKRotation =
      FMath::RInterpTo(LeftFootIKRotation, NewLockedLRot, DeltaSeconds,
                       AdaptiveInterpSpeedL * 1.12f);
  RightFootIKRotation =
      FMath::RInterpTo(RightFootIKRotation, NewLockedRRot, DeltaSeconds,
                       AdaptiveInterpSpeedR * 1.12f);
  ...
  PelvisIKOffset = FMath::FInterpTo(PelvisIKOffset, TargetPelvis, DeltaSeconds,
                                    PelvisInterpSpeed);
  ...
  if (GEngine && Mesh) {
    ...
    FVector LeftEffectorWS =
        MeshCompXform.TransformPosition(DampedLeftEffector);
    FVector RightEffectorWS =
        MeshCompXform.TransformPosition(DampedRightEffector);

    DrawDebugSphere(Mesh->GetWorld(), LeftEffectorWS, 3.f, 12, FColor::Green,
                    false, 0.12f, 0, 1.5f);
    DrawDebugSphere(Mesh->GetWorld(), RightEffectorWS, 3.f, 12, FColor::Blue,
                    false, 0.12f, 0, 1.5f);

    // --- Draw component origin/root for reference ---
    DrawDebugSphere(Mesh->GetWorld(), MeshCompXform.GetLocation(), 4.f, 12,
                    FColor::White, false, 0.12f, 0, 2.f);

    // --- Draw ground contact points for each foot (traces) ---
    DrawDebugSphere(Mesh->GetWorld(), LAnkleHit.ImpactPoint, 2.f, 8,
                    FColor::Cyan, false, 0.12f, 0, 0.8f);
    DrawDebugSphere(Mesh->GetWorld(), LToeHit.ImpactPoint, 2.f, 8,
                    FColor::Magenta, false, 0.12f, 0, 0.8f);
    DrawDebugSphere(Mesh->GetWorld(), RAnkleHit.ImpactPoint, 2.f, 8,
                    FColor::Cyan, false, 0.12f, 0, 0.8f);
    DrawDebugSphere(Mesh->GetWorld(), RToeHit.ImpactPoint, 2.f, 8,
                    FColor::Magenta, false, 0.12f, 0, 0.8f);

    // --- Draw ghost (predicted) step location ---
    DrawDebugSphere(Mesh->GetWorld(), PredictedStep_L, 3.f, 8, FColor::Yellow,
                    false, 0.12f, 0, 1.5f);
    DrawDebugSphere(Mesh->GetWorld(), PredictedStep_R, 3.f, 8, FColor::Orange,
                    false, 0.12f, 0, 1.5f);

    // --- Draw Spline Effector Positions ---
    DrawDebugSphere(Mesh->GetWorld(),
                    MeshCompXform.TransformPosition(LSplineEffector), 2.f, 8,
                    FColor::Emerald, false, 0.12f, 0, 1.0f);
    DrawDebugSphere(Mesh->GetWorld(),
                    MeshCompXform.TransformPosition(RSplineEffector), 2.f, 8,
                    FColor::Turquoise, false, 0.12f, 0, 1.0f);

    // --- Draw Locked Effector Positions ---
    DrawDebugSphere(Mesh->GetWorld(),
                    MeshCompXform.TransformPosition(NewLockedLEffector), 2.2f,
                    10, FColor::Purple, false, 0.12f, 0, 1.2f);
    DrawDebugSphere(Mesh->GetWorld(),
                    MeshCompXform.TransformPosition(NewLockedREffector), 2.2f,
                    10, FColor::Silver, false, 0.12f, 0, 1.2f);

    // --- Draw Highest Floor Probes ---
    FVector LHProbe = LSplineEffector;
    LHProbe.Z = LHFloorZ;
    FVector RHProbe = RSplineEffector;
    RHProbe.Z = RHFloorZ;
    DrawDebugBox(Mesh->GetWorld(), MeshCompXform.TransformPosition(LHProbe),
                 FVector(2, 2, 2), FColor::Cyan, false, 0.12f, 0, 0.8f);
    DrawDebugBox(Mesh->GetWorld(), MeshCompXform.TransformPosition(RHProbe),
                 FVector(2, 2, 2), FColor::Cyan, false, 0.12f, 0, 0.8f);

    // --- Draw Pelvis world ---
    DrawDebugSphere(Mesh->GetWorld(), PelvisWorld, 2.f, 12, FColor::Red, false,
                    0.12f, 0, 0.9f);

    // --- Annotate all spheres for quick ID ---
    DrawDebugString(Mesh->GetWorld(), LeftEffectorWS + FVector(0, 0, 8),
                    TEXT("LeftEffector"), nullptr, FColor::Green, 0.12f, false);
    DrawDebugString(Mesh->GetWorld(), RightEffectorWS + FVector(0, 0, 8),
                    TEXT("RightEffector"), nullptr, FColor::Blue, 0.12f, false);
    DrawDebugString(
        Mesh->GetWorld(), MeshCompXform.GetLocation() + FVector(0, 0, 10),
        TEXT("Root/Component"), nullptr, FColor::White, 0.12f, false);
    DrawDebugString(Mesh->GetWorld(), PredictedStep_L + FVector(0, 0, 10),
                    TEXT("Ghost L"), nullptr, FColor::Yellow, 0.12f, false);
    DrawDebugString(Mesh->GetWorld(), PredictedStep_R + FVector(0, 0, 10),
                    TEXT("Ghost R"), nullptr, FColor::Orange, 0.12f, false);

    // --- Display stride debug info as text in the world ---
    FString DebugInfoL = FString::Printf(
        TEXT("StridePhase:%d\nConf:%.2f\nGhost:%.2f"), LeftStridePhase,
        LeftFootStrideConfidence, LeftGhostLookaheadTime);
    FString DebugInfoR = FString::Printf(
        TEXT("StridePhase:%d\nConf:%.2f\nGhost:%.2f"), RightStridePhase,
        RightFootStrideConfidence, RightGhostLookaheadTime);

    DrawDebugString(Mesh->GetWorld(), LeftEffectorWS + FVector(0, 10, 20),
                    DebugInfoL, nullptr, FColor::Green, 0.12f, false);
    DrawDebugString(Mesh->GetWorld(), RightEffectorWS + FVector(0, -10, 20),
                    DebugInfoR, nullptr, FColor::Blue, 0.12f, false);

    // --- Log values for additional tuning (optional) ---
    UE_LOG(
        LogTemp, Log,
        TEXT("[StrideIK] LEffComp:%s  REffComp:%s  LEffWorld:%s  REffWorld:%s"),
        *DampedLeftEffector.ToString(), *DampedRightEffector.ToString(),
        *LeftEffectorWS.ToString(), *RightEffectorWS.ToString());
    UE_LOG(LogTemp, Log,
           TEXT("[StrideIK] LStridePhase:%d  RStridePhase:%d  LConf:%.2f  "
                "RConf:%.2f  LGhost:%.2f  RGhost:%.2f"),
           LeftStridePhase, RightStridePhase, LeftFootStrideConfidence,
           RightFootStrideConfidence, LeftGhostLookaheadTime,
           RightGhostLookaheadTime);
  }
#endif
}
#if 0
