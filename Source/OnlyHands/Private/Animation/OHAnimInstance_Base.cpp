...
static FVector CriticallyDampedSmoothing(const FVector& Current, const FVector& Target, float DeltaTime,
                                         float Spring = 80.f, float Damping = 1.2f) {
...
bool UOHAnimInstance_Base::CapsuleGroundTrace(const USkeletalMeshComponent* Mesh, const FName& BoneName,
...
bool UOHAnimInstance_Base::SphereGroundTrace(const USkeletalMeshComponent* Mesh, const FName& BoneName,
                                             float MaxDistance, float ForwardBias, float SphereRadius,
                                             const FVector& Velocity, FHitResult& OutHit, bool bDrawDebug) {
    if (!Mesh)
        return false;
    FVector Start = Mesh->GetBoneLocation(BoneName) +
                    (Velocity.IsNearlyZero() ? FVector::ZeroVector : Velocity.GetSafeNormal()) * ForwardBias;
    FVector End = Start - FVector(0, 0, MaxDistance);
    FCollisionQueryParams Params;
    Params.AddIgnoredActor(Mesh->GetOwner());

    bool bHit = Mesh->GetWorld()->SweepSingleByChannel(OutHit, Start, End, FQuat::Identity, ECC_Visibility,
...
FVector UOHAnimInstance_Base::CatmullRomInterp(const TArray<FVector>& P, float T) {
    {
        if (P.Num() < 3)
            return (P.Num() == 2) ? FMath::Lerp(P[0], P[1], T) : (P.Num() ? P[0] : FVector::ZeroVector);
        const FVector &P0 = P[0], &P1 = P[1], &P2 = P[2];
        float t2 = T * T;
        float t3 = t2 * T;
        return 0.5f *
               ((2 * P1) + (-P0 + P2) * T + (2 * P0 - 5 * P1 + 4 * P2 - P0) * t2 + (-P0 + 3 * P1 - 3 * P2 + P0) * t3);
    }
}
...
FVector UOHAnimInstance_Base::CriticallyDampedSmoothing(const FVector& Current, const FVector& Target, float DeltaTime,
...
void UOHAnimInstance_Base::NativeUpdateAnimation(float DeltaSeconds) {
    ...
    if (!bMeasuredFootLength) {
        ...
        if (IKFootCapsuleHalfHeight <= 0.f)
            IKFootCapsuleHalfHeight = FMath::Clamp(AutoFootLength * 0.48f, 7.0f, 16.f);
    ...
    bool bLAnkleHit = SafeDoubleTrace(Mesh, FName("foot_l"), IKFootProbeDepth, BiasLeft, IKFootSphereRadius, CharVel,
                                      LAnkleHit, LAnkleBackup, FloorProbeDepth);
    bool bLToeHit = SafeDoubleTrace(Mesh, FName("ball_l"), IKFootProbeDepth, BiasLeft, IKFootSphereRadius, CharVel,
                                    LToeHit, LToeBackup, FloorProbeDepth);
    bool bRAnkleHit = SafeDoubleTrace(Mesh, FName("foot_r"), IKFootProbeDepth, BiasRight, IKFootSphereRadius, CharVel,
                                      RAnkleHit, RAnkleBackup, FloorProbeDepth);
    bool bRToeHit = SafeDoubleTrace(Mesh, FName("ball_r"), IKFootProbeDepth, BiasRight, IKFootSphereRadius, CharVel,
    ...
    float LFootGroundZ =
        (FMath::Abs(LAnkleZ - LToeZ) > MaxStairHeight) ? FMath::Min(LAnkleZ, LToeZ) : FMath::Lerp(BlendL, MaxL, 0.6f);

    float BlendR = FMath::Lerp(RAnkleZ, RToeZ, HeelToeAlphaRight);
    float MaxR = FMath::Max(RAnkleZ, RToeZ);
    float RFootGroundZ =
        (FMath::Abs(RAnkleZ - RToeZ) > MaxStairHeight) ? FMath::Min(RAnkleZ, RToeZ) : FMath::Lerp(BlendR, MaxR, 0.6f);
    ...
    if (bLFootFirstUpdate || LFootZBufValidSamples < 2) {
        ...
        SmoothedLFootGroundZ = FMath::FInterpTo(SmoothedLFootGroundZ, MedianLFootZ, DeltaSeconds, OffsetInterpSpeed);
    }
    if (bRFootFirstUpdate || RFootZBufValidSamples < 2) {
        SmoothedRFootGroundZ = MedianRFootZ;
        bRFootFirstUpdate = false;
    } else {
        SmoothedRFootGroundZ = FMath::FInterpTo(SmoothedRFootGroundZ, MedianRFootZ, DeltaSeconds, OffsetInterpSpeed);
    }
    ...
    if (PredictedLeftPhase == EFootPhase::Plant) {
        ...
        LeftPlantAlpha = FMath::FInterpTo(LeftPlantAlpha, 0.f, DeltaSeconds, PlantFadeSpeed);
    if (PredictedRightPhase == EFootPhase::Plant) {
        RightPlantAlpha = 1.f;
        LastRightPlantTime = TimeNow;
    } else
        RightPlantAlpha = FMath::FInterpTo(RightPlantAlpha, 0.f, DeltaSeconds, PlantFadeSpeed);

    // --- 18. Plant lock/hysteresis ---
    if (LeftPhase == EFootPhase::Plant && bLAnkleHit) {
        LastValidLeftImpact = LAnkleHit.ImpactPoint;
        LastValidLeftNormal = bLToeHit ? ((LAnkleHit.ImpactNormal + LToeHit.ImpactNormal) * 0.5f).GetSafeNormal()
    ...
    if (RightPhase == EFootPhase::Plant && bRAnkleHit) {
        ...
        LastValidRightNormal = bRToeHit ? ((RAnkleHit.ImpactNormal + RToeHit.ImpactNormal) * 0.5f).GetSafeNormal()
    ...
    if (LeftPhase == EFootPhase::Swing || LeftPhase == EFootPhase::Lift)
        NewLockedLEffector =
            FMath::Lerp(NewLockedLEffector, Mesh->GetComponentTransform().InverseTransformPosition(PredictedStep_L),
                        GhostStepBlend);
    if (RightPhase == EFootPhase::Swing || RightPhase == EFootPhase::Lift)
        NewLockedREffector =
            FMath::Lerp(NewLockedREffector, Mesh->GetComponentTransform().InverseTransformPosition(PredictedStep_R),
                        GhostStepBlend);
    ...
    if (PelvisSpeed > 1.f) {
        ...
        if (SideAngle > SideStepAngleThreshold) {
            ...
            NewLockedLEffector +=
                OutDir * FMath::Sign(FVector::DotProduct(AvgStrideVelL, OutDir)) * SideStepBlend * AutoFootLength;
            NewLockedREffector +=
                OutDir * FMath::Sign(FVector::DotProduct(AvgStrideVelR, OutDir)) * SideStepBlend * AutoFootLength;
        }
    }
    ...
    DampedLeftEffector =
        CriticallyDampedSmoothing(DampedLeftEffector, NewLockedLEffector, DeltaSeconds, 80.f, AdaptiveInterpSpeedL);
    DampedRightEffector =
        CriticallyDampedSmoothing(DampedRightEffector, NewLockedREffector, DeltaSeconds, 80.f, AdaptiveInterpSpeedR);
    ...
    LeftFootIKRotation =
        FMath::RInterpTo(LeftFootIKRotation, NewLockedLRot, DeltaSeconds, AdaptiveInterpSpeedL * 1.12f);
    RightFootIKRotation =
        FMath::RInterpTo(RightFootIKRotation, NewLockedRRot, DeltaSeconds, AdaptiveInterpSpeedR * 1.12f);
    ...
    if (GEngine && Mesh && bDrawDebug) {
        ...
        DrawDebugSphere(Mesh->GetWorld(), MeshCompXform.TransformPosition(LSplineEffector), 2.f, 8, FColor::Emerald,
                        false, 0.12f, 0, 1.0f);
        DrawDebugSphere(Mesh->GetWorld(), MeshCompXform.TransformPosition(RSplineEffector), 2.f, 8, FColor::Turquoise,
                        false, 0.12f, 0, 1.0f);

        // --- Draw Locked Effector Positions ---
        DrawDebugSphere(Mesh->GetWorld(), MeshCompXform.TransformPosition(NewLockedLEffector), 2.2f, 10, FColor::Purple,
                        false, 0.12f, 0, 1.2f);
        DrawDebugSphere(Mesh->GetWorld(), MeshCompXform.TransformPosition(NewLockedREffector), 2.2f, 10, FColor::Silver,
                        false, 0.12f, 0, 1.2f);
        ...
        RHProbe.Z = RHFloorZ;
        DrawDebugBox(Mesh->GetWorld(), MeshCompXform.TransformPosition(LHProbe), FVector(2, 2, 2), FColor::Cyan, false,
                     0.12f, 0, 0.8f);
        DrawDebugBox(Mesh->GetWorld(), MeshCompXform.TransformPosition(RHProbe), FVector(2, 2, 2), FColor::Cyan, false,
                     0.12f, 0, 0.8f);
        ...
        DrawDebugString(Mesh->GetWorld(), LeftEffectorWS + FVector(0, 0, 8), TEXT("LeftEffector"), nullptr,
                        FColor::Green, 0.12f, false);
        DrawDebugString(Mesh->GetWorld(), RightEffectorWS + FVector(0, 0, 8), TEXT("RightEffector"), nullptr,
                        FColor::Blue, 0.12f, false);
        DrawDebugString(Mesh->GetWorld(), MeshCompXform.GetLocation() + FVector(0, 0, 10), TEXT("Root/Component"),
                        nullptr, FColor::White, 0.12f, false);
        DrawDebugString(Mesh->GetWorld(), PredictedStep_L + FVector(0, 0, 10), TEXT("Ghost L"), nullptr, FColor::Yellow,
                        0.12f, false);
        DrawDebugString(Mesh->GetWorld(), PredictedStep_R + FVector(0, 0, 10), TEXT("Ghost R"), nullptr, FColor::Orange,
                        0.12f, false);
        ...
        DrawDebugString(Mesh->GetWorld(), LeftEffectorWS + FVector(0, 10, 20), DebugInfoL, nullptr, FColor::Green,
                        0.12f, false);
        DrawDebugString(Mesh->GetWorld(), RightEffectorWS + FVector(0, -10, 20), DebugInfoR, nullptr, FColor::Blue,
                        0.12f, false);

        // --- Log values for additional tuning (optional) ---
        UE_LOG(LogTemp, Log, TEXT("[StrideIK] LEffComp:%s  REffComp:%s  LEffWorld:%s  REffWorld:%s"),
               *DampedLeftEffector.ToString(), *DampedRightEffector.ToString(), *LeftEffectorWS.ToString(),
               *RightEffectorWS.ToString());
        UE_LOG(LogTemp, Log,
               TEXT("[StrideIK] LStridePhase:%d  RStridePhase:%d  LConf:%.2f  RConf:%.2f  LGhost:%.2f  RGhost:%.2f"),
               LeftStridePhase, RightStridePhase, LeftFootStrideConfidence, RightFootStrideConfidence,
               LeftGhostLookaheadTime, RightGhostLookaheadTime);
    }
#endif
}
#if 0
