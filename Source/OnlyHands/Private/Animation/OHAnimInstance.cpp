...
UOHAnimInstance::UOHAnimInstance()
	...
	, bHasDynamicPhysics(false)
	, OverrideResetDelay(0.3f)
{}
...
void UOHAnimInstance::DriveCompressionFromHit(FVector HitLocation, float Impulse)
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		World->GetTimerManager().SetTimer(
			CompressionResetTimer,
			this,
			&UOHAnimInstance::ClearCompressionEffector,
			OverrideResetDelay,
			false
		);
	}
...
void UOHAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (bSimulatePreview || PhysicsManager == nullptr)
	{
		...
		ExternalLinearVelocity = PreviewSimulatedVelocity.IsZero() ? 
			FVector(100.f, 0.f, 0.f) : PreviewSimulatedVelocity;
            
		// Set a sensible predicted target that can't be null
		const FVector SafeTarget = SkelMesh ? 
			SkelMesh->GetComponentLocation() + PreviewOffsetDirection.GetSafeNormal() * 50.f :
