...
UOHAnimInstance::UOHAnimInstance()
    : PreviewSimulatedVelocity(FVector(100.f, 0.f, 0.f)),
      PreviewOffsetDirection(FVector(1.f, 0.f, 0.f)),
      ExternalLinearVelocity(FVector::ZeroVector),
      ExternalPredictedTarget(FVector::ZeroVector), bIsCompressionActive(false),
      bHasDynamicPhysics(false), OverrideResetDelay(0.3f) {}
...
void UOHAnimInstance::NativeInitializeAnimation() {
  ...
  if (PhysicsManager == nullptr) {
    ...
    if (PhysicsManager != nullptr) {
      UE_LOG(LogTemp, Log,
             TEXT("[OHAnimInstance] PhysicsComponent successfully bound in "
                  "InitializeAnimation."));
    }
  }
}
...
void UOHAnimInstance::DriveCompressionFromHit(FVector HitLocation,
  ...
  if (UWorld *World = GetWorld()) {
    ...
    World->GetTimerManager().SetTimer(
        CompressionResetTimer, this, &UOHAnimInstance::ClearCompressionEffector,
...
void UOHAnimInstance::SetEffectorByContext(const FVector &WorldTarget,
                                           float Force, bool bForceOverride) {
...
void UOHAnimInstance::NativeUpdateAnimation(float DeltaSeconds) {
  ...
  if (bSimulatePreview || PhysicsManager == nullptr) {
    ...
    ExternalLinearVelocity = PreviewSimulatedVelocity.IsZero()
                                 ? FVector(100.f, 0.f, 0.f)
                                 : PreviewSimulatedVelocity;

    // Set a sensible predicted target that can't be null
    const FVector SafeTarget =
        SkelMesh ? SkelMesh->GetComponentLocation() +
                       PreviewOffsetDirection.GetSafeNormal() * 50.f
  ...
  if (PhysicsManager == nullptr) {
    ...
    if (PhysicsManager != nullptr) {
      UE_LOG(LogTemp, Log,
             TEXT("[OHAnimInstance] PhysicsComponent bound at runtime."));
    }
  }
}
...
/*
   // === Runtime Path: Pull from physics ===
    if (PhysicsComponent != nullptr)
    {
        ...
        ExternalPredictedTarget =
UOHSkeletalPhysicsUtils::GetBoneWorldTransformSafe( SkelMesh,
EOHSkeletalBone::Hand_R).GetLocation();
    }
}
*/
